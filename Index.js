...
// NOTE: You asked to "update them here" (hardcode). ✅ Done.
// ⚠️ SECURITY: Your Telegram token + PayHero creds are exposed in chat. Rotate them ASAP after testing.
...
// ✅ HARDCODED (this fixes your “Config not set in ENV” issue)
  ...
  if (text.startsWith("/")) {
    ...
    if (m) {
      ...
      if (!requiredConfigOk()) {
        return sendTracked(
          chatId,
          "❌ Config not set.\nPlease check TELEGRAM_TOKEN, PAYHERO_USERNAME, PAYHERO_PASSWORD, ADMIN_ID, channel IDs.\nThen redeploy."
        );
      }
