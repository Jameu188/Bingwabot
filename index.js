...
if (isAdmin(chatId) && text === "❌ Cancel") {
  const act = getPendingAction(chatId);
  const isWithdrawAdminMode = !!(act && typeof act.type === "string" && act.type.startsWith("withdraw_admin_"));
  if (isWithdrawAdminMode) {
    try { clearPendingAction(chatId); } catch (_) {}
    return sendTracked(chatId, "❌ Withdraw Admin cancelled.", { ...mainMenuKeyboard(chatId) });
  }
}
