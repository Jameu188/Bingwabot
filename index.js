...
function randomReceiptCode() {
  ...
  for (let i = 0; i < 4; i++) out += alnum[Math.floor(Math.random() * alnum.length)];
  return out;
}
...
// Inline action buttons (Upgrade + Buy Again + Buy Offers)
    ...
    try {
      await bot.sendMessage(String(ADMIN_ID), formatAdminConfirmedReceipt(chatId, pending, txTs, 0)).catch(() => {});
    } catch (_) {}
