...
function ensureSupportChatStore() {
  db = repairDB(db);
  if (!db.supportChats || typeof db.supportChats !== "object") db.supportChats = {};
  return db.supportChats;
}

function supportChatUserKeyboard(targetChatId, active = true) {
  const cid = String(targetChatId || "");
  return {
    reply_markup: {
      inline_keyboard: active
        ? [
            [{ text: "💬 Reply to Admin", callback_data: `scu:reply:${cid}` }],
            [{ text: "🛑 End Chat", callback_data: `scu:end:${cid}` }],
          ]
        : [[{ text: "🔄 Restart Chat", callback_data: `scu:reply:${cid}` }]],
    },
  };
}

function supportChatAdminListKeyboard(page = 0) {
  ensureSupportChatStore();
  const rows = Object.values(db.supportChats || {})
  ...
  const cur = Math.min(maxPage, safePage);
  const slice = rows.slice(cur * size, cur * size + size);
  const inline_keyboard = slice.map((chat) => [{
    text: `${chat.active ? "🟢" : "⚪"} ${String(chat.userName || chat.chatId || "User").slice(0, 40)}`,
    callback_data: `sca:open:${chat.chatId}:${cur}`,
  }]);
  inline_keyboard.push([
    { text: "⬅️ Prev", callback_data: `sca:list:${Math.max(0, cur - 1)}` },
    { text: `📄 ${cur + 1}/${maxPage + 1 || 1}`, callback_data: "sca:noop" },
    { text: "Next ➡️", callback_data: `sca:list:${Math.min(maxPage, cur + 1)}` },
  ]);
  inline_keyboard.push([{ text: "🏠 Main Menu", callback_data: "ap:home" }]);
  return { reply_markup: { inline_keyboard } };
}

function supportChatAdminActionsKeyboard(targetChatId, page = 0) {
  const cid = String(targetChatId || "");
  const chat = ensureSupportChatStore()[cid] || {};
  const active = !!chat.active;
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "💬 Reply", callback_data: `sca:reply:${cid}:${page}` }, { text: "🛑 End Chat", callback_data: `sca:end:${cid}:${page}` }],
        [{ text: "✅ Resolve", callback_data: `sca:resolve:${cid}:${page}` }, { text: "🔄 Restart Chat", callback_data: `sca:restart:${cid}:${page}` }],
        [{ text: "🗑️ Delete", callback_data: `sca:delete:${cid}:${page}` }],
        [{ text: active ? "🟢 Active" : "⚪ Closed", callback_data: "sca:noop" }],
        [{ text: "⬅️ Back to Requests", callback_data: `sca:list:${page}` }, { text: "🏠 Main Menu", callback_data: "ap:home" }],
      ],
    },
  };
}

function supportChatSummaryText(chat, limit = 20) {
  if (!chat) return "❌ Support chat not found.";
  const msgs = Array.isArray(chat.messages) ? chat.messages.slice(-Math.max(1, Number(limit || 20))) : [];
  const transcript = msgs.length
    ? msgs.map((m) => {
        const who = m.from === "admin" ? "ADMIN" : "USER";
        const when = kenyaDateTimeFromTs(Number(m.at || 0));
        const body = String(m.text || m.kind || "message").slice(0, 350);
  ...
  return (
    ...
    `Chat ID: ${chat.chatId}\n` +
    `Phone: ${chat.phone || "-"}\n` +
    `Status: ${chat.status || (chat.active ? "Active" : "Closed")}\n` +
    `Started: ${kenyaDateTimeFromTs(Number(chat.createdAt || 0))}\n` +
    `Updated: ${kenyaDateTimeFromTs(Number(chat.updatedAt || chat.createdAt || 0))}\n\n` +
    `Recent Messages:\n${transcript}`
  );
}

function getOrCreateSupportChat(chatId) {
  const cid = String(chatId || "");
  ensureSupportChatStore();
  let chat = db.supportChats[cid];
  if (!chat) {
    const u = getUser(chatId);
    chat = db.supportChats[cid] = {
      chatId: cid,
      userName: safeDisplayName(u, chatId),
      phone: formatTo07(getUserPhone(chatId) || "") || getUserPhone(chatId) || "",
      active: true,
    ...
    const u = getUser(chatId);
    chat.userName = safeDisplayName(u, chatId);
    chat.phone = formatTo07(getUserPhone(chatId) || "") || getUserPhone(chatId) || chat.phone || "";
  }
  saveDB();
  return chat;
}

function appendSupportChatMessage(chatId, fromRole, msg) {
  const chat = getOrCreateSupportChat(chatId);
  const kind = supportMessageKind(msg);
  const text = supportMessageSummary(msg);
  chat.messages.push({
    id: String(msg?.message_id || Date.now()),
    from: String(fromRole || "user"),
    kind,
    text: String(text || kind || "message"),
    at: Date.now(),
  });
  if (chat.messages.length > 200) chat.messages = chat.messages.slice(-200);
  chat.updatedAt = Date.now();
  chat.lastMessageAt = Date.now();
  if (fromRole === "admin") chat.status = "Admin replied";
  else if (!chat.active) { chat.active = true; chat.status = "Active"; }
  saveDB();
  return chat;
}

function setSupportChatStatus(chatId, status, active) {
  const chat = getOrCreateSupportChat(chatId);
  chat.status = String(status || chat.status || "Closed");
  chat.active = !!active;
  chat.updatedAt = Date.now();
  saveDB();
  return chat;
...
async function forwardSupportChatToAdmin(userMsg) {
  const fromChatId = userMsg.chat.id;
  const chat = appendSupportChatMessage(fromChatId, "user", userMsg);
  try {
    await bot.copyMessage(ADMIN_ID, fromChatId, userMsg.message_id, supportChatAdminActionsKeyboard(fromChatId, 0));
  } catch (e) {
  ...
  try {
    await bot.sendMessage(
      ADMIN_ID,
      `💬 Live Support from ${chat.userName}\nChat ID: ${chat.chatId}\nPhone: ${chat.phone || "-"}\nStatus: ${chat.status}`,
      supportChatAdminActionsKeyboard(fromChatId, 0)
    );
  } catch (e) {
...
async function sendSupportChatFromAdmin(targetChatId, adminMsg) {
  const chat = appendSupportChatMessage(targetChatId, "admin", adminMsg);
  await bot.copyMessage(Number(targetChatId), adminMsg.chat.id, adminMsg.message_id, supportChatUserKeyboard(targetChatId, true));
  try {
    await bot.sendMessage(Number(targetChatId), `💬 Admin replied. Chat is still open.`, supportChatUserKeyboard(targetChatId, true));
  } catch (_) {}
  return chat;
}
...
if (data.startsWith("admin:")) {
  ...
  if (action === "support") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    ensureSupportChatStore();
    const count = Object.keys(db.supportChats || {}).length;
    await bot.sendMessage(chatId, `💬 Support Requests\n\nTap a user below to open that chat.\nTotal chats: ${count}`, supportChatAdminListKeyboard(0)).catch(() => {});
    return;
  }
}
...
if (data === "profile_support:menu") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await bot.sendMessage(chatId, "🆘 Support Center\n\nChoose an issue below or open live chat with admin.", profileSupportIssueInlineKeyboard()).catch(() => {});
  return;
}

if (data === "profile_support:profile") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  return showUserProfile(chatId);
}

if (data === "profile_support:home") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await sendTracked(chatId, "🏠 Main Menu", { ...mainMenuKeyboard(chatId) });
  return;
}

if (data === "profile_support:tickets") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await bot.sendMessage(chatId, mySupportTicketsText(chatId), supportStatusKeyboard()).catch(() => {});
  return;
}

if (data === "profile_support:status") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const t = latestUserSupportTicket(chatId);
  await bot.sendMessage(chatId, t ? `📌 Ticket Status\n\nTicket ID: ${t.ticketId}\nType: ${t.issueLabel}\nStatus: ${t.status}\nUpdated: ${kenyaDateTimeFromTs(Number(t.updatedAt || t.createdAt || 0))}` : "📌 No support ticket found yet.", supportStatusKeyboard()).catch(() => {});
  return;
}

if (data.startsWith("profile_support:issue:")) {
  const issueKey = String(data.split(":").pop() || "");
  await bot.answerCallbackQuery(q.id).catch(() => {});
  if (issueKey === "admin") {
    getOrCreateSupportChat(chatId);
    supportState.set(String(chatId), { step: "support", startedAt: Date.now() });
    await bot.sendMessage(chatId, "💬 Live chat with admin is now open.\n\nSend text, photo, video, voice, documents, stickers, or any proof here.\nTap End Chat or send /cancel to stop.", supportChatUserKeyboard(chatId, true)).catch(() => {});
    return;
  }
  setPendingAction(chatId, { type: "support_issue", issueKey });
  const auto = supportAutoReply(chatId, issueKey);
  await bot.sendMessage(chatId, `${auto}\n\n${supportPromptFor(issueKey)}\n\nSend /cancel to stop.`, supportStatusKeyboard()).catch(() => {});
  return;
}
...
if (data.startsWith("sca:list:")) {
  const page = Number(data.split(":")[2] || 0) || 0;
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await bot.sendMessage(chatId, `💬 Support Requests\n\nTap a user below to open that chat.`, supportChatAdminListKeyboard(page)).catch(() => {});
  return;
}

if (data.startsWith("sca:open:")) {
  const parts = data.split(":");
  const targetChatId = String(parts[2] || "");
  const page = Number(parts[3] || 0) || 0;
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const chat = ensureSupportChatStore()[targetChatId];
  await bot.sendMessage(chatId, supportChatSummaryText(chat), supportChatAdminActionsKeyboard(targetChatId, page)).catch(() => {});
  return;
}

if (data.startsWith("sca:reply:")) {
  const parts = data.split(":");
  const targetChatId = String(parts[2] || "");
  const page = Number(parts[3] || 0) || 0;
  adminReplyState.set(String(chatId), { type: "support_chat", targetChatId, page, startedAt: Date.now() });
  await bot.answerCallbackQuery(q.id, { text: "Reply mode on" }).catch(() => {});
  await bot.sendMessage(chatId, `💬 Support reply mode is ON for ${targetChatId}.\nSend text, photo, video, voice, document, sticker or GIF.\nSend /cancel to stop.`, supportChatAdminActionsKeyboard(targetChatId, page)).catch(() => {});
  return;
}

if (data.startsWith("sca:end:") || data.startsWith("sca:resolve:") || data.startsWith("sca:restart:") || data.startsWith("sca:delete:")) {
  const parts = data.split(":");
  ...
  const chat = ensureSupportChatStore()[targetChatId];
  if (!chat) {
    await bot.answerCallbackQuery(q.id, { text: "Chat not found." }).catch(() => {});
    return;
  }
  if (action === "restart") {
    chat.active = true;
    ...
    chat.updatedAt = Date.now();
    saveDB();
    supportState.set(String(targetChatId), { step: "support", startedAt: Date.now() });
    await bot.answerCallbackQuery(q.id, { text: "Chat restarted" }).catch(() => {});
    await bot.sendMessage(Number(targetChatId), "💬 Admin reopened your support chat. You can continue replying here.", supportChatUserKeyboard(targetChatId, true)).catch(() => {});
  } else if (action === "end") {
    setSupportChatStatus(targetChatId, "Ended by admin", false);
    supportState.delete(String(targetChatId));
    await bot.answerCallbackQuery(q.id, { text: "Chat ended" }).catch(() => {});
    await bot.sendMessage(Number(targetChatId), "🛑 Your live support chat was closed by admin.", supportChatUserKeyboard(targetChatId, false)).catch(() => {});
  } else if (action === "resolve") {
    setSupportChatStatus(targetChatId, "Resolved", false);
    supportState.delete(String(targetChatId));
    await bot.answerCallbackQuery(q.id, { text: "Resolved" }).catch(() => {});
    await bot.sendMessage(Number(targetChatId), "✅ Admin marked your support chat as resolved.", supportChatUserKeyboard(targetChatId, false)).catch(() => {});
    ...
    await bot.sendMessage(Number(targetChatId), "❌ Your support chat was removed from the active support queue.", supportChatUserKeyboard(targetChatId, false)).catch(() => {});
  }
  await bot.sendMessage(chatId, supportChatSummaryText(ensureSupportChatStore()[targetChatId]), supportChatAdminActionsKeyboard(targetChatId, page)).catch(() => {});
  return;
}

if (data.startsWith("scu:reply:")) {
  const targetChatId = String(data.split(":")[2] || "");
  ...
  getOrCreateSupportChat(chatId);
  supportState.set(String(chatId), { step: "support", startedAt: Date.now() });
  await bot.answerCallbackQuery(q.id, { text: "Chat open" }).catch(() => {});
  await bot.sendMessage(chatId, "💬 Your live chat with admin is open. Send your message or media now.", supportChatUserKeyboard(chatId, true)).catch(() => {});
  return;
}

if (data.startsWith("scu:end:")) {
  const targetChatId = String(data.split(":")[2] || "");
  if (String(chatId) !== targetChatId) {
    await bot.answerCallbackQuery(q.id, { text: "Not allowed." }).catch(() => {});
    return;
  ...
  setSupportChatStatus(chatId, "Ended by user", false);
  await bot.answerCallbackQuery(q.id, { text: "Chat closed" }).catch(() => {});
  await bot.sendMessage(chatId, "🛑 Your support chat is now closed.", supportChatUserKeyboard(chatId, false)).catch(() => {});
  await bot.sendMessage(ADMIN_ID, `🛑 ${safeDisplayName(getUser(chatId), chatId)} ended the live support chat.`, supportChatAdminActionsKeyboard(chatId, 0)).catch(() => {});
  return;
}
          ...
          ? {
              ...
              reply_markup: {
                inline_keyboard: [
                  ...
                  ...((helpAnswerKeyboard()?.reply_markup?.inline_keyboard) || []),
                ],
              },
            }
...
if (/^\/refaudit\s+\d+$/i.test(text)) {
  ...
  if (isAdmin(chatId)) {
    ...
    if (r && text !== "/cancel" && !text.startsWith("/reply")) {
      try {
        if (r.type === "support_chat") {
          await sendSupportChatFromAdmin(r.targetChatId, msg);
          return sendTracked(chatId, `✅ Sent to support chat ${r.targetChatId}`);
        }
        if (!r.until || Date.now() < (r.until || 0)) {
