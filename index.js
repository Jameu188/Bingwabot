...
function buildSessionPreservingCart(chatId, patch = {}) {
  const prev = sessions.get(chatId) || {};
  const next = { ...patch };
  if (Array.isArray(prev.cart)) next.cart = prev.cart.map((item) => ({ ...item }));
  if (prev.cartEditIndex !== undefined && prev.cartEditIndex !== null) next.cartEditIndex = prev.cartEditIndex;
  if (prev.cartEditField !== undefined && prev.cartEditField !== null) next.cartEditField = prev.cartEditField;
  if (prev.cartReminderMsgId) next.cartReminderMsgId = prev.cartReminderMsgId;
  if (prev.cartLastActionAt) next.cartLastActionAt = prev.cartLastActionAt;
  if (prev.cartLastReminderAt) next.cartLastReminderAt = prev.cartLastReminderAt;
  return next;
}
...
if (text === "🏠 Main Menu") {
  try { clearPendingAction(chatId); } catch (_) {}
  try {
    const preserved = buildSessionPreservingCart(chatId, { step: "menu" });
    sessions.set(chatId, preserved);
  } catch (_) {}
  await sendTracked(chatId, "🏠 Main Menu", { ...mainMenuKeyboard(chatId) });
  return;
}
...
💡 Tip: You can also search offers using inline:`, {
    ...
    // /buy
    if (/^\/buy$/i.test(text)) {
      sessions.set(chatId, buildSessionPreservingCart(chatId, { step: "category", category: null, pkgKey: null, createdAt: Date.now() }));
      await sendBannerIfAvailable(chatId);
      return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard(chatId));
    }
...
• 📥 Change Receive Number`,
  ...
  if (text === "🏠 Main Menu") {
    sessions.set(chatId, buildSessionPreservingCart(chatId, { step: "menu" }));
    return sendTracked(chatId, "🏠 Main Menu", { ...mainMenuKeyboard(chatId) });
  }
  if (isBtn(text, "🛒 Buy Offers", "🛒 Nunua Ofa")) {
    sessions.set(chatId, buildSessionPreservingCart(chatId, { step: "category", category: null, pkgKey: null, prev: [] }));
    return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard(chatId));
  }
