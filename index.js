...
async function renderSessionSnapshot(chatId, snap) {
  ...
  const s = { ...current, ...snap, createdAt: Date.now() };
  if (!Array.isArray(s.cart) && Array.isArray(current.cart)) s.cart = current.cart.map((item) => ({ ...item }));
  sessions.set(chatId, s);
...
await sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
  ...
  addPendingPayment(ref, { chatId, category: s.category, pkgLabel: pkg.label, phone254: phone, receivePhone254: receivePhone, price: pkg.price, cartOrders: Array.isArray(s?.cart) && s.cart.length ? s.cart.map((item) => ({ ...item })) : null });
...
function getMergedCartItems(s) {
  ...
  for (const item of cart) {
    ...
    const price = Number(item?.price || 0);
    const key = [category, pkgLabel, receivePhone254].join('|');
    if (!groups.has(key)) {
      groups.set(key, {
        category,
        pkgLabel,
...
This number already bought Bingwa today.`, { parse_mode: "Markdown", ...cartKeyboard() });
    ...
    const totalPkg = { label: `Cart Checkout (${workingCart.length} orders)`, price: workingCart.reduce((n, x) => n + Number(x?.price || 0), 0) };
    const stkSession = { ...s, category: "Cart Checkout", pkgKey: totalPkg.label, cart: workingCart.map((item) => ({ ...item })) };
...
if (pending && pending.category === "Business Account") {
  ...
  if (plan) {
    ...
    try { const ss = sessions.get(chatId); if (ss && Array.isArray(ss.cart) && ss.cart.length) { clearSessionCart(ss); ss.cartEditIndex = null; sessions.set(chatId, ss); } } catch (_) {}
...
if (s.step === "cart_remove_pick") {
  ...
  const cart = getSessionCartItems(s);
  if (idx === null || !cart[idx]) return sendTracked(chatId, "Choose an item from the list.", cartRemoveKeyboard(s));
