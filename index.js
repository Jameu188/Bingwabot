...
if (action === "quals") {
  ...
  await bot.sendMessage(chatId, "✅ Weekly Draw Qualifications (mark/unmark and set values):", weeklyDrawQualificationsKeyboard()).catch(() => {});
  return;
}
...
if (action === "qual_toggle") {
  ...
  await bot.sendMessage(chatId, "✅ Updated.", weeklyDrawQualificationsKeyboard()).catch(() => {});
  return;
}
...
if (action === "qual_set") {
  ...
  c.qualifications = c.qualifications || {};
  c.qualifications[key] = c.qualifications[key] || { enabled: true };
  setWeeklyDrawAdminState(chatId, { mode: "set_qual", key });
  await bot.answerCallbackQuery(q.id, { text: "Send value" }).catch(() => {});
  await bot.sendMessage(chatId, qualificationPromptFor(key), weeklyDrawQualificationsKeyboard()).catch(() => {});
  return;
}
...
if (_wdState.mode === "set_qual") {
  ...
  if (!res.ok) {
    return bot.sendMessage(chatId, `❌ ${res.err || "Invalid value."}\n\n${qualificationPromptFor(key)}`, weeklyDrawQualificationsKeyboard()).catch(() => {});
  }
  try { saveDB(); } catch (_) {}
  setWeeklyDrawAdminState(chatId, null);
  return bot.sendMessage(chatId, "✅ Qualification updated.", weeklyDrawQualificationsKeyboard()).catch(() => {});
}
}
