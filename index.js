...
bot.on("messanst chatId = msg.chat.id;
...
const _bot.listeners("message").slice();
...
bot.on("message", async (msg) => {
  ...
  // 🚫 BLOCK CHECK
  if (!isAdmin(chatId) && isBlocked(chatId)) {
    await bot.sendMr account is currently blocked.\n\nPlease contact admin for assistance."
    );
    return;
  }
  const chatId = msg.chat.id;
...
// Helper to calculate total lifalUserSpend(chatId) {
