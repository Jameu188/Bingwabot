...
bot && bot.onText(/\/settoken\s+(.+)/i, async (msg, match) => {
...
bot && bot.on("inline_query", async (q) => {
  try {
...
bot && bot.on("callback_query", async (q) => {
  try {
...
if (typeof bot !== "undefined") {
  bot && bot.onText(/\/showtokenstatus/, async (msg) => {
    const chatId = msg.chat.id;
...
// Privacy
bot && bot.onText(/📄?\s*Terms of Service/i, async (msg)=>{
  const chatId=msg.chat.id;
...
// Privacy
bot && bot.onText(/🔒?\s*Privacy/i, async (msg)=>{
  const chatId=msg.chat.id;
  bot && bot.sendMessage(chatId,
`🔒 Privacy Policy
...
bot && bot.onText(/🏆?\s*Leaderboard/i, async (msg)=>{
  const chatId=msg.chat.id;
...
bot && bot.onText(/📜?\s*History/i, async (msg)=>{
  const chatId=msg.chat.id;
...
// Toggle command
bot && bot.onText(/🔁\s*STK Retry/i, (msg)=>{
  const chatId=msg.chat.id;
  try{
    const u=getUser(chatId);
