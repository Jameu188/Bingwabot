...
bot.on("inline_query", async (q) => {
  try {
    ...
    // Show matching commands (e.g., "help", "status", "withdraw")
    const cmdHits = commandMatchesInline(query).slice(0, 6);
    for (const c of cmdHits) {
      results.push({
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "🛒 Open Bot Menu", callback_data: "buy_offers" }],
          ],
        },
      });
    }

    // Show matching categories (e.g., "sms", "unlimited")
    const catHits = categoryMatchesInline(query).slice(0, 6);
    for (const cat of catHits) {
      results.push({
        ...
        input_message_content: { message_text: `📦 ${cat}` },
        reply_markup: {
          inline_keyboard: [
            [{ text: "📦 View Packages", callback_data: `inline_open_cat:${base64UrlEncode(cat)}` }],
    ...
    // Show matching offers (all packages)
      ...
      results.push({
        ...
        input_message_content: {
          message_text:
            `🛒 *${escapeMd(h.category)}*\n` +
            `📦 *${escapeMd(h.label)}*\n` +
            `💰 *Ksh ${escapeMd(String(h.price))}*`,
          parse_mode: "Markdown",
        },
        reply_markup: {
          inline_keyboard: [
            [{ text: "✅ Buy This Offer", callback_data: `inline_buy:${base64UrlEncode(h.category)}:${base64UrlEncode(h.label)}` }],
            [{ text: "📦 View Category", callback_data: `inline_open_cat:${base64UrlEncode(h.category)}` }],
          ],
        },
      });
    }

    // If nothing typed, show suggestions
    if (!query) {
      results.unshift(
        {
          type: "article",
          ...
          input_message_content: { message_text: "🔎 Search offers: try 2GB / 24HRS / SMS / 7 DAYS" },
        }
      );
    }
...
// Inline action buttons (Upgrade + Buy Last Offer + Buy Offers)
    ...
    const upsell = pickUpsell(pending?.category, pending?.pkgLabel, pending?.price);
    ...
    if (upsellPkg) {
      inline_keyboard.push([
        { text: `🚀 Upgrade: ${upsellPkg.price} Ksh`, callback_data: `ui:upgrade:${base64UrlEncode(upsell.category)}:${base64UrlEncode(upsellPkg.label)}` }
      ]);
    }
