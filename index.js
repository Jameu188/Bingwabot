...
const cartReminderTimers = new Map();
...
await sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
  ...
  const isCartCheckout = String(s?.category || "") === "Cart Checkout" && Array.isArray(s?.cart) && s.cart.length;
  addPendingPayment(ref, { chatId, category: s.category, pkgLabel: pkg.label, phone254: phone, receivePhone254: receivePhone, price: pkg.price, cartOrders: isCartCheckout ? s.cart.map((item) => ({ ...item })) : null });
  ...
  try {
    ...
    if (Array.isArray(s?.cart) && s.cart.length) {
      const keep = sessions.get(chatId) || { createdAt: Date.now() };
      keep.cart = s.cart.map((item) => ({ ...item }));
      keep.cartLastActionAt = Date.now();
      keep.cartEditIndex = null;
      sessions.set(chatId, keep);
      scheduleCartReminder(chatId);
    } else {
      sessions.delete(chatId);
    }
...
function clearSessionCart(s) {
  ...
  s.cartLastActionAt = null;
}

function stopCartReminder(chatId) {
  const key = String(chatId || "");
  const t = cartReminderTimers.get(key);
  if (t) clearTimeout(t);
  cartReminderTimers.delete(key);
}

function scheduleCartReminder(chatId) {
  const key = String(chatId || "");
  stopCartReminder(key);
  const s = sessions.get(chatId);
  if (!s || !Array.isArray(s.cart) || !s.cart.length) return;
  const lastActionAt = Number(s.cartLastActionAt || Date.now());
  const now = Date.now();
  const idleMs = Math.max(0, now - lastActionAt);
  const TEN_MIN = 10 * 60 * 1000;
  const THIRTY_MIN = 30 * 60 * 1000;

  let nextDelay = THIRTY_MIN - idleMs;
  if (idleMs < TEN_MIN) nextDelay = TEN_MIN - idleMs;
  else if (idleMs < (2 * TEN_MIN)) nextDelay = (2 * TEN_MIN) - idleMs;

  const timer = setTimeout(async () => {
    try {
      const live = sessions.get(chatId);
      if (!live || !Array.isArray(live.cart) || !live.cart.length) {
        stopCartReminder(chatId);
        return;
      }

      const liveIdleMs = Math.max(0, Date.now() - Number(live.cartLastActionAt || Date.now()));
      if (liveIdleMs >= THIRTY_MIN) {
        clearSessionCart(live);
        live.step = live.step === "cart_review" || live.step === "cart_edit_pick" || live.step === "cart_remove_pick" ? "category" : live.step;
        if (live.step === "category") {
          live.category = null;
          live.pkgKey = null;
        }
        sessions.set(chatId, live);
        stopCartReminder(chatId);
        await sendTracked(chatId, "🗑️ Cart cleared after 30 minutes of no action.", { ...mainMenuKeyboard(chatId) });
        return;
      }

      await sendTracked(
        chatId,
        `⏰ Cart reminder\n\nYou still have *${mdEscape(String(getSessionCartItems(live).length))}* item(s) in cart.\nTotal: *Ksh ${mdEscape(String(getSessionCartTotal(live)))}*\n\nOpen your cart to complete, edit, remove, or cancel.`,
        { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
      );
      scheduleCartReminder(chatId);
    } catch (_) {
  ...
  cartReminderTimers.set(key, timer);
}

function touchCartActivity(chatId, s) {
...
function confirmKeyboard(hasSavedPhone, hasReceivePhone = true, chatId = null) {
  ...
  const multiActionBtn = cartHasItems ? "🛒 View Orders in Cart" : "➕ Add to Cart";
  if (hasSavedPhone) rows.push(multiEnabled ? ["✅ Proceed", multiActionBtn] : ["✅ Proceed"]);
  else if (multiEnabled) rows.push([multiActionBtn]);
...
try {
  ...
  if (text === "🛒 View Cart") {
    const s = sessions.get(chatId) || {};
    if (!isMultiPurchaseEnabled(chatId)) {
      return sendTracked(chatId, "⚙️ Multi Purchase is OFF. Enable it from /multipurchase or Settings to use cart.", { ...mainMenuKeyboard(chatId) });
    }
    if (!getSessionCartItems(s).length) {
      return sendTracked(chatId, "🛒 Your cart is empty.", { ...mainMenuKeyboard(chatId) });
    }
    s.step = "cart_review";
    touchCartActivity(chatId, s);
...
Choose a number below or tap ➕ New Number:`;
  ...
  const cartHasItems = getSessionCartItems(s).length > 0;
  const multiActionLine = multiEnabled
    ? `
• ${cartHasItems ? "🛒 View Orders in Cart" : "➕ Add to Cart"}`
    : "";
  const msgText =
    `✅ Selected:
*${pkg.label}*
...
${buildCartSummaryText(chatId, s)}`,
      ...
      if (isBtn(text, "✅ Proceed", "✅ Endelea")) {
        if (Array.isArray(s.cart) && s.cart.length) touchCartActivity(chatId, s);
...
Current: ${maskPhone(cart[idx].phone254 || "") || "Not set"}
• 07XXXXXXXX
• 01XXXXXXXX
• 2547XXXXXXXX
• 2541XXXXXXXX`, confirmKeyboard(false, !!cart[idx].receivePhone254));
}
...
• 2541XXXXXXXX`,
  ...
  if (s.cartEditIndex !== null && s.cartEditIndex !== undefined && Array.isArray(s.cart) && s.cart[s.cartEditIndex]) {
    ...
    s.step = "cart_review";
    touchCartActivity(chatId, s);
    return sendTracked(chatId, `✅ Numbers updated for cart item.
