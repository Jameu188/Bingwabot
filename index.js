...
async function renderSessionSnapshot(chatId, snap) {
  ...
  if (s.step === "package") {
    ...
    return sendTracked(chatId, section(String(s.category || "").toUpperCase()) + `👇 ${s.category} packages:`, packagesKeyboard(chatId, s.category, s));
  }

  if (s.step === "confirm") {
    if (!s.category) return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard(chatId));
    const pkg = findPackageByLabel(s.category, s.pkgKey);
    if (!pkg) return sendTracked(chatId, section(String(s.category || "").toUpperCase()) + `👇 ${s.category} packages:`, packagesKeyboard(chatId, s.category, s));
...
function openCategoryQuick(chatId, category) {
  ...
  s.pkgKey = null;
  sessions.set(chatId, s);
  return sendTracked(chatId, `✅ *${mdEscape(s.category)}*\n\nSelect a package:`, { parse_mode: "Markdown", ...packagesKeyboard(chatId, s.category, s) });
}
...
function packagesKeyboard(category) {
  // Back-compat signature: packagesKeyboard(category)
  // New signature: packagesKeyboard(chatId, category, session)
  let chatId = null;
  let s = null;
  let cat = category;

  // If called as packagesKeyboard(chatId, category, session)
  if ((typeof arguments[0] === "number" || typeof arguments[0] === "string") && typeof arguments[1] === "string") {
    chatId = arguments[0];
    cat = arguments[1];
    s = arguments[2] || null;
  }
  ...
  // Map displayed labels -> canonical labels so Swahili-translated buttons still resolve correctly.
  if (s) s.pkgDisplayMap = s.pkgDisplayMap || {};

  for (let i = 0; i < list.length; i += 2) {
    const pair = list.slice(i, i + 2).map((x) => {
      const canonical = String(x?.label || "");
      const display = isSwahili(chatId) ? swPackageLabel(canonical) : canonical;
      if (s && display) s.pkgDisplayMap[display] = canonical;
      return display;
    });
    rows.push(pair);
  }
...
// Convert common package time words into Swahili.
// Keeps the structure of your existing labels like: "Ksh 49 • 1.5GB 3HRS".
function swPackageLabel(label) {
  let s = String(label || "");
  s = s.replace(/\s+/g, " ").trim();

  // Time units
  s = s.replace(/\b24\s*HRS\b/gi, "SAA 24");
  s = s.replace(/\b(\d+(?:\.\d+)?)\s*HRS\b/gi, "SAA $1");
  s = s.replace(/\b(\d+(?:\.\d+)?)\s*HR\b/gi, "SAA $1");
  s = s.replace(/\b(\d+)\s*DAYS\b/gi, "SIKU $1");
  s = s.replace(/\b(\d+)\s*DAY\b/gi, "SIKU $1");
  s = s.replace(/\bMIDNIGHT\b/gi, "USIKU WA MANANE");

  // Common words
  s = s.replace(/\bWEEKS?\b/gi, "WIKI");
...
try {
  ...
  if (hit && hit.category) {
    ...
    if (!favPkg || !favPkg.pkgLabel || !favPkg.amount) {
      ...
      pushPrev(chatId, sessions.get(chatId) || { step: null });
      const s = { step: "package", category: hit.category, pkgKey: null, createdAt: Date.now() };
      sessions.set(chatId, s);
      await sendBannerIfAvailable(chatId);
      return sendTracked(chatId, section(String(hit.category || "").toUpperCase()) + `👇 ${hit.category} packages:`, packagesKeyboard(chatId, hit.category, s));
    }
  ...
  try {
    ...
    if (text === "⬅ Back" || text === "⬅️ Back" || text === "⬅ Rudi" || text === "⬅️ Rudi") {
      if (s.step === "phone" || s.step === "confirm") {
        s.step = "package";
        sessions.set(chatId, s);
        return sendTracked(chatId, `✅ Choose a ${s.category} package:`, packagesKeyboard(chatId, s.category, s));
      }
      s.step = "category";
      s.category = null;
      s.pkgKey = null;
    ...
    if (s.step === "fast_phone_pick") {
      ...
      if (text === PREV_BTN || text === "⬅ Prev" || text === "⬅ Nyuma" || text === "⬅️ Nyuma") {
        s.step = "package";
        sessions.set(chatId, s);
        return sendTracked(chatId, `👇 ${s.category} packages:`, packagesKeyboard(chatId, s.category, s));
      }
      ...
      if (s.fpPhoneMap && s.fpPhoneMap[text]) {
        ...
        if (!pkg) {
          s.step = "package";
          sessions.set(chatId, s);
          return sendTracked(chatId, "Tap a package button:", packagesKeyboard(chatId, s.category, s));
        }
    ...
    if (s.step === "category") {
      ...
      return sendTracked(chatId, `👇 ${category} packages:`, packagesKeyboard(chatId, category, s));
    }


// STEP 2: PACKAGE BUTTON
if (s.step === "package") {
  const canonicalLabel = (s && s.pkgDisplayMap && s.pkgDisplayMap[text]) ? s.pkgDisplayMap[text] : text;
  const pkg = findPackageByLabel(s.category, canonicalLabel);
  if (!pkg) return sendTracked(chatId, "Tap a package button from the list:", packagesKeyboard(chatId, s.category, s));
