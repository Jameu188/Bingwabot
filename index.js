...
// ===================== GLOBAL BUTTONS =====================
  ...
  // 🎁 Redeem feature permanently disabled
// Redeem removed from home menu
...
• Cycle resets in: ${st.daysLeft} day(s)`,
    ...
    if (bal < item.cost) {
      return sendTracked(chatId, `❌ Not enough points.
You have: ${bal.toFixed(2)} pts
Need: ${item.cost} pts`, { ...mainMenuKeyboard(chatId) });
    }
...
async function deleteIfExists(chatId, messageId) {
  try {
    if (messageId) {
      await bot.deleteMessage(chatId, messageId);
    }
