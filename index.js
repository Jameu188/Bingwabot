...
async function showUserProfile(chatId) {
  ...
  // Last 5 transactions
  const last5 = (u.txLog || []).slice(0, 5);
  const txLines = last5.length ? last5.map(t => `• ${fmtTxLine(t)} (${shortKenyaTime(t.ts)})`).join("\n") : "• (no recent transactions)";

  // Last 5 numbers used today (masked)
  const todaysPhones = [];
  for (const t of (u.txLog || [])) {
    ...
    if (todaysPhones.length >= 5) break;
  }
  const phonesLine = todaysPhones.length ? todaysPhones.map(p => `• ${maskPhone(p)}`).join("\n") : "• (none today)";
