...
function formatAdminConfirmedReceipt(chatId, pending, txTs, txCostKsh = 0) {
  ...
  const receive = String(pending?.receivePhone254 || pending?.phone254 || "").trim();
  const rawName = String(safeDisplayName(getUser(chatId), chatId) || "").trim();
  const name = rawName.replace(/\s+/g, " ");
  const balanceKes = Number(pointsToKes(getPoints(chatId) || 0)).toFixed(2);
  const cost = Number(txCostKsh || 0).toFixed(2);

  const senderPart = [receive, name].filter(Boolean).join(" ");
  return `${randomReceiptCode(10)} Confirmed.on ${parts.date} at ${parts.time} Ksh${amount} received from ${senderPart}. New Account balance is Ksh${balanceKes}. Transaction cost, Ksh${cost}.`;
}
...
// Inline action buttons (Upgrade + Buy Again + Buy Offers)
    ...
    try {
      await notifyAdmin(formatAdminConfirmedReceipt(chatId, pending, whenTs, 0));
    } catch (_) {}
