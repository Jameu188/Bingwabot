...
if (_wdState.mode === "set_period") {
  ...
  // Option A: explicit date range (backward compatible)
  const parts = raw.split(/\s+/).filter(Boolean);
  ...
  // Option B: duration (starts now)
  const durMs = parseDurationMs(raw);
  if (durMs && durMs >= 60 * 1000) { // at least 1 minute
    const now = Date.now();
    c.startAt = now;
    c.endAt = now + durMs;
    c.status = c.status === "OFF" ? "RUNNING" : c.status;
   ...
   return;
        ...
        if (/^\/weeklydraw_end$/i.test(text)) {
          ...
          const winners = (db.campaign.winners || []).map((w, i) => {
            const pos = i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉";
            return `${pos} ${(w.name || safeDisplayName(getUser(w.chatId), w.chatId) || w.chatId)} — Ksh ${Number(w.prize || 0)} (Pending ⏳)`;
          }).join("\n") || "No eligible entries.";
