...
async function initiatePurchaseStk(chatId, s, pkg, phone254, receivePhone254) {
  ...
  if (!canBypassBingwaLimit(chatId) && s.category === "Bingwa Deals" && bingwaAlreadyPurchasedToday(receivePhone || phone)) {
    sessions.delete(chatId);
    return sendTracked(
      chatId,
      `🚫 *Bingwa Deals limit reached*\nNumber: *${maskPhone(receivePhone || phone)}*\nThis number already bought Bingwa today.\n\nUse a different number or try again tomorrow.`,
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  }
    ...
    if (pending) {
      ...
      for (const paidOrder of paidOrders) {
        ...
        if (paidOrder.category === "Bingwa Deals" && paidOrder.phone254) {
          markBingwaPurchasedToday(paidOrder.receivePhone254 || paidOrder.phone254);
        }
