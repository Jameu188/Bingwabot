...
const REFERRAL_SHARE_MESSAGES = [
  `🔥 Join Bingwa Data Bot today and save more on your daily offers. Earn points on purchases and stand a chance to win weekly prizes 🏆\n\nUse my invite link to join: {link}`,
  `🚀 Want cheap offers, rewards, and weekly prize chances? Join Bingwa Data Bot now using my invite link 👇\n\n{link}`,
  `🎁 Buy offers fast, earn points, and enjoy exciting rewards with Bingwa Data Bot. Tap my invite link to get started: {link}`,
  `💸 Start buying cheaper offers on Bingwa Data Bot and earn points as you go. Join here now: {link}`,
  `🏆 Bingwa Data Bot makes it easy to buy offers, collect points, and compete for weekly prizes. Join with my link: {link}`,
  `⚡ Save time, buy cheap offers, and earn rewards on Bingwa Data Bot. Use my referral link and join now: {link}`,
  `📲 Join Bingwa Data Bot today for cheap offers, points on purchases, and exciting weekly prize chances. My invite link: {link}`
];

function buildRandomReferralMessage(referralLink) {
  const arr = Array.isArray(REFERRAL_SHARE_MESSAGES) ? REFERRAL_SHARE_MESSAGES : [];
  if (!arr.length) return referralLink;
  const i = Math.floor(Math.random() * arr.length);
  return String(arr[i]).replace(/\{link\}/g, referralLink);
}
...
function gmailPercentButtons(stockCount) {
  const labels = [1, 3, 5, 7, 10, 15, 21, 27, 30];
  const rows = [];
  const seen = new Set();

  for (const qtyRaw of labels) {
    const qty = Math.max(1, Number(qtyRaw || 0));
    if (qty > stockCount || seen.has(qty)) continue;
    seen.add(qty);
    rows.push([{ text: `📧 Buy ${qty} Email${qty > 1 ? "s" : ""} - Ksh ${qty * gmailPrice()}`, callback_data: `gmail_qty:${qty}` }]);
  }

  if (!rows.length && stockCount > 0) {
    rows.push([{ text: `📧 Buy ${stockCount} Email${stockCount > 1 ? "s" : ""} - Ksh ${stockCount * gmailPrice()}`, callback_data: `gmail_qty:${stockCount}` }]);
  }
