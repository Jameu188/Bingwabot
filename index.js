...
function maybeRewardInviterOnSuccessPurchase(buyerChatId, amountKsh, when) {
  ...
  // Referral rewards only apply within REFERRAL.EXPIRY_DAYS after referral was set
  ...
  const bonusKes = amt * REFERRAL.BONUS_PERCENT;
  ...
  // ✅ Count unique successful referrals only once (for stats/eligibility)
  ...
  const inviterPtsNow = getPoints(inviterId);

  // ✅ Notify inviter on EVERY rewarded purchase
  try {
    const msg =
      `🎉 Referral purchase successful!\n` +
      `👤 Referred user: ${buyerChatId}\n` +
      `💳 Amount: ${amt} KSH\n` +
      `🎁 Bonus: ${cleanBonusPts} pts (≈ ${cleanBonusKes} KSH)\n` +
      `⭐ Your points: ${inviterPtsNow.toFixed(2)} pts`;
    sendTracked(inviterId, msg, { ...mainMenuKeyboard(inviterId) });
  } catch (_) {}
  ...
  notifyAdmin(
    `🎯 *Referral Reward*\n` +
      ...
      `Bonus: *${mdEscape(cleanBonusPts)} pts* (Ksh ${mdEscape(cleanBonusKes)})\n` +
