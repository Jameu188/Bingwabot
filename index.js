...
// Ensure user fields
function ensureWithdrawFields(u) {
  if (!u.points) u.points = 0;
  if (!u.totalSpent) u.totalSpent = 0;
  if (!u.lastWithdrawDay) u.lastWithdrawDay = "";
}

// Withdraw buttons
const WITHDRAW_AMOUNTS = [50, 100, 150, 500, 1000];

// Add withdraw button to main menu
const OLD_MAIN_MENU = mainMenuKeyboard;
mainMenuKeyboard = function() {
  const kb = OLD_MAIN_MENU();
  kb.reply_markup.keyboard.unshift(["💸 Withdraw"]);
  return kb;
};
...
bot.on("message", async (msg) => {
  const chatId = msg.chat.id;
  const text = (msg.text || "").trim();
  if (text !== "💸 Withdraw" && !WITHDRAW_AMOUNTS.map(a=>`${a} Ksh`).includes(text)) return;

  const u = getUser(chatId);
  ensureWithdrawFields(u);

  if (text === "💸 Withdraw") {
    return bot.sendMessage(chatId, "Select amount to withdraw:", {
      reply_markup: {
        keyboard: WITHDRAW_AMOUNTS.map(a => [`${a} Ksh`]).concat([["❌ Cancel"]]),
        resize_keyboard: true
      }
    });
  }
  ...
  // Anti-fraud checks
  const totalPurchases = Object.values(u.purchasesByDay || {}).reduce((a,b)=>a+b,0);
  if (totalPurchases < 10) {
    return bot.sendMessage(chatId, `🔒 Withdrawal locked. Purchases: ${totalPurchases}/10`, mainMenuKeyboard());
  }
  if ((u.totalSpent || 0) < 200) {
    return bot.sendMessage(chatId, `🔒 Withdrawal locked. Total spent: Ksh ${u.totalSpent}/200`, mainMenuKeyboard());
  }
  if (u.lastWithdrawDay === today) {
    return bot.sendMessage(chatId, "❌ You already withdrew today.", mainMenuKeyboard());
  }
  if (u.points < amount) {
    return bot.sendMessage(chatId, "❌ Insufficient balance.", mainMenuKeyboard());
  }
  ...
  // Admin notification (full number)
  await bot.sendMessage(
    ADMIN_ID,
    `💸 New Withdrawal Request\nUser: ${chatId}\nMpesa: ${u.phone}\nAmount: Ksh ${amount}\nTime: ${kenyaNow()}\nTotal Purchases: ${totalPurchases}\nTotal Spent: Ksh ${u.totalSpent}`
  );
});
