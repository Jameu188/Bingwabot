...
function maskPhoneDisplay(phone254) {
  ...
  if (/^254\d{9}$/.test(String(phone254 || ""))) {
    const as07 = formatTo07(String(phone254 || ""));
  ...
  try {
    ...
    if (data.startsWith("RD|")) {
      ...
      if (action === "ACCEPT") {
        ...
        // 250MB once per day per phone
        if (String(req.itemKey) === "FREE_250MB") {
...
⏳ Waiting for admin approval…`,
  ...
  if (text === "🏠 Main Menu") {
    ...
    supportState.delete(String(chatId));
    clearPendingAction(chatId);
    return sendTracked(chatId, "🏠 Main Menu", { ...mainMenuKeyboard() });
  }

  if (text === "🔁 Retry STK") {
    const u = getUser(chatId);

    if (hasPendingPaymentForChat(chatId)) {
