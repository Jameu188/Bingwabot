...
function todayKey() {
  ...
  try {
    ...
    if (data.startsWith("withdraw_")) {
      ...
      if (!found) {
        await bot.answerCallbackQuery(q.id, { text: "Already handled.", show_alert: true }).catch(() => {});
        return;
      }
      ...
      if (action === "withdraw_accept") {
        ...
        await bot.sendMessage(
          targetChatId,
          `✅ Withdrawal Approved!\n\nAmount: ${amount} pts\nMPESA: ${req.phone}\n\nYou will receive the money shortly.`,
          { ...mainMenuKeyboard() }
        ).catch(() => {});
...
bot.on("callback_query", async (q) => {
  try {
    ...
    try { await bot.answerCallbackQuery(q.id, { text: "Error.", show_alert: true }); 
    ).catch(() => {});
        return;
      }
