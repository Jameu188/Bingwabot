...
// Save as: /home/container/index.js
...
// ✅ CHANNEL IDS (SET THESE)
// DATA channel used by: Bingwa Deals + Unlimited Deals
const PAYHERO_CHANNEL_ID_DATA = 2486;
// SMS channel used by: SMS Offers + Bonga Points + Flex Deals
const PAYHERO_CHANNEL_ID_SMS = 5577;
// MINUTES channel used by: Minutes
...
//        "lastSeen": 0,
//        "purchasesByDay": { "YYYY-MM-DD": 1 },
//        "weeklyPurchases": { "YYYY-WW": { "<packageLabel>": 4 } },
//        "inviterId": "<chatId>" | "",
//        "lastReferralRewardDay": "YYYY-MM-DD" | "",
//        "points": 0,
//        "pendingAction": null | { kind, step, data, createdAt }   // (redeem only)
//     }
//   },
//   "bingwaByPhone": { "YYYY-MM-DD": { "2547xxxxxxx": 1 } },       // ✅ once-per-day per phone
...
function loadDB() {
  try {
    if (!fs.existsSync(DB_FILE)) {
      const init = { users: {}, bingwaByPhone: {}, stats: { totalPurchases: 0, totalBroadcasts: 0 } };
...
const PACKAGES = {
  ...
  Minutes: [
    // ✅ updated + new
    { label: "Ksh 25 • 20 MIN MIDNIGHT", price: 25 }, // updated from 21 -> 25
...
// Admin broadcast session
const adminState = new Map();

// Broadcast album buffers (admin only)  ✅ declared ONCE ONLY
const broadcastAlbums = new Map(); // media_group_id -> { chatId, items: [msg], timer }

// Support mode (user -> admin)
const supportState = new Map();

// Admin reply target (admin -> user)
const adminReplyState = new Map();

// ===================== CLEANUP (best effort) =====================
// Deletes ALL previous bot messages + deletes user inputs.
// EXCEPTION: /start does NOT trigger cleanup (requested).
...
// ===================== BROADCAST HELPERS (ANY TYPE + ALBUMS) =====================
...
function queueBroadcastAlbum(adminChatId, msg) {
  ...
  existing.timer = setTimeout(async () => {
    ...
    try {
      ...
      incBroadcastStats();
      await sendTracked(adminChatId, `📣 Broadcast album done.\nSent: ${result.sent}\nFailed: ${result.failed}\nUsers: ${result.total}`);
    } catch (e) {
...
bot.on("message", async (msg) => {
  ...
  if (isAdmin(chatId) && aState?.step === "await_broadcast") {
    ...
    try {
      ...
      incBroadcastStats();
      adminState.delete(String(chatId));
      return sendTracked(chatId, `📣 Broadcast done.\nSent: ${result.sent}\nFailed: ${result.failed}\nUsers: ${result.total}`);
    } catch (e) {
  ...
  if (text === "🔗 My Referral") {
    ...
    return sendTracked(
      chatId,
      `🔗 Your referral:\n${link}\n\n⭐ Your points: ${getPoints(chatId).toFixed(1)} pts\n\nRule: You earn +${REFERRAL.POINTS_PER_DAY_FIRST_STK} pts when your referred user makes their *first STK of the day* (Data + Unlimited only).`,
      { ...mainMenuKeyboard() }
    );
  }
  ...
  try {
    ...
    if (s.step === "confirm") {
      ...
      if (text === "📞 Change Number") {
        s.step = "phone";
        sessions.set(chatId, s);
        return sendTracked(chatId, "📱 Send your phone number:\n• 07XXXXXXXX\n• 01XXXXXXXX\n• 2547XXXXXXXX\n• 2541XXXXXXXX", confirmKeyboard(false));
      }
      ...
      if (text === "✅ Proceed") {
        ...
        if (!savedPhone) {
          s.step = "phone";
          sessions.set(chatId, s);
          return sendTracked(chatId, "📱 No saved number found. Send phone:\n• 07XXXXXXXX\n• 01XXXXXXXX\n• 2547XXXXXXXX\n• 2541XXXXXXXX", confirmKeyboard(false));
        }
        ...
        try {
          ...
          // ✅ mark bingwa phone as used today AFTER success
          if (s.category === "Bingwa Deals") {
            markBingwaPurchasedToday(savedPhone);
          }

          incPurchaseStats(chatId, pkg.label);
          maybeRewardInviterOnSuccessPurchase(chatId, s.category);

          sessions.delete(chatId);

          await notifyAdmin(`✅ *STK Sent OK*\nChatID: \`${chatId}\`\nRef: \`${ref}\`\nAmount: *Ksh ${pkg.price}*\nChannel: *${channelId}*`);
    ...
    if (s.step === "phone") {
      const phone = normalizePhone(text);
      if (!phone) {
        return sendTracked(chatId, "❌ Invalid phone.\nUse: 07XXXXXXXX / 01XXXXXXXX / 2547XXXXXXXX / 2541XXXXXXXX", confirmKeyboard(false));
      }
