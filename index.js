...
function ensureWeeklyDrawObject() {
  ...
  function weeklyDrawUserMenuKeyboard(chatId) {
    const opted = isWeeklyDrawOptedIn(chatId);

    // Show only the relevant opt button
    const rows = [];
    if (!opted) {
      rows.push([{ text: "✅ OPT IN", callback_data: "wdusr:optin" }]);
    } else {
      rows.push([{ text: "🚫 OPT OUT", callback_data: "wdusr:optout" }]);
    }
...
function weeklyDrawMoreText() {
    ...
    const p = c.prizes || {};
    const prizesTxt = `🥇 ${Number(p.first||0)} KSH  •  🥈 ${Number(p.second||0)} KSH  •  🥉 ${Number(p.third||0)} KSH`;
...
` +
      `What is this?
Weekly Draw is a weekly giveaway where you earn tickets from successful purchases. More tickets = higher chance to win.
...
How to participate:
1) Tap ✅ OPT IN (so you receive ticket updates, reminders & winner announcements)
...
How to qualify (to appear on Top Participants & win):
• 5+ successful purchases of 20 KSH or more
• 3 active referrals (3 referred users who have at least 1 successful purchase)

What is an active referral?
A person who joined using your referral link and has made at least one successful purchase.

Tip:
Buy more to climb the Top 10 and stay there until the draw ends.`
    );
  }
...
function weeklyDrawQualification(chatId) {
  ...
  const purchases20 = Number(u.qPurch20Count || 0);
  const activeRefs = countActiveReferrals(chatId);

  const okAge = ageDays >= 3;
  const okPurch = purchases20 >= 5;
  const okRefs = activeRefs >= 3;

  // Business Fast Track:
  // While subscription is active, if user has 5+ active referrals AND spent 500+ KSH in the current campaign,
  // they qualify even if missing ONE requirement (i.e., any 2 of the 3 core requirements).
  const c = ensureWeeklyDrawObject();
  ...
  const satisfiedCount = (okAge ? 1 : 0) + (okPurch ? 1 : 0) + (okRefs ? 1 : 0);
  const qualified = (okAge && okPurch && okRefs) || (fastTrack && satisfiedCount >= 2);

  // Return remaining requirements ONLY (completed ones are omitted)
  const remaining = [];
  if (!okAge) remaining.push(`Stay ${Math.max(0, 3 - ageDays)} more day(s)`);
  if (!okPurch) remaining.push(`${Math.max(0, 5 - purchases20)} more purchases (≥20 KSH)`);
  if (!okRefs) remaining.push(`${Math.max(0, 3 - activeRefs)} more active referrals`);

  return { qualified, fastTrack, spent, ageDays, purchases20, activeRefs, remaining };
}
...
if (data.startsWith("wdusr:")) {
  ...
  if (action === "optin") {
    setWeeklyDrawOptIn(chatId, true);
    await bot.answerCallbackQuery(q.id, { text: "Opted in ✅" }).catch(() => {});
    await bot.sendMessage(chatId, `✅ You have OPTED IN to WEEKLY DRAW!\n\nYou will now receive ticket updates, reminders and winner announcements.\n\n🎉 You could be a winner — keep buying & inviting to increase your tickets!`, mainMenuKeyboard(chatId)).catch(() => {});
  } else if (action === "optout") {
    setWeeklyDrawOptIn(chatId, false);
    await bot.answerCallbackQuery(q.id, { text: "Opted out" }).catch(() => {});
    await bot.sendMessage(chatId, `😔 You have OPTED OUT of WEEKLY DRAW.\n\nYou will no longer receive campaign messages.\n\nYou can opt in again anytime from 🎉 WEEKLY DRAW.`, mainMenuKeyboard(chatId)).catch(() => {});
  } else if (action === "more") {
    await bot.answerCallbackQuery(q.id, { text: "More info" }).catch(() => {});
    const txt = weeklyDrawMoreText() + weeklyDrawLastWinnersText();
    const lastImg = String((c.lastEnded && c.lastEnded.image) || "").trim();
    if (lastImg) {
      await bot.sendPhoto(chatId, lastImg, { caption: txt, ...mainMenuKeyboard(chatId) }).catch(async () => {
        await bot.sendMessage(chatId, txt, mainMenuKeyboard(chatId)).catch(() => {});
      });
    } else {
      await bot.sendMessage(chatId, txt, mainMenuKeyboard(chatId)).catch(() => {});
    }
    ...
    const txt = weeklyDrawLastWinnersText();
    const lastImg = String((c.lastEnded && c.lastEnded.image) || "").trim();
    if (!txt.trim()) {
      await bot.sendMessage(chatId, "No previous winners yet.", mainMenuKeyboard(chatId)).catch(() => {});
      return;
    }
    if (lastImg) {
      await bot.sendPhoto(chatId, lastImg, { caption: txt, ...mainMenuKeyboard(chatId) }).catch(async () => {
    ...
    // Re-render the weekly draw view
    // We simulate by sending the same view caption+image with user menu.
    ...
    const optedLine = isWeeklyDrawOptedIn(chatId) ? "✅ You are opted in (you will receive updates)." : "ℹ️ You are not opted in (no campaign messages). Tap ✅ OPT IN.";
    const caption =
      `🎉 ${heading}\n` +
      `Status: ${status}\n` +
    ...
    if (img) {
      await bot.sendPhoto(chatId, img, { caption, ...weeklyDrawUserMenuKeyboard(chatId) }).catch(async () => {
        await bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId)).catch(() => {});
      });
    } else {
      await bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId)).catch(() => {});
    }
...
if (data.startsWith("wdadm:")) {
  ...
  if (action === "menu") {
    ...
    await bot.sendMessage(chatId, statusText, weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }
  ...
  if (action === "set_prizes") {
    setWeeklyDrawAdminState(chatId, { mode: "set_prizes" });
    await bot.answerCallbackQuery(q.id, { text: "Send prizes like: 250 150 100" }).catch(() => {});
    await bot.sendMessage(chatId, "📝 Send prizes as: 250 150 100 (total ≤ 500).", weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }

  if (action === "set_period") {
    setWeeklyDrawAdminState(chatId, { mode: "set_period" });
    await bot.answerCallbackQuery(q.id, { text: "Send period like: 2026-03-01 2026-03-08" }).catch(() => {});
    await bot.sendMessage(chatId, "🗓 Send period as: YYYY-MM-DD YYYY-MM-DD", weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }

  
if (action === "set_image") {
  setWeeklyDrawAdminState(chatId, { mode: "set_image" });
  await bot.answerCallbackQuery(q.id, { text: "Send image (photo) or paste image URL" }).catch(() => {});
  await bot.sendMessage(chatId, "🖼 Send a PHOTO now, or paste an image URL for WEEKLY DRAW.", weeklyDrawAdminMenuKeyboard()).catch(() => {});
  return;
}

if (action === "set_heading") {
    setWeeklyDrawAdminState(chatId, { mode: "set_heading" });
    await bot.answerCallbackQuery(q.id, { text: "Send heading text" }).catch(() => {});
    await bot.sendMessage(chatId, "📝 Send new heading text for WEEKLY DRAW.", weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }
  ...
  if (action === "end") {
    ...
    // End & announce winners to ALL users + DM winners
  ...
  if (action === "pay") {
    ...
    await bot.sendMessage(chatId, "💰 Tap a winner to toggle Paid/Pending.", weeklyDrawPayWinnersKeyboard()).catch(() => {});
    return;
  }
  ...
  if (action === "toggle_paid") {
    ...
    await bot.sendMessage(chatId, "💰 Winners status updated.", weeklyDrawPayWinnersKeyboard()).catch(() => {});
    return;
  }
...
if (text === "🎉 WEEKLY DRAW") {
  ...
  const optedLine = isWeeklyDrawOptedIn(chatId)
    ...
    : "ℹ️ You are not opted in (no campaign messages). Tap ✅ OPT IN to participate.";
...
if (_wdState.mode === "set_image") {
  ...
  if (!img) {
    await bot.sendMessage(chatId, "❌ Please send a PHOTO or a direct image URL (https://...).", weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }

  c.image = img;
  try { saveDB(); } catch (_) {}
  await bot.sendMessage(chatId, "✅ WEEKLY DRAW image updated.", weeklyDrawAdminMenuKeyboard()).catch(() => {});
  setWeeklyDrawAdminState(chatId, null);
  return;
}
...
setWeeklyDrawAdminState(chatId, null);
        const statusText = formatWeeklyDrawAdminStatus();
        return bot.sendMessage(chatId, `✅ Prizes updated.\n\n${statusText}`, weeklyDrawAdminMenuKeyboard()).catch(() => {});
      }
    }
    return bot.sendMessage(chatId, "❌ Invalid prizes. Send: 250 150 100 (total must be ≤ 500).", weeklyDrawAdminMenuKeyboard()).catch(() => {});
  }
  ...
  if (_wdState.mode === "set_period") {
    ...
    if (parts.length >= 2) {
      ...
      if (Number.isFinite(s) && Number.isFinite(e) && e > s) {
        ...
        setWeeklyDrawAdminState(chatId, null);
        const statusText = formatWeeklyDrawAdminStatus();
        return bot.sendMessage(chatId, `✅ Period updated.\n\n${statusText}`, weeklyDrawAdminMenuKeyboard()).catch(() => {});
      }
    }
    return bot.sendMessage(chatId, "❌ Invalid period. Send: 2026-03-01 2026-03-08", weeklyDrawAdminMenuKeyboard()).catch(() => {});
  }
  ...
  if (_wdState.mode === "set_heading") {
    ...
    if (heading.length >= 3) {
      ...
      setWeeklyDrawAdminState(chatId, null);
      const statusText = formatWeeklyDrawAdminStatus();
      return bot.sendMessage(chatId, `✅ Heading updated.\n\n${statusText}`, weeklyDrawAdminMenuKeyboard()).catch(() => {});
    }
    return bot.sendMessage(chatId, "❌ Heading too short. Send a valid heading text.", weeklyDrawAdminMenuKeyboard()).catch(() => {});
  }
}
