...
Help: ${HELP_PHONE}`, { ...mainMenuKeyboard() });
    ...
    let chatId = NaN;
    // Resolve chatId ONLY from our saved mapping using externalRef
    try {
      db = repairDB(db);
      const rec = db.pendingPayByRef && db.pendingPayByRef[String(externalRef)];
      if (rec && Number.isFinite(Number(rec.chatId))) chatId = Number(rec.chatId);
    } catch (_) {}
    if (!Number.isFinite(chatId) || chatId <= 0) {
      // fallback: extract trailing digits from externalRef (if your format includes it)
      const ex = String(externalRef || "");
      const m = ex.match(/(\d{5,})\s*$/);
      if (m && m[1]) chatId = Number(m[1]);
    }
    // END chatId resolve
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // Map externalRef -> chatId so callback can message user without PayHero chatId
    try {
      db = repairDB(db);
      db.pendingPayByRef[String(externalRef)] = {
        chatId,
        phone254,
        pkgLabel: pending?.pkgLabel || pkgLabel || "",
        amount: Number(amount || 0),
        createdAt: Date.now(),
      };
      saveDB();
    } catch (e) {
      console.log("pendingPayByRef save failed:", e?.message || e);
    }
