...
function setUserPhone(chatId, phone254) {
  ...
  // NOTE: This does NOT stop multiple accounts from purchasing using the same number if they type it manually.
  db = repairDB(db);
  const cid = String(chatId);
  const u = getUser(chatId);
...
" +
        "✅ You can still *purchase* using this number by entering it when needed.
" +
        "❗ But you cannot *save* the same number on two accounts at the same time.
  ...
  return sendTracked(chatId, msgText, { 
    ...
    if (s.step === "confirm") {
      ...
      if (phoneMaybe) {
        ...
        if (!saveRes.ok) {
          return sendTracked(chatId, saveRes.reason, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
        }
