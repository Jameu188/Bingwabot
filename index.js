...
if (data.startsWith("nav:")) {
  ...
  if (action === "weeklydraw") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    // Weekly Draw button already handled by user weekly draw menu entrypoint
    await showWeeklyDrawMain(chatId, q.message?.message_id);
    return;
  }
  ...
  if (action === "wdadmin") {
    ...
    await showWeeklyDrawAdminPanel(chatId, q.message?.message_id);
    return;
  }

  if (action === "buyoffers") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    await sendTracked(chatId, "🛒 Choose a category:", categoriesKeyboard());
    return;
  }
  ...
  if (action === "cat") {
    ...
    await bot.answerCallbackQuery(q.id).catch(() => {});
    // openCategoryQuick expects exact category label keys
    await openCategoryQuick(chatId, label);
    return;
  }
  ...
  if (action === "pkg") {
    ...
    const list = PACKAGES[cat] || [];
    const pkg = list[idx];
    await bot.answerCallbackQuery(q.id).catch(() => {});
    if (!pkg) {
      await sendTracked(chatId, "❌ Package not found. Please select again.", { ...packagesKeyboard(cat) });
      return;
    }
  ...
  if (action === "withdraw") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    // existing flow expects showing withdraw keyboard
    await sendTracked(chatId, "💸 Choose points to withdraw:", withdrawKeyboard());
    return;
  }

  if (action === "withdraw_amt") {
    const pts = Number(parts[2] || "0");
    await bot.answerCallbackQuery(q.id).catch(() => {});
    // Reuse existing text handler by setting a session value and prompting for phone, etc.
    const s = sessions.get(chatId) || {};
    s.withdrawPts = pts;
    s.step = "withdraw_phone";
    sessions.set(chatId, s);
    await sendTracked(chatId, `📞 Enter your M-Pesa number to withdraw *${pts} pts* (07/01/2547/2541):`, { parse_mode: "Markdown" });
    return;
  }
  ...
  if (action === "redeem") {
    ...
    s.step = "redeem_phone";
    sessions.set(chatId, s);
    await sendTracked(chatId, `📞 Enter your Safaricom number to redeem *${pts} pts* (07/01/2547/2541):`, { parse_mode: "Markdown" });
    return;
  }

  if (action === "referral") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    await showReferralScreen(chatId);
    return;
  }

  if (action === "help") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    await showHelpScreen(chatId);
    return;
  }
  ...
  if (action === "biz") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    const plan = parts[2] || "";
    if (plan === "1m") return startBusinessPurchase(chatId, 105, 30);
    if (plan === "3m") return startBusinessPurchase(chatId, 260, 90);
    return;
  }
...
function mainMenuKeyboard(chatId = null) {
  ...
  // WEEKLY DRAW (global visibility) — always first
  if (db.campaign && db.campaign.enabled === true) {
    rows.push([{ text: "🎉 WEEKLY DRAW", callback_data: "nav:weeklydraw" }]);
  }

  rows.push([{ text: "🛒 Buy Offers", callback_data: "nav:buyoffers" }]);

  if (chatId) {
    const quick = getQuickCategories(chatId);
    if (Array.isArray(quick) && quick.length) {
      // show up to 3 quick category buttons
      const quickRow = [];
      for (const q of quick.slice(0, 3)) {
        quickRow.push({ text: q.text, callback_data: `nav:cat:${encodeURIComponent(q.text)}` });
      }
      if (quickRow.length) rows.push(quickRow);
    }
  }

  rows.push([{ text: "💸 Withdraw Points", callback_data: "nav:withdraw" }]);
  rows.push([
    { text: "🔗 My Referral", callback_data: "nav:referral" },
    { text: "ℹ️ Help", callback_data: "nav:help" },
  ]);

  if (chatId && (typeof isAdmin === "function" && isAdmin(chatId))) {
    rows.push([{ text: "🎛 WEEKLY DRAW ADMIN", callback_data: "nav:wdadmin" }]);
  }
...
function categoriesKeyboard() {
  const rows = [
    [
      { text: "📦 Bingwa Deals", callback_data: "nav:cat:📦%20Bingwa%20Deals" },
      { text: "∞ Unlimited Deals", callback_data: "nav:cat:∞%20Unlimited%20Deals" },
    ],
    [
      { text: "✉️ SMS Offers", callback_data: "nav:cat:✉️%20SMS%20Offers" },
      { text: "📞 Minutes", callback_data: "nav:cat:📞%20Minutes" },
    ],
    [
      { text: "⭐ Bonga Points", callback_data: "nav:cat:⭐%20Bonga%20Points" },
      { text: "🌀 Flex Deals", callback_data: "nav:cat:🌀%20Flex%20Deals" },
    ],
...
function packagesKeyboard(category) {
  ...
  for (let i = 0; i < list.length; i += 2) {
    ...
    const a = list[i];
    const b = list[i + 1];
    if (a) row.push({ text: a.label, callback_data: `nav:pkg:${encodeURIComponent(category)}:${i}` });
    if (b) row.push({ text: b.label, callback_data: `nav:pkg:${encodeURIComponent(category)}:${i+1}` });
    if (row.length) rows.push(row);
  }
  rows.push([{ text: "⬅️ Back", callback_data: "nav:categories" }]);
  return { reply_markup: { inline_keyboard: rows } };
}
...
function confirmKeyboard(hasSavedPhone) {
  const rows = [];
  const first = [];
  first.push({ text: "✅ Proceed", callback_data: "uic:proceed" });
  if (hasSavedPhone) first.push({ text: "📞 Change Number", callback_data: "uic:change" });
  rows.push(first);
  rows.push([{ text: "⬅️ Back", callback_data: "uic:back" }]);
  return { reply_markup: { inline_keyboard: rows } };
}
...
function redeemKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "1️⃣ 20 free SMS (5 pts)", callback_data: "nav:redeem:5" }],
        [{ text: "2️⃣ 250MB free (20 pts)", callback_data: "nav:redeem:20" }],
        [{ text: "3️⃣ 20mins midnight free (25 pts)", callback_data: "nav:redeem:25" }],
        [{ text: "⬅️ Back", callback_data: "nav:home" }],
      ],
    },
  };
...
function withdrawKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [
          { text: "25 pts", callback_data: "nav:withdraw_amt:25" },
          { text: "50 pts", callback_data: "nav:withdraw_amt:50" },
        ],
        [
          { text: "100 pts", callback_data: "nav:withdraw_amt:100" },
          { text: "250 pts", callback_data: "nav:withdraw_amt:250" },
        ],
        [
          { text: "500 pts", callback_data: "nav:withdraw_amt:500" },
          { text: "1000 pts", callback_data: "nav:withdraw_amt:1000" },
        ],
        [{ text: "2000 pts", callback_data: "nav:withdraw_amt:2000" }],
        [{ text: "⬅️ Back", callback_data: "nav:home" }],
      ],
    },
  };
...
function yesNoKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [
          { text: "✅ Confirm", callback_data: "nav:yes" },
          { text: "❌ Cancel", callback_data: "nav:no" },
        ],
      ],
    },
  };
...
function sellKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "💼 Business 1 Month — Ksh 105", callback_data: "nav:biz:1m" }],
        [{ text: "💼 Business 3 Months — Ksh 260", callback_data: "nav:biz:3m" }],
        [{ text: "⬅️ Back", callback_data: "nav:home" }],
      ],
    },
  };
...
// ===== INLINE UI HELPERS (wrappers) =====
async function showHelpScreen(chatId) {
  const msg =
    "ℹ️ *Help*\n\n" +
    "• Use *🛒 Buy Offers* to purchase.\n" +
    "• Use *💸 Withdraw Points* to withdraw points.\n" +
    "• Use *🔗 My Referral* to view your referral link.\n" +
    "• Use *🎉 WEEKLY DRAW* to join campaigns.\n\n" +
    "For support, contact admin.";
  return sendTracked(chatId, msg, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
}

async function showReferralScreen(chatId) {
  // Reuse existing text-trigger handler if available
  // This function is expected to exist in your file already; if not, we fallback.
  if (typeof renderReferralMessage === "function") {
    const text = await renderReferralMessage(chatId);
    return sendTracked(chatId, text, { ...mainMenuKeyboard(chatId) });
  }
  return sendTracked(chatId, "🔗 Referral screen is temporarily unavailable.", { ...mainMenuKeyboard(chatId) });
}

async function startBusinessPurchase(chatId, amountKsh, days) {
  const hasSaved = !!getUserPhone(chatId);
  const s = sessions.get(chatId) || {};
  pushPrev(chatId, s);
  s.step = "business_confirm";
  s.business = { amountKsh, days };
  sessions.set(chatId, s);
  ...
  const txt =
    ...
    `Plan: *${days} days*\n` +
    `Price: *Ksh ${amountKsh}*\n\n` +
    `Tap *Proceed* to pay via STK.`;
  return sendTracked(chatId, txt, { parse_mode: "Markdown", ...confirmKeyboard(hasSaved) });
}

// Weekly Draw wrappers (expected to exist from earlier additions)
async function showWeeklyDrawMain(chatId, messageId = null) {
  if (typeof renderWeeklyDrawMainInline === "function") {
    return renderWeeklyDrawMainInline(chatId, messageId);
  }
  // fallback: open the weekly draw command
  return sendTracked(chatId, "🎉 WEEKLY DRAW", { ...mainMenuKeyboard(chatId) });
}
async function showWeeklyDrawAdminPanel(chatId, messageId = null) {
  if (typeof renderWeeklyDrawAdminInline === "function") {
    return renderWeeklyDrawAdminInline(chatId, messageId);
  }
  return sendTracked(chatId, "🎛 WEEKLY DRAW ADMIN", { ...mainMenuKeyboard(chatId) });
}
// ===================== REDEEM ELIGIBILITY =====================
