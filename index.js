...
// Action/session timeout (10 minutes)
...
if (!db0.analytics.transactionsByDay || typeof db0.analytics.transactionsByDay !== "object") db0.analytics.transactionsByDay = {};
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.pendingActionAt !== \"number\") x.pendingActionAt = 0;
  }
...
function setPendingAction(chatId, obj) {
  ...
  u.pendingActionAt = obj ? Date.now() : 0;
  saveDB();
}
function getPendingAction(chatId) {
  const u = getUser(chatId);
  // Auto-expire pending actions
  const at = Number(u.pendingActionAt || 0);
  if (u.pendingAction && at && Date.now() - at > ACTION_TIMEOUT_MS) {
...
const pageHistory = new Map(); // chatId -> [{ text, opts }]
const lastPage = new Map();    // chatId -> { text, opts }

function isHomeOpts(opts) {
  return !!(opts && opts.__is_home);
}

function stripInternalOpts(opts) {
  if (!opts || typeof opts !== "object") return opts;
  const clean = { ...opts };
  delete clean.__is_home;
  delete clean.__skip_history;
  return clean;
}

function rememberPage(chatId, text, opts) {
  const key = String(chatId);
  lastPage.set(key, { text, opts: stripInternalOpts(opts) });
}

function pushPage(chatId, page) {
  if (!page || !page.opts) return;
  const key = String(chatId);
  const arr = pageHistory.get(key) || [];
  const last = arr[arr.length - 1];
  // avoid duplicate consecutive pages (by text + keyboard JSON)
  const same =
    last &&
    last.text === page.text &&
    JSON.stringify(last.opts?.reply_markup?.keyboard || null) ===
      JSON.stringify(page.opts?.reply_markup?.keyboard || null);

  if (!same) {
    arr.push(page);
    // cap history to avoid memory bloat
    while (arr.length > 20) arr.shift();
    pageHistory.set(key, arr);
  }
}

function popPage(chatId) {
  const key = String(chatId);
  const arr = pageHistory.get(key) || [];
  const page = arr.pop();
  pageHistory.set(key, arr);
...
async function renderSessionSnapshot(chatId, snap) {
  ...
  if (!snap || !snap.step) {
    ...
    return sendTracked(chatId, "✅ Main menu:", { ...mainMenuKeyboard(chatId), __is_home: true });
  }
...
async function sendTracked(chatId, text, options = {}) {
  ...
  // History: store the last page before navigating to a new one
  const skipHistory = !!options.__skip_history;
  if (!skipHistory && !isWelcomeMessage) {
    const lp = lastPage.get(key);
    if (lp) pushPage(chatId, lp);
  }
  ...
  const cleanOptions = stripInternalOpts(options);
  const sent = await bot.sendMessage(chatId, text, cleanOptions);

  // Only track non-welcome messages
  if (!isWelcomeMessage) {
    lastBotMessage.set(chatId, sent.message_id);
    rememberPage(chatId, text, options);
  }
...
function mainMenuWithPrevKeyboard(chatId = null) {
  const base = mainMenuKeyboard(chatId);
  try {
    const kb = base?.reply_markup?.keyboard;
    if (Array.isArray(kb)) {
      // Add Prev row at the bottom (NOT on home page)
      base.reply_markup.keyboard = [...kb, [PREV_BTN]];
    }
  } catch (_) {}
  return base;
}
...
bot.on("message", async (msg) => {
  ...
  // Any in-progress action/session expires after 10 minutes (secret/internal).
  if (!isAdmin(chatId) && !isStartCmd) {
    let expired = false;

    // 1) Purchase session
    const ss = sessions.get(chatId);
    if (ss && Date.now() - Number(ss.createdAt || 0) > ACTION_TIMEOUT_MS) {
      sessions.delete(chatId);
      expired = true;
    }

    // 2) Pending action (redeem flow etc.) stored in DB
    const uu = getUser(chatId);
    const pa = uu.pendingAction;
    const paAt = Number(uu.pendingActionAt || 0);
    if (pa && paAt && Date.now() - paAt > ACTION_TIMEOUT_MS) {
      uu.pendingAction = null;
      uu.pendingActionAt = 0;
      saveDB();
      expired = true;
    }

    // 3) Support mode
    const sup0 = supportState.get(String(chatId));
    const supAt = Number(sup0?.at || 0);
    if (sup0 && supAt && Date.now() - supAt > ACTION_TIMEOUT_MS) {
      supportState.delete(String(chatId));
      expired = true;
    }

    if (expired) {
      return sendTracked(chatId, "⏱️ Session expired. Back to main menu.", { ...mainMenuKeyboard(chatId) });
    }
  }
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && !isAdmin(chatId) && act.kind === "redeem") {
    ...
    if (text === PREV_BTN) {
      clearPendingAction(chatId);
      return sendTracked(chatId, "⬅ Prev.", { ...mainMenuKeyboard(chatId) });
    }
...
// ===================== GLOBAL BUTTONS =====================
  ...
  if (text === "ℹ️ Help") return sendTracked(chatId, helpText(), { ...mainMenuWithPrevKeyboard(chatId) });
...
` +
      ...
      { parse_mode: "Markdown", ...mainMenuWithPrevKeyboard(chatId) }
    );
  }
  ...
  if (text === "🎁 Redeem Points") {
    return sendTracked(chatId, "🚫 Redeem is disabled.", { ...mainMenuWithPrevKeyboard(chatId) });
  }
  ...
  // ===================== PREV (GLOBAL) =====================
if (text === PREV_BTN) {
  // Clear any active flows (but keep history)
  sessions.delete(chatId);
  supportState.delete(String(chatId));
  clearPendingAction(chatId);

  const prev = popPage(chatId);
  if (prev) {
    return sendTracked(chatId, prev.text, { ...prev.opts, __skip_history: true });
  }
  return sendTracked(chatId, "✅ Main menu:", { ...mainMenuKeyboard(chatId), __is_home: true, __skip_history: true });
}
    return sendTracked(chatId, "✅ Main menu:", { ...mainMenuKeyboard(chatId), __is_home: true });
  }
  ...
  if (text === "🔄 Resend STK") {
    ...
    try {
      ...
      return sendTracked(chatId, `⚠️ Resend failed: ${err.message}`, { ...mainMenuWithPrevKeyboard(chatId) });
    }
  }
...
if (text === "💸 Withdraw Points") {
  ...
  if (!savedPhone) {
    return sendTracked(chatId, "📱 Please save your phone number first before withdrawing.", { ...mainMenuWithPrevKeyboard(chatId) });
  }
...
// ===================== PURCHASE FLOW =====================
  ...
  if (Date.now() - (s.createdAt || Date.now()) > 10 * 60 * 1000) {
    sessions.delete(chatId);
    return sendTracked(chatId, "⏱️ Session expired. Back to main menu.", { ...mainMenuKeyboard(chatId) });
  }
