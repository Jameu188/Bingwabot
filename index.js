  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: "🖼 Add Media", callback_data: "wdadm:add_media" }, { text: "🧹 Clear Media", callback_data: "wdadm:clear_media" }],
...
const FORCE_JOIN_URL = "https://t.me/bingwadata";
const WD_BROADCAST_JOIN_KB = { reply_markup: { inline_keyboard: [[{ text: "✅ Join Channel", url: FORCE_JOIN_URL }]] } };
...
function scheduleAutoDelete(sentMsg) {
  try {
...
// Monkey-patch sendMessage + sendPhoto to auto-delete
try {
  const _sendMessage = bot.sendMessage.bind(bot);
  bot.sendMessage = async (...args) => {
    const m = await _sendMessage(...args);
    scheduleAutoDelete(m);
    return m;
  };

  const _sendPhoto = bot.sendPhoto.bind(bot);
  bot.sendPhoto = async (...args) => {
    const m = await _sendPhoto(...args);
    scheduleAutoDelete(m);
    return m;
  };
} catch (_) {}
...
${statusText}`, weeklyDrawAdminMenuKeyboard());
  ...
  if (_wdState.mode === "add_media") {
    const t = String(text || "").trim();
    if (/^done$/i.test(t)) {
      setWeeklyDrawAdminState(chatId, null);
      const statusText = formatWeeklyDrawAdminStatus();
      await editAdminStateMessage(chatId, _wdState, `✅ Media saved.
...
${statusText}`, weeklyDrawAdminMenuKeyboard());
    ...
    if (msg.photo && Array.isArray(msg.photo) && msg.photo.length) {
      ...
      await editAdminStateMessage(chatId, _wdState, "❌ Send a PHOTO or VIDEO. Send `DONE` when finished (or /cancel).", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:menu" }]] } });
      return;
    }

    c.media = Array.isArray(c.media) ? c.media : [];
    c.media.push({ type, fileId });
    // keep legacy image for single-photo fallback
    if (!c.image && type === "photo") c.image = fileId;

    try { saveDB(); } catch (_) {}
    await bot.sendMessage(chatId, `✅ Added ${type}. Total media: ${c.media.length}. Send more or type DONE.`, { parse_mode: "Markdown" }).catch(() => {});
    return;
  }
  ...
  if (_wdState.mode === "set_image") {
    // accept photo upload OR image url/file_id
    let img = "";
    if (msg.photo && Array.isArray(msg.photo) && msg.photo.length) {
      img = msg.photo[msg.photo.length - 1].file_id; // largest
    } else {
      img = String(text || "").trim();
    }

    if (!img) {
      await editAdminStateMessage(chatId, _wdState, "❌ Send a PHOTO, or paste a valid image URL (or /cancel).", { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:menu" }]] } });
      return;
    }

    // basic url validation if looks like url
    if (/^https?:\/\//i.test(img) && img.length < 12) {
      await editAdminStateMessage(chatId, _wdState, "❌ Invalid URL. Send a full image URL or a photo (or /cancel).", { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:menu" }]] } });
      return;
    }
    ...
    if (!/^https?:\/\//i.test(img)) {
      c.media = Array.isArray(c.media) ? c.media : [];
      // if setting a new image, replace existing single-photo media
      c.media = [{ type: "photo", fileId: img }];
    }
    try { saveDB(); } catch (_) {}
    setWeeklyDrawAdminState(chatId, null);
    const statusText = formatWeeklyDrawAdminStatus();
  ...
  if (_wdState.mode === "set_prizes") {
    const nums = String(text || "").trim().split(/[^0-9]+/).filter(Boolean).map(Number);
    if (nums.length < 3 || nums.some(n => !Number.isFinite(n) || n < 0)) {
      await editAdminStateMessage(chatId, _wdState, "❌ Invalid prizes. Send like: 250 150 100 (or /cancel).", { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:menu" }]] } });
      return;
    }
    const [p1,p2,p3]=nums;
    if (p1+p2+p3 > 500) {
      await editAdminStateMessage(chatId, _wdState, "❌ Total prizes must be ≤ 500. Send again (or /cancel).", { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:menu" }]] } });
      return;
    }
    c.prizes = { first: Math.floor(p1), second: Math.floor(p2), third: Math.floor(p3) };
    try { saveDB(); } catch (_) {}
    setWeeklyDrawAdminState(chatId, null);
    const statusText = formatWeeklyDrawAdminStatus();
  ...
  if (_wdState.mode === "set_period") {
    ...
    // Option A: explicit date range (YYYY-MM-DD YYYY-MM-DD)
    const parts = raw.split(/\s+/).filter(Boolean);
    ...
    // Option B: duration string (starts now)
    const durMs = parseDurationMs(raw);
    if (durMs && durMs >= 60 * 1000) {
      const now = Date.now();
      c.startAt = now;
      c.endAt = now + durMs;
      c.status = c.status === "OFF" ? "RUNNING" : c.status;
      ...
      setWeeklyDrawAdminState(chatId, null);
      const statusText = formatWeeklyDrawAdminStatus();
      await editAdminStateMessage(chatId, _wdState, `✅ Period updated using duration: ${raw}\n\n${statusText}`, weeklyDrawAdminMenuKeyboard());
      return;
    }

    await editAdminStateMessage(chatId, _wdState,
      "❌ Invalid period. Send either:\n• Dates: 2026-03-01 2026-03-08\n• Duration: 5d, 1w, 2d 1h, 1mo, 1yr (or just 5 = 5 days)\n• Range: 5,6 (uses max)\n\n(or /cancel)",
      { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:menu" }]] } }
    );
    return;
  }
  ...
  if (_wdState.mode === "set_qual") {
    ...
    if (!res || res.ok !== true) {
      const err = res?.err || "Invalid value.";
      await editAdminStateMessage(chatId, _wdState, `❌ ${err}\n\n${qualificationPromptFor(key)}\n(or /cancel)`, { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "wdadm:quals" }]] } });
      return;
    }
    try { saveDB(); } catch (_) {}
    setWeeklyDrawAdminState(chatId, null);
    const statusText = formatWeeklyDrawAdminStatus();
    await editAdminStateMessage(chatId, _wdState, `✅ Qualification updated.\n\n${statusText}`, weeklyDrawAdminMenuKeyboard());
    return;
  }
}
...
// ===== WEEKLY DRAW: WINNER payout claim flow =====
if (act && act.type === "wd_claim") {
  const step = String(act.step || "phone");

  // allow user to cancel
  if (/^\s*\/cancel\s*$/i.test(text) || text === "❌ Cancel") {
    clearPendingAction(chatId);
    await bot.sendMessage(chatId, "❌ Cancelled payout submission.", mainMenuKeyboard(chatId)).catch(()=>{});
    return;
  }
  ...
  if (!rec) {
    clearPendingAction(chatId);
    await bot.sendMessage(chatId, "❌ You are not among the winners. Submission declined.", mainMenuKeyboard(chatId)).catch(()=>{});
    return;
  }
  if (rec.paid === true) {
    clearPendingAction(chatId);
    await bot.sendMessage(chatId, "✅ Your payout is already marked as PAID.", mainMenuKeyboard(chatId)).catch(()=>{});
    return;
  }
  if (rec.payoutSubmitted === true) {
    clearPendingAction(chatId);
    await bot.sendMessage(chatId, "✅ You already submitted payout details. Please wait for payment.", mainMenuKeyboard(chatId)).catch(()=>{});
    return;
  }
  ...
  if (step === "phone") {
    ...
    if (!phone254) {
      await bot.sendMessage(chatId, "❌ Invalid number. Send Safaricom M-PESA number like 07XXXXXXXX / 01XXXXXXXX / 2547XXXXXXXX / 2541XXXXXXXX.").catch(()=>{});
      return;
    }
    act.phone254 = phone254;
    act.step = "name";
    setPendingAction(chatId, act);

    await bot.sendMessage(chatId, `✅ Number saved: ${phone254}\n\nNow send the M-PESA account name (as it appears on M-PESA).`).catch(()=>{});
    return;
  }
  ...
  if (step === "name") {
    ...
    if (!name || name.length < 3) {
      await bot.sendMessage(chatId, "❌ Send a valid M-PESA name (at least 3 characters).").catch(()=>{});
      return;
    }
    act.mpesaName = name;
    act.step = "confirm";
    ...
    await bot.sendMessage(
      chatId,
      `⚠️ Please confirm payout details:\n\nM-PESA Number: ${act.phone254}\nM-PESA Name: ${act.mpesaName}\n\nTap ✅ Confirm to submit or ❌ Cancel.`,
      yesNoKeyboard()
    ).catch(()=>{});
    return;
  }
...
// 🚫 Anti-fraud: block same M-PESA number used by multiple winners
try {
  const c0 = ensureWeeklyDrawObject();
  const phone = String(act.phone254 || "");
  const isDup = (arr) => Array.isArray(arr) && arr.some(x => x && String(x.chatId) !== String(chatId) && String(x.payoutPhone || "") === phone);
  if (phone && (isDup(c0.winners) || isDup(c0.lastEnded?.winners))) {
    await bot.sendMessage(chatId, "❌ This M-PESA number is already used by another winner. Please contact admin.", mainMenuKeyboard(chatId)).catch(()=>{});
    clearPendingAction(chatId);
    return;
  }
} catch (_) {}

      // Save to winner record (both current and lastEnded if exists)
      rec.payoutSubmitted = true;
      rec.payoutPhone = act.phone254;
      rec.payoutName = act.mpesaName;
      rec.payoutSubmittedAt = Date.now();

      const c = ensureWeeklyDrawObject();
      if (c.lastEnded && Array.isArray(c.lastEnded.winners)) {
        const w2 = c.lastEnded.winners.find(x => String(x.chatId) === String(chatId));
        if (w2) {
          w2.payoutSubmitted = true;
          w2.payoutPhone = act.phone254;
          w2.payoutName = act.mpesaName;
          w2.payoutSubmittedAt = Date.now();
        }
      }
      try { saveDB(); } catch (_) {}
      ...
      try {
        const winName = (rec.name || safeDisplayName(getUser(chatId), chatId) || String(chatId));
        const prizeAmt = Number(rec.prize || 0);
        await bot.sendMessage(
          String(ADMIN_ID),
          `📥 Weekly Draw Payout Details\n\nWinner: ${winName}\nChatId: ${chatId}\nPrize: Ksh ${prizeAmt}\nM-PESA Number: ${act.phone254}\nM-PESA Name: ${act.mpesaName}\nStatus: ⏳ Pending payment`
        ).catch(()=>{});
      } catch (_) {}

      // Update inline keyboard to hide submit button
      try {
        if (act.msgChatId && act.msgId) {
          const caption = buildWeeklyDrawMainCaption(chatId);
          await bot.editMessageText(caption, {
            chat_id: act.msgChatId,
            message_id: act.msgId,
            ...weeklyDrawUserMenuKeyboard(chatId, "menu"),
          }).catch(async () => {
            await bot.editMessageReplyMarkup(
              weeklyDrawUserMenuKeyboard(chatId, "menu").reply_markup,
      ...
      clearPendingAction(chatId);
      await bot.sendMessage(chatId, "✅ Submitted! Your payout details were sent to admin. Please wait for payment.", mainMenuKeyboard(chatId)).catch(()=>{});
      return;
    }
...
// 🎉 WEEKLY DRAW button (robust matching: emoji/spacing/case)
if (/\bweekly\s*draw\b/i.test(text)) {
...
Total earned: Ksh ${Number(totalEarned || 0).toFixed(2)}

Rule: You earn when your referred user completes a successful purchase.`,
            { ...mainMenuKeyboard(chatId) } // ✅ no parse_mode (plain text; avoids Telegram formatting errors)
          );
}
