...
try {
  ...
  bot.sendMessage = (chatId, text, options = {}) => _sendMessage(chatId, translateOutgoing(chatId, text), options);

  const _editMessageText = bot.editMessageText.bind(bot);
  bot.editMessageText = (text, options = {}) => {
    const cid = options && (options.chat_id || (options.chatId));
    return _editMessageText(translateOutgoing(cid, text), options);
  };
...
case "settings":
      return tr(chatId, `⚙️ *Welcome to Settings*
...
${baseSupport}`, `⚙️ *Karibu kwenye Mipangilio*

Washa/Zima chaguo hapa chini ili kubadilisha jinsi bot inavyofanya kazi:

• *Ununuzi Haraka* – Nunua data kwa haraka bila hatua nyingi za uthibitisho.
• *Rudia STK Kiotomatiki* – Huonyesha upya STK push ikiwa malipo yameshindwa au yamechelewa.
• *Zima Arifa* – Zima arifa za kampeni, cashback na matangazo.
• *Futa kiotomatiki (Saa 24)* – Ujumbe wa bot utafutwa baada ya saa 24.
• *Ficha Vitufe Kiotomatiki* – Huficha vitufe baada ya matumizi ili chat ibaki safi.
• *Ficha Vitendo vya Haraka* – Huondoa Quick Actions kwenye menyu kuu.
...
function translateOutgoing(chatId, text) {
  ...
  const repl = [
    ...
    ["🎛 WEEKLY DRAW ADMIN", "🎛 ADMIN WA DROO"],
...
async function sendTracked(chatId, text, options = {}) {
  ...
  const sent = await bot.sendMessage(chatId, translateOutgoing(chatId, text), options);
