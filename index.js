...
const PAYHERO_CHANNEL_ID_BUSINESS = Number(process.env.PAYHERO_CHANNEL_ID_BUSINESS || PAYHERO_CHANNEL_ID_DATA || 2486); // Business subscription channel (env override supported)
...
if (data === "mp:toggle") {
  ...
  await bot.answerCallbackQuery(q.id, {
    text: u.multiPurchaseEnabled
      ? "✅ Multi Purchase ON"
      : "⬜️ Multi Purchase OFF"
  }).catch(() => {});

  try {
    const mid = q.message?.message_id;
    if (mid) {
      await bot.editMessageReplyMarkup(fastPurchaseHelpKeyboard(chatId).reply_markup, {
        chat_id: chatId,
        message_id: mid,
      }).catch(() => {});
    }
...
function buildCartSummaryText(chatId, s) {
  ...
  lines.push("");
  lines.push("Choose: Proceed, Edit Numbers, Add More, or Clear Cart.");
  return lines.join("\n");
}
...
try { adminNotify(`⚠️ Business STK failed\nChatID: ${chatId}\nPhone: ${phone254}\nPlan: ${plan.label}\nAmount: Ksh ${plan.price}\nChannel: ${PAYHERO_CHANNEL_ID_BUSINESS}\nError: ${errMsg}`); } catch (_) {}
await sendTracked(
  chatId,
  `❌ Failed to send STK. ${mdEscape(errMsg)}\n\nHelp: ${HELP_PHONE}`,
  { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
);
  }
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && act.type === "sell_business") {
    ...
    try {
      ...
      const errMsg = String(e?.message || e || "Unknown error");
      try { adminNotify(`⚠️ Business STK failed\nChatID: ${chatId}\nPhone: ${phone254}\nPlan: ${plan.label}\nAmount: Ksh ${plan.price}\nChannel: ${PAYHERO_CHANNEL_ID_BUSINESS}\nError: ${errMsg}`); } catch (_) {}
      await sendTracked(chatId, `❌ STK failed: ${mdEscape(errMsg)}\n\nHelp: ${HELP_PHONE}`, {
        parse_mode: "Markdown",
        ...mainMenuKeyboard(chatId),
      });
      return;
...
if (s.step === "cart_review") {
  ...
  if (text === "🗑️ Clear Cart") {
    ...
    s.pkgKey = null;
    sessions.set(chatId, s);
    return sendTracked(chatId, "🗑️ Cart cleared. Choose a category:", categoriesKeyboard(chatId));
  }
