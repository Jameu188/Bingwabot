...
function applyUserBackup(chatId, payload) {
  ...
  if (String(payload.chatId) !== cid) throw new Error("Backup does not belong to this account");
...
if (text === "/restorebackup" || text === "/restore") {
  ...
  saveDB();
  await bot.sendMessage(chatId, "📤 Send the backup JSON file now (the one you received from /mybackup).").catch(()=>{});
  return;
}
...
try {
  ...
  if (u?.pendingAction?.type === "await_restore_backup" && msg.document) {
    ...
    await bot.sendMessage(chatId, "⏳ Restoring your backup...").catch(()=>{});
    ...
    await bot.sendMessage(chatId, "✅ Backup restored successfully.").catch(()=>{});
    return;
  }
} catch (e) {
