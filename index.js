...
if (isUserCancelled) {
  ...
  await sendTracked(
    ...
    `❌ Payment cancelled at ${when}.\n` +
      (details ? `${details}\n` : "") +
      `Payment Number: ${cbPhone ? formatTo07(cbPhone) : (pending?.phone254 ? formatTo07(pending.phone254) : "N/A")}\n` +
      `Receive Number: ${pending?.phone254 ? formatTo07(pending.phone254) : (cbPhone ? formatTo07(cbPhone) : "N/A")}\n` +
      `Receipt: ${mpesaOut || "N/A"}\n` +
...
Help: ${HELP_PHONE}`;
  ...
  const paymentNumber = String(cbPhone || pending?.phone254 || "").trim();
  const receiveNumber = String(pending?.phone254 || cbPhone || "").trim();
  const receiptNumber = String(mpesaOut || "N/A").trim() || "N/A";
...
` +
      `📱 Payment Number: ${paymentNumber ? formatTo07(paymentNumber) : "N/A"}
` +
      `📥 Receive Number: ${receiveNumber ? formatTo07(receiveNumber) : "N/A"}
` +
...
` +
      `📱 Namba ya Malipo: ${paymentNumber ? formatTo07(paymentNumber) : "N/A"}
` +
      `📥 Namba ya Kupokea: ${receiveNumber ? formatTo07(receiveNumber) : "N/A"}
` +
...
` +
      `📦 Package: *${packageName}*
` +
      `📱 Payment Number: *${mdEscape(paymentNumber ? formatTo07(paymentNumber) : "N/A")}*
` +
      `📥 Receive Number: *${mdEscape(receiveNumber ? formatTo07(receiveNumber) : "N/A")}*
` +
      `🧾 Receipt: *${mdEscape(receiptNumber)}*
` +
