...
// ===================== TRUST SCORE SYSTEM =====================
function increaseTrust(chatId, amount = 2) {
  const u = getUser(chatId);
  u.trustScore = Number(u.trustScore || 0) + amount;
  saveDB();
}

function decreaseTrust(chatId, amount = 5) {
  const u = getUser(chatId);
  u.trustScore = Math.max(0, Number(u.trustScore || 0) - amount);
  u.lastViolationAt = Date.now();
  saveDB();
}

function getWithdrawDelayByTrust(u) {
  const trust = Number(u.trustScore || 0);

  if (trust >= 50) return 10 * 60 * 1000;     // 10 mins
  if (trust >= 20) return 20 * 60 * 1000;     // 20 mins
  return 30 * 60 * 1000;                      // default 30 mins
}



// ===================== GLOBAL DAILY WITHDRAW CAP =====================
const DAILY_GLOBAL_WITHDRAW_CAP_KES = 5000;

function kenyaTodayString() {
  return new Date().toLocaleDateString("en-CA", { timeZone: "Africa/Nairobi" });
}

function ensureGlobalWithdrawStats() {
  if (!db.globalWithdrawStats) {
    db.globalWithdrawStats = {
      date: kenyaTodayString(),
      totalKesToday: 0
    };
  }

  const today = kenyaTodayString();
  if (db.globalWithdrawStats.date !== today) {
    db.globalWithdrawStats.date = today;
    db.globalWithdrawStats.totalKesToday = 0;
    saveDB();
  }
}

function canProcessGlobalWithdraw(amountKes) {
  ensureGlobalWithdrawStats();
  return (db.globalWithdrawStats.totalKesToday + amountKes) <= DAILY_GLOBAL_WITHDRAW_CAP_KES;
}

function recordGlobalWithdraw(amountKes) {
  ensureGlobalWithdrawStats();
  db.globalWithdrawStats.totalKesToday += amountKes;
  saveDB();
}



// ===================== DAILY PHONE PURCHASE PROTECTION =====================
function kenyaToday() {
  const now = new Date();
  return now.toLocaleDateString("en-CA", { timeZone: "Africa/Nairobi" });
}

function isPhoneBlockedToday(phone07, chatId) {
  if (!db.dailyPhonePurchases) db.dailyPhonePurchases = {};
  const today = kenyaToday();
  const entry = db.dailyPhonePurchases[phone07];
  if (!entry) return false;
  if (entry.date === today && String(entry.chatId) !== String(chatId)) {
    return true;
  }
  return false;
}

function recordPhonePurchaseToday(phone07, chatId) {
  if (!db.dailyPhonePurchases) db.dailyPhonePurchases = {};
  db.dailyPhonePurchases[phone07] = {
...
function trackUserAction(chatId) {
  const u = getUser(chatId);
  const now = Date.now();

  if (!Array.isArray(u.actionTimestamps)) u.actionTimestamps = [];
  u.actionTimestamps.push(now);

  // keep only last 60 seconds
  u.actionTimestamps = u.actionTimestamps.filter(t => now - Number(t || 0) < 60000);

  if (u.actionTimestamps.length >= 10) {
    u.cooldownUntil = now + (5 * 60 * 1000); // 5 min cooldown
    decreaseTrust(chatId, 5);
      u.riskScore = Number(u.riskScore || 0) + 10;
    u.actionTimestamps = [];
    saveDB();
  }
}
  ...
  try {
    if (!fs.existsSync(DB_FILE)) {
      const init = {
        ...
        processedPayments: {},
        stats: { totalPurchases: 0, totalBroadcasts: 0, totalKesWithdrawn: 0, totalWithdrawals: 0 },
      };
...
function repairDB(obj) {
  ...
  if (typeof db0.stats.totalKesWithdrawn !== "number") db0.stats.totalKesWithdrawn = 0;
  if (typeof db0.stats.totalWithdrawals !== "number") db0.stats.totalWithdrawals = 0;
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    // ✅ Anti-fraud + withdraw history fields
    if (typeof x.createdAt !== "number") x.createdAt = Date.now();
    if (typeof x.phoneLastChangedAt !== "number") x.phoneLastChangedAt = 0;
    if (!Array.isArray(x.failedPayments)) x.failedPayments = [];
    if (typeof x.withdrawBlockedUntil !== "number") x.withdrawBlockedUntil = 0;
    if (typeof x.riskScore !== "number") x.riskScore = 0;
    if (!Array.isArray(x.withdrawHistory)) x.withdrawHistory = [];
    if (typeof x.totalKesWithdrawn !== "number") x.totalKesWithdrawn = 0;
    if (typeof x.totalWithdrawals !== "number") x.totalWithdrawals = 0;
    if (typeof x.frozenUntil !== "number") x.frozenUntil = 0;
    if (typeof x.freezeNotifiedAt !== "number") x.freezeNotifiedAt = 0;
    if (typeof x.lastRiskDecayAt !== "number") x.lastRiskDecayAt = 0;
    if (typeof x.lastViolationAt !== "number") x.lastViolationAt = 0;
    if (typeof x.trustScore !== "number") x.trustScore = 0;
    if (!Array.isArray(x.stkFailAttempts)) x.stkFailAttempts = [];
    if (typeof x.stkCooldownUntil !== "number") x.stkCooldownUntil = 0;
    if (typeof x.stkEscalationLevel !== "number") x.stkEscalationLevel = 0;
    if (typeof x.privilegedBypass !== "boolean") x.privilegedBypass = false;
    if (typeof x.withdrawCapacityPts !== "number") x.withdrawCapacityPts = 0;
    if (typeof x.totalWithdrawnPts !== "number") x.totalWithdrawnPts = 0;
    if (typeof x.pendingWithdrawReleaseAt !== "number") x.pendingWithdrawReleaseAt = 0;
    if (typeof x.lastReferralRewardAt !== "number") x.lastReferralRewardAt = 0;
    if (!Array.isArray(x.actionTimestamps)) x.actionTimestamps = [];
    if (typeof x.cooldownUntil !== "number") x.cooldownUntil = 0;
...
function getUser(chatId) {
  ...
  if (typeof db.users[cid].createdAt !== "number") db.users[cid].createdAt = Date.now();
  if (typeof db.users[cid].phoneLastChangedAt !== "number") db.users[cid].phoneLastChangedAt = 0;
  if (!Array.isArray(db.users[cid].failedPayments)) db.users[cid].failedPayments = [];
  if (typeof db.users[cid].withdrawBlockedUntil !== "number") db.users[cid].withdrawBlockedUntil = 0;
  if (typeof db.users[cid].riskScore !== "number") db.users[cid].riskScore = 0;
  if (!Array.isArray(db.users[cid].withdrawHistory)) db.users[cid].withdrawHistory = [];
  if (typeof db.users[cid].totalKesWithdrawn !== "number") db.users[cid].totalKesWithdrawn = 0;
  if (typeof db.users[cid].totalWithdrawals !== "number") db.users[cid].totalWithdrawals = 0;
  if (typeof db.users[cid].frozenUntil !== "number") db.users[cid].frozenUntil = 0;
  if (typeof db.users[cid].freezeNotifiedAt !== "number") db.users[cid].freezeNotifiedAt = 0;
  if (typeof db.users[cid].lastRiskDecayAt !== "number") db.users[cid].lastRiskDecayAt = Date.now();
  if (typeof db.users[cid].lastViolationAt !== "number") db.users[cid].lastViolationAt = 0;
  if (!Array.isArray(db.users[cid].stkFailAttempts)) db.users[cid].stkFailAttempts = [];
  if (typeof db.users[cid].stkCooldownUntil !== "number") db.users[cid].stkCooldownUntil = 0;
  if (typeof db.users[cid].stkEscalationLevel !== "number") db.users[cid].stkEscalationLevel = 0;
  return db.users[cid];
}
...
function setUserPhone(chatId, phone254) {
  const u = getUser(chatId);
  const prev = String(u.phone || "");
  const next = String(phone254 || "");
  if (prev !== next) {
    u.phoneLastChangedAt = Date.now();
  }
  u.phone = next;
  saveDB();
}
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (!data.startsWith("redeem_") && !data.startsWith("withdraw_")) return;
...
// --------------------- WITHDRAW INLINE ACTIONS ---------------------
if (action === "withdraw_accept") {
  // schedule release after trust-based delay (default 30m)
  req.status = "accepted";
  db.pendingRedeems[String(chatKey)] = req;
  saveDB();

  const uu = getUser(targetChatId);
  const delay = (typeof getWithdrawDelayByTrust === "function") ? getWithdrawDelayByTrust(uu) : (30 * 60 * 1000);

  // Global cap check (unless bypass)
  const isBypassT = isAdmin(chatId) || !!uu.privilegedBypass;
  const amountPts = Number(req?.amount || 0);
  const amountKes = Math.floor((amountPts / 100) * 50);

  if (!isBypassT && typeof canProcessGlobalWithdraw === "function" && !canProcessGlobalWithdraw(amountKes)) {
    await bot.answerCallbackQuery(q.id, { text: "Daily payout cap reached.", show_alert: true }).catch(() => {});
    return;
  }
  if (typeof recordGlobalWithdraw === "function") {
    recordGlobalWithdraw(amountKes);
  }

  uu.pendingWithdrawReleaseAt = Date.now() + delay;
  uu.totalWithdrawnPts = Number(uu.totalWithdrawnPts || 0) + amountPts;
  saveDB();

  clearPendingRedeem(targetChatId);
  clearPendingAction(targetChatId);

  await bot.answerCallbackQuery(q.id, { text: "Approved ✅" }).catch(() => {});
...
if (action === "withdraw_decline") {
  ...
  // refund points
  const amountPts = Number(req?.amount || 0);
  addPoints(targetChatId, amountPts);

  clearPendingRedeem(targetChatId);
  clearPendingAction(targetChatId);

  await bot.answerCallbackQuery(q.id, { text: "Declined ❌" }).catch(() => {});
  await bot.sendMessage(
    targetChatId,
...
const RISK_DECAY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const RISK_DECAY_AMOUNT = 5;
const RISK_GOOD_BEHAVIOR_DECAY = 10; // extra decay on successful payment

const RISK_RESET_DAYS = 30;
const RISK_RESET_MS = RISK_RESET_DAYS * 24 * 60 * 60 * 1000;

// STK escalation window + cooldown
const STK_FAIL_WINDOW_MS = 30 * 60 * 1000; // 30 minutes
const STK_ESCALATION_THRESHOLD = 10;
const STK_ESCALATION_BASE_MS = 3 * 60 * 60 * 1000; // 3 hours
// Risk acceleration during spammy STK fails
const STK_RISK_ACCEL_START = 5;       // after 5 fails in window, start adding extra risk
const STK_RISK_ACCEL_PER_FAIL = 2;    // extra risk per fail after start
const STK_RISK_ESCALATION_BONUS = 15; // extra risk when escalation triggers

// Freeze only withdraw + redeem (STK still allowed)
const AUTO_FREEZE_MS = 7 * 24 * 60 * 60 * 1000; // 7 days

function isFrozen(u) {
  const until = Number(u?.frozenUntil || 0);
  return until && Date.now() < until;
}

function freezeRemainingDays(u) {
  const until = Number(u?.frozenUntil || 0);
  const rem = until - Date.now();
  return rem > 0 ? Math.ceil(rem / (24 * 60 * 60 * 1000)) : 0;
}

function autoFreezeIfHighRisk(chatId, reason) {
  const u = getUser(chatId);
  if (!isHighRisk(u)) return false;

  const now = Date.now();
  if (!u.frozenUntil || now >= Number(u.frozenUntil || 0)) {
    u.frozenUntil = now + AUTO_FREEZE_MS;
  }

  // notify admin at most once per hour per user
  const lastN = Number(u.freezeNotifiedAt || 0);
  if (!lastN || now - lastN > 60 * 60 * 1000) {
    u.freezeNotifiedAt = now;
    saveDB();
    fraudAlert("AUTO-FREEZE (High Risk)", chatId, reason || "RiskScore >= threshold");
  } else {
    saveDB();
  }
  return true;
...
function rewardGoodBehavior(chatId) {
  const u = getUser(chatId);
  u.riskScore = Math.max(0, Number(u.riskScore || 0) - RISK_GOOD_BEHAVIOR_DECAY);
  // Successful payment clears current STK fail window
  u.stkFailAttempts = [];
  saveDB();
}

function applyRiskDecay(chatId) {
  const u = getUser(chatId);
  const now = Date.now();
  if (!u.lastRiskDecayAt) {
    u.lastRiskDecayAt = now;
    saveDB();
    return;
  }
  const elapsed = now - u.lastRiskDecayAt;
  if (elapsed >= RISK_DECAY_INTERVAL_MS) {
    const cycles = Math.floor(elapsed / RISK_DECAY_INTERVAL_MS);
    const reduction = cycles * RISK_DECAY_AMOUNT;
    u.riskScore = Math.max(0, Number(u.riskScore || 0) - reduction);
    u.lastRiskDecayAt = now;
    saveDB();
  }
}

function checkRiskReset(chatId) {
  const u = getUser(chatId);
  const now = Date.now();
  const lv = Number(u.lastViolationAt || 0);
  if (!lv) return;
  if (now - lv >= RISK_RESET_MS) {
    u.riskScore = 0;
    u.failedPayments = [];
    u.withdrawBlockedUntil = 0;
    u.stkFailAttempts = [];
    u.stkCooldownUntil = 0;
    u.stkEscalationLevel = 0;
    saveDB();
  }
}

function applyStkFailWindowAndMaybeEscalate(chatId) {
  const u = getUser(chatId);
  const now = Date.now();
  if (!Array.isArray(u.stkFailAttempts)) u.stkFailAttempts = [];
  u.stkFailAttempts.push(now);
  u.stkFailAttempts = u.stkFailAttempts.filter((t) => now - Number(t || 0) < STK_FAIL_WINDOW_MS);

  const failCount = u.stkFailAttempts.length;
  if (failCount >= STK_RISK_ACCEL_START) {
    const extraRisk = (failCount - STK_RISK_ACCEL_START + 1) * STK_RISK_ACCEL_PER_FAIL;
    decreaseTrust(chatId, 5);
      u.riskScore = Number(u.riskScore || 0) + extraRisk;
    u.lastViolationAt = now;
  }

  if (failCount >= STK_ESCALATION_THRESHOLD) {
    u.stkEscalationLevel = Number(u.stkEscalationLevel || 0) + 1;
    const extra = u.stkEscalationLevel * STK_ESCALATION_BASE_MS; // 3h, 6h, 9h...
    u.stkCooldownUntil = now + extra;
    u.stkFailAttempts = []; // reset after escalation
    decreaseTrust(chatId, 5);
      u.riskScore = Number(u.riskScore || 0) + STK_RISK_ESCALATION_BONUS;
    u.lastViolationAt = now;
    saveDB();
    fraudAlert("STK Escalation Cooldown", chatId, `Cooldown: ${Math.ceil(extra / 3600000)} hrs (repeated failed STK)`);
    autoFreezeIfHighRisk(chatId, "High STK failure rate escalation triggered");
    return;
  }
...
const WITHDRAW_MIN_SPEND_KES = 200; // must spend at least this before first withdraw
const WITHDRAW_ONCE_PER_DAY_MS = 24 * 60 * 60 * 1000;
const PHONE_CHANGE_WITHDRAW_LOCK_MS = 24 * 60 * 60 * 1000;
const FAILED_PAY_WINDOW_MS = 2 * 60 * 1000; // 2 minutes
const FAILED_PAY_BLOCK_MS = 30 * 60 * 1000; // 30 minutes
const ACCOUNT_AGE_WITHDRAW_LOCK_MS = 24 * 60 * 60 * 1000; // 1 day // <2 days => no withdraw (day 3+)
const HIGH_RISK_SCORE_BLOCK = 50;

function isHighRisk(u) {
  return Number(u?.riskScore || 0) >= HIGH_RISK_SCORE_BLOCK;
}

function isTemporarilyBlocked(u) {
  const until = Number(u?.withdrawBlockedUntil || 0);
  return until && Date.now() < until;
}

function tempBlockRemainingMin(u) {
  const until = Number(u?.withdrawBlockedUntil || 0);
  const rem = until - Date.now();
  return rem > 0 ? Math.ceil(rem / 60000) : 0;
}

function recordFailedPaymentAndMaybeBlock(chatId) {
  const u = getUser(chatId);
  const now = Date.now();
  if (!Array.isArray(u.failedPayments)) u.failedPayments = [];
  u.failedPayments.push(now);
  // keep last 2 minutes
  u.failedPayments = u.failedPayments.filter((t) => now - Number(t || 0) < FAILED_PAY_WINDOW_MS);

  if (u.failedPayments.length >= 3) {
    u.withdrawBlockedUntil = now + FAILED_PAY_BLOCK_MS;
    decreaseTrust(chatId, 5);
      u.riskScore = Number(u.riskScore || 0) + 20;
    // alert admin on temporary block
    fraudAlert("Temp block: 3 failed payments/2min", chatId, "Blocked 30 minutes");
  }
  saveDB();
}

function addWithdrawHistory(chatId, pts, phone07) {
  const u = getUser(chatId);
  if (!Array.isArray(u.withdrawHistory)) u.withdrawHistory = [];
  const kes = pointsToKes(pts);
  u.withdrawHistory.push({
    time: Date.now(),
    pts: Number(pts || 0),
    kes: Number(kes || 0),
    mpesa: String(phone07 || ""),
  });
  // keep last 50
  if (u.withdrawHistory.length > 50) u.withdrawHistory = u.withdrawHistory.slice(-50);

  u.totalKesWithdrawn = Number(u.totalKesWithdrawn || 0) + Number(kes || 0);
  u.totalWithdrawals = Number(u.totalWithdrawals || 0) + 1;

  db.stats.totalKesWithdrawn = Number(db.stats.totalKesWithdrawn || 0) + Number(kes || 0);
  db.stats.totalWithdrawals = Number(db.stats.totalWithdrawals || 0) + 1;
...
// ✅ Admin fraud alerts (Markdown-safe)
async function fraudAlert(title, chatId, details) {
  try {
    const u = getUser(chatId);
    ...
    const msg =
      ...
      (phone ? `Phone: *${mdEscape(phone)}*\n` : "") +
      `RiskScore: *${mdEscape(risk)}*\n` +
      (details ? `${mdEscape(details)}\n` : "") +
...
function mainMenuKeyboard() {
  return {
    reply_markup: {
      keyboard: [["🛒 Buy Offers"], ["💸 Withdraw Points"], ["🔗 My Referral"], ["🔄 Resend STK"], ["ℹ️ Help", "❌ Cancel"]],
...
function confirmKeyboard(hasSavedPhone) {
  const rows = [];
  if (hasSavedPhone) rows.push(["✅ Proceed (use saved number)", "📞 Change Number"]);
  rows.push(["⬅ Back", "❌ Cancel"]);
  return { reply_markup: { keyboard: rows, resize_keyboard: true, one_time_keyboard: false } };
}
...
function maybeRewardInviterOnSuccessPurchase(buyerChatId, amountKsh, when) {
  ...
  // ✅ Anti-fraud: only reward referral after buyer has spent at least KES 50 total
  if (Number(buyer.totalSpentKsh || 0) < 50) return;
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    if (!isSuccess) {
      ...
      // ✅ Anti-fraud: track failed payments and block after 3 fails in 2 minutes
      recordFailedPaymentAndMaybeBlock(chatId);
...
bot.on("message", async (msg) => {
  ...
  const uSpam = getUser(chatId);
  if (!isAdmin(chatId) && !uSpam.privilegedBypass && uSpam.cooldownUntil && Date.now() < uSpam.cooldownUntil &&
      !isAdmin(chatId) && !uSpam.privilegedBypass) {
    return sendTracked(chatId,
      "⏳ Too many actions. Please slow down and try again shortly.",
      { ...mainMenuKeyboard() }
    );
  }

  trackUserAction(chatId);
  applyRiskDecay(chatId);
  checkRiskReset(chatId);
...
if (!isBypass && (isFrozen(uWR) || isHighRisk(uWR))) {
  autoFreezeIfHighRisk(chatId, "User attempted withdraw/redeem while high risk");
  const d = freezeRemainingDays(uWR);
  fraudAlert("Blocked action: withdraw/redeem frozen", chatId, `Remaining: ${d} day(s)`);
  return sendTracked(chatId, `🚫 Withdrawals & Redeems frozen for security review. Try again in ${d} day(s) or contact support.`, { ...mainMenuKeyboard() });
}
...
// ===================== PRIVILEGED BYPASS COMMANDS =====================
// /pb <chatId>   -> grant bypass
// /upb <chatId>  -> remove bypass

const pb = text.match(/^\/pb\s+(\d+)$/i);
if (pb && isAdmin(chatId)) {
  const targetId = pb[1];
  const u = getUser(targetId);
  u.privilegedBypass = true;
  saveDB();
  await bot.sendMessage(Number(targetId),
    "✅ You have been granted privileged bypass. All limitations are disabled for your account.",
    { ...mainMenuKeyboard() }
  ).catch(() => {});
  return sendTracked(chatId, `✅ Bypass granted to ${targetId}`);
}

const upb = text.match(/^\/upb\s+(\d+)$/i);
if (upb && isAdmin(chatId)) {
  const targetId = upb[1];
  const u = getUser(targetId);
  u.privilegedBypass = false;
  saveDB();
  await bot.sendMessage(Number(targetId),
    "⚠️ Your privileged bypass has been removed.",
    { ...mainMenuKeyboard() }
  ).catch(() => {});
  return sendTracked(chatId, `✅ Bypass removed from ${targetId}`);
}
...
const apm = text.match(/^\/addpoints\s+(\d+)\s+([0-9.]+)$/i);
      ...
      if (/^\/stats$/i.test(text)) {
        ...
        const out =
          ...
          `Total broadcasts: *${Number(db.stats.totalBroadcasts || 0)}*\nTotal withdrawals: *${Number(db.stats.totalWithdrawals || 0)}*\nTotal withdrawn (KES): *${Number(db.stats.totalKesWithdrawn || 0).toFixed(2)}*\n\n` +
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && !isAdmin(chatId) && act.kind === "redeem") {
    ...
    if (rem > 0) {
      const mins = Math.ceil(rem / 60000);
      clearPendingAction(chatId);
      return sendTracked(chatId, `⏳ Please wait ${mins} minute(s) before sending another redeem request.`, { ...mainMenuKeyboard() });
    }
    ...
    if (act.step === "confirm") {
      if (text === "✅ Confirm") {
        ...
        if (savedPhone) {
          ...
          if (hasPendingRedeem(chatId)) {
            clearPendingAction(chatId);
            return sendTracked(chatId, "⏳ You already have a pending redeem request. Please wait for admin approval.", { ...mainMenuKeyboard() });
          }
          ...
          try {
            ...
            clearPendingRedeem(chatId);
            return sendTracked(chatId, `⚠️ Failed to submit redeem request. Points refunded: ${cost} pts`, { ...mainMenuKeyboard() });
          }
    ...
    if (act.step === "number") {
      ...
      if (!phone254) return sendTracked(chatId, "❌ Invalid phone. Use 07/01/2547/2541 format.", { ...mainMenuKeyboard() });
...
// ===================== GLOBAL BUTTONS =====================
  ...
  if (text === "⬅ Back") {
    ...
    supportState.delete(String(chatId));
    clearPendingAction(chatId);
    return sendTracked(chatId, "⬅ Back to main menu.", { ...mainMenuKeyboard() });
  }
...
const u0 = getUser(chatId);
if (isTemporarilyBlocked(u0)) {
  const mins = tempBlockRemainingMin(u0);
            fraudAlert("Blocked action: temp payment-fail block", chatId, `Remaining: ${mins} min(s)`);
return sendTracked(chatId, `🚫 Account temporarily blocked due to repeated failed payments. Try again in ${mins} min(s).`, { ...mainMenuKeyboard() });
}
if (isHighRisk(u0)) {
            fraudAlert("Blocked action: high risk score", chatId, "User attempted protected action");
return sendTracked(chatId, "🚫 Account flagged for security review. Please contact support.", { ...mainMenuKeyboard() });
}
...
• Cycle resets in: ${st.daysLeft} day(s)`,
    ...
    if (bal < item.cost) {
      return sendTracked(chatId, `❌ Not enough points.
You have: ${bal.toFixed(2)} pts
Need: ${item.cost} pts`, { ...mainMenuKeyboard() });
    }
...
Tap ✅ Confirm to continue.`, {
  ...
  if (text === "🔄 Resend STK") {
    ...
    if (!found) {
      return sendTracked(chatId, "❌ No recent pending STK found.", { ...mainMenuKeyboard() });
    }
    ...
    if (!isAdmin(chatId) && lastAt && now - lastAt < 60 * 1000) {
      const wait = Math.ceil((60 * 1000 - (now - lastAt)) / 1000);
      return sendTracked(chatId, `⏳ You must wait ${wait}s before resending STK.`, { ...mainMenuKeyboard() });
    }
    ...
    try {
      ...
      return sendTracked(chatId, "✅ STK resent successfully. Check your phone.", { ...mainMenuKeyboard() });
    } catch (err) {
      return sendTracked(chatId, `⚠️ Resend failed: ${err.message}`, { ...mainMenuKeyboard() });
    }
  }
...
// ===================== WITHDRAW RATE =====================
// 100 pts = 50 KES  →  1 pt = 0.5 KES
function pointsToKes(pts) {
  return Number(pts || 0) * 0.5;
}
...
if (text === "💸 Withdraw Points") {
  ...
  const uWR = getUser(chatId);
  const isBypass = isAdmin(chatId) || !!uWR.privilegedBypass;


  // 🚨 Referral instant-withdraw protection (5 min cooldown)
  const nowRef = Date.now();
  if (!isAdmin(chatId) && !uWR.privilegedBypass &&
      uWR.lastReferralRewardAt &&
      nowRef - uWR.lastReferralRewardAt < 5 * 60 * 1000) {

      uWR.withdrawBlockedUntil = nowRef + (5 * 60 * 1000);
      uWR.riskScore = Number(uWR.riskScore || 0) + 10;
      saveDB();

      return sendTracked(chatId,
        "⏳ Withdrawal temporarily unavailable. Please try again shortly.",
        { ...mainMenuKeyboard() }
      );
  }

    const availableLimit = Math.max(0, Number(uWR.withdrawCapacityPts || 0) - Number(uWR.totalWithdrawnPts || 0));
if (!isBypass && (isFrozen(uWR) || isHighRisk(uWR))) {
  autoFreezeIfHighRisk(chatId, "User attempted withdraw/redeem while high risk");
  const d = freezeRemainingDays(uWR);
  fraudAlert("Blocked action: withdraw/redeem frozen", chatId, `Remaining: ${d} day(s)`);
...
const u = getUser(chatId);
if (!isBypass && isTemporarilyBlocked(u)) {
  const mins = tempBlockRemainingMin(u);
  return sendTracked(chatId, `🚫 Account temporarily blocked due to repeated failed payments. Try again in ${mins} min(s).`, { ...mainMenuKeyboard() });
}
if (!isBypass && isHighRisk(u)) {
  return sendTracked(chatId, "🚫 Account flagged for security review. Please contact support.", { ...mainMenuKeyboard() });
}
// Account age lock (day 3+ only)
const ageMs = Date.now() - Number(u.createdAt || 0);
if (!isBypass && ageMs > 0 && ageMs < ACCOUNT_AGE_WITHDRAW_LOCK_MS) {
      fraudAlert("Withdraw blocked: account age < 2 days", chatId, "Auto-declined until day 3");
return sendTracked(chatId, "🚫 Withdrawals are available after day 1 of account activity.", { ...mainMenuKeyboard() });
}
// Minimum spend before withdraw
if (!isBypass && Number(u.totalSpentKsh || 0) < WITHDRAW_MIN_SPEND_KES) {
      fraudAlert("Withdraw blocked: below minimum spend", chatId, `Total spent KES ${Number(u.totalSpentKsh||0)}`);
return sendTracked(chatId, `🚫 You must spend at least KES ${WITHDRAW_MIN_SPEND_KES} before withdrawing.`, { ...mainMenuKeyboard() });
}
// Phone change lock
const plc = Number(u.phoneLastChangedAt || 0);
if (plc && Date.now() - plc < PHONE_CHANGE_WITHDRAW_LOCK_MS) {
  decreaseTrust(chatId, 5);
      u.riskScore = Number(u.riskScore || 0) + 10;
  saveDB();
      fraudAlert("Withdraw blocked: phone changed <24h", chatId, "24h lock triggered");
return sendTracked(chatId, "🚫 Withdraw locked for 24h after changing your phone number.", { ...mainMenuKeyboard() });
}

  const savedPhone = getUserPhone(chatId);
  if (!savedPhone) {
    return sendTracked(chatId, "📱 Please save your phone number first before withdrawing.", { ...mainMenuKeyboard() });
  }
...
if (["25 pts", "50 pts", "100 pts", "250 pts", "500 pts"].includes(text)) {
if (!isBypass && (isFrozen(uWR) || isHighRisk(uWR))) {
  autoFreezeIfHighRisk(chatId, "User attempted withdraw/redeem while high risk");
  const d = freezeRemainingDays(uWR);
  fraudAlert("Blocked action: withdraw/redeem frozen", chatId, `Remaining: ${d} day(s)`);
...
const uW = getUser(chatId);
if (isTemporarilyBlocked(uW)) {
  const mins = tempBlockRemainingMin(uW);
  return sendTracked(chatId, `🚫 Account temporarily blocked due to repeated failed payments. Try again in ${mins} min(s).`, { ...mainMenuKeyboard() });
}
if (isHighRisk(uW)) {
  return sendTracked(chatId, "🚫 Account flagged for security review. Please contact support.", { ...mainMenuKeyboard() });
...
// once per day limit
if (Array.isArray(uW.withdrawHistory) && uW.withdrawHistory.length) {
  const last = uW.withdrawHistory[uW.withdrawHistory.length - 1];
  const lastAt = Number(last?.time || 0);
  if (lastAt && Date.now() - lastAt < WITHDRAW_ONCE_PER_DAY_MS) {
    uW.riskScore = Number(uW.riskScore || 0) + 10;
    saveDB();
          fraudAlert("Withdraw blocked: daily limit", chatId, "Attempted multiple withdrawals in 24h");
return sendTracked(chatId, "🚫 You can only withdraw once every 24 hours.", { ...mainMenuKeyboard() });
  }
}
  ...
  if (bal < amount) {
    return sendTracked(chatId, `❌ Not enough points. You have ${bal} pts`, { ...mainMenuKeyboard() });
  
  // Enforce withdraw capacity limit (secret). Admin/Privileged bypass.
  const uCap = getUser(chatId);
  const isBypass2 = isAdmin(chatId) || !!uCap.privilegedBypass;
  const availableLimit2 = Math.max(0, Number(uCap.withdrawCapacityPts || 0) - Number(uCap.totalWithdrawnPts || 0));
  if (!isBypass2 && amount > availableLimit2) {
    return sendTracked(chatId,
      "🚫 Withdrawal limit reached. Continue using the bot to unlock more withdrawals.",
      { ...mainMenuKeyboard() }
    );
  }
  ...
  if (hasPendingRedeem(chatId)) {
    return sendTracked(chatId, "⏳ You already have a pending request.", { ...mainMenuKeyboard() });
  }
...
// ===================== PURCHASE FLOW =====================
  ...
  try {
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed (use saved number)" || text === "✅ Proceed (use saved number)") {

  // 🔒 Bingwa cross-account phone protection
  const uPhoneCheck = getUser(chatId);
  const phone07Check = formatTo07(getUserPhone(chatId));

  if (!isAdmin(chatId) && !uPhoneCheck.privilegedBypass &&
      isPhoneBlockedToday(phone07Check, chatId)) {

    fraudAlert("Cross-account Bingwa attempt", chatId, `Phone: ${phone07Check}`);

    return sendTracked(chatId,
      "🚫 This phone number already purchased a Bingwa deal today using another account. Try again tomorrow.",
      { ...mainMenuKeyboard() }
    );
  }
        const uStk = getUser(chatId);
        if (!isAdmin(chatId) && !uStk.privilegedBypass && uStk.stkCooldownUntil && Date.now() < uStk.stkCooldownUntil) {
          const hrs = Math.ceil((uStk.stkCooldownUntil - Date.now()) / 3600000);
          return sendTracked(chatId, `🚫 STK temporarily locked. Try again in ${hrs} hour(s).`, { ...mainMenuKeyboard() });
        }

const u = getUser(chatId);
if (isTemporarilyBlocked(u)) {
  const mins = tempBlockRemainingMin(u);
  return sendTracked(chatId, `🚫 Account temporarily blocked due to repeated failed payments. Try again in ${mins} min(s).`, { ...mainMenuKeyboard() });
}
if (isHighRisk(u)) {
...
// ===================== WITHDRAW RELEASE ENGINE =====================
setInterval(() => {
  const now = Date.now();
  for (const cid in db.users) {
    const u = db.users[cid];
    if (u.pendingWithdrawReleaseAt && now >= u.pendingWithdrawReleaseAt) {
      u.pendingWithdrawReleaseAt = 0;
      saveDB();
      bot.sendMessage(Number(cid),
