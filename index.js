...
Help: ${HELP_PHONE}`, { ...mainMenuKeyboard() });
    ...
    // ✅ Redeem decline reason selection
    if (data.startsWith("redeem_decline_reason:")) {
      const parts = data.split(":"); // redeem_decline_reason:reqId:reason
      const reqId2 = parts[1];
      const reasonKey = parts[2] || "other";
...
Your request is being processed.`, { ...mainMenuKeyboard() }).catch(() => {});
    ...
    if (action === "redeem_decline") {
      // Show decline reason options
      await bot.answerCallbackQuery(q.id, { text: "Select a reason" }).catch(() => {});
      await bot.editMessageReplyMarkup(
        {
          inline_keyboard: [
            [{ text: "Insufficient points", callback_data: `redeem_decline_reason:${reqId}:insufficient` }],
            [{ text: "Invalid number", callback_data: `redeem_decline_reason:${reqId}:invalid_number` }],
            [{ text: "Offer unavailable", callback_data: `redeem_decline_reason:${reqId}:offer_unavailable` }],
            [{ text: "Suspected fraud", callback_data: `redeem_decline_reason:${reqId}:fraud` }],
            [{ text: "Other", callback_data: `redeem_decline_reason:${reqId}:other` }],
          ],
        },
...
function getActivePendingPaymentForChat(chatId) {
  db = repairDB(db);
  const now = Date.now();
  const entries = Object.entries(db.pendingPayments || {});
  for (const [ref, p] of entries) {
    if (!p || Number(p.chatId) !== Number(chatId)) continue;
    const createdAt = Number(p.createdAt || 0);
    if (createdAt && now - createdAt < RETRY_PENDING_TIMEOUT_MS) {
      return { ref, pending: p, ageMs: now - createdAt };
    }
  }
  return null;
}
      ...
      if (/^\/pending$/i.test(text)) {
        ...
        for (const r of pend.slice(0, 25)) {
          const msgText =
            ...
            `Total referrals: *${mdEscape(r.totalReferrals || 0)}*\n` +
            `Total purchase: *${mdEscape(r.totalSpent || 0)}ksh*\n` +
            `Time: \`${mdEscape(r.time || "")}\``;

          // eslint-disable-next-line no-await-in-loop
          await bot.sendMessage(ADMIN_ID, msgText, {
            parse_mode: "Markdown",
            reply_markup: {
              inline_keyboard: [[
                { text: "✅ Accept", callback_data: `redeem_accept:${r.reqId}` },
                { text: "❌ Decline", callback_data: `redeem_decline:${r.reqId}` },
              ]],
            },
          }).catch(() => {});
        }
...
` +
          `User: ${mdEscape(name)}
` +
          `Item: ${mdEscape(item.label)}
` +
          `Cost: *${mdEscape(cost)} pts*
` +
          `Load To: *${mdEscape(loadTo07)}*
` +
          `Total referrals: *${mdEscape(totalReferrals)}*
` +
          `Total purchase: *${mdEscape(totalSpent)}ksh*
` +
          `Time: \`${mdEscape(when)}\``;
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  ...
                  { text: "❌ Decline", callback_data: `redeem_decline:${reqId}` },
                ],
              ],
            },
          });
...
if (text === "🔁 Retry STK") {
  ...
  if (!isAdmin(chatId) && String(last.category) === "Bingwa Deals" && bingwaAlreadyPurchasedToday(String(last.phone254))) {
    return sendTracked(chatId, "🚫 Bingwa Deals limit reached. This number already bought Bingwa today. Use a different number or try again tomorrow.", { ...mainMenuKeyboard() });
  }
  ...
  sendTracked(chatId, "🔁 Retrying STK push… Check your phone.");
  ...
  payheroStkPush({ amount: Number(last.price || 0), phone: String(last.phone254), externalRef: ref, channelId })
    .then(() => {
      return sendTracked(chatId, "✅ STK resent successfully. When payment completes you will receive confirmation here.", { ...mainMenuKeyboard() });
    })
    .catch((err) => {
  ...
  return;
    ...
    if (!Number(last.failedAt) || Date.now() - Number(last.failedAt) > RETRY_PENDING_TIMEOUT_MS) {
      return sendTracked(
        chatId,
        `⏱️ Retry window expired (over 5 minutes).
Please start the purchase again using 🛒 Buy Offers.`,
        { ...mainMenuKeyboard() }
      );
    }

    // ✅ Bingwa-only daily check
    if (!isAdmin(chatId) && String(last.category) === "Bingwa Deals" && bingwaAlreadyPurchasedToday(String(last.phone254))) {
      return sendTracked(
        chatId,
...
If delay: ${HELP_PHONE}`,
      ...
      return sendTracked(chatId, `⚠️ Retry failed: ${err.message}`, { ...mainMenuKeyboard() });
    }
  }

    // If user has a last STK that was sent but no callback and it's older than 5 mins: force user to purchase again
    const lastSent = u.lastStkAttempt && u.lastStkAttempt.status === "sent" ? u.lastStkAttempt : null;
    if (lastSent && Number(lastSent.sentAt || 0) && Date.now() - Number(lastSent.sentAt || 0) >= RETRY_PENDING_TIMEOUT_MS) {
      return sendTracked(
        chatId,
        `⏱️ The previous request has timed out (over 5 minutes).
Please start the purchase again using 🛒 Buy Offers.`,
        { ...mainMenuKeyboard() }
      );
    }
    ...
    const last = u.lastFailedStk;
    if (!last || !last.phone254 || !last.pkgLabel || !last.category) {
      return sendTracked(chatId, "ℹ️ No failed STK found to retry. Please start again using 🛒 Buy Offers.", {
        ...mainMenuKeyboard(),
      });
    }
