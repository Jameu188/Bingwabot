...
if (!isSuccess) {
  ...
  return;
    ...
    if (pending) {
    // 🎁 BOX GAME AUTO-INTEGRATION
    if (pending && pending.category === "BOX_GAME" && (payload?.status === true || payload?.ResultCode === 0 || payload?.ResultCode === "0")) {
      const chatId = pending.chatId;

      incrementBoxPlay(chatId);

      const rewards = generateBoxRewards();

      boxGameSessions.set(chatId, {
        rewards,
...
// =====================================================
// 🎁 PICK A BOX GAME (4 KSH STK OR 7 PTS ENTRY)
// Daily limit: 25 plays
// Selection expires after 60 seconds
// Max reward: 100 pts (very rare)
// =====================================================

const boxGameSessions = new Map(); // chatId -> session
const boxGameDailyLimit = 25;

function getTodayKey() {
  return new Date().toISOString().slice(0, 10);
}

function canPlayBox(chatId) {
  const u = getUser(chatId);
  const today = getTodayKey();

  if (!u.boxGameStats) {
    u.boxGameStats = { date: today, count: 0 };
  }

  if (u.boxGameStats.date !== today) {
    u.boxGameStats = { date: today, count: 0 };
  }

  return u.boxGameStats.count < boxGameDailyLimit;
}

function incrementBoxPlay(chatId) {
  const u = getUser(chatId);
  const today = getTodayKey();

  if (!u.boxGameStats || u.boxGameStats.date !== today) {
    u.boxGameStats = { date: today, count: 1 };
  } else {
    u.boxGameStats.count++;
  }

  saveDB();
}

function generateBoxRewards() {
  let rewards = [];

  const rareRoll = Math.random();

  if (rareRoll < 0.001) {
    rewards.push(Math.floor(Math.random() * 20) + 81); // 81–100 (0.1%)
  } else if (rareRoll < 0.02) {
    rewards.push(Math.floor(Math.random() * 40) + 41); // 41–80 (~1.9%)
  }

  while (rewards.length < 5) {
    const roll = Math.random();
    if (roll < 0.35) rewards.push(0);
    else if (roll < 0.65) rewards.push(Math.floor(Math.random() * 5) + 1);
    else if (roll < 0.85) rewards.push(Math.floor(Math.random() * 10) + 6);
    else if (roll < 0.95) rewards.push(Math.floor(Math.random() * 15) + 16);
    else rewards.push(Math.floor(Math.random() * 15) + 26);
  }
...
bot.on("message", async (msg) => {
  ...
  // Start Game
  if (text === "🎁 Pick a Box") {
    if (!canPlayBox(chatId)) {
      return bot.sendMessage(chatId, "❌ Daily limit reached (25 plays). Come back tomorrow.");
    }
  ...
  // STK Option
  if (text === "💳 Pay 4 KSH (STK)") {
    if (!canPlayBox(chatId)) {
      return bot.sendMessage(chatId, "❌ Daily limit reached (25 plays).");
    }

    const userPhone = getUser(chatId).phone;
    if (!userPhone) {
      return bot.sendMessage(chatId, "⚠️ Please save your phone number first.");
    }

    const ref = `BOX-${Date.now()}-${chatId}`;
    ...
    try {
      ...
      return bot.sendMessage(chatId, "📲 STK sent. Complete payment to continue...");
    } catch (e) {
      return bot.sendMessage(chatId, "❌ STK failed. Try again.");
    }
  }
  ...
  if (text === "⭐ Pay 7 pts") {
    ...
    const currentPts = getUser(chatId).points || 0;

    if (currentPts < 7) {
      return bot.sendMessage(chatId, "❌ Not enough points. Need 7 pts.");
    }

    getUser(chatId).points -= 7;
    saveDB();
    ...
    return bot.sendMessage(chatId, "🎁 Choose a box number:\n\n1  2  3  4  5");
  }

  // Handle selection
  if (["1", "2", "3", "4", "5"].includes(text)) {
    if (!boxGameSessions.has(chatId)) return;

    const session = boxGameSessions.get(chatId);

    if (Date.now() > session.expires) {
      boxGameSessions.delete(chatId);
      return bot.sendMessage(chatId, "⌛ Selection expired. Start again.");
    }
    ...
    const win = rewards[choice];

    getUser(chatId).points += win;
    saveDB();

    const resultText = rewards.map((r, i) => {
      if (i === choice) return `${i+1} – ${r} pts ⭐`;
      return `${i+1} – ${r} pts`;
    }).join("\n");

    return bot.sendMessage(
      chatId,
      `🎁 Opening box #${choice+1}...\n\n📦 Final Results:\n\n${resultText}\n\n🎉 You won ${win} pts!`
    );
  }
});

// Handle successful STK callback for box game
// (Hook into your existing callback success logic by checking category === "BOX_GAME")




// =====================================================
// 🎁 BOX GAME CALLBACK INTEGRATION
// Add this logic inside your existing PayHero SUCCESS callback
// where ResultCode/Status === true and pending payment exists
// =====================================================

// Inside your successful callback block, add:

/*
if (pending && pending.category === "BOX_GAME") {
  const chatId = pending.chatId;
