...
async function promoMessageStillExists(chatId, messageId) {
  try {
    ...
    try {
      await bot.deleteMessage(String(ADMIN_ID), probe.message_id).catch(() => {});
    } catch (_) {}
      ...
      try {
        ...
        if (newMessageId) {
          ...
          try {
            await bot.sendMessage(
              String(ADMIN_ID),
              `♻️ Promo auto-reposted (unpinned)\nChat: ${String(chatId)}`
            ).catch(() => {});
          } catch (_) {}
        } else {
          setPromoWatchdog(chatId, {
    ...
    for (const [chatId, rec] of Object.entries(metrics.byChannel || {})) {
      try {
        ...
        try {
          ...
          chat = { id: chatId, title: rec.title || chatId, username: rec.username || "", type: "channel" };
        }
