...
function formatStkSentMessage({ amount, pkgLabel, paymentPhone, receivePhone, receivePhones, ref, status = "Sent", extraLines = [] } = {}) {
  ...
  if (receiveList.length > 1) {
    lines.push("📥 Received Numbers:");
    for (const n of receiveList) lines.push(`• ${maskPhone(n)}`);
  } else {
    const singleReceive = receiveList[0] || receivePhone || paymentPhone;
    if (singleReceive) lines.push(`📥 Received Number: ${maskPhone(singleReceive)}`);
  }

  if (paymentPhone) lines.push(`📱 Payment Number: ${maskPhone(paymentPhone)}`);
...
async function deleteCartReminderMessage(chatId, s) {
  if (!s || !s.cartReminderMsgId) return;
  try { await safeDelete(chatId, s.cartReminderMsgId); } catch (_) {}
  s.cartReminderMsgId = null;
}
...
Tap 🛒 View Cart to continue, edit numbers, remove items, or complete all cart.`,
  ...
  if (sent && sent.message_id) s.cartReminderMsgId = sent.message_id;
