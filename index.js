...
Rule: You earn when your referred user completes a successful purchase.`,
  { parse_mode: "HTML", ...mainMenuKeyboard(chatId) }
    ...
    return sendTracked(
      ...
      { parse_mode: "Markdown", ...confirmKeyboardV2(chatId, s, hasSaved) }
    );
  }
  ...
  if (s.step === "confirm") {
    ...
    const msgText =
      ...
      (isFastPurchaseEnabled(chatId)
        ? `Choose a number below or tap ➕ Add Number.\n\nOnce you pick a number, STK will be sent automatically.`
        : `Once you pick a number, STK will be sent automatically.`);


    return sendTracked(chatId, msgText, { parse_mode: "Markdown", ...confirmKeyboardV2(chatId, s, hasSaved) });
  }
...
async function proceedPurchase(chatId, s) {
  ...
  // ✅ Bingwa once per day per phone (enforced here)
  ...
  if (recent) {
    return sendTracked(
      chatId,
      "⏳ You already have a pending payment request for this offer & number.\nPlease complete it or wait a moment.",
      { ...mainMenuKeyboard(chatId) }
    );
  }
  ...
  upsertPendingPayment(pending);

  try {
    const channelId = getPayheroChannelId(s.category);
    await payheroStkPush({ amount: pkg.price, phone: savedPhone, externalRef: ref, channelId });

    return sendTracked(
      chatId,
      `📲 STK sent to *${maskPhone(savedPhone)}*\n\n` +
        `Offer: *${mdEscape(pkg.label)}*\n` +
        `Amount: *Ksh ${mdEscape(pkg.price)}*\n\n` +
        "Complete the prompt on your phone to finish payment.",
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  } catch (err) {
    sessions.delete(chatId);
