...
function repairDB(obj) {
  ...
  if (!db0.autoBackupState || typeof db0.autoBackupState !== "object") db0.autoBackupState = { lastRunDate: "" };
...
const AUTO_RETRY_STK_DELAY_MS = 66 * 1000; // 1.1 minutes
  ...
  const scheduleOne = (attemptNo) => {
    const tid = setTimeout(async () => {
      try {
        ...
        await bot.sendMessage(chatId, `🔁 Auto Retry STK sent (attempt ${attemptNo}/${AUTO_RETRY_STK_MAX}). Check your phone.`).catch(() => {});
      } catch (_) {
        // Silent on retry errors to avoid noisy chats.
      }
    }, AUTO_RETRY_STK_DELAY_MS);
...
// ===================== AUTO DELETE TXN HISTORY TOGGLE =====================
if (data === "th:toggle") {
  const u = getUser(chatId);
  u.autoDeleteTxnHistory = !u.autoDeleteTxnHistory;
  saveDB();

  await bot.answerCallbackQuery(q.id, { text: u.autoDeleteTxnHistory ? "✅ Auto-delete transactions ON" : "⬜️ Auto-delete transactions OFF" }).catch(() => {});
  try {
    const mid = q.message?.message_id;
    if (mid) {
      await refreshCommandsSettings(chatId, mid);
    }
  } catch (_) {}
  return;
}

// ===================== BUSINESS AUTO DAILY BACKUP TOGGLE =====================
if (data === "biz:autobk_toggle") {
  if (!isBusinessActive(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "💼 Business only. Type /sell to upgrade." }).catch(() => {});
    return;
  }
  ...
  const u = getUser(chatId);
  const cur = (u.autoDailyBackupEnabled === undefined) ? true : !!u.autoDailyBackupEnabled;
  u.autoDailyBackupEnabled = !cur;
  saveDB();

  await bot.answerCallbackQuery(q.id, { text: u.autoDailyBackupEnabled ? "✅ Auto Daily Backup ON" : "⬜️ Auto Daily Backup OFF" }).catch(() => {});
  try {
    const mid = q.message?.message_id;
    if (mid) {
      await refreshCommandsSettings(chatId, mid);
...
if (data === "ar:toggle") {
 ...
 if (data === "nf:campaign") {
   ...
   try {
     const mid = q.message?.message_id;
     if (mid) await refreshCommandsSettings(chatId, mid);
   } catch (_) {}
   return;
 }
...
async function sendHelpAnswer(chatId, key, ctx = {}) {
  ...
  const opts = key === "cmds" ? { parse_mode: "Markdown", ...fastPurchaseHelpKeyboard(chatId) } : { parse_mode: "Markdown", ...helpAnswerKeyboard() };

  const edited = await safeEditMessageText(chatId, ctx.messageId, text, opts);
  if (edited) return edited;

  // For Commands & Settings, always send a fresh panel if edit fails
  if (key === "cmds") {
    return refreshCommandsSettings(chatId, null);
  }
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: `${txnHistoryAutoDeleteMark(chatId)} Auto delete last transactions`, callback_data: "th:toggle" }],

        ...(isBusinessActive(chatId) ? [[{ text: `${autoDailyBackupMark(chatId)} Auto Daily Backup (8PM)`, callback_data: "biz:autobk_toggle" }]] : []),
...
async function refreshCommandsSettings(chatId, messageId = null) {
  try {
    // Prefer editing existing message
    if (messageId) {
      const ok = await bot.editMessageReplyMarkup(
        fastPurchaseHelpKeyboard(chatId).reply_markup,
        { chat_id: chatId, message_id: messageId }
      ).catch(() => null);
      if (ok) return ok;
    }

    // Fallback: send a fresh Commands & Settings panel
    const txt = helpAnswerText("cmds");
    // Try markdown first, then plain text
    try {
      return await sendTracked(chatId, txt, { parse_mode: "Markdown", ...fastPurchaseHelpKeyboard(chatId) });
    } catch (_) {
      return await sendTracked(chatId, txt, { ...fastPurchaseHelpKeyboard(chatId) });
    }
  } catch (_) {
    return null;
  }
...
async function sendTxnHistory(chatId, textMsg, opts = {}) {
  const sent = await sendTracked(chatId, textMsg, opts);
  try {
    const u = getUser(chatId);
    if (u.autoDeleteTxnHistory && sent && sent.message_id) {
      setTimeout(() => {
...
// ===================== AUTO DAILY BUSINESS BACKUP (8PM Africa/Nairobi) =====================
function nairobiNowParts() {
  try {
    ...
    const d = new Date();
    return { date: d.toISOString().slice(0, 10), hour: d.getHours(), minute: d.getMinutes() };
  }
}

async function runAutoDailyBusinessBackup() {
  try {
    db = repairDB(db);
    const state = db.autoBackupState && typeof db.autoBackupState === "object" ? db.autoBackupState : { lastRunDate: "" };
    const now = nairobiNowParts();

    if (now.hour !== 20 || now.minute !== 0) return;
    if (state.lastRunDate === now.date) return;

    db.autoBackupState = state;
    db.autoBackupState.lastRunDate = now.date;
    saveDB();

    const users = db.users && typeof db.users === "object" ? Object.keys(db.users) : [];
    for (const cid of users) {
      const chatId = String(cid);
      try {
        if (typeof isBusinessActive === "function" && !isBusinessActive(chatId)) continue;

        const u = getUser(chatId);
        const enabled = (u.autoDailyBackupEnabled === undefined) ? true : !!u.autoDailyBackupEnabled;
        if (!enabled) continue;

        const payload = buildUserBackup(chatId);
        const pretty = JSON.stringify(payload, null, 2);
        const fname = `daily_backup_${chatId}_${now.date}.json`;

        await bot.sendDocument(
          chatId,
          Buffer.from(pretty, "utf8"),
          { caption: "📦 Daily Business Backup (8PM)" },
          { filename: fname, contentType: "application/json" }
        ).catch(() => {});
      } catch (_) {}
    }
  } catch (e) {
    console.log("AutoDailyBackup error:", e?.message || e);
  }
}

function startAutoDailyBusinessBackupLoop() {
  try {
    runAutoDailyBusinessBackup().catch(() => {});
    setInterval(() => {
      runAutoDailyBusinessBackup().catch(() => {});
    }, 60 * 1000);
  } catch (_) {}
...
startAutoDailyBusinessBackupLoop();
