...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data.startsWith("bcast:")) {
      ...
      const { fromChatId, messageId, repliedMsg, days } = st;

      try {
        if (choice === "once") {
          const result = await broadcastAsIsReply(fromChatId, repliedMsg);
          incBroadcastStats();
          await bot.sendMessage(
            chatId,
...
const albumItems = getCachedAlbumItemsByMsg(repliedMsg);
const albumMedia = albumItems && albumItems.length > 1
  ? albumItems.map((m, i) => buildInputMediaFromMsg(m, i === 0)).filter(Boolean)
  : null;
const rot = addRotation({ fromChatId, messageId, albumMedia, days });
...
// Album cache for /bcast reply-albums (resend as true album)
const albumCache = new Map(); // media_group_id -> { items: [msg], timer }
...
async function broadcastAlbum(items) {
  ...
  return { sent, failed, total: userIds.length };
async function broadcastAlbumMedia(prebuiltMedia) {
  const userIds = getAllUserIds();
  let sent = 0;
  let failed = 0;
  ...
  for (const uid of userIds) {
    try {
      // eslint-disable-next-line no-await-in-loop
      await bot.sendMediaGroup(uid, prebuiltMedia);
      sent++;
    } catch (_) {
      failed++;
    }
...
function getCachedAlbumItemsByMsg(msg) {
  if (!msg?.media_group_id) return null;
  const gid = String(msg.media_group_id);
  const pack = albumCache.get(gid);
  if (!pack?.items?.length) return null;
  // Keep Telegram album order
  return [...pack.items].sort((a, b) => (a.message_id || 0) - (b.message_id || 0));
}

// Broadcast exactly what admin replied to:
// - If reply is part of an album and we have the cached album => send as true album
// - Otherwise, copy the single message as-is
async function broadcastAsIsReply(fromChatId, repliedMsg) {
  const albumItems = getCachedAlbumItemsByMsg(repliedMsg);
  if (albumItems && albumItems.length > 1) {
    return broadcastAlbum(albumItems);
  }
  return broadcastCopyMessage(fromChatId, repliedMsg.message_id);
}
...
function addRotation({ fromChatId, messageId, days }) {
  ...
  const rot = {
    ...
    albumMedia: Array.isArray(albumMedia) ? albumMedia : null,
    days: safeDays,
    createdAt: now,
    nextRunAt: now,
...
async function runDueRotations() {
  try {
    ...
    for (const r of list) {
      ...
      try {
        const result = (r.albumMedia && Array.isArray(r.albumMedia) && r.albumMedia.length)
          ? await broadcastAlbumMedia(r.albumMedia)
          : await broadcastCopyMessage(r.fromChatId, r.messageId);
        incBroadcastStats();
...
// ---- Album collector (so /bcast can resend replied album as an album) ----
if (msg.media_group_id) {
  const gid = String(msg.media_group_id);
  const existing = albumCache.get(gid);
  if (!existing) {
    const timer = setTimeout(() => albumCache.delete(gid), ALBUM_CACHE_TTL_MS);
    albumCache.set(gid, { items: [msg], timer });
  } else {
    existing.items.push(msg);
  }
}
...
if (bcastMatch) {
  ...
  if (repliedMsg?.media_group_id) {
    const items = getCachedAlbumItemsByMsg(repliedMsg) || [];
    const count = items.length || 1;
    albumInfoText = `\n\n📦 Detected album: ${count} item${count > 1 ? "s" : ""}\nIt will be sent as a media album.`;
  }
  ...
  return bot.sendMessage(chatId, `Choose broadcast mode (rotation = ${days} day(s)) :${albumInfoText}`, {
    reply_markup: {
      inline_keyboard: [
