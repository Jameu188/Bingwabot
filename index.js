...
async function sendAdminPaymentSms({ payingPhone254 = "", receivingPhone254 = "" } = {}) {
  try {
    if (!twilioClient || !TWILIO_FROM || !ADMIN_SMS_TO) {
      console.log("TWILIO DEBUG: missing config");
      return { ok:false };
    }
    ...
    console.log("TWILIO ERROR:", err?.message || err);
    return { ok:false };
  }
}
...
if (isBusinessActive(chatId) && String(u.businessPlanKey || "") === planKey) {
  ...
  try {
    ...
    try {
        ...
        console.log("TWILIO CALL ERROR:", e?.message || e);
      }
...
if (/^\/testsms(?:@\w+)?$/i.test(text)) {
  ...
  const u = getUser(chatId);
  const payingPhone254 = String(u.phone || "254707071631");
  const receivingPhone254 =
    String((u.pendingAction && u.pendingAction.receivingPhone254) || "") || payingPhone254;

  try {
    const result = await sendAdminPaymentSms({ payingPhone254, receivingPhone254 });

    if (result && result.ok) {
      await sendTracked(
        chatId,
        `✅ Test SMS sent.\n\nTo: +${String(normalizePhone(process.env.ADMIN_SMS_TO || "0759423842") || "254759423842")}\nPaying: ${formatTo07(payingPhone254)}\nReceiving: ${formatTo07(receivingPhone254)}`,
        { ...mainMenuKeyboard(chatId) }
      );
    } else {
      await sendTracked(
        chatId,
        `❌ Test SMS failed.\n\nCheck Render logs for TWILIO ERROR / TWILIO DEBUG.`,
        { ...mainMenuKeyboard(chatId) }
      );
    }
  } catch (e) {
