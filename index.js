...
function helpMenuKeyboard() {
  const rows = [
    ...
    [
      ...
      { text: "💼 Business Settings", callback_data: "help:biz_settings" },
    ],
    [
...
function businessSettingsKeyboard(chatId) {
  const u = getUser(chatId);
  const isBiz = isBusiness(chatId);
  ...
  const rows = [
    [{ text: `${mark(!!u.stkAutoRetry)} Auto-retry STK`, callback_data: "biz:stk:toggle" }],
    [{ text: `🔁 Retry count: ${u.stkRetryCount || 2}`, callback_data: "biz:stk:count" }],
    [{ text: `⏱ Retry delay: ${msLabel(u.stkRetryDelayMs || 10000)}`, callback_data: "biz:stk:delay" }],
    [{ text: `${mark(!!u.skipConfirmation)} Skip confirmation`, callback_data: "biz:qb:skip" }],
    [{ text: `${mark(!!u.rememberLastCategory)} Remember last category`, callback_data: "biz:qb:remember" }],
    [{ text: `${mark(!!u.repeatLastButton)} Repeat last button`, callback_data: "biz:qb:repeat" }],
  ...
  const text =
    ...
    `• Retry count: ${u.stkRetryCount || 2}\n` +
    `• Retry delay: ${msLabel(u.stkRetryDelayMs || 10000)}\n\n` +
    "🚀 *Quick Buy*\n" +
...
async function payheroStkPushWithBizRetry(chatId, payload) {
  const u = getUser(chatId);
  const enabled = !!u.stkAutoRetry && isBusiness(chatId);
  ...
  for (let attempt = 1; attempt <= maxTries; attempt++) {
    try {
      ...
      lastErr = e;
      if (attempt < maxTries) {
        await sendTracked(chatId, `⚠️ STK failed. Retrying (${attempt}/${maxTries - 1})…`).catch(() => {});
...
function packagesKeyboard(category) {
  ...
  const rows = [
    ...
    // Business quick button: Repeat Last Purchase
    try {
      const u = getUser(chatId);
      if (isBusiness(chatId) && u.repeatLastButton) {
        rows.unshift(["🔁 Repeat Last Purchase"]);
      }
    } catch (_) {}
];
...
if (text === "🛒 Buy Offers") {
    ...
    // 🚀 Quick Buy: Remember last category (Business)
    try {
      const u = getUser(chatId);
      if (isBusiness(chatId) && u.rememberLastCategory && u.lastCategory && PACKAGES[u.lastCategory]) {
        pushPrev(chatId, sessions.get(chatId) || { step: null });
        sessions.set(chatId, { step: "package", category: u.lastCategory, pkgKey: null, createdAt: Date.now() });
        await sendBannerIfAvailable(chatId);
        await sendTracked(chatId, `⚡ Last category: *${u.lastCategory}*\\nChoose a package:`, { parse_mode: "Markdown" });
...
// ===================== PURCHASE FLOW =====================
  ...
  try {
    ...
    if (s.step === "category") {
      ...
      try {
        const u = getUser(chatId);
        if (isBusiness(chatId) && u.rememberLastCategory) { u.lastCategory = category; saveDB(); }
      } catch (_) {}
...
if (s.step === "package") {
  ...
  // 🚀 Quick Buy: Skip confirmation (Business)
  const u = getUser(chatId);
  const pkg2 = pkg;
  const savedPhone = getUserPhone(chatId);
  if (isBusiness(chatId) && u.skipConfirmation && savedPhone) {
    // send STK immediately using saved phone
    sessions.delete(chatId);
    try { await bot.sendMessage(chatId, `📲 Sending STK push… (${maskPhone(savedPhone)})`); } catch (_) {}
    try { await initiatePurchaseStk(chatId, { ...s, step: "confirm" }, pkg2, savedPhone); } catch (e) {
      return sendTracked(chatId, `❌ Failed to send STK. Try again.\n\nHelp: ${HELP_PHONE}`, { ...mainMenuKeyboard(chatId) });
    }
    return;
  } else if (isBusiness(chatId) && u.skipConfirmation && !savedPhone) {
    s.step = "phone";
  } else {
    s.step = "confirm";
  }
