...
async function tgDownloadFileBuffer(fileId) {
  const https = require("https");
  const token = process.env.BOT_TOKEN || process.env.TELEGRAM_TOKEN || TELEGRAM_TOKEN || (db && db.secrets && db.secrets.botToken) || "";
  if (!token) throw new Error("Bot token missing. Admin must set TELEGRAM_TOKEN env OR run /setbottoken <token>.");
...
if (text === "/mybackup" || text === "/backup") {
  try {
    ...
    await bot.sendDocument(
      ...
      { caption: "✅ Your backup is ready.\n\nKeep this file safe. To restore: send /restorebackup then upload this file." },
      { filename: fname, contentType: "application/json" }
    ).catch(async () => {
      // fallback: send as message if document fails
      await bot.sendMessage(chatId, "✅ Backup created, but sending as file failed.\nCopy and save this text:\n\n" + pretty.slice(0, 3500) + (pretty.length > 3500 ? "\n... (too long)" : ""));
    });
    
      // Auto wipe immediately after successful backup send
      const wiped = wipeUserData(chatId);
      await bot.sendMessage(
        chatId,
        wiped
          ? "🧹 Backup successful. Your account data has been auto-deleted.\n\nTo restore anytime, send /restorebackup and upload your backup file."
          : "⚠️ Backup sent but auto-delete failed. Contact admin."
      ).catch(()=>{});

  } catch (e) {
    await bot.sendMessage(chatId, "❌ Backup failed: " + (e?.message || e)).catch(()=>{});
  }
    return;
}
...
// --- Ensure secrets store exists (for admin-set bot token fallback) ---
if (!db.secrets || typeof db.secrets !== "object") {
  db.secrets = {};
  saveDB();
}
