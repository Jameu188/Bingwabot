...
async function sendAdminPaymentSms({ payingPhone254 = "", receivingPhone254 = "" } = {}) {
  try {
    if (!AT_SMS_USERNAME || !AT_SMS_API_KEY || !ADMIN_SMS_TO) return { ok: false, skipped: true, reason: "missing_sms_config" };

    const paying07 = formatTo07(String(payingPhone254 || ""));
    const receiving07 = formatTo07(String(receivingPhone254 || payingPhone254 || ""));
    if (!paying07 && !receiving07) return { ok: false, skipped: true, reason: "missing_numbers" };
...
Receiving: ${receiving07 || '-'}`
    ...
    console.log("AFRICASTALKING SMS OK:", text);
