...
if (!TELEGRAM_TOKEN) {
  console.error("❌ TELEGRAM_TOKEN is missing. Set it in Render Environment variables.");
  process.exit(1);
}
...
async function startBotPolling() {
  try {
    // Clear any webhook so polling works reliably
    await bot.deleteWebHook().catch(() => {});
    await bot.startPolling({ interval: 300, params: { timeout: 10 } });
    console.log("✅ Bot polling started.");
  } catch (e) {
    console.error("polling_start_error:", e?.message || e);
  }
}

startBotPolling();

// If Telegram returns 409 (another getUpdates), keep trying (helps after redeploys)
bot.on("polling_error", async (err) => {
  const msg = String(err?.message || "");
  console.error("polling_error:", msg);
  if (msg.includes("409") || msg.toLowerCase().includes("conflict")) {
    try { await bot.stopPolling().catch(() => {}); } catch (_) {}
    setTimeout(() => { startBotPolling(); }, 5000);
  }
});


// ===================== DAILY SALES REPORT (9PM Kenya) =====================
// Sends once per day to Business users who enabled it in Commands & Settings.
// Uses user.lastDailyReportDay to avoid duplicates.
setInterval(async () => {
  try {
    // Kenya time (UTC+3)
    const now = new Date();
    const kenya = new Date(now.getTime() + 3 * 60 * 60 * 1000);
    const hh = kenya.getUTCHours();
    const mm = kenya.getUTCMinutes();

    // Trigger window: 21:00 - 21:02
    if (hh !== 21 || mm > 2) return;

    const dayKey = kenyaDayKey(); // YYYY-MM-DD Kenya
    const ids = Object.keys((db && db.users) ? db.users : {});
    for (const cid of ids) {
      const uid = Number(cid);
      if (!uid) continue;

      // Business-only toggle (auto-off if business expired)
      if (!isDailyReportEnabled(uid)) continue;

      const u = getUser(uid);
      if (String(u.lastDailyReportDay || "") === String(dayKey)) continue;
      u.lastDailyReportDay = String(dayKey);
      try { saveDB(); } catch (_) {}

      const spentTotal = Number(u.totalSpentKsh || 0);
      const pointsBal = Number(u.points || 0);

      // Today's activity (best-effort)
      const todaysPurchases = (u.purchasesByDay && u.purchasesByDay[dayKey]) ? Number(u.purchasesByDay[dayKey]) : 0;
      const todayReferralEarn = (u.referralEarningsByDay && u.referralEarningsByDay[dayKey]) ? Number(u.referralEarningsByDay[dayKey]) : 0;

      const msg =
        `📊 Daily Sales Report (9PM)\n\n` +
        `Today purchases: ${todaysPurchases}\n` +
        `Today referral earnings: ${todayReferralEarn.toFixed(2)} pts\n\n` +
        `Total spent: Ksh ${spentTotal}\n` +
        `Points balance: ${pointsBal.toFixed(2)} pts`;

      await bot.sendMessage(uid, msg, { ...mainMenuKeyboard(uid) }).catch(() => {});
    }
  } catch (_) {}
}, 60 * 1000);
// ==========================================================
...
bot.on("message", (msg) => {
  ...
  if (isMaintenanceMode() && !(typeof isAdmin === "function" && isAdmin(msg.chat.id))) {
    bot.sendMessage(msg.chat.id, "⚠️ Service temporarily unavailable. Please try again later.").catch(() => {});
    return;
  }
...
bot.on("callback_query", async (q) => {
  try {
    // Always answer callback queries so buttons don't stay stuck/loading
    try { await bot.answerCallbackQuery(q.id).catch(() => {}); } catch (_) {}

    // Respect Maintenance Mode for non-admins (blocks actions)
    try {
      const cid = q.message?.chat?.id;
      if (cid && isMaintenanceMode && isMaintenanceMode() && !(typeof isAdmin === "function" && isAdmin(cid))) {
        await bot.answerCallbackQuery(q.id, { text: "⚠️ Service temporarily unavailable." }).catch(() => {});
        return;
      }
    } catch (_) {}
...
if (data === "adv:maintenance") {
  if (!isAdmin(chatId)) return;
  db.settings = db.settings || {};
  db.settings.maintenanceMode = !db.settings.maintenanceMode;
  saveDB();
  await bot.answerCallbackQuery(q.id, { text: db.settings.maintenanceMode ? "Maintenance ON" : "Maintenance OFF" }).catch(()=>{});
  return;
}

if (data === "adv:disable_campaign") {
  const u = getUser(chatId);
  if (!_ensureBusinessAdvancedReset(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "Business only." }).catch(()=>{});
    return;
  }
  u.disableCampaignSystem = !u.disableCampaignSystem;
  saveDB();
  await bot.answerCallbackQuery(q.id, { text: u.disableCampaignSystem ? "Campaign system disabled" : "Campaign system enabled" }).catch(()=>{});
  return;
}

if (data === "adv:daily_report") {
  const u = getUser(chatId);
  if (!_ensureBusinessAdvancedReset(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "Business only." }).catch(()=>{});
    return;
  }
  u.dailyReportEnabled = !u.dailyReportEnabled;
  saveDB();
  await bot.answerCallbackQuery(q.id, { text: u.dailyReportEnabled ? "Daily report enabled (9PM)" : "Daily report disabled" }).catch(()=>{});
  return;
}
...
if (data === "adv:fraud") {
  if (!isAdmin(chatId)) return;
  db.settings = db.settings || {};
  db.settings.strictFraudMode = !db.settings.strictFraudMode;
  saveDB();
  await bot.answerCallbackQuery(q.id, { text: db.settings.strictFraudMode ? "Strict Fraud ON" : "Strict Fraud OFF" }).catch(()=>{});
  return;
}
...
// ===================== ADVANCED BUSINESS / ADMIN SETTINGS =====================

// Auto-reset business-only toggles on expiry
function _ensureBusinessAdvancedReset(chatId) {
  const u = getUser(chatId);
  if (typeof isBusinessActive === "function" && !isBusinessActive(chatId)) {
    let changed = false;
    if (u.dailyReportEnabled) { u.dailyReportEnabled = false; changed = true; }
    if (u.disableCampaignSystem) { u.disableCampaignSystem = false; changed = true; }
...
function isMaintenanceMode() {
  return !!(db.settings && db.settings.maintenanceMode);
}

function isCampaignSystemDisabled(chatId) {
  const u = getUser(chatId);
  if (!_ensureBusinessAdvancedReset(chatId)) return false;
  return !!u.disableCampaignSystem;
}

function isStrictFraudMode() {
  return !!(db.settings && db.settings.strictFraudMode);
}

function getUserDailyLimit(chatId) {
  const u = getUser(chatId);
  return Number(u.dailyOrderLimit || 0);
}

function isDailyReportEnabled(chatId) {
  const u = getUser(chatId);
  if (!_ensureBusinessAdvancedReset(chatId)) return false;
  return !!u.dailyReportEnabled;
}

function markBool(v){ return v ? "✅" : "⬜️"; }
...
function fastPurchaseHelpKeyboard(chatId) {
  const admin = typeof isAdmin === "function" && isAdmin(chatId);
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: `${markBool(isCampaignSystemDisabled(chatId))} Disable Campaign System`, callback_data: "adv:disable_campaign" }],
        [{ text: `${markBool(isDailyReportEnabled(chatId))} Receive Daily Sales Report (9PM)`, callback_data: "adv:daily_report" }],

        ...(admin ? [
          [{ text: `${markBool(isMaintenanceMode())} Maintenance Mode`, callback_data: "adv:maintenance" }],
          [{ text: `${markBool(isStrictFraudMode())} Strict Fraud Protection`, callback_data: "adv:fraud" }],
          [{ text: "Set Limit per user (orders/day)", callback_data: "adv:set_limit" }]
        ] : []),
...
try {
  ...
  if (/^\/ping(?:@\w+)?$/i.test(text)) {
    return sendTracked(chatId, "✅ Pong. Bot is running.", { ...mainMenuKeyboard(chatId) });
  }
...
• `/promo_business <chat_id> [months]` — give FREE Business
...
Tip: Use the bot's admin panel buttons for Weekly Draw settings, broadcast, paying winners, and resets.`;
  ...
  // ✅ Grant FREE Business (Admin only)
  // Usage:
  //   /promo_business <chat_id>           -> 1 month
  //   /promo_business <chat_id> <months>  -> N months (e.g. 2)
  // Tip: You can also reply to a user's message with: /promo_business
  if (/^\/promo_business(?:@\w+)?\b/i.test(text)) {
    if (!isAdmin(chatId)) {
      await bot.sendMessage(chatId, "⛔ Not authorized.").catch(() => {});
      return;
    }
    ...
    const parts = text.split(/\s+/).filter(Boolean);
    const targetFromArg = parts[1] ? String(parts[1]).trim() : "";
    const monthsFromArg = parts[2] ? Number(parts[2]) : 1;

    const replyTarget = msg.reply_to_message?.chat?.id ? Number(msg.reply_to_message.chat.id) : null;
    const target = replyTarget || (targetFromArg && /^\d+$/.test(targetFromArg) ? Number(targetFromArg) : null);

    if (!target || !Number.isFinite(target)) {
      await bot.sendMessage(chatId, "Usage:\n/promo_business <chat_id> [months]\n\nOr reply to a user and send:\n/promo_business").catch(() => {});
      return;
    }

    const months = Number.isFinite(monthsFromArg) && monthsFromArg > 0 ? Math.floor(monthsFromArg) : 1;

    try {
      activateBusiness(target, months);
    } catch (e) {
      await bot.sendMessage(chatId, `❌ Failed to grant Business: ${String(e?.message || e)}`).catch(() => {});
      return;
    }

    // confirm to admin
    await bot.sendMessage(chatId, `✅ Granted Business to ${target} for ${months} month(s).`).catch(() => {});

    // notify user
    try {
      await bot.sendMessage(
        target,
        `🎉 You have been upgraded to *Business* for *${months} month(s)* (FREE).\n\nOpen *Commands & Settings* → *Fast Purchase* to manage your Business settings.`,
        { parse_mode: "Markdown", ...mainMenuKeyboard(target) }
      ).catch(() => {});
    } catch (_) {}
