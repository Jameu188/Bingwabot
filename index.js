...
// ===================== 🧹 AUTO DELETE SETTINGS (HELP) =====================
if (data === "ad:open") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const res = autoDeleteSettingsMessage(chatId);
  const mid = q.message?.message_id;
  if (mid) {
    const edited = await safeEditMessageText(chatId, mid, res.text, res.opts);
    if (edited) return;
  }
...
if (data.startsWith("ad:set:")) {
  const ms = Number(data.split(":")[2] || "");
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const u = getUser(chatId);
  if (!isBusiness(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "🔒 Business only. Type /sell", show_alert: true }).catch(() => {});
    return;
  }
...
if (data === "ad:back") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const mid = q.message?.message_id;
  return sendHelpAnswer(chatId, "cmds", { messageId: mid });
}

// ===================== 💼 BUSINESS SETTINGS (HELP) =====================
if (data === "biz:open") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const res = businessSettingsMessage(chatId);
  const mid = q.message?.message_id;
  if (mid) {
    const edited = await safeEditMessageText(chatId, mid, res.text, res.opts);
    if (edited) return;
...
if (data === "fp:toggle") {
  ...
  if (!isBusinessActive(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "💼 Business only. Type /sell to upgrade.", show_alert: true }).catch(() => {});
    return;
  }
  ...
  // Refresh the Commands & Settings help screen so the mark updates immediately
  return sendHelpAnswer(chatId, "cmds", { messageId: mid });
}
...
case "cmds":
  return (
    ...
    "• /sell — business upgrade / subscription\n\n" +
    "*Other commands:*\n" +
    "• /changenumber — save/change your phone number\n\n" +
...
async function sendHelpAnswer(chatId, key, ctx = {}) {
  ...
  const opts = key === "cmds" ? { parse_mode: "Markdown", ...cmdsSettingsKeyboard(chatId) } : { parse_mode: "Markdown", ...helpAnswerKeyboard() };
...
function fastPurchaseHelpKeyboard(chatId) {
  ...
  return cmdsSettingsKeyboard(chatId);


function cmdsSettingsKeyboard(chatId) {
  const u = getUser(chatId);

  const isBiz = isBusinessActive(chatId); // active subscription required for fast purchase + biz settings
  const isBizAny = isBusiness(chatId);

  const rows = [
    [{ text: `${fastPurchaseToggleMark(chatId)} Fast Purchase`, callback_data: "fp:toggle" }],
    [{ text: "🧹 Auto Delete", callback_data: "ad:open" }],
    [
      { text: `${markBox(isBizAny && !!u.stkAutoRetry)} Auto-retry STK`, callback_data: "biz:stk:toggle" },
      { text: `${markBox(isBizAny && !!u.skipConfirmation)} Skip confirm`, callback_data: "biz:qb:skip" },
    ],
    [{ text: "⚙️ More Business Settings", callback_data: "biz:open" }],
    [{ text: "⬅️ Back to Help", callback_data: "help:menu" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
  ];

  // If not business, still show buttons (they will explain), but no mark
  if (!isBizAny) {
    rows[2][0].text = `⬜️ Auto-retry STK`;
    rows[2][1].text = `⬜️ Skip confirm`;
  }

  return { reply_markup: { inline_keyboard: rows } };
}

// ===== 🧹 Auto Delete Settings (inside Help) =====
function autoDeleteLabel(ms) {
  if (!ms || ms === 24 * 60 * 60 * 1000) return "24hr";
  if (ms === 10 * 60 * 1000) return "10min";
  if (ms === 60 * 60 * 1000) return "1h";
  return `${Math.round(ms / 60000)}min`;
}

function autoDeleteSettingsMessage(chatId) {
  const u = getUser(chatId);
  if (!isBusiness(chatId)) {
    return {
      text: "🔒 Auto Delete is available for Business accounts only.\nType /sell to upgrade.",
      opts: {
        reply_markup: {
          inline_keyboard: [
            [{ text: "💼 Upgrade to Business", callback_data: "go_sell" }],
            [{ text: "⬅️ Back", callback_data: "ad:back" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
          ],
        },
      },
    };
  ...
  const cur = autoDeleteLabel(u.autoDeleteMs);
  const text =
    "🧹 *Auto Delete*\n\n" +
    `Current: *${cur}*\n\n` +
    "Choose how long bot messages should stay before auto-deleting.\n" +
    "Note: Telegram bots can only delete *bot messages* (not your typed messages).";

  const rows = [
    [
      { text: `${markBox(u.autoDeleteMs === 10 * 60 * 1000)} 10min`, callback_data: "ad:set:600000" },
      { text: `${markBox(u.autoDeleteMs === 60 * 60 * 1000)} 1h`, callback_data: "ad:set:3600000" },
    ],
    [
      { text: `${markBox(!u.autoDeleteMs || u.autoDeleteMs === 24 * 60 * 60 * 1000)} 24hr`, callback_data: "ad:set:86400000" },
      { text: `${markBox(!u.autoDeleteMs || u.autoDeleteMs === 24 * 60 * 60 * 1000)} OFF`, callback_data: "ad:set:86400000" },
    ],
    [{ text: "⬅️ Back", callback_data: "ad:back" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
  ];
...
function businessSettingsMessage(chatId) {
  const u = getUser(chatId);
  if (!isBusiness(chatId)) {
    return {
      ...
      opts: {
        reply_markup: {
          inline_keyboard: [
            [{ text: "💼 Upgrade to Business", callback_data: "go_sell" }],
            [{ text: "⬅️ Back", callback_data: "help:cmds" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
          ],
        },
      },
    };
  ...
  const rows = [
    ...
    [{ text: `${markBox(!!u.repeatLastButton)} Repeat last purchase button`, callback_data: "biz:qb:repeat" }],
    [{ text: "⬅️ Back", callback_data: "help:cmds" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
  ];
  ...
  const text =
    ...
    `• Repeat last purchase button: ${u.repeatLastButton ? "ON" : "OFF"}`;
