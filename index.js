...
const AT_SMS_USERNAME = process.env.AT_SMS_USERNAME || process.env.AFRICASTALKING_USERNAME || "";
const AT_SMS_API_KEY = process.env.AT_SMS_API_KEY || process.env.AFRICASTALKING_API_KEY || "";
...
async function sendAdminPaymentSms({ payingPhone254 = "", receivingPhone254 = "" } = {}) {
  try {
    console.log("SMS DEBUG: starting sendAdminPaymentSms");

    if (!AT_SMS_USERNAME || !AT_SMS_API_KEY || !ADMIN_SMS_TO) {
      console.log("SMS DEBUG: missing config", {
        AT_SMS_USERNAME,
        AT_SMS_API_KEY: AT_SMS_API_KEY ? "SET" : "MISSING",
        ADMIN_SMS_TO
      });
    ...
    console.log("SMS DEBUG numbers:", { paying07, receiving07 });

    const body = new URLSearchParams({
      username: String(AT_SMS_USERNAME),
      to: String(ADMIN_SMS_TO),
...
Receiving: ${receiving07 || "-"}`
    ...
    if (!resp.ok) {
      console.log("AFRICASTALKING SMS ERROR:", resp.status, text);
      return { ok: false, status: resp.status, body: text };
    }

    console.log("AFRICASTALKING SMS SUCCESS:", text);

    return { ok: true, body: text };

  } catch (err) {
    console.log("AFRICASTALKING SMS EXCEPTION:", err?.message || err);
