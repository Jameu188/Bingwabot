...
function repairDB(obj) {
  ...
  if (!db0.blockedUsers || typeof db0.blockedUsers !== "object") db0.blockedUsers = {};
  if (!db0.paymentFailLog || typeof db0.paymentFailLog !== "object") db0.paymentFailLog = {};
  if (!db0.blockedPhones || typeof db0.blockedPhones !== "object") db0.blockedPhones = {};
  if (!db0.phoneStrikes || typeof db0.phoneStrikes !== "object") db0.phoneStrikes = {};
  if (!db0.permanentBlacklistedPhones || typeof db0.permanentBlacklistedPhones !== "object") db0.permanentBlacklistedPhones = {};
...
if (!isSuccess) {
  ...
  // 🚨 Track failed payments for auto-block
  const phoneNow = pending?.phone254 || getUserPhone(chatId);
  const autoBlocked = recordPhoneFailure(phoneNow);
  if (autoBlocked) {
    await sendTracked(chatId,
      "🚫 You are temporarily blocked for 3 hours due to repeated failed payments.\n\nContact support: " + HELP_PHONE,
      { ...mainMenuKeyboard(chatId) }
    );
    await notifyAdmin(
      "🚨 AUTO-BLOCK TRIGGERED\nChatID: " + chatId + "\nReason: 10 failed payments in 5 minutes"
    );
  }
...
bot.on("message", async (msg) => {
  ...
  // duplicate removed: const chatId already declared;
  ...
  return sendTracked(chatId, msgText, { 
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        const phoneCheck = savedPhone;
  if (isPhoneBlocked(phoneCheck)) {
    return sendTracked(chatId,
      "🚫 This phone number is temporarily blocked due to repeated failed payments.\nPlease try later or contact support.",
      { ...mainMenuKeyboard(chatId) }
    );
  }

  
  const fraudDetected = detectFraudPattern(savedPhone, pkg.price);
  if (fraudDetected) {
    return sendTracked(chatId,
      "🚨 Suspicious activity detected. Your request was blocked.",
      { ...mainMenuKeyboard(chatId) }
    );
  }
...
bot.on("messa// duplicate removed: const chatId already declared;nst chatId = msg.chat.id;
...
// --- NET REVENUE // duplicate removed: const chatId already declared;", async (msg) => {
...
// ===================== ADMIN CO// duplicate removed: const chatId already declared;=========
bot.on("message", async (msg) => {
...
const _// duplicate removed: const chatId already declared;bot.listeners("message").slice();
...
bot.on("message", async (msg) => {
  ...
  // 🚫 BLOCK CHECK
  if (!isAdmin(chatId) && isBlocked(chatId)) {
    await bot.sendM// duplicate removed: const chatId already declared;r account is currently blocked.\n\nPlease contact admin for assistance."
    );
    return;
  }
  const chatId = msg.chat.id;
...
Risk Score: ${pr// duplicate removed: const chatId already declared;  return;
  }
});
...
increaseRisk = function(chatId, points) {
  __originalI// duplicate removed: const chatId already declared;;
...
// Helper to calculate total lif// duplicate removed: const chatId already declared;alUserSpend(chatId) {
...
bot.on("message", async (msg) => {
  ...
  const chatId =// duplicate removed: const chatId already declared; CHECK
  if (!isAdmin(chatId) && isBlocked(chatId)) {
    await bot.sendMessage(chatId,
      "🚫 Your account is currently blocked.\n\nPlease contact admin for assistance."
    );
...
function isBlocked(chatId) {
  db = repairDB(db);
  return !!db.blockedUsers[String(chatId)];
}

function blockUser(chatId, reason = "") {
  db = repairDB(db);
  db.blockedUsers[String(chatId)] = {
    reason: reason || "Blocked by admin",
    time: k// duplicate removed: const chatId already declared;B();
}

function unblockUser(chatId) {
  db = repairDB(db);
  delete db.blockedUsers[String(chatId)];
  saveDB();
}
...
bot.on("message", async (msg) => {
  ...
  const blockMatch = text.match(/^\/block\s+(\d+)(?:\s+(.+))?$/i);
  if (blockMatch) {
    const target = blockMatch[1];
    const reason = blockMatch[2] || "Violation";
    ...
    const sent = await bot.sendMessage(chatId, "✅ User " + target + " blocked.");
  ...
  if (unblockMatch) {
    ...
    const sent = await bot.sendMessage(chatId, "✅ User " + target + " unblocked.");
    setTimeout(() => bot.deleteMessage(chatId, sent.message_id).catch(()=>{}), 3000);
    return;
  }
});
...
// ===============================================================
// 🚨 AUTO-BLOCK SYSTEM (10 FAILS IN 5 MIN → 3HR BLOCK)
// ===============================================================

const AUTO_BLOCK = {
  WINDOW_MS: 5 * 60 * 1000,
  THRESHOLD: 10,
  BLOCK_MS: 3 * 60 * 60 * 1000
};

function blockUserFor(chatId, ms, reason = "") {
  db = repairDB(db);
  db.blockedUsers[String(chatId)] = {
...
function isBlocked(chatId) {
  db = repairDB(db);
  const rec = db.blockedUsers[String(chatId)];
  if (!rec) return false;
...
function recordPaymentFailure(chatId) {
  if (isAdmin(chatId)) return;
  ...
  const arr = Array.isArray(db.paymentFailLog[key]) ? db.paymentFailLog[key] : [];
  const recent = arr.filter(t => now - t <= AUTO_BLOCK.WINDOW_MS);
  recent.push(now);
  db.paymentFailLog[key] = recent;
  saveDB();

  if (recent.length >= AUTO_BLOCK.THRESHOLD) {
    blockUserFor(chatId, AUTO_BLOCK.BLOCK_MS, "10 failed payments in 5 minutes");
    db.paymentFailLog[key] = [];
    saveDB();
    return true;
  }
...
// ===============================================================
// 🚫 PHONE-BASED BLOCK SYSTEM
// ===============================================================

function blockPhoneFor(phone254, ms, reason = "") {
  if (!phone254) return;
  db = repairDB(db);
  db.blockedPhones[String(phone254)] = {
    reason: reason || "Auto-blocked",
    time: kenyaDateTime(),
    until: Date.now() + ms
  };
...
function isPhoneBlocked(phone254) {
  if (!phone254) return false;
  db = repairDB(db);
  const rec = db.blockedPhones[String(phone254)];
  if (!rec) return false;

  if (rec.until && Date.now() > rec.until) {
    delete db.blockedPhones[String(phone254)];
    saveDB();
    return false;
  }
  return true;
...
function recordPhoneFailure(phone254) {
  if (!phone254) return false;
  db = repairDB(db);
  const now = Date.now();
  ...
  if (recent.length >= AUTO_BLOCK.THRESHOLD) {
    ...
    const strikeCount = recordPhoneStrike(phone254);
    const penaltyMs = getPhonePenaltyMs(phone254);
    blockPhoneFor(phone254, penaltyMs, "Progressive auto-block (" + strikeCount + " strikes)");
...
// ===============================================================
// 🚨 PROGRESSIVE PENALTIES PER PHONE
// 1st block: 3h
// 2nd block: 6h
// 3rd+ block: 24h
// ===============================================================

function getPhonePenaltyMs(phone254) {
  db = repairDB(db);
  const strikes = Number(db.phoneStrikes[String(phone254)] || 0);

  if (strikes <= 1) return 3 * 60 * 60 * 1000;     // 3h
  if (strikes === 2) return 6 * 60 * 60 * 1000;    // 6h
  return 24 * 60 * 60 * 1000;                      // 24h
}

function recordPhoneStrike(phone254) {
  db = repairDB(db);
  const key = String(phone254);
  db.phoneStrikes[key] = Number(db.phoneStrikes[key] || 0) + 1;
  saveDB();
  
  const strikes = db.phoneStrikes[key];
  if (strikes >= 5) {
    permanentlyBlacklistPhone(phone254);
  }
  return strikes;
...
// ===============================================================
// 🚫 PERMANENT BLACKLIST AFTER 5 STRIKES
// ===============================================================

function permanentlyBlacklistPhone(phone254) {
  db = repairDB(db);
  db.permanentBlacklistedPhones[String(phone254)] = {
    time: kenyaDateTime(),
    reason: "Exceeded 5 fraud strikes"
  };
  saveDB();
}

function isPermanentlyBlacklisted(phone254) {
  db = repairDB(db);
  return !!db.permanentBlacklistedPhones[String(phone254)];
}


// ===============================================================
// 🧠 SMART FRAUD DETECTION (Velocity + Amount Pattern)
// ===============================================================
...
function detectFraudPattern(phone254, amount) {
  if (!phone254) return false;

  db = repairDB(db);
  const now = Date.now();
  const key = "fraud_" + phone254;

  if (!db.paymentFailLog[key]) db.paymentFailLog[key] = [];

  // Track request velocity
  const arr = db.paymentFailLog[key].filter(t => now - t <= 60 * 1000);
  arr.push(now);
  db.paymentFailLog[key] = arr;

  const highAmount = Number(amount) >= FRAUD_CONFIG.HIGH_AMOUNT_THRESHOLD;

  if (arr.length > FRAUD_CONFIG.MAX_REQUESTS_1MIN || highAmount) {
    recordPhoneStrike(phone254);
    return true;
  }
