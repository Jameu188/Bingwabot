...
async function broadcastWeeklyDrawLaunch(force = false) {
  ...
  // Per-run de-dupe (prevents the same target being sent twice in a single broadcast call)
  const _sentTargets = new Set();
  ...
  async function sendToChat(targetId) {
    ...
    if (_sentTargets.has(tkey)) return;
    _sentTargets.add(tkey);
    try {
      if (img) {
  ...
  const chKey = String(FORCE_JOIN_CHANNEL || "");
  for (const cid of userIds) {
    if (!cid) continue;
    // Prevent channel from being sent again via the users loop
    if (chKey && String(cid) === chKey) continue;
