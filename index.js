...
function shouldAutoDeleteOutbound(textOrCaption, options = {}) {
  const t = String(textOrCaption || "");
  if (_isProtectedAnnouncementText(t)) return false;

  // Auto-delete "button messages" (inline/reply keyboards), except protected announcements
  const rm = options && options.reply_markup ? options.reply_markup : null;
  if (rm && (rm.inline_keyboard || rm.keyboard)) return true;

  // Auto-delete prompts expecting user input
  if (/^📱\s*Enter\b/i.test(t)) return true;
  if (/^\s*Send\b/i.test(t) && !/^\s*Send\s+help/i.test(t)) return true;
  if (/\bEnter\s+M-?PESA\b/i.test(t)) return true;
...
try {
  ...
  bot.sendMessage = async (chatId, text, options = {}) => {
    ...
    if (shouldAutoDeleteOutbound(text, options)) scheduleAutoDelete(m);
    return m;
  };
  ...
  bot.sendPhoto = async (chatId, photo, options = {}) => {
    ...
    if (shouldAutoDeleteOutbound(cap, options)) scheduleAutoDelete(m);
    return m;
  };
} catch (_) {}
...
if (isStartCmd) {
  const startArgs = text.replace(/^\/start\b/i, "").trim();
