...
// ===================== /SET ADMIN PANEL (FULL FINAL) =====================

// Helper buttons
function setBackMenuKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [
...
// Show categories
function showSetCategories(chatId) {
  return bot.sendMessage(chatId, "⚙️ Select category to update payment ID:", {
    reply_markup: {
      inline_keyboard: [
...
bot.onText(/\/set/, async (msg) => {
  ...
  return showSetCategories(chatId);
});
...
bot.on("callback_query", async (q) => {
  ...
  // MAIN MENU
  if (data === "set:home") {
    return bot.sendMessage(chatId, "🏠 Main Menu", mainMenuKeyboard(chatId));
  }
  ...
  if (data.startsWith("setcat:")) {
    if (Number(chatId) !== Number(ADMIN_ID)) {
      return bot.answerCallbackQuery(q.id, { text: "Not allowed", show_alert: true });
    }
...
// HANDLE ADMIN INPUT CLEANLY (NO DUPLICATES)
bot.on("message", async (msg) => {
  const chatId = msg.chat.id;
  const text = (msg.text || "").trim();
  ...
  if (Number(chatId) === Number(ADMIN_ID) && u.pendingAction?.type === "SET_PAYMENT_ID") {
    const category = u.pendingAction.category;

    db = repairDB(db);
    db.paymentIds = db.paymentIds || {};

    db.paymentIds[category] = text;
