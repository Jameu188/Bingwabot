...
function getUser(chatId) {
  ...
  if (!Number.isFinite(Number(db.users[cid].cashbackEarnedTotalPts))) db.users[cid].cashbackEarnedTotalPts = 0;
...
try {
  ...
  try {
    const u = getUser(chatId);
    u.cashbackEarnedTotalPts = Number(u.cashbackEarnedTotalPts || 0) + Number(cashbackPts || 0);
    saveDB();
  } catch (_) {}
...
if (pending && pending.category === "Business Account") {
  ...
  if (plan) {
    ...
    const transactionDateTime = kenyaDateTimeFromTs(whenTs);
    const totalCashbackEarnedPts = Number(getUser(chatId)?.cashbackEarnedTotalPts || 0);
    const totalCashbackEarnedKes = Number(pointsToKes(totalCashbackEarnedPts) || 0).toFixed(2);
...
` +
      `💵 Total Cashback Earned So Far: *Ksh ${totalCashbackEarnedKes}* (${totalCashbackEarnedPts.toFixed(2)} pts)
` +
