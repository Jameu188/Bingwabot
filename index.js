...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // externalRef mapping (PayHero may use many key names / nesting)
    const externalRef =
    ...
    // ✅ Resolve pending payment details (robust):
    // PayHero sometimes returns a slightly different reference key than what we stored.
    // If we can't find by externalRef, fall back to the latest non-expired pending payment for this chatId.
    let pending = getPendingPayment(externalRef);
    let pendingRef = externalRef;
    if (!pending) {
      const alt = getLatestPendingForChat(chatId);
      if (alt && alt.ref) {
        pending = alt;
        pendingRef = String(alt.ref);
        console.log("⚠️ Pending not found by externalRef, using latest pending for chat:", pendingRef);
      } else {
        console.log("⚠️ Pending not found for externalRef or chatId:", externalRef, chatId);
      }
    }
    ...
    if (pending) {
      ...
      if (!amountOk || !phoneOk) {
        ...
        if (!alreadyProcessed(failKey)) {
          markProcessed(failKey);
          deletePendingPayment(pendingRef);
...
if (pending && pending.category === "Business Account") {
  ...
  if (plan) {
    ...
    const amountPaid = pending ? Number(pending.price || 0) : cbAmount;
    const packageName = pending?.pkgLabel || (amountPaid > 0 ? "Payment" : "Unknown Package");
...
// Inline action buttons (Upgrade + Buy Last Offer + Buy Offers)
    ...
    // ✅ Notify user (we will EDIT this same message to append WEEKLY DRAW, so user only sees 1 message)
    let __payMsgOpts = null;
    let __payMsgSent = null;

    if (pending?.category === "Business Account") {
      // Business: no inline buttons, no data upsell recommendations
      __payMsgOpts = { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) };
      __payMsgSent = await sendTracked(chatId, successMsg, __payMsgOpts);
    } else {
      __payMsgOpts = {
        parse_mode: "Markdown",
...
` +
    ...
    // ✅ Append WEEKLY DRAW UPDATE into the SAME payment-confirmation message (edit in place)
    if (isWeeklyDrawOptedIn(chatId) && __payMsgSent && __payMsgSent.message_id) {
      const combinedMsg = `${successMsg}

${msg}`;
      await bot.editMessageText(combinedMsg, {
        chat_id: chatId,
        message_id: __payMsgSent.message_id,
        ...(typeof __payMsgOpts === "object" ? __payMsgOpts : { parse_mode: "Markdown" })
      }).catch(() => {});
    }
