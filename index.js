...
function awardSuccessPurchasePoints(chatId, pending) {
  ...
  // 20 SMS -> +5 pts
  if (label.includes("20 sms")) addPoints(chatId, 5);

  // 250MB -> +20 pts
  if (label.includes("250mb")) addPoints(chatId, 20);
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // ✅ SUCCESS RULE (matches your "ResultCode/Status true = success" and "missing = fail"):
    // - If ResultCode exists -> success only when 0
    // - Else if Status === true/"true"/1/"1" -> success
    // - Else if MpesaReceiptNumber exists -> success
    // - Else if Woo status is completed/processing -> success
    // - Else -> fail (especially when ResultCode/Status is empty)
    let isSuccess = false;
    ...
    // If failed/cancelled -> SEND FAILURE MESSAGE ONCE
    if (!isSuccess) {
      ...
      // Optional: admin failure log (commented out)
      // await notifyAdmin(
      //   `❌ *Payment Failed*\n` +
