...
async function broadcastWeeklyDrawLaunch(force = false) {
  ...
  // Send to channel
  try { await sendToChat(FORCE_JOIN_CHANNEL); } catch (_) {}
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: "🎲 Pick Winners", callback_data: "wdadm:pick" }],
...
function getUser(chatId) {
  ...
  if (!db.users[cid]) {
    db.users[cid] = {
      ...
      // ⚙️ User Settings
      notifications: { weekly: true, broadcasts: true, payments: true },
      wdReminderMs: 24 * 60 * 60 * 1000,
      defaultNumber: "",
      quickMode: false,
      silentMode: false,
      pinHash: "",
...
if (db.users[cid].fraudScore === undefined) db.users[cid].fraudScore = 0;
if (db.users[cid].lastReferralNudgeAt === undefined) db.users[cid].lastReferralNudgeAt = 0;
if (db.users[cid].lastLoginBonusDay === undefined) db.users[cid].lastLoginBonusDay = "";
if (db.users[cid].failRecoveryBonus === undefined) db.users[cid].failRecoveryBonus = 0;
if (db.users[cid].streak === undefined) db.users[cid].streak = { lastDay: "", count: 0 };
if (db.users[cid].lastTicketAt === undefined) db.users[cid].lastTicketAt = 0;
if (db.users[cid].archived === undefined) db.users[cid].archived = false;
if (db.users[cid].lastWithdrawAt === undefined) db.users[cid].lastWithdrawAt = 0;

  if (db.users[cid].notifications === undefined) db.users[cid].notifications = { weekly: true, broadcasts: true, payments: true };
  if (db.users[cid].wdReminderMs === undefined) db.users[cid].wdReminderMs = 24 * 60 * 60 * 1000;
  if (db.users[cid].defaultNumber === undefined) db.users[cid].defaultNumber = "";
  if (db.users[cid].quickMode === undefined) db.users[cid].quickMode = false;
  if (db.users[cid].silentMode === undefined) db.users[cid].silentMode = false;
  if (db.users[cid].pinHash === undefined) db.users[cid].pinHash = "";
  if (db.users[cid].hideLeaderboard === undefined) db.users[cid].hideLeaderboard = false;
  if (db.users[cid].theme === undefined) db.users[cid].theme = "detailed";
...
function touchLastSeen(chatId) {
  const u = getUser(chatId);
  u.lastSeen = Date.now();
  updateLoginBonus(chatId);
  maybeArchiveUser(chatId);
...
bot.on("callback_query", async (q) => {
  try {
    ...
    const chatId = (q && q.message && q.message.chat && q.message.chat.id) ? q.message.chat.id : (q && q.from ? q.from.id : null);
    const messageId = (q && q.message) ? q.message.message_id : null;
    if (!chatId) { await bot.answerCallbackQuery(q.id).catch(() => {}); return; }
...
if (data === "set:menu") {
  ...
  try {
    await bot.editMessageText(settingsMenuText(chatId), {
      chat_id: q.message.chat.id,
      message_id: q.message.message_id,
      parse_mode: "Markdown",
      ...settingsMenuKeyboard(chatId),
    });
  } catch (_) {
    await bot.sendMessage(chatId, settingsMenuText(chatId), { parse_mode: "Markdown", ...settingsMenuKeyboard(chatId) }).catch(() => {});
  }
  return;
}

if (data.startsWith("set:notif_")) {
  const u = getUser(chatId);
  u.notifications = u.notifications || { weekly: true, broadcasts: true, payments: true };
  if (data === "set:notif_weekly") u.notifications.weekly = !u.notifications.weekly;
  if (data === "set:notif_broadcasts") u.notifications.broadcasts = !u.notifications.broadcasts;
  if (data === "set:notif_payments") u.notifications.payments = !u.notifications.payments;
  try { saveDB(); } catch (_) {}
  await bot.answerCallbackQuery(q.id, { text: "✅ Updated" }).catch(() => {});
  try {
    await bot.editMessageText(settingsMenuText(chatId), {
      chat_id: q.message.chat.id,
      message_id: q.message.message_id,
...
if (data === "set:quickmode") {
  const u = getUser(chatId);
  u.quickMode = !u.quickMode;
  try { saveDB(); } catch (_) {}
  await bot.answerCallbackQuery(q.id, { text: u.quickMode ? "Quick Mode ON" : "Quick Mode OFF" }).catch(() => {});
  try {
    await bot.editMessageText(settingsMenuText(chatId), {
      chat_id: q.message.chat.id,
      message_id: q.message.message_id,
...
if (data === "set:silentmode") {
  const u = getUser(chatId);
  u.silentMode = !u.silentMode;
  try { saveDB(); } catch (_) {}
  await bot.answerCallbackQuery(q.id, { text: u.silentMode ? "Silent Mode ON" : "Silent Mode OFF" }).catch(() => {});
  try {
    await bot.editMessageText(settingsMenuText(chatId), {
      chat_id: q.message.chat.id,
      message_id: q.message.message_id,
...
if (data === "set:leaderboard") {
  const u = getUser(chatId);
  u.hideLeaderboard = !u.hideLeaderboard;
  try { saveDB(); } catch (_) {}
  await bot.answerCallbackQuery(q.id, { text: u.hideLeaderboard ? "Hidden from leaderboard" : "Visible on leaderboard" }).catch(() => {});
  try {
    await bot.editMessageText(settingsMenuText(chatId), {
      chat_id: q.message.chat.id,
      message_id: q.message.message_id,
...
if (data === "set:theme") {
  const u = getUser(chatId);
  u.theme = (u.theme === "simple") ? "detailed" : "simple";
  try { saveDB(); } catch (_) {}
  await bot.answerCallbackQuery(q.id, { text: `Theme: ${u.theme}` }).catch(() => {});
  try {
    await bot.editMessageText(settingsMenuText(chatId), {
      chat_id: q.message.chat.id,
      message_id: q.message.message_id,
...
if (data === "set:autodelete") {
  setSettingsState(chatId, { mode: "autodelete" });
  await bot.answerCallbackQuery(q.id, { text: "Send time e.g. 10min, 1h, 24hr" }).catch(() => {});
  await bot.sendMessage(chatId, "🧹 Send auto-delete time (examples: 1s, 10min, 1h, 24hr). Max 24hr. Send /cancel to stop.", mainMenuKeyboard(chatId)).catch(() => {});
  return;
}

if (data === "set:default_number") {
  setSettingsState(chatId, { mode: "default_number" });
  await bot.answerCallbackQuery(q.id, { text: "Send M-PESA number" }).catch(() => {});
  await bot.sendMessage(chatId, "📱 Send your default M-PESA phone number (e.g. 07XXXXXXXX). Send /cancel to stop.", mainMenuKeyboard(chatId)).catch(() => {});
  return;
}

if (data === "set:pin") {
  setSettingsState(chatId, { mode: "pin" });
  await bot.answerCallbackQuery(q.id, { text: "Send new PIN" }).catch(() => {});
  await bot.sendMessage(chatId, "🔐 Send a 4-6 digit PIN. Send /cancel to stop.", mainMenuKeyboard(chatId)).catch(() => {});
  return;
}

if (data === "set:admin_multiplier") {
  if (!isAdmin(chatId)) { await bot.answerCallbackQuery(q.id, { text: "Not authorized", show_alert: true }).catch(() => {}); return; }
  setSettingsState(chatId, { mode: "admin_multiplier" });
  await bot.answerCallbackQuery(q.id, { text: "Send: 2x 1h (multiplier and duration)" }).catch(() => {});
  await bot.sendMessage(chatId, "🎟 Send ticket multiplier like: `2x 1h` or `3x 30min` (max 24hr). Send 1x 0 to disable.", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
  return;
}

if (data === "set:admin_withdraw_cd") {
  if (!isAdmin(chatId)) { await bot.answerCallbackQuery(q.id, { text: "Not authorized", show_alert: true }).catch(() => {}); return; }
  setSettingsState(chatId, { mode: "admin_withdraw_cd" });
  await bot.answerCallbackQuery(q.id, { text: "Send time e.g. 24hr" }).catch(() => {});
  await bot.sendMessage(chatId, "🏧 Send withdraw cooldown (e.g. 24hr, 12h, 6h). Max 24hr.", mainMenuKeyboard(chatId)).catch(() => {});
  return;
}
...
if (!isSuccess) {
  ...
  try { const u = getUser(chatId); u.failRecoveryBonus = Number(u.failRecoveryBonus||0) + 1; saveDB(); } catch(_) {}
  await sendTracked(chatId, out + "\n\n🎟 Retry now and get +1 extra ticket when payment succeeds!", { ...mainMenuKeyboard(chatId) });
  return;
}
...
const _setState = getSettingsState(chatId);
if (_setState && _setState.mode) {
  const mode = _setState.mode;

  if (/^\s*\/cancel\s*$/i.test(text)) {
    setSettingsState(chatId, null);
    await bot.sendMessage(chatId, "❌ Cancelled.", mainMenuKeyboard(chatId)).catch(() => {});
    return;
  }
  ...
  if (mode === "autodelete") {
    ...
    if (!ms) {
      await bot.sendMessage(chatId, "❌ Invalid. Use: 1s, 10min, 1h, 24hr (max 24hr).", mainMenuKeyboard(chatId)).catch(() => {});
      return;
    }
    const u = getUser(chatId);
    u.autoDeleteMs = ms;
    try { saveDB(); } catch (_) {}
    setSettingsState(chatId, null);
    await bot.sendMessage(chatId, `✅ Auto-delete set to *${formatMsShort(ms)}*`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
    return;
  }

  if (mode === "default_number") {
    const num = String(text || "").trim();
    if (!/^0\d{9}$/.test(num)) {
      await bot.sendMessage(chatId, "❌ Invalid number. Example: 07XXXXXXXX", mainMenuKeyboard(chatId)).catch(() => {});
      return;
    }
    const u = getUser(chatId);
    u.defaultNumber = num;
    try { saveDB(); } catch (_) {}
    setSettingsState(chatId, null);
    await bot.sendMessage(chatId, `✅ Default number saved: *${num}*`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
    return;
  }
  ...
  if (mode === "pin") {
    ...
    if (!/^\d{4,6}$/.test(pin)) {
      await bot.sendMessage(chatId, "❌ PIN must be 4-6 digits.", mainMenuKeyboard(chatId)).catch(() => {});
      return;
    }
    const u = getUser(chatId);
    ...
    try { saveDB(); } catch (_) {}
    setSettingsState(chatId, null);
    await bot.sendMessage(chatId, "✅ PIN set successfully.", mainMenuKeyboard(chatId)).catch(() => {});
    return;
  }
}

if (mode === "admin_refbonus") {
  if (!isAdmin(chatId)) { setSettingsState(chatId, null); return; }
  const v = Number(String(text || "").trim());
  if (!Number.isFinite(v) || v < 0 || v > 100) {
    await bot.sendMessage(chatId, "❌ Invalid. Send a number 0 - 100.", mainMenuKeyboard(chatId)).catch(() => {});
    return;
  }
  db.botConfig = db.botConfig || { referralBonusPct: 0, broadcastTarget: "all", buttonCooldownMs: 3000, ticketMultiplier: 1, ticketMultiplierUntil: 0, withdrawCooldownMs: 24 * 60 * 60 * 1000 };
  db.botConfig.referralBonusPct = v;
  try { saveDB(); } catch (_) {}
  setSettingsState(chatId, null);
  await bot.sendMessage(chatId, `✅ Referral bonus set to ${v.toFixed(2)}%`, mainMenuKeyboard(chatId)).catch(() => {});
  return;
}

if (mode === "admin_bcast_target") {
  if (!isAdmin(chatId)) { setSettingsState(chatId, null); return; }
  const v = String(text || "").trim().toLowerCase();
  if (!["all", "business", "weekly"].includes(v)) {
    await bot.sendMessage(chatId, "❌ Invalid. Use: all | business | weekly", mainMenuKeyboard(chatId)).catch(() => {});
    return;
  }
  db.botConfig = db.botConfig || { referralBonusPct: 0, broadcastTarget: "all", buttonCooldownMs: 3000 };
  db.botConfig.broadcastTarget = v;
  try { saveDB(); } catch (_) {}
  setSettingsState(chatId, null);
  await bot.sendMessage(chatId, `✅ Broadcast target set to ${v}`, mainMenuKeyboard(chatId)).catch(() => {});
  return;
}

if (mode === "admin_cooldown") {
  if (!isAdmin(chatId)) { setSettingsState(chatId, null); return; }
  const sec = Number(String(text || "").trim());
  if (!Number.isFinite(sec) || sec < 1 || sec > 10) {
    await bot.sendMessage(chatId, "❌ Invalid. Send seconds 1 - 10.", mainMenuKeyboard(chatId)).catch(() => {});
    return;
  }
  db.botConfig = db.botConfig || { referralBonusPct: 0, broadcastTarget: "all", buttonCooldownMs: 3000 };
  db.botConfig.buttonCooldownMs = Math.round(sec * 1000);
  try { saveDB(); } catch (_) {}
  setSettingsState(chatId, null);
  await bot.sendMessage(chatId, `✅ Button cooldown set to ${Math.round(sec)}s`, mainMenuKeyboard(chatId)).catch(() => {});
  return;
}



if (mode === "admin_multiplier") {
  if (!isAdmin(chatId)) { setSettingsState(chatId, null); return; }
  const s = String(text || "").trim().toLowerCase();
  // expects: "2x 1h" or "3x 30min" or "1x 0"
  const mm = s.match(/^([0-9]+)\s*x\s*(.*)$/i);
  if (!mm) { await bot.sendMessage(chatId, "❌ Format: 2x 1h (or 1x 0 to disable)", mainMenuKeyboard(chatId)).catch(()=>{}); return; }
  const mult = Number(mm[1]);
  const dur = String(mm[2] || "").trim();
  let until = 0;
  if (mult <= 1 || dur === "0" || dur === "off") {
    db.botConfig = db.botConfig || {};
    db.botConfig.ticketMultiplier = 1;
    db.botConfig.ticketMultiplierUntil = 0;
  } else {
    const ms = parseDurationToMs(dur);
    if (!ms) { await bot.sendMessage(chatId, "❌ Invalid duration. Example: 2x 1h, 3x 10min (max 24hr)", mainMenuKeyboard(chatId)).catch(()=>{}); return; }
    db.botConfig = db.botConfig || {};
    db.botConfig.ticketMultiplier = Math.max(2, Math.min(10, Math.floor(mult)));
    db.botConfig.ticketMultiplierUntil = Date.now() + ms;
  }
  try { saveDB(); } catch (_) {}
  setSettingsState(chatId, null);
  await bot.sendMessage(chatId, "✅ Ticket multiplier updated.", mainMenuKeyboard(chatId)).catch(()=>{});
  return;
}

if (mode === "admin_withdraw_cd") {
  if (!isAdmin(chatId)) { setSettingsState(chatId, null); return; }
  const ms = parseDurationToMs(text);
  if (!ms) { await bot.sendMessage(chatId, "❌ Invalid. Example: 24hr, 12h, 6h (max 24hr)", mainMenuKeyboard(chatId)).catch(()=>{}); return; }
  db.botConfig = db.botConfig || {};
  db.botConfig.withdrawCooldownMs = ms;
  try { saveDB(); } catch (_) {}
  setSettingsState(chatId, null);
  await bot.sendMessage(chatId, `✅ Withdraw cooldown set to ${formatMsShort(ms)}.`, mainMenuKeyboard(chatId)).catch(()=>{});
  return;
}


  // ===================== END SETTINGS INPUT HANDLER =====================
...
if (/^\/dashboard(?:@\w+)?$/i.test(text)) {
  if (!isAdmin(chatId)) { await bot.sendMessage(chatId, "⛔ Not authorized.").catch(()=>{}); return; }
  try {
    const users = Object.values((db && db.users) ? db.users : {});
    const usersCount = users.length;
    const active24h = users.filter(u => u && Number(u.lastSeen||0) > Date.now() - 24*60*60*1000 && !u.archived).length;
    const business = users.filter(u => u && u.isBusiness && !u.archived).length;
    const weeklyOpt = users.filter(u => u && u.wdOptIn && !u.archived).length;

    const day = todayKey();
    const revenueByDay = (db.analytics && db.analytics.revenueByDay) ? db.analytics.revenueByDay : {};
    const txByDay = (db.analytics && db.analytics.transactionsByDay) ? db.analytics.transactionsByDay : {};
    const todayRev = Number(revenueByDay[day] || 0);
    const todayTx = Number(txByDay[day] || 0);

    const pkgDay = (db.analytics && db.analytics.revenueByPackageDay) ? db.analytics.revenueByPackageDay : {};
    const todayPkgs = pkgDay[day] && typeof pkgDay[day] === "object" ? pkgDay[day] : {};
    const top = Object.entries(todayPkgs).sort((a,b)=>Number(b[1]||0)-Number(a[1]||0)).slice(0,5)
      .map(([k,v],i)=>`${i+1}. ${k} — Ksh ${Number(v||0).toFixed(2)}`).join("\n") || "No data yet.";

    const mult = currentTicketMultiplier();
    const multTxt = mult > 1 ? `✅ ${mult}x active` : "OFF";

    const msg =
      `📟 *Dashboard*\n\n` +
      `📅 Today: *${day}*\n` +
      `💰 Revenue today: *Ksh ${todayRev.toFixed(2)}*\n` +
      `🧾 Purchases today: *${todayTx}*\n` +
      `🎟 Ticket Multiplier: *${multTxt}*\n\n` +
      `👥 Users: *${usersCount}*\n` +
      `🟢 Active (24h): *${active24h}*\n` +
      `🏢 Business: *${business}*\n` +
      `🎉 Weekly Opt-in: *${weeklyOpt}*\n\n` +
      `🔥 Top Packages Today\n${top}`;
    await bot.sendMessage(chatId, msg, { parse_mode: "Markdown" }).catch(()=>{});
  } catch (_) {
    await bot.sendMessage(chatId, "⚠️ Dashboard error.").catch(()=>{});
  }
  return;
}

if (/^\/toppackages(?:@\w+)?$/i.test(text)) {
  if (!isAdmin(chatId)) { await bot.sendMessage(chatId, "⛔ Not authorized.").catch(()=>{}); return; }
  try {
    const pkgDay = (db.analytics && db.analytics.revenueByPackageDay) ? db.analytics.revenueByPackageDay : {};
    const total = {};
    for (const day of Object.keys(pkgDay || {})) {
      const o = pkgDay[day];
      if (!o || typeof o !== "object") continue;
      for (const [k,v] of Object.entries(o)) total[k] = Number(total[k]||0) + Number(v||0);
    }
    const top = Object.entries(total).sort((a,b)=>Number(b[1]||0)-Number(a[1]||0)).slice(0,10)
      .map(([k,v],i)=>`${i+1}. ${k} — Ksh ${Number(v||0).toFixed(2)}`).join("\n") || "No data yet.";
    await bot.sendMessage(chatId, `📦 *Top Packages (All Time)*\n\n${top}`, { parse_mode: "Markdown" }).catch(()=>{});
  } catch (_) { await bot.sendMessage(chatId, "⚠️ Error.").catch(()=>{}); }
  return;
}
...
// ===================== GROWTH / SECURITY HELPERS =====================
function isMultiplierActive() {
  try {
    const mult = Number((db.botConfig || {}).ticketMultiplier || 1);
    const until = Number((db.botConfig || {}).ticketMultiplierUntil || 0);
    if (!Number.isFinite(mult) || mult <= 1) return false;
    if (!Number.isFinite(until) || until <= 0) return false;
    return Date.now() < until;
  } catch (_) { return false; }
}
function currentTicketMultiplier() {
  try { return isMultiplierActive() ? Number((db.botConfig || {}).ticketMultiplier || 1) : 1; } catch (_) { return 1; }
}
function applyTicketMultiplier(baseTickets) {
  const mult = currentTicketMultiplier();
  const t = Math.max(0, Math.floor(Number(baseTickets || 0)));
  if (mult <= 1) return t;
  return Math.max(0, Math.floor(t * mult));
}
function bumpFraud(chatId, points, reason) {
  try {
    if (!chatId) return;
    const u = getUser(chatId);
    u.fraudScore = Number(u.fraudScore || 0) + Number(points || 0);
    if (u.fraudScore >= 20) u.banned = true;
    saveDB();
    if (reason) notifyAdmin(`⚠️ *Fraud Score +${mdEscape(points)}*\nChatID: \`${mdEscape(chatId)}\`\nReason: ${mdEscape(reason)}\nScore: *${mdEscape(u.fraudScore)}*`).catch(() => {});
  } catch (_) {}
}
function maybeArchiveUser(chatId) {
  try {
    const u = getUser(chatId);
    const last = Number(u.lastSeen || 0);
    if (last && Date.now() - last > 90 * 24 * 60 * 60 * 1000) u.archived = true;
  } catch (_) {}
}
function updateLoginBonus(chatId) {
  try {
    const u = getUser(chatId);
    const day = todayKey();
    if (u.lastLoginBonusDay !== day) {
      u.lastLoginBonusDay = day;
      // daily login: +1 ticket
      ensureWeeklyDrawObject();
      const c = db.campaign;
      c.entries = c.entries || {};
      c.entries[String(chatId)] = Number(c.entries[String(chatId)] || 0) + 1;
      u.tickets = Number(u.tickets || 0) + 1;
      u.lastTicketAt = Date.now();
      saveDB();
    }
  } catch (_) {}
}
function maybeSendReferralNudge(chatId) {
  try {
    const u = getUser(chatId);
    const myPurch = Array.isArray(u.orders) ? u.orders.length : 0;
    if (myPurch >= 3 && Number(u.referralSuccessCount || 0) === 0) {
      const week = 7 * 24 * 60 * 60 * 1000;
      if (!u.lastReferralNudgeAt || Date.now() - u.lastReferralNudgeAt > week) {
        u.lastReferralNudgeAt = Date.now();
        saveDB();
        const code = u.refCode || (`ref_${chatId}`);
        const link = `https://t.me/${BOT_USERNAME}?start=${code}`;
        sendTracked(chatId, `🎁 Invite friends and earn points!\n\nShare your referral link:\n${link}\n\nEvery successful purchase by your referral earns you rewards.`, { ...mainMenuKeyboard(chatId) }).catch(() => {});
      }
    }
  } catch (_) {}
}
function awardReferralMilestones(chatId) {
  try {
    const u = getUser(chatId);
    const n = Number(u.referralSuccessCount || 0);
    u.refMilestones = u.refMilestones || {};
    const rewards = { 5: 20, 10: 50, 20: 100 };
    if (rewards[n] && !u.refMilestones[String(n)]) {
      u.refMilestones[String(n)] = true;
      ensureWeeklyDrawObject();
      db.campaign.entries = db.campaign.entries || {};
      db.campaign.entries[String(chatId)] = Number(db.campaign.entries[String(chatId)] || 0) + rewards[n];
      u.tickets = Number(u.tickets || 0) + rewards[n];
      u.lastTicketAt = Date.now();
      saveDB();
      sendTracked(chatId, `🎉 Referral milestone!\nYou reached *${n}* successful referrals and earned *${rewards[n]} bonus tickets* 🎟`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
    }
  } catch (_) {}
}
...
function updatePurchaseStreakAndBonus(chatId, addTicketsFn) {
  try {
    const u = getUser(chatId);
    const day = todayKey();
    const last = (u.streak && u.streak.lastDay) ? u.streak.lastDay : "";
    const prev = yesterdayKey();
    if (last === day) return;
    if (last === prev) u.streak.count = Number(u.streak.count || 0) + 1;
    else u.streak.count = 1;
    u.streak.lastDay = day;

    if (u.streak.count >= 3) {
      addTicketsFn(3);
      u.streak.count = 0; // reset after reward
    }
    saveDB();
  } catch (_) {}
}
function shouldAllowWithdraw(chatId) {
  try {
    const u = getUser(chatId);
    const cd = Number((db.botConfig || {}).withdrawCooldownMs || 24*60*60*1000);
    if (!u.lastWithdrawAt) return true;
    return (Date.now() - Number(u.lastWithdrawAt || 0)) >= cd;
  } catch (_) { return true; }
}
// ===================== END GROWTH / SECURITY HELPERS =====================

// ===================== SETTINGS MENU (INLINE) =====================
function settingsMenuText(chatId) {
  const u = getUser(chatId);
  const n = u.notifications || { weekly: true, broadcasts: true, payments: true };
  const lines = [];
  lines.push("⚙️ *Settings*");
  lines.push("");
  lines.push(`🔔 Notifications: Weekly=${n.weekly ? "ON" : "OFF"}, Broadcasts=${n.broadcasts ? "ON" : "OFF"}, Payments=${n.payments ? "ON" : "OFF"}`);
  lines.push(`⏱ Auto-delete bot messages: *${formatMsShort(getAutoDeleteMsForChat(chatId))}*`);
  lines.push(`📱 Default M-PESA number: *${u.defaultNumber ? u.defaultNumber : "Not set"}*`);
  lines.push(`⚡ Quick Mode: *${u.quickMode ? "ON" : "OFF"}*`);
  lines.push(`🌙 Silent Mode: *${u.silentMode ? "ON" : "OFF"}*`);
  lines.push(`🔐 PIN: *${u.pinHash ? "SET" : "Not set"}*`);
  lines.push(`🏆 Leaderboard privacy: *${u.hideLeaderboard ? "HIDDEN" : "VISIBLE"}*`);
  lines.push(`🎨 Theme: *${u.theme === "simple" ? "simple" : "detailed"}*`);
  lines.push("");
  if (isAdmin(chatId)) {
    lines.push("");
    lines.push("👑 *Admin config*");
    lines.push(`🎁 Referral bonus: *${Number((db.botConfig || {}).referralBonusPct || 0).toFixed(2)}%*`);
    lines.push(`🎯 Broadcast target: *${String((db.botConfig || {}).broadcastTarget || "all")}*`);
    lines.push(`⏳ Button cooldown: *${Math.round(Number((db.botConfig || {}).buttonCooldownMs || 3000) / 1000)}s*`);
  }
  lines.push("Tip: Tap any button below to change.");
  return lines.join("\n");
}
...
function settingsMenuKeyboard(chatId) {
  ...
  const rows = [
    [{ text: `🎉 Weekly Updates: ${n.weekly ? "ON ✅" : "OFF ❌"}`, callback_data: "set:notif_weekly" }],
    [{ text: `📢 Broadcasts: ${n.broadcasts ? "ON ✅" : "OFF ❌"}`, callback_data: "set:notif_broadcasts" }],
    [{ text: `💳 Payments: ${n.payments ? "ON ✅" : "OFF ❌"}`, callback_data: "set:notif_payments" }],
    [{ text: "🧹 Auto-delete time", callback_data: "set:autodelete" }],
    [{ text: "📱 Set Default Number", callback_data: "set:default_number" }],
    [{ text: `⚡ Quick Mode: ${u.quickMode ? "ON ✅" : "OFF ❌"}`, callback_data: "set:quickmode" }],
    [{ text: `🌙 Silent Mode: ${u.silentMode ? "ON ✅" : "OFF ❌"}`, callback_data: "set:silentmode" }],
    [{ text: "🔐 Set/Change PIN", callback_data: "set:pin" }],
    [{ text: `🏆 Leaderboard: ${u.hideLeaderboard ? "Hidden" : "Visible"}`, callback_data: "set:leaderboard" }],
    [{ text: `🎨 Theme: ${u.theme === "simple" ? "simple" : "detailed"}`, callback_data: "set:theme" }],
  ];

  if (isAdmin(chatId)) {
    rows.push([{ text: "🎁 Set Referral Bonus %", callback_data: "set:admin_refbonus" }]);
    rows.push([{ text: "🎯 Broadcast Target", callback_data: "set:admin_bcast_target" }]);
    rows.push([{ text: "⏳ Button Cooldown", callback_data: "set:admin_cooldown" }]);
    rows.push([{ text: "🎟 Ticket Multiplier", callback_data: "set:admin_multiplier" }]);
    rows.push([{ text: "🏧 Withdraw Cooldown", callback_data: "set:admin_withdraw_cd" }]);
  }

  rows.push([{ text: "⬅️ Back to Home", callback_data: "ui:home" }]);

  return { reply_markup: { inline_keyboard: rows } };
}

function setSettingsState(chatId, state) {
  try {
    db.settingsState = db.settingsState || {};
    if (!state) delete db.settingsState[String(chatId)];
    else db.settingsState[String(chatId)] = state;
    try { saveDB(); } catch (_) {}
  } catch (_) {}
}
function getSettingsState(chatId) {
  try {
    return (db.settingsState || {})[String(chatId)] || null;
  } catch (_) { return null; }
}
...
if (/^\/suggest(?:@\w+)?$/i.test(text)) {
  const u = getUser(chatId);
  // heuristic: recommend favourite package if available, else show suggested packages list if present
  if (u && u.favPackage && u.favAmount) {
    const msgTxt =
      `🤖 *Smart Suggestion*\n\n` +
      `You often buy: *${u.favPackage}*\n` +
      `Amount: *Ksh ${Number(u.favAmount || 0)}*\n\n` +
      `Tap Quick Actions to buy faster.`;
    await bot.sendMessage(chatId, msgTxt, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(()=>{});
  } else {
    await bot.sendMessage(chatId, "🤖 Smart Suggestion: Use Quick Actions or buy your favourite package to train recommendations.", mainMenuKeyboard(chatId)).catch(()=>{});
  }
  return;
}
// ===================== END /suggest =====================

// ===================== USER COMMAND: /mystats =====================
if (/^\/mystats(?:@\w+)?$/i.test(text)) {
  const u = getUser(chatId);
  const pts = Number(u.points || 0);
  const spent = Number(u.totalSpent || 0);
  const tickets = Number(u.tickets || 0);
  const rf = Number(u.referralSuccessCount || 0);
  const msgTxt =
    "📊 *My Stats*\n\n" +
    `💰 Total spent: *Ksh ${spent.toFixed(2)}*\n` +
    `🎟 Tickets: *${tickets}*\n` +
    `🎁 Points: *${pts.toFixed(2)}*\n` +
    `👥 Successful referrals: *${rf}*\n`;
  await bot.sendMessage(chatId, msgTxt, { parse_mode: "Markdown" }).catch(() => {});
  return;
}
// ===================== END /mystats =====================
// ===================== END /settings =====================
