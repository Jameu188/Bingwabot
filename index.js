...
bot.on("callback_query", async (q) => {
  try {
    ...
    // 🔒 Auto-hide buttons: keep Help/settings hidden, but allow inline-mode purchase actions.
    // IMPORTANT: Silent ignore (no popup text), so blocked buttons still "do nothing" while hidden.
    try {
      const allowHiddenCallback = (
        ...
        data === "ui:buy_last" ||
        data === "ui:close" ||
        data.startsWith("retry_stk:")
      );
      if (isAutoHideButtonsEnabled(chatId) && !allowHiddenCallback) {
...
function isHiddenButtonsHelpEntry(text) {
  const t = String(text || "").trim();
  return t === "ℹ️ Help" || /^\/help(?:@\w+)?$/i.test(t) || /^help$/i.test(t);
}

function isInlineModeTriggerText(text) {
  const t = String(text || "").trim();
  return t === "#help_inline" || t.startsWith("#cat ") || t.startsWith("#buy ");
}

function isHiddenButtonsSessionStep(step) {
  return ["category", "package", "confirm", "phone", "fast_phone_pick"].includes(String(step || ""));
}

function shouldAllowWhileButtonsHidden(chatId, text) {
  if (!isAutoHideButtonsEnabled(chatId)) return true;
  if (isHiddenButtonsHelpEntry(text)) return true;
  if (isInlineModeTriggerText(text)) return true;
  try {
    const s = sessions.get(chatId);
    if (s && isHiddenButtonsSessionStep(s.step)) return true;
  } catch (_) {}
  return false;
}
...
try {
  ...
  // If buttons are hidden, block normal typed menu actions, but keep inline-mode flows working.
  // IMPORTANT: Silent ignore (no reply) until unhidden.
  try {
    if (!shouldAllowWhileButtonsHidden(chatId, text)) {
...
bot.on("message", async (msg) => {
  try {
    ...
    if (!/^\/langstatus(?:@\w+)?$/i.test(text)) return;
    const lang = getLang(chatId);
    await bot.sendMessage(chatId, lang === "sw" ? "✅ Lugha ya sasa: Kiswahili" : "✅ Current language: English").catch(() => {});
  } catch (_) {}
});
...
// ============================================================================
bot.onText(/\/lang (en|sw)/i, (msg, match) => {
  try {
    const chatId = msg.chat.id;
    const lang = String(match[1] || "").toLowerCase();
    const user = getUser(chatId);
    user.lang = lang === "sw" ? "sw" : "en";
    saveDB();

    if (user.lang === "sw") {
      bot.sendMessage(chatId, "✅ Lugha imewekwa kuwa Kiswahili").catch(()=>{});
    } else {
      bot.sendMessage(chatId, "✅ Language set to English").catch(()=>{});
    }
  } catch (_) {}
});
