...
async function cleanupBeforeReply(chatId, userMsgId) {
  ...
  // Always delete previous bot messages before sending a new reply.
...
async function sendTracked(chatId, text, options = {}) {
  ...
  // Delete previous bot message(s) before sending a new one.
  // /start is already excluded earlier via cleanupBeforeReply(), and welcome stays permanent.
  if (!isWelcomeMessage) {
    await clearTrackedBotMessages(chatId);
  }
