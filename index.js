...
if (data === "biz_back") {
  ...
  if (!prev || prev === "biz_overview") {
    ...
    const nav = helpAnswerKeyboard(chatId, key)?.reply_markup?.inline_keyboard || [];
...
function helpAnswerKeyboard(chatId = null, key = "") {
  const rows = [];

  if (key === "referrals" && chatId) {
    const botUsername = process.env.BOT_USERNAME || BOT_USERNAME || "";
    const referralLink = botUsername ? `https://t.me/${botUsername}?start=${chatId}` : "";
    if (referralLink) rows.push([{ text: "🔗 My Referral", url: referralLink }]);
  }
  ...
  const opts =
    key === "cmds"
      ...
      : key === "settings"
        ...
        : { parse_mode: "Markdown", ...helpAnswerKeyboard(chatId, key) };
...
function gmailQuantityButtons(stockCount) {
  const presets = [1, 3, 5, 10, 20, 30, 50, 100, 200, 300, 500, 600, 1000];
  const rows = [];
  const seen = new Set();
  ...
  for (const qty of presets) {
    ...
    rows.push([{ text: `📧 Buy ${qty} Email${qty > 1 ? "s" : ""} - Ksh ${total}`, callback_data: `gmail_qty:${qty}` }]);
  }

  if (stockCount > 0 && !seen.has(stockCount)) {
    const total = stockCount * gmailPrice();
    rows.push([{ text: `📧 Buy ${stockCount} Email${stockCount > 1 ? "s" : ""} - Ksh ${total}`, callback_data: `gmail_qty:${stockCount}` }]);
  }
...
async function beginGmailPurchase(chatId, qty, editCtx = null) {
  const count = gmailAvailableCount();
  const price = gmailPrice();
  const q = Math.max(1, Number(qty || 0));

  if (count < q) {
    const body = `❌ Sorry, this quantity is no longer available.`;
    if (editCtx && editCtx.chat_id && editCtx.message_id) {
      await bot.editMessageText(body, {
        chat_id: editCtx.chat_id,
        message_id: editCtx.message_id,
        reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "gmail_menu" }]] }
      }).catch(() => {});
    } else {
  ...
  if (savedPhone) {
    await confirmGmailPhoneAndPush(chatId, q, savedPhone, total, editCtx);
    return;
  }

  setGmailAwait(chatId, { mode: "await_phone", qty: q, total, inlineMessageId: editCtx?.message_id || null, inlineChatId: editCtx?.chat_id || null });
  const body = `📧 Gmail Accounts

Package: Buy ${q} Email${q > 1 ? "s" : ""}
...
Send the M-Pesa phone number to pay.`;
  ...
  if (editCtx && editCtx.chat_id && editCtx.message_id) {
    await bot.editMessageText(body, {
      chat_id: editCtx.chat_id,
      message_id: editCtx.message_id,
      reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: `gmail_qty:${q}` }]] }
    }).catch(async () => {
      await bot.sendMessage(chatId, body).catch(() => {});
    });
    return;
...
async function confirmGmailPhoneAndPush(chatId, qty, phoneInput, total, editCtx = null) {
  const phone254 = normalizePhone(phoneInput);
  const count = gmailAvailableCount();

  if (!phone254) {
    setGmailAwait(chatId, { mode: "await_phone", qty: qty, total: total, inlineMessageId: editCtx?.message_id || null, inlineChatId: editCtx?.chat_id || null });
  ...
  try {
    ...
    setGmailAwait(chatId, null);
    const stkBody = `✅ STK sent for Gmail Accounts.\n\n📦 Package: ${pkgLabel}\n💰 Amount: Ksh ${total}\n📱 Number: ${formatTo07(phone254)}\n\nCheck your phone and complete payment.`;
    if (editCtx && editCtx.chat_id && editCtx.message_id) {
      await bot.editMessageText(stkBody, {
        chat_id: editCtx.chat_id,
        message_id: editCtx.message_id,
        reply_markup: { inline_keyboard: [[{ text: "🛒 Buy Offers", callback_data: "buy_offers" }], [{ text: "❌ Close", callback_data: "ui:close" }]] }
      }).catch(async () => {
        await bot.sendMessage(chatId, stkBody).catch(() => {});
      });
    } else {
      await bot.sendMessage(chatId, stkBody).catch(() => {});
    }
...
bot.on("message", async (msg) => {
  try {
    ...
    if (state && state.mode === "await_phone" && text && !text.startsWith("/")) {
      const editCtx = state.inlineChatId && state.inlineMessageId
        ? { chat_id: state.inlineChatId, message_id: state.inlineMessageId }
        : null;
      await confirmGmailPhoneAndPush(chatId, Number(state.qty || 0), text, Number(state.total || 0), editCtx);
      return;
    }
  } catch (_) {}
});
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data.startsWith("gmail_qty:")) {
      ...
      if (count < qty) {
        await bot.answerCallbackQuery(q.id, { text: `This quantity is no longer available.` }).catch(() => {});
        return;
      }
    ...
    if (data.startsWith("gmail_buy:")) {
      const qty = Math.max(1, Number(data.split(":")[1] || 0));
      await bot.answerCallbackQuery(q.id).catch(() => {});
      await beginGmailPurchase(chatId, qty, { chat_id: q.message.chat.id, message_id: q.message.message_id });
      return;
    }
  } catch (_) {}
});
