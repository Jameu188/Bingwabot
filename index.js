...
function formatPrivateConfirmationDateTime(ts = Date.now()) {
  try {
    ...
    hh = hh % 12 || 12;
    const time = `${hh}:${String(d.getMinutes()).padStart(2, "0")} ${ampm}`;
    return { date, time };
  }
}

function buildPrivateConfirmationMessage({ reference, amount, phone, agentName = PRIVATE_CONFIRMATIONS_AGENT_NAME, accountBalance = PRIVATE_CONFIRMATIONS_ACCOUNT_BALANCE, txCost = PRIVATE_CONFIRMATIONS_TX_COST, whenTs = Date.now() }) {
  const fmt = formatPrivateConfirmationDateTime(whenTs);
  const ref = String(reference || "N/A").trim();
  const amountNum = Number(amount || 0);
  const amountText = Number.isFinite(amountNum) ? amountNum.toFixed(2) : String(amount || "0.00");
  const sender = String(phone || "").trim() || "N/A";
  return `${ref} Confirmed.on ${fmt.date} at ${fmt.time} Ksh${amountText} received from ${sender} ${agentName}. New Account balance is ${accountBalance}. Transaction cost, ${txCost}.`;
}

async function sendPrivateConfirmationToGroup(details) {
  try {
    if (!PRIVATE_CONFIRMATIONS_GROUP_ID) return;
    if (!bot || typeof bot.sendMessage !== "function") return;
    const text = buildPrivateConfirmationMessage(details || {});
    await bot.sendMessage(PRIVATE_CONFIRMATIONS_GROUP_ID, text).catch((err) => {
      console.log("private confirmation group send failed:", sanitizeLogText(err?.message || err));
    });
  } catch (err) {
    console.log("private confirmation group error:", sanitizeLogText(err?.message || err));
  }
}
...
// Private confirmations group
const PRIVATE_CONFIRMATIONS_GROUP_ID = Number(process.env.PRIVATE_CONFIRMATIONS_GROUP_ID || -1003797242439);
const PRIVATE_CONFIRMATIONS_AGENT_NAME = process.env.PRIVATE_CONFIRMATIONS_AGENT_NAME || "𝐀𝐠𝐞𝐧𝐭 𝐓𝐨𝐛𝐢𝐚𝐬";
const PRIVATE_CONFIRMATIONS_ACCOUNT_BALANCE = process.env.PRIVATE_CONFIRMATIONS_ACCOUNT_BALANCE || "Ksh0.63";
const PRIVATE_CONFIRMATIONS_TX_COST = process.env.PRIVATE_CONFIRMATIONS_TX_COST || "Ksh0.00";
...
` +
    ...
    try {
      await sendPrivateConfirmationToGroup({
        reference: referenceNumber,
