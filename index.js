...
function weeklyDrawBroadcastText() {
  ...
  const how = "✅ Tap 🎉 WEEKLY DRAW in the bot menu to view status, join/leave, and track your rank.";
...
try {
  ...
  if (rec && rec.paid !== true && rec.payoutSubmitted !== true) {
    rows.push([{ text: "📲 Submit M-PESA Details", callback_data: "wdusr:claim" }]);
          } else if (rec && rec.paid !== true && rec.payoutSubmitted === true) {
            rows.push([{ text: "✏️ Edit M-PESA Details", callback_data: "wdusr:editclaim" }]);
  }
  if (rec && rec.paid === true) {
    rows.push([{ text: "✅ Payout PAID", callback_data: "wdusr:refresh" }]);
  }
} catch (_) {}
...
function weeklyDrawQualificationsInfoLines() {
  ...
  const rrTxt = weeklyRandomRuleText();
  if (rrTxt) add(rrTxt);
  ...
  if (qcfg.tickets?.enabled) add(`Earn at least ${Number(qcfg.tickets.count || 0)} ticket(s)`);
  if (qcfg.points?.enabled) add(`Hold at least ${Number(qcfg.points.amount || 0)} point(s)`);
  ...
  if (qcfg.bizMonths?.enabled) add(`Business subscription: at least ${Number(qcfg.bizMonths.minMonths || 0)} month(s) remaining`);
...
${weeklyRandomRuleText() ? ("✅ " + weeklyRandomRuleText() + "\n") : ""}
...
function weeklyDrawLastWinnersText() {
    ...
    const lines = last.winners.slice(0, (c.winnersCount || 3)).map((w,i)=>{
      const pos = i===0?"🥇":i===1?"🥈":"🥉";
      return `${pos} ${(w.name || safeDisplayName(getUser(w.chatId), w.chatId) || w.chatId)} — Ksh ${Number(w.prize||0)} (${w.paid ? "Paid ✅" : "Pending ⏳"})`;
    }).join("\n");
    return `
...
let optedLine = "ℹ️ You are not opted in (no campaign messages). Tap ✅ JOIN to participate.";
if (isWeeklyDrawOptedIn(chatId)) {
  ...
  if (myPos >= 1 && myPos <= 3) {
    optedLine = `🎊 Congratulations! You are among the TOP 3 this week (#${myPos}). Keep buying to maintain your position.`;
  } else if (myPos >= 4 && myPos <= 10) {
    optedLine = `👏 Great! You are close to winning (#${myPos} in Top 10). Keep buying to climb into Top 3.`;
  } else {
    optedLine = "✅ You are opted in (you will receive updates).";
  }
}
...
function startWeeklyDrawLiveUpdate(chatId, messageId, preferCaption = true) {
  ...
  const tick = async () => {
    try {
      const caption = buildWeeklyDrawMainCaption(chatId) + (weeklyDrawLastWinnersText() || "");
...
async function openWeeklyDrawUserMenu(chatId) {
  const c = ensureWeeklyDrawObject();
  if (!c.enabled) {
    return bot.sendMessage(chatId, "❌ WEEKLY DRAW is not active right now.", mainMenuKeyboard(chatId)).catch(() => {});
  }
  ...
  try {
    if (img) {
      ...
      const sent = await bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId));
      // Live update (text)
    ...
    // fallback to message
    const sent = await bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId)).catch(() => null);
    if (sent) {
      try { startWeeklyDrawLiveUpdate(chatId, sent.message_id, false); } catch (_) {}
      return sent;
    }
...
function createGlobalBackup(adminChatId) {
  const id = `bk-${backupIdNow()}`;
  ...
  const meta = {
    id,
    createdAt: Date.now(),
    createdBy: adminChatId,
    schema: 1,
  };

  try {
    fs.writeFileSync(file, JSON.stringify({ meta, db: snapshot }, null, 2));
...
function listBackups(limit = 10) {
  try {
    if (!fs.existsSync(BACKUP_DIR)) return [];
    const files = fs.readdirSync(BACKUP_DIR).filter((f) => f.endsWith(".json"));
    files.sort((a, b) => {
      try { return fs.statSync(path.join(BACKUP_DIR, b)).mtimeMs - fs.statSync(path.join(BACKUP_DIR, a)).mtimeMs; } catch (_) { return 0; }
    });
    return files.slice(0, limit).map((f) => f.replace(/\.json$/i, ""));
  } catch (_) {
    return [];
  }
}

function restoreGlobalBackup(backupId, adminChatId) {
  const id = String(backupId || "").trim();
  if (!id) return { ok: false, error: "Missing backup id." };

  // One-time restore per backup id (persisted outside restored db)
  db = repairDB(db);
  db.usedBackupIds = db.usedBackupIds || {};
  if (db.usedBackupIds[id]) return { ok: false, error: "This backup has already been restored once." };
  ...
  const snapshot = payload?.db;
  if (!snapshot || typeof snapshot !== "object") return { ok: false, error: "Invalid backup contents." };

  // Preserve restore-lock map across restores
  const keepUsed = deepClone(db.usedBackupIds || {});
  keepUsed[id] = true;

  db = repairDB(snapshot);
  db.usedBackupIds = keepUsed;
  try {
    saveDB();
  } catch (_) {}
...
function buildUserBackup(chatId) {
  ...
  // One-time restore token for this backup file
...
function isBackupAlreadyUsed(backupId) {
  try {
    db = repairDB(db);
    const id = String(backupId || "").trim();
    if (!id) return false;
    return !!db.usedBackupIds[id];
  } catch (_) {
    return false;
  }
}

function markBackupUsed(backupId) {
  try {
    db = repairDB(db);
    const id = String(backupId || "").trim();
    if (!id) return;
    db.usedBackupIds[id] = Date.now();
    saveDB();
  } catch (_) {}
}
...
function applyUserBackup(chatId, payload) {
  ...
  // NOTE: Restore is allowed from any account (backup not tied to current chatId)
if (!db.users || typeof db.users !== "object") db.users = {};
  db.users[cid] = payload.user && typeof payload.user === "object" ? payload.user : {};
  ...
  // We replace ONLY this user's events; other users' events stay untouched.
  if (payload.ledger !== undefined) {
    ...
    // keep last 20k events like your ledger uses
    if (db.ledger.length > 20000) db.ledger = db.ledger.slice(-20000);
  }

  // Repair/normalize then save
  db = repairDB(db);
  saveDB();
}
...
function backupUniqueKey(payloadObj) {
  try {
    const bid = String(payloadObj && payloadObj.backupId || "").trim();
    if (bid) return "id:" + bid;

    // Fallback for older backups with no backupId: hash stable JSON
...
// Download a Telegram document into a Buffer (no external deps)
...
if (data === "help:home") {
  ...
  sendTracked(chatId, "🏠 Main Menu", { ...mainMenuKeyboard(chatId) });
  return;
}
...
if (data === "biz_activate") {
  ...
  sendTracked(chatId, header, { parse_mode: "Markdown", ...businessPlansInlineKeyboard(chatId) });
  return;
}
...
if (data === "biz_back") {
  ...
  if (!prev || prev === "biz_overview") {
    ...
    const text = helpAnswerText("business");
...
if (data === "biz:backup_once") {
  if (!isBusinessActive(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "💼 Business only. Type /sell to upgrade." }).catch(() => {});
    return;
  }
  ...
  if (db.usedInlineActions[actionKey]) {
    await bot.answerCallbackQuery(q.id, { text: "✅ Already used. Backup button is expired." }).catch(() => {});
    return;
  }
  ...
  await bot.answerCallbackQuery(q.id, { text: "⏳ Creating your backup..." }).catch(() => {});

  try {
    const payload = buildUserBackup(chatId);
    const pretty = JSON.stringify(payload, null, 2);
    const ts = new Date().toISOString().slice(0, 10);
    const fname = `bingwa_backup_${chatId}_${ts}.json`;

    // Send backup file
    bot.sendDocument(
      chatId,
      Buffer.from(pretty, "utf8"),
      { caption: "✅ Your backup is ready.\n\n⚠️ Your account will now be cleared (blank).\nTo restore (ONE-TIME per file): send /restbackup then upload this backup file." },
      { filename: fname, contentType: "application/json" }
    ).catch(()=>{});

    // Clear user data after successful send
    const ok = wipeUserData(chatId);
    try { getUser(chatId); } catch (_) {} // re-create blank user shell
    try { saveDB(); } catch (_) {}
    ...
    await bot.sendMessage(chatId, "✅ Backup saved.", mainMenuKeyboard(chatId)).catch(() => {});
  } catch (e) {
    ...
    await bot.sendMessage(chatId, "❌ Failed to create backup: " + (e?.message || e), mainMenuKeyboard(chatId)).catch(() => {});
  }
  return;
}
...
if (data.startsWith("wdusr:")) {
  ...
  if (action === "optin") {
    ...
    // After joining, show suggested quick-buy offers on the same Weekly Draw message
    try {
...
function helpAnswerText(key) {
  ...
  const baseSupport = `📞 *Support:* ${HELP_PHONE}`;
  ...
  switch (key) {
    case "buy":
      return (
        ...
        "3) Pick a package\n" +
        "4) Enter your M-PESA number\n" +
        "5) Tap ✅ *Proceed* and confirm the STK on your phone\n\n" +
        "✅ On success: bundle is delivered + you earn 🎁 points + 🎟 Weekly Draw tickets (and referral bonus if you were invited).\n" +
        "❌ On fail/cancel: no points/tickets/bonus are awarded.\n"
      );

    case "payments":
      return (
        "💳 *Payments & STK issues*\n\n" +
        "• After tapping ✅ *Proceed*, an STK prompt is sent to your phone.\n" +
        "• If you *cancel* or the STK *fails*, the order is not processed.\n\n" +
        "Common fixes:\n" +
        "✅ Ensure you have enough M-PESA balance\n" +
        "✅ Ensure your phone is on / not in airplane mode\n" +
        "✅ Wait 10–30 seconds then try again\n\n" +
        "If you paid but did not receive the offer, share your M-PESA message with support.\n\n" +
        baseSupport
      );

    case "weekly_draw":
      return (
        "🎉 *Weekly Draw / Campaigns*\n\n" +
        "Each week we run a competition where *Top 3* participants win cash prizes 🏆💸\n\n" +
        "How to join:\n" +
        "1) Tap 🎉 *WEEKLY DRAW*\n" +
        "2) Tap ✅ *JOIN*\n" +
        "3) Buy offers to earn tickets\n\n" +
        "🎟 Ticket rule: *5 Ksh spent = 1 ticket*\n" +
    ...
    case "winners":
      return (
        ...
        "• Winners are announced inside the bot after the campaign ends.\n" +
        "• If you win, you’ll be asked to confirm your M-PESA details.\n" +
        "• Admin marks the payout as *Paid* after sending.\n\n" +
        "⚠️ Always use the same name/number you want to be paid with.\n"
      );

    case "withdraw":
      return (
        "💸 *Withdraw Points (rules + minimum)*\n\n" +
        `🔓 Unlock: you must spend at least *Ksh ${spendUnlock}* before withdrawing.\n` +
        (minPoints > 0 ? `📌 Minimum points per withdrawal: *${minPoints}*\n` : "") +
        `💱 Rate: *${rateStr}*\n\n` +
        "How it works:\n" +
        "1) Tap *💸 Withdraw Points*\n" +
        "2) Submit your M-PESA number if asked\n" +
        "3) Admin reviews your request\n" +
        "4) If approved → paid to M-PESA\n" +
        "5) If declined → points are refunded\n\n" +
        "⏳ Cooldown: after a decision, you can request again after a short wait.\n\n" +
        baseSupport
      );
    ...
    case "redeem":
      return (
        "🎁 *Redeem rewards*\n\n" +
        "You can use points to redeem rewards like:\n" +
        "• ✉️ SMS\n" +
        "• 📶 Data\n" +
        "• 🌙 Midnight minutes\n\n" +
        "⚠️ Some rewards may be limited per phone per day.\n"
      );

    case "inline":
      return (
        "🔎 *Inline search (fast search)*\n\n" +
        "You can search offers directly from Telegram search bar:\n\n" +
        `Type: *@${(process.env.BOT_USERNAME || "YourBotUsername")} 1GB*\n` +
        `Type: *@${(process.env.BOT_USERNAME || "YourBotUsername")} 200 SMS*\n\n` +
        "Then tap the result to open the offer instantly.\n"
      );

    case "referrals":
      return (
        "👥 *Referrals (earn legit bonus)*\n\n" +
        `• Share your link from *🔗 My Referral*\n` +
        `• You earn *${bonusPct}%* points from each successful purchase your referral makes (for *${bonusDays} days*).\n\n` +
        "✅ Legit referrals:\n" +
        "• Real users + real purchases\n" +
        "• Different phone numbers\n\n" +
        "❌ Not allowed:\n" +
        "• Self-referral / fake accounts\n" +
        "• Multiple accounts using the same phone number\n"
      );

    case "penalties":
      return (
        "⚠️ *Penalties & limitations*\n\n" +
        "📌 Bingwa Deals:\n" +
        "• Only *once per day per phone number*\n" +
        "• The lock applies only after SUCCESS payment\n\n" +
        "🚫 Abuse / fraud may lead to:\n" +
        "• Referral bonus removal\n" +
        "• Reward removal\n" +
        "• Temporary or permanent ban\n\n" +
        "🤖 Spam protection:\n" +
        "• Too many fast clicks/messages may limit access temporarily\n"
      );

    case "business":
      return (
        "🏢 *Business Account*\n\n" +
        "Upgrade your account to unlock premium features and higher earning potential.\n\n" +
        "*Business Benefits:*\n" +
        "• Unlimited referrals\n" +
        "• Unlimited rewards\n" +
        "• Fewer usage limitations\n" +
        "• +5% Weekly Draw ticket bonus\n" +
        "• Extra account settings\n" +
        "• Unlimited withdrawals\n\n" +
        "*Exclusive Business Features:*\n" +
        "• Higher cashback points on purchases\n" +
        "• Priority processing for withdrawals\n" +
        "• Early access to new data offers\n" +
        "• Faster reward accumulation\n" +
        "• Premium support priority\n" +
        "• Access to exclusive business-only deals\n" +
        "• Sell data offers to others and earn **higher commissions**\n\n" +
        "Tap *Activate* below to subscribe to a Business plan.\n"
      );
case "cmds":
  return (
    "⚙️ *Commands & Settings*\n\n" +
    "*Main commands:*\n" +
    "• /start — open welcome + 🏠 main menu\n" +
    "• /menu — open main menu (same as /start)\n" +
    "• /help — open this Help Center\n" +
    "• /status — check if you have a pending STK request\n" +
    "• /profile — view your profile & stats\n\n" +
    "*Quick buttons (same as commands):*\n" +
    "• 🛒 Buy Offers — browse categories & packages\n" +
    "• 💸 Withdraw Points — request withdrawal\n" +
    "• 🔗 My Referral — get your referral link\n" +
    "• 🎉 Weekly Draw — join campaign\n\n" +
    "*Settings:*\n" +
    "🧹 Auto-delete bot messages:\n" +
    "• /autodelete 10min\n" +
    "• /autodelete 1h\n" +
    "• /autodelete 24hr (max)\n" +
    "• /autodelete off (sets to 24hr max)\n\n" +
    "💾 Business backup (business only): open Commands & Settings and tap the inline button 'Backup (Auto-Delete)'.\n\n" +
...
case "support":
    default:
      return (
        "📞 *Support*\n\n" +
        "For help with payments, bundles, withdrawals, or campaigns:\n\n" +
        baseSupport
      );
  }
}
...
async function sendHelpAnswer(chatId, key, ctx = {}) {
  ...
  const opts = key === "cmds"
    ? { parse_mode: "Markdown", ...fastPurchaseHelpKeyboard(chatId) }
...
function wdSuggestedOffersKeyboard(offers) {
  ...
  rows.push([{ text: "🛒 View All Offers", callback_data: "ui:buy_offers" }]);
  rows.push([{ text: "⬅️ Back", callback_data: "wdusr:menu" }]);
  return { reply_markup: { inline_keyboard: rows } };
}
...
function wdJoinSuggestedPayload() {
  ...
  const msg =
    ...
    `🔥 Boost your tickets now by buying a quick deal below.\n` +
    `🏆 Buy any deal and you might win up to *Ksh 1500* weekly.\n\n` +
    `👇 Choose one package:`;
...
function fastPurchaseHelpKeyboard(chatId) {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: `${fastPurchaseToggleMark(chatId)} Fast Purchase`, callback_data: "fp:toggle" }],
        [{ text: `${autoRetryToggleMark(chatId)} Auto Retry STK`, callback_data: "ar:toggle" }],

        [{ text: `${campaignNotifsMark(chatId)} Stop campaign notifications`, callback_data: "nf:campaign" }],
        [{ text: `${cashbackNotifsMark(chatId)} Stop cashback notifications`, callback_data: "nf:cashback" }],
        [{ text: `${referralNotifsMark(chatId)} Stop referral earnings & joins`, callback_data: "nf:referral" }],

        [{ text: `${autoDelete24hMark(chatId)} Auto delete messages (24hrs)`, callback_data: "ad:24h" }],

        [{ text: `${autoHideButtonsMark(chatId)} Auto hide buttons (except Help)`, callback_data: "bh:toggle" }],

        [{ text: `${hideQuickActionsMark(chatId)} Hide Quick Actions (main menu)`, callback_data: "qa:toggle" }],

        ...(isBusinessActive(chatId) ? [[{ text: "💾 Backup (Auto-Delete)", callback_data: "biz:backup_once" }]] : []),
  ...
  for (const [ref, p] of Object.entries(db.pendingPayments || {})) {
    if (!p || isPendingExpired(p, ttl)) {
      ...
      try {
        ...
        if (cid) {
          bot.sendMessage(cid, "⌛ STK request expired. Please try again if you still want to pay.").catch(() => {});
        }
      } catch (_) {}
...
sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
...
function queueBroadcastAlbum(adminChatId, msg) {
  ...
  existing.timer = setTimeout(async () => {
    ...
    try {
      ...
      sendTracked(adminChatId, `⚠️ Album broadcast error: ${e.message || e}`);
    }
  }, 1800);
}
...
function mainMenuKeyboard(chatId = null) {
  ...
  try {
    if (chatId && isAutoHideButtonsEnabled(chatId)) {
      return { reply_markup: { keyboard: [["ℹ️ Help"]], resize_keyboard: true } };
    }
  } catch (_) {}
...
if (quick && quick.length > 0) {
  ...
  if (!u.hideQuickActionsEnabled) {
    ...
    keyboard.push(["🙈 Hide Quick Actions"]);
  }
}
  }

  keyboard.push(
    ["👤 My Profile", "💸 Withdraw Points"],
    ["🔗 My Referral", "ℹ️ Help"]
  );
...
Help: ${HELP_PHONE}`;
    sendTracked(chatId, msg, { ...mainMenuKeyboard(chatId) });
    return;
  }
...
try {
  ...
  sendTracked(chatId, out, { ...mainMenuKeyboard(chatId) });
  return;
}
...
// Inline action buttons (Upgrade + Buy Last Offer + Buy Offers)
    ...
    if (pending?.category === "Business Account") {
      ...
      sendTracked(chatId, successMsg, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
    } else {
      sendTracked(chatId, successMsg, {
        parse_mode: "Markdown",
        reply_markup: { inline_keyboard }
      });
    }
...
// ✅ GLOBAL NAV OVERRIDE (works even during active purchase/session flows)
// If user taps main buttons while in a session, we should respond and not treat it as a package label.
const navText = (text || "");
const isNav =
  navText === "👤 My Profile" ||
  navText === "💸 Withdraw Points" ||
  navText === "🔗 My Referral" ||
  navText === "ℹ️ Help" ||
  navText === "🏠 Main Menu" ||
  navText === "🛒 Buy Offers" ||
  /^\/profile(?:@\w+)?$/i.test(navText) ||
  /^\/help(?:@\w+)?$/i.test(navText) ||
  /^\/start(?:@\w+)?$/i.test(navText) ||
  /^\/buy(?:@\w+)?$/i.test(navText);

if (isNav) {
  // Clear any active session to avoid "no reply" issues
  try { sessions.delete(chatId); } catch (_) {}

  if (navText === "👤 My Profile" || /^\/profile(?:@\w+)?$/i.test(navText)) return showUserProfile(chatId);
  if (navText === "ℹ️ Help" || /^\/help(?:@\w+)?$/i.test(navText)) return sendTracked(chatId, helpText(), { ...mainMenuKeyboard(chatId) });
  if (navText === "🔗 My Referral") return sendReferral(chatId);
  if (navText === "💸 Withdraw Points") return startWithdrawFlow(chatId);
  if (navText === "🛒 Buy Offers" || /^\/buy(?:@\w+)?$/i.test(navText)) {
    sessions.set(chatId, { step: "category" });
    return sendTracked(chatId, "✅ Choose what you need:", { reply_markup: categoryKeyboard(chatId).reply_markup });
  }
  // main menu
...
if (text === "/mybackup" || text === "/backup") {
  try {
    ...
    try {
      ...
      // fallback: send as message if document fails
      try {
        await bot.sendMessage(
          chatId,
          "✅ Backup created, but sending as file failed.\nCopy and save this text (may be truncated):\n\n" +
            pretty.slice(0, 3500) +
            (pretty.length > 3500 ? "\n... (too long)" : "")
        );
        sentOk = true;
    ...
    if (sentOk) {
      // ✅ After successful backup, delete/reset all account data (leave blank)
      const ok = wipeUserData(chatId);
      try { getUser(chatId); } catch (_) {} // re-create blank user shell
      try { saveDB(); } catch (_) {}
      ...
      await bot.sendMessage(
        ...
        ok
          ? "✅ Backup saved."
          : "⚠️ Backup delivered, but clearing data failed. Contact admin."
      ).catch(()=>{});
    }

  } catch (e) {
    await bot.sendMessage(chatId, "❌ Failed to create backup: " + (e?.message || e)).catch(()=>{});
  }
  return;
}

// ===================== USER BACKUP + RESET: /mybackupreset =====================
if (text === "/mybackupreset" || text === "/backupreset") {
  try {
    const payload = buildUserBackup(chatId);
    const pretty = JSON.stringify(payload, null, 2);
    const ts = new Date().toISOString().slice(0, 10);
    ...
    // Send backup file first. Only wipe AFTER successful send.
    bot.sendDocument(
      chatId,
      Buffer.from(pretty, "utf8"),
      { caption: "✅ Backup created.\n\n⚠️ Now resetting your account data... (you can restore anytime with /restbackup)" },
      { filename: fname, contentType: "application/json" }
    ).catch(()=>{});
    ...
    await bot.sendMessage(
      chatId,
      ok
        ? "🧹 Reset complete.\n\nYour data is deleted from the bot.\nTo restore: send /restbackup then upload your backup file."
        : "⚠️ Backup sent, but reset failed. Contact admin."
    ).catch(()=>{});

  } catch (e) {
    await bot.sendMessage(chatId, "❌ Failed to backup+reset: " + (e?.message || e)).catch(()=>{});
  }
  return;
}


// ===================== USER RESTORE: /restbackup =====================
if (text === "/restbackup" || text === "/rest") {
  const u = getUser(chatId);
  u.pendingAction = { type: "await_restore_backup", at: Date.now() };
  saveDB();
  await bot.sendMessage(chatId, "📤 Send the backup JSON file now (the one you received from /mybackup).", mainMenuKeyboard(chatId)).catch(()=>{});
  return;
}

// If user uploads a document while waiting for restore
try {
  const u = getUser(chatId);
  if (u?.pendingAction?.type === "await_restore_backup" && msg.document) {
    const doc = msg.document;
    const fileId = doc.file_id;

    // Basic validation: only accept json
    const name = String(doc.file_name || "");
    if (name && !name.toLowerCase().endsWith(".json")) {
      await bot.sendMessage(chatId, "❌ Please upload the .json backup file.").catch(()=>{});
      return;
    }

    await bot.sendMessage(chatId, "⏳ Restoring your backup...", mainMenuKeyboard(chatId)).catch(()=>{});

    const buf = await tgDownloadFileBuffer(fileId);
    let payload;
    try {
      payload = JSON.parse(buf.toString("utf8"));
    } catch (e) {
      await bot.sendMessage(chatId, "❌ Backup file is not valid JSON.").catch(()=>{});
      return;
    }

    // ✅ One-time restore: never allow restoring the same backup twice
    const backupId = String(payload.backupId || "").trim() || sha256Hex(buf);
    if (backupId && isBackupAlreadyUsed(backupId)) {
      // Clear pending action
      u.pendingAction = null;
      saveDB();
      await bot.sendMessage(chatId, "❌ This backup file was already used for restoration. You cannot restore the same file twice.").catch(()=>{});
      return;
    }

    const bKey = backupUniqueKey(payload);
      if (!bKey) throw new Error("Invalid backup file");
      db = repairDB(db);
      if (db.usedBackupIds && db.usedBackupIds[bKey]) {
        throw new Error("This backup file has already been used and is now expired.");
      }

      // Normal users: replace (wipe then restore). Business: merge/add on top.
      let _isBiz = false;
      try { _isBiz = (typeof isBusinessActive === "function" && isBusinessActive(chatId)) || (getUser(chatId).accountType === "business"); } catch (_) { _isBiz = false; }
      if (_isBiz) {
        mergeUserBackup(chatId, payload);
      } else {
        try { wipeUserData(chatId); } catch (_) {}
        applyUserBackup(chatId, payload);
      }

      db.usedBackupIds = db.usedBackupIds && typeof db.usedBackupIds === "object" ? db.usedBackupIds : {};
      db.usedBackupIds[bKey] = Date.now();
      saveDB();


    if (backupId) markBackupUsed(backupId);

    // Clear pending action
    u.pendingAction = null;
    saveDB();

    await bot.sendMessage(chatId, "✅ Backup restored successfully.", mainMenuKeyboard(chatId)).catch(()=>{});
    return;
  }
} catch (e) {
  await bot.sendMessage(chatId, "❌ Restore failed: " + (e?.message || e)).catch(()=>{});
  try {
    const u = getUser(chatId);
    u.pendingAction = null;
    saveDB();
...
Tip: Use the bot's admin panel buttons for Weekly Draw settings, broadcast, paying winners, and resets.`;
  ...
  if (/^\/backup(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) return sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });

  (async () => {
    try {
      // Ensure latest DB is flushed before snapshot
      try { saveDB(); } catch (_) {}

      // Full snapshot (all users, histories, settings, added packages, admin settings, etc.)
      const snapshot = {
        meta: { createdAt: Date.now(), schema: 1 },
        db,
        packages: PACKAGES,
      };

      const tmpPath = path.join(__dirname, `db_backup_${Date.now()}.json`);
      fs.writeFileSync(tmpPath, JSON.stringify(snapshot, null, 2));

      bot.sendDocument(chatId, tmpPath, {}, { filename: "db.json" }).catch(()=>{});

      try { fs.unlinkSync(tmpPath); } catch (_) {}

      // Wipe database after sending (reset all users), keep restore-lock map if any
      const keep = (db && db.usedBackupIds) ? db.usedBackupIds : {};
      db = repairDB({});
      db.usedBackupIds = keep;
      saveDB();

      sendTracked(chatId, "✅ Backup sent. Bot database wiped.", { ...mainMenuKeyboard(chatId) });
    } catch (e) {
      sendTracked(chatId, "⚠️ Backup failed: " + String(e?.message || e), { ...mainMenuKeyboard(chatId) });
    }
  })();
...
// ✅ /start cooldown (5 seconds) to reduce spam
  ...
  if (/^\/status$/i.test(text)) {
    const p = getLatestPendingForChat(chatId);
    if (!p) {
      sendTracked(chatId, `✅ No pending STK request found.
...
if (text === "✅ Activate" || text === "⚙️ Manage Plans") {
  ...
  sendTracked(chatId, header, { parse_mode: "Markdown", ...sellKeyboard(chatId) });
  return;
}
...
if (/^\/orders$/i.test(text)) {
  ...
  if (!arr.length) {
    sendTracked(chatId, "ℹ️ No recent orders yet.", { ...mainMenuKeyboard(chatId) });
    return;
  }
...
if (_bizPlanButtons.includes(text)) {
  ...
  if (!plan) {
    sendTracked(chatId, "❌ Invalid plan. Type /sell again.", { ...mainMenuKeyboard(chatId) });
    return;
  }
...
if (/^\/report$/i.test(text)) {
  if (!isAdmin(chatId)) {
    sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  ...
  sendTracked(chatId, msg, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
  return;
}
...
//   /addplan SMS Offers | Ksh 5 • 20 SMS 24HRS | 5
//   /addplan Bingwa Deals | Ksh 12 • 100MB 1HR | 12
// ✅ /backup (admin): create a global backup of DB (all users, pending, stats, etc.)
if (/^\/backup(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) return sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });

  const r = createGlobalBackup(chatId);
  if (!r.ok) return sendTracked(chatId, `⚠️ Backup failed: ${r.error}`, { ...mainMenuKeyboard(chatId) });

  return sendTracked(
    chatId,
    `✅ Global backup created.\n\nID: *${mdEscape(r.id)}*\n\nRestore (one-time):\n\`/rest ${mdEscape(r.id)}\`\n\nList:\n\`/backups\``,
    { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
  );
}

// ✅ /backups (admin): list recent backups
if (/^\/backups(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) return sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });

  const ids = listBackups(15);
  if (ids.length === 0) return sendTracked(chatId, "ℹ️ No backups found.", { ...mainMenuKeyboard(chatId) });

  const lines = ids.map((id) => {
    const used = (db?.usedBackupIds && db.usedBackupIds[id]) ? "✅ USED" : "🆕";
    return `• ${mdEscape(id)} — ${used}`;
  }).join("\n");
...
// ✅ /rest (admin): restore DB from a backup id (ONE TIME PER BACKUP FILE)
if (/^\/rest(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) return sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });

  const id = text.replace(/^\/rest(?:@\w+)?/i, "").trim();
  if (!id) {
    return sendTracked(
      chatId,
      "♻️ *Restore Backup*\n\nUse:\n`/rest bk-YYYYMMDD-HHMMSS`\n\n⚠️ One-time restore per backup file.",
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  }

  const r = restoreGlobalBackup(id, chatId);
  if (!r.ok) return sendTracked(chatId, `⚠️ Restore failed: ${r.error}`, { ...mainMenuKeyboard(chatId) });

  return sendTracked(chatId, `✅ Restored backup: *${mdEscape(r.id)}*`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
}
...
if (/(^|\n)\/addplan(?:@\w+)?\b/i.test(text)) {
  ...
  if (addLines.length === 0) {
    sendTracked(chatId, "⚠️ No /addplan lines found.", { ...mainMenuKeyboard(chatId) });
    return;
  }
...
if (/^\/removeoffer(?:@\w+)?\b/i.test(text)) {
  ...
  const catKey = Object.keys(PACKAGES || {}).find((c) => String(c).toLowerCase() === String(catInput).toLowerCase());
  if (!catKey) {
    sendTracked(chatId, `❌ Unknown category: *${mdEscape(catInput)}*`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
    return;
  }
...
if (/^\/addedoffers(?:@\w+)?\b/i.test(text)) {
  ...
  if (matchedCats.length === 0) {
    sendTracked(chatId, "ℹ️ No added offers found.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  ...
  sendTracked(chatId, out, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
  return;
}
...
if (/^\/credit\s+\d+\s+-?\d+\s+-?\d+/i.test(text)) {
  ...
  if (!targetId) {
    sendTracked(chatId, "❌ Invalid target.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  const u = getUser(targetId);
  ...
  try { saveDB(); } catch (_) {}
  sendTracked(chatId, `✅ Credited ${targetId}: tickets ${tickets}, pts ${pts}.`, { ...mainMenuKeyboard(chatId) });
...
if (/^\/refaudit\s+\d+$/i.test(text)) {
  ...
  if (!a) {
    sendTracked(chatId, "ℹ️ No referral audit data for that user.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  const lines = [];
  ...
  if (!pend) {
    sendTracked(chatId, "❌ No pending STK found for that user.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  try {
    await payheroStkPush({
    ...
    sendTracked(chatId, `✅ Resent STK to ${targetId}.`, { ...mainMenuKeyboard(chatId) });
  } catch (e) {
    sendTracked(chatId, `❌ Resend failed: ${mdEscape(e?.message || e)}`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
  }
  return;
}
...
Waiting for admin approval…`,
  ...
  if (text.startsWith("/")) {
    ...
    if (startPayload !== null) {
      ...
      sendTracked(chatId, welcomeText(msg.from?.first_name), {
        parse_mode: "Markdown",
        ...mainMenuKeyboard(chatId),
      });
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && act.type === "change_number") {
    ...
    if (/^\s*\/cancel\s*$/i.test(text) || text === "❌ Cancel") {
      clearPendingAction(chatId);
      sendTracked(chatId, "✅ Cancelled.", { ...mainMenuKeyboard(chatId) });
      return;
    }
    ...
    sendTracked(chatId, `✅ Number saved: *${mdEscape(maskPhone(phone))}*`, {
      parse_mode: "Markdown",
      ...mainMenuKeyboard(chatId),
    });
    return;
  ...
  if (act && act.type === "sell_business") {
    ...
    if (!plan) {
      clearPendingAction(chatId);
      sendTracked(chatId, "❌ Plan expired. Type /sell again.", { ...mainMenuKeyboard(chatId) });
      return;
    }
    ...
    try {
      ...
      deletePendingPayment(externalRef);
      sendTracked(chatId, `❌ STK failed: ${mdEscape(e?.message || e)}\n\nHelp: ${HELP_PHONE}`, {
        parse_mode: "Markdown",
        ...mainMenuKeyboard(chatId),
      });
      return;
...
if (text === "🛒 Buy Offers") {
    ...
    sessions.set(chatId, { step: "category", category: null, pkgKey: null, createdAt: Date.now() });
    await sendBannerIfAvailable(chatId);
    sendTracked(chatId, packagesOverviewText(), { parse_mode: "Markdown" });
    return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard());
  }
...
// ===================== HELP MENU (BUTTON) =====================
if (text === "ℹ️ Help" || /^\/help\b/i.test(text) || /^help$/i.test(text)) {
  await sendHelpMenu(chatId);
  return;
}
...
if (text === "👤 My Profile" || /^\/profile(?:@\w+)?$/i.test(text)) {
  return showUserProfile(chatId);
}
...
// ===================== PURCHASE FLOW =====================
  ...
  try {
    ...
    if (s.step === "fast_phone_pick") {
      ...
      if (text === "➕ New Number" || text === "+ New Number") {
        s.step = "phone";
        sessions.set(chatId, s);
        return sendTracked(
          chatId,
    ...
    if (s.step === "category") {
      ...
      if (category === "Bingwa Deals") {
        sendTracked(chatId, `📦 *Bingwa Deals*\nRule: *Once per day per phone number* (ONLY after success).\n\nChoose a package:`, {
          parse_mode: "Markdown",
        });
      } else {
        sendTracked(chatId, `✅ Choose a ${category} package:`);
      }
...
Approve override?`,
    ...
    throw new Error("⚠️ This phone number is locked to another account. An approval request has been sent to admin.");
  }
...
bot.on("callback_query", async (q) => {
  ...
  if (action === "phone_decline") {
    ...
    await bot.sendMessage(
      Number(req.requestingChatId),
      `❌ Phone usage denied. This number is locked to another account.`
    ).catch(() => {});
...
if (/^\/rest(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) return sendTracked(chatId,"❌ Not authorized.",{...mainMenuKeyboard(chatId)});
  restoreWaiting = chatId;
  return sendTracked(chatId,"📥 Send the backup JSON file now (db.json).",{...mainMenuKeyboard(chatId)});
}

// handle uploaded backup file
if (msg.document && restoreWaiting === chatId) {
  (async () => {
    try {
      ...
      const data = (payload && payload.db && typeof payload.db === "object") ? payload.db : payload;

      // Restore packages if included (custom offers etc.)
      try {
        const pk = payload && payload.packages;
      ...
      sendTracked(chatId, "✅ Backup restored successfully.", { ...mainMenuKeyboard(chatId) });
    } catch (e) {
      sendTracked(chatId, "⚠️ Restore failed: " + String(e?.message || e), { ...mainMenuKeyboard(chatId) });
    }
  })();
