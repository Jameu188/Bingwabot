...
// ✅ Sends M-Pesa code in success message: "Payment confirmed ref: XXXXX at time on date"
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // ✅ Notify user (includes M-Pesa code + time/date)
    await sendTracked(
      chatId,
      `✅ Payment confirmed ref: ${mpesaOut} at ${when}.${details}\n\nYour package will be processed now.`,
      { ...mainMenuKeyboard() }
    );
