...
async function broadcastToAllUsers(messageText) {
  ...
  for (const cid of ids) {
    ...
    if (!chatId) continue;
    try { await bot.sendMessage(chatId, messageText, mainMenuKeyboard(chatId)); } catch (_) {}
  }
}
...
async function weeklyDrawFreezeAndAnnounce(reasonText) {
  ...
  for (const w of winners) {
    try {
      ...
      w.name = u.fullName || u.username || String(w.chatId);
    } catch (_) {}
  }
...
async function weeklyDrawRankChangeNotify() {
  ...
  for (const [cid, newRank] of Object.entries(newRanks)) {
    ...
    if (newRank < oldRank) {
      ...
      try { await bot.sendMessage(Number(cid), msg, mainMenuKeyboard(Number(cid))); } catch (_) {}
    } else {
      // dropped (overtaken)
      const msg =
  ...
  try {
    ...
    bot.on("message", async (msg) => {
      ...
      if (text === "🎉 WEEKLY DRAW") {
        ...
        const status = String(db.campaign.status || "OFF").toUpperCase();
        const heading = String(db.campaign.heading || "WEEKLY DRAW").toUpperCase();

        // Countdown (if endAt set)
        let countdown = "N/A";
        const endAt = Number(db.campaign.endAt || 0);
        if (endAt > Date.now()) {
          const ms = endAt - Date.now();
          const d = Math.floor(ms / (24*60*60*1000));
          const h = Math.floor((ms % (24*60*60*1000)) / (60*60*1000));
        ...
        } else if (endAt > 0 && endAt <= Date.now()) {
          countdown = "Ended";
        }

        // Show frozen winners if ended
        if (status === "ENDED" && Array.isArray(db.campaign.winners) && db.campaign.winners.length) {
          const lines = db.campaign.winners.slice(0, 3).map((w, i) => {
            const pos = i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉";
            const paid = w.paid ? "Paid ✅" : "Pending ⏳";
            return `${pos} ${w.name || w.chatId} — Ksh ${Number(w.prize || 0)} (${paid})`;
          });
          
// Build Top 10 participants (by tickets), qualified only
const entries = (db.campaign && db.campaign.entries && typeof db.campaign.entries === "object") ? db.campaign.entries : {};
const ranked = Object.entries(entries)
...
const img = String((db.campaign && db.campaign.image) || "").trim();
if (img) {
  return bot.sendPhoto(chatId, img, { caption, ...mainMenuKeyboard(chatId) }).catch(() => bot.sendMessage(chatId, caption, mainMenuKeyboard(chatId)).catch(()=>{}));
}
return bot.sendMessage(chatId, caption, mainMenuKeyboard(chatId)).catch(() => {});
return bot.sendMessage(chatId, msgText, mainMenuKeyboard(chatId)).catch(() => {});
        ...
        const msgText =
          ...
          `Status: ${status}\n` +
          `⏳ Ends in: ${countdown}\n\n` +
          `ℹ️ Weekly Draw is currently available.`;
        return bot.sendMessage(chatId, msgText, mainMenuKeyboard(chatId)).catch(() => {});
      }
