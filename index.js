...
if (data.startsWith("wdusr:")) {
  ...
  if (action === "optin") {
    ...
    // After joining, suggest quick packages to boost tickets
    try { await wdSendJoinSuggestedPackages(actorId); } catch (_) {}
    return;
  }
...
async function wdSendJoinSuggestedPackages(chatId) {
  const offers = wdPickSuggestedOffers(4);
  const msg =
    `🎉 You joined the Weekly Draw!\n\n` +
    `🔥 Boost your tickets now by buying a quick deal below.\n` +
    `🏆 Buy any deal and you might win up to *Ksh 1500* weekly.\n\n` +
    `👇 Choose one package:`;
  return sendTracked(chatId, msg, { parse_mode: "Markdown", ...wdSuggestedOffersKeyboard(offers) });
}
