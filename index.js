...
async function broadcastWeeklyDrawLaunch(force = false) {
  ...
  for (const cid of userIds) {
    ...
    try {
      if (img) {
        await bot.sendPhoto(cid, img, { caption: msgText }).catch(async () => {
          await bot.sendMessage(cid, msgText).catch(() => {});
        });
      } else {
        await bot.sendMessage(cid, msgText).catch(() => {});
      }
...
function writeDailyBackupIfNeeded() {
  try {
    ...
    bot.sendMessage(String(ADMIN_ID), `✅ DB backup saved: backup_${day}.json`).catch(() => {});
  } catch (e) {
    bot.sendMessage(String(ADMIN_ID), `⚠️ DB backup failed: ${String(e?.message || e)}`).catch(() => {});
  }
}
// check every hour
setInterval(() => { setTimeout(() => { try { writeDailyBackupIfNeeded(); } catch (_) {} }, 10 * 1000); }, 60 * 60 * 1000);
...
function healthPayheroFail(err) {
  ...
  if (_payheroFailCount >= 5) {
    _payheroFailCount = 0;
    bot.sendMessage(String(ADMIN_ID), `🚨 PayHero callback errors (>=5 in 10min). Latest: ${String(err?.message || err)}`).catch(() => {});
  }
}
