...
function setUserPhone(chatId, phone254) {
  ...
  if (owner && owner !== cid) {
    return {
      ok: false,
      reason: "This phone number is already saved on another account.
  ...
  if (act && !isAdmin(chatId) && act.kind === "redeem") {
    ...
    if (act.step === "confirm") {
      if (text === "✅ Confirm") {
        ...
        if (savedPhone && isPhoneLockedToAnotherAccount(savedPhone, chatId)) {
          sessions.delete(chatId);
  ...
  return sendTracked(chatId, msgText, { 
    ...
    if (s.step === "confirm") {
      ...
      const phoneMaybe = normalizePhone(text);
        if (phoneMaybe && isPhoneLockedToAnotherAccount(phoneMaybe, chatId)) {
          return sendTracked(chatId, "This phone number is saved on another account and cannot be used.", { ...mainMenuKeyboard(chatId) });
        }
      if (phoneMaybe) {
