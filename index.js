...
async function autoAddChatToForceJoin(chat) {
  try {
    ...
    // Register the chat so the admin can manually MARK it in the
    // Channel Promo > Force Join Targets selector.
    registerInlineKnownChat(chat);

    // Keep channel activity metadata, but DO NOT auto-add any new
    // channel/group to force join anymore.
    ...
    try {
      ...
      await bot.sendMessage(
        String(ADMIN_ID),
        `ℹ️ New ${String(chat.type || "chat")} detected:
${label}
Not added to force join automatically. Mark it manually in Channel Promo > Force Join Targets if needed.`
      ).catch(() => {});
    } catch (_) {}
