...
function restoreBusinessSettings(user) {
  if (!user || !user._bizBackup) return;
  user.fastPurchaseEnabled = !!user._bizBackup.fastPurchaseEnabled;
  user.autoRetryStkEnabled = !!user._bizBackup.autoRetryStkEnabled;
}
...
function isBusinessActive(chatId) {
  ...
  if (Date.now() > u.subscriptionExpiry) {
    ...
    try { // handled by disableBusinessFeatures } catch (_) {}
    saveDB();
    return false;}
  return true;
}
...
if (data === "ar:toggle") {
  ...
  if (!isBusinessActive(chatId)) {
    ...
    if (u.autoRetryStkEnabled) {  try { saveDB(); } catch (_) {} }
    await bot.answerCallbackQuery(q.id, { text: "💼 Business only. Type /sell to upgrade." }).catch(() => {});
    return;
  }
  ...
  if (typeof isBusinessActive === "function") {
    if (!isBusinessActive(chatId)) {
      if (u.fastPurchaseEnabled) {
        // handled by disableBusinessFeatures
        saveDB();
      }
      return false;
    }
...
function _ensureBusinessAndResetPrefs(chatId) {
  ...
  if (typeof isBusinessActive === "function") {
    if (!isBusinessActive(chatId)) {
      let changed = false;
      if (u.autoRetryStkEnabled) {  changed = true; }
