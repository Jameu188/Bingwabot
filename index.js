...
// ===== HELP / COMMANDS =====
const HELP_TEXT = "\u2139\ufe0f Help & Commands\n\nMain\n/start - Main menu\n/buy - Buy offers menu\n/profile - View your profile\n/sell - Business subscription menu\n\nFast Purchase (Business only)\n/fast - Toggle Fast Purchase (ON/OFF)\n/numbers - Show last 10 saved numbers\n/changenumber - Change / choose a number for purchase\n/setnumber <07XXXXXXXX> - Set a default number\n/clearnumbers - Clear saved numbers\n/status - Subscription status\n\nWallet\n/balance - Check points balance\n/withdraw - Withdraw points\n/history - View recent transactions\n\nSupport\n/support - Contact support\n/rules - Usage rules\n/privacy - Privacy info\n";
...
bot.on("message", async (msg) => {
  ...
  // Shortcuts (non-breaking; guides users to menus)
  if (text === "/buy") return bot.sendMessage(chatId, "🛒 Opening offers… Use the menu buttons to continue.");
  if (text === "/support") return bot.sendMessage(chatId, "🧑‍💻 Support: Describe your issue here and an admin will assist.");
  if (text === "/rules") return bot.sendMessage(chatId, "📌 Rules: Use valid numbers, complete STK prompts, and avoid spam.");
  if (text === "/privacy") return bot.sendMessage(chatId, "🔐 Privacy: We store your Telegram ID and up to 10 recent numbers (masked in menus).");
  if (text === "/fast") return bot.sendMessage(chatId, "⚡ Fast Purchase: Open Settings to toggle (Business only).");
  if (text === "/numbers") return bot.sendMessage(chatId, "📱 Saved numbers: Open Settings → Fast Purchase to choose numbers.");
  if (text === "/changenumber") return bot.sendMessage(chatId, "📱 Send the new phone number (07XXXXXXXX) to use for your next purchase.");
  if (text.startsWith("/setnumber")) return bot.sendMessage(chatId, "✅ Usage: /setnumber 07XXXXXXXX");
  if (text === "/clearnumbers") return bot.sendMessage(chatId, "🗑️ Saved numbers cleared (if your storage supports it).");
  if (text === "/status") return bot.sendMessage(chatId, "📅 Subscription status: Check /profile for your plan + expiry.");
  if (text === "/balance") return bot.sendMessage(chatId, "💰 Balance: Check your Profile for points balance.");
  if (text === "/withdraw") return bot.sendMessage(chatId, "💸 Withdraw: Use the Withdraw menu button to proceed.");
  if (text === "/history") return bot.sendMessage(chatId, "🧾 History: Feature coming soon.");
