...
function isWeeklyDrawLeaderboardEligible(chatId) {
  ...
  try {
    ...
    if (page === "more") {
      rows.push([{ text: tr(chatId,"⬅️ BACK","⬅️ NYUMA"), callback_data: "wdusr:back" }]);
      // keep opt toggle visible on info pages too
      if (!opted) rows.push([{ text: tr(chatId,"✅ JOIN","✅ JIUNGE"), callback_data: "wdusr:optin" }]);
      else rows.push([{ text: tr(chatId,"🚫 LEAVE","🚫 ONDOKA"), callback_data: "wdusr:optout" }]);
      rows.push([{ text: tr(chatId,"🏆 PREVIOUS WINNERS","🏆 WASHINDI WA AWALI"), callback_data: "wdusr:prev" }]);
...
try {
  ...
  if (rec && rec.paid === true) {
    ...
    rows.push(
      [{ text: tr(chatId,"ℹ️ MORE…","ℹ️ ZAIDI…"), callback_data: "wdusr:more" }],
      [{ text: tr(chatId,"🏆 PREVIOUS WINNERS","🏆 WASHINDI WA AWALI"), callback_data: "wdusr:prev" }]
    );
...
function weeklyDrawLastWinnersText(chatId) {
  ...
  function buildWeeklyDrawMainCaption(chatId) {
    ...
    const listLines = ranked.length
      ...
      : tr(chatId, "No qualified participants yet.", "Bado hakuna washiriki walioqualify.");
...
let optedLine = tr(chatId, "ℹ️ You are not opted in (no campaign messages). Tap ✅ JOIN to participate.", "ℹ️ Hujajiunga (hutapokea ujumbe wa kampeni). Bonyeza ✅ JIUNGE kushiriki.");
...
async function openWeeklyDrawUserMenu(chatId) {
  const c = ensureWeeklyDrawObject();
  if (!c.enabled) {
    return bot.sendMessage(chatId, tr(chatId,"❌ WEEKLY DRAW is not active right now.","❌ Droo ya Wiki haiko hewani kwa sasa."), mainMenuKeyboard(chatId)).catch(() => {});
}
...
if (data.startsWith("wdusr:")) {
  ...
  if (action === "prev") {
    ...
    const txt = weeklyDrawLastWinnersText(chatId).trim() || "No previous winners yet.";
...
if (isBtn(text,"🛒 Buy Offers","🛒 Nunua Ofa")) {
  ...
  if (isBtn(text, "🎉 WEEKLY DRAW", "🎉 DROO YA WIKI") || /^\/weeklydraw(?:@\w+)?$/i.test(text) || /^droo ya wiki$/i.test(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    try { sessions.delete(chatId); } catch (_) {}
