...
// ===============================
// 🎁 PICK A BOX GAME (Entry: 10 KSH OR 15 pts)
// Daily limit: 25 plays
// Selection expires after 60 seconds
// Max reward: 100 pts (ultra rare)
// ===============================

const boxGameSessions = new Map(); 
const boxGameDaily = new Map(); 

function todayKeyLocal() {
  return new Date().toISOString().slice(0,10);
}

function canPlayBoxGame(chatId) {
  const today = todayKeyLocal();
  if (!boxGameDaily.has(chatId)) {
    boxGameDaily.set(chatId, { date: today, count: 0 });
    return true;
  }
  const record = boxGameDaily.get(chatId);
  if (record.date !== today) {
    boxGameDaily.set(chatId, { date: today, count: 0 });
    return true;
  }
  return record.count < 25;
}

function incrementPlay(chatId) {
  const today = todayKeyLocal();
  if (!boxGameDaily.has(chatId)) {
    boxGameDaily.set(chatId, { date: today, count: 1 });
    return;
  }
  const record = boxGameDaily.get(chatId);
  if (record.date !== today) {
...
function generateRewards() {
  ...
  if (rareRoll < 0.001) {
    rewards.push(Math.floor(Math.random() * 20) + 81);
  } else if (rareRoll < 0.015) {
    rewards.push(Math.floor(Math.random() * 40) + 41);
  }
