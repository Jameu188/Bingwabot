...
// ============================================================================
// ✅ PATCH: 3-HOUR PROTECTED PROMO POSTS + 30-MINUTE REPOST + RESTORE/REVOKE
// ============================================================================
(function installProtectedPromoPatch(){
  const PROTECTED_PROMO_MS = 3 * 60 * 60 * 1000;
  const DEFAULT_REPOST_MS = 30 * 60 * 1000;

  function ensureProtectedPromoStore() {
    db = repairDB(db);
    const store = ensureChannelPromoStore();
    if (!store.protected || typeof store.protected !== 'object') store.protected = {};
    if (!store.protected.posts || typeof store.protected.posts !== 'object') store.protected.posts = {};
    if (!store.protected.byMessage || typeof store.protected.byMessage !== 'object') store.protected.byMessage = {};
    if (!store.protected.defaults || typeof store.protected.defaults !== 'object') {
      store.protected.defaults = {
        protectMs: PROTECTED_PROMO_MS,
        repostMs: DEFAULT_REPOST_MS,
        earningsPts: 0
      };
    }
    if (!store.protected.audit || typeof store.protected.audit !== 'object') store.protected.audit = {};

    const rcfg = channelPromoRepostCfg();
    if (!Number(rcfg.intervalMs || 0) || Number(rcfg.intervalMs || 0) !== DEFAULT_REPOST_MS) {
      rcfg.intervalMs = DEFAULT_REPOST_MS;
      if (!Number(rcfg.nextRunAt || 0)) rcfg.nextRunAt = Date.now() + DEFAULT_REPOST_MS;
    }
    if (typeof rcfg.enabled !== 'boolean') rcfg.enabled = true;

    return store.protected;
  }

  function promoSnapshotFromConfig() {
    const cfg = channelPromoCfg();
    const media = Array.isArray(cfg.media) ? cfg.media.map((m) => ({ type: String(m && m.type || 'photo'), file_id: String(m && m.file_id || '') })).filter((m) => m.file_id) : [];
    return {
      mode: String(cfg.mode || 'text'),
      text: String(cfg.text || ''),
      caption: String(cfg.caption || ''),
      media,
      buttonText: String(cfg.buttonText || ''),
      buttonUrl: String(cfg.buttonUrl || ''),
      pinNew: !!cfg.pinNew,
      unpinOld: !!cfg.unpinOld
    };
  }

  function findPromoOwnerIdByChat(chatId) {
    try {
      const suffix = `__${String(chatId)}`;
      for (const [k, rec] of Object.entries((db && db.promoLinkOwners) ? db.promoLinkOwners : {})) {
        if (String(k).endsWith(suffix) && rec && rec.ownerId) return String(rec.ownerId);
      }
    } catch (_) {}
    return '';
  }

  function nextProtectedPromoId(chatId, messageId) {
    return `pp_${String(chatId)}_${String(messageId)}_${Date.now()}`;
  }

  function getProtectedPromoByMessage(chatId, messageId) {
    try {
      const store = ensureProtectedPromoStore();
      const key = `${String(chatId)}:${String(messageId)}`;
      const promoId = store.byMessage[key];
      if (!promoId) return null;
      return store.posts[promoId] || null;
    } catch (_) {
      return null;
    }
  }

  function isProtectedPromoActive(chatId, messageId) {
    const rec = getProtectedPromoByMessage(chatId, messageId);
    if (!rec) return false;
    return !rec.deleted && !rec.revoked && Date.now() < Number(rec.protectedUntil || 0);
  }

  function registerProtectedPromoPost(chatId, messageId, extra = {}) {
    try {
      const store = ensureProtectedPromoStore();
      const key = `${String(chatId)}:${String(messageId)}`;
      const existingId = store.byMessage[key];
      const ownerId = String(extra.ownerId || findPromoOwnerIdByChat(chatId) || '');
      const defaults = store.defaults || {};
      const earningsPts = Math.max(0, Number(extra.earningsPts != null ? extra.earningsPts : defaults.earningsPts || 0));
      const now = Date.now();
      const rec = existingId && store.posts[existingId] ? store.posts[existingId] : {
        promoId: nextProtectedPromoId(chatId, messageId),
        chatId: String(chatId),
        title: String((ensurePromoChannelMetric(chatId) || {}).title || chatId),
        ownerId,
        createdAt: now,
        protectedUntil: now + Math.max(1000, Number(defaults.protectMs || PROTECTED_PROMO_MS)),
        repostEveryMs: Math.max(60000, Number(defaults.repostMs || DEFAULT_REPOST_MS)),
        nextRepostAt: now + Math.max(60000, Number(defaults.repostMs || DEFAULT_REPOST_MS)),
        earningsPts,
        earningsCredited: false,
        earningsReversed: false,
        deleted: false,
        revoked: false,
        restoredAt: 0,
        deletedAt: 0,
        deleteReason: '',
        messageIds: [],
        configSnapshot: promoSnapshotFromConfig(),
        history: []
      };

      rec.chatId = String(chatId);
      rec.title = String((ensurePromoChannelMetric(chatId) || {}).title || rec.title || chatId);
      rec.ownerId = ownerId || rec.ownerId || '';
      rec.lastMessageId = Number(messageId || 0);
      if (!Array.isArray(rec.messageIds)) rec.messageIds = [];
      if (!rec.messageIds.includes(Number(messageId || 0))) rec.messageIds.push(Number(messageId || 0));
      rec.configSnapshot = extra.configSnapshot || rec.configSnapshot || promoSnapshotFromConfig();
      rec.repostedAt = now;
      if (!rec.createdAt) rec.createdAt = now;
      if (!rec.protectedUntil) rec.protectedUntil = now + Math.max(1000, Number(defaults.protectMs || PROTECTED_PROMO_MS));
      if (!rec.repostEveryMs) rec.repostEveryMs = Math.max(60000, Number(defaults.repostMs || DEFAULT_REPOST_MS));
      rec.nextRepostAt = now + Number(rec.repostEveryMs || DEFAULT_REPOST_MS);
      rec.deleted = false;
      rec.revoked = false;
      rec.deletedAt = 0;
      rec.deleteReason = '';
      if (!Array.isArray(rec.history)) rec.history = [];
      rec.history.push({ at: now, action: existingId ? 'repost' : 'create', messageId: Number(messageId || 0) });

      store.posts[rec.promoId] = rec;
      store.byMessage[key] = rec.promoId;

      if (rec.ownerId && earningsPts > 0 && !rec.earningsCredited) {
        try {
          addPoints(rec.ownerId, earningsPts);
          rec.earningsCredited = true;
          rec.history.push({ at: now, action: 'credit', ownerId: rec.ownerId, pts: earningsPts });
        } catch (_) {}
      }
  ...
  async function sendProtectedPromoSnapshot(chatId, snapshot) {
    const snap = snapshot || promoSnapshotFromConfig();
    const kb = promoTrackedButtonKeyboard(chatId);
    let messageId = 0;

    if (String(snap.mode || '') === 'video' && Array.isArray(snap.media) && snap.media[0] && snap.media[0].file_id) {
      const sent = await bot.sendVideo(chatId, snap.media[0].file_id, {
        caption: String(snap.caption || snap.text || ''),
        ...(kb || {})
      }).catch(() => null);
      if (sent && sent.message_id) messageId = Number(sent.message_id || 0);
    } else if (String(snap.mode || '') === 'photo' && Array.isArray(snap.media) && snap.media[0] && snap.media[0].file_id) {
      const sent = await bot.sendPhoto(chatId, snap.media[0].file_id, {
        caption: String(snap.caption || snap.text || ''),
        ...(kb || {})
      }).catch(() => null);
      if (sent && sent.message_id) messageId = Number(sent.message_id || 0);
    } else if (Array.isArray(snap.media) && snap.media[0] && snap.media[0].file_id) {
      const sent = await bot.sendPhoto(chatId, snap.media[0].file_id, {
        caption: String(snap.caption || snap.text || ''),
        ...(kb || {})
      }).catch(() => null);
      ...
      const sent = await bot.sendMessage(chatId, String(snap.text || snap.caption || ''), kb || {}).catch(() => null);
      if (sent && sent.message_id) messageId = Number(sent.message_id || 0);
    }

    if (messageId && snap.pinNew) {
      try { await bot.pinChatMessage(chatId, messageId, { disable_notification: true }); } catch (_) {}
    }
    if (messageId) {
      try { trackPromoSend(chatId, messageId); } catch (_) {}
    }
    return messageId;
  }

  async function reverseProtectedPromoEarnings(rec, reason) {
    try {
      if (!rec || !rec.ownerId) return false;
      const pts = Number(rec.earningsPts || 0);
      if (!(pts > 0)) return false;
      if (!rec.earningsCredited || rec.earningsReversed) return false;
      deductPoints(rec.ownerId, pts);
      rec.earningsReversed = true;
      if (!Array.isArray(rec.history)) rec.history = [];
      rec.history.push({ at: Date.now(), action: 'reverse', ownerId: rec.ownerId, pts, reason: String(reason || 'delete_lt_3h') });
      saveDB();
      return true;
    } catch (_) {
      return false;
  ...
  async function revokeProtectedPromo(rec, reason) {
    if (!rec) return false;
    const now = Date.now();
    let deletedOne = false;
    const ids = Array.isArray(rec.messageIds) ? rec.messageIds.slice().reverse() : [rec.lastMessageId].filter(Boolean);
    for (const mid of ids) {
      const ok = await bot.deleteMessage(rec.chatId, mid, { __forceProtectedDelete: true, __protectedReason: reason || 'revoke' }).then(() => true).catch(() => false);
      if (ok) deletedOne = true;
    }
    rec.deleted = true;
    rec.revoked = true;
    rec.deletedAt = now;
    rec.deleteReason = String(reason || 'revoke');
    if (now < Number(rec.protectedUntil || 0)) {
      await reverseProtectedPromoEarnings(rec, reason || 'revoke_lt_3h').catch(() => false);
    }
    if (!Array.isArray(rec.history)) rec.history = [];
    rec.history.push({ at: now, action: 'revoke', reason: String(reason || 'revoke') });
    saveDB();
    return deletedOne;
  }

  async function restoreProtectedPromo(rec) {
    if (!rec) return false;
    const messageId = await sendProtectedPromoSnapshot(rec.chatId, rec.configSnapshot || promoSnapshotFromConfig()).catch(() => 0);
    ...
    if (!Array.isArray(rec.messageIds)) rec.messageIds = [];
    if (!rec.messageIds.includes(Number(messageId || 0))) rec.messageIds.push(Number(messageId || 0));
    store.byMessage[`${String(rec.chatId)}:${String(messageId)}`] = rec.promoId;
    if (!Array.isArray(rec.history)) rec.history = [];
    rec.history.push({ at: now, action: 'restore', messageId: Number(messageId || 0) });
    saveDB();
    return true;
  }

  function getProtectedPromoByRef(raw) {
    const input = String(raw || '').trim();
    if (!input) return null;
    const store = ensureProtectedPromoStore();
    if (store.posts[input]) return store.posts[input];
    if (/^-?\d+$/.test(input)) {
      const byChat = Object.values(store.posts).filter((x) => x && String(x.chatId) === input).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
      if (byChat.length) return byChat[0];
      const byMsg = Object.values(store.posts).filter((x) => x && Array.isArray(x.messageIds) && x.messageIds.includes(Number(input))).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
      if (byMsg.length) return byMsg[0];
    }
    return null;
  }

  const __origTrackPromoSend = trackPromoSend;
  trackPromoSend = function patchedTrackPromoSend(chatId, messageId) {
    try { __origTrackPromoSend(chatId, messageId); } catch (_) {}
    try {
      registerProtectedPromoPost(chatId, messageId, {
        ownerId: findPromoOwnerIdByChat(chatId),
        configSnapshot: promoSnapshotFromConfig()
      });
    } catch (_) {}
  };

  const __origDeleteMessage = bot.deleteMessage.bind(bot);
  bot.deleteMessage = async function patchedDeleteMessage(chatId, messageId, options) {
    const opts = options && typeof options === 'object' ? options : {};
    if (!opts.__forceProtectedDelete && isProtectedPromoActive(chatId, messageId)) {
      return false;
    }
    const out = await __origDeleteMessage(chatId, messageId);
    try {
      const rec = getProtectedPromoByMessage(chatId, messageId);
      if (rec && !opts.__skipProtectedBookkeeping) {
        rec.deleted = true;
        rec.deletedAt = Date.now();
        rec.deleteReason = String(opts.__protectedReason || 'delete');
        if (!Array.isArray(rec.history)) rec.history = [];
        rec.history.push({ at: Date.now(), action: 'delete', messageId: Number(messageId || 0), reason: rec.deleteReason });
        if (Date.now() < Number(rec.protectedUntil || 0)) {
          await reverseProtectedPromoEarnings(rec, rec.deleteReason).catch(() => false);
        }
        saveDB();
      }
    } catch (_) {}
  ...
  const __origRunChannelPromoRepostIfDue = runChannelPromoRepostIfDue;
  runChannelPromoRepostIfDue = async function patchedRunChannelPromoRepostIfDue() {
    try { ensureProtectedPromoStore(); } catch (_) {}
    try { await __origRunChannelPromoRepostIfDue(); } catch (_) {}

    try {
      const store = ensureProtectedPromoStore();
      const now = Date.now();
      for (const rec of Object.values(store.posts || {})) {
        if (!rec || rec.deleted || rec.revoked) continue;
        if (!rec.chatId || !rec.lastMessageId) continue;
        if (now < Number(rec.nextRepostAt || 0)) continue;
        const messageId = await sendProtectedPromoSnapshot(rec.chatId, rec.configSnapshot || promoSnapshotFromConfig()).catch(() => 0);
        if (messageId) {
          rec.lastMessageId = Number(messageId || 0);
          if (!Array.isArray(rec.messageIds)) rec.messageIds = [];
          if (!rec.messageIds.includes(Number(messageId || 0))) rec.messageIds.push(Number(messageId || 0));
          rec.nextRepostAt = now + Math.max(60000, Number(rec.repostEveryMs || DEFAULT_REPOST_MS));
          rec.repostedAt = now;
          rec.deleted = false;
          rec.deleteReason = '';
          store.byMessage[`${String(rec.chatId)}:${String(messageId)}`] = rec.promoId;
          if (!Array.isArray(rec.history)) rec.history = [];
          rec.history.push({ at: now, action: 'scheduler_repost', messageId: Number(messageId || 0) });
          saveDB();
        }
      }
    } catch (_) {}
  ...
  bot.on('message', async (msg) => {
    try {
      ...
      if (/^\/setpromoearnings(?:@\w+)?\s+(-?\d+(?:\.\d+)?)$/i.test(text)) {
        const m = text.match(/^\/setpromoearnings(?:@\w+)?\s+(-?\d+(?:\.\d+)?)$/i);
        const pts = Math.max(0, Number(m && m[1] || 0));
        ensureProtectedPromoStore().defaults.earningsPts = pts;
        saveDB();
        await bot.sendMessage(chatId, `✅ Promo earnings per protected post set to ${pts} pts.`).catch(() => {});
        return;
      }

      if (/^\/restorepost(?:@\w+)?\s+(.+)$/i.test(text)) {
        const m = text.match(/^\/restorepost(?:@\w+)?\s+(.+)$/i);
        const rec = getProtectedPromoByRef(m && m[1]);
        if (!rec) {
          await bot.sendMessage(chatId, '❌ Protected promo not found. Use promoId, chatId, or tracked messageId.').catch(() => {});
          return;
        }
        const ok = await restoreProtectedPromo(rec).catch(() => false);
        await bot.sendMessage(chatId, ok ? `✅ Promo restored.
Promo ID: ${rec.promoId}
Target: ${rec.title || rec.chatId}` : '❌ Failed to restore that promo.').catch(() => {});
        return;
      }

      if (/^\/revokepost(?:@\w+)?\s+(.+)$/i.test(text)) {
        const m = text.match(/^\/revokepost(?:@\w+)?\s+(.+)$/i);
        const rec = getProtectedPromoByRef(m && m[1]);
        if (!rec) {
          await bot.sendMessage(chatId, '❌ Protected promo not found. Use promoId, chatId, or tracked messageId.').catch(() => {});
          return;
        }
        const ok = await revokeProtectedPromo(rec, 'admin_revoke').catch(() => false);
        await bot.sendMessage(chatId, ok ? `✅ Promo revoked.
Promo ID: ${rec.promoId}
Target: ${rec.title || rec.chatId}` : '❌ Failed to revoke that promo.').catch(() => {});
        return;
      }

      if (/^\/promostatus(?:@\w+)?(?:\s+(.+))?$/i.test(text)) {
        const m = text.match(/^\/promostatus(?:@\w+)?(?:\s+(.+))?$/i);
        const ref = m && m[1] ? m[1] : '';
        let rec = ref ? getProtectedPromoByRef(ref) : null;
        if (!rec) {
          const store = ensureProtectedPromoStore();
          rec = Object.values(store.posts || {}).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0))[0] || null;
        }
        if (!rec) {
          await bot.sendMessage(chatId, 'ℹ️ No protected promo posts tracked yet.').catch(() => {});
          return;
        }
        const minsLeft = Math.max(0, Math.ceil((Number(rec.protectedUntil || 0) - Date.now()) / 60000));
        await bot.sendMessage(chatId,
          `📣 Protected Promo Status\n\n` +
          `Promo ID: ${rec.promoId}\n` +
          `Target: ${rec.title || rec.chatId}\n` +
          `Chat ID: ${rec.chatId}\n` +
          `Last Message ID: ${rec.lastMessageId || 0}\n` +
          `Protected: ${Date.now() < Number(rec.protectedUntil || 0) ? 'YES' : 'NO'}\n` +
          `Minutes left: ${minsLeft}\n` +
          `Repost every: ${Math.round(Number(rec.repostEveryMs || DEFAULT_REPOST_MS) / 60000)} minute(s)\n` +
          `Earnings pts: ${Number(rec.earningsPts || 0)}\n` +
          `Credited: ${rec.earningsCredited ? 'YES' : 'NO'}\n` +
          `Reversed: ${rec.earningsReversed ? 'YES' : 'NO'}\n` +
          `Deleted: ${rec.deleted ? 'YES' : 'NO'}\n` +
  ...
  try { ensureProtectedPromoStore(); saveDB(); } catch (_) {}
})();
