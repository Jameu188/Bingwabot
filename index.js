...
// Merge keeping current data, but snapshot overwrites matching fields/keys.
// Objects: recursive merge. Arrays: union unique (keeps current + snapshot).
function mergeKeepCurrentButRestoreSnapshot(currentObj, snapshotObj) {
  if (Array.isArray(currentObj) || Array.isArray(snapshotObj)) {
    const a = Array.isArray(currentObj) ? currentObj : [];
    const b = Array.isArray(snapshotObj) ? snapshotObj : [];
    const seen = new Set();
    const out = [];
    for (const it of [...a, ...b]) {
  ...
  if (!isPlainObject(currentObj) && !isPlainObject(snapshotObj)) {
    return snapshotObj !== undefined ? snapshotObj : currentObj;
  }

  const cur = isPlainObject(currentObj) ? currentObj : {};
  const snap = isPlainObject(snapshotObj) ? snapshotObj : {};
  const out = { ...cur };

  for (const k of Object.keys(snap)) {
    if (k in cur) out[k] = mergeKeepCurrentButRestoreSnapshot(cur[k], snap[k]);
    else out[k] = snap[k];
  }
  return out;
}
...
function restoreGlobalBackup(backupId, adminChatId) {
  ...
  const currentDB = repairDB(db);
  const merged = mergeKeepCurrentButRestoreSnapshot(currentDB, snapshot);
  db = repairDB(merged);
...
🧹 Bot data wiped (all users reset).\n\nID: *${mdEscape(r.id)}*\n\nRestore (one-time):\n\`/restore ${mdEscape(r.id)}\`\n\nList:\n\`/backups\``,
    { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
  );
}
...
// ✅ /restore (admin): restore DB from a backup id (ONE TIME PER BACKUP FILE)
if (/^\/restore(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) return sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });

  const id = text.replace(/^\/restore(?:@\w+)?/i, "").trim();
  if (!id) {
    return sendTracked(
      chatId,
      "♻️ *Restore Backup*\n\nUse:\n`/restore bk-YYYYMMDD-HHMMSS`\n\n⚠️ One-time restore per backup file.",
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  }
