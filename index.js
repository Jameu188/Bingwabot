...
bot.on("message", async (msg) => {
  ...
  if (text === "🎁 Redeem Points") {
    const u = getUser(chatId);
    const pts = getPoints(chatId);
    const totalPurchases = Object.values(u.purchasesByDay || {}).reduce((a, b) => a + Number(b || 0), 0);
    const refs = Number(u.referralSuccessCount || 0);

    // 🔒 Redeem lock: must have 20 successful referrals + 10 successful purchases
    if (!isAdmin(chatId) && (refs < 20 || totalPurchases < 10)) {
      return sendTracked(
        chatId,
        `🔒 Redeem locked\n\nRequirements:\n• Referrals: ${refs}/20\n• Purchases: ${totalPurchases}/10\n\nKeep buying & inviting to unlock redeem.`,
        { ...mainMenuKeyboard() }
      );
    }
  ...
  if (text === "1️⃣ 20 free SMS (5 pts)" || text === "2️⃣ 250MB free (20 pts)" || text === "3️⃣ 20mins midnight free (25 pts)") {
    ...
    // ✅ Redeem is automatically applied to the currently saved number (no confirm / no number prompt)
...
To set your number, buy any offer once (or use 📞 Change Number during purchase). Then try redeem again.",
        { ...mainMenuKeyboard() }
      );
    }
    ...
    // 🔒 Redeem lock: must have 20 successful referrals + 10 successful purchases (admin bypass)
    if (!isAdmin(chatId) && (refs < 20 || totalPurchases < 10)) {
      return sendTracked(
        chatId,
        `🔒 Redeem locked
...
ChatID: \`${mdEscape(chatId)}\`
Item: *${mdEscape(item.label)}*
Cost: *${mdEscape(item.cost)} pts*
Load To: *${mdEscape(formatTo07(savedPhone))}*
Time: ${mdEscape(nowISO())}`
    );
