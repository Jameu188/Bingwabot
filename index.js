...
function weeklyDrawLastWinnersText(chatId) {
  ...
  function buildWeeklyDrawMainCaption(chatId) {
    ...
    const heading = String(c.heading || "WEEKLY DRAW").toUpperCase();
    const countdown = status === "ENDED"
      ? tr(chatId, "Ended", "Imeisha")
    ...
    if (status === "ENDED") {
      try {
        const winnerInfo = getWeeklyDrawWinnerRecord(chatId);
        const rec = winnerInfo && winnerInfo.rec ? winnerInfo.rec : null;
        if (rec) {
          const prize = Number(rec.prize || 0);
          if (rec.paid === true) {
            optedLine = tr(
              chatId,
              `🎉 Congratulations! You won Ksh ${prize}. Your payout has already been marked as PAID.`,
              `🎉 Hongera! Umeshinda Ksh ${prize}. Malipo yako tayari yamewekewa alama ya YAMELIPWA.`
            );
          } else if (rec.payoutSubmitted === true) {
            const tries = Math.max(0, Number(rec.payoutSubmitCount || 0));
            optedLine = tr(
              chatId,
              `🎉 Congratulations! You won Ksh ${prize}. Your M-PESA payout details were submitted successfully${tries ? ` (${tries}/3)` : ""}. Please wait for admin review and payment.`,
              `🎉 Hongera! Umeshinda Ksh ${prize}. Maelezo yako ya malipo ya M-PESA yametumwa kikamilifu${tries ? ` (${tries}/3)` : ""}. Tafadhali subiri ukaguzi wa admin na malipo.`
            );
          } else {
            optedLine = tr(
              chatId,
              `🎉 Congratulations! You won Ksh ${prize}. Tap 📲 Submit M-PESA Details below so admin can process your payout.`,
              `🎉 Hongera! Umeshinda Ksh ${prize}. Bonyeza 📲 Tuma Maelezo ya M-PESA hapa chini ili admin ashughulikie malipo yako.`
            );
          }
        } else if (isWeeklyDrawOptedIn(chatId)) {
          optedLine = tr(chatId, '✅ This round has ended. Watch for the next Weekly Draw.', '✅ Raundi hii imeisha. Subiri Weekly Draw inayofuata.');
        }
      } catch (_) {}
    } else if (isWeeklyDrawOptedIn(chatId)) {
      const myPos = ranked.findIndex((r) => String(r.cid) === String(chatId)) + 1;
      if (myPos >= 1 && myPos <= 3) {
...
function adminCampaignApplyTicketDelta(targetIds, delta) {
  ...
  for (const rawId of (targetIds || [])) {
    ...
    if (next > 0) c.entries[id] = next;
    else delete c.entries[id];
    count += 1;
  }
  try { saveDB(); } catch (_) {}
  return count;
