...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (action === "redeem_decline") {
      ...
      await bot.sendMessage(chatId, `❌ Redeem declined by admin.\nReserved points refunded: ${cost} pts\nCurrent balance: ${getPoints(chatId).toFixed(2)} pts`).catch(() => {});
      await bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: q.message.chat.id, message_id: q.message.message_id }).catch(() => {});
      return;
    }
  } catch (e) {
...
function applyReferralIfAny(chatId, payload) {
  ...
  // Set inviter once (first time only)
  if (!u.inviterId) {
    u.inviterId = String(inviter);
    saveDB();

    // ✅ Track unique referrals for eligibility checks (20 referrals requirement)
  ...
  if (act) {
    ...
    if (act.kind === "redeem") {
      ...
      if (act.step === "number") {
        ...
        // ✅ Admin bypass: admins are not blocked by balance/eligibility, but points are still deducted
        if (!isAdmin(chatId)) {
...
Need: ${cost} pts`, {
          ...
          // Admin: no balance check, but deduct anyway (deductPoints clamps at 0)
          deductPoints(chatId, cost);
        }
        clearPendingAction(chatId);
