...
if (!BOT_TOKEN) {
  throw new Error(
    "BOT_TOKEN missing. Add it in Render: Service -> Environment -> BOT_TOKEN"
  );
}

const ADMIN_ID = process.env.ADMIN_ID || ""; // set in Render env
...
// ===================== Utility functions used by core modules =====================
function todayKey() {
...
async function payheroStkPush(payload) {
  // Placeholder (your actual STK logic should be here)
  return { ok: false, message: "STK push not implemented yet." };
}
...
function maybeRewardInviterOnSuccessPurchase(buyerChatId, amountKsh, when) {
  if (!__coreCtx) return;
  return ReferralSystem.maybeRewardInviterOnSuccessPurchase(
    __coreCtx,
    buyerChatId,
    amountKsh,
    when
...
console.log("🚀 BingwaBot started successfully");function kenyaDateTime() {
