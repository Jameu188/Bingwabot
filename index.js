...
bot.onText(/\/set/, async (msg) => {
  ...
  const categories = [
    { key: "BINGWA", label: "📶 Bingwa Deals" },
    { key: "SMS", label: "💬 SMS Offers" },
    { key: "MINUTES", label: "📞 Minutes" },
    { key: "UNLIMITED", label: "♾ Unlimited Deals" },
    { key: "BUSINESS", label: "🏢 Business Plans" }
  ];

  const buttons = categories.map(c => ([
    { text: c.label, callback_data: `setcat:${c.key}` }
  ]));

  await bot.sendMessage(chatId, "⚙️ Select category to update payment ID:", {
    reply_markup: { inline_keyboard: buttons }
  });
});
...
bot.on("callback_query", async (q) => {
  ...
  if (!data.startsWith("setcat:")) return;
  ...
  await bot.sendMessage(chatId, `✏️ Send new payment ID for ${category}:`);
  await bot.answerCallbackQuery(q.id);
});
