...
setInterval(async () => {
  try {
    const now = new Date();
    const hour = now.getUTCHours() + 3; // Kenya UTC+3 approx
    if (hour !== 21) return;

    for (const cid of Object.keys(db.users || {})) {
      const chatId = Number(cid);
      if (!isDailyReportEnabled(chatId)) continue;
...
bot.on("message", (msg) => {
  ...
  if (isMaintenanceMode() && !(typeof isAdmin === "function" && isAdmin(msg.chat.id))) {
    await bot.sendMessage(msg.chat.id, "⚠️ Service temporarily unavailable. Please try again later.");
    return;
  }
