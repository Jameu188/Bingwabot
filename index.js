...
// Generate short 3-digit unique approval code
...
MPESA: ${phone}
Total purchases: ${db.users?.[chatId]?.totalPurchases || 0} ksh
Total referrals: ${db.users?.[chatId]?.referrals || 0}
Remaining balance: ${getPoints(chatId).toFixed(2)} pts
...
function mainMenuKeyboard() {
  return {
    reply_markup: {
      keyboard: [["🛒 Buy Offers", "🎁 Redeem Points"], ["💸 Withdraw Points"], ["🔗 My Referral"], ["🔄 Resend STK"], ["ℹ️ Help", "❌ Cancel"]],
...
// /withdraws  -> list all pending withdrawals with numbers
if (/^\/withdraws$/i.test(text)) {
  db = repairDB(db);

  const pending = Object.entries(db.pendingRedeems || {})
    .filter(([, r]) => r && r.type === "withdraw" && r.status === "pending");

  if (!pending.length) {
    return sendTracked(chatId, "✅ No pending withdrawal requests.");
  }

  let msgText = "💸 Pending Withdrawal Requests:\n\n";

  pending.forEach(([cid, r], index) => {
    msgText += `${index + 1}. User: ${cid}\n` +
               `   Amount: ${r.amount} pts\n` +
               `   KES: ${pointsToKes(r.amount)}\n` +
               `   MPESA: ${r.phone}\n\n`;
  });
...
if (approveMatch) {
  const num = Number(approveMatch[1]);

  db = repairDB(db);
  const pending = Object.entries(db.pendingRedeems || {})
    .filter(([, r]) => r && r.type === "withdraw" && r.status === "pending");

  if (!pending.length || num < 1 || num > pending.length) {
    return sendTracked(chatId, "❌ Invalid number.");
  }

  const [targetId, req] = pending[num - 1];
  ...
  await bot.sendMessage(
    Number(targetId),
    `✅ Withdrawal Approved\n\nAmount: ${amount} pts\nKES Sent: ${pointsToKes(amount)}\nMPESA: ${req.phone}\n\nYour withdrawal is being processed.`,
    { ...mainMenuKeyboard() }
  ).catch(() => {});

  return sendTracked(chatId, `✅ Withdrawal approved for user ${targetId}`);
}

// /decline 1
const declineMatch = text.match(/^\/decline\s+(\d+)$/i);
if (declineMatch) {
  const num = Number(declineMatch[1]);
  ...
  await bot.sendMessage(
    Number(targetId),
    `❌ Withdrawal Declined\n\nAmount refunded: ${amount} pts\nCurrent balance: ${getPoints(targetId).toFixed(2)} pts`,
    { ...mainMenuKeyboard() }
  ).catch(() => {});

  return sendTracked(chatId, `❌ Withdrawal declined for user ${targetId}`);
}

// ===================== END ADMIN WITHDRAW MANAGEMENT =====================
...
Total spent: Ksh ${spent}
Remaining: Ksh ${remaining}`, { ...mainMenuKeyboard() });
  }
