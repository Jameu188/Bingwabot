...
bot.on("callback_query", async (q) => {
  try {
    ...
    // Admin only
    if (!isAdmin(chatId)) {
      await bot.answerCallbackQuery(q.id, { text: "Not allowed.", show_alert: true }).catch(() => {});
      return;
    }
    ...
    if (!reqId) {
      await bot.answerCallbackQuery(q.id, { text: "Invalid request.", show_alert: true }).catch(() => {});
      return;
    }
    ...
    if (!found) {
      await bot.answerCallbackQuery(q.id, { text: "Request already handled.", show_alert: true }).catch(() => {});
      return;
    }
    ...
    if (action === "redeem_accept") {
      ...
      await bot.answerCallbackQuery(q.id, { text: "Accepted ✅" }).catch(() => {});
...
// ===============================================================
// 🧩 TIERED WITHDRAW UNLOCK THRESHOLDS BY POINTS AMOUNT
// - 25 to 50 pts  => requires 300 Ksh total spend
// - 51 to 1000 pts => requires 500 Ksh total spend
// - 1001 to 2000 pts => requires 1000 Ksh total spend
// ===============================================================

function requiredSpendForWithdrawPts(pts) {
  const p = Number(pts || 0);
  if (p >= 25 && p <= 50) return 300;
  if (p >= 51 && p <= 1000) return 500;
  if (p >= 1001 && p <= 2000) return 1000;
  return 0; // default: no extra requirement (or unsupported amount)
}

function parseWithdrawPtsFromText(text) {
  // expects formats like "25 pts", "50 pts", "1000 pts"
  const m = String(text || "").trim().match(/^(\d+)\s*pts$/i);
  if (!m) return null;
  return Number(m[1]);
}

// This listener enforces tiered spend gating early when user selects withdrawal amount.
// (Non-breaking: it only blocks when thresholds aren't met; otherwise it allows existing flow.)
bot.on("message", async (msg) => {
  const chatId = msg.chat.id;
  const text = (msg.text || "").trim();

  const pts = parseWithdrawPtsFromText(text);
  if (!pts) return;

  // Only enforce for the defined withdrawal range
  if (pts < 25 || pts > 2000) return;

  const required = requiredSpendForWithdrawPts(pts);
  if (!required) return;
  ...
  if (totalSpent < required) {
    await bot.sendMessage(
      chatId,
      `🚫 Withdrawal Locked\n\n` +
      `To withdraw *${pts} pts*, you must spend at least *Ksh ${required}* total.\n\n` +
      `Your Total Spend: *Ksh ${totalSpent}*`,
      { parse_mode: "Markdown" }
    );
    return;
...
console.log("🧩 Tiered withdrawal unlock thresholds enabled.");




// ===============================================================
// ✅ FIX: ALLOW UNLIMITED PURCHASING ACROSS ACCOUNTS (NO PHONE LOCK)
// - Removes STK phone lock/admin approval gating
// - Keeps referral-bonus protections (daily-first-on-success) intact
// ===============================================================

(function disablePhoneLockForPurchasing() {
  try {
    // If the phone-lock wrapper exists, revert payheroStkPush to the original implementation.
    if (typeof __originalPayheroStkPush === "function") {
      payheroStkPush = __originalPayheroStkPush;
      console.log("✅ Phone-lock disabled: payheroStkPush restored to original.");
    }

    // Ensure canUsePhone always allows usage (prevents override attempts & admin prompts).
    if (typeof canUsePhone === "function") {
      canUsePhone = function () {
        return { ok: true };
      };
      console.log("✅ Phone-lock disabled: canUsePhone now always ok.");
    }

    // Optional: keep recording last usage (for analytics) but never block.
    if (typeof markPhoneUsage === "function") {
      const __markPhoneUsage = markPhoneUsage;
      markPhoneUsage = function (chatId, phone254) {
        try { __markPhoneUsage(chatId, phone254); } catch (e) {}
      };
    }

    // Prevent any pending override records from affecting behavior (kept for history only).
    // (No need to delete to preserve audit trail)
  } catch (e) {
    console.log("⚠️ disablePhoneLockForPurchasing error:", e?.message || e);
  }
})();
...
// ===============================================================
// ✅ REFERRAL BONUS: FLAT 2% ON EVERY SUCCESSFUL PURCHASE
// - Applies ONLY to the first account per phone per day (after first success sets owner)
// - Same account (daily owner) continues earning on every successful purchase that day
// - Other accounts using same phone same day earn NO referral bonus
// ===============================================================

const REFERRAL_FLAT_RATE = 0.02;

function __getKenyaDayKeySafe() {
  try { return todayKey(); } catch (e) { return new Date().toISOString().slice(0, 10); }
}

function __ensurePhoneDailyOwnerStore() {
  db.phoneDailyFirstAccount = db.phoneDailyFirstAccount || {};
  if (typeof db.phoneDailyFirstAccount !== "object") db.phoneDailyFirstAccount = {};
}

function __isDailyOwner(phone254, chatId) {
  __ensurePhoneDailyOwnerStore();
  const dayKey = __getKenyaDayKeySafe();
  const rec = db.phoneDailyFirstAccount?.[phone254]?.[dayKey];
  if (!rec) return { ok: true, dayKey, isFirst: true };
  return { ok: String(rec.firstChatId) === String(chatId), dayKey, isFirst: false };
}

function __setDailyOwnerIfMissing(phone254, chatId, dayKey) {
  __ensurePhoneDailyOwnerStore();
  db.phoneDailyFirstAccount[phone254] = db.phoneDailyFirstAccount[phone254] || {};
  if (!db.phoneDailyFirstAccount[phone254][dayKey]) {
    db.phoneDailyFirstAccount[phone254][dayKey] = { firstChatId: String(chatId), ts: Date.now() };
    saveDB();
  }
}

// Override referral reward logic (final authority)
maybeRewardInviterOnSuccessPurchase = function (buyerChatId, amountKsh, when) {
  const buyer = getUser(buyerChatId);
  if (!buyer) return;
  ...
  // Gate by daily owner of phone
  const gate = __isDailyOwner(phone, buyerChatId);
  if (!gate.ok) return;

  // On first successful purchase of the day for this phone, set owner
  if (gate.isFirst) {
    __setDailyOwnerIfMissing(phone, buyerChatId, gate.dayKey);
  }
  ...
  const rewardKsh = amt * REFERRAL_FLAT_RATE;

  // Convert to points using existing helper if present
  let rewardPts;
  try {
    rewardPts = (typeof kesToPoints === "function") ? kesToPoints(rewardKsh) : rewardKsh;
  } catch (e) {
    rewardPts = rewardKsh;
  }

  addPoints(inviterId, rewardPts);
  ...
  bot.sendMessage(
    ...
    `🎉 Referral Bonus Earned!\n\n` +
    `Purchase: Ksh ${amt}\n` +
    `Rate: ${(REFERRAL_FLAT_RATE * 100).toFixed(2)}%\n` +
    `Reward: Ksh ${rewardKsh.toFixed(2)}`
  ).catch(() => {});
};

console.log("✅ Referral: flat 2% on every success (daily-first phone owner rule).");




// ===============================================================
// 🔎 CLARITY PATCH: REFERRAL BONUS GOES TO INVITER (NOT BUYER)
// - Inviter = the user who shared the link/code (buyer.invitedBy)
// - Buyer never receives referral bonus from their own purchase
// ===============================================================

// Re-override with explicit naming + optional buyer notice disabled by default
const REFERRAL_NOTIFY_BUYER = false;
...
maybeRewardInviterOnSuccessPurchase = function (buyerChatId, amountKsh, when) {
  ...
  // Gate: only the DAILY OWNER account for this phone can trigger inviter bonus
  const gate = __isDailyOwner(buyerPhone, buyerChatId);
  if (!gate.ok) return;

  // First successful purchase today for this phone sets owner to this buyerChatId
  if (gate.isFirst) __setDailyOwnerIfMissing(buyerPhone, buyerChatId, gate.dayKey);

  // Inviter (the person who referred this buyer)
  const inviterChatId = buyer.invitedBy;
  ...
  // ✅ CREDIT INVITER
  addPoints(inviterChatId, rewardPts);

  addLedgerEvent({
    type: "REFERRAL_REWARD",
    chatId: String(inviterChatId), // credited user (inviter)
    buyer: String(buyerChatId),
    amountKsh: amt,
    rewardKsh: rewardKsh,
    rate: REFERRAL_FLAT_RATE
  ...
  bot.sendMessage(
    inviterChatId,
    `🎉 Referral Bonus Earned!\n\n` +
    `Buyer Purchase: Ksh ${amt}\n` +
    `Rate: ${(REFERRAL_FLAT_RATE * 100).toFixed(2)}%\n` +
    `You Earned: Ksh ${rewardKsh.toFixed(2)}`
  ).catch(() => {});

  // Optional: notify buyer (off by default)
  if (REFERRAL_NOTIFY_BUYER) {
    bot.sendMessage(
      buyerChatId,
      `✅ Purchase confirmed.\nReferral bonus credited to your inviter.`
    ).catch(() => {});
  }
};

console.log("🔎 Referral bonus explicitly credits INVITER (buyer never receives it).");




// ===============================================================
// 💸 BUYER ALSO EARNS CASHBACK (TIERED %) ON SUCCESSFUL PURCHASES
// - Buyer cashback tiers (Ksh):
//   1–10 => 1.2%
//   11–150 => 1.5%
//   151–500 => 2%
//   501–1000 => 2.5%
//   1001–10000 => 3%
// - Credited to BUYER (chatId) as points (integer-safe centi-points system)
// - Inviter bonus logic remains as configured elsewhere
// ===============================================================

function getBuyerCashbackRate(amountKsh) {
  const amt = Number(amountKsh || 0);
  if (amt >= 1 && amt <= 10) return 0.012;
  if (amt >= 11 && amt <= 150) return 0.015;
  if (amt >= 151 && amt <= 500) return 0.02;
...
function creditBuyerCashback(buyerChatId, amountKsh, meta) {
  const rate = getBuyerCashbackRate(amountKsh);
  if (rate <= 0) return;

  const amt = Number(amountKsh || 0);
  if (!(amt > 0)) return;

  const cashbackKsh = amt * rate;

  let cashbackPts;
  try {
    cashbackPts = (typeof kesToPoints === "function") ? kesToPoints(cashbackKsh) : cashbackKsh;
  } catch (e) {
    cashbackPts = cashbackKsh;
  }

  // ✅ CREDIT BUYER (cashback)
  addPoints(buyerChatId, cashbackPts);

  addLedgerEvent({
    type: "BUYER_CASHBACK",
    chatId: String(buyerChatId),
    amountKsh: amt,
    cashbackKsh: cashbackKsh,
    rate: rate,
    meta: meta || {}
  });
  ...
  bot.sendMessage(
    ...
    `Purchase: Ksh ${amt}\n` +
    `Rate: ${(rate * 100).toFixed(2)}%\n` +
    `Cashback: Ksh ${cashbackKsh.toFixed(2)}`
  ).catch(() => {});
}

// Hook into successful purchase recording (called once per successful payment)
const __recordSuccessfulRevenue_cashback = recordSuccessfulRevenue;
recordSuccessfulRevenue = function (pending) {
  try {
    if (pending && pending.chatId && pending.price) {
      creditBuyerCashback(
        pending.chatId,
        pending.price,
        { category: pending.category, pkg: pending.pkgLabel, phone: pending.phone254 }
      );
    }
  } catch (e) {
    console.log("⚠️ cashback hook error:", e?.message || e);
  }
  return __recordSuccessfulRevenue_cashback(pending);
};

console.log("💸 Buyer cashback tiers enabled (1.2%..3%).");




// ===============================================================
// 🛠 ADMIN COMMAND: /resetphone 07XXXXXXXX
// - Clears daily referral ownership for that phone
// - Does NOT delete lifetime data
// - Allows referral testing again same day
// ===============================================================

function normalizePhoneForReset(input) {
...
bot.on("message", async (msg) => {
  ...
  const text = (msg.text || "").trim();
  const match = text.match(/^\/resetphone\s+(\S+)/i);
  if (!match) return;

  const phoneInput = match[1];
  const phone254 = normalizePhoneForReset(phoneInput);

  if (!phone254) {
    return bot.sendMessage(chatId, "❌ Invalid phone format.");
  }

  if (db.phoneDailyFirstAccount && db.phoneDailyFirstAccount[phone254]) {
    delete db.phoneDailyFirstAccount[phone254];
    saveDB();

    return bot.sendMessage(
      chatId,
      `✅ Daily referral lock cleared for ${phoneInput}`
    );
  } else {
    return bot.sendMessage(
      chatId,
      `ℹ️ No daily referral lock found for ${phoneInput}`
    );
  }
});

console.log("🛠 /resetphone admin command enabled.");
...
// ===============================================================
// 🧰 ADMIN TOOLS
//   /phonestatus 07XXXXXXXX  → show current daily owner (Kenya day)
//   /resetuser <chatId>      → clear risk + referral-related locks for user
//   /refstats <chatId> [days]→ referral earnings for inviter (from ledger)
// ===============================================================

function normalizePhoneAny(input) {
  if (!input) return null;
  let p = String(input).replace(/\D/g, "");
  if (!p) return null;
  if (p.startsWith("254")) return p;
...
function getKenyaDayKeyForAdmin() {
  try { return todayKey(); } catch (e) { return new Date().toISOString().slice(0, 10); }
}
...
bot.on("message", async (msg) => {
  ...
  const text = (msg.text || "").trim();
  const m = text.match(/^\/phonestatus\s+(\S+)/i);
  if (!m) return;

  const phone254 = normalizePhoneAny(m[1]);
  if (!phone254) return bot.sendMessage(chatId, "❌ Invalid phone format.");

  const dayKey = getKenyaDayKeyForAdmin();
  const rec = db.phoneDailyFirstAccount?.[phone254]?.[dayKey];

  if (!rec) {
    return bot.sendMessage(chatId, `ℹ️ No daily owner set for ${m[1]} today (${dayKey}).`);
  }
  ...
  return bot.sendMessage(
    ...
    `📱 Phone Status (${dayKey})\n\n` +
    `Phone: ${m[1]}\n` +
    `Daily Owner ChatID: ${rec.firstChatId}\n` +
    `Set At: ${new Date(rec.ts || Date.now()).toLocaleString()}`
  );
});
...
bot.on("message", async (msg) => {
  ...
  const text = (msg.text || "").trim();
  const m = text.match(/^\/resetuser\s+(\d+)/i);
  if (!m) return;

  const targetId = String(m[1]);

  db = repairDB(db);

  // 1) Clear risk profile
  if (db.riskProfiles && db.riskProfiles[targetId]) {
    delete db.riskProfiles[targetId];
  }

  // 2) Clear user referral linkage (buyer invitedBy) so they won't trigger inviter bonuses until they rejoin via ref link
  const u = db.users?.[targetId];
  const phone254 = u?.phone;
  if (u) {
    if (u.invitedBy) delete u.invitedBy;
    // Optional: clear any inviter counters if you have them
    if (u.referredUsers) delete u.referredUsers;
  }

  // 3) Clear phone daily owner entries if they belong to this user (today only)
  const dayKey = getKenyaDayKeyForAdmin();
  if (phone254 && db.phoneDailyFirstAccount?.[phone254]?.[dayKey]) {
    const rec = db.phoneDailyFirstAccount[phone254][dayKey];
    if (String(rec.firstChatId) === targetId) {
      delete db.phoneDailyFirstAccount[phone254][dayKey];
      // cleanup empty
      if (Object.keys(db.phoneDailyFirstAccount[phone254]).length === 0) {
        delete db.phoneDailyFirstAccount[phone254];
      }
    }
  }

  // 4) Clear any phone referral lock store (if present)
  if (phone254 && db.phoneReferralLock?.[phone254]) {
    delete db.phoneReferralLock[phone254];
  }

  // 5) Clear pending phone overrides (if present)
  if (db.pendingPhoneOverrides && typeof db.pendingPhoneOverrides === "object") {
    for (const [k, v] of Object.entries(db.pendingPhoneOverrides)) {
      if (String(v?.requestingChatId) === targetId) delete db.pendingPhoneOverrides[k];
    }
  }
  ...
  return bot.sendMessage(
    ...
    `✅ User reset complete\n\n` +
    `ChatID: ${targetId}\n` +
    `Risk profile cleared\n` +
    `Referral link cleared (invitedBy removed)\n` +
    `${phone254 ? `Phone locks cleared for: ${formatTo07(phone254)} (where applicable)\n` : ""}` +
    `\nNote: Ledger/history not deleted.`
  );
});

// ---------- /refstats ----------
function __msDaysAgo(days) {
  return Date.now() - (Number(days || 0) * 24 * 60 * 60 * 1000);
}
...
bot.on("message", async (msg) => {
  ...
  const text = (msg.text || "").trim();
  // /refstats <chatId> [days]
  const m = text.match(/^\/refstats\s+(\d+)(?:\s+(\d+))?$/i);
  if (!m) return;

  const targetInviter = String(m[1]);
  const days = Math.max(0, Math.min(365, Number(m[2] || 30))); // default 30 days
  const since = __msDaysAgo(days);
  ...
  for (const e of ledger) {
    if (!e || e.type !== "REFERRAL_REWARD") continue;
    if (String(e.chatId) !== targetInviter) continue;
    if (Number(e.ts || 0) < since) continue;

    count += 1;
    totalKsh += Number(e.rewardKsh || 0);

    // points could be recorded as pointsC (centi) or points (float) depending on build history
    if (typeof e.pointsC === "number") totalPts += Number(e.pointsC) / 100;
    else if (typeof e.points === "number") totalPts += Number(e.points);
  }

  return bot.sendMessage(
    adminId,
    `👥 Referral Stats\n\n` +
    `Inviter ChatID: ${targetInviter}\n` +
    `Window: last ${days} day(s)\n\n` +
    `Rewards Count: ${count}\n` +
    `Total Reward: Ksh ${totalKsh.toFixed(2)}\n` +
    `Total Points Credited: ${totalPts.toFixed(2)} pts`
  );
});

console.log("🧰 Admin tools enabled: /phonestatus, /resetuser, /refstats");




// ===============================================================
// ✅ REFERRAL RESTORE (PREVIOUS SIMPLE BEHAVIOR)
// - Phone can be used on multiple accounts (no purchase blocking)
// - Referral bonus pays the INVITER whenever a referred buyer makes a SUCCESSFUL purchase
// - No daily phone ownership gating (restores "as it was")
// - Uses tiered referral rates (1.2%..3%) based on purchase amount
// ===============================================================

function __tieredReferralRate(amountKsh) {
  const amt = Number(amountKsh || 0);
  if (amt >= 1 && amt <= 10) return 0.012;
  if (amt >= 11 && amt <= 150) return 0.015;
  if (amt >= 151 && amt <= 500) return 0.02;
...
// Final override: pay inviter on every successful referred purchase (simple)
maybeRewardInviterOnSuccessPurchase = function (buyerChatId, amountKsh, when) {
  const buyer = getUser(buyerChatId);
  if (!buyer) return;

  const inviterChatId = buyer.invitedBy; // inviter gets the bonus
  if (!inviterChatId) return; // not referred

  const amt = Number(amountKsh || 0);
  if (!(amt > 0)) return;

  const rate = __tieredReferralRate(amt);
  if (rate <= 0) return;
...
console.log("✅ Referral restored: inviter paid on every successful referred purchase (no daily phone gating).");
...
// ===============================================================
// 💰 FINAL LOGIC:
// BUYER earns tiered cashback (1.2% → 3%)
// INVITER earns additional flat 2% if buyer was referred
// No phone gating. Unlimited usage.
// ===============================================================

// ---------- BUYER CASHBACK TIERS ----------
function __buyerTierRate(amountKsh) {
  const amt = Number(amountKsh || 0);
  if (amt >= 1 && amt <= 10) return 0.012;
  if (amt >= 11 && amt <= 150) return 0.015;
  if (amt >= 151 && amt <= 500) return 0.02;
...
const __INVITER_FLAT_RATE = 0.02;

// Hook successful revenue
const __originalRecordRevenue_FINAL = recordSuccessfulRevenue;

recordSuccessfulRevenue = function (pending) {
  if (!pending || !pending.chatId || !pending.price) {
    return __originalRecordRevenue_FINAL(pending);
  }

  const buyerChatId = pending.chatId;
  const amount = Number(pending.price || 0);

  // ================= BUYER CASHBACK =================
  const buyerRate = __buyerTierRate(amount);
  if (buyerRate > 0) {
    const cashbackKsh = amount * buyerRate;
    const cashbackPts = (typeof kesToPoints === "function")
      ? kesToPoints(cashbackKsh)
      : cashbackKsh;
  ...
  if (buyer && buyer.invitedBy) {
    const inviterId = buyer.invitedBy;
    const inviterRewardKsh = amount * __INVITER_FLAT_RATE;

    const inviterPts = (typeof kesToPoints === "function")
      ? kesToPoints(inviterRewardKsh)
      : inviterRewardKsh;

    addPoints(inviterId, inviterPts);
    ...
    addLedgerEvent({
      ...
      buyer: String(buyerChatId),
      amountKsh: amount,
      rewardKsh: inviterRewardKsh,
      rate: __INVITER_FLAT_RATE
    });

    bot.sendMessage(
      inviterId,
      `🎉 Referral Bonus!\n\nBuyer Purchase: Ksh ${amount}\nRate: 2%\nYou Earned: Ksh ${inviterRewardKsh.toFixed(2)}`
    ).catch(()=>{});
  }
...
console.log("💰 Buyer tier cashback + inviter 2% bonus enabled.");
...
// ===============================================================
// 🛑 DAILY CAPS SYSTEM
// Buyer cashback cap: 50 pts per day
// Inviter referral cap: 50 pts per day (max 1 rewarded referral per buyer per day effectively)
// ===============================================================

const DAILY_BUYER_CAP_PTS = 50;
const DAILY_REFERRAL_CAP_PTS = 50;

function __todayKeySafe() {
  try { return todayKey(); } catch (e) { return new Date().toISOString().slice(0,10); }
}

if (!db.dailyCaps) db.dailyCaps = {};

// Track daily totals
function __getDailyUserCaps(chatId) {
  const day = __todayKeySafe();
  db.dailyCaps[chatId] = db.dailyCaps[chatId] || {};
  db.dailyCaps[chatId][day] = db.dailyCaps[chatId][day] || {
    buyerPts: 0,
    referralPts: 0
  };
  return db.dailyCaps[chatId][day];
}

// Override revenue hook again to enforce caps
const __originalRevenueWithCaps = recordSuccessfulRevenue;

recordSuccessfulRevenue = function(pending) {

  if (!pending || !pending.chatId || !pending.price) {
    return __originalRevenueWithCaps(pending);
  }
  ...
  // ================= BUYER CASHBACK WITH CAP =================
  const buyerRate = __buyerTierRate(amount);
  if (buyerRate > 0) {
    const cashbackKsh = amount * buyerRate;
    const cashbackPtsRaw = (typeof kesToPoints === "function")
      ? kesToPoints(cashbackKsh)
      : cashbackKsh;

    const caps = __getDailyUserCaps(buyerChatId);

    let available = DAILY_BUYER_CAP_PTS - caps.buyerPts;
    if (available > 0) {
      const ptsToCredit = Math.min(available, cashbackPtsRaw);
      if (ptsToCredit > 0) {
        addPoints(buyerChatId, ptsToCredit);
        caps.buyerPts += ptsToCredit;

        addLedgerEvent({
          type: "BUYER_CASHBACK",
          chatId: String(buyerChatId),
          amountKsh: amount,
          cashbackPts: ptsToCredit,
          capped: ptsToCredit < cashbackPtsRaw
        });
      }
    }
  }

  // ================= INVITER REFERRAL WITH CAP =================
  if (buyer && buyer.invitedBy) {
    const inviterId = buyer.invitedBy;

    const inviterCaps = __getDailyUserCaps(inviterId);

    let availableRef = DAILY_REFERRAL_CAP_PTS - inviterCaps.referralPts;

    if (availableRef > 0) {
      const rewardKsh = amount * 0.02;
      const rewardPtsRaw = (typeof kesToPoints === "function")
        ? kesToPoints(rewardKsh)
        : rewardKsh;

      const ptsToCredit = Math.min(availableRef, rewardPtsRaw);

      if (ptsToCredit > 0) {
        addPoints(inviterId, ptsToCredit);
        inviterCaps.referralPts += ptsToCredit;

        addLedgerEvent({
          type: "REFERRAL_REWARD",
          chatId: String(inviterId),
          buyer: String(buyerChatId),
          rewardPts: ptsToCredit,
          capped: ptsToCredit < rewardPtsRaw
        });
      }
    }
  }
...
console.log("🛑 Daily caps enabled: Buyer 50pts/day, Referral 50pts/day");




// ===============================================================
// ✅ CLEAN FINAL PAYOUT ENGINE (A: caps in POINTS)
// Fixes "override stacking" by installing ONE final handler at EOF.
// - Buyer cashback tiers (1.2%..3%) credited as points
// - Buyer daily cashback cap: 50 pts/day
// - Inviter bonus: flat 2% credited as points if buyer.invitedBy exists
// - Inviter daily referral cap: 50 pts/day
// - Idempotent per successful transaction (no double credit)
// ===============================================================

(function installCleanPayoutEngine() {
  // --- Config ---
  const BUYER_CAP_PTS = 50;
  const INVITER_CAP_PTS = 50;
  const INVITER_RATE = 0.02;

  // --- Storage init ---
  db = repairDB(db);
  if (!db.dailyCaps || typeof db.dailyCaps !== "object") db.dailyCaps = {};
  if (!db.payoutFlags || typeof db.payoutFlags !== "object") db.payoutFlags = {}; // per externalRef idempotency
  saveDB();
  ...
  function getCaps(chatId) {
    const day = dayKeySafe();
    db.dailyCaps[String(chatId)] = db.dailyCaps[String(chatId)] || {};
    db.dailyCaps[String(chatId)][day] = db.dailyCaps[String(chatId)][day] || { buyerPts: 0, referralPts: 0 };
    return db.dailyCaps[String(chatId)][day];
  }

  function buyerRate(amountKsh) {
    const amt = Number(amountKsh || 0);
    if (amt >= 1 && amt <= 10) return 0.012;
    if (amt >= 11 && amt <= 150) return 0.015;
    if (amt >= 151 && amt <= 500) return 0.02;
  ...
  function toPtsFromKes(ksh) {
    try {
      return (typeof kesToPoints === "function") ? kesToPoints(Number(ksh || 0)) : Number(ksh || 0);
    } catch (e) {
      return Number(ksh || 0);
    }
  }

  function payoutKeyFromPending(pending) {
    // Prefer truly unique external references if present
    const p = pending || {};
    return String(
      p.externalRef ||
      p.merchantReference ||
      ...
      p.ref ||
      p.orderRef ||
      // fallback: should still be stable enough for a single callback run
      `${p.chatId || ""}|${p.phone254 || p.phone || ""}|${p.price || ""}|${p.pkgLabel || ""}|${p.category || ""}|${p.ts || p.time || ""}`
    );
  }

  // Capture current recordSuccessfulRevenue as base (whatever it currently does for revenue tracking)
  const __baseRecordSuccessfulRevenue = recordSuccessfulRevenue;

  // Install ONE final handler
  recordSuccessfulRevenue = function (pending) {
    // Always run base first (so revenue buckets / fulfillment remain intact)
    const baseRes = __baseRecordSuccessfulRevenue(pending);

    try {
      if (!pending || !pending.chatId || !pending.price) return baseRes;

      db = repairDB(db);

      const key = payoutKeyFromPending(pending);
      if (!key || key.length < 5) return baseRes;

      // Idempotency: only payout once per key
      if (!db.payoutFlags[key]) db.payoutFlags[key] = { buyer: false, inviter: false, ts: Date.now() };

      const buyerChatId = String(pending.chatId);
      const amount = Number(pending.price || 0);
      if (!(amount > 0)) return baseRes;

      // ---------------- BUYER CASHBACK (tiered) with cap ----------------
      if (!db.payoutFlags[key].buyer) {
        const rate = buyerRate(amount);
        if (rate > 0) {
          const cashbackKsh = amount * rate;
          const cashbackPtsRaw = toPtsFromKes(cashbackKsh);

          const caps = getCaps(buyerChatId);
          const available = Math.max(0, BUYER_CAP_PTS - Number(caps.buyerPts || 0));
          const ptsToCredit = Math.min(available, Number(cashbackPtsRaw || 0));

          if (ptsToCredit > 0) {
            addPoints(buyerChatId, ptsToCredit);
            caps.buyerPts = Number(caps.buyerPts || 0) + ptsToCredit;
            ...
            bot.sendMessage(
              buyerChatId,
              `💸 Cashback Earned!\n\nPurchase: Ksh ${amount}\nRate: ${(rate * 100).toFixed(2)}%\nCashback: ${ptsToCredit.toFixed(2)} pts` +
              (ptsToCredit < cashbackPtsRaw ? `\n(Reached daily cap)` : ``)
            ).catch(() => {});
          }
        }
        db.payoutFlags[key].buyer = true;
      }

      // ---------------- INVITER BONUS (flat 2%) with cap ----------------
      const buyer = getUser(buyerChatId);
      const inviterId = buyer?.invitedBy ? String(buyer.invitedBy) : null;

      if (inviterId && !db.payoutFlags[key].inviter) {
        const rewardKsh = amount * INVITER_RATE;
        const rewardPtsRaw = toPtsFromKes(rewardKsh);

        const capsInv = getCaps(inviterId);
        const availableInv = Math.max(0, INVITER_CAP_PTS - Number(capsInv.referralPts || 0));
        const ptsToCreditInv = Math.min(availableInv, Number(rewardPtsRaw || 0));

        if (ptsToCreditInv > 0) {
          addPoints(inviterId, ptsToCreditInv);
          capsInv.referralPts = Number(capsInv.referralPts || 0) + ptsToCreditInv;
          ...
          addLedgerEvent({
            ...
            amountKsh: amount,
            rewardKsh: rewardKsh,
            rewardPts: ptsToCreditInv,
            rate: INVITER_RATE,
            capped: ptsToCreditInv < rewardPtsRaw,
            ref: key
          });

          bot.sendMessage(
            inviterId,
            `🎉 Referral Bonus!\n\nBuyer Purchase: Ksh ${amount}\nRate: 2%\nBonus: ${ptsToCreditInv.toFixed(2)} pts` +
            (ptsToCreditInv < rewardPtsRaw ? `\n(Reached daily cap)` : ``)
          ).catch(() => {});
        }
        db.payoutFlags[key].inviter = true;
      }

      db.payoutFlags[key].ts = Date.now();
      saveDB();
    } catch (e) {
      console.log("⚠️ Clean payout engine error:", e?.message || e);
    }

    return baseRes;
  };

  console.log("✅ Clean payout engine installed (caps: 50 pts buyer/day, 50 pts inviter/day).");
})();
...
// ===============================================================
// 🧾 ADMIN COMMAND: /whoinvited <chatId>
// Shows the invitedBy (referrer) for a user (if any)
// ===============================================================
...
bot.on("message", async (msg) => {
  ...
  const text = (msg.text || "").trim();
  const m = text.match(/^\/whoinvited\s+(\d+)$/i);
  if (!m) return;

  const targetId = String(m[1]);

  db = repairDB(db);
  const u = db.users?.[targetId] || getUser(targetId);

  if (!u) {
    return bot.sendMessage(adminId, `❌ User not found: ${targetId}`);
  }

  const invitedBy = u.invitedBy ? String(u.invitedBy) : null;

  if (!invitedBy) {
    return bot.sendMessage(
      adminId,
      `ℹ️ User ${targetId} has no inviter (invitedBy not set).`
    );
  }
...
console.log("🧾 /whoinvited admin command enabled.");




// ===============================================================
// 🧯 FIX: DUPLICATE CASHBACK / REFERRAL PAYOUTS (multiple handlers)
// Reason: layered patches may credit points multiple times per same tx.
// Solution: set a per-transaction context (externalRef) and dedupe addPoints + notifications.
// ===============================================================

(function installTxDedupeGuard() {
  db = repairDB(db);
  if (!db.txDedupe || typeof db.txDedupe !== "object") db.txDedupe = {};
  // prune helper
  function pruneTxDedupe(max = 5000) {
    const keys = Object.keys(db.txDedupe);
    if (keys.length <= max) return;
    // remove oldest
    keys.sort((a,b) => (db.txDedupe[a]?.ts||0) - (db.txDedupe[b]?.ts||0));
    for (let i = 0; i < keys.length - max; i++) delete db.txDedupe[keys[i]];
  }

  // current tx context
  global.__CURRENT_TX_REF = null;

  function txKey() {
    if (!global.__CURRENT_TX_REF) return null;
    return String(global.__CURRENT_TX_REF);
  }

  // ---- Wrap recordSuccessfulRevenue to set tx context ----
  const __rsr_prev = recordSuccessfulRevenue;
  recordSuccessfulRevenue = function(pending) {
    try {
      const ref = pending?.externalRef || pending?.merchantReference || pending?.checkoutRequestId || pending?.ref || pending?.orderRef || null;
      global.__CURRENT_TX_REF = ref ? String(ref) : null;
      return __rsr_prev(pending);
    } finally {
      global.__CURRENT_TX_REF = null;
      try { pruneTxDedupe(); saveDB(); } catch(e) {}
    }
  };

  // ---- Wrap addPoints to dedupe identical credits within the same tx ----
  const __addPoints_prev = addPoints;
  addPoints = function(chatId, pts) {
    const ref = txKey();
    if (ref) {
      const ptsC = Math.round(Number(pts || 0) * 100); // normalize
      const k = `P|${ref}|${chatId}|${ptsC}`;
      if (db.txDedupe[k]) {
        return; // skip duplicate
      }
      db.txDedupe[k] = { ts: Date.now() };
      // also store a looser key to prevent same-amount spam even if minor float differs
      db.txDedupe[`P2|${ref}|${chatId}`] = { ts: Date.now() };
      saveDB();
    }
    return __addPoints_prev(chatId, pts);
  };

  // ---- Wrap bot.sendMessage to dedupe cashback/ref messages within same tx ----
  const __send_prev = bot.sendMessage.bind(bot);
  bot.sendMessage = function(chatId, text, opts) {
    const ref = txKey();
    if (ref && typeof text === "string") {
      // only dedupe our payout messages
      if (text.includes("Cashback Earned") || text.includes("Referral Bonus")) {
        const k = `M|${ref}|${chatId}|${text.slice(0,80)}`;
        if (db.txDedupe[k]) {
    ...
    return __send_prev(chatId, text, opts);
  };

  console.log("🧯 Tx dedupe guard installed (prevents duplicate cashback/referral per transaction).");
})();
