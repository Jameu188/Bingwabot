...
bot.on("message", async (msg) => {
  ...
  if (!u) return bot.sendMessage(adminId, `❌ User not found: ${targetId}`);

  const invitedBy = u.invitedBy ? String(u.invitedBy) : null;
  if (!invitedBy) return bot.sendMessage(adminId, `ℹ️ User ${targetId} has no inviter (invitedBy not set).`);

  return bot.sendMessage(adminId, `👤 Who Invited\n\nUser: ${targetId}\nInvited By: ${invitedBy}`);
});

// ===============================================================
// 🔁 EXTRA SAFETY: Deduplicate payout *messages* per transaction
// (Points already deduped by payoutFlags in clean engine.)
// ===============================================================
(function installPayoutMessageDedupe() {
  db = repairDB(db);
  if (!db.payoutMsgFlags || typeof db.payoutMsgFlags !== "object") db.payoutMsgFlags = {};
  saveDB();

  // Wrap bot.sendMessage lightly (no blocking of other messages)
  const __send = bot.sendMessage.bind(bot);
  bot.sendMessage = function(chatId, text, opts) {
    try {
      if (typeof text === "string" && (text.includes("Cashback Earned") || text.includes("Referral Bonus"))) {
        // try to read last tx ref that clean engine stores in ledger ref field, but if unavailable just hash content/time window
        const key = `${String(chatId)}|${text.slice(0,80)}|${todayKey ? todayKey() : new Date().toISOString().slice(0,10)}`;
        const now = Date.now();
        const rec = db.payoutMsgFlags[key];
        if (rec && (now - rec.ts) < 60 * 1000) {
          return Promise.resolve(null); // suppress duplicate within 60s
        }
        db.payoutMsgFlags[key] = { ts: now };
        saveDB();
      }
    } catch(e) {}
    return __send(chatId, text, opts);
  };

  console.log("🔁 Payout message dedupe installed (60s window).");
})();
