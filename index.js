...
// ===================== SUPPORT (2-WAY) =====================
function ensureSupportDirectStore() {
  db = repairDB(db);
  if (!db.supportDirectChats || typeof db.supportDirectChats !== "object") db.supportDirectChats = {};
  return db.supportDirectChats;
}

function getSupportDirectChat(chatId) {
  const store = ensureSupportDirectStore();
  const key = String(chatId || "");
...
function supportAdminInlineKeyboard(targetChatId) {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "💬 Reply", callback_data: `supadm:reply:${targetChatId}` }],
      ],
    },
  };
}

function supportAdminPanelText(targetChatId, panel = {}) {
  const u = getUser(targetChatId);
  const tgUser = u?.telegramProfile || {};
  const name = safeDisplayName(u, targetChatId) || `${tgUser.first_name || ""} ${tgUser.last_name || ""}`.trim() || "User";
  const uname = tgUser.username ? `@${tgUser.username}` : (u?.username ? `@${u.username}` : "no_username");
  const phone = formatTo07(getUserPhone(targetChatId) || "") || getUserPhone(targetChatId) || "-";
  const count = Number(panel.messageCount || 0);
  const opened = kenyaDateTimeFromTs(Number(panel.openedAt || panel.updatedAt || Date.now()));
  const updated = kenyaDateTimeFromTs(Number(panel.updatedAt || panel.lastUserMessageAt || Date.now()));
  const latest = String(panel.lastPreview || "(no messages yet)").trim();
  return (
    `💬 *Support Panel*
...
` +
    `ChatID: \`${mdEscape(String(targetChatId))}\`
` +
    `Phone: ${mdEscape(phone)}
` +
    `Messages: *${count}*
` +
    `Opened: ${mdEscape(opened)}
` +
    `Updated: ${mdEscape(updated)}
...
async function upsertSupportAdminPanel(targetChatId) {
  const panel = getSupportDirectChat(targetChatId);
  const text = supportAdminPanelText(targetChatId, panel);
  const opts = { parse_mode: "Markdown", ...supportAdminInlineKeyboard(targetChatId) };

  if (panel.panelMessageId) {
    try {
      await bot.editMessageText(text, {
        chat_id: ADMIN_ID,
        message_id: Number(panel.panelMessageId),
        ...opts,
      });
      return panel.panelMessageId;
    } catch (e) {
      const msg = String(e?.message || "");
      if (!msg.includes("message is not modified") && !msg.includes("MESSAGE_NOT_MODIFIED")) {
        panel.panelMessageId = 0;
      }
    }
  }
  ...
  if (sent?.message_id) {
    panel.panelMessageId = Number(sent.message_id);
...
function supportMessagePreview(msg = {}) {
...
async function forwardToAdminWithUserTag(userMsg) {
  ...
  const panel = getSupportDirectChat(fromChatId);
  panel.messageCount = Number(panel.messageCount || 0) + 1;
  panel.lastPreview = supportMessagePreview(userMsg);
  panel.lastUserMessageAt = Date.now();
  panel.updatedAt = Date.now();
  saveDB();

  let copied = false;
  try {
    await bot.forwardMessage(ADMIN_ID, fromChatId, userMsg.message_id);
    copied = true;
  } catch (e) {
    console.log("support forward to admin failed:", e?.message || e);
  }
  ...
  try {
    await upsertSupportAdminPanel(fromChatId);
  } catch (e) {
    console.log("support panel upsert failed:", e?.message || e);
  }
}

async function sendToUserFromAdmin(targetChatId, adminMsg) {
  try {
    await bot.forwardMessage(targetChatId, adminMsg.chat.id, adminMsg.message_id);
  } catch (_) {
...
bot.on("callback_query", async (q) => {
  try {
    const data = String(q?.data || "");
    const adminChatId = Number(q?.message?.chat?.id || 0);
    if (!data.startsWith("supadm:reply:")) return;
    if (!isAdmin(adminChatId)) {
      await bot.answerCallbackQuery(q.id, { text: "Admins only" }).catch(() => {});
      return;
    }
    const target = Number(data.split(":")[2] || 0);
    if (!target) {
      await bot.answerCallbackQuery(q.id, { text: "Invalid chat" }).catch(() => {});
      return;
    }
    adminReplyState.set(String(adminChatId), { targetChatId: target, until: Date.now() + 10 * 60 * 1000 });
    await bot.answerCallbackQuery(q.id, { text: "Reply mode ON" }).catch(() => {});
    await bot.sendMessage(adminChatId, `✅ Reply mode ON.
Next message you send will go to: ${target}
Send /cancel to stop.`).catch(() => {});
  } catch (e) {
    console.log("support reply callback failed:", e?.message || e);
  }
});
