...
function ensureWeeklyDrawObject() {
  ...
  function setWeeklyDrawOptIn(chatId, value) {
    ...
    if (page === "more") {
      ...
      if (!opted) rows.push([{ text: "✅ OPT IN", callback_data: "wdusr:optin" }]);
      else rows.push([{ text: "🚫 OPT OUT", callback_data: "wdusr:optout" }]);
...
if (data.startsWith("wdusr:")) {
  ...
  if (action === "optin") {
    ...
    await bot.sendMessage(
      chatId,
      `✅ You have OPTED IN to WEEKLY DRAW!\n\nYou will now receive ticket updates, reminders and winner announcements.\n\n🎉 You could be a winner — keep buying & inviting to increase your tickets!`,
      mainMenuKeyboard(chatId)
    ).catch(() => {});
    return;
  }
  ...
  if (action === "optout") {
    ...
    await bot.sendMessage(
      chatId,
      `😔 You have OPTED OUT of WEEKLY DRAW.\n\nYou will no longer receive campaign messages.\n\nYou can opt in again anytime from 🎉 WEEKLY DRAW.`,
      mainMenuKeyboard(chatId)
    ).catch(() => {});
    return;
  }
