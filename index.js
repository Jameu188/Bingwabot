  ...
  return sendTracked(
    ...
    confirmKeyboardV2(chatId, s, false)
      ...
      if (text === "🛒 View Orders in Cart" || text === "🛒 View Cart") {
        const liveSession = s || sessions.get(chatId) || { step: "menu" };
        if (!Array.isArray(liveSession.cart)) liveSession.cart = [];
        if (!getSessionCartItems(liveSession).length) {
          sessions.set(chatId, liveSession);
...
Tap ➕ Add to Cart from any offer to start building your cart.`, { ...mainMenuKeyboard(chatId) });
        ...
        liveSession.step = "cart_review";
        markCartActivity(liveSession, true);
        sessions.set(chatId, liveSession);
        return sendTracked(chatId, buildCartSummaryText(chatId, liveSession), { parse_mode: "Markdown", ...cartKeyboard() });
      }
