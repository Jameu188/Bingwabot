...
// ✅ Prevents double notifications on PayHero retries (processedPayments)
...
// ✅ User payment message: REMOVED "ref" (no more "Payment confirmed ref: ...")
// ✅ Referral bonus credit time = the exact callback-confirmed time (Kenya time) included in the Referral Reward admin log
//
// NOTE:
...
// ✅ Referral bonus credit time is the callback-confirmed Kenya time `when`
function maybeRewardInviterOnSuccessPurchase(buyerChatId, category, when) {
...
// ✅ PayHero callback endpoint (FIXED: don't treat cancelled as success)
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // ✅ STRICT SUCCESS RULE (so cancelled never confirms):
    // 1) If ResultCode exists -> MUST be 0
    // 2) Else if M-Pesa receipt exists -> SUCCESS (receipt only appears after success)
    // 3) Else if woocommerce status -> complete/completed/processing
    // 4) Else fallback to status strings that look success AND not failure words
    let isSuccess = false;
    ...
    if (resultCodeNum !== null && Number.isFinite(resultCodeNum)) {
      ...
      isSuccess = looksSuccess && !isFailureByText;
    }
    ...
    // If failed/cancelled, DO NOT confirm payment
    if (!isSuccess) {
      // Optional user notice (uncomment if you want)
      // await sendTracked(chatId, `⚠️ Payment not completed.\nRef: ${externalRef}\nStatus: ${statusText || statusRaw}`, { ...mainMenuKeyboard() });
      return;
    }

    // Prevent double-processing if PayHero retries
    ...
    if (pending) {
      ...
      // ✅ Referral credit time is EXACTLY `when` (callback confirmed)
      if (pending.category) maybeRewardInviterOnSuccessPurchase(chatId, pending.category, when);
    ...
    // ✅ Notify user (REMOVED "ref:" completely)
    await sendTracked(
      chatId,
      `✅ Payment confirmed at ${when}.${details}\n\nYour package will be processed now.`,
      { ...mainMenuKeyboard() }
    );
