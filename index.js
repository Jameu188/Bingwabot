...
function listActiveBusinessSubscribers({ limit = 50 } = {}) {
  db = repairDB(db);
  const now = Date.now();
  const out = [];
  for (const [cid, u] of Object.entries(db.users || {})) {
    ...
    const exp = Number(u.subscriptionExpiry || 0);
  ...
  out.sort((a, b) => Number(a.expiryTs) - Number(b.expiryTs)); // soonest first
  return out.slice(0, Math.max(1, Math.min(500, Number(limit || 50))));
}

function forceExpireBusiness(chatId) {
  const u = getUser(chatId);
  if (!u) return { ok: false, reason: "no_user" };
  if (u.accountType !== "business" && !u.subscriptionExpiry) return { ok: false, reason: "not_business" };

  u.accountType = "normal";
  u.subscriptionStart = 0;
  u.subscriptionExpiry = 0;
  u.businessReminderLastDay = "";
  u.businessExpiredNotifiedAt = 0;
  saveDB();
  return { ok: true };
}

function grantBusinessDays(chatId, days) {
  ...
  u.accountType = "business";
  u.subscriptionStart = now;
  u.subscriptionExpiry = now + d * 24 * 60 * 60 * 1000;
  // Do NOT reset businessIntroShown (should not repeat announcements)
  saveDB();
  return { ok: true, days: d, expiryTs: u.subscriptionExpiry };
}
...
const COMMANDS_LIST = [
  ...
  { cmd: "/report", desc: "Revenue & business report (admin only)", adminOnly: true },
  { cmd: "/bizlist", desc: "List active Business subscribers (admin only)", adminOnly: true },
  { cmd: "/grantbiz", desc: "Grant Business to a user: /grantbiz USER_ID DAYS (admin only)", adminOnly: true },
  { cmd: "/forceexpire", desc: "Force-expire Business: /forceexpire USER_ID (admin only)", adminOnly: true },
  { cmd: "/net", desc: "Net revenue summary: /net [days] (admin only)", adminOnly: true },
  { cmd: "/block", desc: "Block withdrawals 72h: /block USER_ID (admin only)", adminOnly: true },
  { cmd: "/unblock", desc: "Unblock withdrawals: /unblock USER_ID (admin only)", adminOnly: true },
  { cmd: "/reply", desc: "Reply to user support: /reply CHAT_ID (admin only)", adminOnly: true },
  { cmd: "/commands", desc: "Show full command list", adminOnly: false },
];
...
// - Normal purchases: 2% of amount paid
// - Business subscription purchases: 10% of amount paid
function maybeRewardInviterOnSuccessPurchase(buyerChatId, amountKsh, when, opts = {}) {
  ...
  if (!Number.isFinite(amt) || amt <= 0) return;
    ...
    const percent = Number(opts.percent ?? REFERRAL.BONUS_PERCENT);
  const safePercent = Number.isFinite(percent) && percent > 0 ? percent : REFERRAL.BONUS_PERCENT;

  const bonus = amt * safePercent;
    ...
    if (pending) {
      ...
      if (pending?.category !== "Business Account") {
        ...
        // ✅ Referral bonus ONLY on success, 2% of amount (callback-confirmed time)
                const isBiz = pending && pending.category === "Business Account";
        maybeRewardInviterOnSuccessPurchase(chatId, pending.price, when, { percent: isBiz ? 0.10 : REFERRAL.BONUS_PERCENT });
      }
...
// ===================== ADMIN: LIST ALL COMMANDS =====================
  if (isAdmin(chatId) && (text === "/cmds" || text === "/")) {
...
// ✅ /bizlist (admin): list active Business subscribers
const bizListMatch = text.match(/^\/bizlist(?:\s+(\d+))?$/i);
if (bizListMatch) {
  if (!isAdmin(chatId)) {
    await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
    return;
  ...
  const limit = Math.max(1, Math.min(200, Number(bizListMatch[1] || 50)));
  const list = listActiveBusinessSubscribers({ limit });
  const totalActive = countBusinessSubscribers({ activeOnly: true });

  if (!list.length) {
    await sendTracked(chatId, `💼 Active Business subscribers: *0*`, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
    return;
  }

  const lines = list.map((x, i) => {
    const exp = kenyaDateTimeFromTs(x.expiryTs);
    const ph = x.phone ? maskPhone(x.phone) : "N/A";
    return `${i + 1}. \`${mdEscape(x.chatId)}\` • ${mdEscape(ph)} • expires: \`${mdEscape(exp)}\` • ${mdEscape(String(x.daysLeft))} day(s)`;
  });

  const msg =
    `💼 *Active Business Subscribers*\n` +
    `Total active: *${mdEscape(String(totalActive))}*\n` +
    `Showing: *${mdEscape(String(list.length))}*\n\n` +
    lines.join("\n");
...
// ✅ /forceexpire USER_ID (admin)
const forceMatch = text.match(/^\/forceexpire\s+(\d+)$/i);
if (forceMatch) {
  if (!isAdmin(chatId)) {
    await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
    return;
  ...
  const targetId = forceMatch[1];
  const res = forceExpireBusiness(targetId);

  if (!res.ok) {
    await sendTracked(chatId, `⚠️ Cannot expire Business for ${mdEscape(targetId)} (${mdEscape(res.reason)}).`, {
      parse_mode: "Markdown",
      ...mainMenuKeyboard(chatId),
    });
    return;
  ...
  await sendTracked(chatId, `✅ Forced Business expiry for user \`${mdEscape(targetId)}\`.`, {
    parse_mode: "Markdown",
    ...mainMenuKeyboard(chatId),
  });
  ...
  await sendTracked(
    targetId,
    `❌ *Business Subscription Expired*\n\nYour Business account was expired by admin.\nTo renew, type /sell`,
    { parse_mode: "Markdown", ...mainMenuKeyboard(targetId) }
  ).catch(() => {});
  return;
}

// ✅ /grantbiz USER_ID DAYS (admin)
const grantMatch = text.match(/^\/grantbiz\s+(\d+)\s+(\d+)$/i);
if (grantMatch) {
  if (!isAdmin(chatId)) {
    await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
    return;
  ...
  const days = Number(grantMatch[2] || 0);
  const g = grantBusinessDays(targetId, days);
  const expStr = kenyaDateTimeFromTs(g.expiryTs);

  await sendTracked(
    chatId,
    `✅ Granted Business to \`${mdEscape(targetId)}\` for *${mdEscape(String(g.days))}* day(s).\nExpiry: \`${mdEscape(expStr)}\``,
    { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
  );
  ...
  await sendTracked(
    targetId,
    `✅ *Business Account Activated!*\n\n` +
      `⏳ Duration: *${mdEscape(String(g.days))} day(s)*\n` +
      `Expiry: \`${mdEscape(expStr)}\`\n\n` +
      `🚀 Benefits (during active subscription):\n` +
      `• Unlimited referrals\n` +
      `• Unlimited rewards\n` +
...
// ✅ /admin (admin): show admin commands & usage
if (/^\/admin$/i.test(text)) {
  ...
  const msgText =
    ...
    `📊 Reports\\n` +
    `• /report — Revenue + Business subscribers summary\\n` +
    `• /net 7 — Net revenue summary for last 7 days\\n\\n` +
    `💼 Business Management\\n` +
    `• /bizlist [limit] — List active Business subscribers\\n` +
    `• /grantbiz USER_ID DAYS — Grant Business for DAYS (e.g. /grantbiz 123456 30)\\n` +
    `• /forceexpire USER_ID — Expire a user's Business immediately\\n\\n` +
    `🔒 Risk / Withdraw Control\\n` +
    `• /block USER_ID — Block withdrawals for 72 hours\\n` +
    `• /unblock USER_ID — Remove withdrawal block\\n\\n` +
    `💬 Support\\n` +
    `• /reply CHAT_ID — Reply to a user's support message\\n\\n` +
    `📜 Help\\n` +
    `• /commands — Full bot command list`;

  await sendTracked(chatId, msgText, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
  return;
}

// ✅ /commands (user): show full command list
if (/^\/commands$/i.test(text)) {
  const msgText =
    ...
    `• /menu — Open main menu\\n` +
    `• /status — Check pending STK status\\n` +
    `• /help — Help & support\\n` +
    `• /sell — Subscribe to Business account\\n` +
    `• /changenumber — Change or save new phone number\\n\\n` +
    `🛒 *Features*\\n` +
    `• 🛒 Buy Offers — Browse packages\\n` +
    `• 🔁 Buy Last Offer — Repeat last purchase\\n` +
    `• 💸 Withdraw Points — Withdraw cashback\\n` +
    `• 🔗 My Referral — Get referral link\\n` +
    `• 🎁 Redeem Points — Redeem earned points\\n\\n` +
    `🛠 *Admin Commands*\\n` +
    `• /admin — Admin shortcuts\\n` +
    `• /report — Revenue & business report\\n` +
    `• /commands — Show this full command list`;
