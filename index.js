...
function getUser(chatId) {
  ...
  if (!db.users[cid]) {
    db.users[cid] = {
      ...
      theme: "professional", // professional | fun | handwriting | minimal
...
if (data.startsWith("help:")) {
  ...
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await sendHelpAnswer(chatId, key);
  return;
}
...
if (data === "theme:menu") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const msg = applyThemeText(chatId, themeMenuText(chatId));
  await sendTracked(chatId, msg, { parse_mode: "Markdown", ...themeMenuKeyboard(chatId) });
  return;
}
if (data.startsWith("theme:set:")) {
  const key = data.split(":")[2] || "professional";
  setTheme(chatId, key);
  await bot.answerCallbackQuery(q.id, { text: "✅ Theme updated!" }).catch(() => {});
  const msg = applyThemeText(chatId, themeMenuText(chatId));
  await sendTracked(chatId, msg, { parse_mode: "Markdown", ...themeMenuKeyboard(chatId) });
  return;
}
...
// ===================== THEME (USER STYLE) =====================
// Telegram cannot change real fonts; themes are simulated via formatting/emojis.
const THEMES = {
  professional: { label: "💼 Professional" },
  fun: { label: "🎉 Fun" },
  handwriting: { label: "✍️ Handwriting" },
  minimal: { label: "🧾 Minimal" },
};

function getTheme(chatId) {
  const u = getUser(chatId);
  const t = String(u.theme || "professional").toLowerCase();
  return THEMES[t] ? t : "professional";
}

function setTheme(chatId, themeKey) {
  const u = getUser(chatId);
  const key = String(themeKey || "").toLowerCase();
  u.theme = THEMES[key] ? key : "professional";
  saveDB();
  return u.theme;
}

function applyThemeText(chatId, text) {
  const theme = getTheme(chatId);
  const s = String(text || "");

  if (theme === "fun") {
    // light-touch: add a fun header but don't alter markdown structure
    return s.startsWith("🎉") ? s : ("🎉 " + s);
  }
  if (theme === "handwriting") {
    return s.startsWith("✍️") ? s : ("✍️ " + s);
  }
  // minimal & professional: keep original (safe)
  return s;
}

function themeMenuKeyboard(chatId) {
  const current = getTheme(chatId);
  const mk = (key) => {
    const on = current === key ? " ✅" : "";
    return { text: (THEMES[key].label + on), callback_data: `theme:set:${key}` };
  };

  return {
    reply_markup: {
      inline_keyboard: [
        [mk("professional"), mk("fun")],
        [mk("handwriting"), mk("minimal")],
        [
...
function themeMenuText(chatId) {
  const t = getTheme(chatId);
  const label = THEMES[t]?.label || "💼 Professional";
  return (
    "🎨 *Theme & Style*\n\n" +
    `Current: *${label}*\n\n` +
    "Choose how bot messages look for you.\n" +
    "Note: Telegram doesn't support real fonts, so themes are simulated.\n"
  );
}
...
case "cmds":
  return (
    ...
    "🎨 Theme & style:\n" +
    "• /theme — choose how bot messages look\n\n" +
...
async function sendHelpMenu(chatId) {
  return sendTracked(chatId, applyThemeText(chatId, helpMenuText()), { parse_mode: "Markdown", ...helpMenuKeyboard() });
}

async function sendHelpAnswer(chatId, key) {
  return sendTracked(chatId, applyThemeText(chatId, helpAnswerText(key)), { parse_mode: "Markdown", ...helpAnswerKeyboard() });
}
...
async function sendTracked(chatId, text, options = {}) {
  ...
  const finalText = isWelcomeMessage ? text : applyThemeText(chatId, text);
  const sent = await bot.sendMessage(chatId, finalText, options);
...
// ===================== THEME COMMAND =====================
if (text === "🎨 Theme" || /^\/theme\b/i.test(text) || /^theme$/i.test(text)) {
  const msg = applyThemeText(chatId, themeMenuText(chatId));
  await sendTracked(chatId, msg, { parse_mode: "Markdown", ...themeMenuKeyboard(chatId) });
  return;
}
