...
function showAnalytics(chatId){
  try{
    ...
    for(const id in users){
      ...
      totalPurchases += Number(u.totalPurchases || 0);
    }

    let risky = 0;
    for(const id in risk){
      if((risk[id].riskScore || 0) > 50){
        risky++;
      }
    }

    const txt =
`📊 Bingwa Bot Analytics

👥 Users: ${totalUsers}
🛒 Purchases: ${totalPurchases}
💰 Points Issued: ${totalPoints}
⏳ Pending Payments: ${pendingCount}
⚠️ High Risk Users: ${risky}
📢 Broadcasts: ${stats.totalBroadcasts || 0}
...
⚙️ System Status: Online`;
    ...
    bot.sendMessage(chatId, txt, {
      reply_markup:{
        inline_keyboard:[
          [{text:"🔄 Refresh",callback_data:"admin_refresh_stats"}],
          [{text:"🏠 Main Menu",callback_data:"help:home"}]
        ]
      }
    });
    ...
    bot.sendMessage(chatId,"Analytics unavailable.");
  }
}

// Admin command
bot.onText(/\/analytics/i, async (msg)=>{
  const chatId = msg.chat.id;
  if(typeof isAdmin === "function" && !isAdmin(chatId)) return;
  return showAnalytics(chatId);
});
