...
const STK_FAIL_WINDOW_MS = 30 * 60 * 1000; // 30 minutes window for counting repeated failed STK attempts
const STK_RISK_ACCEL_START = 5;   // after 5 failed STK in 30 min, start adding extra risk
const STK_RISK_ACCEL_PER_FAIL = 2; // extra risk added per fail after accel start
const STK_RISK_ESCALATION_BONUS = 15; // extra risk when escalation cooldown triggers
...
// ===================== RISK SCORE DECAY SYSTEM =====================
// Every 24 hours reduce risk score by 5 points (if no new violations)
...
function checkRiskReset(chatId) {
  const u = getUser(chatId);
  const now = Date.now();
  if (!u.lastViolationAt) return;

  if (now - u.lastViolationAt >= RISK_RESET_MS) {
    u.riskScore = 0;
    u.failedPayments = [];
    saveDB();
  }
...
function autoFreezeIfHighRisk(chatId, reason) {
  ...
  // notify admin at most once per hour per user to avoid spam
...
function mainMenuKeyboard() {
  return {
    reply_markup: {
      keyboard: [["🛒 Buy Offers", "💸 Withdraw Points"], ["💸 Withdraw Points"], ["🔗 My Referral"], ["🔄 Resend STK"], ["ℹ️ Help", "❌ Cancel"]],
...
function helpText() {
  return (
    ...
    `🎁 Redeem points: Tap "💸 Withdraw Points"\n\n` +
    `Help: ${HELP_PHONE}`
  );
}
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    if (!isSuccess) {
      ...
      const uEsc = getUser(chatId);
      const nowEsc = Date.now();
      uEsc.lastViolationAt = nowEsc;

      // ✅ Escalating STK cooldown:
      // If user keeps failing STK within a 30 minute window (>=10 fails) -> 3 hours cooldown.
      // If it happens again -> +3 hours each time.
      if (!Array.isArray(uEsc.stkFailAttempts)) uEsc.stkFailAttempts = [];
      uEsc.stkFailAttempts.push(nowEsc);
      uEsc.stkFailAttempts = uEsc.stkFailAttempts.filter((t) => nowEsc - Number(t || 0) < STK_FAIL_WINDOW_MS);

// ✅ Risk acceleration: frequent STK failures within 30 minutes raise risk faster
const failCount = uEsc.stkFailAttempts.length;
if (failCount >= STK_RISK_ACCEL_START) {
  const extraRisk = (failCount - STK_RISK_ACCEL_START + 1) * STK_RISK_ACCEL_PER_FAIL;
  uEsc.riskScore = Number(uEsc.riskScore || 0) + extraRisk;
}

      if (uEsc.stkFailAttempts.length >= STK_ESCALATION_THRESHOLD) {
        uEsc.stkEscalationLevel = Number(uEsc.stkEscalationLevel || 0) + 1;
        const extra = uEsc.stkEscalationLevel * STK_ESCALATION_BASE_MS; // 3h, 6h, 9h...
        uEsc.stkCooldownUntil = nowEsc + extra;
        uEsc.riskScore = Number(uEsc.riskScore || 0) + STK_RISK_ESCALATION_BONUS;
        autoFreezeIfHighRisk(chatId, "High STK failure rate escalation triggered");
        uEsc.stkFailAttempts = []; // reset window after escalation triggers
...
// ✅ AUTO-FREEZE enforcement (blocks protected actions while frozen)
const uuFreeze = getUser(chatId);
if (!isAdmin(chatId) && isFrozen(uuFreeze)) {
  const d = freezeRemainingDays(uuFreeze);
  // allow basic commands
  const allow = /^\/(start|help|cancel)\b/i.test(text);
  const allowButtons = (text === "ℹ️ Help" || text === "❌ Cancel" || text === "🔗 My Referral");
  if (!allow && !allowButtons) {
    fraudAlert("Frozen user attempted action", chatId, `Remaining: ${d} day(s)`);
    return sendTracked(chatId, `🚫 Your withdrawals are frozen for security review. Please try again in ${d} day(s) or contact support.`, { ...mainMenuKeyboard() });
  }
}
...
// ===================== COMMANDS (TEXT) =====================
    ...
    if (redeemCmd && !isAdmin(chatId)) {
      const uFreezeRedeem = getUser(chatId);
      if (isHighRisk(uFreezeRedeem) || isFrozen(uFreezeRedeem)) {
        autoFreezeIfHighRisk(chatId, "User attempted redeem while high risk");
        const d = freezeRemainingDays(uFreezeRedeem);
        return sendTracked(chatId,
          `🚫 Withdrawals & Redeems frozen for security review. Try again in ${d} day(s) or contact support.`,
          { ...mainMenuKeyboard() }
        );
      }
...
// ===================== ADMIN FREEZE COMMANDS =====================
// Usage:
//   /freeze <chatId> [days]   (default 7 days)
//   /unfreeze <chatId>
const fz = text.match(/^\/freeze\s+(\d+)(?:\s+(\d+))?$/i);
if (fz) {
  const targetId = fz[1];
  const days = Number(fz[2] || 7);
  const u = getUser(targetId);
  const ms = Math.max(1, days) * 24 * 60 * 60 * 1000;
  u.frozenUntil = Date.now() + ms;
  u.freezeNotifiedAt = Date.now();
  u.riskScore = Math.max(Number(u.riskScore || 0), HIGH_RISK_SCORE_BLOCK);
  saveDB();

  await bot.sendMessage(Number(targetId), `🚫 Your account has been frozen for security review for ${days} day(s). If this is a mistake, contact support.`, { ...mainMenuKeyboard() }).catch(() => {});
  await fraudAlert("Manual freeze", targetId, `Frozen for ${days} day(s)`);
  return sendTracked(chatId, `✅ Frozen ${targetId} for ${days} day(s).`);
}

const ufz = text.match(/^\/unfreeze\s+(\d+)$/i);
if (ufz) {
  const targetId = ufz[1];
  const u = getUser(targetId);
  u.frozenUntil = 0;
  saveDB();
  await bot.sendMessage(Number(targetId), "✅ Your account has been unfrozen. You can use the bot now.", { ...mainMenuKeyboard() }).catch(() => {});
  await fraudAlert("Manual unfreeze", targetId, "Unfrozen by admin");
  return sendTracked(chatId, `✅ Unfrozen ${targetId}.`);
}
...
if (text === "💸 Withdraw Points") {
  const uFreeze1 = getUser(chatId);
  if (isHighRisk(uFreeze1) || isFrozen(uFreeze1)) {
    autoFreezeIfHighRisk(chatId, "User attempted withdraw/redeem while high risk");
    const d = freezeRemainingDays(uFreeze1);
    return sendTracked(chatId,
      `🚫 Withdrawals & Redeems frozen for security review. Try again in ${d} day(s) or contact support.`,
      { ...mainMenuKeyboard() }
    );
...
if (["25 pts", "50 pts", "100 pts", "250 pts", "500 pts"].includes(text)) {
 const uFreeze2 = getUser(chatId);;
  if (isHighRisk(uFreeze2) || isFrozen(uFreeze2)) {
    autoFreezeIfHighRisk(chatId, "User attempted withdraw/redeem while high risk");
    const d = freezeRemainingDays(uFreeze2);
    return sendTracked(chatId,
      `🚫 Withdrawals & Redeems frozen for security review. Try again in ${d} day(s) or contact support.`,
      { ...mainMenuKeyboard() }
    );
...
// ===================== PURCHASE FLOW =====================
  ...
  try {
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
      const uEscCheck = getUser(chatId);
      if (uEscCheck.stkCooldownUntil && Date.now() < uEscCheck.stkCooldownUntil) {
        const hrs = Math.ceil((uEscCheck.stkCooldownUntil - Date.now()) / 3600000);
