...
const BOT_MESSAGE_AUTO_DELETE_MS = Math.max(0, Number(process.env.BOT_MESSAGE_AUTO_DELETE_MS || (10 * 60 * 1000)));
...
function getWithdrawSettings() {
  db = repairDB(db);
  return db.withdrawSettings || {
    enabled: true,
    minSpendKsh: MIN_WITHDRAW_SPEND,
    unlockPointsRequired: 0,
    amounts: [25, 50, 100, 250, 500, 1000, 2000],
    exemptUsers: {},
  };
}

function isWithdrawExempt(chatId) {
  try {
    if (isAdmin(chatId)) return true;
    const ws = getWithdrawSettings();
    return !!(ws.exemptUsers && ws.exemptUsers[String(chatId)]);
  } catch (_) {
    return false;
  }
}
...
function getWithdrawAmounts() {
  const ws = getWithdrawSettings();
  const arr = Array.isArray(ws.amounts) ? ws.amounts : [];
  const cleaned = Array.from(new Set(arr.map((x) => Number(x || 0)).filter((x) => Number.isFinite(x) && x > 0))).sort((a, b) => a - b);
  return cleaned.length ? cleaned : [25, 50, 100, 250, 500, 1000, 2000];
}

function getWithdrawMinSpendKsh() {
  const ws = getWithdrawSettings();
  const n = Number(ws.minSpendKsh || 0);
  return n > 0 ? n : 0;
}

function getWithdrawUnlockPointsRequired() {
  const ws = getWithdrawSettings();
  const n = Number(ws.unlockPointsRequired || 0);
  return n > 0 ? n : 0;
}

function isWithdrawEnabledFor(chatId) {
  if (isWithdrawExempt(chatId)) return true;
  const ws = getWithdrawSettings();
  return ws.enabled !== false;
}

function getWithdrawUnavailableMessage(chatId) {
  return tr(
    chatId,
    "❌ Withdraw not available at this time.",
    "❌ Withdraw haipatikani kwa sasa."
  );
}

function getWithdrawLockReason(chatId) {
  if (isWithdrawExempt(chatId)) return null;

  if (!isWithdrawEnabledFor(chatId)) {
    return { type: "disabled", message: getWithdrawUnavailableMessage(chatId) };
  }

  const u = getUser(chatId);
  const minSpend = getWithdrawMinSpendKsh();
  if (minSpend > 0 && Number(u.totalSpentKsh || 0) < minSpend) {
    const spent = Number(u.totalSpentKsh || 0);
    const remaining = Math.max(0, minSpend - spent);
    return {
      type: "min_spend",
      message: tr(
        chatId,
...
You must spend at least *Ksh ${minSpend}* before you can withdraw points.
...
Lazima uwe umetumia angalau *Ksh ${minSpend}* kabla ya kuweza kutoa pointi.
...
Endelea kununua ofa ili kufungua huduma ya kutoa.`
  ...
  const minPts = getWithdrawUnlockPointsRequired();
  const bal = Number(getPoints(chatId) || 0);
  if (minPts > 0 && bal < minPts) {
    return {
      type: "unlock_points",
      message: tr(
        chatId,
...
You need at least *${minPts} pts* in your balance to unlock withdrawals.

💎 Your balance: ${bal.toFixed(2)} pts`,
        `🚫 Kutoa Kumezuiwa

Unahitaji angalau *${minPts} pts* kwenye salio lako ili kufungua kutoa.

💎 Salio lako: ${bal.toFixed(2)} pts`
      ),
      parseMode: "Markdown",
    };
  }
...
function withdrawAdminText() {
  const ws = getWithdrawSettings();
  const enabled = ws.enabled !== false ? "ON" : "OFF";
  const amounts = getWithdrawAmounts().map((x) => `${x} pts`).join(", ");
  const minSpend = getWithdrawMinSpendKsh();
  const unlockPts = getWithdrawUnlockPointsRequired();
  const exempt = Object.keys(ws.exemptUsers || {});
  return (
    `💸 WITHDRAW ADMIN\n\n` +
    `Status: ${enabled}\n` +
    `Withdraw plans: ${amounts}\n` +
    `Min spend required: ${minSpend > 0 ? `Ksh ${minSpend}` : "OFF"}\n` +
    `Points required to unlock: ${unlockPts > 0 ? `${unlockPts} pts` : "OFF"}\n` +
    `Exempt user IDs: ${exempt.length ? exempt.join(", ") : "None"}`
  );
}

function withdrawAdminKeyboard() {
  const ws = getWithdrawSettings();
  return {
    reply_markup: {
      keyboard: [
        [ws.enabled !== false ? "🔴 Turn Off Withdrawal" : "🟢 Turn On Withdrawal"],
        ["✏️ Edit Withdraw Plans", "💰 Set Min Require"],
        ["🧹 Remove Min Require", "💎 Set Points Unlock"],
        ["✅ Allow User ID", "🚫 Remove User ID"],
        ["🏠 Main Menu"],
      ],
...
try {
  ...
  if (!db0.withdrawSettings || typeof db0.withdrawSettings !== "object") db0.withdrawSettings = {};
  if (typeof db0.withdrawSettings.enabled !== "boolean") db0.withdrawSettings.enabled = true;
  if (!Array.isArray(db0.withdrawSettings.amounts) || !db0.withdrawSettings.amounts.length) {
    db0.withdrawSettings.amounts = [25, 50, 100, 250, 500, 1000, 2000];
  }
  db0.withdrawSettings.amounts = Array.from(
    new Set(
      db0.withdrawSettings.amounts
        .map((x) => Number(x || 0))
        .filter((x) => Number.isFinite(x) && x > 0)
    )
  ).sort((a, b) => a - b);

  if (typeof db0.withdrawSettings.minSpendKsh !== "number") db0.withdrawSettings.minSpendKsh = MIN_WITHDRAW_SPEND;
  if (typeof db0.withdrawSettings.unlockPointsRequired !== "number") db0.withdrawSettings.unlockPointsRequired = 0;
  if (!db0.withdrawSettings.exemptUsers || typeof db0.withdrawSettings.exemptUsers !== "object") db0.withdrawSettings.exemptUsers = {};
...
function scheduleBotMessageAutoDelete(chatId, messageId) {
  try {
    ...
    if (!BOT_MESSAGE_AUTO_DELETE_MS || !Number.isFinite(cid) || !Number.isFinite(mid)) return;
    setTimeout(() => {
      bot.deleteMessage(cid, mid).catch(() => {});
    }, BOT_MESSAGE_AUTO_DELETE_MS);
  } catch (_) {}
}
...
bot.on("callback_query", async (q) => {
  try {
    ...
    try {
      if (isAutoHideButtonsEnabled(chatId) && !data.startsWith("help:") && data !== "bh:toggle") {
...
function helpAnswerText(key, chatId) {
  ...
  const spendUnlock = getWithdrawMinSpendKsh();
  const withdrawAmounts = getWithdrawAmounts();
  const minPoints = withdrawAmounts.length ? Math.min(...withdrawAmounts) : 25;
...
async function initiatePurchaseStk(chatId, s, pkg, phone254) {
...
await sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
  ...
  addPendingPayment(ref, { chatId, category: s.category, pkgLabel: pkg.label, phone254: phone, price: pkg.price });
...
function categoriesKeyboard(chatId = null) {
  ...
  // Keep the original keyboard layout the same, then append any new/renamed categories below it.
  try {
    const extraCats = Object.keys(PACKAGES || {})
      .filter((cat) => !defaultCats.has(String(cat)))
      .sort((a, b) => String(a).localeCompare(String(b)));

    for (let i = 0; i < extraCats.length; i += 2) {
      rows.push(extraCats.slice(i, i + 2).map((cat) => String(cat)));
    }
  } catch (_) {}
...
function sortPackagesByTier(list) {
  ...
  arr.sort((a, b) => {
    ...
    // Always arrange offers by exact price ascending:
    // 10, 14, 20, 29, 35, 99, 100, ...
...
async function proceedPurchase(chatId, s) {
  ...
  return initiatePurchaseStk(chatId, s, pkg, savedPhone);
}
...
function withdrawKeyboard() {
  const amounts = getWithdrawAmounts();
  const rows = [];
  for (let i = 0; i < amounts.length; i += 2) {
    rows.push(amounts.slice(i, i + 2).map((x) => `${x} pts`));
  }
  rows.push(["⬅ Back", PREV_BTN, "🏠 Main Menu"]);
...
function packagesOverviewText() {
  const lines = [];
  const baseOrder = ["Bingwa Deals", "Unlimited Deals", "SMS Offers", "Minutes", "Bonga Points", "Flex Deals"];
  const extraOrder = Object.keys(PACKAGES || {})
    .filter((cat) => !baseOrder.includes(String(cat)))
    .sort((a, b) => String(a).localeCompare(String(b)));
...
function categoryNameFromButton(text) {
  ...
  // Accept existing default button labels first.
  ...
  // Fallback for new/renamed categories from Plans Admin.
  const normalize = (s) =>
    String(s || "")
      .toLowerCase()
  ...
  for (const cat of Object.keys(PACKAGES || {})) {
    ...
    if (!normCat) continue;
    if (needle === normCat || needle.includes(normCat) || normCat.includes(needle)) {
      return cat;
    }
  }
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && act.type === "withdraw_admin_set_amounts") {
    if (!isAdmin(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
      return;
    ...
    if (/^\s*\/cancel\s*$/i.test(text) || text === "❌ Cancel") {
      clearPendingAction(chatId);
      await sendTracked(chatId, withdrawAdminText(), withdrawAdminKeyboard());
      return;
    }
    ...
    if (!amounts.length) {
      await sendTracked(chatId, "❌ Send valid amounts like: 25,50,100,250", withdrawAdminKeyboard());
      return;
    }
    ...
    saveDB();
    clearPendingAction(chatId);
    await sendTracked(chatId, "✅ Withdraw plans updated.\n\n" + withdrawAdminText(), withdrawAdminKeyboard());
    return;
  }

  if (act && act.type === "withdraw_admin_set_min_spend") {
    if (!isAdmin(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
      return;
    ...
    if (!Number.isFinite(amount) || amount < 0) {
      await sendTracked(chatId, "❌ Send a valid Ksh amount, e.g. 300", withdrawAdminKeyboard());
      return;
    }

    const ws = getWithdrawSettings();
    ws.minSpendKsh = Math.floor(amount);
    saveDB();
    clearPendingAction(chatId);
    await sendTracked(chatId, "✅ Minimum spend requirement updated.\n\n" + withdrawAdminText(), withdrawAdminKeyboard());
    return;
  }

  if (act && act.type === "withdraw_admin_set_unlock_points") {
    if (!isAdmin(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
      return;
    ...
    const amount = Number(String(text || "").trim());
    if (!Number.isFinite(amount) || amount < 0) {
      await sendTracked(chatId, "❌ Send valid points amount, e.g. 100", withdrawAdminKeyboard());
      return;
    }

    const ws = getWithdrawSettings();
    ws.unlockPointsRequired = Math.floor(amount);
    saveDB();
    clearPendingAction(chatId);
    await sendTracked(chatId, "✅ Unlock points requirement updated.\n\n" + withdrawAdminText(), withdrawAdminKeyboard());
    return;
  }

  if (act && act.type === "withdraw_admin_allow_user") {
    if (!isAdmin(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
      return;
    ...
    const targetId = String(text || "").trim();
    if (!/^\d{4,}$/.test(targetId)) {
      await sendTracked(chatId, "❌ Send a valid numeric user ID.", withdrawAdminKeyboard());
      return;
    }

    const ws = getWithdrawSettings();
    ws.exemptUsers = ws.exemptUsers || {};
    ws.exemptUsers[targetId] = true;
    saveDB();
    clearPendingAction(chatId);
    await sendTracked(chatId, `✅ User ID ${targetId} can now bypass withdraw limits.\n\n` + withdrawAdminText(), withdrawAdminKeyboard());
    return;
  }

  if (act && act.type === "withdraw_admin_remove_user") {
    if (!isAdmin(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
      return;
    ...
    const ws = getWithdrawSettings();
    ws.exemptUsers = ws.exemptUsers || {};
    delete ws.exemptUsers[targetId];
    saveDB();
    clearPendingAction(chatId);
    await sendTracked(chatId, `✅ User ID ${targetId} removed from withdraw bypass list.\n\n` + withdrawAdminText(), withdrawAdminKeyboard());
    return;
...
if (isAdmin(chatId) && text === "💸 Withdraw Admin") {
  try { clearPendingAction(chatId); } catch (_) {}
  return sendTracked(chatId, withdrawAdminText(), withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "✏️ Edit Withdraw Plans") {
  setPendingAction(chatId, { type: "withdraw_admin_set_amounts" });
  return sendTracked(chatId, "✏️ Send withdraw plans separated by comma or space.\nExample: 25, 50, 100, 250", withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "💰 Set Min Require") {
  setPendingAction(chatId, { type: "withdraw_admin_set_min_spend" });
  return sendTracked(chatId, "💰 Send minimum spend in Ksh to unlock withdraw.\nExample: 300", withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "🧹 Remove Min Require") {
  const ws = getWithdrawSettings();
  ws.minSpendKsh = 0;
  saveDB();
  try { clearPendingAction(chatId); } catch (_) {}
  return sendTracked(chatId, "✅ Minimum spend requirement removed.\n\n" + withdrawAdminText(), withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "💎 Set Points Unlock") {
  setPendingAction(chatId, { type: "withdraw_admin_set_unlock_points" });
  return sendTracked(chatId, "💎 Send points required to unlock withdraw. Send 0 to remove.\nExample: 100", withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "✅ Allow User ID") {
  setPendingAction(chatId, { type: "withdraw_admin_allow_user" });
  return sendTracked(chatId, "✅ Send the user ID to bypass withdraw limits.", withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "🚫 Remove User ID") {
  setPendingAction(chatId, { type: "withdraw_admin_remove_user" });
  return sendTracked(chatId, "🚫 Send the user ID to remove from withdraw bypass list.", withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "🔴 Turn Off Withdrawal") {
  const ws = getWithdrawSettings();
  ws.enabled = false;
  saveDB();
  try { clearPendingAction(chatId); } catch (_) {}
  return sendTracked(chatId, "✅ Withdrawal turned OFF. Users will see: Withdraw not available at this time.\n\n" + withdrawAdminText(), withdrawAdminKeyboard());
}

if (isAdmin(chatId) && text === "🟢 Turn On Withdrawal") {
  const ws = getWithdrawSettings();
  ws.enabled = true;
  saveDB();
  try { clearPendingAction(chatId); } catch (_) {}
  return sendTracked(chatId, "✅ Withdrawal turned ON.\n\n" + withdrawAdminText(), withdrawAdminKeyboard());
}
...
if (isBtn(text,"💸 Withdraw Points","💸 Toa Pointi")) {
  ...
  const lock = getWithdrawLockReason(chatId);
  if (lock) {
    return sendTracked(
      chatId,
      lock.message,
      { ...(lock.parseMode ? { parse_mode: lock.parseMode } : {}), ...mainMenuKeyboard(chatId) }
    );
  }
...
if (getWithdrawAmounts().map((x) => `${x} pts`).includes(text)) {
...
// ===============================================================
// 💳 DYNAMIC WITHDRAW SETTINGS ENFORCEMENT
// ===============================================================
...
bot.onText(/^(\d+) pts$/i, async (msg) => {
  const chatId = msg.chat.id;
  const amount = Number((msg.text || "").trim().split(" ")[0] || 0);
  if (!getWithdrawAmounts().includes(amount)) return;
  ...
  if (lock) {
    await bot.sendMessage(
      chatId,
      lock.message,
      { ...(lock.parseMode ? { parse_mode: lock.parseMode } : {}) }
    ).catch(() => {});
    return;
  }
});

console.log("💳 Dynamic withdrawal settings enabled.");
...
function promoButtonKeyboard() {
  const cfg = channelPromoCfg();
  const txt = String(cfg.buttonText || "").trim();
  const url = normalizePromoUrl(cfg.buttonUrl);
  if (!txt || !url) return undefined;

  return {
    reply_markup: {
      inline_keyboard: [[{ text: txt, url }]]
    }
  };
}
...
function channelPromoAdminText() {
  ...
  return (
    ...
    `Pin new: ${cfg.pinNew ? "ON" : "OFF"}\n` +
    `Unpin old: ${cfg.unpinOld ? "ON" : "OFF"}\n\n` +
    "Use the buttons below to edit the auto promo that is sent 3 minutes after the bot becomes admin in a channel or group."
  );
}
...
async function sendConfiguredChannelPromo(targetChatId) {
  ...
  if (cfg.pinNew && pinMessageId) {
    try {
      await bot.pinChatMessage(targetChatId, pinMessageId, { disable_notification: true });
    } catch (_) {}
  }
...
function gmailPercentButtons(stockCount) {
  ...
  for (const pct of labels) {
    ...
    seen.add(qty);
    rows.push([{ text: `📧 ${qty} Account${qty > 1 ? "s" : ""}`, callback_data: `gmail_qty:${qty}` }]);
  }

  if (!rows.length && stockCount > 0) {
    rows.push([{ text: `📧 ${stockCount} Account${stockCount > 1 ? "s" : ""}`, callback_data: `gmail_qty:${stockCount}` }]);
  }
...
function updatePlanAtIndex(cat, idx, fields = {}) {
  ...
  if (fields.label !== undefined) {
    nextLabel = String(fields.label || "").trim();
    if (!nextLabel) return { ok: false, err: "Invalid plan name." };
  }
  ...
  const duplicate = list.some((x, j) => j !== i && String(x?.label || "").trim().toLowerCase() === nextLabel.toLowerCase());
  if (duplicate) return { ok: false, err: "Plan name already exists in this category." };
...
// ===== Invite Text Feature =====
if (text === "📩 Get Invite Text") {
  try {
    const BOT_USERNAME = process.env.BOT_USERNAME || "";
    const referralLink = `https://t.me/${BOT_USERNAME}?start=${chatId}`;

    const normalMsgs = [
`🎁 Join Bingwa Data Bot today!

Get access to affordable offers and exciting rewards every week.

Tap to join:
${referralLink}`,

`🚀 Save money on bundles with Bingwa Data Bot!

Enjoy deals and weekly rewards.

Join here:
${referralLink}`,

`📶 Cheap offers + rewards every week!

Start using Bingwa Data Bot today.

Join:
${referralLink}`,

`🔥 Don't miss the best data offers on Telegram!

Join Bingwa Data Bot now.

Start here:
${referralLink}`,

`💰 Buy bundles, earn points and rewards.
...
Affordable offers + rewards every week.

${referralLink}`,
`🚀 Unlock amazing bundle deals today!

Join here:
${referralLink}`,
`📶 Cheap bundles and weekly prizes.

Start now:
${referralLink}`,
`🔥 Best Telegram deals available.

Join:
${referralLink}`,
`💰 Earn rewards while buying bundles.

${referralLink}`,
`🎉 Bingwa Data Bot is giving users rewards weekly!

${referralLink}`,
`⚡ Fast, cheap and rewarding bundles.

${referralLink}`,
`🎯 Smart users are saving money with Bingwa Data Bot.

${referralLink}`,
`🏆 Weekly rewards waiting for you.

${referralLink}`,
`📊 Affordable bundles for everyone.

${referralLink}`,
`💎 Premium level savings start here.

${referralLink}`,
`📡 Telegram’s easiest bundle deals.

${referralLink}`,
`🎊 Join thousands already saving.

${referralLink}`,
`🌟 Best bundle bot experience.

${referralLink}`,
`🔓 Unlock better deals now.

${referralLink}`,
`💡 Smart bundles, smart savings.

${referralLink}`,
`📱 Data deals made simple.
...
${referralLink}`
    ...
    const isPremium =
      typeof isBusinessActive === "function" && isBusinessActive(chatId);

    const arr = isPremium ? premiumMsgs : normalMsgs;
...
// ===== End Invite Text Feature =====


// ============================================================================
// INLINE CHANNEL TARGET MANAGEMENT SYSTEM
// Adds inline button management for:
// - Auto Post targets
// - Repost targets
// - Post to target now
// - Channel target settings (pin, unpin old, delete previous, repost interval)
// - Force join targets
//
// Channels/groups are registered when the bot becomes admin.
// They are NOT automatically added to force join.
// Admin must manually select them via inline buttons.
//
// This block is appended only and does not modify existing logic.
// ============================================================================

if (typeof db !== "undefined") {
  db.inlineChannelManager = db.inlineChannelManager || {
    known: {},
    autoPost: {},
    repost: {},
    postNow: {},
...
function __registerInlineChannel(chat){
  try{
    if(!chat || !chat.id) return;
    if(!db.inlineChannelManager) return;
    const id=String(chat.id);
    db.inlineChannelManager.known[id]={
      id:id,
...
// register channels when bot becomes admin
bot.on("my_chat_member",async(u)=>{
  try{
    const chat=u?.chat;
    const newStatus=String(u?.new_chat_member?.status||"");
    const oldStatus=String(u?.old_chat_member?.status||"");
    if(!chat) return;
    ...
    if(becameAdmin){
      __registerInlineChannel(chat);
    }
  }catch(_){}
});

// safe keyboard generator
function __inlineChannelKeyboard(mode){
  try{
    const rows=[];
    const store=db.inlineChannelManager||{};
    const list=Object.values(store.known||{});

    for(const c of list){
      const mark=(store[mode] && store[mode][c.id])?"✅":"⬜️";
      rows.push([{text:mark+" "+(c.title||c.id),callback_data:"ich:"+mode+":"+c.id}]);
    }

    rows.push([
      {text:"⬅ Back",callback_data:"ich:back"},
      {text:"🏠 Main Menu",callback_data:"ich:home"}
    ]);
...
bot.on("callback_query",async(q)=>{
  try{
    ...
    if(store[mode][id]) delete store[mode][id];
    else store[mode][id]=true;

    if(typeof saveDB==="function") saveDB();

    await bot.editMessageReplyMarkup(
      __inlineChannelKeyboard(mode).reply_markup,
      {chat_id:q.message.chat.id,message_id:q.message.message_id}
    ).catch(()=>{});
...
// ============================================================================
// SMART CUSTOMER SUPPORT (ENSURE PRESENT)
// If not already defined, create basic support menu
// ============================================================================

if(typeof openSmartSupportMenu==="undefined"){
  function openSmartSupportMenu(chatId){
    const kb={
      reply_markup:{
        keyboard:[
          ...
          ["7️⃣ Talk to admin","🏠 Main Menu"]
        ],
        resize_keyboard:true
      }
    };
    bot.sendMessage(chatId,"🆘 Smart Customer Support\nChoose an issue:",kb).catch(()=>{});
  }
}

// ============================================================================
// END INLINE CHANNEL TARGET MANAGEMENT SYSTEM
// ============================================================================


/*SIZE_PADDING_*/

/*PAD_*/
