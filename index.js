...
function parseSignedCallbackData(action, data) {
  ...
  return { ok: true, payload: raw, sig };
}
...
function consumeScopedActionToken(chatId, token, scope = null) {
  try {
    ...
    if (String(rec.chatId || "") !== String(chatId)) return { ok: false, reason: "owner" };
    if (scope && String(rec.scope || "") !== String(scope)) return { ok: false, reason: "scope" };
...
if (isStartCmd) {
  ...
  if (/^\/recoverycheck(?:@\w+)?$/i.test(text)) {
    ...
    const mismatchCount = runPaymentReconciliation();
