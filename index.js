...
bot.on("inline_query", async (q) => {
  try {
    ...
    if (!query) {
      ...
      const top = getTopTrendingOffers(5).filter((t) => canUseCategoryEvenIfClosed(q.from?.id, t.category));
      if (top.length) {
        for (const t of top) {
          results.push({
            type: "article",
...
function searchOffersInline(query, viewerChatId = null) {
  ...
  return all.filter((x) => {
    if (!canUseCategoryEvenIfClosed(viewerChatId, x.category)) return false;
...
Toggles now: ${closedOfferAllowedSummary(target)}`,
      ...
      await bot.sendMessage(chatId, `✅ User ${target} removed from closed-services access.`, { ...mainMenuKeyboard(chatId) }).catch(() => {});
    }
    return;
  }
});
