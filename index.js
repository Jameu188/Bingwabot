...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (action === "buy_again") {
      ...
      // Safety: never repeat Bingwa
      if (String(lp.category) === "Bingwa Deals") {
        await bot.answerCallbackQuery(q.id, { text: "Bingwa deals can't be repeated." }).catch(() => {});
        return;
      }
...
function linkAccountToPhone(chatId, phone254) {
  ...
  // Strict limit: max 3 accounts per phone number (4th requires admin approval)
  if (list.length >= 3) {
...
function checkAndMarkSpam(chatId, sig, phone254) {
  ...
  const r = linkAccountToPhone(chatId, phone254);

  if (!r.ok && r.reason === "limit_reached") {
    return { ok: false, reason: "phone_account_limit", total: r.total };
  }
  if (!r.ok && r.reason === "needs_approval") {
    return { ok: false, reason: "phone_needs_approval", approvalId: r.approvalId, total: r.total };
  }
  return { ok: true };
}
...
` +
    ...
    // ✅ Save last successful purchase for 🔁 Buy Same Again (exclude Bingwa)
    try {
    ...
    if (canBuyAgain) inlineRows.push([{ text: "🔁 Buy Same Again", callback_data: "ui:buy_again" }]);
...
📱 *Linked accounts for this number (max 3):*
...
Approve override?`,
    ...
    throw new Error("This phone number is currently at the 3-account limit. An approval request has been sent to admin.");
  }
