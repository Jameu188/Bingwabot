...
function getUser(chatId) {
  ...
  if (!db.users[cid]) {
    db.users[cid] = {
      ...
      stkAutoRetry: true,
      stkRetryCount: 2,        // 1..3
      stkRetryDelayMs: 10000,  // 10s/20s/30s

      // 🚀 Quick Buy behavior (Business)
      skipConfirmation: false,
      rememberLastCategory: true,
      repeatLastButton: true,
      lastCategory: "",
      lastPurchase: null,      // { category, pkgKey, pkgLabel, amount, phone }
      
      // 📱 Recent numbers (for Fast Purchase pick)
      recentPhones: [],        // array of phone254 strings
};
  }
  ...
  // Ensure newer business settings exist (backward compatible)
  if (db.users[cid].stkAutoRetry === undefined) db.users[cid].stkAutoRetry = true;
  if (db.users[cid].stkRetryCount === undefined) db.users[cid].stkRetryCount = 2;
  if (db.users[cid].stkRetryDelayMs === undefined) db.users[cid].stkRetryDelayMs = 10000;

  if (db.users[cid].skipConfirmation === undefined) db.users[cid].skipConfirmation = false;
  if (db.users[cid].rememberLastCategory === undefined) db.users[cid].rememberLastCategory = true;
  if (db.users[cid].repeatLastButton === undefined) db.users[cid].repeatLastButton = true;
  if (db.users[cid].lastCategory === undefined) db.users[cid].lastCategory = "";
  if (db.users[cid].lastPurchase === undefined) db.users[cid].lastPurchase = null;
  if (!Array.isArray(db.users[cid].recentPhones)) db.users[cid].recentPhones = [];
...
bot.on("callback_query", async (q) => {
  try {
    ...
    const chatId = q.message?.chat?.id;
    if (!chatId) {
      await bot.answerCallbackQuery(q.id, { text: "Ope
  // Business Settings menu
  if (data === "help:biz") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }

  // Business settings toggles
  if (data === "biz:stk:toggle") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    if (!isBusiness(chatId)) return sendTracked(chatId, "🔒 Business only. Type /sell to upgrade.", mainMenuKeyboard(chatId));
    const u = getUser(chatId);
    u.stkAutoRetry = !u.stkAutoRetry;
    saveDB();
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }

  if (data === "biz:stk:count") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    if (!isBusiness(chatId)) return sendTracked(chatId, "🔒 Business only. Type /sell to upgrade.", mainMenuKeyboard(chatId));
    const u = getUser(chatId);
    u.stkRetryCount = (u.stkRetryCount >= 3) ? 1 : (u.stkRetryCount + 1);
    saveDB();
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }

  if (data === "biz:stk:delay") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    if (!isBusiness(chatId)) return sendTracked(chatId, "🔒 Business only. Type /sell to upgrade.", mainMenuKeyboard(chatId));
    const u = getUser(chatId);
    u.stkRetryDelayMs = (u.stkRetryDelayMs === 10000) ? 20000 : (u.stkRetryDelayMs === 20000) ? 30000 : 10000;
    saveDB();
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }
  ...
  if (data === "biz:qb:skip") {
    ...
    if (!isBusiness(chatId)) return sendTracked(chatId, "🔒 Business only. Type /sell to upgrade.", mainMenuKeyboard(chatId));
    const u = getUser(chatId);
    u.skipConfirmation = !u.skipConfirmation;
    saveDB();
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }

  if (data === "biz:qb:remember") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    if (!isBusiness(chatId)) return sendTracked(chatId, "🔒 Business only. Type /sell to upgrade.", mainMenuKeyboard(chatId));
    const u = getUser(chatId);
    u.rememberLastCategory = !u.rememberLastCategory;
    saveDB();
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }

  if (data === "biz:qb:repeat") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    if (!isBusiness(chatId)) return sendTracked(chatId, "🔒 Business only. Type /sell to upgrade.", mainMenuKeyboard(chatId));
    const u = getUser(chatId);
    u.repeatLastButton = !u.repeatLastButton;
    saveDB();
    const res = businessSettingsInline(chatId);
    return sendTracked(chatId, res.text, res.opts);
  }

n the bot chat to continue." }).catch(() => {});
      return;
    }
...
// ===== 💼 Business Settings (STK + Quick Buy) =====
function markBox(on) { return on ? "✅" : "⬜️"; }
...
function businessSettingsInline(chatId) {
  const u = getUser(chatId);
  if (!isBusiness(chatId)) {
    return {
      text: "🔒 Business Settings are available for Business accounts only.\nType /sell to upgrade.",
      opts: { reply_markup: { inline_keyboard: [[{ text: "💼 Upgrade to Business", callback_data: "go_sell" }],[{ text: "🏠 Main Menu", callback_data: "help:home" }]] } }
    };
  }

  const rows = [
    [{ text: `${markBox(!!u.stkAutoRetry)} Auto-retry STK`, callback_data: "biz:stk:toggle" }],
    [{ text: `🔁 Retry count: ${u.stkRetryCount}`, callback_data: "biz:stk:count" }],
    [{ text: `⏱ Retry delay: ${delayLabel(u.stkRetryDelayMs)}`, callback_data: "biz:stk:delay" }],
    [{ text: `${markBox(!!u.skipConfirmation)} Skip confirmation`, callback_data: "biz:qb:skip" }],
    [{ text: `${markBox(!!u.rememberLastCategory)} Remember last category`, callback_data: "biz:qb:remember" }],
    [{ text: `${markBox(!!u.repeatLastButton)} Repeat last button`, callback_data: "biz:qb:repeat" }],
    [{ text: "⬅️ Back", callback_data: "help:menu" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
  ];

  const text =
    "💼 *Business Settings*\n\n" +
    "🔁 *Payment / STK tools*\n" +
    `• Auto-retry: ${u.stkAutoRetry ? "ON" : "OFF"}\n` +
    `• Retry count: ${u.stkRetryCount}\n` +
    `• Retry delay: ${delayLabel(u.stkRetryDelayMs)}\n\n` +
    "🚀 *Quick Buy behavior*\n" +
    `• Skip confirmation: ${u.skipConfirmation ? "ON" : "OFF"}\n` +
    `• Remember last category: ${u.rememberLastCategory ? "ON" : "OFF"}\n` +
    `• Repeat last button: ${u.repeatLastButton ? "ON" : "OFF"}`;

  return { text, opts: { parse_mode: "Markdown", reply_markup: { inline_keyboard: rows } } };
}
...
async function initiatePurchaseStk(chatId, s, pkg, phone254) {
  ...
  try {
    await payheroStkPushWithBizRetry(chatId, { amount: pkg.price, phone, externalRef: ref, channelId });

  // Save last purchase (for Business repeat)
  try {
    const u = getUser(chatId);
    u.lastPurchase = { category: pkg.category || session.category || "", pkgKey: session.pkgKey || pkg.label || "", pkgLabel: pkg.label, amount: pkg.price, phone };
    saveDB();
  } catch (_) {}
...
async function payheroStkPushWithBizRetry(chatId, payload) {
  const u = getUser(chatId);
  const enabled = isBusiness(chatId) && !!u.stkAutoRetry;
  const maxTries = enabled ? Math.max(1, Math.min(3, Number(u.stkRetryCount || 2))) : 1;
  const delayMs = enabled ? Number(u.stkRetryDelayMs || 10000) : 0;

  let lastErr = null;
  for (let attempt = 1; attempt <= maxTries; attempt++) {
    try {
      return await payheroStkPush(payload);
    } catch (e) {
      lastErr = e;
      if (attempt < maxTries) {
        try { await sendTracked(chatId, `⚠️ STK failed. Retrying (${attempt}/${maxTries - 1})…`); } catch (_) {}
        await new Promise(r => setTimeout(r, delayMs));
      }
    }
  }
  throw lastErr || new Error("STK failed");
}
...
if (text === "🛒 Buy Offers") {
    ...
    // 🚀 Business: Remember last category
    try {
      const u = getUser(chatId);
      if (isBusiness(chatId) && u.rememberLastCategory && u.lastCategory && PACKAGES[u.lastCategory]) {
        const s2 = { step: "package", category: u.lastCategory, pkgKey: null, createdAt: Date.now() };
        sessions.set(chatId, s2);
        await sendTracked(chatId, `⚡ Last category: *${u.lastCategory}*`, { parse_mode: "Markdown" });
        return sendTracked(chatId, `👇 ${u.lastCategory} packages:`, packagesKeyboard(u.lastCategory));
      }
    } catch (_) {}
...
// ===================== WITHDRAW FLOW =====================
  ...
  if (text === "🔁 Repeat Last Purchase" || text === "/repeat") {
    const u = getUser(chatId);
    if (!isBusiness(chatId) || !u.lastPurchase) {
      return sendTracked(chatId, "ℹ️ No previous purchase found yet. Buy an offer first.", mainMenuKeyboard(chatId));
    }
    const lp = u.lastPurchase;
    if (!PACKAGES[lp.category]) {
      u.lastPurchase = null; saveDB();
      return sendTracked(chatId, "ℹ️ Last purchase is no longer available. Please buy again.", mainMenuKeyboard(chatId));
    }
    const pkg = findPackageByLabel(lp.category, lp.pkgKey);
    if (!pkg) {
      return sendTracked(chatId, "ℹ️ Last package not found. Please buy again.", mainMenuKeyboard(chatId));
    }
    const phone = lp.phone || getUserPhone(chatId);
    if (!phone) {
      const s = { step: "phone", category: lp.category, pkgKey: lp.pkgKey, createdAt: Date.now() };
      sessions.set(chatId, s);
      return sendTracked(chatId, "📱 Send your phone number:", confirmKeyboard(false));
    }
    try { await bot.sendMessage(chatId, `📲 Sending STK push… (${maskPhone(phone)})`); } catch (_) {}
    await initiatePurchaseStk(chatId, { step: "confirm", category: lp.category, pkgKey: lp.pkgKey }, pkg, phone);
    return;
  }
...
if (s.step === "package") {
  ...
  if (isFastPurchaseEnabled(chatId)) {
        ...
        // 🚀 Business: Skip confirmation
        try {
          const u = getUser(chatId);
          const savedPhone = getUserPhone(chatId);
          if (isBusiness(chatId) && u.skipConfirmation && savedPhone) {
            sessions.delete(chatId);
            try { await bot.sendMessage(chatId, `📲 Sending STK push… (${maskPhone(savedPhone)})`); } catch (_) {}
            await initiatePurchaseStk(chatId, { ...s }, pkg, savedPhone);
            return;
          }
          if (isBusiness(chatId) && u.skipConfirmation && !savedPhone) {
            s.step = "phone";
