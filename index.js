...
function clearSessionCart(s) {
  ...
  s.cartPaymentEditCount = 0;
}

function addItemToSessionCart(s, item) {
  if (!s) return;
  const cart = getSessionCartItems(s);
  const sharedPaymentPhone = getSessionCartSharedPaymentNumber(s) || String(item?.phone254 || "");
  cart.push({
    category: String(item?.category || ""),
    pkgLabel: String(item?.pkgLabel || ""),
    price: Number(item?.price || 0),
    phone254: sharedPaymentPhone,
...
function getSessionCartSharedPaymentNumber(s) {
  const cart = getSessionCartItems(s);
  if (!cart.length) return "";
  return String(cart[0]?.phone254 || "");
}

function getSessionCartPaymentEditCount(s) {
  return Number(s?.cartPaymentEditCount || 0);
}

function canEditCartPaymentNumber(s) {
  return getSessionCartPaymentEditCount(s) < 1;
}

function markCartPaymentNumberEdited(s) {
  if (!s) return;
  s.cartPaymentEditCount = getSessionCartPaymentEditCount(s) + 1;
}
...
function buildCartSummaryText(chatId, s) {
  ...
  lines.push(`📱 *Cart Payment Number: ${mdEscape(maskPhone(getSessionCartSharedPaymentNumber(s) || "") || "Not set")}*`);
  lines.push(`✏️ *Payment number edits left this cart round: ${mdEscape(String(Math.max(0, 1 - getSessionCartPaymentEditCount(s))))}*`);
  lines.push("");
...
function cartEditActionKeyboard(s) {
  ...
  const paymentEditAllowed = canEditCartPaymentNumber(s);
  return {
    reply_markup: {
      keyboard: [
        [paymentEditAllowed ? `📞 Edit Payment Number` : `🔒 Payment Number Edit Used`],
        [`📥 Edit Receive Number`],
        [paymentEditAllowed ? `✏️ Edit Both Numbers` : `🔒 Both Numbers Edit Locked`],
        [PREV_BTN, "🏠 Main Menu"],
      ],
      resize_keyboard: true,
      one_time_keyboard: false,
...
async function proceedPurchase(chatId, s) {
  ...
  if (cart.length) {
    ...
    const sharedPaymentPhone = getSessionCartSharedPaymentNumber({ cart: workingCart }) || String(workingCart[0]?.phone254 || "");
    workingCart = workingCart.map((item) => ({ ...item, phone254: sharedPaymentPhone }));
    s.cart = workingCart.map((item) => ({ ...item }));
...
if (isUserCancelled) {
  ...
  try { if (pending && pending.phone254) phoneStkLock.delete(String(pending.phone254)); } catch (_) {}
  try {
    if (pending && Array.isArray(pending.cartOrders) && pending.cartOrders.length) {
      const ss = sessions.get(chatId) || { step: "cart_review" };
      ss.cart = pending.cartOrders.map((item) => ({ ...item }));
      ss.step = "cart_review";
      sessions.set(chatId, ss);
    }
  } catch (_) {}
...
📱 Shared cart payment number: *${mdEscape(maskPhone(getSessionCartSharedPaymentNumber(s) || paymentPhone) || "Not set")}*
...
${buildCartSummaryText(chatId, s)}`,
      ...
      if (phoneMaybe) {
        ...
        if (Array.isArray(s.cart) && s.cart.length) {
          s.cart = s.cart.map((item) => ({ ...item, phone254: phoneMaybe }));
          if (s.cartEditField === "payment" || s.cartEditField === "both") markCartPaymentNumberEdited(s);
          markCartActivity(s, true);
        }
...
if (s.step === "cart_review") {
  ...
  if (text === "✏️ Edit Numbers") {
    ...
    markCartActivity(s, true);
    sessions.set(chatId, s);
    return sendTracked(chatId, `✏️ Choose a cart item to edit numbers.

📱 One payment number is used for all cart orders. Payment number can only be edited once per cart round.`, cartEditKeyboard(s));
  }
...
📱 Shared cart payment: *${mdEscape(maskPhone(getSessionCartSharedPaymentNumber(s) || cart[idx].phone254 || "") || "Not set")}*
📥 Receive: *${mdEscape(maskPhone(cart[idx].receivePhone254 || cart[idx].phone254 || "") || "Not set")}*
✏️ Payment edit remaining this cart round: *${mdEscape(String(Math.max(0, 1 - getSessionCartPaymentEditCount(s))))}*`, { parse_mode: "Markdown", ...cartEditActionKeyboard(s) });
}
...
📱 One payment number is used for all cart orders. Payment number can only be edited once per cart round.`, cartEditKeyboard(s));
  ...
  if (text === "📞 Edit Payment Number") {
    if (!canEditCartPaymentNumber(s)) {
      return sendTracked(chatId, "🔒 Cart payment number can only be edited once per cart round.", cartEditActionKeyboard(s));
    }
...
• 2541XXXXXXXX`,
  ...
  if (s.cartEditIndex !== null && s.cartEditIndex !== undefined && Array.isArray(s.cart) && s.cart[s.cartEditIndex]) {
    ...
    markCartActivity(s, true);
    sessions.set(chatId, s);
    return sendTracked(chatId, `✅ ${updatedField === "receive" ? "Receive number updated for selected cart item" : "Cart numbers updated"}.
