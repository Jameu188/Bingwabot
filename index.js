...
// ✅ Global Back handler for reply-keyboard pages (e.g., /sell)
// NOTE: If user is in the Buy Offers session flow, the session Back handler will manage it.
try {
  const backRe = /^(?:⬅️?\s*)?back$/i;
  if (backRe.test(text)) {
    const s = sessions.get(chatId);
    const inBuyFlow = !!(s && (s.step === "package" || s.step === "phone" || s.step === "confirm"));
    if (!inBuyFlow) {
      try { setPendingAction(chatId, null); } catch (_) {}
