...
const DEFAULT_WITHDRAW_PLANS = [25, 50, 100, 250, 500, 1000, 2000];
function getWithdrawManagerCfg() {
  try {
    db = repairDB(db);
    if (!db.withdrawManager || typeof db.withdrawManager !== "object") db.withdrawManager = {};
    const cfg = db.withdrawManager;
    if (typeof cfg.enabled !== "boolean") cfg.enabled = true;
    if (!Array.isArray(cfg.plans) || !cfg.plans.length) cfg.plans = DEFAULT_WITHDRAW_PLANS.slice();
    cfg.plans = Array.from(new Set(cfg.plans.map((x) => Math.floor(Number(x || 0))).filter((x) => x > 0))).sort((a, b) => a - b);
    if (!Number.isFinite(Number(cfg.minWithdraw)) || Number(cfg.minWithdraw) <= 0) cfg.minWithdraw = cfg.plans[0] || 25;
    if (!Number.isFinite(Number(cfg.maxWithdraw)) || Number(cfg.maxWithdraw) <= 0) cfg.maxWithdraw = cfg.plans[cfg.plans.length - 1] || 2000;
    if (!cfg.autoDecline || typeof cfg.autoDecline !== "object") cfg.autoDecline = {};
    if (typeof cfg.autoDecline.phoneBan !== "boolean") cfg.autoDecline.phoneBan = true;
    if (typeof cfg.autoDecline.outsidePlans !== "boolean") cfg.autoDecline.outsidePlans = true;
    if (!Array.isArray(cfg.phoneBanList)) cfg.phoneBanList = [];
    cfg.phoneBanList = Array.from(new Set(cfg.phoneBanList.map((x) => normalizePhone(x)).filter(Boolean)));
    return cfg;
  } catch (_) {
    return {
      enabled: true,
      minWithdraw: 25,
      maxWithdraw: 2000,
      plans: DEFAULT_WITHDRAW_PLANS.slice(),
      autoDecline: { phoneBan: true, outsidePlans: true },
      phoneBanList: [],
    };
  }
}
function saveWithdrawManagerCfg(cfg) {
  try {
    db = repairDB(db);
    db.withdrawManager = cfg && typeof cfg === "object" ? cfg : getWithdrawManagerCfg();
    saveDB();
  } catch (_) {}
}
function isPhoneBannedForWithdraw(phone) {
  const n = normalizePhone(phone);
  if (!n) return false;
  const cfg = getWithdrawManagerCfg();
  return Array.isArray(cfg.phoneBanList) && cfg.phoneBanList.includes(n);
}
...
function repairDB(obj) {
  ...
  if (!db0.withdrawManager || typeof db0.withdrawManager !== "object") db0.withdrawManager = {};
...
const callbackClickCounter = new Map();
function isDuplicateCallbackClick(q) {
  try {
    const userId = String(q?.from?.id || "");
    const msgId = String(q?.message?.message_id || q?.inline_message_id || "0");
    const data = String(q?.data || "");
    if (!userId || !data) return false;
    const key = `${userId}:${msgId}:${data}`;
    const now = Date.now();
    const last = Number(callbackClickCounter.get(key) || 0);
    callbackClickCounter.set(key, now);
    if (callbackClickCounter.size > 5000) {
      for (const [k, ts] of callbackClickCounter.entries()) {
        if (now - Number(ts || 0) > 10 * 60 * 1000) callbackClickCounter.delete(k);
      }
    }
...
bot.on("callback_query", async (q) => {
  try {
    if (!q || !q.data) return;
    if (!isDuplicateCallbackClick(q)) return;
    q.data = `__dup__:${String(q.data || "")}`;
    await bot.answerCallbackQuery(q.id, { text: "Already counted.", show_alert: false }).catch(() => {});
  } catch (_) {}
});
...
bot.on("callback_query", async (q) => {
  try {
    if (!q || !q.data || !String(q.data).startsWith("kb:")) return;
    const token = String(q.data).slice(3);
    const row = resolveInlineReplyButton(token);
    if (!row) {
      await bot.answerCallbackQuery(q.id, { text: "This button expired. Please open the menu again.", show_alert: false }).catch(() => {});
      return;
    }

    const chatId = Number(q.message?.chat?.id || q.from?.id || row.chatId || 0);
    const text = String(row.text || "");

    await bot.answerCallbackQuery(q.id).catch(() => {});

    if (chatId && bot && typeof bot.emit === "function") {
      bot.emit("message", {
        message_id: 0,
        date: Math.floor(Date.now() / 1000),
        chat: { id: chatId, type: q.message?.chat?.type || "private" },
        from: q.from,
        text,
        via_inline_button: true,
      });
    }
  } catch (_) {}
});
...
bot.on("inline_query", async (q) => {
  try {
    ...
    // Categories
    const catHits = categoryMatchesInline(query).filter((cat) => !isOfferCategoryClosed(cat) || isAdmin(q.from?.id)).slice(0, 8);
    ...
    if (!query) {
      ...
      const top = getTopTrendingOffers(5).filter((t) => !isOfferCategoryClosed(t.category) || isAdmin(q.from?.id));
      if (top.length) {
        for (const t of top) {
          results.push({
            type: "article",
...
function searchOffersInline(query, viewerChatId = null) {
  ...
  return all.filter((x) => {
    if (isOfferCategoryClosed(x.category) && !(viewerChatId && isAdmin(viewerChatId))) return false;
...
const inlineReplyButtonStore = new Map();

function makeInlineReplyToken() {
  return Math.random().toString(36).slice(2, 10) + Date.now().toString(36).slice(-4);
}

function registerInlineReplyButton(chatId, text) {
  const token = makeInlineReplyToken();
  inlineReplyButtonStore.set(token, {
    chatId: Number(chatId || 0),
    text: String(text || ""),
    createdAt: Date.now(),
  });
  if (inlineReplyButtonStore.size > 5000) {
    const now = Date.now();
    for (const [k, v] of inlineReplyButtonStore.entries()) {
      if (!v || now - Number(v.createdAt || 0) > 24 * 60 * 60 * 1000) inlineReplyButtonStore.delete(k);
    }
  }
  return token;
}

function resolveInlineReplyButton(token) {
  const row = inlineReplyButtonStore.get(String(token || ""));
  if (!row) return null;
  if (Date.now() - Number(row.createdAt || 0) > 24 * 60 * 60 * 1000) {
    inlineReplyButtonStore.delete(String(token || ""));
    return null;
  }
  return row;
}
...
function translateReplyMarkup(chatId, replyMarkup) {
  try {
    ...
    if (replyMarkup.inline_keyboard || (replyMarkup.reply_markup && replyMarkup.reply_markup.inline_keyboard)) return replyMarkup;

    const keyboard = replyMarkup.keyboard || (replyMarkup.reply_markup && replyMarkup.reply_markup.keyboard);
    ...
    const inline_keyboard = keyboard
      .map((row) => {
        ...
        return row
          .map((btn) => {
            const label = typeof btn === "string" ? btn : (btn && typeof btn.text === "string" ? btn.text : "");
            if (!label) return null;
            const token = registerInlineReplyButton(chatId, label);
            return { text: label, callback_data: `kb:${token}` };
          })
          .filter(Boolean);
      })
...
function withdrawKeyboard() {
  const cfg = getWithdrawManagerCfg();
  const plans = Array.isArray(cfg.plans) && cfg.plans.length ? cfg.plans.slice() : DEFAULT_WITHDRAW_PLANS.slice();
  const rows = [];
  for (let i = 0; i < plans.length; i += 2) rows.push(plans.slice(i, i + 2).map((x) => `${x} pts`));
...
Tip: Use the bot's admin panel buttons for Weekly Draw settings, broadcast, paying winners, and resets.`;
    await bot.sendMessage(chatId, msg, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "🧩 Full Admin Panel", callback_data: "xadm:main" }],
...
if (isBtn(text,"💸 Withdraw Points","💸 Toa Pointi")) {
  const u = getUser(chatId);
  const savedPhone = getUserPhone(chatId);
  const withdrawCfg = getWithdrawManagerCfg();

  if (!isAdmin(chatId) && !withdrawCfg.enabled) {
    return sendTracked(chatId, "🚫 Withdrawals are currently OFF. Please try again later.", { ...mainMenuKeyboard(chatId) });
  }

  if (!isAdmin(chatId) && isPhoneBannedForWithdraw(savedPhone)) {
    return sendTracked(chatId, "🚫 This phone number is blocked from withdraw requests. Contact admin for help.", { ...mainMenuKeyboard(chatId) });
  }
  ...
  try {
    const wdPermit = xadmEvaluateWithdrawPermission(chatId, null, savedPhone);
    if (!wdPermit.ok) {
      return sendTracked(chatId, `🚫 ${wdPermit.reason}`, { ...mainMenuKeyboard(chatId) });
    }
  } catch (_) {}
...
if ((getWithdrawManagerCfg().plans || DEFAULT_WITHDRAW_PLANS).map((x) => `${x} pts`).includes(text)) {
  ...
  if (!isAdmin(chatId) && isPhoneBannedForWithdraw(phone07)) {
    return sendTracked(chatId, "🚫 This phone number is blocked from withdraw requests. Contact admin for help.", { ...mainMenuKeyboard(chatId) });
  }

  if (!isAdmin(chatId) && (amount < Number(withdrawCfg.minWithdraw || 0) || amount > Number(withdrawCfg.maxWithdraw || 0))) {
    return sendTracked(chatId, `❌ Withdraw amount must be between ${Number(withdrawCfg.minWithdraw || 0)} pts and ${Number(withdrawCfg.maxWithdraw || 0)} pts.`, { ...mainMenuKeyboard(chatId) });
  }

  if (!isAdmin(chatId) && Array.isArray(withdrawCfg.plans) && withdrawCfg.autoDecline?.outsidePlans && !withdrawCfg.plans.includes(amount)) {
    return sendTracked(chatId, "❌ That withdraw plan is not allowed right now.", { ...mainMenuKeyboard(chatId) });
  }

  try {
    const wdPermit = xadmEvaluateWithdrawPermission(chatId, amount, phone07);
    if (!wdPermit.ok) {
      return sendTracked(chatId, `🚫 ${wdPermit.reason}`, { ...mainMenuKeyboard(chatId) });
    }
  } catch (_) {}
...
// ===================== PURCHASE FLOW =====================
  ...
  try {
    ...
    if (s.step === "category") {
      ...
      if (typeof isOfferCategoryClosed === "function" && isOfferCategoryClosed(category) && !isAdmin(chatId)) {
        sessions.delete(chatId);
...
function isClosedOfferAllowedUser(chatId) {
  try {
    db = repairDB(db);
    if (isAdmin(chatId)) return true;
    return !!db.closedOfferAllowedUsers[String(chatId)];
  } catch (_) {
    return false;
  }
}

function setClosedOfferAllowedUser(chatId, value) {
  try {
    db = repairDB(db);
    db.closedOfferAllowedUsers[String(chatId)] = !!value;
    if (!value) delete db.closedOfferAllowedUsers[String(chatId)];
    saveDB();
  } catch (_) {}
}

function canUseCategoryEvenIfClosed(chatId, category) {
  return !isOfferCategoryClosed(category) || isClosedOfferAllowedUser(chatId);
}
...
function adminPanelOfferKeyboard() {
  ...
  rows.push([{ text: "✅ Allow User ID", callback_data: "ap:allow_closed_user" }, { text: "🚫 Disallow User ID", callback_data: "ap:disallow_closed_user" }]);
...
bot.on("message", async (msg) => {
  ...
  if (act.type === "allow_closed_offer_user" || act.type === "disallow_closed_offer_user") {
    const target = String(text || "").trim();
    if (!/^\d+$/.test(target)) {
      await bot.sendMessage(chatId, "❌ Send a valid numeric user ID or tap ❌ Cancel.").catch(() => {});
      return;
    }
    setClosedOfferAllowedUser(target, act.type === "allow_closed_offer_user");
    clearPendingAction(chatId);
    await bot.sendMessage(
      chatId,
      act.type === "allow_closed_offer_user"
        ? `✅ User ${target} can now use closed services.`
        : `✅ User ${target} removed from closed-services access.`,
      { ...mainMenuKeyboard(chatId) }
    ).catch(() => {});
    return;
  }
...
bot.on("callback_query", async (q) => {
  ...
  if (data === "ap:allow_closed_user") {
    ...
    await adminPanelSendOrEdit(chatId, "✅ Send the user ID to ALLOW using closed services.", adminPanelSimpleKeyboard("ap:offers"), q);
    return;
  }
  ...
  if (data === "ap:business") {
    await adminPanelSendOrEdit(chatId, adminPanelBusinessText(), { reply_markup: { inline_keyboard: [
  ...
  if (data === "ap:campaign") {
    await adminPanelSendOrEdit(chatId, adminPanelCampaignText(), { reply_markup: { inline_keyboard: [
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '📣 Post Now', callback_data: `cpadv:postnow:${targetId}` }],
...
// ===================== XADMIN INLINE PANEL =====================
function xadmEnsureDb() {
  db = repairDB(db);
  if (!db.xadmin || typeof db.xadmin !== "object") db.xadmin = {};
  const xa = db.xadmin;
  if (!xa.withdrawRequirements || typeof xa.withdrawRequirements !== "object") xa.withdrawRequirements = {};
  const wr = xa.withdrawRequirements;
  if (typeof wr.requireSpentEnabled !== "boolean") wr.requireSpentEnabled = false;
  if (!Number.isFinite(Number(wr.requireSpentMinKsh))) wr.requireSpentMinKsh = MIN_WITHDRAW_SPEND;
  if (typeof wr.requireReferralsEnabled !== "boolean") wr.requireReferralsEnabled = false;
  if (!Number.isFinite(Number(wr.requireReferralsMin))) wr.requireReferralsMin = 1;
  if (typeof wr.requireCampaignEnabled !== "boolean") wr.requireCampaignEnabled = false;
  if (typeof wr.requireGmailPurchase !== "boolean") wr.requireGmailPurchase = false;
  if (typeof wr.requireBingwaPurchase !== "boolean") wr.requireBingwaPurchase = false;
  if (typeof wr.requireUnlimitedPurchase !== "boolean") wr.requireUnlimitedPurchase = false;
  if (typeof wr.requireMinBalanceEnabled !== "boolean") wr.requireMinBalanceEnabled = false;
  if (!Number.isFinite(Number(wr.minBalancePts))) wr.minBalancePts = 0;
  if (!xa.userWithdrawControls || typeof xa.userWithdrawControls !== "object") xa.userWithdrawControls = {};
  if (!Array.isArray(xa.reports)) xa.reports = [];
  if (!xa.clickCounter || typeof xa.clickCounter !== "object") xa.clickCounter = {};
  if (!xa.adminState || typeof xa.adminState !== "object") xa.adminState = {};
  return xa;
}
function xadmGetAdminState(chatId) {
  const xa = xadmEnsureDb();
  return xa.adminState[String(chatId)] || null;
}
function xadmSetAdminState(chatId, state) {
  const xa = xadmEnsureDb();
  if (state && typeof state === "object") xa.adminState[String(chatId)] = state;
  else delete xa.adminState[String(chatId)];
  saveDB();
}
function xadmClearAdminState(chatId) { xadmSetAdminState(chatId, null); }
function xadmGetWithdrawRequirements() { return xadmEnsureDb().withdrawRequirements; }
function xadmGetUserWithdrawControl(chatId) {
  const xa = xadmEnsureDb();
  const key = String(chatId);
  const obj = xa.userWithdrawControls[key] && typeof xa.userWithdrawControls[key] === "object" ? xa.userWithdrawControls[key] : {};
  if (typeof obj.mode !== "string") obj.mode = "default";
  if (!Number.isFinite(Number(obj.min))) obj.min = 0;
  if (!Number.isFinite(Number(obj.max))) obj.max = 0;
  if (typeof obj.unlimited !== "boolean") obj.unlimited = false;
  if (typeof obj.freePromo !== "boolean") obj.freePromo = false;
  xa.userWithdrawControls[key] = obj;
  return obj;
}
function xadmSetUserWithdrawControl(chatId, patch) {
  const xa = xadmEnsureDb();
  const cur = xadmGetUserWithdrawControl(chatId);
  xa.userWithdrawControls[String(chatId)] = { ...cur, ...patch };
  saveDB();
  return xa.userWithdrawControls[String(chatId)];
}
function xadmCountCategoryPurchases(chatId, category) {
  try {
    const u = getUser(chatId);
    return Number((((u||{}).stats||{}).categoryCounts||{})[String(category||"")] || 0);
  } catch (_) { return 0; }
}
function xadmHasCampaignParticipation(chatId) {
  try {
    const c = ensureWeeklyDrawObject();
    if (c && c.entries && c.entries[String(chatId)]) return true;
  } catch (_) {}
  try {
    return !!(((db||{}).campaign||{}).entries||{})[String(chatId)];
  } catch (_) {}
  return false;
}
function xadmResolveUser(input) {
  const raw = String(input || "").trim();
  if (!raw) return null;
  const norm = normalizePhone(raw);
  db = repairDB(db);
  if (/^\d{5,}$/.test(raw) && db.users && db.users[raw]) return { chatId: raw, user: getUser(raw), match: "chat_id" };
  if (norm && db.users) {
    for (const [cid, u] of Object.entries(db.users || {})) {
      const phone = normalizePhone(u && (u.phone || u.lastLinkedPhone || ""));
      if (phone && phone === norm) return { chatId: cid, user: getUser(cid), match: "phone" };
    }
  }
  return null;
}
function xadmEvaluateWithdrawPermission(chatId, amount = null, phone = "") {
  try {
    if (isAdmin(chatId)) return { ok: true, reason: "admin bypass" };
    const ctrl = xadmGetUserWithdrawControl(chatId);
    const req = xadmGetWithdrawRequirements();
    const u = getUser(chatId);
    const amt = Number(amount || 0);
    if (ctrl.mode === "ban") return { ok: false, reason: "Withdraw is banned for this user." };
    if (ctrl.mode === "disallow") return { ok: false, reason: "Withdraw is disabled for this user." };
    if (req.requireSpentEnabled && !ctrl.freePromo) {
      const need = Number(req.requireSpentMinKsh || 0);
      const spent = Number(u.totalSpentKsh || 0);
      if (spent < need) return { ok: false, reason: `User must spend at least Ksh ${need}. Current spend: Ksh ${spent}.` };
    }
    if (req.requireReferralsEnabled && !ctrl.freePromo) {
      const needRefs = Number(req.requireReferralsMin || 0);
      const gotRefs = Number(u.referralSuccessCount || 0);
      if (gotRefs < needRefs) return { ok: false, reason: `User needs ${needRefs} successful referral(s). Current: ${gotRefs}.` };
    }
    if (req.requireCampaignEnabled && !ctrl.freePromo && !xadmHasCampaignParticipation(chatId)) {
      return { ok: false, reason: "Campaign participation is required before withdrawing." };
    }
    if (req.requireGmailPurchase && !ctrl.freePromo && xadmCountCategoryPurchases(chatId, "Gmail Accounts") < 1) {
      return { ok: false, reason: "At least one Gmail purchase is required before withdrawing." };
    }
    if (req.requireBingwaPurchase && !ctrl.freePromo && xadmCountCategoryPurchases(chatId, "Bingwa Deals") < 1) {
      return { ok: false, reason: "At least one Bingwa purchase is required before withdrawing." };
    }
    if (req.requireUnlimitedPurchase && !ctrl.freePromo && xadmCountCategoryPurchases(chatId, "Unlimited Deals") < 1) {
      return { ok: false, reason: "At least one Unlimited purchase is required before withdrawing." };
    }
    if (req.requireMinBalanceEnabled) {
      const bal = Number(getPoints(chatId) || 0);
      const needBal = Number(req.minBalancePts || 0);
      if (bal < needBal) return { ok: false, reason: `Minimum balance is ${needBal} pts. Current balance: ${bal.toFixed(2)} pts.` };
    }
    if (!ctrl.unlimited && amt > 0) {
      if (Number(ctrl.min || 0) > 0 && amt < Number(ctrl.min || 0)) return { ok: false, reason: `User minimum withdraw is ${Number(ctrl.min || 0)} pts.` };
      if (Number(ctrl.max || 0) > 0 && amt > Number(ctrl.max || 0)) return { ok: false, reason: `User maximum withdraw is ${Number(ctrl.max || 0)} pts.` };
    }
    return { ok: true, reason: "ok" };
  } catch (e) {
    return { ok: true, reason: String(e && e.message || e || "ok") };
  }
}
function xadmRiskScore(chatId) {
  const u = getUser(chatId);
  let score = 0;
  if (u.banned) score += 50;
  if (!u.phone) score += 10;
  score += Math.min(15, Number((u.withdrawStats||{}).declinedCount || 0) * 5);
  score -= Math.min(20, Number(u.referralSuccessCount || 0) * 2);
  score -= Math.min(10, Math.floor(Number(u.totalSpentKsh || 0) / 500));
  score = Math.max(0, Math.min(100, score));
  const level = score >= 60 ? "HIGH" : (score >= 30 ? "MEDIUM" : "LOW");
  return { score, level };
}
function xadmFmtDate(ts) { return ts ? kenyaDateTimeFromTs(Number(ts || 0)) : "N/A"; }
function xadmMainKb() {
  return { reply_markup: { inline_keyboard: [
    [{ text: "💸 Withdraw Manager", callback_data: "xadm:wm" }, { text: "✅ Withdraw Requirements", callback_data: "xadm:req" }],
    [{ text: "👤 User Withdraw Control", callback_data: "xadm:usr" }, { text: "💎 Points Manager", callback_data: "xadm:pts" }],
    [{ text: "🕵️ User Inspector", callback_data: "xadm:insp" }, { text: "📲 STK Request", callback_data: "xadm:stk" }],
    [{ text: "🏢 Business Tools", callback_data: "xadm:biz" }, { text: "🚨 Reports", callback_data: "xadm:rep" }],
  ] } };
}
function xadmBackRow(section) { return [{ text: "⬅️ Back", callback_data: section ? `xadm:${section}` : "xadm:main" }]; }
async function xadmRenderMain(qOrChat) {
  const chatId = typeof qOrChat === "object" && qOrChat.message ? qOrChat.message.chat.id : qOrChat;
  const text = "🧩 *Full Admin Panel*\n\nChoose a module below.";
  if (typeof qOrChat === "object" && qOrChat.message) return editInlineMessage(qOrChat, text, { parse_mode: "Markdown", ...xadmMainKb() });
  return bot.sendMessage(chatId, text, { parse_mode: "Markdown", ...xadmMainKb() }).catch(() => {});
}
async function xadmRenderWithdrawManager(q) {
  const cfg = getWithdrawManagerCfg();
  const txt = `💸 *Withdraw Manager*\n\nStatus: *${cfg.enabled ? "ON" : "OFF"}*\nMin: *${Number(cfg.minWithdraw || 0)} pts*\nMax: *${Number(cfg.maxWithdraw || 0)} pts*\nPlans: *${(cfg.plans || []).join(", ") || "none"}*\nPhone ban auto-decline: *${cfg.autoDecline?.phoneBan ? "ON" : "OFF"}*\nOutside plans auto-decline: *${cfg.autoDecline?.outsidePlans ? "ON" : "OFF"}*\nPhone ban list: *${(cfg.phoneBanList || []).length}* number(s)`;
  return editInlineMessage(q, txt, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [
    [{ text: cfg.enabled ? "🟥 Turn OFF" : "🟩 Turn ON", callback_data: "xadm:wm_toggle" }],
    [{ text: "✏️ Set Min", callback_data: "xadm:wm_setmin" }, { text: "✏️ Set Max", callback_data: "xadm:wm_setmax" }],
    [{ text: "🧾 Set Plans", callback_data: "xadm:wm_setplans" }],
    [{ text: `📵 Phone Ban ${cfg.autoDecline?.phoneBan ? "ON" : "OFF"}`, callback_data: "xadm:wm_toggle_phoneban" }, { text: `📋 Outside Plans ${cfg.autoDecline?.outsidePlans ? "ON" : "OFF"}`, callback_data: "xadm:wm_toggle_outside" }],
    [{ text: "➕ Add Phone Ban", callback_data: "xadm:wm_addban" }, { text: "➖ Remove Phone Ban", callback_data: "xadm:wm_delban" }],
    xadmBackRow(),
  ] } });
}
async function xadmRenderRequirements(q) {
  const r = xadmGetWithdrawRequirements();
  const txt = `✅ *Withdraw Requirement Toggles*\n\nSpend required: *${r.requireSpentEnabled ? "ON" : "OFF"}* (Ksh ${Number(r.requireSpentMinKsh || 0)})\nReferrals required: *${r.requireReferralsEnabled ? "ON" : "OFF"}* (${Number(r.requireReferralsMin || 0)})\nCampaign participation: *${r.requireCampaignEnabled ? "ON" : "OFF"}*\nGmail purchase: *${r.requireGmailPurchase ? "ON" : "OFF"}*\nBingwa purchase: *${r.requireBingwaPurchase ? "ON" : "OFF"}*\nUnlimited purchase: *${r.requireUnlimitedPurchase ? "ON" : "OFF"}*\nMinimum balance: *${r.requireMinBalanceEnabled ? "ON" : "OFF"}* (${Number(r.minBalancePts || 0)} pts)`;
  return editInlineMessage(q, txt, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [
    [{ text: `💰 Spend ${r.requireSpentEnabled ? "ON" : "OFF"}`, callback_data: "xadm:req_spend_toggle" }, { text: "Set Spend Ksh", callback_data: "xadm:req_spend_set" }],
    [{ text: `👥 Referrals ${r.requireReferralsEnabled ? "ON" : "OFF"}`, callback_data: "xadm:req_ref_toggle" }, { text: "Set Referral Count", callback_data: "xadm:req_ref_set" }],
    [{ text: `🎉 Campaign ${r.requireCampaignEnabled ? "ON" : "OFF"}`, callback_data: "xadm:req_campaign_toggle" }],
    [{ text: `📧 Gmail ${r.requireGmailPurchase ? "ON" : "OFF"}`, callback_data: "xadm:req_gmail_toggle" }, { text: `📦 Bingwa ${r.requireBingwaPurchase ? "ON" : "OFF"}`, callback_data: "xadm:req_bingwa_toggle" }],
    [{ text: `∞ Unlimited ${r.requireUnlimitedPurchase ? "ON" : "OFF"}`, callback_data: "xadm:req_unlimited_toggle" }],
    [{ text: `💎 Min Balance ${r.requireMinBalanceEnabled ? "ON" : "OFF"}`, callback_data: "xadm:req_minbal_toggle" }, { text: "Set Balance", callback_data: "xadm:req_minbal_set" }],
    xadmBackRow(),
  ] } });
}
async function xadmRenderUserControl(q) {
  const txt = "👤 *User Withdraw Control*\n\nChoose an action. The panel will ask for a user chat ID or saved phone number.";
  return editInlineMessage(q, txt, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [
    [{ text: "✅ Allow", callback_data: "xadm:usr_allow" }, { text: "🚫 Disallow", callback_data: "xadm:usr_disallow" }],
    [{ text: "⛔ Ban Withdraw", callback_data: "xadm:usr_ban" }, { text: "♾ Unlimited", callback_data: "xadm:usr_unlimited" }],
    [{ text: "🎁 Free Promo", callback_data: "xadm:usr_freepromo" }, { text: "🔄 Reset User", callback_data: "xadm:usr_reset" }],
    [{ text: "✏️ Set User Min", callback_data: "xadm:usr_setmin" }, { text: "✏️ Set User Max", callback_data: "xadm:usr_setmax" }],
    xadmBackRow(),
  ] } });
}
async function xadmRenderPoints(q) {
  const txt = "💎 *Points Manager*\n\nChoose an action. You will then send the required values.";
  return editInlineMessage(q, txt, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [
    [{ text: "➕ Add Points", callback_data: "xadm:pts_add" }, { text: "➖ Remove Points", callback_data: "xadm:pts_remove" }],
    [{ text: "🎯 Set Exact Balance", callback_data: "xadm:pts_set" }, { text: "🔁 Transfer Points", callback_data: "xadm:pts_transfer" }],
    xadmBackRow(),
  ] } });
}
async function xadmRenderInspector(q) {
  return editInlineMessage(q, "🕵️ *User Inspector*\n\nSend a user chat ID or saved phone number.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [xadmBackRow()] } });
}
async function xadmRenderStk(q) {
  return editInlineMessage(q, "📲 *STK Request Tool*\n\nSend details as:\n`254712345678 100`\n\nor use command:\n`/stk 254712345678 100`", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [xadmBackRow()] } });
}
async function xadmRenderBusiness(q) {
  const txt = "🏢 *Business Account Tools*\n\nChoose an action. The panel will ask for the user and days when needed.";
  return editInlineMessage(q, txt, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [
    [{ text: "🎁 Grant Duration", callback_data: "xadm:biz_grant" }, { text: "🗑 Remove Business", callback_data: "xadm:biz_remove" }],
    xadmBackRow(),
  ] } });
}
function xadmGetOpenReports() {
  return (xadmEnsureDb().reports || []).filter(r => r && !r.reviewed);
}
async function xadmRenderReports(q) {
  const open = xadmGetOpenReports();
  const lines = ["🚨 *Reports System*", "", `Open reports: *${open.length}*`];
  if (open[0]) {
    lines.push("", `Latest: #${open[0].id}`, `Reporter: ${open[0].reporterId}`, `Target: ${open[0].targetId || open[0].targetPhone || "N/A"}`, `Reason: ${mdEscape(String(open[0].reason || ""))}`);
  }
  const rows = open.slice(0, 5).map(r => [{ text: `Review #${r.id}`, callback_data: `xadm:rep_view:${r.id}` }, { text: `Mark Done #${r.id}`, callback_data: `xadm:rep_done:${r.id}` }]);
  rows.push([{ text: "📝 How users report", callback_data: "xadm:rep_help" }]);
  rows.push(xadmBackRow());
  return editInlineMessage(q, lines.join("\n"), { parse_mode: "Markdown", reply_markup: { inline_keyboard: rows } });
}
function xadmBuildInspectorText(targetId) {
  const u = getUser(targetId);
  ensureUserStats(u);
  ensureTxLog(u);
  const risk = xadmRiskScore(targetId);
  const lastTx = Array.isArray(u.txLog) && u.txLog[0] ? u.txLog[0] : null;
  const ctrl = xadmGetUserWithdrawControl(targetId);
  return [
    `🕵️ *User Inspector*`,
    ``,
    `User: \`${targetId}\``,
    `Phone: *${mdEscape(String(u.phone || "N/A"))}*`,
    `Balance: *${Number(getPoints(targetId) || 0).toFixed(2)} pts*`,
    `Risk score: *${risk.score}/100 (${risk.level})*`,
    `Total purchases: *${Number(u.stats?.purchaseCount || 0)}*`,
    `Last purchase: *${mdEscape(lastTx ? fmtTxLine(lastTx) + " @ " + shortKenyaTime(lastTx.ts) : "N/A")}*`,
    `Join date: *${mdEscape(xadmFmtDate(u.createdAt || u.firstSeen || 0))}*`,
    `Referral stats: *${Number(u.referralSuccessCount || 0)} successful*`,
    `Withdraw history: *approved ${Number((u.withdrawStats||{}).approvedCount || 0)}, declined ${Number((u.withdrawStats||{}).declinedCount || 0)}*`,
    `Withdraw control: *${mdEscape(JSON.stringify(ctrl))}*`,
    `Business: *${isBusinessActive(targetId) ? "ACTIVE" : "NO"}*`,
  ].join("\n");
}
async function xadmSendAdminPrompt(chatId, mode, prompt, extra = {}) {
  xadmSetAdminState(chatId, { mode, ...extra });
  await bot.sendMessage(chatId, prompt, { parse_mode: "Markdown" }).catch(() => {});
}
async function xadmRunStk(adminChatId, phoneRaw, amountRaw) {
  const phone = normalizePhone(phoneRaw);
  const amount = Number(amountRaw || 0);
  if (!phone || !Number.isFinite(amount) || amount <= 0) throw new Error("Invalid phone or amount.");
  const ref = `ADMSTK-${Date.now()}-${adminChatId}`;
  await payheroStkPush({ amount, phone, externalRef: ref, channelId: PAYHERO_CHANNEL_ID_DATA });
  return { phone, amount, ref };
}
async function xadmHandleCallback(q) {
  const data = String(q && q.data || "");
  if (!data.startsWith("xadm:")) return;
  const chatId = Number(q.from && q.from.id || 0);
  if (!isAdmin(chatId)) {
  ...
  const xa = xadmEnsureDb();
  try {
    xa.clickCounter[String(chatId)] = xa.clickCounter[String(chatId)] || {};
    const now = Date.now();
    const last = Number(xa.clickCounter[String(chatId)][data] || 0);
    if (last && (now - last) < 1200) return;
    xa.clickCounter[String(chatId)][data] = now;
    saveDB();
  } catch (_) {}
  if (data === "xadm:main") return xadmRenderMain(q);
  if (data === "xadm:wm") return xadmRenderWithdrawManager(q);
  if (data === "xadm:req") return xadmRenderRequirements(q);
  if (data === "xadm:usr") return xadmRenderUserControl(q);
  if (data === "xadm:pts") return xadmRenderPoints(q);
  if (data === "xadm:insp") { await xadmSendAdminPrompt(chatId, "inspect_user", "Send user *chat ID* or *saved phone*."); return xadmRenderInspector(q); }
  if (data === "xadm:stk") { await xadmSendAdminPrompt(chatId, "stk_request", "Send: `254712345678 100`"); return xadmRenderStk(q); }
  if (data === "xadm:biz") return xadmRenderBusiness(q);
  if (data === "xadm:rep") return xadmRenderReports(q);
  const cfg = getWithdrawManagerCfg();
  const req = xadmGetWithdrawRequirements();
  if (data === "xadm:wm_toggle") { cfg.enabled = !cfg.enabled; saveWithdrawManagerCfg(cfg); return xadmRenderWithdrawManager(q); }
  if (data === "xadm:wm_toggle_phoneban") { cfg.autoDecline.phoneBan = !cfg.autoDecline.phoneBan; saveWithdrawManagerCfg(cfg); return xadmRenderWithdrawManager(q); }
  if (data === "xadm:wm_toggle_outside") { cfg.autoDecline.outsidePlans = !cfg.autoDecline.outsidePlans; saveWithdrawManagerCfg(cfg); return xadmRenderWithdrawManager(q); }
  if (data === "xadm:wm_setmin") return xadmSendAdminPrompt(chatId, "wm_setmin", "Send new minimum withdraw in *points*.");
  if (data === "xadm:wm_setmax") return xadmSendAdminPrompt(chatId, "wm_setmax", "Send new maximum withdraw in *points*.");
  if (data === "xadm:wm_setplans") return xadmSendAdminPrompt(chatId, "wm_setplans", "Send allowed withdraw plans separated by commas. Example: `25,50,100,250`.");
  if (data === "xadm:wm_addban") return xadmSendAdminPrompt(chatId, "wm_addban", "Send phone to add to withdraw ban list.");
  if (data === "xadm:wm_delban") return xadmSendAdminPrompt(chatId, "wm_delban", "Send phone to remove from withdraw ban list.");
  if (data === "xadm:req_spend_toggle") { req.requireSpentEnabled = !req.requireSpentEnabled; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_spend_set") return xadmSendAdminPrompt(chatId, "req_spend_set", "Send minimum spend in *Ksh*.");
  if (data === "xadm:req_ref_toggle") { req.requireReferralsEnabled = !req.requireReferralsEnabled; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_ref_set") return xadmSendAdminPrompt(chatId, "req_ref_set", "Send minimum successful referrals required.");
  if (data === "xadm:req_campaign_toggle") { req.requireCampaignEnabled = !req.requireCampaignEnabled; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_gmail_toggle") { req.requireGmailPurchase = !req.requireGmailPurchase; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_bingwa_toggle") { req.requireBingwaPurchase = !req.requireBingwaPurchase; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_unlimited_toggle") { req.requireUnlimitedPurchase = !req.requireUnlimitedPurchase; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_minbal_toggle") { req.requireMinBalanceEnabled = !req.requireMinBalanceEnabled; saveDB(); return xadmRenderRequirements(q); }
  if (data === "xadm:req_minbal_set") return xadmSendAdminPrompt(chatId, "req_minbal_set", "Send minimum balance in *points*.");
  if (data.startsWith("xadm:usr_")) return xadmSendAdminPrompt(chatId, data.slice(5), "Send target user *chat ID* or *saved phone*.\nFor min/max use: `target value`.");
  if (data.startsWith("xadm:pts_")) return xadmSendAdminPrompt(chatId, data.slice(5), data === "xadm:pts_transfer" ? "Send: `fromUser toUser amount`" : "Send: `target amount`" );
  if (data === "xadm:biz_grant") return xadmSendAdminPrompt(chatId, "biz_grant", "Send: `targetUser days`\nExample: `7859465542 30`" );
  if (data === "xadm:biz_remove") return xadmSendAdminPrompt(chatId, "biz_remove", "Send target user chat ID or saved phone.");
  if (data === "xadm:rep_help") return bot.sendMessage(chatId, "Users can report with:\n/report <chat_id or phone> <reason>").catch(() => {});
  if (data.startsWith("xadm:rep_view:")) {
    const id = String(data.split(":")[2] || "");
    const rep = (xa.reports || []).find(r => String(r.id) === id);
    if (!rep) return bot.sendMessage(chatId, "❌ Report not found.").catch(() => {});
    const out = `🚨 *Report #${rep.id}*\n\nReporter: \`${rep.reporterId}\`\nTarget: *${mdEscape(String(rep.targetId || rep.targetPhone || "N/A"))}*\nWhen: *${mdEscape(xadmFmtDate(rep.createdAt))}*\nReason: ${mdEscape(String(rep.reason || ""))}`;
    return bot.sendMessage(chatId, out, { parse_mode: "Markdown" }).catch(() => {});
  }
  if (data.startsWith("xadm:rep_done:")) {
    const id = String(data.split(":")[2] || "");
    const rep = (xa.reports || []).find(r => String(r.id) === id);
    if (rep) { rep.reviewed = true; rep.reviewedAt = Date.now(); rep.reviewedBy = String(chatId); saveDB(); }
    return xadmRenderReports(q);
  }
}
bot.on("callback_query", async (q) => { try { await xadmHandleCallback(q); } catch (e) { try { await bot.answerCallbackQuery(q.id, { text: String(e && e.message || e || "Error"), show_alert: true }); } catch (_) {} } });

bot.on("message", async (msg) => {
  try {
    const chatId = Number(msg && msg.chat && msg.chat.id || 0);
    const text = String(msg && msg.text || "").trim();
    if (!chatId || !text) return;
    if (/^\/report\b/i.test(text) && !isAdmin(chatId)) {
      const parts = text.split(/\s+/);
      if (parts.length < 3) {
        await bot.sendMessage(chatId, "Usage: /report <chat_id_or_phone> <reason>").catch(() => {});
        return;
      }
      const targetRaw = parts[1];
      const reason = parts.slice(2).join(" ").trim();
      const target = xadmResolveUser(targetRaw);
      const xa = xadmEnsureDb();
      const id = (xa.reports[0] ? Number(xa.reports[0].id || 0) + 1 : 1);
      xa.reports.unshift({ id, reporterId: String(chatId), targetId: target ? String(target.chatId) : "", targetPhone: normalizePhone(targetRaw) || "", reason, reviewed: false, createdAt: Date.now() });
      saveDB();
      await bot.sendMessage(chatId, `✅ Report submitted. Reference: #${id}`).catch(() => {});
      try { await bot.sendMessage(ADMIN_ID, `🚨 New report #${id}\nReporter: ${chatId}\nTarget: ${target ? target.chatId : (normalizePhone(targetRaw) || targetRaw)}\nReason: ${reason}`, { reply_markup: { inline_keyboard: [[{ text: `Open Reports`, callback_data: "xadm:rep" }]] } }); } catch (_) {}
      return;
    }
    if (/^\/stk\b/i.test(text)) {
      if (!isAdmin(chatId)) { await bot.sendMessage(chatId, "⛔ Not authorized.").catch(() => {}); return; }
      const parts = text.split(/\s+/);
      if (parts.length < 3) { await bot.sendMessage(chatId, "Usage: /stk 254712345678 100").catch(() => {}); return; }
      const r = await xadmRunStk(chatId, parts[1], parts[2]);
      await bot.sendMessage(chatId, `✅ STK sent to ${r.phone} for Ksh ${r.amount}.\nRef: ${r.ref}`).catch(() => {});
      return;
    }
    const state = xadmGetAdminState(chatId);
    if (!state || !isAdmin(chatId)) return;
    if (text.toUpperCase() === "CANCEL") { xadmClearAdminState(chatId); await bot.sendMessage(chatId, "✅ Cancelled.", xadmMainKb()).catch(() => {}); return; }
    const cfg = getWithdrawManagerCfg();
    const req = xadmGetWithdrawRequirements();
    if (state.mode === "wm_setmin") { cfg.minWithdraw = Math.max(1, Math.floor(Number(text || 0))); saveWithdrawManagerCfg(cfg); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Min withdraw set to ${cfg.minWithdraw} pts.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "wm_setmax") { cfg.maxWithdraw = Math.max(1, Math.floor(Number(text || 0))); saveWithdrawManagerCfg(cfg); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Max withdraw set to ${cfg.maxWithdraw} pts.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "wm_setplans") { cfg.plans = Array.from(new Set(String(text).split(/[,\s]+/).map(x => Math.floor(Number(x||0))).filter(x => x > 0))).sort((a,b)=>a-b); saveWithdrawManagerCfg(cfg); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Withdraw plans updated: ${cfg.plans.join(", ")}`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "wm_addban" || state.mode === "wm_delban") { const ph = normalizePhone(text); if (!ph) { await bot.sendMessage(chatId, "❌ Invalid phone.").catch(() => {}); return; } cfg.phoneBanList = Array.from(new Set((cfg.phoneBanList || []).map(x => normalizePhone(x)).filter(Boolean))); if (state.mode === "wm_addban") { if (!cfg.phoneBanList.includes(ph)) cfg.phoneBanList.push(ph); } else { cfg.phoneBanList = cfg.phoneBanList.filter(x => x !== ph); } saveWithdrawManagerCfg(cfg); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Phone ban list updated.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "req_spend_set") { req.requireSpentMinKsh = Math.max(0, Number(text || 0)); saveDB(); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Spend requirement set to Ksh ${req.requireSpentMinKsh}.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "req_ref_set") { req.requireReferralsMin = Math.max(0, Math.floor(Number(text || 0))); saveDB(); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Referral requirement set to ${req.requireReferralsMin}.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "req_minbal_set") { req.minBalancePts = Math.max(0, Number(text || 0)); saveDB(); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Minimum balance set to ${req.minBalancePts} pts.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "inspect_user") { const target = xadmResolveUser(text); if (!target) { await bot.sendMessage(chatId, "❌ User not found.").catch(() => {}); return; } xadmClearAdminState(chatId); await bot.sendMessage(chatId, xadmBuildInspectorText(target.chatId), { parse_mode: "Markdown", ...xadmMainKb() }).catch(() => {}); return; }
    if (state.mode === "stk_request") { const parts = text.split(/\s+/); const r = await xadmRunStk(chatId, parts[0], parts[1]); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ STK sent to ${r.phone} for Ksh ${r.amount}.\nRef: ${r.ref}`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "biz_grant") { const parts = text.split(/\s+/); const target = xadmResolveUser(parts[0]); const days = Math.max(1, Math.floor(Number(parts[1] || 0))); if (!target || !days) { await bot.sendMessage(chatId, "❌ Send: target days").catch(() => {}); return; } await giveFreeBusinessDays(Number(target.chatId), days); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Business granted for ${days} day(s) to ${target.chatId}.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode === "biz_remove") { const target = xadmResolveUser(text); if (!target) { await bot.sendMessage(chatId, "❌ User not found.").catch(() => {}); return; } deactivateBusiness(Number(target.chatId)); xadmClearAdminState(chatId); await bot.sendMessage(chatId, `✅ Business removed for ${target.chatId}.`, xadmMainKb()).catch(() => {}); return; }
    if (state.mode.startsWith("usr_")) {
      const parts = text.split(/\s+/);
      const target = xadmResolveUser(parts[0]);
      if (!target) { await bot.sendMessage(chatId, "❌ User not found.").catch(() => {}); return; }
      const action = state.mode;
      if (action === "usr_allow") xadmSetUserWithdrawControl(target.chatId, { mode: "allow" });
      else if (action === "usr_disallow") xadmSetUserWithdrawControl(target.chatId, { mode: "disallow" });
      else if (action === "usr_ban") xadmSetUserWithdrawControl(target.chatId, { mode: "ban" });
      else if (action === "usr_unlimited") xadmSetUserWithdrawControl(target.chatId, { unlimited: !xadmGetUserWithdrawControl(target.chatId).unlimited, mode: "allow" });
      else if (action === "usr_freepromo") xadmSetUserWithdrawControl(target.chatId, { freePromo: !xadmGetUserWithdrawControl(target.chatId).freePromo, mode: "allow" });
      else if (action === "usr_reset") xadmSetUserWithdrawControl(target.chatId, { mode: "default", min: 0, max: 0, unlimited: false, freePromo: false });
      else if (action === "usr_setmin") { const v = Number(parts[1] || 0); xadmSetUserWithdrawControl(target.chatId, { min: Math.max(0, v) }); }
      else if (action === "usr_setmax") { const v = Number(parts[1] || 0); xadmSetUserWithdrawControl(target.chatId, { max: Math.max(0, v) }); }
      xadmClearAdminState(chatId);
      await bot.sendMessage(chatId, `✅ User withdraw control updated for ${target.chatId}.`, xadmMainKb()).catch(() => {});
      return;
    }
    if (state.mode.startsWith("pts_")) {
      const parts = text.split(/\s+/);
      if (state.mode === "pts_transfer") {
        const from = xadmResolveUser(parts[0]); const to = xadmResolveUser(parts[1]); const amt = Number(parts[2] || 0);
        if (!from || !to || !(amt > 0)) { await bot.sendMessage(chatId, "❌ Send: fromUser toUser amount").catch(() => {}); return; }
        if (Number(getPoints(from.chatId) || 0) < amt) { await bot.sendMessage(chatId, "❌ Sender does not have enough points.").catch(() => {}); return; }
        deductPoints(Number(from.chatId), amt); addPoints(Number(to.chatId), amt); xadmClearAdminState(chatId);
        await bot.sendMessage(chatId, `✅ Transferred ${amt} pts from ${from.chatId} to ${to.chatId}.`, xadmMainKb()).catch(() => {}); return;
      }
      const target = xadmResolveUser(parts[0]); const amt = Number(parts[1] || 0);
      if (!target || !Number.isFinite(amt)) { await bot.sendMessage(chatId, "❌ Send: target amount").catch(() => {}); return; }
      if (state.mode === "pts_add") addPoints(Number(target.chatId), amt);
      else if (state.mode === "pts_remove") deductPoints(Number(target.chatId), amt);
      else if (state.mode === "pts_set") { const u = getUser(target.chatId); u.points = Math.max(0, Number(amt || 0)); saveDB(); }
      xadmClearAdminState(chatId);
      await bot.sendMessage(chatId, `✅ Points updated for ${target.chatId}. Balance: ${Number(getPoints(target.chatId) || 0).toFixed(2)} pts`, xadmMainKb()).catch(() => {});
      return;
    }
  } catch (e) {
    await bot.sendMessage(Number(msg && msg.chat && msg.chat.id || 0), `❌ ${String(e && e.message || e || "Failed")}`).catch(() => {});
  }
});
