...
function scheduleAutoDelete(sentMsg, ms = 24 * 60 * 60 * 1000) {
  // Disabled: only delete previous bot messages when the user enables the toggle.
  return;
}
...
function shouldAutoDeleteOutbound() {
  // Timer-based auto delete removed.
  return false;
}
...
try {
  ...
  bot.sendMessage = async (chatId, text, options = {}) => {
    ...
    if (!skipCleanupTrack && m && m.message_id) trackBotMsg(chatId, m.message_id);
    if (shouldAutoDeleteOutbound(text, opt)) scheduleAutoDelete(m, getAutoDeleteMsForChat(chatId));
    return m;
  };
  ...
  bot.sendPhoto = async (chatId, photo, options = {}) => {
    ...
    if (!skipCleanupTrack && m && m.message_id) trackBotMsg(chatId, m.message_id);
    if (shouldAutoDeleteOutbound(cap, opt)) scheduleAutoDelete(m, getAutoDeleteMsForChat(chatId));
    return m;
  };
} catch (_) {}
...
async function cleanupBeforeReply(chatId, userMsgId) {
  ...
  // Only delete previously sent bot messages when the user has explicitly enabled auto delete.
  if (!getAutoDeleteMsForChat(chatId)) return;
        ...
        try {
          ...
          await sendTracked(
            ...
            `⏳ Reminder: You haven't completed the payment yet.\n\nTap 🔄 Resend STK if you didn't receive the prompt.\n\nHelp: ${HELP_PHONE}`,
            {
              reply_markup: {
                inline_keyboard: [
                  [{ text: "🔄 Resend STK", callback_data: `ui:retry_stk:${ref}` }],
                  [{ text: "❌ Cancel", callback_data: `ui:stk_cancel:${ref}` }]
                ]
              }
            }
          );
...
async function sendTracked(chatId, text, options = {}) {
  ...
  if (!isWelcomeMessage && getAutoDeleteMsForChat(chatId)) {
    await clearTrackedBotMessages(chatId);
  }

  const opt = { ...(options || {}) };
  if (isWelcomeMessage) opt.__skipCleanupTrack = true;

  const sent = await bot.sendMessage(chatId, text, opt);

  // Only remember non-welcome messages
  if (!isWelcomeMessage) {
...
function findPackageByPrice(category, price) {
  const list = PACKAGES[category] || [];
  const amt = Number(price || 0);
  if (!amt) return null;
  return list.find((x) => Number(x.price || 0) === amt) || null;
}

function categoryFromExternalRef(externalRef) {
  try {
    const parts = String(externalRef || "").split("-");
    if (parts.length < 5) return "";
    return String(parts.slice(2, -2).join("-") || "").replace(/_/g, " ").trim();
  } catch (_) {
    return "";
  }
}

function kenyaTransactionTimeFromTs(ts) {
  const t = Number(ts || Date.now());
  try {
    const parts = new Intl.DateTimeFormat("en-GB", {
      timeZone: "Africa/Nairobi",
...
if (pending && pending.category === "Business Account") {
  ...
  if (plan) {
    ...
    user.totalSpentKsh = Number(user.totalSpentKsh || 0) + Number(pending?.price || cbAmount || amount || 0);
    try {
    ...
    const fallbackCategory = categoryFromExternalRef(externalRef);
    const fallbackPackage = findPackageByPrice(fallbackCategory, cbAmount || pending?.price || amount || 0);
    const packageName =
      pending?.pkgLabel ||
      fallbackPackage?.label ||
      payload?.package_name ||
      body?.package_name ||
      `${fallbackCategory ? `${fallbackCategory} • ` : ""}Ksh ${Number(cbAmount || pending?.price || amount || 0)}`.trim();
    const paymentNumber = String(pending?.phone254 || cbPhone || payload?.phone_number || payload?.PhoneNumber || payload?.phone || body?.phone_number || body?.phone || "").trim();
    const receiveNumber = String(pending?.receivePhone254 || pending?.phone254 || cbPhone || "").trim();
    const amountPaid = Number(pending?.price || cbAmount || amount || 0);
    const transactionTime = kenyaTransactionTimeFromTs(whenTs);
    try { addUserTx(chatId, { status: "success", pkgLabel: packageName, amountKsh: amountPaid, phone254: paymentNumber || String(pending?.phone254 || "") }); } catch (_) {}
...
` +
      `📦 Package: ${packageName}
` +
      `📱 Payment Number: ${maskPhone(paymentNumber)}
` +
      `📥 Receive Number: ${maskPhone(receiveNumber)}
` +
      `🧾 Transaction Time: ${transactionTime}
` +
      `💰 Amount: Ksh ${amountPaid}
` +
      `🛒 Total Purchases: ${totalPurchases}
