...
// Returning-user welcome after N days inactive
const INACTIVE_WELCOME_DAYS = Number(process.env.INACTIVE_WELCOME_DAYS || 4);
...
// promos + bot settings
if (!db0.promos || typeof db0.promos !== "object") db0.promos = {};
if (!db0.promoRedemptions || typeof db0.promoRedemptions !== "object") db0.promoRedemptions = {}; // {chatId:{CODE:ts}}
if (!db0.settings || typeof db0.settings !== "object") db0.settings = {};
if (typeof db0.settings.aiSupportEnabled !== "boolean") db0.settings.aiSupportEnabled = true;
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.lastWelcomeBackDay !== "string") x.lastWelcomeBackDay = "";
...
try {
  ...
  if (!Array.isArray(db0.ledger)) db0.ledger = [];
...
function isWelcomeBackDue(u) {
  // Send welcome-back only if user has been inactive for INACTIVE_WELCOME_DAYS+ days,
  // and we haven't sent it yet today.
  if (!u) return false;
  const lastSeen = Number(u.lastSeen || 0);
  if (!lastSeen) return true; // first time user -> treat as needing a welcome
  const ms = Number(INACTIVE_WELCOME_DAYS || 4) * 24 * 60 * 60 * 1000;
  if (Date.now() - lastSeen < ms) return false;
  const day = todayKey();
  return String(u.lastWelcomeBackDay || "") !== String(day);
}

function markWelcomeBackSent(chatId) {
  const u = getUser(chatId);
  u.lastWelcomeBackDay = todayKey();
  saveDB();
}
...
function addPoints(chatId, pts, meta = {}) {
  const u = getUser(chatId);
...
function deductPoints(chatId, pts, meta = {}) {
  const u = getUser(chatId);
  u.points = Number(u.points || 0) - Number(pts || 0);
...
function toPtsDisplay(raw) {
  const n = Number(raw || 0);
  if (!Number.isFinite(n)) return 0;
  // if integer and large enough, likely centi-points
  if (Number.isInteger(n) && Math.abs(n) >= 100) return n / 100;
  return n;
}

function fmtPts(raw) {
  return toPtsDisplay(raw).toFixed(2);
}

const HUGE_PTS_THRESHOLD = 50;
...
function addLedgerEvent(evt) {
  ...
  const entry = {
    ...
    chatId: evt?.chatId !== undefined ? String(evt.chatId) : undefined,
    points: evt?.points,
    amountPts: evt?.amountPts,
    amountKsh: evt?.amountKsh,
    phone: evt?.phone,
    ref: evt?.ref,
    pkg: evt?.pkg,
    tag: evt?.tag ? String(evt.tag) : undefined,
    actorId: evt?.actorId ? String(evt.actorId) : undefined,
    reqId: evt?.reqId ? String(evt.reqId) : undefined,
    note: evt?.note ? String(evt.note) : undefined
  };

  for (const k of Object.keys(evt || {})) {
    if (!(k in entry)) entry[k] = evt[k];
  }
  ...
  // Huge points alert to admin (>= 50 pts)
  try {
    const raw = (Number.isFinite(Number(entry.points)) ? Number(entry.points) :
                (Number.isFinite(Number(entry.amountPts)) ? Number(entry.amountPts) : null));
    if (raw !== null) {
      const absPts = Math.abs(toPtsDisplay(raw));
      if (absPts >= HUGE_PTS_THRESHOLD) {
        notifyAdmin(
          `🚨 *Huge Points Alert*\n` +
          `Type: *${mdEscape(String(entry.type || "UNKNOWN"))}*\n` +
          (entry.tag ? `Tag: *${mdEscape(entry.tag)}*\n` : "") +
          (entry.chatId ? `User: \`${mdEscape(entry.chatId)}\`\n` : "") +
          `Pts: *${mdEscape(fmtPts(raw))}*\n` +
          (entry.reqId ? `ReqId: \`${mdEscape(entry.reqId)}\`\n` : "") +
          (entry.actorId ? `By: \`${mdEscape(entry.actorId)}\`\n` : "") +
          `Time: \`${mdEscape(kenyaDateTimeFromTs(entry.ts))}\``
        );
      }
    }
  } catch (_) {}
...
function buildStatementLines(targetChatId, limit = 20) {
  ensureLedger();
  const rows = (db.ledger || [])
    .filter(e => String(e.chatId || "") === String(targetChatId))
    .slice(-5000)
    .reverse()
  ...
  function fmtMoney(e) {
    if (Number.isFinite(Number(e.amountKsh))) return `Ksh ${Number(e.amountKsh).toFixed(2)}`;
    return "";
  }

  function lineFor(e) {
    const t = String(e.type || "UNKNOWN");
    const when = kenyaDateTimeFromTs(e.ts);

    const ptsRaw = Number.isFinite(Number(e.points)) ? e.points :
                   (Number.isFinite(Number(e.amountPts)) ? e.amountPts : null);
    const ptsTxt = ptsRaw === null ? "" : `${fmtPts(ptsRaw)}pts`;

    const tag = e.tag ? ` from ${e.tag}` : " from unknown";
    const by = e.actorId ? ` by id ${e.actorId}` : "";
    const rid = e.reqId ? ` id ${e.reqId}` : "";

    if (t === "ORDER_SUCCESS") {
      const ksh = fmtMoney(e) || "Ksh ?";
      const item = e.pkg ? ` (${e.pkg})` : (e.ref ? ` (${e.ref})` : "");
      return `+ ${ksh}${item}${tag}${rid}${by} on ${when}`;
    }

    if (t === "WITHDRAW_APPROVED") {
      const ksh = fmtMoney(e) || "Ksh ?";
      return `- ${ksh} withdraw (${ptsTxt})${rid}${by} on ${when}`;
    }

    if (t === "WITHDRAW_DECLINED") {
      return `ℹ️ withdraw declined (${ptsTxt})${rid}${by} on ${when}`;
    }

    if (t === "REDEEM_ACCEPTED") {
      return `- ${ptsTxt} redeem${tag}${rid}${by} on ${when}`;
    }

    if (t === "REDEEM_DECLINED") {
      return `ℹ️ redeem declined (${ptsTxt})${rid}${by} on ${when}`;
    }

    if (t === "POINTS_ADD") {
      return `+ ${ptsTxt}${tag}${rid}${by} on ${when}`;
    }
    ...
    const ksh = fmtMoney(e);
    const extra = [ptsTxt, ksh].filter(Boolean).join(" | ");
    return `${t}${rid}${by} on ${when}${extra ? ` — ${extra}` : ""}`;
  }

  return rows.map(lineFor);
}


/**
 * ===================== USER DASHBOARD =====================
 * Shows: points, total purchases/spent, referrals, and recent statement lines.
 */
function buildUserDashboardText(chatId) {
  const u = getUser(chatId);
  const points = getPoints(chatId);
  const spent = Number(u.totalSpentKsh || 0);
  const purchases = Number(u.totalPurchases || 0);
  const referrals = Number(u.referralSuccessCount || 0);

  const recent = buildStatementLines(chatId, 8) || [];
  const recentTxt = recent.length ? recent.join("\n") : "• (no transactions yet)";
  ...
  return (
    ...
    `🧾 Recent activity:\n${recentTxt}\n\n` +
    `Tip: use /statement to view more.`
  );
}
 ...
 * ===================== PROMO CAMPAIGNS =====================
 */
function promoCodeNormalize(code) {
...
function ensurePromos() {
  ...
  if (!db.promoRedemptions || typeof db.promoRedemptions !== "object") db.promoRedemptions = {};
  if (!db.settings || typeof db.settings !== "object") db.settings = {};
  if (typeof db.settings.aiSupportEnabled !== "boolean") db.settings.aiSupportEnabled = true;
  saveDB();
}
...
function promoRemainingUses(p) {
  const max = Number(p?.maxUses || 0);
  const used = Number(p?.uses || 0);
  if (!max) return null; // unlimited
  return Math.max(0, max - used);
}

function userAlreadyRedeemedPromo(chatId, code) {
  ensurePromos();
  const c = promoCodeNormalize(code);
  const map = db.promoRedemptions[String(chatId)] || {};
  return !!map[c];
}

function markUserRedeemedPromo(chatId, code) {
  ensurePromos();
  const c = promoCodeNormalize(code);
  db.promoRedemptions[String(chatId)] = db.promoRedemptions[String(chatId)] || {};
  db.promoRedemptions[String(chatId)][c] = Date.now();
  saveDB();
}

function redeemPromo(chatId, rawCode) {
  ensurePromos();
  const code = promoCodeNormalize(rawCode);
  if (!code) return { ok: false, msg: "❌ Please provide a promo code." };

  const p = db.promos[code];
  if (!p || p.enabled === false) return { ok: false, msg: "❌ Invalid promo code." };
  if (promoIsExpired(p)) return { ok: false, msg: "❌ This promo code has expired." };

  const rem = promoRemainingUses(p);
  if (rem !== null && rem <= 0) return { ok: false, msg: "❌ Promo code limit reached." };

  if (userAlreadyRedeemedPromo(chatId, code)) return { ok: false, msg: "ℹ️ You already used this promo code." };

  const pts = Number(p.points || 0);
  if (!Number.isFinite(pts) || pts <= 0) return { ok: false, msg: "❌ Promo misconfigured. Contact support." };

  // apply
  p.uses = Number(p.uses || 0) + 1;
  db.promos[code] = p;
  markUserRedeemedPromo(chatId, code);

  // ledger + points (avoid double-ledger: use base addPoints if wrapper exists)
  try {
    addLedgerEvent({
      type: "PROMO_REDEEM",
      chatId: String(chatId),
  ...
  if (typeof __originalAddPoints === "function") {
    __originalAddPoints(chatId, pts);
  } else {
    addPoints(chatId, pts);
  }
  ...
  try {
    notifyAdmin(
      `🎁 *Promo Redeemed*\n` +
      `Code: *${mdEscape(code)}*\n` +
      `User: \`${mdEscape(chatId)}\`\n` +
      `Pts: *${mdEscape(pts.toFixed(2))}*\n` +
      `Time: \`${mdEscape(kenyaDateTime())}\``
    );
  } catch (_) {}

  return { ok: true, msg: `✅ Promo applied!\n\nCode: ${code}\nReward: +${pts.toFixed(2)} pts\nBalance: ${getPoints(chatId).toFixed(2)} pts` };
}

/**
 * ===================== "AI" SUPPORT AUTO-REPLY (RULE-BASED) =====================
 */
function aiSupportEnabled() {
  try {
    db = repairDB(db);
    return db.settings?.aiSupportEnabled !== false;
  } catch (_) {
    return true;
  }
}

function aiSupportReply(textRaw) {
  const t = String(textRaw || "").toLowerCase().trim();
  if (!t) return null;

  if (/(agent|human|operator|admin|support team|talk to)/i.test(t)) return null;

  const faq = [
    { re: /(withdraw|cashout|payout)/i, reply: `💸 *Withdraw Points*\n\n• Tap *💸 Withdraw Points*\n• Choose amount\n• Enter M-PESA number\n\nNote: Withdraw is approved by admin.` },
    { re: /(redeem|free sms|250mb|min)/i, reply: `🎁 *Redeem Points*\n\n• Tap *🎁 Redeem Points*\n• Choose item\n• Follow prompts\n\nIf it's locked, you need more purchases first.` },
    { re: /(stk|mpesa|payment|payhero|prompt)/i, reply: `📲 *STK/Payment Help*\n\n• Ensure M-PESA has enough balance\n• Use the correct phone number\n• If STK didn't arrive, tap *🔄 Resend STK*\n\nIf still failing, send screenshot here.` },
    { re: /(referral|invite|link)/i, reply: `🔗 *Referral*\n\nTap *🔗 My Referral* to get your link.\nYou earn points when your invite makes a successful purchase.` },
    { re: /(hours|open|working time|time)/i, reply: `⏰ Support is available daily. If urgent call: *${HELP_PHONE}*.` },
  ];

  for (const f of faq) {
    if (f.re.test(t)) return f.reply;
  }

  if (t.length >= 8 && /[?]/.test(t)) {
    return `✅ Got it.\n\nPlease share:\n• Your phone number (masked is ok)\n• The package you tried\n• Screenshot of the error/STK prompt\n\nOr call: *${HELP_PHONE}*`;
  }
  ...
  try {
    ...
    try {
      addLedgerEvent({
        ...
        chatId: String(uid),
        amountPts: pts,
        amountKsh: Number(kes || 0),
        phone,
        tag: "withdraw",
        actorId: adminChatId ? String(adminChatId) : undefined,
        reqId: req.reqId || undefined
      });
    } catch (_) {}
...
async function __withdrawDecline(targetId, adminChatId, adminMsgId) {
  ...
  if (Number.isFinite(amt) && amt > 0) addPoints(Number(targetId), amt, { tag: "withdraw refund", actorId: adminChatId ? String(adminChatId) : undefined, reqId: req.reqId || undefined });
...
// Phone override approvals (daily phone limit)
if (data.startsWith("phone_")) {
  ...
  if (action === "phone_approve") {
    req.status = "approved";
    markPhoneUsage(req.chatId, req.phone);
    delete db.pendingPhoneOverrides[overrideId];
    saveDB();

    await bot.sendMessage(chatId, "✅ Approved.").catch(() => {});
    await bot.sendMessage(req.chatId, "✅ Approved! You can proceed now.").catch(() => {});
  } else if (action === "phone_decline") {
    req.status = "declined";
    delete db.pendingPhoneOverrides[overrideId];
    saveDB();

    await bot.sendMessage(chatId, "❌ Declined.").catch(() => {});
    await bot.sendMessage(req.chatId, "❌ Declined. Please use a different phone number or try later.").catch(() => {});
  }
...
// Withdraw approvals via inline buttons
if (action === "withdraw_accept" || action === "withdraw_decline") {
  if (String(req?.type || "") !== "withdraw") {
    await bot.sendMessage(chatId, "❌ This is not a withdraw request.").catch(() => {});
    await bot.answerCallbackQuery(q.id).catch(() => {});
    return;
  }

  if (action === "withdraw_accept") {
    await __withdrawApprove(targetChatId, chatId, q.message?.message_id);
    await bot.answerCallbackQuery(q.id).catch(() => {});
    return;
  }

  if (action === "withdraw_decline") {
    await __withdrawDecline(targetChatId, chatId, q.message?.message_id);
    await bot.answerCallbackQuery(q.id).catch(() => {});
    return;
  }
}
    ...
    if (action === "redeem_accept") {
      ...
      try {
        addLedgerEvent({
          type: "REDEEM_ACCEPTED",
          chatId: String(targetChatId),
          amountPts: cost,
          tag: "redeem",
          actorId: chatId ? String(chatId) : undefined,
          reqId: reqId,
          note: req.itemLabel
        });
      } catch (_) {}
  ...
  const sent = await bot.sendMessage(
    ...
    if (action === "redeem_decline") {
      ...
      if (Number.isFinite(cost) && cost > 0) addPoints(targetChatId, cost, { tag: "redeem refund", actorId: chatId ? String(chatId) : undefined, reqId });
      clearPendingRedeem(targetChatId);
      clearPendingAction(targetChatId);
      setRedeemCooldown(targetChatId, 5 * 60 * 1000);
      try {
        addLedgerEvent({
          type: "REDEEM_DECLINED",
          chatId: String(targetChatId),
          amountPts: cost,
          tag: "redeem",
          actorId: chatId ? String(chatId) : undefined,
...
async function renderSessionSnapshot(chatId, snap) {
  ...
  if (s.step === "package") {
    if (!s.category) return sendTracked(chatId, "✅ Choose a category:", categoriesKeyboard());
  ...
  if (s.step === "confirm") {
    ...
    if (!pkg) return sendTracked(chatId, `👇 ${s.category} packages:`, packagesKeyboard(s.category));
...
async function sendTracked(chatId, text, options = {}) {
  ...
  const isWelcomeMessage = !!(text && (text.startsWith("👋 Welcome") || text.startsWith("👋 Hello") || text.startsWith("🎉 Welcome")));
...
const keyboard = [
  ["🛒 Buy Offers", "📊 Dashboard"],
  ["💸 Withdraw Points", "🎁 Promo"],
  ["🔗 My Referral", "ℹ️ Help"]
];
...
// ===================== TEXTS =====================
function returningWelcomeText(name, isNew = false) {
  return (
...
function referralWelcomeText(name, inviterId) {
  return (
    `🎉 Welcome ${name || "there"}!\n\n` +
    `You joined using a referral link.\n` +
    `Referrer ID: *${mdEscape(String(inviterId || ""))}*\n\n` +
    `✅ Make a successful purchase and you\'ll start earning points automatically.`
  );
}
...
function maybeRewardInviterOnSuccessPurchase(buyerChatId, amountKsh, when) {
  ...
  addPoints(inviterId, bonus, { tag: "referral bonus", actorId: String(buyerChatId), note: `Purchase Ksh ${amt}` });
...
` +
    ...
    try {
      addLedgerEvent({
        ...
        phone: pending?.phone254 ? String(pending.phone254) : undefined,
        ref: String(referenceNumber || externalRef || ""),
...
bot.on("message", async (msg) => {
  ...
  // ensure user exists + lastSeen (capture BEFORE we update lastSeen)
  const u = getUser(chatId);
  const wasFirstTime = !Number(u.lastSeen || 0);
  const welcomeBackDue = !isAdmin(chatId) && isWelcomeBackDue(u);
  ...
  // ✅ Returning-user welcome (after inactivity)
  if (welcomeBackDue && !isStartCmd) {
    markWelcomeBackSent(chatId);
    await sendTracked(chatId, returningWelcomeText(msg.from?.first_name, wasFirstTime), {
      parse_mode: "Markdown",
      ...mainMenuKeyboard(chatId),
    });
  }
...
• Cycle resets in: ${st.daysLeft} day(s)`,
      ...
      deductPoints(chatId, amount, { tag: "redeem request", actorId: String(chatId), reqId, note: item.label });
      const u = getUser(chatId);
...
Waiting for admin approval…`,
  ...
  if (text.startsWith("/")) {
    ...
    if (startPayload !== null) {
      ...
      // ✅ /start: NO default welcome unless (a) referral link used, or (b) first time / inactive return
      if (startPayload && startPayload.startsWith("ref_")) {
        // Referral welcome
        const inviter = String(startPayload.slice(4).trim() || "");
        await sendTracked(chatId, referralWelcomeText(msg.from?.first_name, inviter), {
          parse_mode: "Markdown",
          ...mainMenuKeyboard(chatId),
        });
      } else if (welcomeBackDue || wasFirstTime) {
        markWelcomeBackSent(chatId);
        await sendTracked(chatId, returningWelcomeText(msg.from?.first_name, wasFirstTime), {
          parse_mode: "Markdown",
          ...mainMenuKeyboard(chatId),
...
if (/^\/dashboard$/i.test(text) || text === "📊 Dashboard") {
  const dash = buildUserDashboardText(chatId);
  return sendTracked(chatId, dash, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
}

// 🎁 Promo (user) - start promo entry mode
if (!isAdmin(chatId) && (text === "🎁 Promo" || /^\/promo$/i.test(text))) {
  setPendingAction(chatId, { kind: "promo", data: {} });
  return sendTracked(chatId, "🎁 Send your promo code (e.g. ABC123) or /cancel to stop.", { ...mainMenuKeyboard(chatId) });
}

// /promo CODE (one-line redeem)
const promoInline = text.match(/^\/promo\s+(.+)$/i);
if (promoInline) {
  const code = promoInline[1];
  const r = redeemPromo(chatId, code);
  return sendTracked(chatId, r.msg, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
}

// /statement [N] -> show your last N entries (default 20, max 50)
// Admin: /statement <chatId> [N]
if (/^\/statement\b/i.test(text)) {
  const parts = text.split(/\s+/).filter(Boolean);
  let target = String(chatId);
  let limit = 20;

  if (parts.length >= 2 && /^\d+$/.test(parts[1]) && isAdmin(chatId)) {
    target = String(parts[1]);
    if (parts[2] && /^\d+$/.test(parts[2])) limit = Number(parts[2]);
  ...
  const lines = buildStatementLines(target, limit);
  if (!lines) return sendTracked(chatId, "ℹ️ No statement entries yet.", { ...mainMenuKeyboard(chatId) });

  const header =
    `📄 Statement for ${target}\n` +
    `Showing last ${lines.length} entr${lines.length === 1 ? "y" : "ies"}:\n\n`;

  return sendTracked(chatId, header + lines.join("\n"), { ...mainMenuKeyboard(chatId) });
}
...
// /ai on|off  (toggle AI auto-reply for support)
const aiToggle = text.match(/^\/ai\s+(on|off)$/i);
if (aiToggle) {
  ensurePromos();
  db.settings.aiSupportEnabled = aiToggle[1].toLowerCase() === "on";
  saveDB();
  return sendTracked(chatId, `✅ AI support is now *${db.settings.aiSupportEnabled ? "ON" : "OFF"}*`, { parse_mode: "Markdown" });
}

// PROMO MANAGEMENT (admin)
// /promo_create CODE PTS [maxUses] [expiresDays]
const pc = text.match(/^\/promo_create\s+(\S+)\s+([0-9]+(?:\.[0-9]+)?)\s*(\d+)?\s*(\d+)?$/i);
if (pc) {
  ensurePromos();
  const code = promoCodeNormalize(pc[1]);
  const pts = Number(pc[2]);
  const maxUses = pc[3] ? Number(pc[3]) : 0; // 0 = unlimited
  const expiresDays = pc[4] ? Number(pc[4]) : 0; // 0 = never
  if (!code || !Number.isFinite(pts) || pts <= 0) return sendTracked(chatId, "❌ Usage: /promo_create CODE PTS [maxUses] [expiresDays]");
  ...
  db.promos[code] = {
    ...
    uses: Number(db.promos?.[code]?.uses || 0),
    expiresAt,
    enabled: true,
    createdAt: Date.now()
  };
  ...
  return sendTracked(
    chatId,
    `✅ Promo created\nCode: ${code}\nReward: +${pts.toFixed(2)} pts\nMaxUses: ${maxUses > 0 ? maxUses : "unlimited"}\nExpires: ${expiresDays > 0 ? expiresDays + " day(s)" : "never"}`
  );
}

// /promo_disable CODE
const pd = text.match(/^\/promo_disable\s+(\S+)$/i);
if (pd) {
  ensurePromos();
  const code = promoCodeNormalize(pd[1]);
  if (!db.promos[code]) return sendTracked(chatId, "❌ Promo not found.");
  db.promos[code].enabled = false;
  saveDB();
  return sendTracked(chatId, `✅ Promo disabled: ${code}`);
}

// /promo_list
if (/^\/promo_list$/i.test(text)) {
  ensurePromos();
  const rows = Object.values(db.promos || {})
    .map(p => ({
      code: p.code,
      ...
      expired: promoIsExpired(p)
    }))
    .sort((a,b)=> (a.enabled===b.enabled?0:(a.enabled?-1:1)) || (a.code > b.code ? 1 : -1))
    .slice(0, 50);

  if (!rows.length) return sendTracked(chatId, "ℹ️ No promos yet.");
  const lines = rows.map(r =>
    `• ${r.code} — +${r.pts.toFixed(2)} pts — uses ${r.uses}${r.maxUses?"/"+r.maxUses:""} — ${r.enabled ? (r.expired ? "EXPIRED" : "ON") : "OFF"}`
  );
  return sendTracked(chatId, `🎁 Promos\n\n${lines.join("\n")}`);
}
...
// (withdraw helpers are defined above)
...
// AI auto-reply (rule-based). If it matches, respond immediately and do not forward.
if (aiSupportEnabled()) {
  const ai = aiSupportReply(text);
  if (ai) {
    return sendTracked(chatId, ai + "\n\nIf you still need an agent, reply: *agent*", {
      parse_mode: "Markdown",
      ...mainMenuKeyboard(chatId),
    });
  }
...
// Promo entry mode
if (act && !isAdmin(chatId) && act.kind === "promo") {
  if (text === "⬅ Back") {
    clearPendingAction(chatId);
  ...
  const r = redeemPromo(chatId, text);
  clearPendingAction(chatId);
  return sendTracked(chatId, r.msg, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
}
    ...
    if (act.step === "confirm") {
      if (text === "✅ Confirm") {
        ...
        if (savedPhone) {
          ...
          deductPoints(chatId, cost, { tag: "redeem request", actorId: String(chatId), reqId, note: item.label });
...
Remaining: Ksh ${remaining}`, { ...mainMenuKeyboard(chatId) });
  ...
  deductPoints(chatId, amount, { tag: "withdraw request", actorId: String(chatId), reqId });
