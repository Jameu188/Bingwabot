...
//    - SMS Offers: added 4 new SMS packages (same SMS channel)   [NOTE: you didn't provide the 4 new ones; add them below if needed]
//    - Minutes: updated + added new minutes packages (same Minutes channel) [NOTE: you didn't provide new ones; add them below if needed]
...
// ✅ Max reward per day is 150 points (caps ALL point additions: referral + purchase points)   [PER USER]
// ✅ Transaction history: last 15 transactions (success/failed) with date/time, package, price, status
//
// IMPORTANT CHANGE (per your last request):
// ✅ REMOVED LOGIN/REGISTER COMPLETELY (NO AUTH, NO ACCOUNTS). EVERYTHING WORKS AS BEFORE.
//
// NOTE:
...
// Rewards caps (PER USER)
const REWARDS = {
  MAX_PER_DAY: 150, // ✅ max points per day per user
};
...
//   "pendingPayments": { "<externalRef>": { chatId, category, pkgLabel, phone254, price, buyerName, createdAt } },
...
function repairDB(obj) {
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (x.transactions.length > 60) x.transactions = x.transactions.slice(0, 60);
  }
...
function getUser(chatId) {
  ...
  if (!Array.isArray(db.users[cid].transactions)) db.users[cid].transactions = [];
  return db.users[cid];
}
...
// ===================== POINTS (PER USER) =====================
function addUserPointsCapped(chatId, pts, whenDayKey = todayKey()) {
  const u = getUser(chatId);
  ...
  u.dailyRewards = u.dailyRewards || {};
  const earnedToday = Number(u.dailyRewards[day] || 0);
  ...
  u.points = Number(u.points || 0) + toAdd;
  u.dailyRewards[day] = earnedToday + toAdd;
...
function pushUserTransaction(chatId, tx) {
  const u = getUser(chatId);
  if (!Array.isArray(u.transactions)) u.transactions = [];
  u.transactions.unshift({ at: Date.now(), ...tx });
  if (u.transactions.length > 60) u.transactions = u.transactions.slice(0, 60);
  saveDB();
}

function getLastUserTransactions(chatId, limit = 15) {
  const u = getUser(chatId);
  const arr = Array.isArray(u.transactions) ? u.transactions : [];
...
// ✅ Earn points only on SUCCESS purchase (PER USER)
function awardSuccessPurchasePointsToUser(chatId, pending, whenDayKey) {
  if (!pending || !pending.pkgLabel) return;
  const label = String(pending.pkgLabel).toLowerCase();

  if (label.includes("20 sms")) addUserPointsCapped(chatId, 5, whenDayKey);
  if (label.includes("250mb")) addUserPointsCapped(chatId, 20, whenDayKey);
  if (label.includes("20 min")) addUserPointsCapped(chatId, 20, whenDayKey);
}
...
const PACKAGES = {
  ...
  "SMS Offers": [
    ...
    // ➕ Add your 4 new SMS packages here if needed:
    // { label: "Ksh ?? • ...", price: ?? },
  ],
  Minutes: [
    ...
    // ➕ Add your updated/new Minutes packages here if needed:
    // { label: "Ksh ?? • ...", price: ?? },
  ],
...
// mask 07 phone like 0723xxx435
function mask07(phone07) {
  const p = String(phone07 || "").replace(/\s+/g, "");
  if (!/^0[71]\d{8}$/.test(p)) return p || "";
...
async function notifyAdminReferralReward({ buyerName, amountKsh, buyerPhone07, bonusPts, when }) {
  await notifyAdmin(
    `🎯 *Referral Reward*\n` +
      `Buyer: *${mdEscape(String(buyerName || "Unknown"))}*\n` +
      `Purchase: *Ksh ${mdEscape(Number(amountKsh || 0))}*\n` +
      `Phone number: *${mdEscape(mask07(buyerPhone07 || ""))}*\n` +
      `Bonus: *${mdEscape(Number(bonusPts || 0).toFixed(2))} pts*\n` +
      `Time: \`${mdEscape(String(when || kenyaDateTime()))}\``
  );
}

// credit inviter points on success (2% between min..max), capped daily
async function rewardInviterOnSuccess(buyerChatId, buyerName, buyerPhone07, amountKsh, when) {
  ...
  const r = addUserPointsCapped(Number(inviterChatId), bonus, todayKey());

  // admin format message (as you requested)
  await notifyAdminReferralReward({
    buyerName,
    amountKsh: amt,
    buyerPhone07,
    bonusPts: r.added,
  ...
  // OPTIONAL: you can also notify inviter user (keep if you want)
  // await sendTracked(Number(inviterChatId), `🎯 Referral Reward\nBuyer: ${buyerName}\nPurchase: Ksh ${amt}\nBonus: ${r.added.toFixed(2)} pts\nTime: ${when}`, mainMenuKeyboard());
}
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    if (pending) {
      ...
      // purchase points
      awardSuccessPurchasePointsToUser(chatId, pending, dayKey);
      ...
      await rewardInviterOnSuccess(chatId, pending.buyerName || "", buyerPhone07, pending.price, when);
...
bot.on("message", async (msg) => {
  ...
  if (text.startsWith("/")) {
    ...
    if (startPayload !== null) {
      ...
      // keep welcome + packages overview always (no login)
      const u = getUser(chatId);
      if (!u.onboarded) {
        u.onboarded = true;
        saveDB();
        await sendTracked(chatId, welcomeTextAgent(), { ...mainMenuKeyboard() });
      }
  ...
  if (text === "⭐ Points Balance") {
    const pts = getUserPoints(chatId);
    return sendTracked(chatId, `⭐ Your points: *${pts.toFixed(2)} pts*\n\nDaily cap: ${REWARDS.MAX_PER_DAY} pts/day`, {
      parse_mode: "Markdown",
      ...mainMenuKeyboard(),
    });
  }
  ...
  if (text === "📜 History") {
    const list = getLastUserTransactions(chatId, 15);
    if (!list.length) return sendTracked(chatId, "📜 No transactions yet.", { ...mainMenuKeyboard() });
  ...
  if (text === "🔗 My Referral") {
    const link = BOT_USERNAME ? `https://t.me/${BOT_USERNAME}?start=ref_${chatId}` : `Use: /start ref_${chatId}`;

    return sendTracked(
      chatId,
      `🔗 Your referral link:\n${link}\n\nRules:\n• Bonus = 2% of purchase\n• Only for purchases between Ksh ${REFERRAL.MIN_KSH} and Ksh ${REFERRAL.MAX_KSH}\n• Daily cap: ${REWARDS.MAX_PER_DAY} pts`,
      { ...mainMenuKeyboard() }
    );
  }
  ...
  if (text === "1️⃣ 20 free SMS (5 pts)" || text === "2️⃣ 250MB free (20 pts)") {
    ...
    const bal = getUserPoints(chatId);
    if (bal < item.cost) {
  ...
  if (act && !isAdmin(chatId)) {
    ...
    if (act.kind === "redeem") {
      ...
      if (act.step === "number") {
        ...
        // deduct from user
        const u = getUser(chatId);
        u.points = Number(u.points || 0) - Number(cost || 0);
