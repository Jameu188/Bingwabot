...
function getUser(chatId) {
  ...
  // Ensure Business settings exist (backward compatible)
  if (db.users[cid].stkAutoRetry === undefined) db.users[cid].stkAutoRetry = false;
  if (db.users[cid].stkRetryCount === undefined) db.users[cid].stkRetryCount = 2;
  if (db.users[cid].stkRetryDelayMs === undefined) db.users[cid].stkRetryDelayMs = 30000;
  ...
  if (db.users[cid].repeatLastButton === undefined) db.users[cid].repeatLastButton = false;
...
function isBusinessActive(chatId) {
  ...
  if (Date.now() > u.subscriptionExpiry) {
    // Auto-revert on expiry + unmark all Business settings
    u.accountType = "normal";
    u.subscriptionStart = 0;
    u.subscriptionExpiry = 0;

    // Unmark / reset Business-only settings
    u.fastPurchaseEnabled = false;
    u.stkAutoRetry = false;
    u.stkRetryCount = 2;
    u.stkRetryDelayMs = 30000;
    u.skipConfirmation = false;
    u.rememberLastCategory = true;
    u.repeatLastButton = false;
    u.lastCategory = "";

    // Auto Delete back to default (24h)
...
// ===================== SETTINGS (HELP -> COMMANDS & SETTINGS) =====================
if (data === "set:fp") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  if (!isBusinessActive(chatId)) {
    return sendTracked(chatId, "🔒 Fast Purchase is Business only. Type /sell to upgrade.", { ...helpCmdsKeyboard(chatId) });
  }
  const u = getUser(chatId);
  u.fastPurchaseEnabled = !u.fastPurchaseEnabled;
  saveDB();
  await sendHelpAnswer(chatId, "cmds", { messageId: _helpMsgId });
  return;
}

if (data === "set:stkretry") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  if (!isBusinessActive(chatId)) {
    return sendTracked(chatId, "🔒 Auto Retry STK is Business only. Type /sell to upgrade.", { ...helpCmdsKeyboard(chatId) });
  }
  const u = getUser(chatId);
...
if (data === "set:skip") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  if (!isBusinessActive(chatId)) {
    return sendTracked(chatId, "🔒 Skip confirmation is Business only. Type /sell to upgrade.", { ...helpCmdsKeyboard(chatId) });
  }
  const u = getUser(chatId);
...
if (data === "set:autodel") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  if (!isBusinessActive(chatId)) {
    return sendTracked(chatId, "🔒 Auto Delete is Business only. Type /sell to upgrade.", { ...helpCmdsKeyboard(chatId) });
  }
  // cycle: 24h (default) -> 10m -> 1h -> 24h
  const u = getUser(chatId);
  const cur = Number(u.autoDeleteMs || 24 * 60 * 60 * 1000);
  const v10 = 10 * 60 * 1000;
  const v1h = 60 * 60 * 1000;
  const v24 = 24 * 60 * 60 * 1000;
  u.autoDeleteMs = (cur === v24) ? v10 : (cur === v10) ? v1h : v24;
  saveDB();
  await sendHelpAnswer(chatId, "cmds", { messageId: _helpMsgId });
  return;
}
...
function helpCmdsKeyboard(chatId) {
  const u = getUser(chatId);
  const biz = isBusinessActive(chatId);

  // If not business, show locked (unmarked) and prevent toggles in handlers
  const fp = biz && !!u.fastPurchaseEnabled;
  const ad = biz && (u.autoDeleteMs && u.autoDeleteMs !== 24 * 60 * 60 * 1000);
  const stk = biz && !!u.stkAutoRetry;
  const skip = biz && !!u.skipConfirmation;

  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: `${markBox(fp)} Fast Purchase`, callback_data: "set:fp" }],
        [{ text: `${markBox(ad)} Auto Delete`, callback_data: "set:autodel" }],
        [{ text: `${markBox(stk)} Auto Retry STK`, callback_data: "set:stkretry" }],
        [{ text: `${markBox(skip)} Skip Confirmation`, callback_data: "set:skip" }],
        [{ text: "⬅ Back to Help", callback_data: "help:menu" }, { text: "🏠 Main Menu", callback_data: "help:home" }],
      ],
    },
  };
}

function helpAnswerKeyboardForKey(chatId, key) {
  if (key === "cmds") return helpCmdsKeyboard(chatId);
  return helpAnswerKeyboard();
}
...
async function showUserProfile(chatId) {
  ...
  const joined = u.joinedAt ? kenyaDateTimeFromTs(u.joinedAt) : "N/A";
...
async function sendHelpAnswer(chatId, key, ctx = {}) {
  ...
  const opts = { parse_mode: "Markdown", ...helpAnswerKeyboard() };
...
function addPendingPayment(externalRef, payload) {
  db = repairDB(db);
  db.pendingPayments[String(externalRef)] = { ...payload, createdAt: Date.now(), autoRetryUsed: 0, nextRetryAt: 0 };
  saveDB();
}
...
function updatePendingPayment(externalRef, patch) {
  db = repairDB(db);
  const key = String(externalRef);
  const cur = db.pendingPayments[key];
  if (!cur || typeof cur !== "object") return null;
  db.pendingPayments[key] = { ...cur, ...patch };
  saveDB();
  return db.pendingPayments[key];
}

// Auto STK retry scheduler (Business-only). Retries use SAME externalRef within STK_RETRY_EXPIRE_MS window.
function scheduleAutoStkRetry(externalRef, reason = "") {
  try {
    const pending = getPendingPayment(externalRef);
    if (!pending) return false;

    const chatId = Number(pending.chatId || 0);
    if (!chatId) return false;

    if (typeof isBusinessActive === "function" && !isBusinessActive(chatId)) return false;

    const u = getUser(chatId);
    if (!u || !u.stkAutoRetry) return false;

    const max = Math.max(0, Number(u.stkRetryCount || 0));
    const used = Math.max(0, Number(pending.autoRetryUsed || 0));
    if (max <= 0 || used >= max) return false;

    const ageMs = Date.now() - Number(pending.createdAt || 0);
    if (ageMs > STK_RETRY_EXPIRE_MS) return false;

    const delayMs = Math.max(30 * 1000, Number(u.stkRetryDelayMs || 30 * 1000)); // enforce >=30s
    updatePendingPayment(externalRef, { nextRetryAt: Date.now() + delayMs });
...
// ===================== AUTO STK RETRY WORKER (Business-only) =====================
// Scans pending payments and auto-resends STK after configured delay (default 30s) when nextRetryAt is due.
setInterval(async () => {
  try {
    ...
    for (const [ref, p] of Object.entries(db.pendingPayments || {})) {
      ...
      const nextAt = Number(p.nextRetryAt || 0);
      if (!nextAt || nextAt > now) continue;

      const chatId = Number(p.chatId || 0);
      if (!chatId) {
        db.pendingPayments[ref].nextRetryAt = 0;
        continue;
      }

      // Business gate + toggle
      if (typeof isBusinessActive === "function" && !isBusinessActive(chatId)) {
        db.pendingPayments[ref].nextRetryAt = 0;
        db.pendingPayments[ref].autoRetryUsed = 0;
        continue;
      }
      ...
      const max = Math.max(0, Number(u.stkRetryCount || 0));
      const used = Math.max(0, Number(p.autoRetryUsed || 0));
      if (max <= 0 || used >= max) {
        db.pendingPayments[ref].nextRetryAt = 0;
        continue;
      }
      ...
      // consume this scheduled retry
      db.pendingPayments[ref].autoRetryUsed = used + 1;
      db.pendingPayments[ref].nextRetryAt = 0;
      saveDB();
      ...
      try {
        ...
        bot.sendMessage(
          chatId,
          `🔁 Auto Retry STK sent (${used + 1}/${max}). Please check your phone.`,
          { ...mainMenuKeyboard(chatId) }
        ).catch(() => {});
      } catch (err) {
        // if still delayed, schedule next retry if available
        if (isStkDelayedError(err)) {
          const after = Math.max(30 * 1000, Number(u.stkRetryDelayMs || 30 * 1000));
          const cur = getPendingPayment(ref);
          const curUsed = Math.max(0, Number(cur?.autoRetryUsed || used + 1));
          if (curUsed < max) {
            updatePendingPayment(ref, { nextRetryAt: Date.now() + after });
          }
        }
      }
    }
...
function openCategoryQuick(chatId, category) {
  ...
  s.pkgKey = null;
  sessions.set(chatId, s);
  const _auto = scheduleAutoStkRetry(ref, "delayed");
...
if (!isSuccess) {
  ...
  const out =
    ...
    (details ? `${details}\n` : "") +
    `Your package could not be processed.` + (autoRetryScheduled ? `\n\n🔁 Auto Retry is ON — I will resend STK in 30 seconds.` : ` Please try again or contact customer support.`) + `\n\nHelp: ${HELP_PHONE}`;
...
Error: \`${mdEscape(
            ...
            const _msg = `⚠️ *STK Request Delayed*
...
` +
                (_auto ? `
🔁 Auto Retry is ON — I will resend STK in 30 seconds.` : ``);
