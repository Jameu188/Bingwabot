...
function normalizeCartItem(item) {
  ...
  item.pkgLabel = String(item.pkgLabel || "");
  item.price = Number(item.price || 0);
  item.unitPrice = Number(item.unitPrice || item.price || 0);
  item.phone254 = String(item.phone254 || "");
  item.receivePhone254 = String(item.receivePhone254 || item.phone254 || "");
...
function revalidateCartForCheckout(chatId, s) {
  ...
  return { ok: true, cart: rebuilt, total };
}
...
function addPendingPayment(externalRef, payload) {
  db = repairDB(db);
  db.pendingPayments[String(externalRef)] = { ...payload, ownerChatId: String(payload?.chatId || payload?.ownerChatId || ""), createdAt: Date.now(), status: String(payload?.status || "pending") };
  saveDB();
}
...
await sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
  ...
  addPendingPayment(ref, { chatId, category: s.category, pkgLabel: pkg.label, phone254: phone, receivePhone254: receivePhone, price: pkg.price, cartOrders: Array.isArray(s?.cart) && s.cart.length ? (Array.isArray(s.checkoutSnapshot) && s.checkoutSnapshot.length ? s.checkoutSnapshot.map((item) => ({ ...item })) : s.cart.map((item) => ({ ...item }))) : null, checkoutSnapshot: Array.isArray(s.checkoutSnapshot) ? s.checkoutSnapshot.map((item) => ({ ...item })) : null });
  ...
  try {
    ...
    try { const live = ensureSessionSecurityShape(sessions.get(chatId) || s, chatId); clearCheckoutLock(live); live.pendingCheckoutRef = ref; sessions.set(chatId, live); } catch (_) {}
    ...
    try { const live = ensureSessionSecurityShape(sessions.get(chatId) || s, chatId); clearCheckoutLock(live); live.pendingCheckoutRef = null; sessions.set(chatId, live); } catch (_) {}
...
if (pending && pending.category === "Business Account") {
  ...
  if (plan) {
    ...
    try { const ss = ensureSessionSecurityShape(sessions.get(chatId) || {}, chatId); if (ss && Array.isArray(ss.cart) && ss.cart.length) { clearSessionCart(ss); ss.cartEditIndex = null; ss.pendingCheckoutRef = null; clearCheckoutLock(ss); sessions.set(chatId, ss); } } catch (_) {}
...
if (s.step === "cart_review") {
  ...
  if (text === "✅ Yes, Clear Cart") {
    if (!s.cancelConfirmPending) return sendTracked(chatId, buildCartSummaryText(chatId, s), { parse_mode: "Markdown", ...cartKeyboard() });
    await deleteCartReminderMessage(chatId, s);
    clearSessionCart(s);
    s.step = "category";
    s.category = null;
...
Tap ✅ Confirm Checkout to send STK or ↩️ Keep Reviewing.`, { parse_mode: "Markdown", reply_markup: { keyboard: [["✅ Confirm Checkout", "↩️ Keep Reviewing"],[PREV_BTN, "🏠 Main Menu"]], resize_keyboard: true, one_time_keyboard: false } });
  ...
  if (text === "✅ Confirm Checkout") {
    if (!s.checkoutReviewReady) return sendTracked(chatId, buildCartSummaryText(chatId, s), { parse_mode: "Markdown", ...cartKeyboard() });
