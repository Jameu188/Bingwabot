...
function fmtTxLine(tx) {
  ...
  const recv = String(tx?.receivePhone254 || tx?.receivePhone || tx?.phone254 || tx?.phone || "").trim();
  const recvMasked = recv ? maskPhone(recv) : "";
  return `${icon} ${label}${dur} @${price}ksh${recvMasked ? ` - ${recvMasked}` : ""}`;
}
...
👤 Name: *${mdEscape(name)}*
📅 Joined: \`${mdEscape(joined)}${joinedTs ? ` (${mdEscape(timeAgoText(joinedTs))})` : ""}\`
...
function formatStkExpiredMessage(p = {}, chatId = null) {
  const paymentPhone = maskPhone(String(p?.phone254 || ""));
  const requestedAt = kenyaDateTimeCompactFromTs(p?.createdAt || 0);

  const orders = Array.isArray(p?.checkoutSnapshot) && p.checkoutSnapshot.length
    ? p.checkoutSnapshot
    : (Array.isArray(p?.cartOrders) && p.cartOrders.length ? p.cartOrders : []);
  ...
  if (orders.length) {
    orders.forEach((item, idx) => {
      const label = String(item?.pkgLabel || item?.label || p?.pkgLabel || "N/A").trim();
      const recv = maskPhone(String(item?.receivePhone254 || item?.phone254 || p?.receivePhone254 || p?.phone254 || ""));
      offerLines.push(`${idx + 1}. ${label}`);
      receiveLines.push(`${idx + 1}. ${recv}`);
    });
  } else {
    const label = String(p?.pkgLabel || "N/A").trim();
    const recv = maskPhone(String(p?.receivePhone254 || p?.phone254 || ""));
    offerLines.push(`1. ${label}`);
    receiveLines.push(`1. ${recv}`);
  }

  return tr(
    chatId,
    `⌛ STK request expired.\n\nOffers:\n\n${offerLines.join("\n\n")}\n📱 Payment Number: ${paymentPhone}\n📥 Receive Numbers:\n\n${receiveLines.join(",  ")}\nRequested: ${requestedAt}\n\n\nPlease try again if you still want to pay.`,
    `⌛ Ombi la STK limeisha.\n\nOfa:\n\n${offerLines.join("\n\n")}\n📱 Nambari ya Malipo: ${paymentPhone}\n📥 Nambari za Kupokea:\n\n${receiveLines.join(",  ")}\nImeombwa: ${requestedAt}\n\n\nTafadhali jaribu tena kama bado unataka kulipa.`
  );
}
