...
// ============================================================================
// ✅ PROMO DELETE WATCHDOG
// Best-effort: if promo is deleted within 3 hours, repost every 30 minutes
// without pin. Stops once a promo survives 3 hours from first send.
// Telegram Bot API has no true delete event, so this probes message existence.
// ============================================================================

function ensurePromoWatchdogStore() {
  const store = ensureChannelPromoStore();
  if (!store.watchdogs || typeof store.watchdogs !== "object") store.watchdogs = {};
  return store.watchdogs;
}

function setPromoWatchdog(chatId, data) {
  const w = ensurePromoWatchdogStore();
  if (data) w[String(chatId)] = data;
  else delete w[String(chatId)];
  saveDB();
}

function getPromoWatchdog(chatId) {
  const w = ensurePromoWatchdogStore();
  return w[String(chatId)] || null;
}

async function promoMessageStillExists(chatId, messageId) {
  try {
    if (!messageId) return false;
    const probe = await bot.forwardMessage(String(ADMIN_ID), chatId, messageId, {
      disable_notification: true
    }).catch(() => null);

    if (!probe || !probe.message_id) return false;

    try { await bot.deleteMessage(String(ADMIN_ID), probe.message_id).catch(() => {}); } catch (_) {}
    return true;
  } catch (_) {
    return false;
  }
...
async function repostPromoWithoutPin(chatId) {
  const cfg = channelPromoCfg();
  ...
  if (Array.isArray(cfg.media) && cfg.media.length) {
    ...
    if (m0 && m0.file_id) {
      if (m0.type === "video") {
        const sent = await bot.sendVideo(chatId, m0.file_id, {
          caption: String(cfg.caption || cfg.text || ""),
          ...(kb || {})
        }).catch(() => null);
        if (sent && sent.message_id) messageId = sent.message_id;
      } else {
        const sent = await bot.sendPhoto(chatId, m0.file_id, {
          caption: String(cfg.caption || cfg.text || ""),
          ...(kb || {})
        }).catch(() => null);
        if (sent && sent.message_id) messageId = sent.message_id;
    ...
    const sent = await bot.sendMessage(chatId, String(cfg.text || cfg.caption || ""), kb || {}).catch(() => null);
    if (sent && sent.message_id) messageId = sent.message_id;
  }
...
function startPromoDeleteWatchdog(chatId, messageId) {
  try {
    const now = Date.now();
    const existing = getPromoWatchdog(chatId);
    const startedAt = existing && existing.startedAt ? Number(existing.startedAt) : now;

    setPromoWatchdog(chatId, {
      startedAt: startedAt,
      lastMessageId: Number(messageId || 0),
      nextCheckAt: now + (30 * 60 * 1000),
      active: true
    });
  } catch (_) {}
}

async function runPromoDeleteWatchdogSweep() {
  try {
    const all = ensurePromoWatchdogStore();
    const now = Date.now();
    const threeHours = 3 * 60 * 60 * 1000;

    for (const [chatId, rec] of Object.entries(all || {})) {
      try {
        if (!rec || rec.active === false) {
          setPromoWatchdog(chatId, null);
          continue;
        }

        const startedAt = Number(rec.startedAt || 0);
        const lastMessageId = Number(rec.lastMessageId || 0);
        const nextCheckAt = Number(rec.nextCheckAt || 0);

        if (!startedAt || !lastMessageId) {
          setPromoWatchdog(chatId, null);
          continue;
        }
        ...
        if (nextCheckAt && now < nextCheckAt) continue;

        const exists = await promoMessageStillExists(chatId, lastMessageId);
        ...
        const newMessageId = await repostPromoWithoutPin(chatId);
        if (newMessageId) {
          setPromoWatchdog(chatId, {
            startedAt: startedAt,
            lastMessageId: Number(newMessageId || 0),
            nextCheckAt: now + (30 * 60 * 1000),
            active: true
          });

          try {
            await bot.sendMessage(String(ADMIN_ID), `♻️ Promo auto-reposted (unpinned)\nChat: ${String(chatId)}`).catch(() => {});
          } catch (_) {}
        } else {
          setPromoWatchdog(chatId, {
            startedAt: startedAt,
...
setInterval(() => {
  runPromoDeleteWatchdogSweep().catch(() => {});
}, 5 * 60 * 1000);

// ============================================================================
// ✅ LOW-PERFORMING CHANNEL AUTO REMOVAL
// Removes active channels with poor ad performance over the last 7 days
// Rule:
// - sends >= 3
// - clicks < 10
// - avg clicks per advert < 2
// ============================================================================

function promoLastNDaysTotals(rec, days) {
  let clicks = 0;
  let sends = 0;
  for (let i = 0; i < Number(days || 7); i++) {
    const day = kenyaDayKeyOffset(i);
    clicks += Number(rec && rec.clicksByDay && rec.clicksByDay[day] || 0);
    sends += Number(rec && rec.sendsByDay && rec.sendsByDay[day] || 0);
  }
  return { clicks, sends, avg: sends > 0 ? (clicks / sends) : 0 };
}

async function sweepLowPerformingChannels() {
  try {
    const metrics = ensureChannelPromoMetricsStore();
    for (const [chatId, rec] of Object.entries(metrics.byChannel || {})) {
      try {
        ...
        const perf = promoLastNDaysTotals(rec, 7);
        if (!(perf.sends >= 3 && perf.clicks < 10 && perf.avg < 2)) continue;

        let chat = null;
        try { chat = await bot.getChat(chatId); }
        catch (_) { chat = { id: chatId, title: rec.title || chatId, username: rec.username || "", type: "channel" }; }

        await leaveDeadOrInvalidChannel(chat, `low performance (7d clicks=${perf.clicks}, sends=${perf.sends}, avg=${perf.avg.toFixed(2)})`);
      } catch (_) {}
    }
  } catch (_) {}
}
...
setInterval(() => {
  sweepLowPerformingChannels().catch(() => {});
}, 24 * 60 * 60 * 1000);

// ============================================================================
// ✅ AUTO PROMOTION INSIDE GROUPS
// - triggers on keywords
// - cooldown per group
// - daily cap per group
// - reuses current promo content/button
// - no pin
// ============================================================================

function ensureGroupAutoPromoStore() {
  db = repairDB(db);
  if (!db.groupAutoPromo || typeof db.groupAutoPromo !== "object") db.groupAutoPromo = {};
  if (!db.groupAutoPromo.config || typeof db.groupAutoPromo.config !== "object") {
    db.groupAutoPromo.config = {
      enabled: true,
      cooldownMs: 15 * 60 * 1000,
      dailyMax: 5,
      keywords: [
        "data", "bundle", "bundles", "offer", "offers", "minutes", "sms",
        "buy", "cheap data", "cheap bundles", "internet", "airtime"
      ]
    };
  }
  if (!db.groupAutoPromo.byChat || typeof db.groupAutoPromo.byChat !== "object") db.groupAutoPromo.byChat = {};
  return db.groupAutoPromo;
}

function getGroupAutoPromoRec(chatId) {
  const store = ensureGroupAutoPromoStore();
  const key = String(chatId);
  if (!store.byChat[key] || typeof store.byChat[key] !== "object") {
    store.byChat[key] = { enabled: true, lastSentAt: 0, sentToday: 0, dayKey: promoDayKey() };
  }
  return store.byChat[key];
}

function shouldTriggerGroupPromo(text) {
  const store = ensureGroupAutoPromoStore();
  const t = String(text || "").toLowerCase();
  return (store.config.keywords || []).some((kw) => t.includes(String(kw || "").toLowerCase()));
}

async function sendAutoGroupPromo(chatId) {
  try {
    const cfg = channelPromoCfg();
    const kb = promoTrackedButtonKeyboard(chatId);
      ...
      if (m0 && m0.file_id) {
        if (m0.type === "video") {
          ...
          if (sent && sent.message_id) {
            try { trackPromoSend(chatId, sent.message_id); } catch (_) {}
...
bot.on("message", async (msg) => {
  try {
    ...
    const chatId = msg.chat.id;
    const rec = getGroupAutoPromoRec(chatId);
    if (!rec.enabled) return;

    const day = promoDayKey();
    if (String(rec.dayKey || "") !== String(day)) {
      rec.dayKey = day;
      rec.sentToday = 0;
    }

    if (!shouldTriggerGroupPromo(msg.text)) return;
    if (Date.now() - Number(rec.lastSentAt || 0) < Number(store.config.cooldownMs || 15 * 60 * 1000)) return;
    if (Number(rec.sentToday || 0) >= Number(store.config.dailyMax || 5)) return;

    const sentMessageId = await sendAutoGroupPromo(chatId);
    if (sentMessageId) {
      rec.lastSentAt = Date.now();
      rec.sentToday = Number(rec.sentToday || 0) + 1;
      saveDB();
    }
  } catch (_) {}
});

// Override promo sender so successful sends start the watchdog
async function sendConfiguredChannelPromo(targetChatId) {
  const cfg = channelPromoCfg();
  const kb = promoTrackedButtonKeyboard(targetChatId);
  let pinMessageId = null;
  ...
  if (pinMessageId) {
    try { trackPromoSend(targetChatId, pinMessageId); } catch (_) {}
    try { startPromoDeleteWatchdog(targetChatId, pinMessageId); } catch (_) {}
  }
