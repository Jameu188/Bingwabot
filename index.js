...
if (!db0.abuseCounters || typeof db0.abuseCounters !== "object") db0.abuseCounters = {};
if (!Array.isArray(db0.activityLog)) db0.activityLog = [];
if (!db0.strikes || typeof db0.strikes !== "object") db0.strikes = {};

if (!db0.blockedUsers || typeof db0.blockedUsers !== "object") db0.blockedUsers = {};
if (!db0.unblockRequests || typeof db0.unblockRequests !== "object") db0.unblockRequests = {};
...
function cleanupExpiredBlock(chatId) {
  db = repairDB(db);
  const k = String(chatId);
  const rec = db.blockedUsers?.[k];
  if (!rec) return false;
  const exp = rec.expiresAt ? Number(rec.expiresAt) : 0;
  if (exp && Date.now() >= exp) {
    delete db.blockedUsers[k];
    saveDB();
    return true;
  }
  return false;
...
function getBlockRecord(chatId) {
  db = repairDB(db);
  cleanupExpiredBlock(chatId);
  return db.blockedUsers?.[String(chatId)] || null;
}

function isBlocked(chatId) {
  return !!getBlockRecord(chatId);
}

function isShadowBanned(chatId) {
  const rec = getBlockRecord(chatId);
  return !!rec && String(rec.mode || "hard") === "shadow";
}

// mode: "hard" (default) or "shadow"
// expiresAt: unix ms or 0/null for permanent
function blockUser(chatId, reason = "", opts = {}) {
  db = repairDB(db);
  const k = String(chatId);
  const mode = String(opts.mode || "hard");
  const expiresAt = opts.expiresAt ? Number(opts.expiresAt) : 0;
...
function unblockUser(chatId) {
  ...
  delete db.unblockRequests[String(chatId)];
  saveDB();
}

function addActivity(chatId, type, detail = "") {
  db = repairDB(db);
  const entry = {
    t: Date.now(),
  ...
  if (db.activityLog.length > 300) db.activityLog = db.activityLog.slice(-300);
  saveDB();
}

// Strike system (3 strikes => auto hard block)
const STRIKE_TTL_DAYS = 7;

function getStrikes(chatId) {
  db = repairDB(db);
  const k = String(chatId);
  const arr = Array.isArray(db.strikes?.[k]) ? db.strikes[k] : [];
  const cutoff = Date.now() - STRIKE_TTL_DAYS * 24 * 60 * 60 * 1000;
  const filtered = arr.filter((ts) => Number(ts) >= cutoff);
  if (filtered.length !== arr.length) {
    db.strikes[k] = filtered;
    saveDB();
  }
  return filtered;
}

function addStrike(chatId, reason = "Violation") {
  db = repairDB(db);
  const k = String(chatId);
  const arr = getStrikes(chatId);
  arr.push(Date.now());
  db.strikes[k] = arr.slice(-10);
  saveDB();

  addActivity(chatId, "strike", reason);

  if (arr.length >= 3) {
    blockUser(chatId, "Auto-block: 3 strikes", { mode: "hard" });
    addActivity(chatId, "autoblock", "3 strikes threshold");
    return { strikes: arr.length, blocked: true };
  }
  return { strikes: arr.length, blocked: false };
}

function clearStrikes(chatId) {
  db = repairDB(db);
  delete db.strikes[String(chatId)];
  saveDB();
}

// Suspicious flagging: counts per type per rolling window
function flagSuspicious(chatId, type, limit = 3, windowMin = 60, strikeReason = "") {
  db = repairDB(db);
  const k = String(chatId);
  ...
  if (!db.abuseCounters[k]) db.abuseCounters[k] = {};
  const rec = db.abuseCounters[k][t] || { n: 0, since: now };
  if (now - Number(rec.since || now) > win) {
    rec.n = 0;
    rec.since = now;
  }
  rec.n += 1;
  db.abuseCounters[k][t] = rec;
  saveDB();

  addActivity(chatId, "flag", `${t} (${rec.n}/${limit})`);

  if (rec.n >= limit) {
    const res = addStrike(chatId, strikeReason || `Suspicious: ${t}`);
    db = repairDB(db);
    db.abuseCounters[k][t] = { n: 0, since: now };
    saveDB();
    return { flagged: true, ...res };
  }
  return { flagged: false, strikes: getStrikes(chatId).length, blocked: isBlocked(chatId) };
}

async function sendUnblockRequestToAdmin(userChatId, context = "") {
  try {
    db = repairDB(db);
    const k = String(userChatId);
    const now = Date.now();
    const last = Number(db.unblockRequests?.[k]?.lastReqAt || 0);

    if (last && now - last < 10 * 60 * 1000) return;

    const reason = db.blockedUsers?.[k]?.reason || "";
    const rec = db.blockedUsers?.[k] || {};
    const mode = String(rec.mode || "hard");
    const exp = rec.expiresAt ? new Date(Number(rec.expiresAt)).toISOString() : "";

    const userText =
      `🚫 *Blocked User Request*\\n` +
      `ChatID: \\`${mdEscape(k)}\\`\\n` +
      (reason ? `Reason: _${mdEscape(reason)}_\\n` : "") +
      `Mode: _${mdEscape(mode)}_\\n` +
      (exp ? `Expiry: \\`${mdEscape(exp)}\\`\\n` : "") +
      (context ? `Context: _${mdEscape(context)}_\\n` : "") +
      `Time: \\`${mdEscape(kenyaDateTime())}\\`\\n\\n` +
      `Choose action:`;

    const sent = await bot.sendMessage(ADMIN_ID, userText, {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          [{ text: "✅ Override & Unblock", callback_data: `unblock_accept:${k}` }],
          [{ text: "❌ Keep Blocked", callback_data: `unblock_keep:${k}` }],
        ],
      },
    });

    db.unblockRequests[k] = { lastReqAt: now, adminMsgId: sent.message_id };
    saveDB();
  } catch (_) {}
}

function blockedUserKeyboard() {
  return {
    reply_markup: {
      keyboard: [["📩 Request Unblock"], ["ℹ️ Help"]],
...
function shadowBannedReplyKeyboard(chatId) {
...
` +
  ...
  if (text === "🔄 Resend STK") {
      ...
      // 🚫 Blocked users cannot resend STK (shadow-ban supported)
if (!isAdmin(chatId) && isBlocked(chatId)) {
  if (isShadowBanned(chatId)) {
    await sendTracked(
      chatId,
      "⚠️ Sorry, the STK service is temporarily unavailable. Please try again later.",
      shadowBannedReplyKeyboard(chatId)
    );
    return;
  }
  await sendTracked(
    chatId,
    "🚫 Your account is blocked.\n\nYou cannot buy offers or withdraw points until admin unblocks you.\nTap 📩 Request Unblock to ask for review.",
    blockedUserKeyboard()
  );
  await sendUnblockRequestToAdmin(chatId, "Blocked user attempted STK (Resend)");
  // abuse detection
  flagSuspicious(chatId, "resend_stk", 3, 60, "STK resend spam");
  return;
}
...
if (text === "💸 Withdraw Points") {
      ...
      // 🚫 Blocked users cannot withdraw (shadow-ban supported)
if (!isAdmin(chatId) && isBlocked(chatId)) {
  if (isShadowBanned(chatId)) {
    await sendTracked(
      chatId,
      "⚠️ Withdraw service is currently unavailable. Please try again later.",
      shadowBannedReplyKeyboard(chatId)
    );
    return;
  }
  await sendTracked(
    chatId,
    "🚫 Your account is blocked.\n\nWithdrawals are disabled until admin unblocks you.\nTap 📩 Request Unblock to ask for review.",
    blockedUserKeyboard()
  );
  await sendUnblockRequestToAdmin(chatId, "Blocked user attempted Withdraw amount");
  // abuse detection
  flagSuspicious(chatId, "blocked_action", 4, 120, "Blocked user repeated actions");
  return;
}
  ...
  return sendTracked(chatId, msgText, { 
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        // 🚫 Blocked users cannot send STK (shadow-ban supported)
if (!isAdmin(chatId) && isBlocked(chatId)) {
  if (isShadowBanned(chatId)) {
    await sendTracked(
      chatId,
  ...
  await sendUnblockRequestToAdmin(chatId, "Blocked user attempted STK (Proceed)");
  // abuse detection
  flagSuspicious(chatId, "blocked_action", 4, 120, "Blocked user repeated actions");
  sessions.delete(chatId);
  return;
