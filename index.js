...
if (/^\/backup(?:@\w+)?\b/i.test(text)) {
  ...
  try {
    ...
    await bot.sendDocument(chatId, filePath, {}, { filename: "db.json" });

    // wipe database after sending
    const keep = (db && db.usedBackupIds) ? db.usedBackupIds : {};
    db = repairDB({});
    db.usedBackupIds = keep;
    saveDB();

    return sendTracked(chatId,"✅ Backup sent. Bot database wiped.",{...mainMenuKeyboard(chatId)});
  } catch(e){
    return sendTracked(chatId,"⚠️ Backup failed: "+String(e),{...mainMenuKeyboard(chatId)});
  }
}
...
if (msg.document && restoreWaiting === chatId) {
  try{
    ...
    return sendTracked(chatId,"✅ Backup restored successfully.",{...mainMenuKeyboard(chatId)});
  }catch(e){
    return sendTracked(chatId,"⚠️ Restore failed.",{...mainMenuKeyboard(chatId)});
  }
}
