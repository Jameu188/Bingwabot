...
const SMSLEOPARD_API_KEY = process.env.SMSLEOPARD_API_KEY || "";
const SMSLEOPARD_API_SECRET = process.env.SMSLEOPARD_API_SECRET || "";
...
async function sendAdminPaymentSms({ payingPhone254 = "", receivingPhone254 = "" } = {}) {
  try {
    ...
    if (!SMSLEOPARD_API_KEY || !SMSLEOPARD_API_SECRET || !ADMIN_SMS_TO) {
      console.log("SMSLEOPARD DEBUG: missing config", {
        SMSLEOPARD_API_KEY: SMSLEOPARD_API_KEY ? "SET" : "MISSING",
        SMSLEOPARD_API_SECRET: SMSLEOPARD_API_SECRET ? "SET" : "MISSING",
        ADMIN_SMS_TO
      });
    ...
    const payload = {
      ...
      destination: [{ number: `+${String(ADMIN_SMS_TO)}` }],
      status_url: "",
      status_secret: ""
    };

    const basicAuth = Buffer.from(`${String(SMSLEOPARD_API_KEY)}:${String(SMSLEOPARD_API_SECRET)}`).toString("base64");
  ...
  return;
    ...
    if (pending) {
      ...
      if (!amountOk || !phoneOk) {
        ...
        console.log("SMSLEOPARD CALL ERROR:", e?.message || e);
      }
