...
//
// FIXES (latest):
// ✅ Callback will NOT mark success when transaction was cancelled/failed (uses ResultCode / Status / woocommerce_payment_status)
...
async function notifyAdmin(text) {
  try {
    // Escape any dynamic data already inside the string? We don't know.
    // Here we keep the message as-is, but we also prevent parse failures by switching to plain text on error.
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // ✅ SUCCESS RULE:
    // - If ResultCode exists: must be 0
    // - Else fallback to woocommerce_payment_status === "complete"/"completed"/"processing"
    // - Else fallback to payment_status/status string contains success/paid/complete AND NOT failure words
    let isSuccess = false;
    ...
    if (!isSuccess) {
      // Optional: notify user of cancellation
    ...
    // Notify admin (Markdown-safe)
    await notifyAdmin(
      `✅ *Payment Confirmed*\nChatID: \`${mdEscape(chatId)}\`\nRef: \`${mdEscape(externalRef)}\`\nStatus: \`${mdEscape(String(statusRaw || woostatus || resultCodeRaw || ""))}\``
    );
  } catch (e) {
