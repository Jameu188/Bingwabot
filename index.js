...
Help: ${HELP_PHONE}`, { ...mainMenuKeyboard() });
    ...
    let chatId = Number(req.chatId);
    // PayHero callbacks may not include chatId; derive it from externalRef as fallback
    if (!Number.isFinite(chatId) || chatId <= 0) {
      const ex = String(externalRef || "");
      const p = ex.split("-");
      const fromRef = Number(p[p.length - 1]);
      if (Number.isFinite(fromRef) && fromRef > 0) chatId = fromRef;
    }
