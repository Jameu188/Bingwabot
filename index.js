...
if (text === "🔗 My Referral") {
  ...
  const totalEarned = Number(u.referralEarningsTotal || 0);
...
` +
    `🎁 Earn bonus points when your referrals make successful purchases.

` +
    `📊 *Your Referral Stats*
` +
    `👥 Total successful referrals: *${successful}*
` +
    `💰 Earned so far from referrals: *${totalEarned.toFixed(2)} pts*
` +
    `💎 Your current points balance: *${Number(pts).toFixed(2)} pts*
