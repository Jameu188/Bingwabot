...
if (!db0.analytics.transactionsByDay || typeof db0.analytics.transactionsByDay !== "object") db0.analytics.transactionsByDay = {};
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.adminNoLimits !== "boolean") x.adminNoLimits = false;
    if (typeof x.adminBypassBingwa !== "boolean") x.adminBypassBingwa = false;
    if (typeof x.specialSettingsAccess !== "boolean") x.specialSettingsAccess = false;
...
function getUser(chatId) {
  ...
  if (!db.users[cid]) {
    db.users[cid] = {
      ...
      // 🎯 Admin specific-user controls
      adminNoLimits: false,
      adminBypassBingwa: false,
      specialSettingsAccess: false,
};
  }
  ...
  if (db.users[cid].adminNoLimits === undefined) db.users[cid].adminNoLimits = false;
  if (db.users[cid].adminBypassBingwa === undefined) db.users[cid].adminBypassBingwa = false;
  if (db.users[cid].specialSettingsAccess === undefined) db.users[cid].specialSettingsAccess = false;
...
function normalizeAdminPanelButtonText(value) {
  return String(value || "")
    .replace(/[\uFE0F\u200D]/g, "")
    .replace(/\s+/g, " ")
    .trim()
...
if (data.startsWith("admin:")) {
  ...
  if (action === "offers") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    const text = "🔒 OFFER TOGGLES\n\nTap any category to open/close buying.\nYou can also allow specific user IDs to keep using closed services.";
    await adminPanelSendOrEdit(chatId, text, adminPanelOfferKeyboard(), q).catch(async () => {
      await bot.sendMessage(chatId, text, adminPanelOfferKeyboard()).catch(() => {});
    });
    return;
  }
  ...
  if (action === "support") {
    ...
    const kb = rows.length
      ? { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } }
      : adminPanelUsersKeyboard();
    const msg = rows.length
      ? `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`
      : "✅ No open support requests right now.";
    await adminPanelSendOrEdit(chatId, msg, kb, q).catch(async () => {
      await bot.sendMessage(chatId, msg, kb).catch(() => {});
    });
    return;
  }
}
...
function isAdmin(chatId) {
  try {
    ...
    return getAdminIdList().includes(id);
  } catch (_) {
...
async function initiatePurchaseStk(chatId, s, pkg, phone254) {
  ...
  if (!userBypassesBingwaRule(chatId) && s.category === "Bingwa Deals" && bingwaAlreadyPurchasedToday(phone)) {
    sessions.delete(chatId);
    return sendTracked(
      chatId,
...
function getAllUserIds() {
  ...
  return Object.keys(db.users || {})
    .map((x) => Number(x))
    .filter((x) => Number.isFinite(x) && x > 0 && !isAdmin(x));
}

async function broadcastCopyMessage(fromChatId, messageId) {
  if (!isGlobalNotificationsEnabled()) return { sent: 0, failed: 0, total: 0, skipped: true };
    ...
    if (pending) {
      ...
      if (!amountOk || !phoneOk) {
        ...
        if (!alreadyProcessed(failKey)) {
          try { recordFailedTransactionMetric(failKey, pending, "Callback verification failed"); } catch (_) {}
          markProcessed(failKey);
          deletePendingPayment(externalRef);
...
if (!userBypassesBingwaRule(chatId) && s.category === "Bingwa Deals" && bingwaAlreadyPurchasedToday(phoneMaybe)) {
  sessions.delete(chatId);
  return sendTracked(
    chatId,
...
function ensureAdminPanelStore() {
  ...
  db.extraAdminIds = db.extraAdminIds.map((x) => String(x || "").trim()).filter((x) => /^\d+$/.test(x));
  if (!Array.isArray(db.adminLogs)) db.adminLogs = [];
  if (!db.adminMetrics || typeof db.adminMetrics !== "object") db.adminMetrics = {};
  if (!Array.isArray(db.adminMetrics.failedTransactions)) db.adminMetrics.failedTransactions = [];
  if (!db.globalNotifications || typeof db.globalNotifications !== "object") db.globalNotifications = { enabled: true, updatedAt: 0, updatedBy: "" };
  if (typeof db.globalNotifications.enabled !== "boolean") db.globalNotifications.enabled = true;
  if (!Number.isFinite(Number(db.globalNotifications.updatedAt))) db.globalNotifications.updatedAt = 0;
  if (typeof db.globalNotifications.updatedBy !== "string") db.globalNotifications.updatedBy = "";
  return db;
}
...
function getAdminIdList() {
  try {
    ...
    const ids = [String(ADMIN_ID || "").trim(), ...((db.extraAdminIds || []).map((x) => String(x || "").trim()))]
      .filter((x) => /^\d+$/.test(x));
    return Array.from(new Set(ids));
  } catch (_) {
    return /^\d+$/.test(String(ADMIN_ID || "").trim()) ? [String(ADMIN_ID).trim()] : [];
  }
}

function addAdminLog(actorChatId, action, details = "") {
  try {
    ensureAdminPanelStore();
    db.adminLogs.push({ at: Date.now(), actorChatId: String(actorChatId || ""), action: String(action || ""), details: String(details || "") });
    if (db.adminLogs.length > 500) db.adminLogs = db.adminLogs.slice(-500);
    saveDB();
  } catch (_) {}
}

function recentAdminLogsText(limit = 20) {
  try {
    ensureAdminPanelStore();
    const rows = (db.adminLogs || []).slice(-limit).reverse();
    if (!rows.length) return "📜 SYSTEM LOGS\n\n• No logs yet.";
    return "📜 SYSTEM LOGS\n\n" + rows.map((x, i) => `${i + 1}. ${kenyaDateTimeFromTs(Number(x.at || 0))} — ${x.action}${x.details ? `\n   ${x.details}` : ""}`).join("\n");
  } catch (_) {
    return "📜 SYSTEM LOGS\n\n• No logs yet.";
  }
}
...
function isGlobalNotificationsEnabled() {
  try {
    ensureAdminPanelStore();
    return !!db.globalNotifications.enabled;
  } catch (_) { return true; }
}

function setGlobalNotificationsEnabled(value, actorChatId = "") {
  ensureAdminPanelStore();
  db.globalNotifications.enabled = !!value;
  db.globalNotifications.updatedAt = Date.now();
  db.globalNotifications.updatedBy = String(actorChatId || "");
  saveDB();
  addAdminLog(actorChatId, db.globalNotifications.enabled ? "Global notifications enabled" : "Global notifications disabled");
  return db.globalNotifications.enabled;
}

function recordFailedTransactionMetric(externalRef, pending, reasonText) {
  try {
    ensureAdminPanelStore();
    const arr = db.adminMetrics.failedTransactions || [];
    const ref = String(externalRef || "").trim();
    if (ref && arr.some((x) => String(x.externalRef || "") === ref)) return;
    arr.push({
      at: Date.now(),
      externalRef: ref,
      chatId: String(pending?.chatId || ""),
      ...
      phone254: String(pending?.phone254 || ""),
      pkgLabel: String(pending?.pkgLabel || ""),
      reason: String(reasonText || "Payment failed")
    });
    if (arr.length > 300) db.adminMetrics.failedTransactions = arr.slice(-300);
    else db.adminMetrics.failedTransactions = arr;
    saveDB();
  } catch (_) {}
}

function failedTransactionsMonitorText(limit = 20) {
  try {
    ensureAdminPanelStore();
    const rows = (db.adminMetrics.failedTransactions || []).slice(-limit).reverse();
    if (!rows.length) return "📉 FAILED TRANSACTIONS MONITOR\n\n• No recent failed transactions.";
    return "📉 FAILED TRANSACTIONS MONITOR\n\n" + rows.map((x, i) => `${i + 1}. ${kenyaDateTimeFromTs(Number(x.at || 0))} — ${x.pkgLabel || "Unknown"} — Ksh ${Number(x.amount || 0)}\n   User: ${x.chatId || "N/A"} • ${maskPhone(x.phone254 || "")}\n   Reason: ${x.reason || "Payment failed"}`).join("\n");
  } catch (_) {
    return "📉 FAILED TRANSACTIONS MONITOR\n\n• No recent failed transactions.";
  }
}

function suspiciousActivityText(limit = 15) {
  try {
    ensureAdminPanelStore();
    const rows = [];
    const phoneMap = {};
    for (const [cid, u] of Object.entries(db.users || {})) {
      ...
      if (phone) {
        ...
        phoneMap[phone].push(cid);
      }
      if (Number(u?.referralSuccessCount || 0) >= 15 && Number(u?.totalSpentKsh || 0) < 100) rows.push(`Referral-heavy / low-spend: ${adminSafeUserName(cid)} — ${cid} • referrals ${Number(u?.referralSuccessCount || 0)}, spent Ksh ${Number(u?.totalSpentKsh || 0)}`);
      if (Number(u?.qPurch20Count || 0) >= 12 && Number(u?.totalSpentKsh || 0) <= 0) rows.push(`High purchase count without spend tracked: ${adminSafeUserName(cid)} — ${cid}`);
    }
    for (const [phone, ids] of Object.entries(phoneMap)) {
      if (ids.length >= 3) rows.push(`Shared phone across ${ids.length} users: ${formatTo07(phone)} • IDs: ${ids.slice(0, 5).join(", ")}`);
    }
    const banned = Object.entries(db.users || {}).filter(([_, u]) => !!u?.banned).length;
    if (banned >= 5) rows.push(`High banned users count detected: ${banned}`);
    const out = rows.slice(0, limit);
    if (!out.length) return "🧠 SUSPICIOUS ACTIVITY / ABUSE DETECTION\n\n• No obvious abuse flags right now.";
    return "🧠 SUSPICIOUS ACTIVITY / ABUSE DETECTION\n\n" + out.map((x, i) => `${i + 1}. ${x}`).join("\n");
  } catch (_) {
    return "🧠 SUSPICIOUS ACTIVITY / ABUSE DETECTION\n\n• Could not generate abuse report.";
  }
}

function removeInactiveUsersAdmin(days = 60) {
  try {
    ...
    for (const [cid, u] of Object.entries(db.users || {})) {
      ...
      const last = Number(u?.lastSeen || 0);
      if (!last || last < cutoff) {
        try { wipeUserData(cid, { preserveLang: false }); } catch (_) {}
        try { delete db.users[String(cid)]; } catch (_) {}
        removed += 1;
      }
    }
    saveDB();
...
function adminListText() {
  const ids = getAdminIdList();
  return "👮 ADMIN LIST\n\n" + (ids.length ? ids.map((id, i) => `${i + 1}. ${adminSafeUserName(id)} — ${id}${String(id) === String(ADMIN_ID) ? " (owner)" : ""}`).join("\n") : "• No admins configured.");
}
...
function adminPanelRootKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "📊 Today Stats", callback_data: "ap:stats" }],
        [{ text: "👤 Users", callback_data: "ap:users" }, { text: "🔒 Offer Toggles", callback_data: "ap:offers" }],
...
function adminPanelUsersKeyboard() {
  const notifLabel = isGlobalNotificationsEnabled() ? "🔕 Disable Global Notifications" : "🔔 Enable Global Notifications";
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🚫 Ban User", callback_data: "ap:ban" }, { text: "✅ Unban User", callback_data: "ap:unban" }],
        [{ text: "⏳ Temporary Ban", callback_data: "ap:tempban" }],
        [{ text: "📋 Blocked Users", callback_data: "ap:blocked" }],
        [{ text: "🛠 Maintenance Mode", callback_data: "ap:maintenance" }],
        [{ text: "📨 Support Requests", callback_data: "ap:support" }],
        [{ text: "📢 Broadcast Message", callback_data: "ap:broadcast" }],
        [{ text: "📡 API Status", callback_data: "ap:api_status" }, { text: "📜 System Logs", callback_data: "ap:system_logs" }],
        [{ text: notifLabel, callback_data: "ap:toggle_notifications" }],
        [{ text: "🧹 Clear Old Logs", callback_data: "ap:clear_logs" }, { text: "🗑 Remove Inactive Users", callback_data: "ap:remove_inactive" }],
        [{ text: "🗄 Backup Database", callback_data: "ap:backup_db" }],
        [{ text: "➕ Add Admin", callback_data: "ap:add_admin" }, { text: "❌ Remove Admin", callback_data: "ap:remove_admin" }],
        [{ text: "👮 Admin List", callback_data: "ap:admin_list" }],
        [{ text: "🎯 Specific User Tools", callback_data: "ap:specific_user" }],
        [{ text: "📉 Failed TX Monitor", callback_data: "ap:failed_tx" }],
        [{ text: "🧠 Abuse Detection", callback_data: "ap:suspicious" }],
...
bot.on("message", async (msg) => {
  ...
  const text = (msg.text || "").trim();
  const normText = normalizeAdminPanelButtonText(text);
  if (!isAdmin(chatId)) return;

  if (["🛡️ Admin Panel", "🛡 Admin Panel"].includes(text) || normText.replace(/^[^a-z]+/, "") === "admin panel") {
    try { clearPendingAction(chatId); } catch (_) {}
    await openInlineAdminPanel(chatId);
    return;
  }

  // Admin text shortcuts for reply-keyboard/typed labels
  if (["👤 Users", "🔒 Offer Toggles", "⏳ Temporary Ban", "🛠 Maintenance Mode", "📨 Support Requests", "📢 Broadcast Message"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    if (text === "👤 Users") {
      await adminPanelSendOrEdit(chatId, "👤 USER ADMIN\n\nChoose an action below.", adminPanelUsersKeyboard());
      return;
    }
    if (text === "🔒 Offer Toggles") {
      await adminPanelSendOrEdit(chatId, "🔒 OFFER TOGGLES\n\nTap any category to open/close buying.\nYou can also allow specific user IDs to keep using closed services.", adminPanelOfferKeyboard());
      return;
    }
    if (text === "⏳ Temporary Ban" || normText === "temporary ban") {
      adminState.set(String(chatId), { step: "await_temp_ban", createdAt: Date.now() });
      await bot.sendMessage(chatId, "⏳ Temporary Ban\n\nSend details in this format:\n<chat_id> <duration> [reason]\n\nExamples:\n123456789 2d spam\n123456789 12h abuse\n123456789 30m too many attempts\n\nUse /cancel to stop.").catch(() => {});
      return;
    }
    if (text === "🛠 Maintenance Mode" || normText.replace(/^[^a-z]+/, "") === "maintenance mode") {
      const enabled = setMaintenanceMode(!isMaintenanceMode());
      await adminPanelSendOrEdit(chatId, `✅ Maintenance mode is now ${enabled ? "ON ⚠️" : "OFF ✅"}.`, adminPanelUsersKeyboard());
      return;
    }
    if (text === "📨 Support Requests" || normText.replace(/^[^a-z]+/, "") === "support requests") {
      const rows = adminOpenSupportTickets(10);
      const kb = rows.length
        ? { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } }
        : adminPanelUsersKeyboard();
      const msg = rows.length
        ? `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`
        : "✅ No open support requests right now.";
      await adminPanelSendOrEdit(chatId, msg, kb).catch(() => {});
      return;
    }
    if (text === "📢 Broadcast Message" || normText.replace(/^[^a-z]+/, "") === "broadcast message") {
      adminState.set(String(chatId), { step: "await_broadcast", createdAt: Date.now() });
      await bot.sendMessage(chatId, "📣 Broadcast mode ON.\n\nSend ANYTHING to broadcast:\n• text\n• photo / video\n• music/audio / voice\n• document\n• sticker\n• animation/gif\n• album (multi photos/videos as one post)\n\nSend /cancel to stop.").catch(() => {});
      return;
    }
  }
...
${adminPanelBusinessText()}`, adminPanelBusinessKeyboard());
  ...
  if (act.type === "admin_specific_user_open") {
    const target = String(text || '').trim();
    if (!/^\d+$/.test(target)) {
      await bot.sendMessage(chatId, '❌ Send a valid numeric user ID or tap ❌ Cancel.', adminPanelSimpleKeyboard('ap:users')).catch(() => {});
      return;
    }
    clearPendingAction(chatId);
    await adminPanelSendOrEdit(chatId, specificUserSummaryText(target), specificUserToolsKeyboard(target));
    return;
  }

  if (["admin_specific_add_points", "admin_specific_remove_points", "admin_specific_set_points"].includes(act.type)) {
    const target = String(act.target || '').trim();
    const amount = Number(String(text || '').trim());
    if (!/^\d+$/.test(target) || !Number.isFinite(amount) || amount < 0) {
      await bot.sendMessage(chatId, '❌ Send a valid positive number.', adminPanelSimpleKeyboard(`ap:specific_open:${target}`)).catch(() => {});
      return;
    }
    const u = getUser(target);
    if (act.type === 'admin_specific_add_points') u.points = Number(u.points || 0) + amount;
    else if (act.type === 'admin_specific_remove_points') u.points = Math.max(0, Number(u.points || 0) - amount);
    else u.points = Math.max(0, amount);
    saveDB();
    addAdminLog(chatId, 'Specific user points updated', `Target: ${target} • Action: ${act.type} • Amount: ${amount}`);
    clearPendingAction(chatId);
    await adminPanelSendOrEdit(chatId, specificUserSummaryText(target), specificUserToolsKeyboard(target));
    return;
  }

  if (["admin_add_admin", "admin_remove_admin"].includes(act.type)) {
    const target = String(text || "").trim();
    if (!/^\d+$/.test(target)) {
      await bot.sendMessage(chatId, "❌ Send a valid numeric user ID or tap ❌ Cancel.").catch(() => {});
      return;
    ...
    ensureAdminPanelStore();
    if (String(target) === String(ADMIN_ID) && act.type === "admin_remove_admin") {
      await bot.sendMessage(chatId, "❌ Owner admin cannot be removed.", adminPanelSimpleKeyboard("ap:users")).catch(() => {});
      return;
    }
    if (act.type === "admin_add_admin") {
      if (!db.extraAdminIds.includes(String(target))) db.extraAdminIds.push(String(target));
      saveDB();
      addAdminLog(chatId, "Admin added", `Target: ${target}`);
      clearPendingAction(chatId);
      await adminPanelSendOrEdit(chatId, `✅ Added admin ${target}.

${adminListText()}`, adminPanelSimpleKeyboard("ap:users"));
    } else {
      db.extraAdminIds = (db.extraAdminIds || []).filter((x) => String(x) !== String(target));
      saveDB();
      addAdminLog(chatId, "Admin removed", `Target: ${target}`);
      clearPendingAction(chatId);
      await adminPanelSendOrEdit(chatId, `✅ Removed admin ${target}.
...
${adminListText()}`, adminPanelSimpleKeyboard("ap:users"));
  ...
  if (act.type === "admin_remove_inactive") {
    const days = Math.max(1, Number(String(text || "").trim() || 60));
    if (!Number.isFinite(days)) {
      await bot.sendMessage(chatId, "❌ Send number of inactive days, for example 60.", adminPanelSimpleKeyboard("ap:users")).catch(() => {});
      return;
    }
    const removed = removeInactiveUsersAdmin(days);
    clearPendingAction(chatId);
    addAdminLog(chatId, "Inactive users removed", `Days: ${days} • Removed: ${removed}`);
    await adminPanelSendOrEdit(chatId, `✅ Removed ${removed} inactive user(s) older than ${days} day(s).`, adminPanelSimpleKeyboard("ap:users"));
    return;
  }
...
bot.on("callback_query", async (q) => {
  ...
  if (data === "ap:blocked") {
    addAdminLog(chatId, "Viewed blocked users");
  ...
  if (data === "ap:tempban") {
    adminState.set(String(chatId), { step: "await_temp_ban", createdAt: Date.now() });
    await adminPanelSendOrEdit(chatId, "⏳ Temporary Ban\n\nSend details in this format:\n<chat_id> <duration> [reason]\n\nExamples:\n123456789 2d spam\n123456789 12h abuse\n123456789 30m too many attempts\n\nUse /cancel to stop.", adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:maintenance") {
    const enabled = setMaintenanceMode(!isMaintenanceMode());
    addAdminLog(chatId, `Maintenance mode ${enabled ? "enabled" : "disabled"}`);
    await adminPanelSendOrEdit(chatId, `✅ Maintenance mode is now ${enabled ? "ON ⚠️" : "OFF ✅"}.`, adminPanelUsersKeyboard(), q);
    return;
  }
  if (data === "ap:support") {
    addAdminLog(chatId, "Viewed support requests");
    const rows = adminOpenSupportTickets(10);
    if (!rows.length) {
      await adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard(), q);
      return;
    }
    await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } }, q);
    return;
  }
  if (data === "ap:broadcast") {
    adminState.set(String(chatId), { step: "await_broadcast", createdAt: Date.now() });
    await adminPanelSendOrEdit(chatId, "📣 Broadcast mode ON.\n\nSend ANYTHING to broadcast:\n• text\n• photo / video\n• music/audio / voice\n• document\n• sticker\n• animation/gif\n• album (multi photos/videos as one post)\n\nSend /cancel to stop.", adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:api_status") {
    const txt = `📡 API STATUS

• Bot token: ${TELEGRAM_TOKEN ? "Configured ✅" : "Missing ❌"}
• PayHero callback: ${PAYHERO_CALLBACK_URL ? "Configured ✅" : "Missing ❌"}
• Global notifications: ${isGlobalNotificationsEnabled() ? "Enabled ✅" : "Disabled ⛔"}
• Maintenance mode: ${isMaintenanceMode() ? "ON ⚠️" : "OFF ✅"}
• Admins: ${getAdminIdList().length}`;
    addAdminLog(chatId, "Viewed API status");
    await adminPanelSendOrEdit(chatId, txt, adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:system_logs") {
    addAdminLog(chatId, "Viewed system logs");
    await adminPanelSendOrEdit(chatId, recentAdminLogsText(25), adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:toggle_notifications") {
    const enabled = setGlobalNotificationsEnabled(!isGlobalNotificationsEnabled(), chatId);
    await adminPanelSendOrEdit(chatId, `✅ Global notifications are now ${enabled ? "ENABLED" : "DISABLED"}.`, adminPanelUsersKeyboard(), q);
    return;
  }
  if (data === "ap:clear_logs") {
    clearAdminLogs();
    addAdminLog(chatId, "Cleared old logs");
    await adminPanelSendOrEdit(chatId, "✅ Old system logs cleared.", adminPanelUsersKeyboard(), q);
    return;
  }
  if (data === "ap:remove_inactive") {
    setPendingAction(chatId, { type: "admin_remove_inactive" });
    await adminPanelSendOrEdit(chatId, `🗑 Send inactive days threshold.

Example: 60`, adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:backup_db" || data === "ap:backup") {
    addAdminLog(chatId, "Database backup requested");
    try {
      await bot.sendDocument(chatId, DB_FILE, { caption: "🗄 Database backup (db.json)" }).catch(async () => {
        const tmp = path.join(__dirname, `db_backup_${Date.now()}.json`);
        try { fs.writeFileSync(tmp, JSON.stringify(db, null, 2)); } catch (_) {}
  ...
  if (data === "ap:add_admin") {
    setPendingAction(chatId, { type: "admin_add_admin" });
    await adminPanelSendOrEdit(chatId, "➕ Send the user ID to add as admin.", adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:remove_admin") {
    setPendingAction(chatId, { type: "admin_remove_admin" });
    await adminPanelSendOrEdit(chatId, "❌ Send the user ID to remove from admins.", adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:admin_list") {
    await adminPanelSendOrEdit(chatId, adminListText(), adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:specific_user") {
    setPendingAction(chatId, { type: "admin_specific_user_open" });
    await adminPanelSendOrEdit(chatId, "🎯 Send the user ID to open specific-user tools.", adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data.startsWith("ap:specific_open:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    await adminPanelSendOrEdit(chatId, specificUserSummaryText(target), specificUserToolsKeyboard(target), q);
    return;
  }
  if (data.startsWith("ap:specific_add:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    setPendingAction(chatId, { type: "admin_specific_add_points", target });
    await adminPanelSendOrEdit(chatId, `➕ Send points to ADD for user ${target}.`, adminPanelSimpleKeyboard(`ap:specific_open:${target}`), q);
    return;
  }
  if (data.startsWith("ap:specific_remove:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    setPendingAction(chatId, { type: "admin_specific_remove_points", target });
    await adminPanelSendOrEdit(chatId, `➖ Send points to REMOVE for user ${target}.`, adminPanelSimpleKeyboard(`ap:specific_open:${target}`), q);
    return;
  }
  if (data.startsWith("ap:specific_set:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    setPendingAction(chatId, { type: "admin_specific_set_points", target });
    await adminPanelSendOrEdit(chatId, `🎯 Send points to SET for user ${target}.`, adminPanelSimpleKeyboard(`ap:specific_open:${target}`), q);
    return;
  }
  if (data.startsWith("ap:specific_numbers:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    const list = getUserPurchasePhonesSoFar(target);
    const msg = `📱 NUMBERS USED SO FAR\n\nUser: ${adminSafeUserName(target)}\nID: ${adminDisplayUserId(target)}\n\n${list.length ? list.map((x, i) => `${i + 1}. ${adminDisplayPhone(x)}`).join('\n') : '• None'}`;
    await adminPanelSendOrEdit(chatId, msg, adminPanelSimpleKeyboard(`ap:specific_open:${target}`), q);
    return;
  }
  if (data.startsWith("ap:specific_saved:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    const u = getUser(target);
    await adminPanelSendOrEdit(chatId, `💾 CURRENT SAVED NUMBER\n\nUser: ${adminSafeUserName(target)}\nID: ${adminDisplayUserId(target)}\nSaved number: ${adminDisplayPhone(u?.phone || '')}`, adminPanelSimpleKeyboard(`ap:specific_open:${target}`), q);
    return;
  }
  if (data.startsWith("ap:specific_settings:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    await adminPanelSendOrEdit(chatId, `⚙️ SPECIFIC SETTINGS\n\nUser: ${adminSafeUserName(target)}\nID: ${adminDisplayUserId(target)}\n\n${specificUserSettingsSummary(target)}`, specificUserSettingsKeyboard(target), q);
    return;
  }
  if (data.startsWith("ap:specific_limits:")) {
    const target = String(data.split(":").slice(2).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    const u = getUser(target);
    await adminPanelSendOrEdit(chatId, `🚫 LIMIT CONTROLS\n\nUser: ${adminSafeUserName(target)}\nID: ${adminDisplayUserId(target)}\nNo limits: ${u.adminNoLimits ? 'ON' : 'OFF'}\nBypass Bingwa rule: ${u.adminBypassBingwa ? 'ON' : 'OFF'}`, specificUserLimitsKeyboard(target), q);
    return;
  }
  if (data.startsWith("ap:specific_toggle_setting:")) {
    const parts = data.split(":");
    const target = String(parts[2] || '').trim();
    const key = String(parts.slice(3).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
    setSpecificUserToggle(target, key);
    await adminPanelSendOrEdit(chatId, `⚙️ SPECIFIC SETTINGS\n\nUser: ${adminSafeUserName(target)}\nID: ${adminDisplayUserId(target)}\n\n${specificUserSettingsSummary(target)}`, specificUserSettingsKeyboard(target), q);
    return;
  }
  if (data.startsWith("ap:specific_toggle_limit:")) {
    const parts = data.split(":");
    const target = String(parts[2] || '').trim();
    const key = String(parts.slice(3).join(":") || '').trim();
    if (!/^\d+$/.test(target)) return;
  ...
  if (data === "ap:failed_tx") {
    await adminPanelSendOrEdit(chatId, failedTransactionsMonitorText(20), adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
  if (data === "ap:suspicious") {
    await adminPanelSendOrEdit(chatId, suspiciousActivityText(20), adminPanelSimpleKeyboard("ap:users"), q);
    return;
  }
...
// ===================== PRIVACY + SECURITY + GROWTH ADMIN PATCH =====================
function ensureAdvancedSecurityStore() {
  db = repairDB(db);
  if (!db.privacySecurity || typeof db.privacySecurity !== 'object') db.privacySecurity = {};
  const s = db.privacySecurity;
  if (typeof s.maskSensitiveData !== 'boolean') s.maskSensitiveData = false;
  if (!s.adminRoles || typeof s.adminRoles !== 'object') s.adminRoles = {};
  if (!Array.isArray(s.accessLogs)) s.accessLogs = [];
  if (!s.notificationConsent || typeof s.notificationConsent !== 'object') s.notificationConsent = { enabled: false, updatedAt: 0 };
  if (typeof s.notificationConsent.enabled !== 'boolean') s.notificationConsent.enabled = false;
  if (typeof s.autoHideDetails !== 'boolean') s.autoHideDetails = false;
  if (typeof s.twoStepAdminProtection !== 'boolean') s.twoStepAdminProtection = false;
  if (!s.retention || typeof s.retention !== 'object') s.retention = { adminLogsDays: 30, accessLogsDays: 30 };
  if (!Number.isFinite(Number(s.retention.adminLogsDays))) s.retention.adminLogsDays = 30;
  if (!Number.isFinite(Number(s.retention.accessLogsDays))) s.retention.accessLogsDays = 30;
  if (!s.adminSessions || typeof s.adminSessions !== 'object') s.adminSessions = {};
  if (!s.purchaseLimit || typeof s.purchaseLimit !== 'object') s.purchaseLimit = { enabled: false, daily: 0 };
  if (typeof s.purchaseLimit.enabled !== 'boolean') s.purchaseLimit.enabled = false;
  if (!Number.isFinite(Number(s.purchaseLimit.daily))) s.purchaseLimit.daily = 0;
  if (typeof s.pinImportantMessages !== 'boolean') s.pinImportantMessages = true;
  if (!db.purchaseCountsByDay || typeof db.purchaseCountsByDay !== 'object') db.purchaseCountsByDay = {};
  if (!db.tempBans || typeof db.tempBans !== 'object') db.tempBans = {};
  if (!db.adminConfirmations || typeof db.adminConfirmations !== 'object') db.adminConfirmations = {};
  return s;
}

function maskSensitiveValue(value, keep = 2) {
...
function adminMaskOn() {
  try { return !!ensureAdvancedSecurityStore().maskSensitiveData; } catch (_) { return false; }
}

function adminDisplayPhone(phone) {
  const val = String(phone || '');
  if (!val) return 'N/A';
  return adminMaskOn() ? maskSensitiveValue(formatTo07(val) || val, 2) : (formatTo07(val) || val);
}

function adminDisplayUserId(id) {
  const val = String(id || '');
  if (!val) return 'N/A';
  return adminMaskOn() ? maskSensitiveValue(val, 2) : val;
}

function userHasNoLimits(chatId) {
  try {
    if (isAdmin(chatId)) return true;
    return !!getUser(chatId)?.adminNoLimits;
  } catch (_) { return false; }
}

function userBypassesBingwaRule(chatId) {
  try {
    if (isAdmin(chatId)) return true;
    const u = getUser(chatId);
    return !!(u?.adminNoLimits || u?.adminBypassBingwa);
  } catch (_) { return false; }
}

function getUserPurchasePhonesSoFar(targetId) {
  try {
    const out = new Set();
    for (const arr of Object.values(db.analytics?.transactionsByDay || {})) {
      for (const t of (Array.isArray(arr) ? arr : [])) {
        if (String(t?.chatId || '') !== String(targetId || '')) continue;
        const ph = String(t?.phone254 || '').trim();
        if (ph) out.add(formatTo07(ph) || ph);
      }
    }
...
function specificUserSettingsSummary(targetId) {
  const u = getUser(targetId);
  return [
    ...
    `Auto Delete 24h: ${u.autoDelete24hEnabled ? 'ON' : 'OFF'}`,
    `Notification Consent: ${u.notificationConsent ? 'ON' : 'OFF'}`,
    `Special Settings Access: ${u.specialSettingsAccess ? 'ON' : 'OFF'}`
  ].join('\n');
}

function specificUserSummaryText(targetId) {
  const u = getUser(targetId);
  const phones = getUserPurchasePhonesSoFar(targetId);
  return (
    `🎯 SPECIFIC USER TOOLS\n\n` +
    `User: ${adminSafeUserName(targetId)}\n` +
    `ID: ${adminDisplayUserId(targetId)}\n` +
    `Saved number: ${adminDisplayPhone(u?.phone || '')}\n` +
    `Points: ${Number(u?.points || 0)}\n` +
    `No limits: ${u.adminNoLimits ? 'ON' : 'OFF'}\n` +
    `Bypass Bingwa rule: ${u.adminBypassBingwa ? 'ON' : 'OFF'}\n` +
    `Special settings access: ${u.specialSettingsAccess ? 'ON' : 'OFF'}\n` +
    `Numbers used so far: ${phones.length}\n\n` +
    `Settings\n${specificUserSettingsSummary(targetId)}`
  );
}

function specificUserToolsKeyboard(targetId) {
  return { reply_markup: { inline_keyboard: [
    [{ text: '➕ Add Points', callback_data: `ap:specific_add:${targetId}` }, { text: '➖ Remove Points', callback_data: `ap:specific_remove:${targetId}` }],
    [{ text: '🎯 Set Points', callback_data: `ap:specific_set:${targetId}` }],
    [{ text: '📱 Numbers Used', callback_data: `ap:specific_numbers:${targetId}` }, { text: '💾 Saved Number', callback_data: `ap:specific_saved:${targetId}` }],
    [{ text: '⚙️ Specific Settings', callback_data: `ap:specific_settings:${targetId}` }],
    [{ text: '🚫 Remove All Limits', callback_data: `ap:specific_limits:${targetId}` }],
    [{ text: '⬅️ Back', callback_data: 'ap:users' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
  ] } };
}

function specificUserSettingsKeyboard(targetId) {
  const u = getUser(targetId);
  const t = (x, on) => `${on ? '✅' : '⬜'} ${x}`;
  return { reply_markup: { inline_keyboard: [
    [{ text: t('Fast Purchase', u.fastPurchaseEnabled), callback_data: `ap:specific_toggle_setting:${targetId}:fastPurchaseEnabled` }],
    [{ text: t('Auto Retry STK', u.autoRetryStkEnabled), callback_data: `ap:specific_toggle_setting:${targetId}:autoRetryStkEnabled` }],
    [{ text: t('Hide Quick Actions', u.hideQuickActionsEnabled), callback_data: `ap:specific_toggle_setting:${targetId}:hideQuickActionsEnabled` }],
    [{ text: t('Auto Hide Buttons', u.autoHideButtonsEnabled), callback_data: `ap:specific_toggle_setting:${targetId}:autoHideButtonsEnabled` }],
    [{ text: t('Auto Delete 24h', u.autoDelete24hEnabled), callback_data: `ap:specific_toggle_setting:${targetId}:autoDelete24hEnabled` }],
    [{ text: t('Notification Consent', !!u.notificationConsent), callback_data: `ap:specific_toggle_setting:${targetId}:notificationConsent` }],
    [{ text: t('Special Settings Access', u.specialSettingsAccess), callback_data: `ap:specific_toggle_setting:${targetId}:specialSettingsAccess` }],
    [{ text: '⬅️ Back', callback_data: `ap:specific_open:${targetId}` }]
  ] } };
}

function specificUserLimitsKeyboard(targetId) {
  const u = getUser(targetId);
  const t = (x, on) => `${on ? '✅' : '⬜'} ${x}`;
  return { reply_markup: { inline_keyboard: [
    [{ text: t('Remove ALL limits', u.adminNoLimits), callback_data: `ap:specific_toggle_limit:${targetId}:adminNoLimits` }],
    [{ text: t('Bypass Bingwa daily rule', u.adminBypassBingwa), callback_data: `ap:specific_toggle_limit:${targetId}:adminBypassBingwa` }],
    [{ text: '⬅️ Back', callback_data: `ap:specific_open:${targetId}` }]
  ] } };
}

function setSpecificUserToggle(targetId, key) {
  const allowed = ['fastPurchaseEnabled','autoRetryStkEnabled','hideQuickActionsEnabled','autoHideButtonsEnabled','autoDelete24hEnabled','notificationConsent','specialSettingsAccess','adminNoLimits','adminBypassBingwa'];
  if (!allowed.includes(String(key || ''))) return false;
  const u = getUser(targetId);
  u[key] = !u[key];
  saveDB();
  return true;
...
function setAdminRole(targetId, role) {
  const s = ensureAdvancedSecurityStore();
  const rid = String(targetId || '').trim();
  if (!/^\d+$/.test(rid)) return false;
  const safeRole = ['owner', 'full', 'moderator', 'viewer'].includes(String(role || '')) ? String(role) : 'viewer';
  s.adminRoles[rid] = safeRole;
  saveDB();
  return true;
}

function getAdminRole(targetId) {
  const rid = String(targetId || '').trim();
  const s = ensureAdvancedSecurityStore();
  if (rid === String(ADMIN_ID)) return 'owner';
  return String(s.adminRoles[rid] || 'full');
}

function logAdminAccess(actorChatId, action, extra = {}) {
  try {
    const s = ensureAdvancedSecurityStore();
    s.accessLogs.push({
      at: Date.now(),
    ...
    if (s.accessLogs.length > 500) s.accessLogs = s.accessLogs.slice(-500);
    saveDB();
  } catch (_) {}
}

function accessLogsText(limit = 25) {
  try {
    const s = ensureAdvancedSecurityStore();
    const rows = (s.accessLogs || []).slice(-limit).reverse();
    if (!rows.length) return '🧾 ACCESS LOGS\n\n• No access logs yet.';
    return '🧾 ACCESS LOGS\n\n' + rows.map((x, i) => {
      const extra = x.extra && typeof x.extra === 'object' ? Object.entries(x.extra).map(([k, v]) => `${k}: ${v}`).join(' • ') : '';
      return `${i + 1}. ${kenyaDateTimeFromTs(Number(x.at || 0))} — ${x.action}\n   Admin: ${adminDisplayUserId(x.actorChatId || '')}${extra ? `\n   ${extra}` : ''}`;
    }).join('\n');
  } catch (_) {
    return '🧾 ACCESS LOGS\n\n• No access logs yet.';
  }
...
function updateAdminSession(chatId, msg) {
  try {
    if (!isAdmin(chatId)) return;
    const s = ensureAdvancedSecurityStore();
    const key = String(chatId || '');
    const rec = s.adminSessions[key] || {};
    rec.lastSeenAt = Date.now();
    rec.lastChatId = String(msg?.chat?.id || chatId || '');
    rec.lastChatType = String(msg?.chat?.type || 'private');
    rec.lastText = String(msg?.text || msg?.caption || '').slice(0, 40);
    s.adminSessions[key] = rec;
    saveDB();
  } catch (_) {}
}

function adminSessionsText() {
  try {
    const s = ensureAdvancedSecurityStore();
    const rows = Object.entries(s.adminSessions || {}).sort((a, b) => Number(b[1]?.lastSeenAt || 0) - Number(a[1]?.lastSeenAt || 0));
    if (!rows.length) return '📱 ADMIN SESSION AWARENESS\n\n• No admin sessions recorded yet.';
    return '📱 ADMIN SESSION AWARENESS\n\n' + rows.map(([id, rec], i) => `${i + 1}. ${adminSafeUserName(id)} — ${adminDisplayUserId(id)}\n   Last seen: ${kenyaDateTimeFromTs(Number(rec.lastSeenAt || 0))}\n   Chat type: ${rec.lastChatType || 'private'} • Chat: ${adminDisplayUserId(rec.lastChatId || '')}`).join('\n');
  } catch (_) {
    return '📱 ADMIN SESSION AWARENESS\n\n• No admin sessions recorded yet.';
  }
}
...
function adminRolesText() {
  const ids = getAdminIdList();
  if (!ids.length) return '👮 ADMIN ROLES\n\n• No admins configured.';
  return '👮 ADMIN ROLES\n\n' + ids.map((id, i) => `${i + 1}. ${adminSafeUserName(id)} — ${adminDisplayUserId(id)}\n   Role: ${getAdminRole(id)}`).join('\n');
}

function exportUserDataText(targetId) {
  const u = getUser(targetId);
  return `📤 DATA EXPORT READY\n\nUser: ${adminSafeUserName(targetId)}\nID: ${adminDisplayUserId(targetId)}\nPhone: ${adminDisplayPhone(u?.phone || '')}`;
}

function deleteUserDataAdmin(targetId) {
  try {
    const tid = String(targetId || '').trim();
    if (!/^\d+$/.test(tid)) return false;
    wipeUserData(tid, { preserveLang: false });
    try { delete db.users[tid]; } catch (_) {}
    try { delete db.tempBans[tid]; } catch (_) {}
    saveDB();
    return true;
  } catch (_) { return false; }
}

function advancedPrivacySummaryText() {
  const s = ensureAdvancedSecurityStore();
  const activeRot = listActiveRotations().length;
  return (
    `🔒 PRIVACY & SECURITY\n\n` +
    `🙈 Mask sensitive data: ${s.maskSensitiveData ? 'ON' : 'OFF'}\n` +
    `🔕 Notification consent: ${s.notificationConsent.enabled ? 'ON' : 'OFF'}\n` +
    `🙈 Auto-hide details: ${s.autoHideDetails ? 'ON' : 'OFF'}\n` +
    `🛡 Two-step admin protection: ${s.twoStepAdminProtection ? 'ON' : 'OFF'}\n` +
    `🗃 Log retention: ${Number(s.retention.adminLogsDays || 30)} day(s)\n` +
    `🛒 User purchase limit: ${s.purchaseLimit.enabled ? `${Number(s.purchaseLimit.daily || 0)} per day` : 'OFF'}\n` +
    `📌 Pin important messages: ${s.pinImportantMessages ? 'ON' : 'OFF'}\n` +
    `🔁 Scheduled broadcasts: ${activeRot}`
  );
}

function privacySecurityKeyboard() {
  const s = ensureAdvancedSecurityStore();
  return { reply_markup: { inline_keyboard: [
    [{ text: `${s.maskSensitiveData ? '🙈 Unmask Sensitive Data' : '🙈 Mask Sensitive Data'}`, callback_data: 'aps:toggle_mask' }],
    [{ text: '👮 Admin Roles', callback_data: 'aps:admin_roles' }, { text: '🧾 Access Logs', callback_data: 'aps:access_logs' }],
    [{ text: '📤 Data Export', callback_data: 'aps:data_export' }, { text: '🗑 Delete User Data', callback_data: 'aps:delete_user' }],
    [{ text: `${s.notificationConsent.enabled ? '🔔 Notification Consent ON' : '🔕 Notification Consent OFF'}`, callback_data: 'aps:toggle_consent' }],
    [{ text: `${s.autoHideDetails ? '👁 Show Details' : '⏳ Auto-Hide Details'}`, callback_data: 'aps:toggle_autohide' }],
    [{ text: `${s.twoStepAdminProtection ? '✅ Two-Step Admin Protection' : '🛡 Two-Step Admin Protection'}`, callback_data: 'aps:toggle_twostep' }],
    [{ text: '🗃 Data Retention Controls', callback_data: 'aps:retention' }, { text: '📱 Admin Sessions', callback_data: 'aps:sessions' }],
    [{ text: '🛒 Purchase Limit Control', callback_data: 'aps:purchase_limit' }, { text: `${s.pinImportantMessages ? '📌 Pin Important Message ON' : '📌 Pin Important Message OFF'}`, callback_data: 'aps:pin_toggle' }],
    [{ text: '📈 Growth Analytics', callback_data: 'aps:growth' }, { text: '🔁 Scheduled Broadcasts', callback_data: 'aps:scheduled' }],
    [{ text: '⬅️ Back', callback_data: 'ap:users' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
  ] } };
}

function growthAnalyticsText() {
  try {
    const t0 = sumDaysRevenue(0);
    const t6 = sumDaysRevenue(6);
    const t29 = sumDaysRevenue(29);
    const users = Object.keys(db.users || {}).length;
    const active7 = Object.values(db.users || {}).filter((u) => Number(u?.lastSeen || 0) >= Date.now() - 7 * 24 * 60 * 60 * 1000).length;
    const biz = countBusinessSubscribers({ activeOnly: true });
    return (
      `📈 GROWTH ANALYTICS\n\n` +
      `👥 Total users: ${users}\n` +
      `🟢 Active 7d: ${active7}\n` +
      `🏢 Active business: ${biz}\n\n` +
      `💰 Revenue Today: Ksh ${Number(t0.revenueKsh || 0)} (${Number(t0.orders || 0)} orders)\n` +
      `💰 Revenue 7d: Ksh ${Number(t6.revenueKsh || 0)} (${Number(t6.orders || 0)} orders)\n` +
      `💰 Revenue 30d: Ksh ${Number(t29.revenueKsh || 0)} (${Number(t29.orders || 0)} orders)`
    );
  } catch (_) {
...
function scheduledBroadcastsText() {
  try {
    const list = listActiveRotations();
    if (!list.length) return '🔁 SCHEDULED BROADCASTS\n\n• No active scheduled broadcasts.\n\nTip: use /bcast while replying to a message.';
    return '🔁 SCHEDULED BROADCASTS\n\n' + list.slice(0, 20).map((r, i) => `${i + 1}. ${r.id}\n   Next: ${kenyaDateTimeFromTs(Number(r.nextRunAt || 0))}\n   Expires: ${kenyaDateTimeFromTs(Number(r.expiresAt || 0))}\n   Runs: ${Number(r.runs || 0)}`).join('\n');
  } catch (_) {
    return '🔁 SCHEDULED BROADCASTS\n\n• Could not load scheduled broadcasts.';
  }
}

function setTemporaryBan(targetId, durationMs, reason = '', actorChatId = '') {
  ensureAdvancedSecurityStore();
  const tid = String(targetId || '').trim();
  const until = Date.now() + Math.max(60 * 1000, Number(durationMs || 0));
  db.tempBans[tid] = { until, reason: String(reason || ''), actorChatId: String(actorChatId || ''), at: Date.now() };
  saveDB();
  addAdminLog(actorChatId, 'Temporary ban set', `Target: ${tid}`);
  return db.tempBans[tid];
}

function isTemporarilyBanned(targetId) {
  try {
    ensureAdvancedSecurityStore();
    const tid = String(targetId || '').trim();
    const rec = db.tempBans[tid];
    if (!rec) return { active: false, remainingMs: 0, reason: '' };
    const remainingMs = Number(rec.until || 0) - Date.now();
    if (remainingMs <= 0) {
      delete db.tempBans[tid];
      saveDB();
      return { active: false, remainingMs: 0, reason: '' };
    }
    return { active: true, remainingMs, reason: String(rec.reason || ''), until: Number(rec.until || 0) };
  } catch (_) {
    return { active: false, remainingMs: 0, reason: '' };
  }
}
...
function isMaintenanceMode() {
  try { return !!db.maintenanceMode; } catch (_) { return false; }
}
...
function purchaseLimitReached(chatId) {
  try {
    const s = ensureAdvancedSecurityStore();
    if (userHasNoLimits(chatId)) return { hit: false, used: 0, limit: 0 };
    if (!s.purchaseLimit.enabled || !(Number(s.purchaseLimit.daily || 0) > 0)) return { hit: false, used: 0, limit: 0 };
    const day = todayKey();
    const used = Number(db.purchaseCountsByDay?.[day]?.[String(chatId || '')] || 0);
    return { hit: used >= Number(s.purchaseLimit.daily || 0), used, limit: Number(s.purchaseLimit.daily || 0) };
  } catch (_) { return { hit: false, used: 0, limit: 0 }; }
}

function bumpPurchaseCount(chatId) {
  try {
    ensureAdvancedSecurityStore();
    const day = todayKey();
    db.purchaseCountsByDay[day] = db.purchaseCountsByDay[day] || {};
    db.purchaseCountsByDay[day][String(chatId || '')] = Number(db.purchaseCountsByDay[day][String(chatId || '')] || 0) + 1;
    saveDB();
  } catch (_) {}
}

const __patchOldRecordSuccessfulRevenue = recordSuccessfulRevenue;
recordSuccessfulRevenue = function (pending) {
  const out = __patchOldRecordSuccessfulRevenue(pending);
  try { if (pending?.chatId) bumpPurchaseCount(pending.chatId); } catch (_) {}
  return out;
};

const __patchOldPayheroStkPush2 = payheroStkPush;
payheroStkPush = async function (payload) {
  try {
    const chatId = Number(String(payload?.externalRef || '').split('-').pop() || 0);
    if (chatId) {
      const lim = purchaseLimitReached(chatId);
      if (lim.hit) throw new Error(`Daily purchase limit reached (${lim.used}/${lim.limit})`);
    }
  } catch (e) {
    if (String(e?.message || '').includes('Daily purchase limit reached')) throw e;
  }
  return await __patchOldPayheroStkPush2(payload);
};

const __patchOldAutoPinBroadcastMessage = typeof autoPinBroadcastMessage === 'function' ? autoPinBroadcastMessage : null;
if (__patchOldAutoPinBroadcastMessage) {
  autoPinBroadcastMessage = async function (chat, messageId) {
    try {
      const s = ensureAdvancedSecurityStore();
      if (!s.pinImportantMessages) return false;
    } catch (_) {}
    return await __patchOldAutoPinBroadcastMessage(chat, messageId);
  };
}

function maybeRequireAdminConfirm(chatId, key, q, promptText, nextCallback) {
  const s = ensureAdvancedSecurityStore();
  if (!s.twoStepAdminProtection) return false;
  const id = `${String(chatId)}:${String(key)}`;
  const cur = db.adminConfirmations[id];
  const now = Date.now();
  if (cur && (now - Number(cur.at || 0) < 30000)) {
    delete db.adminConfirmations[id];
    saveDB();
    return false;
  }
  db.adminConfirmations[id] = { at: now };
  saveDB();
  adminPanelSendOrEdit(chatId, `${promptText}\n\nTap again within 30 seconds to confirm.`, { reply_markup: { inline_keyboard: [
    [{ text: '✅ Confirm', callback_data: nextCallback }],
    [{ text: '⬅️ Back', callback_data: 'aps:menu' }]
  ] } }, q).catch(() => {});
  return true;
}
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '📉 Failed TX Monitor', callback_data: 'ap:failed_tx' }],
        [{ text: '🧠 Abuse Detection', callback_data: 'ap:suspicious' }],
        [{ text: '🔒 Privacy & Security', callback_data: 'aps:menu' }],
        [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
      ]
    }
  };
...
bot.on('callback_query', async (q) => {
  try {
    ...
    const chatId = q.message?.chat?.id;
    if (!chatId || !isAdmin(chatId)) return;
    if (!data.startsWith('aps:')) return;
    await bot.answerCallbackQuery(q.id).catch(() => {});
    ensureAdvancedSecurityStore();
    logAdminAccess(chatId, 'privacy_menu_action', { action: data });

    if (data === 'aps:menu') {
      await adminPanelSendOrEdit(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:toggle_mask') {
      db.privacySecurity.maskSensitiveData = !db.privacySecurity.maskSensitiveData;
      saveDB();
      await adminPanelSendOrEdit(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:admin_roles') {
      await adminPanelSendOrEdit(chatId, adminRolesText(), { reply_markup: { inline_keyboard: [
        [{ text: '✏️ Set Admin Role', callback_data: 'aps:set_admin_role' }],
        [{ text: '⬅️ Back', callback_data: 'aps:menu' }]
      ] } }, q);
      return;
    }
    if (data === 'aps:set_admin_role') {
      setPendingAction(chatId, { type: 'admin_set_role' });
      await adminPanelSendOrEdit(chatId, '👮 Send: <user_id> <role>\n\nRoles: owner, full, moderator, viewer\nExample: 123456789 moderator', adminPanelSimpleKeyboard('aps:menu'), q);
      return;
    }
    if (data === 'aps:access_logs') {
      await adminPanelSendOrEdit(chatId, accessLogsText(25), { reply_markup: { inline_keyboard: [
        [{ text: '🧹 Clear Access Logs', callback_data: 'aps:clear_access_logs' }],
        [{ text: '⬅️ Back', callback_data: 'aps:menu' }]
      ] } }, q);
      return;
    }
    if (data === 'aps:clear_access_logs') {
      if (maybeRequireAdminConfirm(chatId, 'clear_access_logs', q, '🧹 Clear all access logs?', 'aps:clear_access_logs')) return;
      db.privacySecurity.accessLogs = [];
      saveDB();
      await adminPanelSendOrEdit(chatId, '✅ Access logs cleared.', privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:data_export') {
      setPendingAction(chatId, { type: 'admin_data_export' });
      await adminPanelSendOrEdit(chatId, '📤 Send the user ID to export.', adminPanelSimpleKeyboard('aps:menu'), q);
      return;
    }
    if (data === 'aps:delete_user') {
      setPendingAction(chatId, { type: 'admin_delete_user_data' });
      await adminPanelSendOrEdit(chatId, '🗑 Send the user ID to delete all saved data.', adminPanelSimpleKeyboard('aps:menu'), q);
      return;
    }
    if (data === 'aps:toggle_consent') {
      db.privacySecurity.notificationConsent.enabled = !db.privacySecurity.notificationConsent.enabled;
      db.privacySecurity.notificationConsent.updatedAt = Date.now();
      saveDB();
      await adminPanelSendOrEdit(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:toggle_autohide') {
      db.privacySecurity.autoHideDetails = !db.privacySecurity.autoHideDetails;
      saveDB();
      await adminPanelSendOrEdit(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:toggle_twostep') {
      db.privacySecurity.twoStepAdminProtection = !db.privacySecurity.twoStepAdminProtection;
      saveDB();
      await adminPanelSendOrEdit(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:retention') {
      setPendingAction(chatId, { type: 'admin_set_retention' });
      await adminPanelSendOrEdit(chatId, `🗃 Current retention: ${Number(db.privacySecurity.retention.adminLogsDays || 30)} day(s).\n\nSend new retention days. Example: 30`, adminPanelSimpleKeyboard('aps:menu'), q);
      return;
    }
    if (data === 'aps:sessions') {
      await adminPanelSendOrEdit(chatId, adminSessionsText(), { reply_markup: { inline_keyboard: [[{ text: '⬅️ Back', callback_data: 'aps:menu' }]] } }, q);
      return;
    }
    if (data === 'aps:purchase_limit') {
      setPendingAction(chatId, { type: 'admin_set_purchase_limit' });
      await adminPanelSendOrEdit(chatId, `🛒 Purchase limit is ${db.privacySecurity.purchaseLimit.enabled ? `ON (${db.privacySecurity.purchaseLimit.daily}/day)` : 'OFF'}.\n\nSend a number to enable daily purchase limit. Send 0 to disable.`, adminPanelSimpleKeyboard('aps:menu'), q);
      return;
    }
    if (data === 'aps:pin_toggle') {
      db.privacySecurity.pinImportantMessages = !db.privacySecurity.pinImportantMessages;
      saveDB();
      await adminPanelSendOrEdit(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard(), q);
      return;
    }
    if (data === 'aps:growth') {
      await adminPanelSendOrEdit(chatId, growthAnalyticsText(), { reply_markup: { inline_keyboard: [[{ text: '⬅️ Back', callback_data: 'aps:menu' }]] } }, q);
      return;
    }
    if (data === 'aps:scheduled') {
      await adminPanelSendOrEdit(chatId, scheduledBroadcastsText(), { reply_markup: { inline_keyboard: [[{ text: '⬅️ Back', callback_data: 'aps:menu' }]] } }, q);
      return;
    }
  } catch (_) {}
});
...
bot.on('message', async (msg) => {
  try {
    const chatId = msg.chat?.id;
    if (!chatId) return;
    if (isAdmin(chatId)) updateAdminSession(chatId, msg);
    const text = String(msg.text || '').trim();
    if (!isAdmin(chatId)) {
      if (/^\/(?:consent)(?:@\w+)?\s+(on|off)$/i.test(text)) {
        const on = /on$/i.test(text);
        const u = getUser(chatId);
        u.notificationConsent = !!on;
        saveDB();
        await bot.sendMessage(chatId, on ? '✅ Notification consent enabled.' : '🔕 Notification consent disabled.', { ...mainMenuKeyboard(chatId) }).catch(() => {});
      }
      return;
    }

    const act = getPendingAction(chatId);
    if (act?.type === 'admin_set_role') {
      const parts = text.split(/\s+/).filter(Boolean);
      const target = String(parts[0] || '').trim();
      const role = String(parts[1] || '').trim().toLowerCase();
      if (!/^\d+$/.test(target) || !['owner','full','moderator','viewer'].includes(role)) {
        await bot.sendMessage(chatId, '❌ Send: <user_id> <role>\nRoles: owner, full, moderator, viewer', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
        return;
      }
      setAdminRole(target, role);
      clearPendingAction(chatId);
      await adminPanelSendOrEdit(chatId, `✅ Role updated for ${target}.\n\n${adminRolesText()}`, privacySecurityKeyboard());
      return;
    }

    if (act?.type === 'admin_data_export') {
      const target = String(text || '').trim();
      if (!/^\d+$/.test(target)) {
        await bot.sendMessage(chatId, '❌ Send a valid numeric user ID.', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
        return;
      }
      clearPendingAction(chatId);
      const payload = buildUserBackup(target);
      const out = `/mnt/data/user_export_${target}_${Date.now()}.json`;
      try { fs.writeFileSync(out, JSON.stringify(payload, null, 2)); } catch (_) {}
      await bot.sendDocument(chatId, out, { caption: exportUserDataText(target) }).catch(() => {});
      return;
    }
    ...
    if (act?.type === 'admin_delete_user_data') {
      ...
      if (ensureAdvancedSecurityStore().twoStepAdminProtection) {
        setPendingAction(chatId, { type: 'admin_delete_user_data_confirm', target });
        await bot.sendMessage(chatId, `⚠️ Send YES ${target} to confirm deletion.`, adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
        return;
      }
      const ok = deleteUserDataAdmin(target);
      clearPendingAction(chatId);
      await bot.sendMessage(chatId, ok ? `✅ User data deleted for ${target}.` : '❌ Delete failed.', { ...mainMenuKeyboard(chatId) }).catch(() => {});
      return;
    }

    if (act?.type === 'admin_delete_user_data_confirm') {
      const target = String(act.target || '');
      if (text !== `YES ${target}`) {
        await bot.sendMessage(chatId, `❌ Confirmation failed. Send exactly: YES ${target}`, adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
        return;
      }
      const ok = deleteUserDataAdmin(target);
      clearPendingAction(chatId);
    ...
    if (act?.type === 'admin_set_retention') {
      const days = Math.max(1, Math.min(365, Number(text || 0)));
      if (!Number.isFinite(days) || !(days > 0)) {
        await bot.sendMessage(chatId, '❌ Send a valid number of days, e.g. 30.', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
        return;
      }
      db.privacySecurity.retention.adminLogsDays = days;
      db.privacySecurity.retention.accessLogsDays = days;
      const cutoff = Date.now() - days * 86400000;
      db.adminLogs = (db.adminLogs || []).filter((x) => Number(x?.at || 0) >= cutoff);
      db.privacySecurity.accessLogs = (db.privacySecurity.accessLogs || []).filter((x) => Number(x?.at || 0) >= cutoff);
      saveDB();
      clearPendingAction(chatId);
      await adminPanelSendOrEdit(chatId, `✅ Retention updated to ${days} day(s).`, privacySecurityKeyboard());
      return;
    }

    if (act?.type === 'admin_set_purchase_limit') {
      const daily = Math.max(0, Number(text || 0));
      if (!Number.isFinite(daily)) {
        await bot.sendMessage(chatId, '❌ Send a valid number. 0 disables the limit.', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
        return;
      }
      db.privacySecurity.purchaseLimit.enabled = daily > 0;
      db.privacySecurity.purchaseLimit.daily = Math.floor(daily);
      saveDB();
      clearPendingAction(chatId);
      await adminPanelSendOrEdit(chatId, `✅ Purchase limit ${daily > 0 ? `set to ${Math.floor(daily)} per day` : 'disabled'}.`, privacySecurityKeyboard());
      return;
    }

    if (text === '🔒 Privacy & Security') {
      await sendTracked(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === '🙈 Mask Sensitive Data') {
      db.privacySecurity.maskSensitiveData = !db.privacySecurity.maskSensitiveData;
      saveDB();
      await sendTracked(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard()).catch(() => {});
      return;
    ...
    if (text === '👮 Admin Roles') {
      await sendTracked(chatId, adminRolesText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === '🧾 Access Logs') {
      await sendTracked(chatId, accessLogsText(25), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === '📤 Data Export') {
      setPendingAction(chatId, { type: 'admin_data_export' });
      await bot.sendMessage(chatId, '📤 Send the user ID to export.', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
      return;
    }
    if (text === '🗑 Delete User Data') {
      setPendingAction(chatId, { type: 'admin_delete_user_data' });
      await bot.sendMessage(chatId, '🗑 Send the user ID to delete all saved data.', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
      return;
    }
    if (text === '🔕 Notification Consent') {
      db.privacySecurity.notificationConsent.enabled = !db.privacySecurity.notificationConsent.enabled;
      saveDB();
      await sendTracked(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === '⏳ Auto-Hide Details') {
      db.privacySecurity.autoHideDetails = !db.privacySecurity.autoHideDetails;
      saveDB();
      await sendTracked(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === 'Two-Step Admin Protection') {
      db.privacySecurity.twoStepAdminProtection = !db.privacySecurity.twoStepAdminProtection;
      saveDB();
      await sendTracked(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === 'Data Retention Controls') {
      setPendingAction(chatId, { type: 'admin_set_retention' });
      await bot.sendMessage(chatId, '🗃 Send new retention days. Example: 30', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
      return;
    }
    if (text === 'Session / Device Awareness for Admins') {
      await sendTracked(chatId, adminSessionsText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === 'User Purchase Limit Control') {
      setPendingAction(chatId, { type: 'admin_set_purchase_limit' });
      await bot.sendMessage(chatId, '🛒 Send daily purchase limit. Send 0 to disable.', adminPanelSimpleKeyboard('aps:menu')).catch(() => {});
      return;
    }
    if (text === 'Pin Important Message') {
      db.privacySecurity.pinImportantMessages = !db.privacySecurity.pinImportantMessages;
      saveDB();
      await sendTracked(chatId, advancedPrivacySummaryText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === 'Growth Analytics') {
      await sendTracked(chatId, growthAnalyticsText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
    if (text === 'Scheduled Broadcasts') {
      await sendTracked(chatId, scheduledBroadcastsText(), privacySecurityKeyboard()).catch(() => {});
      return;
    }
  } catch (_) {}
});
...
setInterval(() => {
  try {
    const s = ensureAdvancedSecurityStore();
    const cutoffAdmin = Date.now() - Number(s.retention.adminLogsDays || 30) * 86400000;
    const cutoffAccess = Date.now() - Number(s.retention.accessLogsDays || 30) * 86400000;
    db.adminLogs = (db.adminLogs || []).filter((x) => Number(x?.at || 0) >= cutoffAdmin);
    db.privacySecurity.accessLogs = (db.privacySecurity.accessLogs || []).filter((x) => Number(x?.at || 0) >= cutoffAccess);
    for (const [uid, rec] of Object.entries(db.tempBans || {})) {
      if (Number(rec?.until || 0) <= Date.now()) delete db.tempBans[uid];
    }
    saveDB();
  } catch (_) {}
}, 60 * 60 * 1000);
...
// Admin button fallbacks for reply keyboard / missed callback routes
bot.on("message", async (msg) => {
  try {
    const chatId = msg?.chat?.id;
    const text = String(msg?.text || "").trim();
    const normText = normalizeAdminPanelButtonText(text).replace(/^[^a-z]+/, "");
    if (!chatId || !isAdmin(chatId)) return;
    if (!["🔒 Offer Toggles", "📨 Support Requests", "🛠 Maintenance Mode", "⏳ Temporary Ban", "📢 Broadcast Message"].includes(text) && !["offer toggles", "support requests", "maintenance mode", "temporary ban", "broadcast message"].includes(normText)) return;

    if (text === "🔒 Offer Toggles" || normText === "offer toggles") {
      await adminPanelSendOrEdit(chatId, "🔒 OFFER TOGGLES\n\nTap any category to open/close buying.\nYou can also allow specific user IDs to keep using closed services.", adminPanelOfferKeyboard()).catch(() => {});
      return;
    }
    if (text === "📨 Support Requests" || normText === "support requests") {
      const rows = adminOpenSupportTickets(10);
      if (!rows.length) {
        await adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard()).catch(() => {});
      } else {
        await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } }).catch(() => {});
      }
      return;
    }
    if (text === "🛠 Maintenance Mode" || normText === "maintenance mode") {
      const enabled = setMaintenanceMode(!isMaintenanceMode());
      await adminPanelSendOrEdit(chatId, `✅ Maintenance mode is now ${enabled ? "ON ⚠️" : "OFF ✅"}.`, adminPanelUsersKeyboard()).catch(() => {});
      return;
    }
    if (text === "⏳ Temporary Ban") {
      adminState.set(String(chatId), { step: "await_temp_ban", createdAt: Date.now() });
      await adminPanelSendOrEdit(chatId, "⏳ Temporary Ban\n\nSend details in this format:\n<chat_id> <duration> [reason]\n\nExamples:\n123456789 2d spam\n123456789 12h abuse\n123456789 30m too many attempts\n\nUse /cancel to stop.", adminPanelSimpleKeyboard("ap:users")).catch(() => {});
      return;
    }
    if (text === "📢 Broadcast Message" || normText === "broadcast message") {
      adminState.set(String(chatId), { step: "await_broadcast", createdAt: Date.now() });
      await adminPanelSendOrEdit(chatId, "📣 Broadcast mode ON.\n\nSend ANYTHING to broadcast:\n• text\n• photo / video\n• music/audio / voice\n• document\n• sticker\n• animation/gif\n• album (multi photos/videos as one post)\n\nSend /cancel to stop.", adminPanelSimpleKeyboard("ap:users")).catch(() => {});
      return;
    }
  } catch (_) {}
});
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data === "ap:support") {
      ...
      const rows = adminOpenSupportTickets(10);
      if (!rows.length) {
        await adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard(), q).catch(() => {});
      } else {
        await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } }, q).catch(() => {});
        }
      return;
    }
    if (data === "ap:offers") {
      await bot.answerCallbackQuery(q.id).catch(() => {});
      await adminPanelSendOrEdit(chatId, "🔒 OFFER TOGGLES\n\nTap any category to open/close buying.\nYou can also allow specific user IDs to keep using closed services.", adminPanelOfferKeyboard(), q).catch(() => {});
      return;
    }
  } catch (_) {}
});

// === Hard fallback handlers for buttons that were still not opening ===
try {
  bot.onText(/^📨 Support Requests$/i, async (msg) => {
    try {
      const chatId = msg && msg.chat && msg.chat.id;
      if (!chatId || !isAdmin(chatId)) return;
      try { clearPendingAction(chatId); } catch (_) {}
      const rows = adminOpenSupportTickets(10);
      if (!rows.length) {
        await adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard()).catch(async () => {
          await bot.sendMessage(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard()).catch(() => {});
        });
        return;
      }
      const kb = { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } };
      await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, kb).catch(async () => {
        await bot.sendMessage(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, kb).catch(() => {});
      });
    } catch (_) {}
  });
} catch (_) {}
...
try {
  bot.onText(/^🔒 Offer Toggles$/i, async (msg) => {
    try {
      const chatId = msg && msg.chat && msg.chat.id;
      if (!chatId || !isAdmin(chatId)) return;
      try { clearPendingAction(chatId); } catch (_) {}
      const text = "🔒 OFFER TOGGLES\n\nTap any category to open/close buying.\nYou can also allow specific user IDs to keep using closed services.";
      await adminPanelSendOrEdit(chatId, text, adminPanelOfferKeyboard()).catch(async () => {
        await bot.sendMessage(chatId, text, adminPanelOfferKeyboard()).catch(() => {});
      });
    } catch (_) {}
  });
...
try {
  bot.onText(/admin panel$/i, async (msg) => {
    try {
      const chatId = msg && msg.chat && msg.chat.id;
      if (!chatId || !isAdmin(chatId)) return;
      const text = String(msg && msg.text || "").trim();
      const normText = normalizeAdminPanelButtonText(text).replace(/^[^a-z]+/, "");
      if (normText !== "admin panel") return;
      try { clearPendingAction(chatId); } catch (_) {}
      await openInlineAdminPanel(chatId);
    } catch (_) {}
  });
...
try {
  bot.on("callback_query", async (q) => {
    try {
      const data = String((q && q.data) || "");
      const chatId = q && q.message && q.message.chat && q.message.chat.id;
      if (!chatId || !isAdmin(chatId)) return;
      if (data === "ap:support") {
        await bot.answerCallbackQuery(q.id).catch(() => {});
        try { clearPendingAction(chatId); } catch (_) {}
        const rows = adminOpenSupportTickets(10);
        if (!rows.length) {
          await adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard(), q).catch(async () => {
            await bot.sendMessage(chatId, "✅ No open support requests right now.", adminPanelUsersKeyboard()).catch(() => {});
          });
          return;
        }
        const kb = { reply_markup: { inline_keyboard: rows.map((t) => [{ text: `#${t.ticketId} • ${adminSafeUserName(t.chatId)} • ${String(t.status || "open").toUpperCase()}`, callback_data: `spt:view:${t.ticketId}` }]).concat([[{ text: "⬅️ Back", callback_data: "ap:users" }]]) } };
        await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, kb, q).catch(async () => {
          await bot.sendMessage(chatId, `🆘 Open Support Requests: ${rows.length}\n\nTap a ticket below to manage it.`, kb).catch(() => {});
        });
        return;
      }
