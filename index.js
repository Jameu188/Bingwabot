...
function channelPromoCfg() {
  const cfg = ensureChannelPromoStore().config;
  if (!Array.isArray(cfg.buttons)) cfg.buttons = [];
  return cfg;
}

function normalizePromoButtons(input) {
  const out = [];
  const rows = Array.isArray(input) ? input : [];
  for (const row of rows) {
    const rowOut = [];
    for (const btn of (Array.isArray(row) ? row : [])) {
      const text = String(btn && btn.text || "").trim();
      const url = normalizePromoUrl(btn && btn.url || "");
      if (text && url) rowOut.push({ text, url });
    }
...
function getChannelPromoButtons() {
  ...
  const modern = normalizePromoButtons(cfg.buttons || []);
  if (modern.length) return modern;
  const buttonText = String(cfg.buttonText || "").trim();
  const buttonUrl = normalizePromoUrl(cfg.buttonUrl || "");
  if (buttonText && buttonUrl) return [[{ text: buttonText, url: buttonUrl }]];
  return [];
}

function promoButtonsSummaryText() {
  const rows = getChannelPromoButtons();
  if (!rows.length) return "(none)";
  return rows.map((row, idx) => `${idx + 1}. ` + row.map(btn => `${btn.text} | ${btn.url}`).join(" || ")).join("\n");
}
  ...
  return {
    reply_markup: {
      inline_keyboard: rows.map(row => row.map(btn => ({
        text: String(btn.text || "Open"),
        url: wrapPromoUrlForOwner(btn.url, ownerId, sourceId)
      })))
    }
  };
}
...
function setChannelPromoButton(buttonText, buttonUrl) {
  ...
  if (Array.isArray(buttonText)) {
    const buttons = normalizePromoButtons(buttonText);
    cfg.buttons = buttons;
    if (buttons.length && buttons[0][0]) {
      cfg.buttonText = String(buttons[0][0].text || "BUY NOW");
      cfg.buttonUrl = normalizePromoUrl(buttons[0][0].url || "");
    } else {
      cfg.buttonText = "BUY NOW";
      cfg.buttonUrl = "";
    }
    saveDB();
    return;
  ...
  cfg.buttons = (cfg.buttonText && cfg.buttonUrl) ? [[{ text: cfg.buttonText, url: cfg.buttonUrl }]] : [];
  saveDB();
}
...
function channelPromoAdminText() {
  ...
  const btnSummary = promoButtonsSummaryText();
...
function promoTrackedButtonKeyboard(targetChatId) {
  ...
  const text = String(cfg.buttonText || "").trim();
  let url = String(cfg.buttonUrl || "").trim();
  ...
  try {
    const root = String(BASE_URL || "").trim().replace(/\/+$/, "");
    if (/^https:\/\/.+/i.test(root)) {
      finalUrl = `${root}/promo/go?chat=${encodeURIComponent(targetChatId)}&to=${encodeURIComponent(url)}`;
    }
  } catch (_) {}
...
async function postPromoToAllMarkedTargets() {
  ...
  for (const targetId of ids) {
    const sent = await sendConfiguredChannelPromoToTarget(targetId, { deletePrevious: false }).catch(() => false);
    if (sent) ok += 1;
    else fail += 1;
  }
...
// Fallback: always answer callback queries so buttons do not appear stuck.
bot.on("callback_query", async (q) => { try { if (q && q.id) await bot.answerCallbackQuery(q.id).catch(() => {}); } catch (_) {} });

function getChannelIdByCategory(category) {
  db = repairDB(db);
  const map = db.channelIds || {};

  const key = String(category || "").toLowerCase();

  if (key.includes("bingwa") || key.includes("data") || key.includes("unlimited")) {
    return map.data || PAYHERO_CHANNEL_ID_DATA;
  }

  if (key.includes("sms") || key.includes("bonga") || key.includes("flex")) {
    return map.sms || PAYHERO_CHANNEL_ID_SMS;
  }

  if (key.includes("minutes")) {
    return map.minutes || PAYHERO_CHANNEL_ID_MINUTES;
  }

  if (key.includes("business")) {
    return map.business || PAYHERO_CHANNEL_ID_BUSINESS;
  }

  return map.data || PAYHERO_CHANNEL_ID_DATA;
}

// ===== ADMIN SET CHANNEL UI =====
bot.on("message", async (msg) => {
  const chatId = msg.chat.id;
  const text = msg.text || "";
  ...
  if (text === "/set") {
    ...
    return bot.sendMessage(chatId, "⚙️ Select category:", {
      reply_markup: {
        inline_keyboard: [
          [{ text: "📶 Data", callback_data: "setch:data" }],
          [{ text: "✉️ SMS", callback_data: "setch:sms" }],
          [{ text: "📞 Minutes", callback_data: "setch:minutes" }],
          [{ text: "💼 Business", callback_data: "setch:business" }]
        ]
      }
    });
  }
  ...
  if (pending && pending.type === "set_channel") {
    if (chatId !== ADMIN_ID) return;

    const channelId = Number(text);
    if (!channelId) return bot.sendMessage(chatId, "Invalid ID");

    db = repairDB(db);
    db.channelIds = db.channelIds || {};
    db.channelIds[pending.category] = channelId;
...
bot.on("callback_query", (q) => {
  ...
  if (data.startsWith("setch:")) {
    if (chatId !== ADMIN_ID) return;

    const type = data.split(":")[1];

    const u = getUser(chatId);
    u.pendingAction = { type: "set_channel", category: type };
    saveDB();

    bot.sendMessage(chatId, `Send new ID for ${type}`);
  }
});
