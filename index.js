...
// ===== GET INVITE TEXT =====
if (data === "get_invite_text") {
  const normalMsgs = [
`🔥 Join *Bingwa Data Bot* and start saving on data today!
Earn points on every purchase and win weekly prizes 🏆
Join here 👉 ${refLink}`,
`🚀 Cheap bundles + reward points!
Start using Bingwa Data Bot today.
Sign up here: ${refLink}`,
`💰 Turn your data purchases into rewards!
Join now: ${refLink}`,
`📶 Fast and affordable data deals on Telegram.
Start here: ${refLink}`,
`🎁 Earn points every time you buy bundles!
Use my link: ${refLink}`
];

  const premiumMsgs = [
`🔥 Premium invite: Best deals on Bingwa Data Bot! ${refLink}`,
`🚀 Unlock cheaper bundles and rewards ${refLink}`,
`💰 Save on data and earn points ${refLink}`,
`🎁 Join and start earning ${refLink}`,
`📶 Cheap bundles here ${refLink}`,
`🏆 Weekly prizes waiting ${refLink}`,
`⚡ Join Bingwa today ${refLink}`,
`💎 Exclusive deals inside ${refLink}`,
`🔥 Cheapest bundles ${refLink}`,
`🚀 Data + rewards combo ${refLink}`,
`🎉 Start saving today ${refLink}`,
`📡 Smart users buy here ${refLink}`,
`💰 Earn points every purchase ${refLink}`,
`⚡ Fast sign up ${refLink}`,
`🎁 Rewards waiting ${refLink}`,
`🏆 Weekly wins possible ${refLink}`,
`📶 Cheap data here ${refLink}`,
`💎 Premium savings ${refLink}`,
`🚀 Join today ${refLink}`,
`🔥 Start now ${refLink}`
];

  const isPremium = user && (user.isPremium || user.isBusiness);
