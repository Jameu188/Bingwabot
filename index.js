...
function getQuickCategories(chatId) {
  ...
  if (Number(counts["SMS Offers"] || 0) >= 3) quick.push({ text: "⚡ Quick SMS", category: "SMS Offers" });
  if (Number(counts["Unlimited Deals"] || 0) >= 3) quick.push({ text: "⚡ Quick Unlimited", category: "Unlimited Deals" });
  if (Number(counts["Bingwa Deals"] || 0) >= 3) quick.push({ text: "⚡ Quick Bingwa", category: "Bingwa Deals" });
  if (Number(counts["Minutes"] || 0) >= 3) quick.push({ text: "⚡ Quick Minutes", category: "Minutes" });
  if (Number(counts["Flex Deals"] || 0) >= 3) quick.push({ text: "⚡ Quick Flex", category: "Flex Deals" });
  if (Number(counts["Bonga Points"] || 0) >= 3) quick.push({ text: "⚡ Quick Bonga", category: "Bonga Points" });
  return quick;
}
...
function pickUpsell(category, pkgLabel, price) {
  // Simple mapping. Only returns an upsell if the target package exists in the current list.
  const cat = String(category || "");
  const lbl = String(pkgLabel || "");
  const p = Number(price || 0);

  // Unlimited example requested: 2GB 24HRS (110) -> 5GB 3 DAYS (251)
  if (cat === "Unlimited Deals" && (p === 110 || lbl.includes("2GB 24HRS"))) {
    return { category: "Unlimited Deals", pkgLabel: "Ksh 251 • 5GB 3 DAYS", price: 251 };
  }

  // SMS example: 20 SMS (5) -> 200 SMS (10)
  if (cat === "SMS Offers" && (p === 5 || lbl.includes("20 SMS"))) {
    return { category: "SMS Offers", pkgLabel: "Ksh 10 • 200 SMS 24HRS", price: 10 };
  }

  // Minutes: 43 MIN 3HRS (21) -> 200 MIN 7 DAYS (250)
  if (cat === "Minutes" && (p === 21 || lbl.includes("43 MIN"))) {
    return { category: "Minutes", pkgLabel: "Ksh 250 • 200 MIN 7 DAYS", price: 250 };
  }

  // Flex: Flex 350 (20) -> Flex 1500 (255)
  if (cat === "Flex Deals" && (p === 20 || lbl.includes("Flex 350"))) {
    return { category: "Flex Deals", pkgLabel: "Ksh 255 • Flex 1500 (7 DAYS)", price: 255 };
  }
...
function mainMenuKeyboard(chatId = null) {
  ...
  // Show Buy Last Offer only if user has a last purchase
  if (chatId) {
    const u = getUser(chatId);
    if (u && u.lastPurchase && u.lastPurchase.category && u.lastPurchase.pkgLabel) {
      keyboard.splice(1, 0, ["🔁 Buy Last Offer"]);
    }
  }
...
` +
    ...
    // ✅ Save last successful purchase for 🔁 Buy Last Offer (exclude Bingwa)
    try {
      if (pending && pending.category !== "Bingwa Deals") {
        const uu = getUser(chatId);
        uu.lastPurchase = {
    ...
    const canBuyAgain = !!(uu2.lastPurchase && uu2.lastPurchase.category && uu2.lastPurchase.pkgLabel);

    const showUpgrade =
      pending &&
      pending.category === "Unlimited Deals" &&
      String(pending.pkgLabel || "").includes("2GB 24HRS");

    const inlineRows = [];
    if (showUpgrade) inlineRows.push([{ text: "🚀 Upgrade to 5GB", callback_data: "ui:upgrade_5gb" }]);
    if (canBuyAgain) inlineRows.push([{ text: "🔁 Buy Last Offer", callback_data: "ui:buy_again" }]);
    inlineRows.push([{ text: "🛒 Buy Offers", callback_data: "ui:buy_offers" }]);
