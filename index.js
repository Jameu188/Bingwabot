...
if (data && data.startsWith("withdraw_")) {
  const match = data.match(/(withdraw_accept|withdraw_decline):(.+)/);
  if (!match) return;
  ...
  const entries = Object.entries(db.pendingRedeems || {});
  const found = entries.find(([, r]) =>
    r && r.reqId === reqId && r.status === "pending" && r.type === "withdraw"
  );

  if (!found) {
    await bot.answerCallbackQuery(q.id, {
      text: "Request already handled.",
      show_alert: true,
    }).catch(() => {});
    return;
  }
