...
function repairDB(obj) {
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.isPremium !== "boolean") x.isPremium = false;
    if (typeof x.premiumExpiresAt !== "number") x.premiumExpiresAt = 0;
    if (typeof x.lastPremiumBonusWeek !== "string") x.lastPremiumBonusWeek = "";
    if (typeof x.lastPremiumReminderDay !== "string") x.lastPremiumReminderDay = "";
    if (typeof x.lastPremiumExpiredDay !== "string") x.lastPremiumExpiredDay = "";
...
if (!isSuccess) {
  ...
  return;
    ...
    if (pending) {
      // 💎 PREMIUM activation on successful callback (ResultCode/Status: true)
      if (pending.category === "PREMIUM") {
        ...
        u.isPremium = true;
        u.premiumExpiresAt = base + (30 * 24 * 60 * 60 * 1000); // 30 days
        saveDB();

        await sendTracked(
          chatId,
          `✅ *Premium Activated!* 👑\n\nYour Premium is now active for *30 days*.\n\nBenefits:\n• Referral bonus: *3%*\n• +1% bonus points on purchases\n• Weekly +10 pts bonus\n\nThank you!`,
...
// ===============================
  // 💎 PREMIUM (STK ONLY 150 KSH / 30 days)
  // ===============================

  if (/premium/i.test(text) && text.includes("💎")) {
    const active = isPremiumActive(chatId);
    const u = getUser(chatId);
    const exp = u.premiumExpiresAt ? new Date(u.premiumExpiresAt).toLocaleString("en-KE", { timeZone: "Africa/Nairobi" }) : "N/A";
    ...
    const msgText =
      ...
      `Duration: *30 days*\n\n` +
      `Benefits:\n` +
      `• Referral bonus: *3%* (normal 2%)\n` +
      `• +1% bonus points on every purchase\n` +
      `• Weekly +10 pts bonus\n` +
      `• VIP badge 👑\n\n` +
      `Status: *${active ? "ACTIVE ✅" : "NOT ACTIVE"}*\n` +
      `Expires: *${active ? exp : "—"}*\n\n` +
      `Tap below to upgrade/renew.`;

    return sendTracked(chatId, msgText, {
      parse_mode: "Markdown",
      reply_markup: {
        keyboard: [["💳 Pay 150 KSH (Premium)"], ["⬅️ Back"]],
        resize_keyboard: true
      }
    });
  }
  ...
  if (/pay\s*150\s*ksh/i.test(text) && /premium/i.test(text)) {
    const u = getUser(chatId);
    if (!u.phone) {
      return sendTracked(chatId, "⚠️ Please save your phone number first (use the phone save flow).", { ...mainMenuKeyboard(chatId) });
    }

    const ref = `PREM-${Date.now()}-${chatId}`;
    ...
    try {
      ...
      return sendTracked(chatId, "📲 STK sent for *Premium 150 KSH*. Complete payment to activate Premium.", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
    } catch (e) {
      deletePendingPayment(ref);
      return sendTracked(chatId, "❌ STK failed. Please try again.", { ...mainMenuKeyboard(chatId) });
    }
  }
...
// ===================== PREMIUM WEEKLY BONUS =====================
const PREMIUM_WEEKLY_BONUS = 10;
...
setInterval(async () => {
  try {
    ...
    for (const [cid, u] of Object.entries(db.users || {})) {
      ...
      if (!u.premiumExpiresAt || Date.now() >= u.premiumExpiresAt) {
        u.isPremium = false;
        u.premiumExpiresAt = 0;
        continue;
      }

      if (u.lastPremiumBonusWeek === wk) continue;

      u.points = Number(u.points || 0) + PREMIUM_WEEKLY_BONUS;
      u.lastPremiumBonusWeek = wk;

      try {
        await bot.sendMessage(Number(cid), `🎁 Premium Weekly Bonus: +${PREMIUM_WEEKLY_BONUS} pts added. 👑`, { ...mainMenuKeyboard(Number(cid)) });
        await sleep(35);
      } catch (_) {}
    }
...
const PREMIUM_EXPIRY_REMINDER_DAYS = 3;

setInterval(async () => {
  try {
    db = repairDB(db);
    const today = kenyaDayKey();

    for (const [cid, u] of Object.entries(db.users || {})) {
      if (!u) continue;

      // If premium expired, auto-downgrade + notify once per day
      if (u.isPremium && u.premiumExpiresAt && Date.now() >= u.premiumExpiresAt) {
        u.isPremium = false;
        u.premiumExpiresAt = 0;

        if (u.lastPremiumExpiredDay !== today) {
          u.lastPremiumExpiredDay = today;
          try {
            // eslint-disable-next-line no-await-in-loop
            await bot.sendMessage(
              Number(cid),
              "❌ Your Premium has expired.\n\nRenew Premium to continue enjoying VIP benefits. 👑",
              { ...mainMenuKeyboard(Number(cid)) }
            );
            // eslint-disable-next-line no-await-in-loop
            await sleep(35);
      ...
      // Reminder 3 days before expiry (only while active)
      if (u.isPremium && u.premiumExpiresAt) {
        const msLeft = u.premiumExpiresAt - Date.now();
        const daysLeft = Math.ceil(msLeft / (24 * 60 * 60 * 1000));

        if (daysLeft <= PREMIUM_EXPIRY_REMINDER_DAYS && daysLeft > 0) {
          if (u.lastPremiumReminderDay !== today) {
            u.lastPremiumReminderDay = today;
            try {
              // eslint-disable-next-line no-await-in-loop
              await bot.sendMessage(
                Number(cid),
                `⏳ Premium expires in ${daysLeft} day(s).\n\nRenew now to keep benefits (Referral 3%, Weekly bonus, VIP badge). 👑`,
                { ...mainMenuKeyboard(Number(cid)) }
              );
              // eslint-disable-next-line no-await-in-loop
              await sleep(35);
