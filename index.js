...
async function broadcastWeeklyDrawLaunch(force = false) {
  ...
  try {
    ...
    if (!recentlySentSame) {
      await sendToChat(getPrimaryForceJoinChannel());
...
/* ===================== FORCE JOIN CHANNELS ===================== */
const FORCE_CHANNELS_FILE = path.join(__dirname, "force_channels.json");
const DEFAULT_FORCE_JOIN_CHANNEL = "@bingwadata";
const DEFAULT_FORCE_JOIN_URL = "https://t.me/Bingwadatabot";
...
function ensureForceChannelsFile() {
  try {
    if (!fs.existsSync(FORCE_CHANNELS_FILE)) {
      fs.writeFileSync(FORCE_CHANNELS_FILE, JSON.stringify([{ username: DEFAULT_FORCE_JOIN_CHANNEL, url: DEFAULT_FORCE_JOIN_URL }], null, 2));
    }
  } catch (_) {}
}

function normalizeForceChannelInput(input) {
  ...
  raw = raw.replace(/^@+/, "");
  if (!/^[A-Za-z0-9_]{5,}$/.test(raw)) return null;
  return "@" + raw;
}

function forceChannelUrl(username) {
  const u = String(username || "").replace(/^@/, "");
  return `https://t.me/${u}`;
}

function getForceJoinChannels() {
  ensureForceChannelsFile();
  try {
    const raw = fs.readFileSync(FORCE_CHANNELS_FILE, "utf8");
    ...
    if (Array.isArray(parsed)) {
      for (const item of parsed) {
        const username = typeof item === "string"
          ? normalizeForceChannelInput(item)
          : normalizeForceChannelInput(item && item.username);
        if (!username) continue;
        if (seen.has(username)) continue;
        seen.add(username);
        out.push({ username, url: forceChannelUrl(username) });
      }
    }

    if (!out.length) {
      out.push({ username: DEFAULT_FORCE_JOIN_CHANNEL, url: DEFAULT_FORCE_JOIN_URL });
      fs.writeFileSync(FORCE_CHANNELS_FILE, JSON.stringify(out, null, 2));
    }

    return out;
  } catch (_) {
    return [{ username: DEFAULT_FORCE_JOIN_CHANNEL, url: DEFAULT_FORCE_JOIN_URL }];
  }
}
...
function saveForceJoinChannels(list) {
  ...
  for (const item of Array.isArray(list) ? list : []) {
    const username = normalizeForceChannelInput(item && item.username ? item.username : item);
    if (!username || seen.has(username)) continue;
    seen.add(username);
    clean.push({ username, url: forceChannelUrl(username) });
  }
  fs.writeFileSync(FORCE_CHANNELS_FILE, JSON.stringify(clean, null, 2));
}

function getPrimaryForceJoinChannel() {
  const list = getForceJoinChannels();
  return list.length ? list[0].username : DEFAULT_FORCE_JOIN_CHANNEL;
}
...
async function ensureJoinedChannel(chatId) {
  ...
  for (const ch of channels) {
    try {
      const m = await bot.getChatMember(ch.username, chatId);
      const s = m?.status;
...
async function canUserAddForceChannel(requesterId, channelUsername) {
  try {
    const me = await bot.getMe();
    const requester = await bot.getChatMember(channelUsername, requesterId);
    const botMember = await bot.getChatMember(channelUsername, me.id);
    const requesterOk = ["administrator", "creator"].includes(String(requester?.status || ""));
    const botOk = ["administrator", "creator"].includes(String(botMember?.status || ""));
    return { ok: requesterOk && botOk, requesterOk, botOk, botUsername: me?.username || "" };
  } catch (e) {
    return { ok: false, requesterOk: false, botOk: false, botUsername: "" };
  }
}
...
async function sendJoinGate(chatId) {
  ...
  const channels = getForceJoinChannels();
  const rows = channels.map((ch) => [{ text: `✅ Join ${ch.username}`, url: ch.url }]);
  rows.push([{ text: "🔄 I Joined", callback_data: "forcejoin:check" }]);
...
if (isStartCmd) {
  ...
  if (/^\/addchannel(?:@\w+)?\s+(.+)$/i.test(text)) {
    ...
    const m = text.match(/^\/addchannel(?:@\w+)?\s+(.+)$/i);
    const channelUsername = normalizeForceChannelInput(m && m[1]);
    if (!channelUsername) {
      await bot.sendMessage(chatId, "Usage: /addchannel @channelusername").catch(() => {});
      return;
    }

    const check = await canUserAddForceChannel(chatId, channelUsername);
    if (!check.requesterOk) {
      await bot.sendMessage(chatId, "❌ You must be an admin or owner in that channel before adding it.").catch(() => {});
      return;
    }
    if (!check.botOk) {
      const suffix = check.botUsername ? ` Add @${check.botUsername} as admin first, then try again.` : " Add the bot as admin first, then try again.";
      await bot.sendMessage(chatId, `❌ Bot is not admin in ${channelUsername}.${suffix}`).catch(() => {});
      return;
    }

    const channels = getForceJoinChannels();
    if (channels.some((x) => x.username === channelUsername)) {
      await bot.sendMessage(chatId, `⚠️ ${channelUsername} is already in force join list.`).catch(() => {});
      return;
    }

    channels.push({ username: channelUsername, url: forceChannelUrl(channelUsername) });
    saveForceJoinChannels(channels);
    await bot.sendMessage(chatId, `✅ Added force join channel: ${channelUsername}`).catch(() => {});
    return;
  }
  ...
  if (/^\/removechannel(?:@\w+)?\s+(.+)$/i.test(text)) {
    ...
    const m = text.match(/^\/removechannel(?:@\w+)?\s+(.+)$/i);
    const channelUsername = normalizeForceChannelInput(m && m[1]);
    if (!channelUsername) {
    ...
    const channels = getForceJoinChannels();
    const filtered = channels.filter((x) => x.username !== channelUsername);
    if (filtered.length === channels.length) {
      await bot.sendMessage(chatId, `⚠️ ${channelUsername} is not in force join list.`).catch(() => {});
      return;
    }

    saveForceJoinChannels(filtered);
    await bot.sendMessage(chatId, `✅ Removed force join channel: ${channelUsername}`).catch(() => {});
    return;
  }
...
${channels.map((x, i) => `${i + 1}. ${x.username}`).join("\n")}`
