...
if (data === "biz_activate") {
  ...
  const header = isBusinessActive(chatId)
    ? tr(chatId,
        ...
        "⚙️ *Simamia Akaunti ya Biashara*\n\nGusa mpango wako wenye alama ya ✅ kuuzima, au chagua mpango mwingine kujisajili."
      )
    : tr(chatId,
        "✅ *Activate Business Account*\n\nChoose a plan to subscribe:",
        "✅ *Washa Akaunti ya Biashara*\n\nChagua mpango wa kujisajili:"
      );
  ...
  try {
    ...
    await safeEditMessageText(
      ...
      tr(
        ...
        `✅ Ombi la STK limetumwa. Tafadhali weka PIN yako ya M-Pesa kwenye simu yako.\n\n⏳ Baada ya malipo, utapata uthibitisho hapa.`
      ),
      { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "biz_back" }, { text: "🏠 Main Menu", callback_data: "help:home" }]] } }
    ).catch(() => {});
  } catch (e) {
    ...
    await safeEditMessageText(
      chatId,
      q.message?.message_id,
      tr(chatId, "❌ Failed to send STK. Please try again.", "❌ Imeshindikana kutuma STK. Tafadhali jaribu tena."),
      { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back", callback_data: "biz_back" }, { text: "🏠 Main Menu", callback_data: "help:home" }]] } }
    ).catch(() => {});
  }
...
if (data === "biz:backup_once") {
  ...
  try {
    ...
    try { const nu = getUser(chatId); nu.lang = keepLang; saveDB(); } catch (_) {} // re-create blank user shell + keep language
    try { saveDB(); } catch (_) {}
    ...
    switch (key) {
      case "buy":
        return (
          ...
          "1) Gonga *🛒 Nunua Ofa*\n" +
          "2) Chagua aina ya ofa (mfano: 📦 Bingwa / ✉️ SMS / 📞 Dakika / ♾ Bila Kikomo)\n" +
          "3) Chagua ofa unayotaka\n" +
          "4) Weka nambari ya simu (07XXXXXXXX)\n" +
          "5) Thibitisha kisha lipa kwa M-Pesa STK\n\n" +
...
function translateOutgoing(chatId, text) {
  ...
  const repl = [
    ["✅ Backup key saved. Use /restorebackup YOUR_KEY whenever you want to restore.", "✅ Funguo ya hifadhi imehifadhiwa. Tumia /restorebackup FUNGUO_YAKO wakati wowote unapotaka kurejesha."],
    ["🔐 Send your backup restore key now.", "🔐 Tuma funguo yako ya kurejesha sasa."],
    ["Example:", "Mfano:"],
    ["⏳ Restoring your backup...", "⏳ Inarejesha hifadhi yako..."],
    ["❌ Restore failed: Invalid backup key.", "❌ Imeshindikana kurejesha: Funguo ya hifadhi si sahihi."],
    ["❌ Restore failed: This backup key has already been used.", "❌ Imeshindikana kurejesha: Funguo hii ya hifadhi tayari imetumika."],
    ["❌ Restore failed: This backup key has expired.", "❌ Imeshindikana kurejesha: Funguo hii ya hifadhi imeisha muda."],
    ["✅ Backup restored successfully.", "✅ Hifadhi imerejeshwa kikamilifu."],
    ["⚙️ *Manage Business Account*", "⚙️ *Simamia Akaunti ya Biashara*"],
    ["✅ *Activate Business Account*", "✅ *Washa Akaunti ya Biashara*"],
    ["Tap your ✅ active plan to deactivate, or choose another plan to subscribe.", "Gusa mpango wako wenye alama ya ✅ kuuzima, au chagua mpango mwingine kujisajili."],
    ["Choose a plan to subscribe:", "Chagua mpango wa kujisajili:"],
    ["✅ STK sent. Please enter your M-Pesa PIN on your phone.", "✅ Ombi la STK limetumwa. Tafadhali weka PIN yako ya M-Pesa kwenye simu yako."],
    ["⏳ After payment, you will receive confirmation here.", "⏳ Baada ya malipo, utapata uthibitisho hapa."],
    ["❌ Failed to send STK. Please try again.", "❌ Imeshindikana kutuma STK. Tafadhali jaribu tena."],
    ["Jinsi ya kununua ofa", "Jinsi ya kununua ofa"],
    ["Gonga *🛒 Nunua Ofa*", "Gusa *🛒 Nunua Ofa*"],
    ["Chagua kategoria", "Chagua aina ya ofa"],
    ["📦 Bingwa Deals / ✉️ SMS / 📞 Minutes / ♾ Unlimited", "📦 Bingwa / ✉️ SMS / 📞 Dakika / ♾ Bila Kikomo"],
    ["Weka namba ya simu", "Weka nambari ya simu"],
    ["Thibitisha na ulipie kwa M-Pesa STK", "Thibitisha kisha lipa kwa M-Pesa STK"],
    ["Fast Purchase", "Ununuzi wa Haraka"],
    ["Business / Admin", "Biashara / Admin"],
    ["This is the business settings area (backup/auto-delete etc). If you can't see it, your account is not business.", "Hii ni sehemu ya mipangilio ya biashara (hifadhi, kufuta kiotomatiki n.k.). Kama huioni, akaunti yako si ya biashara."],
    ["Activate", "Washa"]
...
if (text === "/mybackup" || text === "/backup" || text === "/mybackupreset" || text === "/backupreset") {
  try {
    ...
    const ok = wipeUserData(chatId);
    try { const nu = getUser(chatId); nu.lang = keepLang; saveDB(); } catch (_) {}
    try { saveDB(); } catch (_) {}
