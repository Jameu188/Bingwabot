...
const SUPPORT_TYPES = {
  ...
  nointernet: "Bought, received but no internet",
  ...
  return {
    reply_markup: {
      keyboard: [
        ...
        ["3️⃣ Bought, received but no internet", "4️⃣ Wrong number used"],
        ["5️⃣ Withdrawal issue", "6️⃣ Referral / points issue"],
        ["7️⃣ Gmail issue", "8️⃣ Talk to admin"],
        ["⬅ Back", "🏠 Main Menu"],
      ],
...
function isSupportFinalStatus(status) {
  const s = String(status || "").toLowerCase().trim();
  return s === "resolved" || s === "deleted" || s === "closed";
}

function supportTicketKeyboard(ticketId, role = "admin") {
  if (role === "user") {
    const t = (db.supportTickets || {})[String(ticketId)] || null;
    const finalState = isSupportFinalStatus(t?.status);
    const inline_keyboard = [
      [{ text: "📂 My Tickets", callback_data: "uspt:list:0" }],
      [{ text: "💬 Reply to Support", callback_data: `uspt:reply:${ticketId}` }],
    ];
    if (!finalState) inline_keyboard.unshift([{ text: "🔁 Raise Again", callback_data: `uspt:raise:${ticketId}` }]);
    return { reply_markup: { inline_keyboard } };
  }
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "✅ Resolve", callback_data: `spt:resolve:${ticketId}` }, { text: "🗑 Delete", callback_data: `spt:delete:${ticketId}` }],
...
function supportAdminListKeyboard(page = 0, total = 0) {
  const maxPage = Math.max(0, Math.ceil(Number(total || 0) / SUPPORT_PAGE_SIZE) - 1);
  const prev = Math.max(0, Number(page || 0) - 1);
  const next = Math.min(maxPage, Number(page || 0) + 1);
  return {
    reply_markup: {
      inline_keyboard: [
        [
          { text: "⬅ Prev", callback_data: `sptlist:${prev}` },
          { text: `📄 ${Math.min(maxPage + 1, Number(page || 0) + 1)}/${Math.max(1, maxPage + 1)}`, callback_data: "noop:support" },
          { text: "Next ➡", callback_data: `sptlist:${next}` }
        ],
        [
          { text: "⬅ Back", callback_data: "ap:root" },
...
function supportUserHubKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [
          { text: "⚡ STK not received", callback_data: "uspt:new:stk" },
          { text: "💸 Paid not received", callback_data: "uspt:new:paid" }
        ],
        [
          { text: "🌐 No internet", callback_data: "uspt:new:nointernet" },
          { text: "📱 Wrong number", callback_data: "uspt:new:wrong" }
        ],
        [
          { text: "💰 Withdrawal", callback_data: "uspt:new:withdraw" },
          { text: "🎁 Referral / Points", callback_data: "uspt:new:referral" }
        ],
        [
          { text: "📧 Gmail", callback_data: "uspt:new:gmail" },
          { text: "👨‍💼 Talk to admin", callback_data: "uspt:new:admin" }
        ],
        [
          { text: "📂 My Tickets", callback_data: "uspt:list:0" },
          { text: "📌 Latest Status", callback_data: "uspt:status" }
        ]
      ]
    }
  };
...
function supportIssueKeyFromText(text) {
  ...
  if (t === "3️⃣ Bought, received but no internet") return "nointernet";
  if (t === "4️⃣ Wrong number used") return "wrong";
  if (t === "5️⃣ Withdrawal issue") return "withdraw";
  if (t === "6️⃣ Referral / points issue") return "referral";
  if (t === "7️⃣ Gmail issue") return "gmail";
  if (t === "8️⃣ Talk to admin") return "admin";
  return "";
}
...
function supportAutoReply(chatId, issueKey) {
  try {
    ...
    if (issueKey === "nointernet") {
      return "ℹ️ If the bundle arrived but internet is not working, send the phone number used plus the exact offer so admin can check it faster.";
    }
    if (issueKey === "withdraw") {
...
function supportMessageKind(msg = {}) {
...
function supportMessageSummary(msg = {}) {
  const kind = supportMessageKind(msg);
  const body = String(msg.text || msg.caption || "").trim();
  const iconMap = { text: "📝", photo: "🖼", video: "🎥", voice: "🎤", audio: "🎵", document: "📎", sticker: "💟", animation: "🎞", video_note: "📹", contact: "👤", location: "📍" };
  const pretty = { text: "Text", photo: "Photo", video: "Video", voice: "Voice", audio: "Audio", document: "Document", sticker: "Sticker", animation: "GIF", video_note: "Video note", contact: "Contact", location: "Location" };
  if (kind === "text") return body;
  return `${iconMap[kind] || "📩"} ${pretty[kind] || kind}${body ? `
${body}` : ""}`.trim();
}

function adminOpenSupportTickets(limit = SUPPORT_PAGE_SIZE, page = 0) {
  ensureSupportStore();
  const all = Object.values(db.supportTickets || {})
    .filter((x) => !isSupportFinalStatus(x?.status))
    .sort((a, b) => Number(b.updatedAt || b.createdAt || 0) - Number(a.updatedAt || a.createdAt || 0));
  const start = Math.max(0, Number(page || 0)) * Number(limit || SUPPORT_PAGE_SIZE);
  return { total: all.length, page: Math.max(0, Number(page || 0)), rows: all.slice(start, start + Number(limit || SUPPORT_PAGE_SIZE)) };
}

function mySupportTickets(chatId, limit = SUPPORT_PAGE_SIZE, page = 0) {
  ensureSupportStore();
  const all = Object.values(db.supportTickets || {})
...
function userTicketInlineKeyboard(ticketId) {
  return supportTicketKeyboard(ticketId, "user");
}

async function sendSupportAdminTicketCard(adminChatId, ticketId) {
  ensureSupportStore();
  const t = db.supportTickets[String(ticketId)];
  if (!t) return null;
  return bot.sendMessage(Number(adminChatId), supportTicketText(t), supportTicketKeyboard(ticketId)).catch(() => null);
}

async function sendUserTicketList(chatId, page = 0, ctx = null) {
  const view = mySupportTickets(chatId, SUPPORT_PAGE_SIZE, page);
  if (!view.total) {
    return bot.sendMessage(chatId, "📂 You have no support tickets yet.", supportUserHubKeyboard()).catch(() => {});
  }
  await bot.sendMessage(chatId, `📂 My Tickets

Showing ${view.rows.length} of ${view.total} ticket(s).`).catch(() => {});
  for (const t of view.rows) {
    await bot.sendMessage(chatId, supportTicketText(t), userTicketInlineKeyboard(t.ticketId)).catch(() => {});
  }
  const maxPage = Math.max(0, Math.ceil(view.total / SUPPORT_PAGE_SIZE) - 1);
  return bot.sendMessage(chatId, `Page ${view.page + 1}/${maxPage + 1}`, {
    reply_markup: {
      inline_keyboard: [
        [
          { text: "⬅ Prev", callback_data: `uspt:list:${Math.max(0, view.page - 1)}` },
          { text: "Next ➡", callback_data: `uspt:list:${Math.min(maxPage, view.page + 1)}` }
        ],
        [{ text: "🆘 Support Menu", callback_data: "uspt:menu" }]
      ]
    }
  }).catch(() => {});
}

async function sendAdminSupportList(chatId, page = 0, q = null) {
  const view = adminOpenSupportTickets(SUPPORT_PAGE_SIZE, page);
  if (!view.total) {
    return adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelRootKeyboard(), q);
  }
  await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests

Showing ${view.rows.length} of ${view.total} open ticket(s). Each request appears below as a separate card.`, supportAdminListKeyboard(view.page, view.total), q);
  for (const t of view.rows) {
...
async function deliverSupportMessageToUser(ticketId, sourceMsg, extraText = "") {
  ensureSupportStore();
  const t = db.supportTickets[String(ticketId)];
  if (!t) return false;
  const targetChatId = Number(t.chatId);
  try {
    await bot.copyMessage(targetChatId, sourceMsg.chat.id, sourceMsg.message_id).catch(() => {});
    await bot.sendMessage(targetChatId, `💬 Support reply for ${ticketId}${extraText ? `

${extraText}` : ""}`, userTicketInlineKeyboard(ticketId)).catch(() => {});
    return true;
  } catch (_) {
    return false;
  }
...
async function forwardUserSupportReplyToAdmin(ticketId, userMsg, note = "") {
  ensureSupportStore();
  const t = db.supportTickets[String(ticketId)];
  if (!t) return false;
  try {
    await bot.copyMessage(ADMIN_ID, userMsg.chat.id, userMsg.message_id).catch(() => {});
    await bot.sendMessage(ADMIN_ID,
      `💬 *User Reply on ${mdEscape(ticketId)}*
` +
      `User: ${mdEscape(String(t.userName || userMsg.chat.id))}
` +
      `Chat ID: \`${mdEscape(String(t.chatId || ""))}\`
` +
      `Status: *${mdEscape(String(t.status || "Pending"))}*` +
      `${note ? `
Note: ${mdEscape(note)}` : ""}`,
      { parse_mode: "Markdown", ...supportTicketKeyboard(ticketId) }
    ).catch(() => {});
    return true;
  } catch (_) {
    return false;
...
function createSupportTicket(chatId, issueKey, details, meta = {}) {
  ensureSupportStore();
  ...
  db.supportTickets[ticketId] = {
    ...
    adminReply: "",
    lastUserMessageType: String(meta.messageType || "text"),
    raisedCount: 0,
  };
  saveDB();
...
if (data.startsWith("admin:")) {
  ...
  if (action === "support") {
    await bot.answerCallbackQuery(q.id).catch(() => {});
    await sendAdminSupportList(chatId, 0).catch(() => {});
    return;
  }
}
...
if (data === "profile:support") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await bot.sendMessage(chatId, `🆘 Support Center\n\nChoose an issue below or open your tickets.`, supportUserHubKeyboard()).catch(() => {});
  return;
}

if (data === "profile:mytickets") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await sendUserTicketList(chatId, 0).catch(() => {});
  return;
}
...
if (data === "noop:support") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  return;
}

if (data.startsWith("sptlist:")) {
  if (!isAdmin(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "Not authorized." }).catch(() => {});
    return;
  }
  const page = Math.max(0, Number(String(data).split(":")[1] || 0));
  await bot.answerCallbackQuery(q.id).catch(() => {});
  await sendAdminSupportList(chatId, page, q).catch(() => {});
  return;
}

if (data.startsWith("uspt:")) {
  ...
  if (action === "status") {
    ...
    if (!t) {
      await bot.sendMessage(chatId, "📌 No support ticket found yet.", supportUserHubKeyboard()).catch(() => {});
      return;
    }
    await bot.sendMessage(chatId, `📌 Ticket Status

Ticket ID: ${t.ticketId}
Type: ${t.issueLabel}
Status: ${t.status}
Updated: ${kenyaDateTimeFromTs(Number(t.updatedAt || t.createdAt || 0))}`, userTicketInlineKeyboard(t.ticketId)).catch(() => {});
    return;
  }
  if (action === "list") {
    await sendUserTicketList(chatId, Math.max(0, Number(value || 0))).catch(() => {});
    return;
  }
  if (action === "new") {
    setPendingAction(chatId, { type: "support_issue", issueKey: value });
    const auto = supportAutoReply(chatId, value);
    await bot.sendMessage(chatId, `${auto}

${supportPromptFor(value)}

You can send text, photo, video, voice, document or any proof. Send /cancel to stop.`, supportStatusKeyboard()).catch(() => {});
    return;
  }
  if (action === "reply") {
    ensureSupportStore();
    const t = db.supportTickets[value];
    if (!t || String(t.chatId || "") !== String(chatId)) {
      await bot.sendMessage(chatId, "❌ Ticket not found.").catch(() => {});
      return;
    }
    setPendingAction(chatId, { type: "support_user_reply", ticketId: value });
    await bot.sendMessage(chatId, `💬 Send your reply for ticket ${value}. You can send text, image, video, voice, document, sticker, or GIF.
Use /cancel to stop.`, userTicketInlineKeyboard(value)).catch(() => {});
    return;
  }
  if (action === "raise") {
    ensureSupportStore();
    ...
    if (isSupportFinalStatus(t.status)) {
      await bot.sendMessage(chatId, "✅ This ticket is already resolved, so Raise Again is hidden.", userTicketInlineKeyboard(value)).catch(() => {});
      return;
    }
    t.status = "Raised Again";
    t.raisedCount = Number(t.raisedCount || 0) + 1;
    t.updatedAt = Date.now();
    saveDB();
    await bot.sendMessage(ADMIN_ID, `🚨 Ticket ${t.ticketId} was raised again by user ${t.userName} (${t.chatId}).`, supportTicketKeyboard(t.ticketId)).catch(() => {});
    await bot.sendMessage(chatId, `✅ Ticket ${t.ticketId} has been raised again to support.`, userTicketInlineKeyboard(t.ticketId)).catch(() => {});
    return;
  }
}
...
if (data.startsWith("spt:")) {
  ...
  if (action === "resolve") {
    ...
    await bot.sendMessage(Number(t.chatId), `✅ Your support ticket ${ticketId} has been resolved.`, userTicketInlineKeyboard(ticketId)).catch(() => {});
    try {
  ...
  if (action === "delete" || action === "close") {
    ...
    await bot.sendMessage(Number(t.chatId), `❌ Your support ticket ${ticketId} was removed from the active support queue.`, userTicketInlineKeyboard(ticketId)).catch(() => {});
    try {
      await bot.editMessageText(supportTicketText(t), {
        chat_id: q.message.chat.id,
        message_id: q.message.message_id,
  ...
  if (action === "waiting") {
    ...
    await bot.sendMessage(Number(t.chatId), `⏳ Your support ticket ${ticketId} is waiting / under review.`, userTicketInlineKeyboard(ticketId)).catch(() => {});
    try {
      await bot.editMessageText(supportTicketText(t), {
        chat_id: q.message.chat.id,
        message_id: q.message.message_id,
  ...
  if (action === "reply") {
    ...
    await bot.sendMessage(chatId, `💬 Send your reply for ticket ${ticketId}. You can send text, photo, video, voice, document, sticker, or GIF. Use /cancel to stop.`).catch(() => {});
    return;
  }
}
...
`;
  ...
  return sendTracked(chatId, msg, {
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
        [{ text: "🆘 Support / Report Issue", callback_data: "profile:support" }],
        [{ text: "📂 My Tickets", callback_data: "profile:mytickets" }, { text: "🏠 Main Menu", callback_data: "profile:home" }]
      ]
    }
  });
}
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && act.type === "support_user_reply") {
    if (/^\/cancel$/i.test(text) || text === "❌ Cancel") {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Reply cancelled.", supportStatusKeyboard());
      return;
    }
    ...
    if (!t || String(t.chatId || "") !== String(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Ticket not found.", supportStatusKeyboard());
      return;
    }

    t.status = "User Replied";
    t.updatedAt = Date.now();
    saveDB();
    clearPendingAction(chatId);

    await forwardUserSupportReplyToAdmin(ticketId, msg).catch(() => {});
    await sendTracked(chatId, `✅ Your reply for ${ticketId} has been sent to support.`, userTicketInlineKeyboard(ticketId));
    return;
  }
...
if (text === "📂 My Tickets") {
  return sendUserTicketList(chatId, 0);
}
