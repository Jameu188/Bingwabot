...
function clearStaleSession(chatId, opts = {}) {
  try {
    const s = sessions.get(chatId);
    const ttl = Number(ANTI_SPAM.SESSION_TTL_MS || (15 * 60 * 1000));
    const lastTouch = Number(s?.lastActiveAt || s?.createdAt || 0);
    if (s && lastTouch && (Date.now() - lastTouch > ttl)) {
      sessions.delete(chatId);
      sessionHistory.delete(String(chatId));
      try { clearPendingAction(chatId); } catch (_) {}
      if (opts && opts.notify) {
        Promise.resolve()
          .then(() => clearTrackedBotMessages(chatId))
          .then(() => sendTracked(chatId, "⏱️ Session expired. Returning to main menu.", { ...mainMenuKeyboard(chatId) }))
          .catch(() => {});
      }
      return true;
    }
...
setInterval(() => {
  try {
    for (const [chatId, s] of sessions.entries()) {
      const ttl = Number(ANTI_SPAM.SESSION_TTL_MS || (15 * 60 * 1000));
      const lastTouch = Number(s?.lastActiveAt || s?.createdAt || 0);
      if (!lastTouch || (Date.now() - lastTouch <= ttl)) continue;
      clearStaleSession(chatId, { notify: true });
    }
  } catch (_) {}
}, 30 * 1000);
...
function getCurrentReceiveDisplay(chatId, session = null) {
  try {
    const current = String(session?.receivePhone254 || getUserReceivePhone(chatId) || getUserPhone(chatId) || "");
    return current ? maskPhone(current) : "Not set";
  } catch (_) {
    return "Not set";
  }
}

function buildReceiveNumberPrompt(chatId, session = null) {
  const current = getCurrentReceiveDisplay(chatId, session);
  return `📥 Send your RECEIVE number:
Current: ${current}
• 07XXXXXXXX
• 01XXXXXXXX
• 2547XXXXXXXX
...
await sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
  ...
  addPendingPayment(ref, { chatId, category: s.category, pkgLabel: pkg.label, phone254: phone, receivePhone254: receivePhone, price: pkg.price });
...
function confirmKeyboard(hasSavedPhone, hasReceivePhone = true) {
  const rows = [];
  if (hasSavedPhone) rows.push(["✅ Proceed"]);
...
async function proceedPurchase(chatId, s) {
  ...
  if (!savedPhone) {
    ...
    return sendTracked(
      ...
      confirmKeyboard(false, !!(s.receivePhone254 || getUserReceivePhone(chatId)))
    );
  }
...
// Inline action buttons (Upgrade + Buy Again + Buy Offers)
    ...
    try {
      await notifyAdmin(formatAdminConfirmedReceipt(chatId, pending, whenTs, 0, externalRef));
    } catch (_) {}
...
// ===================== PURCHASE FLOW =====================
  ...
  if (!s) {
    ...
    if (maybeCategory && PACKAGES[maybeCategory]) {
      s = { step: "category", category: null, pkgKey: null, createdAt: Date.now(), lastActiveAt: Date.now() };
      sessions.set(chatId, s);
    } else {
      return;
    }
  ...
  if (Date.now() - Number(s.lastActiveAt || s.createdAt || Date.now()) > Number(ANTI_SPAM.SESSION_TTL_MS || (15 * 60 * 1000))) {
    clearStaleSession(chatId, { notify: true });
    return;
  }
    ...
    if (s.step === "confirm") {
      ...
      if (isBtn(text, "📥 Change Receive Number", "📥 Badilisha Namba ya Kupokea")) {
        ...
        return sendTracked(
          chatId,
          `📥 Send your RECEIVE number:\nCurrent: ${maskPhone(getUserReceivePhone(chatId) || getUserPhone(chatId) || "") || "Not set"}\n• 07XXXXXXXX\n• 01XXXXXXXX\n• 2547XXXXXXXX\n• 2541XXXXXXXX`,
          confirmKeyboard(!!getUserPhone(chatId), false)
        );
      }
...
await sendTracked(
  ...
  {
    ...confirmKeyboard(true, !!(s.receivePhone254 || getUserReceivePhone(chatId))),
  }
);
return;
}
...
return sendTracked(
  ...
  confirmKeyboard(!!getUserPhone(chatId), !!(s.receivePhone254 || getUserReceivePhone(chatId)))
);
}
...
• 📞 Change Payment Number
• 📥 Change Receive Number`,
      { parse_mode: "Markdown", ...confirmKeyboard(!!getUserPhone(chatId), true) }
    );
  }
  if (text === "🏠 Main Menu") {
    sessions.delete(chatId);
...
Tap ✅ Proceed to continue.`,
    {
      parse_mode: "Markdown",
      ...confirmKeyboard(!!getUserPhone(chatId), true),
    }
  );
}
...
• 📞 Change Payment Number
• 📥 Change Receive Number`,
      { parse_mode: "Markdown", ...confirmKeyboard(!!getUserPhone(chatId), !!(s.receivePhone254 || getUserReceivePhone(chatId))) }
    );
  }
  if (text === "🏠 Main Menu") {
    sessions.delete(chatId);
  ...
  if (isBtn(text, "📥 Change Receive Number", "📥 Badilisha Namba ya Kupokea")) {
    ...
    return sendTracked(
      chatId,
      buildReceiveNumberPrompt(chatId, s),
      confirmKeyboard(!!getUserPhone(chatId), false)
    );
  }
  const phone = normalizePhone(text);
