...
function weeklyDrawQualificationsInfoLines() {
  ...
  if (qcfg.ageDays?.enabled) add(<code>Stay at least ${Number(qcfg.ageDays.days || 0)} day(s) in the bot<code>);
  if (qcfg.purchases?.enabled) add(<code>${Number(qcfg.purchases.count || 0)}+ successful purchases of ${Number(qcfg.purchases.minAmount || 20)} KSH or more<code>);
  if (qcfg.activeReferrals?.enabled) add(<code>${Number(qcfg.activeReferrals.count || 0)} active referrals (referred users with at least 1 successful purchase)<code>);
  if (qcfg.spend?.enabled) add(<code>Spend at least ${Number(qcfg.spend.amount || 0)} KSH (successful purchases only)<code>);
  if (qcfg.withdraw?.enabled) add(<code>${Number(qcfg.withdraw.count || 0)} withdrawal request(s)<code>);
  if (qcfg.offer?.enabled) {
    const match = String(qcfg.offer.match || "").trim();
    const need = Number(qcfg.offer.count || 1);
    add(match ? <code>Buy "${match}" ${need} time(s)<code> : <code>Buy the required offer (set by admin)<code>);
  }
...
function weeklyDrawPurchasesCountText() {
  ...
  if (qcfg?.purchases?.enabled) {
    return <code>✅ Purchases ≥${Number(qcfg.purchases.minAmount || 20)} KSH count toward qualification.<code>;
  }
  return <code>✅ Any successful purchase earns tickets.<code>;
}
function weeklyDrawMoreText() {
  ...
  const prizesTxt = <code>🥇 1st: ${first} KSH
🥈 2nd: ${second} KSH
🥉 3rd: ${third} KSH
💰 Total: ${total} KSH<code>;
  const periodTxt = (c.startAt && c.endAt)
    ? <code>${new Date(c.startAt).toISOString().slice(0,10)} → ${new Date(c.endAt).toISOString().slice(0,10)}<code>
    : "Not set yet";
...
<code> +
    <code>What is this?
<code> +
    <code>Weekly Draw is a giveaway where you earn tickets from successful purchases. More tickets = higher chance to win.
...
<code> +
    <code>How to participate:
<code> +
    <code>1) Tap ✅ JOIN (so you receive ticket updates, reminders & winner announcements)
<code> +
    <code>2) Buy offers — every 5 KSH = 1 ticket (Business gets +12% bonus tickets)
<code> +
    <code>3) Invite friends using 🔗 My Referral

<code> +
    <code>What purchases count?
<code> +
    <code>✅ Only successful payments.
<code> +
    <code>✅ Purchase must be ≥ 5 KSH to earn tickets.
<code> +
    <code>${weeklyDrawPurchasesCountText()}

<code> +
    <code>How to qualify (current rules):
${qualTxt}

<code> +
    <code>What is an active referral?
<code> +
    <code>A person who joined using your referral link and has made at least one successful purchase.

<code> +
    <code>Tip:
<code> +
    <code>Buy more to earn more tickets and climb the Top Participants list.<code>
  );
}
...
function weeklyDrawLastWinnersText() {
    ...
    const p = last.prizes || {};
    const prizeLine = <code>🥇 ${Number(p.first||0)} • 🥈 ${Number(p.second||0)} • 🥉 ${Number(p.third||0)} (KSH)<code>;
    const lines = last.winners.slice(0,3).map((w,i)=>{
      const pos = i===0?"🥇":i===1?"🥈":"🥉";
      return <code>${pos} ${w.name || w.chatId} — Ksh ${Number(w.prize||0)} (${w.paid ? "Paid ✅" : "Pending ⏳"})<code>;
    }).join("\n");
    return <code>
...
${lines}<code>;
  ...
  function buildWeeklyDrawMainCaption(chatId) {
    ...
    const listLines = ranked.length
      ? ranked.map((r, i) => <code>${i + 1}️⃣ ${r.name} — ${r.tickets} tickets<code>).join("\n")
    ...
    if (status === "ENDED" && Array.isArray(c.winners) && c.winners.length) {
      const lines = c.winners.slice(0, 3).map((w, i) => {
        ...
        return <code>${pos} ${w.name || w.chatId} — Ksh ${Number(w.prize || 0)} (${paid})<code>;
      });
      winnersBlock = <code>\n\n🏁 Winners (Top 3)\n${lines.join("\n")}<code>;
    }
    ...
    const caption =
      <code>🎉 ${heading}\n<code> +
      <code>Status: ${status}\n<code> +
      <code>⏳ Ends in: ${countdown}${winnersBlock}\n\n<code> +
      <code>${optedLine}\n\n<code> +
      <code>🏆 Top Participants (Top 10)\n${listLines}<code>;
...
function weeklyDrawQualificationsKeyboard() {
  ...
  const row = (key, title) => {
    ...
    return [
      { text: <code>${mark} ${title}<code>, callback_data: <code>wdadm:qual_toggle:${key}<code> },
      { text: "⚙️ Set", callback_data: <code>wdadm:qual_set:${key}<code> },
    ];
  };
...
function formatWeeklyDrawAdminStatus() {
  ...
  const p = c.prizes || {};
  const prizesTxt = <code>1st: ${Number(p.first||0)} KSH, 2nd: ${Number(p.second||0)} KSH, 3rd: ${Number(p.third||0)} KSH<code>;
  const winners = Array.isArray(c.winners) ? c.winners : [];
  const wTxt = winners.length
    ? winners.map((w, i) => <code>${i+1}. ${w.name || w.chatId} — ${w.prize} KSH (${w.paid ? "Paid ✅" : "Pending"})<code>).join("\n")
    : "No winners frozen yet.";
  return (
    <code>🎛 WEEKLY DRAW ADMIN\n<code> +
    <code>Heading: ${heading}\n<code> +
    <code>Enabled: ${enabled}\n<code> +
    <code>Status: ${status}\n<code> +
    <code>Period: ${startAt} → ${endAt}\n<code> +
    <code>Prizes: ${prizesTxt}\n\n<code> +
    <code>Winners:\n${wTxt}<code>
  );
}
function weeklyDrawPayWinnersKeyboard() {
  ...
  for (const w of winners) {
    const label = <code>${w.paid ? "✅ Paid" : "⏳ Pending"} — ${w.name || w.chatId} (${w.prize}KSH)<code>;
    rows.push([{ text: label, callback_data: <code>wdadm:toggle_paid:${w.chatId}<code> }]);
  }
  rows.push([{ text: "⬅ Back", callback_data: "wdadm:menu" }]);
  return { reply_markup: { inline_keyboard: rows } };
}
...
function weeklyDrawQualification(chatId) {
  ...
  for (const key of enabledKeys) {
    ...
    if (key === "ageDays") {
      ...
      satisfied[key] = ok;
      if (!ok) remaining.push(<code>Stay ${Math.max(0, need - ageDays)} more day(s)<code>);
      ...
      satisfied[key] = ok;
      if (!ok) remaining.push(<code>${Math.max(0, needCount - purchCount)} more purchases (≥${Number(cfg.minAmount || 20)} KSH)<code>);
    } else if (key === "activeReferrals") {
      const need = Math.max(0, Number(cfg.count || 0));
      const ok = activeRefs >= need;
      satisfied[key] = ok;
      if (!ok) remaining.push(<code>${Math.max(0, need - activeRefs)} more active referrals<code>);
    } else if (key === "spend") {
      const need = Math.max(0, Number(cfg.amount || 0));
      const ok = spent >= need;
      satisfied[key] = ok;
      if (!ok) remaining.push(<code>Spend ${Math.max(0, need - spent)} KSH more<code>);
    } else if (key === "withdraw") {
      const need = Math.max(0, Number(cfg.count || 0));
      const ok = withdrawReq >= need;
      satisfied[key] = ok;
      if (!ok) remaining.push(<code>Make ${Math.max(0, need - withdrawReq)} more withdrawal request(s)<code>);
    } else if (key === "offer") {
      ...
      satisfied[key] = ok;
      if (!ok) {
        if (!match) remaining.push(<code>Buy the required offer (admin will set it)<code>);
        else remaining.push(<code>Buy "${match}" (${Math.max(0, need - offerHits)} more time(s))<code>);
      }
    }
  }
...
function weeklyDrawQualificationText(chatId) {
  ...
  for (const r of q.remaining) lines.push(<code>• ${r}<code>);
  return lines.join("\n");
}
...
function weeklyDrawRankGapText(chatId) {
  ...
  if (info.top3) {
    if (info.rank === 1) return <code>🥇 You are #1! Keep buying to stay on top.<code>;
    return <code>🏆 You are #${info.rank} (Top 3). Keep buying to protect your spot.<code>;
  }
  if (info.gapToTop3 > 0 && info.gapToTop3 <= 100) {
    return <code>🏆 Rank #${info.rank}. Only ${info.gapToTop3} ticket(s) behind Top 3!<code>;
  }
  if (info.gapAbove > 0 && info.gapAbove <= 100) {
    return <code>🏆 Rank #${info.rank}. Only ${info.gapAbove} ticket(s) behind #${info.rank - 1}!<code>;
  }
  return <code>🏆 Rank #${info.rank}.<code>;
}
...
async function weeklyDrawFreezeAndAnnounce(reasonText) {
  ...
  const heading = String(c.heading || "WEEKLY DRAW").toUpperCase();
  const lines = [];
  lines.push(<code>🎉 ${heading} RESULTS<code>);
  ...
  if (!winners.length) {
    ...
    const medals = ["🥇", "🥈", "🥉"];
    winners.forEach((w, i) => {
      lines.push(<code>${medals[i]} ${w.name} — ${w.prize} KSH (${w.paid ? "Paid ✅" : "Pending ⏳"})<code>);
    });
  }
  ...
  for (const w of winners) {
    const winMsg =
      ...
      <code>You are a winner of ${w.prize} KSH from WEEKLY DRAW.\n<code> +
      <code>Status: ${w.paid ? "Paid ✅" : "Pending ⏳"}\n\n<code> +
      <code>Thank you for participating.<code>;
...
async function maybeBroadcastEndSoon3Days() {
  ...
  if (days <= 3 && !c.reminders.endSoon3dSent) {
    ...
    const heading = String(c.heading || "WEEKLY DRAW").toUpperCase();
    const msg =
      <code>⏳ ${heading} ends in ${getCampaignCountdown()}!\n\n<code> +
      <code>Buy offers to earn more tickets and climb the leaderboard. 🎟🔥<code>;
...
async function maybeSendNearQualifyReminder(chatId) {
  ...
  const heading = String(c.heading || "WEEKLY DRAW").toUpperCase();
  const msg =
    <code>🎟 ${heading} qualification tip:\n\n<code> +
    <code>You are close! ${weeklyDrawQualificationText(chatId)}\n\n<code> +
    <code>Keep going — you can qualify and win this week. 🔥<code>;
...
async function weeklyDrawRankChangeNotify() {
  ...
  for (const [cid, newRank] of Object.entries(newRanks)) {
    ...
    if (newRank < oldRank) {
      // climbed
      const msg =
        <code>🚀 NICE! You climbed to #${newRank} on WEEKLY DRAW.\n<code> +
        <code>⏳ Only ${getCampaignCountdown()} left — buy more to stay on top! 🎟🔥<code>;
      try { if (isWeeklyDrawOptedIn(Number(cid))) await bot.sendMessage(Number(cid), msg, mainMenuKeyboard(Number(cid))); } catch (_) {}
    } else {
      // dropped (overtaken)
      const msg =
        <code>⚠️ You were overtaken on WEEKLY DRAW.\n<code> +
        <code>You moved from #${oldRank} to #${newRank}.\n\n<code> +
        <code>⏳ Only ${getCampaignCountdown()} left — buy more to climb back! 🎟🔥<code>;
      try { if (isWeeklyDrawOptedIn(Number(cid))) await bot.sendMessage(Number(cid), msg, mainMenuKeyboard(Number(cid))); } catch (_) {}
    }
  }
...
async function maybeShowBusinessIntroOnce(chatId) {
  ...
  await sendTracked(
    chatId,
    <code>✅ *Business Account Activated!*\n\n<code> +
      <code>🚀 Benefits unlocked for your subscription period:\n<code> +
      <code>• Unlimited referrals\n<code> +
      <code>• Unlimited rewards\n<code> +
      <code>• Unlimited number usage\n<code> +
      <code>• All limits bypassed (except Bingwa daily rule)\n\n<code> +
      <code>⚠️ Self-referral is not allowed.<code>,
    ...
    for (const [cid, u] of Object.entries(db.users || {})) {
      ...
      if (u.accountType === "business" && nowMs > u.subscriptionExpiry) {
        if (u.businessExpiredNotifiedAt !== u.subscriptionExpiry) {
          ...
          await sendTracked(
            cid,
            <code>❌ *Business Subscription Expired*

<code> +
              <code>Your Business account expired on \<code>${mdEscape(expStr)}\<code>.

<code> +
              <code>To renew, type /sell<code>,
            { parse_mode: "HTML", ...mainMenuKeyboard(cid) }
          );
        }
        continue;
      }
      ...
      if (u.accountType === "business" && nowMs <= u.subscriptionExpiry) {
        ...
        if (daysLeft >= 1 && daysLeft <= 3) {
          if (u.businessReminderLastDay !== today) {
            ...
            await sendTracked(
              cid,
              <code>⏳ *Business Subscription Reminder*

<code> +
                <code>Your Business account will expire in *${daysLeft} day${daysLeft === 1 ? "" : "s"}*.
<code> +
                <code>Expiry: \<code>${mdEscape(expStr)}\<code>

<code> +
                <code>✅ Benefits until expiry:
<code> +
                <code>• Unlimited referrals
<code> +
                <code>• Unlimited rewards
...
function formatBuckets(title, obj, limit = 8) {
  ...
  if (!rows.length) return <code>${title}\n• (none)<code>;

  return <code>${title}\n<code> + rows.map((r) => <code>• ${r.k} — Ksh ${r.revenueKsh} (${r.orders} orders)<code>).join("\\n");
}

// ===================== CSV EXPORT (ADMIN) =====================
function csvEscape(v) {
  const s = String(v ?? "");
  if (/[",\n\r]/.test(s)) return <code>"${s.replace(/"/g, '""')}"<code>;
  return s;
}
...
function writeRevenueCsvFile(days) {
  ...
  const filename = <code>revenue_export_${todayKey()}_last${safeDays}d.csv<code>;
...
function awardSuccessPurchasePoints(chatId, pending) {
  ...
  try {
    bot.sendMessage(
      chatId,
      <code>🎁 Cashback Earned!

Purchase: Ksh ${amount}
Reward: ${cashbackPts.toFixed(2)} pts<code>,
...
function formatTodayLeaderboard(limit = 10) {
  ...
  if (!sorted.length) {
    return <code>🏆 *Top Inviters Today* (\<code>${mdEscape(day)}\<code>)\n\nNo successful referrals yet today.<code>;
  }
  const lines = sorted.map((x, i) => {
    const name = getDisplayName(x.chatId);
    return <code>${i + 1}. ${mdEscape(name)} — *${mdEscape(String(x.count))}*<code>;
  });
  return <code>🏆 *Top Inviters Today* (\<code>${mdEscape(day)}\<code>)\n\n${lines.join("\n")}<code>;
}
...
Thank you for using Bingwa Mtaani 💙<code>
).catch(() => {});
...
You can try again later.<code>
  ...
  if (adminChatId) await bot.sendMessage(adminChatId, <code>❌ Withdraw declined for ${targetId}<code>).catch(() => {});
  return true;
}
// ===================== AUTO DELETE SYSTEM =====================
...
bot.on("inline_query", async (q) => {
  try {
    ...
    for (const cat of catHits) {
      results.push({
        type: "article",
        id: <code>cat_${base64UrlEncode(cat)}<code>,
        title: <code>📦 ${cat}<code>,
        description: "Open category packages",
        // Trigger text: bot will immediately open category
        input_message_content: { message_text: <code>#cat ${base64UrlEncode(cat)}<code> },
      });
    }
    ...
    for (const h of hits) {
      results.push({
        type: "article",
        id: <code>offer_${base64UrlEncode(h.category)}_${base64UrlEncode(h.label)}<code>,
        title: h.label,
        description: <code>${h.category} • Ksh ${h.price}<code>,
        // Trigger text: bot will immediately open Proceed / Change Number flow
        input_message_content: {
          message_text: <code>#buy ${base64UrlEncode(h.category)}:${base64UrlEncode(h.label)}<code>,
        },
      });
    }
    ...
    if (!query) {
      ...
      if (top.length) {
        for (const t of top) {
          results.push({
            type: "article",
            id: <code>trend_${base64UrlEncode(t.category)}_${base64UrlEncode(t.label)}<code>,
            title: <code>🔥 ${t.label}<code>,
            description: <code>${t.category} • Ksh ${t.price} • ${t.count} buys<code>,
            input_message_content: { message_text: <code>#buy ${base64UrlEncode(t.category)}:${base64UrlEncode(t.label)}<code> },
          });
        }
      } else {
        results.push({
...
if (action === "set_heading") {
  ...
  if (action === "restart") {
    ...
    const statusText = formatWeeklyDrawAdminStatus();
    await bot.sendMessage(chatId, <code>🔄 WEEKLY DRAW restarted.\n\n${statusText}<code>, weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }
...
if (data === "ui:close") {
  ...
  if (data.startsWith("ui:upgrade:")) {
    ...
    if (action.startsWith("stk_retry_prompt:")) {
      ...
      return sendTracked(
        chatId,
        <code>📲 Do you still have the M-PESA prompt open?\n\n<code> +
          <code>✅ If yes, just complete it (no need to retry).\n<code> +
          <code>❌ If no prompt, you can retry now.<code>,
        {
          parse_mode: "HTML",
          reply_markup: {
            inline_keyboard: [
              [{ text: "✅ I have the prompt", callback_data: <code>ui:stk_have_prompt:${ref}<code> }],
              [{ text: "❌ No prompt, retry", callback_data: <code>ui:retry_stk:${ref}<code> }],
              [{ text: "❌ Cancel Pending", callback_data: <code>ui:stk_cancel:${ref}<code> }],
    ...
    if (action.startsWith("retry_stk:")) {
      ...
      try {
        ...
        return sendTracked(
          chatId,
          <code>⚠️ Still delayed. If no prompt appears, wait 1 minute and retry.\n\nHelp: ${HELP_PHONE}<code>,
          { ...mainMenuKeyboard(chatId) }
        );
      }
    }
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data.startsWith("rot:stop:")) {
      ...
      if (ok) await bot.sendMessage(chatId, <code>✅ Rotation stopped: ${rid}<code>).catch(() => {});
      return;
    }
    ...
    if (data.startsWith("bcast:")) {
      ...
      try {
        if (choice === "once") {
          ...
          await bot.sendMessage(
            chatId,
            <code>📣 Broadcast sent once.\nSent: ${result.sent}\nFailed: ${result.failed}\nUsers: ${result.total}<code>
          ).catch(() => {});
        } else if (choice === "rot_now") {
          const rot = addRotation({ fromChatId, messageId, days });
          rot.nextRunAt = Date.now(); // start now
          saveDB();
          await bot.sendMessage(chatId, <code>✅ Rotation started (NOW)\nID: ${rot.id}\nDays: ${days}\nUse /rotlist to stop.<code>).catch(() => {});
        } else if (choice === "rot_tom") {
          const rot = addRotation({ fromChatId, messageId, days });
          rot.nextRunAt = Date.now() + 24 * 60 * 60 * 1000; // start tomorrow
          saveDB();
          await bot.sendMessage(chatId, <code>✅ Rotation scheduled (TOMORROW)\nID: ${rot.id}\nDays: ${days}\nUse /rotlist to stop.<code>).catch(() => {});
        }
      } catch (e) {
        await bot.sendMessage(chatId, <code>⚠️ Broadcast error: ${e?.message || e}<code>).catch(() => {});
      } finally {
        adminState.delete(String(chatId));
        await bot.answerCallbackQuery(q.id).catch(() => {});
      }
...
// Withdraw quick actions via inline buttons (same behavior as /ac /dc)
  ...
  const sent = await bot.sendMessage(
        targetChatId,
        <code>✅ Redeem approved!\nItem: ${req.itemLabel}\nLoad To: ${mask07(req.loadTo)}\n\nYour request is being processed.<code>,
        { ...mainMenuKeyboard(chatId) }
      ).catch(() => {});
  ...
  const sent = await bot.sendMessage(
        targetChatId,
        <code>❌ Redeem request cancelled.\n\nReserved points refunded: ${cost} pts\nCurrent balance: ${getPoints(targetChatId).toFixed(2)} pts<code>,
        { ...mainMenuKeyboard(chatId) }
      ).catch(() => {});
      await bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: q.message.chat.id, message_id: q.message.message_id }).catch(() => {});
      return;
...
async function renderSessionSnapshot(chatId, snap) {
  ...
  if (s.step === "package") {
    ...
    return sendTracked(chatId, section(String(s.category || "").toUpperCase()) + <code>👇 ${s.category} packages:<code>, packagesKeyboard(s.category));
  }

  if (s.step === "confirm") {
    if (!s.category) return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard());
    const pkg = findPackageByLabel(s.category, s.pkgKey);
    if (!pkg) return sendTracked(chatId, section(String(s.category || "").toUpperCase()) + <code>👇 ${s.category} packages:<code>, packagesKeyboard(s.category));

    const savedPhone = getUserPhone(chatId);
    const hasSaved = !!savedPhone;
    const msgText =
      <code>✅ Selected:\n*${pkg.label}*\n\n<code> +
      (hasSaved ? <code>📱 Saved number: *${maskPhone(savedPhone)}*\n\n<code> : <code>📱 Paste number.\n\n<code>) +
      <code>Choose:\n• ✅ Proceed (use saved number)\n• 📞 Change Number<code>;

    return sendTracked(chatId, msgText, { parse_mode: "HTML", ...confirmKeyboard(hasSaved) });
  }
...
function authHeader() {
  return "Basic " + Buffer.from(<code>${PAYHERO_USERNAME}:${PAYHERO_PASSWORD}<code>).toString("base64");
}
...
function makeExternalRef(chatId, category, price) {
  return <code>BINGWA-${Date.now()}-${String(category).replace(/\s+/g, "_")}-${price}-${chatId}<code>;
}
...
function trendKey(category, pkgLabel) {
  return <code>${String(category || "")}||${String(pkgLabel || "")}<code>;
}
...
function getStkAbuseKey(chatId, phone254) {
  return <code>${chatId}:${String(phone254 || "")}<code>;
}
...
// ===================== INLINE SEARCH =====================
function escapeMd(s) {
  return String(s || "").replace(/[_*\[\]()~<code>>#+\-=|{}.!]/g, "\\$&");
}
...
function searchOffersInline(query) {
  ...
  return all.filter((x) => {
    const hay = <code>${x.category} ${x.label} ${x.price}<code>.toLowerCase();
...
function makeSig({ category, pkgLabel, phone254 }) {
  return <code>${category}||${pkgLabel}||${phone254}<code>;
}
...
function makeApprovalId() {
  return <code>APR-${Date.now()}-${Math.random().toString(16).slice(2, 8)}<code>;
}
...
function linkAccountToPhone(chatId, phone254) {
  ...
  if (!isBusinessActive(chatId) && list.length >= 4) {
    ...
    // notify admin
    notifyAdmin(
      <code>🛂 *Phone Approval Request*
<code> +
      <code>Phone: *${mdEscape(maskPhone(p))}*
<code> +
      <code>Requester ChatID: \<code>${mdEscape(String(chatId))}\<code>

<code> +
      <code>Approve will free 1 slot (oldest) and link this account.<code>,
      {
        parse_mode: "HTML",
        reply_markup: {
          inline_keyboard: [
            [{ text: "✅ Approve", callback_data: <code>phone_appr:approve:${appr.id}<code> }],
            [{ text: "❌ Deny", callback_data: <code>phone_appr:deny:${appr.id}<code> }],
          ],
        },
      }
    ).catch(() => {});
...
function linkedAccountsLabelForPhone(phone254) {
  ...
  return list.map((x, i) => <code>• Account ${i + 1}: ${mdEscape(String(x.chatId))}<code>).join("\\n");
}
...
async function payheroStkPush({ amount, phone, externalRef, channelId }) {
  const payload = {
    ...
    customer_name: <code>tg_${externalRef}<code>,
  };
  ...
  try {
    ...
    data = { message: rawText || <code>HTTP ${res.status}<code> };
  }
...
function mdEscape(s) {
  return String(s ?? "").replace(/([_*\[\]()~<code>>#+\-=|{}.!])/g, "\\$1");
}
...
async function notifyAdmin(text) {
  ...
  const sent = await bot.sendMessage(ADMIN_ID, text, { parse_mode: "HTML" });
  } catch (e) {
...
function buildReferralLeaderboardText(limit = 5) {
  ...
  const lines = entries.map((x, i) => <code>${i + 1}. \<code>${mdEscape(x.cid)}\<code> — *${x.referrals}* referrals<code>);
  return <code>🏆 *Top Referrers This Week*\n\n${lines.join("\n")}\n\nKeep sharing your referral link to earn points!<code>;
}
...
function queueBroadcastAlbum(adminChatId, msg) {
  ...
  existing.timer = setTimeout(async () => {
    ...
    try {
      ...
      await sendTracked(
        adminChatId,
        <code>📣 Broadcast album done.\nSent: ${result.sent}\nFailed: ${result.failed}\nUsers: ${result.total}<code>
      );
    } catch (e) {
      await sendTracked(adminChatId, <code>⚠️ Album broadcast error: ${e.message || e}<code>);
    }
  }, 1800);
}
...
function makeRotId() {
  return <code>ROT-${Date.now()}-${Math.random().toString(16).slice(2, 8)}<code>;
}
  ...
  try {
    ...
    for (const r of list) {
      ...
      try {
        ...
        try {
          await notifyAdmin(
            <code>🔁 *Rotation Broadcast Sent*
<code> +
            <code>ID: \<code>${mdEscape(r.id)}\<code>
<code> +
            <code>Runs: *${mdEscape(String(r.runs))}*
<code> +
            <code>Sent: *${mdEscape(String(result.sent))}* / ${mdEscape(String(result.total))}
<code> +
            <code>Failed: *${mdEscape(String(result.failed))}*
<code> +
            <code>Next: \<code>${mdEscape(kenyaDateTimeFromTs(r.nextRunAt))}\<code>
          );
        } catch (_) {}
      } catch (e) {
...
async function sendDailyAdminReportIfDue() {
  try {
    ...
    const msg =
      <code>📊 *Daily Report* (Kenya)
<code> +
      <code>Date: \<code>${mdEscape(day)}\<code>

<code> +
      <code>Revenue: *Ksh ${mdEscape(String(data.revenueKsh))}*
<code> +
      <code>Orders: *${mdEscape(String(data.orders))}*
<code> +
      <code>AOV: *Ksh ${mdEscape(String(aov))}*

<code> +
      <code>Business active subscribers: *${mdEscape(String(countBusinessSubscribers({ activeOnly: true })))}*
<code> +
      <code>${formatBuckets("📦 *By Category*", data.byCategory, 8)}

<code> +
      <code>${formatBuckets("🏷️ *Top Packages*", data.byPackage, 8)}<code>;
...
function openCategoryQuick(chatId, category) {
  ...
  s.pkgKey = null;
  sessions.set(chatId, s);
  return sendTracked(chatId, <code>✅ *${mdEscape(s.category)}*\n\nSelect a package:<code>, { parse_mode: "HTML", ...packagesKeyboard(s.category) });
}
...
function welcomeText(name) {
  return (
    <code>👋 Hello ${name || "there"}, welcome to *Bingwa Mtaani Data Services*.

<code> +
    <code>⚡ This bot helps you buy *Data bundles*, *Unlimited offers*, *SMS packages*, *Minutes*, *Bonga Points* and *Flex deals* easily via STK — even if you have Okoa Jahazi debt.

<code> +
    <code>✅ Choose what you need:
<code> +
    <code>• 📦 Bingwa Deals (limited)
<code> +
    <code>• ∞ Unlimited Deals
<code> +
    <code>• ✉️ SMS Offers
<code> +
    <code>• 📞 Minutes
...
<code> +
    <code>🎁 Earn points via referrals and redeem.

<code> +
    <code>⚠️ *Bingwa Deals rule:* *Once per day per phone number* (ONLY after success).

<code> +
    <code>✅ Tap 🛒 *Buy Offers* to purchase.

<code> +
    <code>☎️ Help / delays: *${HELP_PHONE}*<code>
  );
}
...
function helpText() {
  return (
    <code>✅ How to buy:\n<code> +
    <code>1) Tap 🛒 Buy Offers\n<code> +
    <code>2) Choose a category\n<code> +
    <code>3) Tap the package button\n<code> +
    <code>4) If phone saved, tap ✅ Proceed (or 📞 Change Number)\n<code> +
    <code>5) STK prompt comes to your phone\n\n<code> +
    <code>📌 Phone formats accepted:\n<code> +
    <code>• 07XXXXXXXX\n<code> +
    <code>• 01XXXXXXXX\n<code> +
    <code>• 2547XXXXXXXX\n<code> +
    <code>• 2541XXXXXXXX\n\n<code> +
    <code>⚠️ Bingwa Deals: *Once per day per phone number* (ONLY after success).\n<code> +
    <code>Other categories: can be purchased *many times per day* on same number.\n\n<code> +
    <code>🎁 Redeem points: Tap "🎁 Redeem Points"\n\n<code> +
    <code>Help: ${HELP_PHONE}<code>
  );
}
...
function packagesOverviewText() {
  ...
  lines.push(<code>📦 *Packages Overview*\n<code>);
  for (const cat of order) {
    if (!PACKAGES[cat]) continue;
    lines.push(<code>${icons[cat] || "•"} *${cat}*<code>);
    lines.push(PACKAGES[cat].map((x) => <code>• ${x.label}<code>).join("\\n"));
    lines.push("");
  }
  lines.push(<code>\n✅ Tap 🛒 *Buy Offers* to purchase.<code>);
  return lines.join("\n");
}
...
notifyAdmin(
  ...
  <code>Inviter: \<code>${mdEscape(inviterId)}\<code>\n<code> +
  <code>Buyer: \<code>${mdEscape(buyerChatId)}\<code>\n<code> +
  <code>Purchase: *Ksh ${mdEscape(amt)}*\n<code> +
  <code>Bonus: *${mdEscape(cleanBonus)}pts*\n<code> +
  <code>Time: \<code>${mdEscape(String(when || kenyaDateTime()))}\<code>
);
}
...
async function forwardToAdminWithUserTag(userMsg) {
  ...
  const name = <code>${u.first_name || ""} ${u.last_name || ""}<code>.trim();
  const uname = u.username ? <code>@${u.username}<code> : "no_username";
  ...
  const sent = await bot.sendMessage(
      ADMIN_ID,
      <code>💬 *Support Message*\nFrom: ${mdEscape(name)}\nUsername: ${mdEscape(uname)}\nChatID: \<code>${mdEscape(fromChatId)}\<code>\n\nReply with:\n/reply ${mdEscape(fromChatId)}<code>,
      { parse_mode: "HTML" }
    );
  } catch (e) {
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // PayHero may send the real fields inside <code>response<code> as a JSON string
    let payload = body;
    ...
    const statusRaw =
      ...
      payload?.Payment_Status ||
      payload?.status ||
      // ✅ Some PayHero callbacks use <code>Status: true/false<code>
      payload?.Status ||
      body?.payment_status ||
      body?.status ||
      body?.Status ||
...
if (!isSuccess) {
  const failKey = <code>FAIL-${externalRef}<code>;
  ...
  const details =
    pending && pending.pkgLabel
      ? <code>Offer: ${pending.pkgLabel}\nFrom: ${maskPhone(pending.phone254)}\nAmount: Ksh ${pending.price}<code>
      : "";

  if (pending) deletePendingPayment(externalRef);

  const out =
    <code>❌ Payment failed at ${when}.\n<code> +
    (details ? <code>${details}\n<code> : "") +
    <code>Your package could not be processed. Please try again or contact customer support.\n\nHelp: ${HELP_PHONE}<code>;
    ...
    if (pending) {
      ...
      if (!amountOk || !phoneOk) {
        ...
        if (!alreadyProcessed(failKey)) {
          ...
          await sendTracked(chatId, <code>❌ Payment verification failed at ${when}.\n\nYour payment details did not match the request. Please contact support.\n\nHelp: ${HELP_PHONE}<code>, { ...mainMenuKeyboard(chatId) });
          await notifyAdmin(
            <code>⚠️ *Callback Verification Failed*\n<code> +
            <code>ChatID: \<code>${mdEscape(chatId)}\<code>\n<code> +
            <code>External Ref: \<code>${mdEscape(externalRef)}\<code>\n<code> +
            <code>Expected Amount: *Ksh ${mdEscape(expectedAmount)}*\n<code> +
            <code>Callback Amount: *Ksh ${mdEscape(cbAmount)}*\n<code> +
            <code>Expected Phone: *${mdEscape(formatTo07(expectedPhone))}*\n<code> +
            <code>Callback Phone: *${mdEscape(String(cbPhone || "N/A"))}*\n<code> +
            <code>Time: \<code>${mdEscape(when)}\<code>
          );
        }
        return;
      }
...
<code> +
              <code>Expiry: \<code>${mdEscape(exp)}\<code>

<code> +
              <code>🚀 Benefits (during active subscription):
<code> +
              <code>• Unlimited referrals
<code> +
              <code>• Unlimited rewards
<code> +
              <code>• Unlimited number usage
<code> +
              <code>• All limits bypassed

<code> +
              <code>⚠️ Bingwa daily rule remains.
<code> +
              <code>⚠️ Never refer yourself.<code>
            );
          })()
        : "";
...
<code> +
      <code>🆔 Reference: *${referenceNumber}*
<code> +
      <code>📦 Package: *${packageName}*
<code> +
      <code>💰 Amount: Ksh ${pending?.price || 0}
<code> +
      <code>🛒 Total Purchases: *${totalPurchases}*

<code> +
      <code>Your payment has been received and your request is being processed.<code> +
      businessExtra;
...
// Inline action buttons (Upgrade + Buy Last Offer + Buy Offers)
    ...
    if (upsellPkg) {
      inline_keyboard.push([
        { text: <code>🚀 Upgrade: ${String(upsellPkg.label).split('•')[1]?.trim() || (upsellPkg.price + ' Ksh')}<code>, callback_data: <code>ui:upgrade:${base64UrlEncode(upsell.category)}:${base64UrlEncode(upsellPkg.label)}<code> }
      ]);
    }
    ...
    if (pending?.category === "Business Account") {
      ...
      await sendTracked(chatId, successMsg, { parse_mode: "HTML", ...mainMenuKeyboard(chatId) });
    } else {
...
try {
  ...
  if (label) {
    ...
    const msg =
      <code>🎟 WEEKLY DRAW UPDATE

<code> +
      <code>Tickets earned: ${t.earned}
<code> +
      <code>Total tickets: ${t.total}
<code> +
      <code>⏳ Ends in: ${getCampaignCountdown()}
...
// ✅ Notify admin (success notification with Reference + Package + Total Purchases)
    await notifyAdmin(
      <code>✅ *Payment Confirmed*\n<code> +
        <code>ChatID: \<code>${mdEscape(chatId)}\<code>\n<code> +
        <code>Reference: \<code>${mdEscape(externalRef)}\<code>\n<code> +
        <code>Provider Reference: \<code>${mdEscape(mpesaOut)}\<code>\n<code> +
        <code>Package: \<code>${mdEscape(packageName)}\<code>\n<code> +
        <code>Amount: *Ksh ${mdEscape(String(pending?.price || 0))}*\n<code> +
        <code>Total Purchases: \<code>${mdEscape(String(totalPurchases))}\<code>\n<code> +
        <code>Time: \<code>${mdEscape(when)}\<code>\n<code> +
        <code>ResultCode/Status: \<code>${mdEscape(String(statusOutRaw ?? ""))}\<code>
    );
  } catch (e) {
...
// ===================== ADMIN: LIST ALL COMMANDS =====================
  if (isAdmin(chatId) && (text === "/commands" || text === "/cmds" || text === "/")) {
    ...
    for (const c of COMMANDS_LIST) {
      const row = <code>• \<code>${c.cmd}\<code> — ${mdEscape(c.desc)}<code>;
  ...
  if (isStartCmd && !isAdmin(chatId)) {
    ...
    if (last && now - last < START_COOLDOWN_MS) {
      ...
      await bot.sendMessage(chatId, <code>⏳ Please wait ${wait}s and try /start again.<code>).catch(() => {});
      return;
    }
  ...
  if (/^\/status$/i.test(text)) {
    ...
    if (!p) {
      await sendTracked(chatId, <code>✅ No pending STK request found.

📱 *Linked accounts for this number (max 4):*
${linkedAccountsLabelForPhone(u.lastLinkedPhone || "")}<code>, { parse_mode: "HTML", ...mainMenuKeyboard(chatId) });
      return;
    }

    const ageMs = Date.now() - Number(p.createdAt || 0);
    const ageSec = Math.max(0, Math.floor(ageMs / 1000));
    const ageLabel = ageSec < 60 ? <code>${ageSec}s<code> : <code>${Math.floor(ageSec / 60)}m ${ageSec % 60}s<code>;

    const out =
      <code>📲 *STK Status*\n<code> +
      <code>• Offer: *${mdEscape(p.pkgLabel || "N/A")}*\n<code> +
      <code>• Amount: *Ksh ${mdEscape(p.price)}*\n<code> +
      <code>• Phone: *${mdEscape(maskPhone(p.phone254 || ""))}*\n<code> +
      <code>• Ref: \<code>${mdEscape(p.ref || p.externalRef || p.reference || p.external_reference || "") || mdEscape(p.ref || "")}\<code>\n<code> +
      <code>• Age: *${ageLabel}*\n\n<code> +
      <code>📱 *Linked accounts for this number (max 4):*\n${mdEscape(linkedAccountsLabelForPhone(u.lastLinkedPhone || ""))}\n\n<code> +
      <code>If you have the M-PESA prompt, complete it.\n<code> +
      <code>If no prompt appears, you can retry or cancel.<code>;

    await sendTracked(chatId, out, {
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
          [{ text: "🔁 Retry STK", callback_data: <code>ui:stk_retry_prompt:${p.ref || p.externalRef || p.reference || p.external_reference || ""}<code> }],
          [{ text: "❌ Cancel Pending", callback_data: <code>ui:stk_cancel:${p.ref || p.externalRef || p.reference || p.external_reference || ""}<code> }],
          [{ text: "🛒 Buy Offers", callback_data: "ui:buy_offers" }],
        ],
      },
    });
...
if (/^\/sell$/i.test(text)) {
  ...
  if (isBusinessActive(chatId)) {
    ...
    await sendTracked(
      chatId,
      <code>✅ You are on *Business Account*.\n\nExpiry: \<code>${mdEscape(exp)}\<code>,
      { parse_mode: "HTML", ...mainMenuKeyboard(chatId) }
    );
    return;
  }
  ...
  await sendTracked(
    chatId,
    <code>💼 *Business Account Subscription*\n\n<code> +
      <code>Choose a plan:\n<code> +
      <code>• Ksh 105 — 1 Month\n<code> +
      <code>• Ksh 260 — 3 Months\n\n<code> +
      <code>✅ Bingwa daily rule remains.\n<code> +
      <code>❌ Self-referral not allowed.<code>,
    { parse_mode: "HTML", ...sellKeyboard() }
  );
  return;
}
...
if (text === "💼 Business 1 Month — Ksh 105" || text === "💼 Business 3 Months — Ksh 260") {
  ...
  addPendingPayment(externalRef, {
    ...
    pkgLabel: <code>${plan.label} (Ksh ${plan.price})<code>,
    phone254,
    price: plan.price,
    planKey,
  });
  ...
  try {
    ...
    deletePendingPayment(externalRef);
    await sendTracked(chatId, <code>❌ STK failed: ${mdEscape(e?.message || e)}\n\nHelp: ${HELP_PHONE}<code>, {
      parse_mode: "HTML",
      ...mainMenuKeyboard(chatId),
    });
    return;
...
if (/^\/report$/i.test(text)) {
  ...
  const msg =
    <code>📊 *Admin Report* (Kenya)\n<code> +
    <code>Date: \<code>${mdEscape(todayKey())}\<code>\n\n<code> +
    <code>👥 Users: *${mdEscape(String(totalUsers))}*\n<code> +
    <code>💼 Business active: *${mdEscape(String(activeBiz))}*\n<code> +
    <code>💼 Business total (ever): *${mdEscape(String(totalBiz))}*\n\n<code> +
    <code>💰 Today Revenue: *Ksh ${mdEscape(String(today.revenueKsh))}* (Orders: *${mdEscape(String(today.orders))}*)\n<code> +
    <code>💰 Yesterday Revenue: *Ksh ${mdEscape(String(yday.revenueKsh))}* (Orders: *${mdEscape(String(yday.orders))}*)\n\n<code> +
    <code>${formatBuckets("📦 *Today By Category*", today.byCategory, 8)}\n\n<code> +
    <code>${formatBuckets("🏷️ *Today Top Packages*", today.byPackage, 8)}<code>;

  await sendTracked(chatId, msg, { parse_mode: "HTML", ...mainMenuKeyboard(chatId) });
  return;
}
  ...
  if (isAdmin(chatId) && aState?.step === "await_broadcast") {
    ...
    try {
      ...
      return sendTracked(chatId, <code>⚠️ Broadcast error: ${e.message || e}\nTry again or /cancel.<code>);
    }
  }
  ...
  if (isAdmin(chatId)) {
    ...
    if (r && Date.now() < (r.until || 0) && text !== "/cancel" && !text.startsWith("/reply")) {
      try {
        ...
        return sendTracked(chatId, <code>✅ Sent to ${r.targetChatId}<code>);
      } catch (e) {
        return sendTracked(chatId, <code>⚠️ Failed sending to ${r.targetChatId}: ${e?.message || e}<code>);
      }
    }
    if (text === "/cancel") {
...
• Cycle resets in: ${st.daysLeft} day(s)<code>,
          { ...mainMenuKeyboard(chatId) }
        );
      }
      ...
      if (rem > 0) {
        ...
        return sendTracked(chatId, <code>⏳ Please wait ${mins} minute(s) before sending another redeem request.<code>, {
          ...mainMenuKeyboard(chatId),
        });
      }
      ...
      const reqId = <code>RDM-${Date.now()}-${chatId}<code>;
      const u = getUser(chatId);

      const req = {
        reqId,
        chatId,
        name: <code>${msg.from?.first_name || ""} ${msg.from?.last_name || ""}<code>.trim(),
        itemKey: item.key,
...
Item: ${item.label}
Cost: ${amount} pts
Load To: ${formatTo07(phone254)}<code>,
        {
          reply_markup: {
            inline_keyboard: [
              [{ text: "✅ Accept", callback_data: <code>redeem_accept:${reqId}<code> }],
              [{ text: "❌ Cancel", callback_data: <code>redeem_decline:${reqId}<code> }],
            ],
          },
        }
      );
      ...
      return sendTracked(
        chatId,
        <code>✅ Redemption request submitted!
...
Waiting for admin approval…<code>,
        { ...mainMenuKeyboard(chatId) }
      );
    }
...
const pts = getPoints(chatId);
      return sendTracked(chatId, <code>⭐ Your points: *${pts.toFixed(2)}*\n\nUse "🎁 Redeem Points" to redeem.<code>, {
        parse_mode: "HTML",
        ...mainMenuKeyboard(chatId),
      });
    }
...
Thank you for using Bingwa Mtaani 💙<code>,
  ...
  await bot.sendMessage(adminChatId, <code>✅ Withdraw approved for ${targetId}<code>).catch(() => {});
  return true;
}
...
You can try again later.<code>,
  ...
  await bot.sendMessage(adminChatId, <code>❌ Withdraw declined for ${targetId}<code>).catch(() => {});
  return true;
}
...
if (dcMatch) {
  ...
  const sent = await bot.sendMessage(
    Number(targetId),
    <code>❌ Withdrawal Declined\n\nAmount refunded: ${amt} pts\nCurrent balance: ${getPoints(targetId).toFixed(2)} pts<code>,
    { ...mainMenuKeyboard(chatId) }
  ).catch(() => {});
...
return sendTracked(chatId, <code>❌ Withdraw declined for ${targetId}<code>);
}
...
const apm = text.match(/^\/addpoints\s+(\d+)\s+([0-9.]+)$/i);
      if (apm) {
        ...
        return sendTracked(chatId, <code>✅ Added ${amount} pts to ${targetId}<code>);
      }
      ...
      if (rpm) {
        ...
        return sendTracked(chatId, <code>✅ Removed ${amount} pts from ${targetId}<code>);
      }
      ...
      if (spm) {
        ...
        u.points = amount;
        saveDB();
        return sendTracked(chatId, <code>✅ Set ${targetId} points to ${amount}<code>);
      }
      ...
      if (/^\/users$/i.test(text)) {
        ...
        return sendTracked(chatId, <code>👥 Total users: ${total}<code>);
      }
      ...
      if (rsm) {
        ...
        if (!tu.inviterId) {
          return sendTracked(chatId, <code>ℹ️ User ${targetId} has no inviter set.<code>);
        }
        ...
        const out =
          ...
          <code>User: ${targetId}\n<code> +
          <code>Inviter: ${tu.inviterId}\n<code> +
          <code>Set at: ${kenyaDateTimeFromTs(setAt)}\n<code> +
          <code>Expiry: ${Number(REFERRAL.EXPIRY_DAYS || 30)} day(s)\n<code> +
          <code>Status: ${expired ? "EXPIRED (no earnings)" : <code>ACTIVE (${daysLeft} day(s) left)<code>}<code>;
      ...
      if (/^\/stats$/i.test(text)) {
        ...
        const weeklyLines =
          Object.entries(weeklyTotals)
            ...
            .map(([label, cnt]) => <code>• ${label} : ${cnt}<code>)
            .join("\n") || "• (No purchases this week)";

        const out =
          <code>📊 *Bot statistics*\n\n<code> +
          <code>Active users: *${activeUsers}*\n<code> +
          <code>Inactive users: *${inactiveUsers}*\n<code> +
          <code>Total users: *${totalUsers}*\n<code> +
          <code>Total purchases: *${Number(db.stats.totalPurchases || 0)}*\n<code> +
          <code>Total broadcasts: *${Number(db.stats.totalBroadcasts || 0)}*\n\n<code> +
          <code>🗓️ *Weekly package purchases* (week ${wk})\n<code> +
          <code>${weeklyLines}<code>;

        return sendTracked(chatId, out, { parse_mode: "HTML" });
      }
...
if (topOffers) {
  ...
  const lines = rows.map((r, i) => <code>${i + 1}. ${r.k} — *Ksh ${r.revenueKsh}* (${r.orders})<code>).join("\\n");
  return sendTracked(chatId, <code>🏆 *Top Offers (last ${days} day(s))*\n\n${lines}<code>, { parse_mode: "HTML" });
}
...
if (exportRev) {
  ...
  try {
    ...
    await bot.sendDocument(chatId, filepath, {
      caption: <code>📎 Revenue CSV export\nWindow: last ${usedDays} day(s)\nRows: ${rowCount}<code>,
    });

    // cleanup file to avoid disk growth
    try { fs.unlinkSync(filepath); } catch (_) {}

    return sendTracked(chatId, <code>✅ Export sent: ${filename}<code>);
  } catch (e) {
    return sendTracked(chatId, <code>⚠️ Export failed: ${e?.message || e}<code>);
  }
}
...
if (/^\/pending$/i.test(text)) {
  ...
  for (const [, r] of pending) {
    const adminText =
      <code>🎁 *Redeem Request*\n<code> +
      <code>ChatID: \<code>${mdEscape(r.chatId)}\<code>\n<code> +
      <code>User: *${mdEscape(r.name || "User")}*\n<code> +
      <code>Item: *${mdEscape(r.itemLabel)}*\n<code> +
      <code>Cost: *${mdEscape(r.cost)} pts*\n<code> +
      <code>Load To: *${mdEscape(r.loadTo)}*\n<code> +
      <code>Time: \<code>${mdEscape(r.time || "")}\<code>\n\n<code> +
      <code>Approve or cancel:<code>;
  ...
  const sent = await bot.sendMessage(chatId, adminText, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "✅ Accept", callback_data: <code>redeem_accept:${r.reqId}<code> }],
          [{ text: "❌ Cancel", callback_data: <code>redeem_decline:${r.reqId}<code> }],
        ],
      },
    });
  }
...
if (bcastMatch) {
  ...
  return bot.sendMessage(chatId, <code>Choose broadcast mode (rotation = ${days} day(s)) :<code>, {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🚀 Send Immediately (No Rotation)", callback_data: "bcast:once" }],
        [{ text: <code>🔁 Rotation Daily (Start Now) — ${days}d<code>, callback_data: "bcast:rot_now" }],
        [{ text: <code>⏭️ Rotation Daily (Start Tomorrow) — ${days}d<code>, callback_data: "bcast:rot_tom" }],
...
if (/^\/rotlist$/i.test(text)) {
  ...
  for (const r of active) {
    ...
    await bot.sendMessage(
      chatId,
      <code>🔁 Rotation ID: ${r.id}\nRuns: ${runs}\nNext: ${next}\nExpires: ${exp}<code>,
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "🛑 Rotation OFF / Stop rotation", callback_data: <code>rot:stop:${r.id}<code> }],
          ],
        },
      }
    ).catch(() => {});
...
if (/^\/broadcast$/i.test(text)) {
      ...
      if (rm) {
        ...
        return sendTracked(chatId, <code>✅ Reply mode ON.\nNext message you send will go to: ${target}\nSend /cancel to stop.<code>);
      }
    }
...
if (act && act.type === "change_number") {
  ...
  await sendTracked(
    chatId,
    <code>✅ Number saved successfully.\n\n📞 New number: *${mdEscape(phone254)}*<code>,
    { parse_mode: "HTML", ...mainMenuKeyboard(chatId) }
  );
  return;
}
...
if (act && act.type === "sell_business" && act.planKey) {
  ...
  await sendTracked(
    chatId,
    <code>📲 Sending STK for *${mdEscape(plan.label)}* (Ksh ${plan.price})…<code>,
    { parse_mode: "HTML", ...mainMenuKeyboard(chatId) }
  );
  ...
  if (!isAdmin(chatId) && (text === "❌ Cancel" || text === "/cancel") && hasPendingRedeem(chatId)) {
    ...
    return sendTracked(
      chatId,
      <code>❌ Redeem cancelled.\n\nReserved points refunded: ${cost} pts\nCurrent balance: ${getPoints(chatId).toFixed(2)} pts<code>,
      { ...mainMenuKeyboard(chatId) }
    );
  }
  ...
  if (act && !isAdmin(chatId) && act.kind === "redeem") {
    ...
    if (rem > 0) {
      const mins = Math.ceil(rem / 60000);
      clearPendingAction(chatId);
      return sendTracked(chatId, <code>⏳ Please wait ${mins} minute(s) before sending another redeem request.<code>, { ...mainMenuKeyboard(chatId) });
    }
      ...
      if (text === "✅ Confirm") {
        ...
        if (savedPhone) {
          ...
          if (item.key === "FREE_250MB" && !isBusinessActive(chatId) && redeemPhoneAlreadyUsedToday(phone254)) {
            ...
            return sendTracked(
              chatId,
              <code>🚫 This phone number already redeemed 250MB today.\n\nUse a different number or try again tomorrow.<code>,
              { ...mainMenuKeyboard(chatId) }
            );
          }
          ...
          if (bal < cost) {
            clearPendingAction(chatId);
            return sendTracked(chatId, <code>❌ Not enough points.\nYou have: ${bal.toFixed(2)} pts\nNeed: ${cost} pts<code>, { ...mainMenuKeyboard(chatId) });
          }
          ...
          const adminText =
            <code>🎁 *Redeem Request*\n<code> +
            <code>ChatID: \<code>${mdEscape(chatId)}\<code>\n<code> +
            <code>User: *${mdEscape(req.name || "User")}*\n<code> +
            <code>Item: *${mdEscape(req.itemLabel)}*\n<code> +
            <code>Cost: *${mdEscape(cost)} pts*\n<code> +
            <code>Load To: *${mdEscape(req.loadTo)}*\n<code> +
            <code>Total purchases: *${mdEscape(totalPurchases)}*\n<code> +
            <code>Time: \<code>${mdEscape(req.time)}\<code>\n\n<code> +
            <code>Approve or cancel:<code>;
  ...
  const sent = await bot.sendMessage(ADMIN_ID, adminText, {
            ...
            clearPendingRedeem(chatId);
            return sendTracked(chatId, <code>⚠️ Failed to submit redeem request. Points refunded: ${cost} pts<code>, { ...mainMenuKeyboard(chatId) });
          }
...
if (text === "⬅ Withdraw Menu") {
  return sendTracked(
    chatId,
    <code>💸 Withdraw Points

Your balance: ${getPoints(chatId).toFixed(2)} pts

Choose amount to withdraw:<code>,
    withdrawKeyboard()
  );
}
...
if (text === "🔗 My Referral" || text === "📎 My Referral" || /^\s*my\s+referral\s*$/i.test(text) || /\bmy\s+referral\b/i.test(text)) {
  console.log("🔗 My Referral clicked:", chatId, "text=", text);
  const referralCode = <code>ref_${chatId}<code>;
  const referralCommand = <code>/start ${referralCode}<code>;
  // ✅ Referral link now matches the same payload as the command (no extra suffix)
  const referralLink = <code>https://t.me/${BOT_USERNAME}?start=${referralCode}<code>;
  ...
  const shareText =
    <code>Join Bingwa Mtaani using my referral and earn rewards after your first successful purchase:
${referralLink}<code>;
  const shareUrl =
    <code>https://t.me/share/url?url=${encodeURIComponent(referralLink)}&text=${encodeURIComponent(shareText)}<code>;

  const leaderboardText = formatTodayLeaderboard(10);

  const pts = Number(u.points || 0).toFixed(2);

  return sendTracked(
    chatId,
    <code>🔗 <b>Your Referral Details</b>

<code> +
      <code>📌 <b>Referral Code:</b>
<code> +
      <code>\<code>${referralCode}\<code>

<code> +
      <code>📌 <b>Referral Command:</b>
<code> +
      <code>\<code>${referralCommand}\<code>

<code> +
      <code>🌍 <b>Referral Link:</b>
<code> +
      <code>${referralLink}

<code> +
      <code>⭐ Your points: *${pts} pts*
<code> +
      <code>👤 Total successful referrals: *${totalReferrals}*
<code> +
      <code>🤑 Total earned: *Ksh ${Number(totalEarned || 0).toFixed(2)}*

<code> +
      <code>📌 Rule: You earn when your referred user completes a successful purchase.<code>,
    { parse_mode: "HTML", ...mainMenuKeyboard(chatId) }
  );
}
  ...
  if (text === "🛒 Buy Offers") {
    ...
    sessions.set(chatId, { step: "category", category: null, pkgKey: null, createdAt: Date.now() });
    await sendBannerIfAvailable(chatId);
    await sendTracked(chatId, packagesOverviewText(), { parse_mode: "HTML" });
    return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard());
  }
  ...
  if (text === "🔄 Resend STK") {
    ...
    if (!isAdmin(chatId) && lastAt && now - lastAt < 60 * 1000) {
      ...
      return sendTracked(chatId, <code>⏳ You must wait ${wait}s before resending STK.<code>, { ...mainMenuKeyboard(chatId) });
    }
    ...
    try {
      ...
      return sendTracked(chatId, <code>⚠️ Resend failed: ${err.message}<code>, { ...mainMenuKeyboard(chatId) });
    }
  }
...
Keep buying to unlock withdrawals.<code>,
      { parse_mode: "HTML", ...mainMenuKeyboard(chatId) }
    );
  }
...
if (["25 pts", "50 pts", "100 pts", "250 pts", "500 pts", "1000 pts", "2000 pts"].includes(text)) {
  ...
  if (!isAdmin(chatId) && Number(u.totalSpentKsh || 0) < MIN_WITHDRAW_SPEND) {
    const spent = Number(u.totalSpentKsh || 0);
    const remaining = MIN_WITHDRAW_SPEND - spent;
    return sendTracked(chatId, <code>🚫 Withdrawal Locked. Spend at least Ksh ${MIN_WITHDRAW_SPEND} first.

Total spent: Ksh ${spent}
Remaining: Ksh ${remaining}<code>, { ...mainMenuKeyboard(chatId) });
  }

  if (bal < amount) {
    return sendTracked(chatId, <code>❌ Not enough points. You have ${bal} pts<code>, { ...mainMenuKeyboard(chatId) });
  }
  ...
  const reqId = <code>WD-${Date.now()}-${chatId}<code>;
...
/dc ${chatId}<code>,
  {
    ...
    reply_markup: {
      inline_keyboard: [
        [
          { text: "✅ Approve", callback_data: <code>ac:${chatId}<code> },
          { text: "❌ Decline", callback_data: <code>dc:${chatId}<code> },
        ],
      ],
    },
  }
...
return sendTracked(
    chatId,
    <code>⏳ Withdrawal request submitted.
Amount: ${amount} pts
  ...
  try {
    ...
    if (text === "⬅ Back") {
      if (s.step === "phone" || s.step === "confirm") {
        s.step = "package";
        sessions.set(chatId, s);
        return sendTracked(chatId, <code>✅ Choose a ${s.category} package:<code>, packagesKeyboard(s.category));
      }
      s.step = "category";
      s.category = null;
      s.pkgKey = null;
    ...
    if (s.step === "category") {
      ...
      if (category === "Bingwa Deals") {
        await sendTracked(chatId, <code>📦 *Bingwa Deals*\nRule: *Once per day per phone number* (ONLY after success).\n\nChoose a package:<code>, {
          parse_mode: "HTML",
        });
      } else {
        await sendTracked(chatId, <code>✅ Choose a ${category} package:<code>);
      }

      return sendTracked(chatId, <code>👇 ${category} packages:<code>, packagesKeyboard(category));
    }
...
if (s.step === "package") {
  ...
  const msgText =
    <code>✅ Selected:\n*${pkg.label}*\n\n<code> +
    (hasSaved 
      ? <code>📱 Saved number: *${maskPhone(savedPhone)}*\n\n<code> 
      : <code>📱 Paste new number.\n\n<code>) +
    <code>Choose:\n• ✅ Proceed (use saved number)\n• 📞 Change Number<code>;
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        if (recent) {
          return sendTracked(
            chatId,
            <code>⚠️ *STK already sent recently*\n\nIf you receive the M-PESA prompt, complete it.\n\nIf no prompt appears within 1 minute:<code>,
            {
              parse_mode: "HTML",
              reply_markup: {
                inline_keyboard: [
                  [{ text: "✅ I have the prompt", callback_data: <code>ui:stk_have_prompt:${recent.ref}<code> }],
                  [{ text: "🔁 Retry STK", callback_data: <code>retry_stk:${recent.ref}<code> }],
                  [{ text: "❌ Cancel Pending", callback_data: <code>ui:stk_cancel:${recent.ref}<code> }],
  ...
  return sendTracked(chatId,
    ...
    { ...mainMenuKeyboard(chatId) }
        ...
        if (blockedUntil) {
          ...
          return sendTracked(chatId, <code>⛔ STK temporarily blocked due to repeated failures.\nTry again in ${secs}s.<code>, { ...mainMenuKeyboard(chatId) });
        }
        ...
        await notifyAdmin(
          <code>📌 *STK Attempt*\n<code> +
            <code>User: ${mdEscape(msg.from?.first_name || "")} (@${mdEscape(msg.from?.username || "no_username")})\n<code> +
            <code>ChatID: \<code>${mdEscape(chatId)}\<code>\n<code> +
            <code>Category: *${mdEscape(s.category)}*\n<code> +
            <code>Offer: *${mdEscape(pkg.label)}*\n<code> +
            <code>Phone: *${mdEscape(formatTo07(savedPhone))}*\n<code> +
            <code>Amount: *Ksh ${mdEscape(pkg.price)}*\n<code> +
            <code>Channel: *${mdEscape(channelId)}*\n<code> +
            <code>Ref: \<code>${mdEscape(ref)}\<code>\n<code> +
            <code>Time: ${mdEscape(nowISO())}<code>
        );
        ...
        try {
          ...
          try {
            ...
            const t = setTimeout(async () => {
              try {
                ...
                if (alreadyProcessed(ref) || alreadyProcessed(<code>FAIL-${ref}<code>)) return;
                await sendTracked(
                  chatId,
                  <code>⏳ Reminder: You haven\'t completed the payment yet.\n\nTap 🔄 Resend STK if you didn\'t receive the prompt.\n\nHelp: ${HELP_PHONE}<code>,
                  { ...mainMenuKeyboard(chatId) }
                );
              } catch (_) {}
            }, 2 * 60 * 1000);
          ...
          if (isStkDelayedError(err)) {
            await notifyAdmin(
              <code>⚠️ *STK Delayed*
ChatID: \<code>${mdEscape(chatId)}\<code>
Ref: \<code>${mdEscape(ref)}\<code>
Error: \<code>${mdEscape(
            ...
            return sendTracked(
              chatId,
              <code>⚠️ *STK Request Delayed*

<code> +
                <code>If you receive the M-PESA prompt, complete it.

<code> +
                <code>If no prompt appears within 1 minute:
<code>,
              {
                parse_mode: "HTML",
                reply_markup: {
                  inline_keyboard: [
                    [{ text: "🔁 Retry STK", callback_data: <code>ui:stk_retry_prompt:${ref}<code> }],
                    [{ text: "❌ Cancel Pending", callback_data: <code>ui:stk_cancel:${ref}<code> }],
                    [{ text: "🛒 Buy Offers", callback_data: "ui:buy_offers" }],
                  ],
                },
...
Error: \<code>${mdEscape(
          ...
          return sendTracked(chatId, <code>⚠️ Error: ${err.message}<code>, { ...mainMenuKeyboard(chatId) });
        }
      }
...
await sendTracked(
  chatId,
  <code>✅ Saved number: ${maskPhone(phoneMaybe)}\nNow tap ✅ Proceed.<code>,
  {
...
if (s.step === "phone") {
  ...
  return sendTracked(
    chatId,
    <code>✅ Number saved: *${maskPhone(phone)}*\n\nSelected: *${pkg.label}*\n\nTap ✅ Proceed to send STK.<code>,
    {
      parse_mode: "HTML",
      ...confirmKeyboard(true),
    }
...
setInterval(async () => {
  try {
    ...
    if (isMonday && db.stats.lastLeaderboardWeek !== weekKey) {
      ...
      for (const uid of userIds) {
        // eslint-disable-next-line no-await-in-loop
        await bot.sendMessage(uid, text, { parse_mode: "HTML", ...mainMenuKeyboard(uid) }).catch(() => {});
        // eslint-disable-next-line no-await-in-loop
        await sleep(35);
      }
    ...
    for (const [cid, u] of Object.entries(db.users || {})) {
      ...
      await bot.sendMessage(
        Number(cid),
        <code>👋 We miss you!\n\nCome back today and grab your best offer.\n\n🎁 Tip: Share your referral link to earn points on every successful purchase!<code>,
...
bot.on("message", async (msg) => {
  ...
  await bot.sendMessage(
    msg.chat.id,
    <code>📈 Net Summary (Last ${days} day(s))\n\n<code> +
    <code>Gross Revenue: Ksh ${revenue.revenueKsh}\n<code> +
    <code>Orders: ${revenue.orders}\n<code> +
    <code>Withdrawals: Ksh ${withdrawalsKsh} (${withdrawalCount})\n<code> +
    <code>Net: Ksh ${net}<code>
  );
});
...
bot.on("message", async (msg) => {
  ...
  await bot.sendMessage(
    msg.chat.id,
    <code>📊 Profit Analytics (Last ${days} day(s))\n\n<code> +
    <code>Gross Revenue: Ksh ${metrics.grossRevenue.toFixed(2)}\n<code> +
    <code>Estimated Cost: Ksh ${metrics.totalCost.toFixed(2)}\n<code> +
    <code>Gross Profit: Ksh ${metrics.grossProfit.toFixed(2)}\n<code> +
    <code>Profit Margin: ${metrics.marginPercent}%<code>
  );
});
...
payheroStkPush = async function (payload) {
  ...
  if (!check.ok) {
    const overrideId = <code>PH-${Date.now()}-${chatId}<code>;
    ...
    await bot.sendMessage(
      ADMIN_ID,
      <code>🚩 Phone Reuse Alert

Phone: ${formatTo07(phone)}
Requested By: ${chatId}
Currently Locked By: ${check.lockedBy}

Approve override?<code>,
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "✅ Approve", callback_data: <code>phone_approve:${overrideId}<code> }],
            [{ text: "❌ Decline", callback_data: <code>phone_decline:${overrideId}<code> }]
          ]
        }
      }
    );
...
bot.on("callback_query", async (q) => {
  ...
  if (action === "phone_approve") {
    ...
    await bot.sendMessage(
      ...
      <code>✅ Admin approved phone usage override. You may proceed.<code>,
  ...
  if (action === "phone_decline") {
    ...
    await bot.sendMessage(
      Number(req.requestingChatId),
      <code>❌ Phone usage denied. This number was used by another account in last 24 hours.<code>
    ).catch(() => {});
...
bot.on("message", async (msg) => {
  ...
  if (isWithdrawalLocked(chatId)) {
    ...
    await bot.sendMessage(
      chatId,
      <code>🚫 Withdrawal temporarily locked due to suspicious activity.

Time remaining: ${hoursLeft} hour(s).
Risk Score: ${profile.riskScore}<code>
    );
    return;
  }
});
...
bot.on("message", async (msg) => {
  ...
  await bot.sendMessage(
    msg.chat.id,
    <code>🚨 Risk Profile for ${targetId}

Override Attempts: ${profile.overrideAttempts}
Risk Score: ${profile.riskScore}
Withdrawal Locked: ${Date.now() < profile.withdrawalLockUntil ? "YES" : "NO"}<code>
  );
});
...
function checkAndApplyRiskBlock(chatId) {
  ...
  if (profile.riskScore > RISK_ALERT_THRESHOLD) {
    ...
    bot.sendMessage(
      ADMIN_ID,
      <code>🚨 HIGH RISK ALERT

User: ${chatId}
Risk Score: ${profile.riskScore}

User has been temporarily blocked from withdrawals for 72 hours.<code>
    ).catch(()=>{});
  }
}
...
bot.on("message", async (msg) => {
  ...
  if (blockMatch) {
    ...
    await bot.sendMessage(msg.chat.id, <code>⛔ User ${targetId} blocked for 72 hours.<code>);
    return;
  }
  ...
  if (unblockMatch) {
    ...
    await bot.sendMessage(msg.chat.id, <code>✅ User ${targetId} unblocked.<code>);
    return;
  }
});
...
maybeRewardInviterOnSuccessPurchase = function(buyerChatId, amountKsh, when) {
  ...
  bot.sendMessage(
    inviterId,
    <code>🎉 Referral Bonus Earned!

<code> +
    <code>Purchase: Ksh ${amountKsh}
<code> +
    <code>Rate: ${(rate * 100).toFixed(2)}%
<code> +
    <code>Reward: Ksh ${rewardKsh.toFixed(2)}<code>
  ).catch(()=>{});
};
...
Your Total Spend: Ksh ${totalSpent}<code>,
      { parse_mode: "HTML" }
    );
    return;
  }
...
setWeeklyDrawAdminState(chatId, null);
        const statusText = formatWeeklyDrawAdminStatus();
        await editAdminStateMessage(chatId, _wdState, <code>✅ Prizes updated.\n\n${statusText}<code>, weeklyDrawAdminMenuKeyboard());
        return;
      }
    }
  ...
  if (_wdState.mode === "set_period") {
    ...
    if (parts.length >= 2) {
      ...
      if (Number.isFinite(s) && Number.isFinite(e) && e > s) {
        ...
        setWeeklyDrawAdminState(chatId, null);
        const statusText = formatWeeklyDrawAdminStatus();
        await editAdminStateMessage(chatId, _wdState, <code>✅ Period updated.\n\n${statusText}<code>, weeklyDrawAdminMenuKeyboard());
        return;
      }
    }
  ...
  if (_wdState.mode === "set_heading") {
    ...
    if (heading.length >= 3) {
      ...
      setWeeklyDrawAdminState(chatId, null);
      const statusText = formatWeeklyDrawAdminStatus();
      await editAdminStateMessage(chatId, _wdState, <code>✅ Heading updated.\n\n${statusText}<code>, weeklyDrawAdminMenuKeyboard());
       return;
    }
...
if (_wdState.mode === "set_qual") {
  ...
  if (!res.ok) {
    await editAdminStateMessage(chatId, _wdState, <code>❌ ${res.err || "Invalid value."}\n\n${qualificationPromptFor(key)}<code>, weeklyDrawQualificationsKeyboard());
    return;
  }
  try { saveDB(); } catch (_) {}
  setWeeklyDrawAdminState(chatId, null);
  await editAdminStateMessage(chatId, _wdState, "✅ Qualification updated.", weeklyDrawQualificationsKeyboard());
   return;
        ...
        const showCampaignStatus = () => {
          ...
          const msgText =
            <code>🎉 ${heading.toUpperCase()}\n<code> +
            <code>Enabled: ${enabled}\n<code> +
            <code>Status: ${st}\n<code> +
            <code>Period: ${startAt} → ${endAt}\n<code> +
            <code>Prizes: 1st ${Number(p.first || 0)} / 2nd ${Number(p.second || 0)} / 3rd ${Number(p.third || 0)} (max 500)\n<code> +
            <code>Entries: ${entryCount}\n\n<code> +
            <code>Winners:\n${winners}\n\n<code> +
            <code>Commands:\n<code> +
            <code>/weeklydraw_on | /weeklydraw_off\n<code> +
            <code>/weeklydraw_start [days]\n<code> +
            <code>/weeklydraw_period YYYY-MM-DD YYYY-MM-DD\n<code> +
            <code>/weeklydraw_heading <text>\n<code> +
            <code>/weeklydraw_prizes <first> <second> <third>\n<code> +
            <code>/weeklydraw_end | /weeklydraw_restart\n<code> +
            <code>/weeklydraw_pay <1|2|3|chatId> | /weeklydraw_unpay <1|2|3|chatId>\n<code> +
            <code>/weeklydraw_remove <chatId>\n<code>;
        ...
        if (mStart) {
          ...
          db.campaign.winners = [];
          try { saveDB(); } catch (_) {}
          return bot.sendMessage(chatId, <code>✅ WEEKLY DRAW started for ${days} day(s).<code>, mainMenuKeyboard(chatId)).catch(() => {});
        }
        ...
        if (mPeriod) {
          ...
          db.campaign.winners = [];
          try { saveDB(); } catch (_) {}
          return bot.sendMessage(chatId, <code>✅ WEEKLY DRAW period set: ${mPeriod[1]} → ${mPeriod[2]}<code>, mainMenuKeyboard(chatId)).catch(() => {});
        }
        ...
        if (mPr) {
          ...
          const total = first + second + third;
          if (total > 500) {
            return bot.sendMessage(chatId, <code>❌ Total prizes cannot exceed 500 KSH. You set ${total}.<code>, mainMenuKeyboard(chatId)).catch(() => {});
          }
          db.campaign.prizes = { first, second, third };
          try { saveDB(); } catch (_) {}
          return bot.sendMessage(chatId, <code>✅ Weekly Draw prizes set: ${first}/${second}/${third} (Total ${total}).<code>, mainMenuKeyboard(chatId)).catch(() => {});
        }
        ...
        if (/^\/weeklydraw_end$/i.test(text)) {
          ...
          const winners = (db.campaign.winners || []).map((w, i) => {
            const pos = i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉";
            return <code>${pos} ${w.name || w.chatId} — Ksh ${Number(w.prize || 0)} (Pending ⏳)<code>;
          }).join("\n") || "No eligible entries.";
          return bot.sendMessage(chatId, <code>🏁 WEEKLY DRAW ended.\n\n🏆 Winners:\n${winners}<code>, mainMenuKeyboard(chatId)).catch(() => {});
        }
        ...
        if (mRm) {
          ...
          try { saveDB(); } catch (_) {}
          return bot.sendMessage(chatId, <code>✅ Removed ${id} from Weekly Draw.<code>, mainMenuKeyboard(chatId)).catch(() => {});
        }
      }
    });
      ...
      try {
        ...
        try {
          if (typeof notifyAdmin === "function") {
            notifyAdmin(
              <code>🎯 Referral Reward\n<code> +
              <code>Inviter: ${inviterId}\n<code> +
              <code>Buyer: ${buyerChatId}\n<code> +
              <code>Amount: Ksh ${amt}\n<code> +
              <code>Rate: ${(rate*100).toFixed(2)}%\n<code> +
              <code>Reward: Ksh ${rewardKsh.toFixed(2)}\n<code> +
              <code>Time: ${when || ""}<code>
            ).catch(() => {});
          }
        } catch (_) {}
