...
function weeklyDrawBroadcastText() {
  ...
  const how = "✅ Tap ${tr(chatId,"🎉 WEEKLY DRAW","🎉 DROO YA WIKI")} in the bot menu to view status, join/leave, and track your rank.";
...
if (isWeeklyDrawOptedIn(chatId)) {
  ...
  if (myPos >= 1 && myPos <= 3) {
    ...
    optedLine = tr(chatId, "${tr(chatId,"✅ You are opted in (you will receive updates).","✅ Umejiunga (utapokea taarifa).")}", "✅ Umejiunga (utapokea taarifa).");
  }
}
...
function translateOutgoing(chatId, text) {
  ...
  const repl = [
    ...
    ["ℹ️ Help", "ℹ️ Msaada"],
    ["${tr(chatId,"🎉 WEEKLY DRAW","🎉 DROO YA WIKI")}", "🎉 DROO YA WIKI"],
...
function mainMenuKeyboard(chatId = null) {
  ...
  if (db.campaign && db.campaign.enabled === true) {
    keyboard.push([tr(chatId,"${tr(chatId,"🎉 WEEKLY DRAW","🎉 DROO YA WIKI")}","🎉 DROO YA WIKI")]);
  }
...
if (isBtn(text,"🛒 Buy Offers","🛒 Nunua Ofa")) {
  ...
  if (isBtn(text, "${tr(chatId,"🎉 WEEKLY DRAW","🎉 DROO YA WIKI")}", "🎉 DROO YA WIKI") || /^\/weeklydraw(?:@\w+)?$/i.test(text) || /^droo ya wiki$/i.test(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    try { sessions.delete(chatId); } catch (_) {}
    ...
    global.mainMenuKeyboard = function mainMenuKeyboard(chatId = null) {
      ...
      if (chatId && typeof getQuickCategories === "function") {
        ...
        if (Array.isArray(baseKb)) {
          ...
          const cleaned = baseKb.filter(row => !(Array.isArray(row) && row.length === 1 && row[0] === "${tr(chatId,"🎉 WEEKLY DRAW","🎉 DROO YA WIKI")}"));
