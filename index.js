...
function withdrawKeyboard() {
  ...
  const plans = Array.isArray(cfg.plans) && cfg.plans.length ? cfg.plans.slice().sort((a,b)=>a-b) : DEFAULT_WITHDRAW_PLANS.slice();
  const rows = [];
  for (let i = 0; i < plans.length; i += 2) {
    const row = [`${plans[i]} pts`];
    if (plans[i + 1]) row.push(`${plans[i + 1]} pts`);
    rows.push(row);
  }
...
const isWithdrawEntry = (
  isBtn(text,"💸 Withdraw Points","💸 Toa Pointi") ||
  /^\/withdraw(?:@\w+)?$/i.test(text) ||
  /^(?:💸\s*)?Withdraw Points$/i.test(text) ||
  /^(?:💸\s*)?Toa Pointi$/i.test(text)
);
...
if (isWithdrawEntry) {
  ...
  // ✅ Admin bypasses ALL limitations
  if (!isAdmin(chatId)) {
    const wdGate = canUserWithdrawNow(chatId, 0);
    if (!wdGate.ok) {
      return sendTracked(chatId, wdGate.message || "🚫 Withdrawal blocked.", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
    }
  }
...
if (isWithdrawAmountText(text)) {
  const amount = Number(String(text).split(" ")[0]);
  ...
  // ✅ Admin bypasses ALL limitations
  if (!isAdmin(chatId)) {
    const wdGate = canUserWithdrawNow(chatId, amount);
    if (!wdGate.ok) {
      return sendTracked(chatId, wdGate.message || "🚫 Withdrawal blocked.", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
    }
  }
...
bot.on("message", async (msg) => {
  ...
  // Reply-keyboard fallbacks for admin tools.
  // This keeps admin controls working even if an inline callback is missed.
  if (["💸 Withdraw Manager", "Withdraw Manager"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    await sendTracked(chatId, withdrawManagerText(), withdrawManagerKb());
    return;
  }
  if (["✅ Withdraw Rules", "Withdraw Rules"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    await sendTracked(chatId, withdrawRequirementsText(), withdrawRequirementsKb());
    return;
  }
  if (["🙍 User Withdraw", "User Withdraw"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    const tid = getRememberedAdminTarget(chatId);
    await sendTracked(chatId, userWithdrawControlText(tid), userWithdrawControlKb(tid));
    return;
  }
  if (["💰 Points Manager", "Points Manager"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    await sendTracked(chatId, `💰 POINTS MANAGER\n\nChoose an action.`, pointsManagerKb());
    return;
  }
  if (["🕵️ User Inspector", "User Inspector"].includes(text)) {
    setPendingAction(chatId, { type: 'apx_inspect' });
    await sendTracked(chatId, 'Send user ID to inspect.', apxBackKb('apx:inspect'));
    return;
  }
  if (["📲 STK Tool", "STK Tool"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    await sendTracked(chatId, stkToolText(), stkToolKb());
    return;
  }
  if (["🏢 Business Tools", "Business Tools"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    await sendTracked(chatId, businessToolsText(), businessToolsKb());
    return;
  }
  if (["🚨 Reports", "Reports"].includes(text)) {
    try { clearPendingAction(chatId); } catch (_) {}
    await sendTracked(chatId, reportsText(), reportsKb());
    return;
  }
...
bot.on('callback_query', async (q) => {
  ...
  if (!chatId || !isAdmin(chatId) || !data.startsWith('apx:')) return;
  if (!adminInlineGuardOk(chatId, data)) {
    await bot.answerCallbackQuery(q.id, { text: 'Ignored duplicate tap' }).catch(() => {});
    return;
  }
  await bot.answerCallbackQuery(q.id).catch(() => {});
  const targetId = getRememberedAdminTarget(chatId);

  if (data === 'apx:wdm') return adminPanelSendOrEdit(chatId, withdrawManagerText(), withdrawManagerKb(), q);
  if (data === 'apx:wdm_toggle') { const cfg = getWithdrawManagerCfg(); cfg.enabled = !cfg.enabled; saveWithdrawManagerCfg(cfg); return adminPanelSendOrEdit(chatId, withdrawManagerText(), withdrawManagerKb(), q); }
  if (data === 'apx:wdm_rule_phone') { const cfg = getWithdrawManagerCfg(); cfg.autoDecline.phoneBan = !cfg.autoDecline.phoneBan; saveWithdrawManagerCfg(cfg); return adminPanelSendOrEdit(chatId, withdrawManagerText(), withdrawManagerKb(), q); }
  if (data === 'apx:wdm_rule_plans') { const cfg = getWithdrawManagerCfg(); cfg.autoDecline.outsidePlans = !cfg.autoDecline.outsidePlans; saveWithdrawManagerCfg(cfg); return adminPanelSendOrEdit(chatId, withdrawManagerText(), withdrawManagerKb(), q); }
  if (data === 'apx:wdm_setmin') { setPendingAction(chatId, { type: 'apx_wdm_setmin' }); return adminPanelSendOrEdit(chatId, 'Send new minimum withdraw in points.', apxBackKb('apx:wdm'), q); }
  if (data === 'apx:wdm_setmax') { setPendingAction(chatId, { type: 'apx_wdm_setmax' }); return adminPanelSendOrEdit(chatId, 'Send new maximum withdraw in points.', apxBackKb('apx:wdm'), q); }
  if (data === 'apx:wdm_setplans') { setPendingAction(chatId, { type: 'apx_wdm_setplans' }); return adminPanelSendOrEdit(chatId, 'Send withdraw plans like: 25 50 100 250', apxBackKb('apx:wdm'), q); }
  if (data === 'apx:wdm_addphone') { setPendingAction(chatId, { type: 'apx_wdm_addphone' }); return adminPanelSendOrEdit(chatId, 'Send phone to add to withdraw phone-ban list.', apxBackKb('apx:wdm'), q); }
  if (data === 'apx:wdm_delphone') { setPendingAction(chatId, { type: 'apx_wdm_delphone' }); return adminPanelSendOrEdit(chatId, 'Send phone to remove from withdraw phone-ban list.', apxBackKb('apx:wdm'), q); }
  if (data === 'apx:wdm_listphones') { const cfg = getWithdrawManagerCfg(); return adminPanelSendOrEdit(chatId, `📋 PHONE BAN LIST

${((cfg.phoneBanList || []).join('\n') || 'Empty')}`, apxBackKb('apx:wdm'), q); }

  if (data === 'apx:wdr') return adminPanelSendOrEdit(chatId, withdrawRequirementsText(), withdrawRequirementsKb(), q);
  if (data.startsWith('apx:wdr_toggle:')) { const k = data.split(':').pop(); const s = ensureAdminOpsStore(); s.withdrawRequirements[k] = !s.withdrawRequirements[k]; saveDB(); return adminPanelSendOrEdit(chatId, withdrawRequirementsText(), withdrawRequirementsKb(), q); }
  if (data === 'apx:wdr_setminbal') { setPendingAction(chatId, { type: 'apx_wdr_setminbal' }); return adminPanelSendOrEdit(chatId, 'Send minimum balance required in points.', apxBackKb('apx:wdr'), q); }

  if (data === 'apx:uwc') return adminPanelSendOrEdit(chatId, userWithdrawControlText(targetId), userWithdrawControlKb(targetId), q);
  if (data === 'apx:uwc_pick') { setPendingAction(chatId, { type: 'apx_uwc_pick' }); return adminPanelSendOrEdit(chatId, 'Send target user ID for withdraw control.', apxBackKb('apx:uwc'), q); }
  if (data.startsWith('apx:uwc_allow:')) { const tid = data.split(':').pop(); setUserWithdrawControl(tid, { allow: !getUserWithdrawControl(tid).allow, banned: false }); rememberAdminTarget(chatId, tid); return adminPanelSendOrEdit(chatId, userWithdrawControlText(tid), userWithdrawControlKb(tid), q); }
  if (data.startsWith('apx:uwc_ban:')) { const tid = data.split(':').pop(); const ctl = getUserWithdrawControl(tid); setUserWithdrawControl(tid, { banned: !ctl.banned, allow: ctl.banned ? true : false }); rememberAdminTarget(chatId, tid); return adminPanelSendOrEdit(chatId, userWithdrawControlText(tid), userWithdrawControlKb(tid), q); }
  if (data.startsWith('apx:uwc_unlimited:')) { const tid = data.split(':').pop(); const ctl = getUserWithdrawControl(tid); setUserWithdrawControl(tid, { unlimited: !ctl.unlimited }); rememberAdminTarget(chatId, tid); return adminPanelSendOrEdit(chatId, userWithdrawControlText(tid), userWithdrawControlKb(tid), q); }
  if (data.startsWith('apx:uwc_free:')) { const tid = data.split(':').pop(); const ctl = getUserWithdrawControl(tid); setUserWithdrawControl(tid, { freePromo: !ctl.freePromo }); rememberAdminTarget(chatId, tid); return adminPanelSendOrEdit(chatId, userWithdrawControlText(tid), userWithdrawControlKb(tid), q); }
  if (data.startsWith('apx:uwc_setmin:')) { const tid = data.split(':').pop(); rememberAdminTarget(chatId, tid); setPendingAction(chatId, { type: 'apx_uwc_setmin', targetId: tid }); return adminPanelSendOrEdit(chatId, 'Send per-user minimum withdraw in points.', apxBackKb('apx:uwc'), q); }
  if (data.startsWith('apx:uwc_setmax:')) { const tid = data.split(':').pop(); rememberAdminTarget(chatId, tid); setPendingAction(chatId, { type: 'apx_uwc_setmax', targetId: tid }); return adminPanelSendOrEdit(chatId, 'Send per-user maximum withdraw in points.', apxBackKb('apx:uwc'), q); }

  if (data === 'apx:pts') return adminPanelSendOrEdit(chatId, `💰 POINTS MANAGER

Choose an action.`, pointsManagerKb(), q);
  if (data === 'apx:pts_add') { setPendingAction(chatId, { type: 'apx_pts_add' }); return adminPanelSendOrEdit(chatId, 'Send: userId amount', apxBackKb('apx:pts'), q); }
  if (data === 'apx:pts_remove') { setPendingAction(chatId, { type: 'apx_pts_remove' }); return adminPanelSendOrEdit(chatId, 'Send: userId amount', apxBackKb('apx:pts'), q); }
  if (data === 'apx:pts_set') { setPendingAction(chatId, { type: 'apx_pts_set' }); return adminPanelSendOrEdit(chatId, 'Send: userId exactBalance', apxBackKb('apx:pts'), q); }
  if (data === 'apx:pts_transfer') { setPendingAction(chatId, { type: 'apx_pts_transfer' }); return adminPanelSendOrEdit(chatId, 'Send: fromUserId toUserId amount', apxBackKb('apx:pts'), q); }

  if (data === 'apx:inspect') { setPendingAction(chatId, { type: 'apx_inspect' }); return adminPanelSendOrEdit(chatId, 'Send user ID to inspect.', apxBackKb('apx:inspect'), q); }
  if (data === 'apx:stk') return adminPanelSendOrEdit(chatId, stkToolText(), stkToolKb(), q);
  if (data === 'apx:biz') return adminPanelSendOrEdit(chatId, businessToolsText(), businessToolsKb(), q);
  if (data === 'apx:biz_grant') { setPendingAction(chatId, { type: 'apx_biz_grant' }); return adminPanelSendOrEdit(chatId, 'Send: userId days', apxBackKb('apx:biz'), q); }
  if (data === 'apx:biz_remove') { setPendingAction(chatId, { type: 'apx_biz_remove' }); return adminPanelSendOrEdit(chatId, 'Send user ID to remove business account.', apxBackKb('apx:biz'), q); }

  if (data === 'apx:reports') return adminPanelSendOrEdit(chatId, reportsText(), reportsKb(), q);
  if (data === 'apx:reports_resolve') { const items = ensureAdminOpsStore().reports.items; const rec = items.find((x) => x.status === 'open'); if (rec) rec.status = 'resolved'; saveDB(); return adminPanelSendOrEdit(chatId, reportsText(), reportsKb(), q); }
  if (data === 'apx:reports_clear') { const s = ensureAdminOpsStore(); s.reports.items = s.reports.items.filter((x) => x.status === 'open'); saveDB(); return adminPanelSendOrEdit(chatId, reportsText(), reportsKb(), q); }
});
...
// ===================== INLINE ADMIN EXPANSION: WITHDRAW / POINTS / REPORTS =====================
const ADMIN_INLINE_CLICK_GUARD = new Map();
function adminInlineGuardOk(chatId, key) {
  try {
    const now = Date.now();
    const k = `${chatId}:${key}`;
    const last = Number(ADMIN_INLINE_CLICK_GUARD.get(k) || 0);
    if (now - last < 1200) return false;
    ADMIN_INLINE_CLICK_GUARD.set(k, now);
    return true;
  } catch (_) { return true; }
}

function ensureAdminOpsStore() {
  db = repairDB(db);
  if (!db.adminOps || typeof db.adminOps !== 'object') db.adminOps = {};
  const s = db.adminOps;
  if (!s.withdrawRequirements || typeof s.withdrawRequirements !== 'object') s.withdrawRequirements = {};
  const wr = s.withdrawRequirements;
  if (typeof wr.pointsSpending !== 'boolean') wr.pointsSpending = true;
  if (typeof wr.referrals !== 'boolean') wr.referrals = false;
  if (typeof wr.campaignParticipation !== 'boolean') wr.campaignParticipation = false;
  if (typeof wr.gmailPurchase !== 'boolean') wr.gmailPurchase = false;
  if (typeof wr.bingwaPurchase !== 'boolean') wr.bingwaPurchase = false;
  if (typeof wr.unlimitedPurchase !== 'boolean') wr.unlimitedPurchase = false;
  if (typeof wr.minimumBalance !== 'boolean') wr.minimumBalance = false;
  if (!Number.isFinite(Number(wr.minimumBalancePts))) wr.minimumBalancePts = 0;
  if (!s.userWithdrawControls || typeof s.userWithdrawControls !== 'object') s.userWithdrawControls = {};
  if (!s.reports || typeof s.reports !== 'object') s.reports = { items: [] };
  if (!Array.isArray(s.reports.items)) s.reports.items = [];
  return s;
}
function getUserWithdrawControl(chatId) {
  const s = ensureAdminOpsStore();
  const key = String(chatId || '');
  const rec = s.userWithdrawControls[key] && typeof s.userWithdrawControls[key] === 'object' ? s.userWithdrawControls[key] : {};
  if (typeof rec.allow !== 'boolean') rec.allow = true;
  if (typeof rec.banned !== 'boolean') rec.banned = false;
  if (typeof rec.unlimited !== 'boolean') rec.unlimited = false;
  if (typeof rec.freePromo !== 'boolean') rec.freePromo = false;
  rec.min = Number.isFinite(Number(rec.min)) ? Number(rec.min) : 0;
  rec.max = Number.isFinite(Number(rec.max)) ? Number(rec.max) : 0;
  s.userWithdrawControls[key] = rec;
  return rec;
}
function setUserWithdrawControl(chatId, patch) {
  const s = ensureAdminOpsStore();
  const key = String(chatId || '');
  const cur = getUserWithdrawControl(key);
  s.userWithdrawControls[key] = Object.assign({}, cur, patch || {});
  saveDB();
  return s.userWithdrawControls[key];
}
function isWithdrawAmountText(text) {
  const cfg = getWithdrawManagerCfg();
  const plans = Array.isArray(cfg.plans) && cfg.plans.length ? cfg.plans : DEFAULT_WITHDRAW_PLANS;
  return plans.map((x) => `${Number(x)} pts`).includes(String(text || '').trim());
}
function getUserTransactionsAll(chatId) {
  const out = [];
  try {
    const byDay = db.analytics && db.analytics.transactionsByDay ? db.analytics.transactionsByDay : {};
    for (const arr of Object.values(byDay)) {
      if (!Array.isArray(arr)) continue;
      for (const t of arr) if (String(t.chatId || '') === String(chatId || '')) out.push(t);
    }
  } catch (_) {}
  return out;
}
function userHasCategoryPurchase(chatId, matcher) {
  try {
    return getUserTransactionsAll(chatId).some((t) => matcher(String(t.category || ''), String(t.pkgLabel || t.reference || '')));
  } catch (_) { return false; }
}
function userCampaignParticipationCount(chatId) {
  try {
    const c = ensureWeeklyDrawObject();
    return Number(c.entries && c.entries[String(chatId)] || 0);
  } catch (_) { return 0; }
}
function referralsCount(chatId) {
  try { return Number(getUser(chatId).referralCount || 0); } catch (_) { return 0; }
}
function canUserWithdrawNow(chatId, amountPts) {
  try {
    const u = getUser(chatId);
    const cfg = getWithdrawManagerCfg();
    const reqs = ensureAdminOpsStore().withdrawRequirements;
    const ctl = getUserWithdrawControl(chatId);
    const amount = Number(amountPts || 0);
    if (!cfg.enabled && !ctl.freePromo) return { ok: false, message: '🚫 Withdraw is currently turned *OFF* by admin.' };
    if (ctl.banned) return { ok: false, message: '🚫 You are banned from withdraw.' };
    if (!ctl.allow) return { ok: false, message: '🚫 Withdraw is disabled for your account.' };
    if (amount > 0 && !ctl.unlimited) {
      const minAllowed = Number(ctl.min || cfg.minWithdraw || 0);
      const maxAllowed = Number(ctl.max || cfg.maxWithdraw || 0);
      if (minAllowed > 0 && amount < minAllowed) return { ok: false, message: `🚫 Minimum withdraw for your account is *${minAllowed} pts*.` };
      if (maxAllowed > 0 && amount > maxAllowed) return { ok: false, message: `🚫 Maximum withdraw for your account is *${maxAllowed} pts*.` };
      if (cfg.autoDecline?.outsidePlans && Array.isArray(cfg.plans) && cfg.plans.length && !cfg.plans.includes(amount)) return { ok: false, message: '🚫 That withdraw amount is not in the allowed plans.' };
    }
    const phone = getUserPhone(chatId);
    if (cfg.autoDecline?.phoneBan && phone && isPhoneBannedForWithdraw(phone)) return { ok: false, message: '🚫 That phone number is blocked for withdraw.' };
    if (!ctl.freePromo && reqs.pointsSpending && Number(u.totalSpentKsh || 0) < MIN_WITHDRAW_SPEND) {
      const spent = Number(u.totalSpentKsh || 0);
      return { ok: false, message: `🚫 Withdrawal Locked

You must spend at least *Ksh ${MIN_WITHDRAW_SPEND}* before you can withdraw points.

💰 Spent: Ksh ${spent}
🟡 Remaining: Ksh ${Math.max(0, MIN_WITHDRAW_SPEND - spent)}` };
    }
    if (reqs.referrals && referralsCount(chatId) < 1) return { ok: false, message: '🚫 You need at least *1 referral* before withdraw.' };
    if (reqs.campaignParticipation && userCampaignParticipationCount(chatId) < 1) return { ok: false, message: '🚫 You need campaign participation before withdraw.' };
    if (reqs.gmailPurchase && !userHasCategoryPurchase(chatId, (c,p) => /gmail/i.test(c) || /gmail/i.test(p))) return { ok: false, message: '🚫 Gmail purchase is required before withdraw.' };
    if (reqs.bingwaPurchase && !userHasCategoryPurchase(chatId, (c,p) => /bingwa/i.test(c) || /bingwa/i.test(p))) return { ok: false, message: '🚫 Bingwa purchase is required before withdraw.' };
    if (reqs.unlimitedPurchase && !userHasCategoryPurchase(chatId, (c,p) => /unlimited/i.test(c) || /unlimited/i.test(p))) return { ok: false, message: '🚫 Unlimited purchase is required before withdraw.' };
    if (reqs.minimumBalance && Number(getPoints(chatId) || 0) < Number(reqs.minimumBalancePts || 0)) return { ok: false, message: `🚫 Minimum balance of *${Number(reqs.minimumBalancePts || 0)} pts* is required.` };
    return { ok: true };
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: '📊 Today Stats', callback_data: 'ap:stats' }],
        [{ text: '👤 Users', callback_data: 'ap:users' }, { text: '🔒 Offer Toggles', callback_data: 'ap:offers' }],
        [{ text: '💸 Withdraw Manager', callback_data: 'apx:wdm' }, { text: '✅ Withdraw Rules', callback_data: 'apx:wdr' }],
        [{ text: '🙍 User Withdraw', callback_data: 'apx:uwc' }, { text: '💰 Points Manager', callback_data: 'apx:pts' }],
        [{ text: '🕵️ User Inspector', callback_data: 'apx:inspect' }, { text: '📲 STK Tool', callback_data: 'apx:stk' }],
        [{ text: '🏢 Business Tools', callback_data: 'apx:biz' }, { text: '🚨 Reports', callback_data: 'apx:reports' }],
        [{ text: '📣 Channels', callback_data: 'ap:channels' }, { text: '🏢 Business', callback_data: 'ap:business' }],
        [{ text: '📧 Gmail', callback_data: 'ap:gmail' }, { text: '🎉 Campaign', callback_data: 'ap:campaign' }],
        [{ text: '🏠 Main Menu', callback_data: 'ap:home' }]
      ]
...
function apxBackKb(back) { return { reply_markup: { inline_keyboard: [[{ text: '⬅️ Back', callback_data: back || 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]] } }; }
function withdrawManagerText() {
  const cfg = getWithdrawManagerCfg();
  return `💸 WITHDRAW MANAGER

Status: ${cfg.enabled ? 'ON' : 'OFF'}
Min: ${cfg.minWithdraw} pts
Max: ${cfg.maxWithdraw} pts
Plans: ${(cfg.plans || []).join(', ') || 'None'}
Phone auto-decline: ${cfg.autoDecline?.phoneBan ? 'ON' : 'OFF'}
Outside-plan auto-decline: ${cfg.autoDecline?.outsidePlans ? 'ON' : 'OFF'}
Phone ban list: ${(cfg.phoneBanList || []).length}`;
}
function withdrawManagerKb() {
  const cfg = getWithdrawManagerCfg();
  return { reply_markup: { inline_keyboard: [
    [{ text: cfg.enabled ? '🔴 Turn OFF' : '🟢 Turn ON', callback_data: 'apx:wdm_toggle' }],
    [{ text: '📉 Set Min', callback_data: 'apx:wdm_setmin' }, { text: '📈 Set Max', callback_data: 'apx:wdm_setmax' }],
    [{ text: '📦 Set Plans', callback_data: 'apx:wdm_setplans' }],
    [{ text: `${cfg.autoDecline?.phoneBan ? '✅' : '⬜️'} Phone Ban`, callback_data: 'apx:wdm_rule_phone' }, { text: `${cfg.autoDecline?.outsidePlans ? '✅' : '⬜️'} Outside Plans`, callback_data: 'apx:wdm_rule_plans' }],
    [{ text: '➕ Add Phone Ban', callback_data: 'apx:wdm_addphone' }, { text: '➖ Remove Phone Ban', callback_data: 'apx:wdm_delphone' }],
    [{ text: '📋 View Phone Ban List', callback_data: 'apx:wdm_listphones' }],
    [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
  ] } };
}
function withdrawRequirementsText() {
  const wr = ensureAdminOpsStore().withdrawRequirements;
  return `✅ WITHDRAW REQUIREMENTS

Points spending: ${wr.pointsSpending ? 'ON' : 'OFF'}
Referrals: ${wr.referrals ? 'ON' : 'OFF'}
Campaign participation: ${wr.campaignParticipation ? 'ON' : 'OFF'}
Gmail purchase: ${wr.gmailPurchase ? 'ON' : 'OFF'}
Bingwa purchase: ${wr.bingwaPurchase ? 'ON' : 'OFF'}
Unlimited purchase: ${wr.unlimitedPurchase ? 'ON' : 'OFF'}
Minimum balance: ${wr.minimumBalance ? 'ON' : 'OFF'} (${Number(wr.minimumBalancePts || 0)} pts)`;
}
function withdrawRequirementsKb() {
  const wr = ensureAdminOpsStore().withdrawRequirements;
  const row = (k,t) => ({ text: `${wr[k] ? '✅' : '⬜️'} ${t}`, callback_data: `apx:wdr_toggle:${k}` });
  return { reply_markup: { inline_keyboard: [
    [row('pointsSpending','Points Spending'), row('referrals','Referrals')],
    [row('campaignParticipation','Campaign'), row('gmailPurchase','Gmail Purchase')],
    [row('bingwaPurchase','Bingwa Purchase'), row('unlimitedPurchase','Unlimited Purchase')],
    [row('minimumBalance','Minimum Balance')],
    [{ text: '🔢 Set Min Balance', callback_data: 'apx:wdr_setminbal' }],
    [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
  ] } };
}
function userWithdrawControlText(targetId) {
  const ctl = getUserWithdrawControl(targetId || '0');
  return `🙍 USER WITHDRAW CONTROL

Target: ${targetId || 'not selected'}
Allow: ${ctl.allow ? 'YES' : 'NO'}
Banned: ${ctl.banned ? 'YES' : 'NO'}
Unlimited: ${ctl.unlimited ? 'YES' : 'NO'}
Free promo: ${ctl.freePromo ? 'YES' : 'NO'}
Min: ${Number(ctl.min || 0)} pts
Max: ${Number(ctl.max || 0)} pts`;
}
function userWithdrawControlKb(targetId) {
  const ctl = getUserWithdrawControl(targetId || '0');
  const tid = String(targetId || '0');
  return { reply_markup: { inline_keyboard: [
    [{ text: '🎯 Set User', callback_data: 'apx:uwc_pick' }],
    [{ text: ctl.allow ? '🚫 Disallow' : '✅ Allow', callback_data: `apx:uwc_allow:${tid}` }, { text: ctl.banned ? '✅ Unban' : '⛔ Ban', callback_data: `apx:uwc_ban:${tid}` }],
    [{ text: `${ctl.unlimited ? '✅' : '⬜️'} Unlimited`, callback_data: `apx:uwc_unlimited:${tid}` }, { text: `${ctl.freePromo ? '✅' : '⬜️'} Free Promo`, callback_data: `apx:uwc_free:${tid}` }],
    [{ text: '📉 Set User Min', callback_data: `apx:uwc_setmin:${tid}` }, { text: '📈 Set User Max', callback_data: `apx:uwc_setmax:${tid}` }],
    [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
  ] } };
}
function pointsManagerKb() { return { reply_markup: { inline_keyboard: [
  [{ text: '➕ Add Points', callback_data: 'apx:pts_add' }, { text: '➖ Remove Points', callback_data: 'apx:pts_remove' }],
  [{ text: '🎯 Set Exact Balance', callback_data: 'apx:pts_set' }],
  [{ text: '🔁 Transfer Points', callback_data: 'apx:pts_transfer' }],
  [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
] } }; }
function inspectorText(chatId) {
  const u = getUser(chatId || '0');
  const ws = u.withdrawStats || {};
  const txs = getUserTransactionsAll(chatId);
  const last = txs.sort((a,b)=>Number(b.ts||0)-Number(a.ts||0))[0] || null;
  return `🕵️ USER INSPECTOR

User: ${chatId}
Balance: ${Number(u.points || 0)} pts
Risk score: ${Number((getRiskProfile && getRiskProfile(chatId).riskScore) || 0)}
Total purchases: ${Number(u.totalPurchases || txs.length || 0)}
Last purchase: ${last ? `${last.pkgLabel || last.reference || 'Unknown'} / Ksh ${Number(last.amount || 0)}` : 'None'}
Join date: ${u.joinedAt ? kenyaDateTimeFromTs(Number(u.joinedAt)) : 'Unknown'}
Referral stats: ${Number(u.referralCount || 0)} referrals
Withdraw history: approved ${Number(ws.approvedCount || 0)} / declined ${Number(ws.declinedCount || 0)} / Ksh ${Number(ws.approvedKsh || 0)} approved`;
}
function reportsText() {
  const items = ensureAdminOpsStore().reports.items || [];
  const open = items.filter((x) => x.status === 'open');
  const lines = open.slice(0, 20).map((r, i) => `${i + 1}. #${r.id} by ${r.by} -> ${r.target}\nReason: ${r.reason}`).join('\n\n');
  return `REPORTS SYSTEM\n\nOpen reports: ${open.length}\n\n${lines || 'No open reports.'}`;
}

function reportsKb() { return { reply_markup: { inline_keyboard: [
  [{ text: '🔄 Refresh', callback_data: 'apx:reports' }],
  [{ text: '✅ Resolve Latest', callback_data: 'apx:reports_resolve' }, { text: '🗑 Clear Closed', callback_data: 'apx:reports_clear' }],
  [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
] } }; }
function businessToolsText() { return `🏢 BUSINESS ACCOUNT TOOLS\n\nGrant business duration or remove business account for a user.`; }
function businessToolsKb() { return { reply_markup: { inline_keyboard: [
  [{ text: '🎁 Grant Duration', callback_data: 'apx:biz_grant' }, { text: '❌ Remove Business', callback_data: 'apx:biz_remove' }],
  [{ text: '⬅️ Back', callback_data: 'ap:root' }, { text: '🏠 Main Menu', callback_data: 'ap:home' }]
] } }; }
function stkToolText() { return `📲 STK REQUEST TOOL\n\nUse command:\n/stk 254712345678 100\n\nOr tap below to get the format again.`; }
function stkToolKb() { return apxBackKb('ap:root'); }
function rememberAdminTarget(chatId, targetId) {
  db = repairDB(db);
  db.adminOpsLastTarget = db.adminOpsLastTarget || {};
  db.adminOpsLastTarget[String(chatId)] = String(targetId || '');
  saveDB();
}
function getRememberedAdminTarget(chatId) {
  try { return String((db.adminOpsLastTarget || {})[String(chatId)] || ''); } catch (_) { return ''; }
}
...
bot.on('message', async (msg) => {
  ...
  if (/^\/report\s+/i.test(text) && !isAdmin(chatId)) {
    const m = text.match(/^\/report\s+(\S+)\s+([\s\S]{3,})$/i);
    if (!m) return sendTracked(chatId, 'Use: /report <chat_id_or_phone> <reason>', { ...mainMenuKeyboard(chatId) });
    const s = ensureAdminOpsStore();
    const rec = { id: Date.now(), by: String(chatId), target: m[1], reason: m[2].trim(), status: 'open', ts: Date.now() };
    s.reports.items.unshift(rec);
    saveDB();
    try { await bot.sendMessage(ADMIN_ID, `🚨 New report\n\nFrom: ${chatId}\nTarget: ${rec.target}\nReason: ${rec.reason}`).catch(() => {}); } catch (_) {}
    return sendTracked(chatId, '✅ Report submitted. Admin will review it.', { ...mainMenuKeyboard(chatId) });
  }

  if (/^\/stk\s+/i.test(text)) {
    if (!isAdmin(chatId)) return sendTracked(chatId, '❌ Not authorized.', { ...mainMenuKeyboard(chatId) });
    const m = text.match(/^\/stk\s+(\+?\d{10,15})\s+(\d+(?:\.\d+)?)$/i);
    if (!m) return sendTracked(chatId, 'Use: /stk 254712345678 100', { ...mainMenuKeyboard(chatId) });
    const phone = normalizePhone(m[1]);
    const amount = Number(m[2]);
    if (!phone || amount <= 0) return sendTracked(chatId, '❌ Invalid phone or amount.', { ...mainMenuKeyboard(chatId) });
    const ref = `ADM-STK-${Date.now()}-${chatId}`;
    try {
      await payheroStkPush({ amount, phone, externalRef: ref, channelId: PAYHERO_CHANNEL_ID_DATA });
    } catch (e) {
      return sendTracked(chatId, `❌ STK failed: ${String((e && e.message) || e)}`, { ...mainMenuKeyboard(chatId) });
    }
    return sendTracked(chatId, `✅ STK request sent.\nPhone: ${phone}\nAmount: Ksh ${amount}\nRef: ${ref}`, { ...mainMenuKeyboard(chatId) });
  }

  if (!isAdmin(chatId)) return;
  const act = getPendingAction(chatId);
  if (!act || !String(act.type || '').startsWith('apx_')) return;
  if (text === '/cancel' || text === '❌ Cancel') {
    clearPendingAction(chatId);
  ...
  if (act.type === 'apx_wdm_setmin') { const cfg = getWithdrawManagerCfg(); cfg.minWithdraw = Math.max(1, Math.floor(num || 0)); saveWithdrawManagerCfg(cfg); clearPendingAction(chatId); return sendTracked(chatId, '✅ Minimum updated.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_wdm_setmax') { const cfg = getWithdrawManagerCfg(); cfg.maxWithdraw = Math.max(1, Math.floor(num || 0)); saveWithdrawManagerCfg(cfg); clearPendingAction(chatId); return sendTracked(chatId, '✅ Maximum updated.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_wdm_setplans') { const vals = Array.from(new Set(String(text).split(/[^\d]+/).map((x) => Math.floor(Number(x || 0))).filter((x) => x > 0))).sort((a, b) => a - b); const cfg = getWithdrawManagerCfg(); if (vals.length) { cfg.plans = vals; if (cfg.minWithdraw < vals[0]) cfg.minWithdraw = vals[0]; if (cfg.maxWithdraw < vals[0] || cfg.maxWithdraw > vals[vals.length - 1]) cfg.maxWithdraw = vals[vals.length - 1]; saveWithdrawManagerCfg(cfg); } clearPendingAction(chatId); return sendTracked(chatId, '✅ Withdraw plans updated.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_wdm_addphone') { const phone = normalizePhone(text); if (!phone) return sendTracked(chatId, '❌ Invalid phone.', { ...mainMenuKeyboard(chatId) }); const cfg = getWithdrawManagerCfg(); cfg.phoneBanList = Array.from(new Set([...(cfg.phoneBanList || []), phone])); saveWithdrawManagerCfg(cfg); clearPendingAction(chatId); return sendTracked(chatId, '✅ Phone added to withdraw ban list.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_wdm_delphone') { const phone = normalizePhone(text); if (!phone) return sendTracked(chatId, '❌ Invalid phone.', { ...mainMenuKeyboard(chatId) }); const cfg = getWithdrawManagerCfg(); cfg.phoneBanList = (cfg.phoneBanList || []).filter((x) => x !== phone); saveWithdrawManagerCfg(cfg); clearPendingAction(chatId); return sendTracked(chatId, '✅ Phone removed from withdraw ban list.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_wdr_setminbal') { const s = ensureAdminOpsStore(); s.withdrawRequirements.minimumBalancePts = Math.max(0, Math.floor(num || 0)); saveDB(); clearPendingAction(chatId); return sendTracked(chatId, '✅ Minimum balance requirement updated.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_uwc_pick') { if (!/^\d+$/.test(text)) return sendTracked(chatId, '❌ Send a numeric user ID.', { ...mainMenuKeyboard(chatId) }); rememberAdminTarget(chatId, text); clearPendingAction(chatId); return sendTracked(chatId, inspectorText(text), { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_uwc_setmin') { const tid = String(act.targetId || ''); setUserWithdrawControl(tid, { min: Math.max(0, Math.floor(num || 0)) }); rememberAdminTarget(chatId, tid); clearPendingAction(chatId); return sendTracked(chatId, '✅ User minimum updated.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_uwc_setmax') { const tid = String(act.targetId || ''); setUserWithdrawControl(tid, { max: Math.max(0, Math.floor(num || 0)) }); rememberAdminTarget(chatId, tid); clearPendingAction(chatId); return sendTracked(chatId, '✅ User maximum updated.', { ...mainMenuKeyboard(chatId) }); }

  if (act.type === 'apx_pts_add' || act.type === 'apx_pts_remove' || act.type === 'apx_pts_set') {
    const m = text.match(/^(\d+)\s+(-?\d+(?:\.\d+)?)$/);
    if (!m) return sendTracked(chatId, 'Use: userId amount', { ...mainMenuKeyboard(chatId) });
    const tid = m[1];
    const amt = Number(m[2]);
    const u = getUser(tid);
    if (act.type === 'apx_pts_add') u.points = Number(u.points || 0) + amt;
    if (act.type === 'apx_pts_remove') u.points = Math.max(0, Number(u.points || 0) - amt);
    if (act.type === 'apx_pts_set') u.points = Math.max(0, amt);
    saveDB();
    clearPendingAction(chatId);
    return sendTracked(chatId, '✅ Points updated.', { ...mainMenuKeyboard(chatId) });
  }

  if (act.type === 'apx_pts_transfer') {
    const m = text.match(/^(\d+)\s+(\d+)\s+(\d+(?:\.\d+)?)$/);
    if (!m) return sendTracked(chatId, 'Use: fromUserId toUserId amount', { ...mainMenuKeyboard(chatId) });
    const from = getUser(m[1]);
    const to = getUser(m[2]);
    const amt = Number(m[3]);
    if (Number(from.points || 0) < amt) return sendTracked(chatId, '❌ Source user lacks enough points.', { ...mainMenuKeyboard(chatId) });
    from.points = Number(from.points || 0) - amt;
    to.points = Number(to.points || 0) + amt;
    saveDB();
    clearPendingAction(chatId);
    return sendTracked(chatId, '✅ Transfer complete.', { ...mainMenuKeyboard(chatId) });
  }

  if (act.type === 'apx_inspect') { if (!/^\d+$/.test(text)) return sendTracked(chatId, '❌ Send a numeric user ID.', { ...mainMenuKeyboard(chatId) }); clearPendingAction(chatId); return sendTracked(chatId, inspectorText(text), { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_biz_grant') { const m = text.match(/^(\d+)\s+(\d+)$/); if (!m) return sendTracked(chatId, 'Use: userId days', { ...mainMenuKeyboard(chatId) }); await giveFreeBusinessDays(m[1], Number(m[2])); clearPendingAction(chatId); return sendTracked(chatId, '✅ Business duration granted.', { ...mainMenuKeyboard(chatId) }); }
  if (act.type === 'apx_biz_remove') { if (!/^\d+$/.test(text)) return sendTracked(chatId, '❌ Send a numeric user ID.', { ...mainMenuKeyboard(chatId) }); deactivateBusiness(text); clearPendingAction(chatId); return sendTracked(chatId, '✅ Business account removed.', { ...mainMenuKeyboard(chatId) }); }
});
