...
async function cleanupBeforeReply(chatId, userMsgId) {
  ...
  let allowDeletePreviousBotMessages = false;
  try {
    const u = getUser(chatId);
    allowDeletePreviousBotMessages = !!(u && u.autoDelete24hEnabled);
  } catch (_) {}
  if (!allowDeletePreviousBotMessages) return;
...
if (/^\/refaudit\s+\d+$/i.test(text)) {
  ...
  // cleanup user message + delete previous bot messages only when user enabled auto-delete (users only), except /start
