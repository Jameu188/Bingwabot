...
const BUSINESS_PLANS = {
  BUS_W1: { label: "Business 1 Week", price: 50, days: 7 },
  ...
  switch (key) {
    ...
    case "business":
      return (
        ...
        "To activate: tap *Activate* button.\\n"
      );
...
async function sendHelpAnswer(chatId, key, ctx = {}) {
  ...
  // Special: Business help should show an inline Activate button under the message,
  // and keep Back to Help / Main Menu on the reply keyboard.
  if (key === "business") {
    // Edit the help message if possible (keeps the help "page" in one place)
    await safeEditMessageText(chatId, ctx.messageId, text, {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [[{ text: "✅ Activate", callback_data: "biz_activate" }]],
      },
    }).catch(() => {});

    // Show navigation as reply keyboard (so it looks exactly like your screenshot)
    await sendTracked(chatId, "Choose option:", {
      reply_markup: {
        keyboard: [["⬅️ Back to Help", "🏠 Main Menu"]],
