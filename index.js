...
async function beginGmailPurchase(chatId, qty, sourceMessageId = null) {
  const count = gmailAvailableCount();
  const price = gmailPrice();
  ...
  if (savedPhone) {
    await confirmGmailPhoneAndPush(chatId, q, savedPhone, total, sourceMessageId);
    return;
  }

  setGmailAwait(chatId, { mode: "await_phone", qty: q, total, sourceMessageId });
  await bot.sendMessage(
    chatId,
...
async function confirmGmailPhoneAndPush(chatId, qty, phoneInput, total, sourceMessageId = null) {
  const phone254 = normalizePhone(phoneInput);
  const count = gmailAvailableCount();
  ...
  try {
    ...
    try {
      if (sourceMessageId) {
        await bot.deleteMessage(chatId, sourceMessageId).catch(() => {});
      }
    } catch (_) {}
...
bot.on("message", async (msg) => {
  try {
    ...
    if (state && state.mode === "await_phone" && text && !text.startsWith("/")) {
      await confirmGmailPhoneAndPush(chatId, Number(state.qty || 0), text, Number(state.total || 0), state.sourceMessageId || null);
      return;
    }
  } catch (_) {}
});
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data.startsWith("gmail_buy:")) {
      const qty = Math.max(1, Number(data.split(":")[1] || 0));
      await bot.answerCallbackQuery(q.id).catch(() => {});
      await beginGmailPurchase(chatId, qty, q.message?.message_id || null);
      return;
    }
  } catch (_) {}
});
...
// ============================================================================
// GMAIL ACCOUNTS STORE + EDITING FLOW
// ============================================================================
function ensureGmailStore() {
  db = repairDB(db);
  if (!db.gmailStore || typeof db.gmailStore !== "object") db.gmailStore = {};
  ...
  if (!db.gmailStore.pendingOrders || typeof db.gmailStore.pendingOrders !== "object") db.gmailStore.pendingOrders = {};
  if (!db.gmailStore.await || typeof db.gmailStore.await !== "object") db.gmailStore.await = {};
  if (!db.gmailStore.cfg || typeof db.gmailStore.cfg !== "object") db.gmailStore.cfg = { price: 29 };
  return db.gmailStore;
}
...
function gmailVisibleToUser(chatId) {
  return gmailAvailableCount() > 0 || (typeof isAdmin === "function" && isAdmin(chatId));
}

function setGmailAwait(chatId, state) {
  const s = ensureGmailStore();
  if (state) s.await[String(chatId)] = state;
  else delete s.await[String(chatId)];
  saveDB();
}
...
function addGmailStockFromLines(lines) {
  const s = ensureGmailStore();
  const seen = new Set((s.stock || []).map(x => String(x.email || "").toLowerCase()));
  let added = 0;
  let skipped = 0;
  for (const line of String(lines || "").split(/\r?\n/)) {
    const rec = normalizeGmailLine(line);
    ...
    seen.add(rec.email);
    s.stock.push({ email: rec.email, password: rec.password, createdAt: Date.now() });
    added += 1;
  }
  saveDB();
  return { added, skipped, total: s.stock.length };
}

function allocateUniqueGmails(qty) {
  const s = ensureGmailStore();
  const q = Math.max(1, Number(qty || 0));
  if (s.stock.length < q) return [];
  const items = s.stock.splice(0, q);
  saveDB();
  return items;
}

function gmailQuantityOptions(stockCount) {
  const count = Math.max(0, Number(stockCount || 0));
  if (count <= 0) return [];
  const percents = [1, 3, 10, 25, 50, 70, 90, 100];
  const out = [];
  const seen = new Set();

  for (const p of percents) {
    let qty = Math.round(count * (p / 100));
    if (qty < 1) qty = 1;
    if (qty > count) qty = count;
    if (qty === count) continue; // do not show buy all
    if (seen.has(qty)) continue;
    seen.add(qty);
    out.push(qty);
  }

  if (!seen.has(1)) out.unshift(1);
  out.sort((a, b) => a - b);
  return out;
}

function gmailButtons(stockCount) {
  const rows = [];
  for (const qty of gmailQuantityOptions(stockCount)) {
    rows.push([{
      text: qty === 1 ? "Buy 1 Email" : `Buy ${qty} Emails`,
      callback_data: `gmail_qty:${qty}`
    }]);
  }
  rows.push([{ text: "❌ Close", callback_data: "ui:close" }]);
  return { reply_markup: { inline_keyboard: rows } };
...
async function editGmailMessage(chatId, messageId, body, opts = {}) {
  try {
    await bot.editMessageText(body, {
      chat_id: chatId,
      message_id: messageId,
    ...
    try { console.log("editGmailMessage:", msg); } catch (_) {}
    return false;
  }
}
...
async function sendGmailOfferMenu(chatId, editCtx = null) {
  ...
  if (editCtx && editCtx.chat_id && editCtx.message_id) {
    await editGmailMessage(editCtx.chat_id, editCtx.message_id, body, kb);
    return;
  }
...
async function beginGmailPurchase(chatId, qty, sourceMessageId = null) {
  ...
  setGmailAwait(chatId, { mode: "await_phone", qty: q, total: total, sourceMessageId: sourceMessageId || null });
  await bot.sendMessage(
    chatId,
    `📧 Gmail Accounts\n\n${q === 1 ? "Package: 1 Email" : "Package: " + q + " Emails"}\nAmount: Ksh ${total}\n\nSend the M-Pesa phone number to pay.`
  ).catch(() => {});
}
...
async function confirmGmailPhoneAndPush(chatId, qty, phoneInput, total, sourceMessageId = null) {
  ...
  if (!phone254) {
    setGmailAwait(chatId, { mode: "await_phone", qty: qty, total: total, sourceMessageId: sourceMessageId || null });
    await bot.sendMessage(chatId, "❌ Invalid phone number. Send a valid Safaricom number like 07xxxxxxxx or 01xxxxxxxx.").catch(() => {});
    return;
  }
  ...
  const s = ensureGmailStore();
  s.pendingOrders[String(externalRef)] = {
    chatId: chatId,
    qty: qty,
    total: total,
    phone254: phone254,
  ...
  try {
    ...
    try {
      if (sourceMessageId) await bot.deleteMessage(chatId, sourceMessageId).catch(() => {});
    } catch (_) {}
    ...
    deletePendingPayment(externalRef);
    delete s.pendingOrders[String(externalRef)];
    saveDB();
    const errMsg = String(e && e.message || e || "Unknown error");
    await bot.sendMessage(chatId, `❌ Failed to send STK for Gmail Accounts.\n\n${errMsg}`).catch(() => {});
  }
...
async function fulfillGmailOrdersSweep() {
  try {
    const s = ensureGmailStore();
    for (const [ref, rec] of Object.entries(s.pendingOrders || {})) {
      try {
        if (!rec || rec.delivered) continue;
        if (!alreadyProcessed(ref)) continue;

        const qty = Math.max(1, Number(rec.qty || 0));
        const chatId = Number(rec.chatId || 0);

        if (gmailAvailableCount() < qty) {
          await bot.sendMessage(chatId, "⚠️ Payment confirmed, but Gmail stock is temporarily insufficient. Admin has been notified.").catch(() => {});
          await bot.sendMessage(String(ADMIN_ID), `⚠️ Gmail delivery pending for ${ref}. Needed ${qty}, available ${gmailAvailableCount()}.`).catch(() => {});
          continue;
        }
        ...
        const body =
          `✅ Payment Confirmed\n\n` +
          `🆔 Reference: ${ref}\n` +
          `📦 Package: ${qty === 1 ? "1 Gmail Account" : qty + " Gmail Accounts"}\n` +
          `💰 Amount: Ksh ${Number(rec.total || (qty * gmailPrice()))}\n` +
          `🛒 Total Purchases: ${totalPurchases}\n\n` +
          `Gmail account details\n` +
          `${details}` +
          `Instructions:\n` +
...
// Admin Gmail stock commands
bot.on("message", async (msg) => {
  try {
    const chatId = msg.chat.id;
    const text = String(msg.text || "").trim();
    if (!(typeof isAdmin === "function" && isAdmin(chatId))) return;
    ...
    const st = getGmailAwait(chatId);
    if (st && st.mode === "await_admin_stock" && text && !text.startsWith("/")) {
      const res = addGmailStockFromLines(text);
      setGmailAwait(chatId, null);
      await bot.sendMessage(chatId, `✅ Gmail stock updated.\nAdded: ${res.added}\nSkipped: ${res.skipped}\nAvailable now: ${res.total}`).catch(() => {});
      return;
...
bot.on("message", async (msg) => {
  try {
    ...
    const st = getGmailAwait(chatId);
    if (st && st.mode === "await_phone" && text && !text.startsWith("/")) {
      await confirmGmailPhoneAndPush(chatId, Number(st.qty || 0), text, Number(st.total || 0), st.sourceMessageId || null);
      return;
    }
  } catch (_) {}
});
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data.startsWith("gmail_qty:")) {
      ...
      const body =
        `📧 Gmail Accounts\n\n` +
        `${qty === 1 ? "Buy 1 Email" : "Buy " + qty + " Emails"}\n` +
        `Price each: Ksh ${gmailPrice()}\n` +
        `Total: Ksh ${total}\n\n` +
        `Tap Proceed to continue.`;
      ...
      await editGmailMessage(q.message.chat.id, q.message.message_id, body, opts);
      return;
    }
