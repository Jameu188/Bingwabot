...
if (!db0.analytics.transactionsByDay || typeof db0.analytics.transactionsByDay !== "object") db0.analytics.transactionsByDay = {};
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.blockBusinessPurchase !== "boolean") x.blockBusinessPurchase = false;
...
function deactivateBusiness(chatId) {
  ...
  // Also clear Business-only toggles so account returns to default behavior
  try {
    u.autoRetryStkEnabled = false;
    u.disableCampaignSystem = false;
    u.dailyReportEnabled = false;

    u.stopCampaignNotifs = false;
    u.stopCashbackNotifs = false;
    u.stopReferralNotifs = false;

    u.autoDelete24hEnabled = false;
    u.autoDeleteMs = 0;

    // optional state
    u.lastDailyReportDay = "";
  } catch (_) {}
  saveDB();
}

function isBusinessPurchaseBlocked(chatId) {
  const u = getUser(chatId);
  return !!u.blockBusinessPurchase;
}
...
*Business*
• /pro <chat_id> <months> — grant FREE Business
• /unpro <chat_id> — remove Business
• /blockpro <chat_id> — block/unblock buying Business
...
Tip: Use the bot's admin panel buttons for Weekly Draw settings, broadcast, paying winners, and resets.`;
  ...
  if (/^\/(ban|unban)(?:@\w+)?\b/i.test(text)) {
    ...
    const cmd = parts[0].t
  // ✅ /pro <chat_id> <months>  (Admin) - grant FREE Business
  // Example: /pro 68799996 1
  if (/^\/pro(?:@\w+)?\b/i.test(text)) {
    if (!isAdmin(chatId)) {
      await bot.sendMessage(chatId, "⛔ Not authorized.").catch(() => {});
      return;
    }
    ...
    const monthsRaw = parts[2] ? Number(parts[2]) : 1;

    const replyTarget = msg.reply_to_message?.chat?.id ? String(msg.reply_to_message.chat.id) : "";
    const finalTarget = replyTarget || target;

    if (!finalTarget || !/^\d+$/.test(finalTarget)) {
    ...
    // If blocked from buying business, still allow admin-grant
    try { activateBusiness(finalTarget, months); } catch (e) {
      await bot.sendMessage(chatId, `❌ Failed: ${String(e?.message || e)}`).catch(() => {});
      return;
    }

    await bot.sendMessage(chatId, `✅ Granted Business to ${finalTarget} for ${months} month(s).`).catch(() => {});
    try {
      await bot.sendMessage(
        finalTarget,
        `🎉 You have been upgraded to *Business* for *${months} month(s)* (FREE).\n\nGo to *Commands & Settings* → *Fast Purchase* to manage Business settings.`,
        { parse_mode: "Markdown", ...mainMenuKeyboard(Number(finalTarget)) }
      ).catch(() => {});
    } catch (_) {}
    return;
  }

  // ✅ /unpro <chat_id>  (Admin) - remove Business immediately
  if (/^\/unpro(?:@\w+)?\b/i.test(text)) {
    if (!isAdmin(chatId)) {
      await bot.sendMessage(chatId, "⛔ Not authorized.").catch(() => {});
      return;
    }
    ...
    try { deactivateBusiness(finalTarget); } catch (e) {
      await bot.sendMessage(chatId, `❌ Failed: ${String(e?.message || e)}`).catch(() => {});
      return;
    }

    await bot.sendMessage(chatId, `✅ Removed Business for ${finalTarget}.`).catch(() => {});
    try {
      await bot.sendMessage(finalTarget, "ℹ️ Your Business account has been removed by admin.", { ...mainMenuKeyboard(Number(finalTarget)) }).catch(() => {});
    } catch (_) {}
    return;
  }

  // ✅ /blockpro <chat_id>  (Admin) - block/unblock buying Business (toggle)
  if (/^\/blockpro(?:@\w+)?\b/i.test(text)) {
    if (!isAdmin(chatId)) {
      await bot.sendMessage(chatId, "⛔ Not authorized.").catch(() => {});
      return;
    }
    ...
    const u = getUser(finalTarget);
    u.blockBusinessPurchase = !u.blockBusinessPurchase;
    saveDB();

    await bot.sendMessage(chatId, u.blockBusinessPurchase ? `✅ Blocked ${finalTarget} from buying Business.` : `✅ Unblocked ${finalTarget} (can buy Business).`).catch(() => {});
    try {
      await bot.sendMessage(finalTarget, u.blockBusinessPurchase ? "⛔ You are blocked from buying Business subscription." : "✅ You can now buy Business subscription again.", { ...mainMenuKeyboard(Number(finalTarget)) }).catch(() => {});
    } catch (_) {}
    return;
  }

oLowerCase().replace(/^\//, "").split("@")[0];
    const target = parts[1] ? String(parts[1]).trim() : "";
...
if (/^\/sell$/i.test(text)) {
  ...
  if (isBusinessPurchaseBlocked(chatId)) {
    await sendTracked(
      chatId,
      "⛔ You are not allowed to buy Business subscription. Please contact support.",
      { ...mainMenuKeyboard(chatId) }
    );
    return;
  }
