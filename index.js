...
function extractSupportPayload(msg) {
  try {
    const caption = String(msg?.caption || "").trim();
    ...
    if (msg?.photo && Array.isArray(msg.photo) && msg.photo.length) {
      ...
      return { kind: "photo", fileId: String(largest.file_id || ""), caption };
    }
    if (msg?.video?.file_id) return { kind: "video", fileId: String(msg.video.file_id), caption };
    if (msg?.audio?.file_id) return { kind: "audio", fileId: String(msg.audio.file_id), caption };
    if (msg?.voice?.file_id) return { kind: "voice", fileId: String(msg.voice.file_id), caption };
    if (msg?.document?.file_id) return { kind: "document", fileId: String(msg.document.file_id), caption, fileName: String(msg.document.file_name || "") };
    if (msg?.sticker?.file_id) return { kind: "sticker", fileId: String(msg.sticker.file_id) };
    if (msg?.animation?.file_id) return { kind: "animation", fileId: String(msg.animation.file_id), caption };
    if (msg?.video_note?.file_id) return { kind: "video_note", fileId: String(msg.video_note.file_id) };
    if (msg?.contact) return { kind: "contact", contact: msg.contact };
    if (msg?.location) return { kind: "location", latitude: Number(msg.location.latitude || 0), longitude: Number(msg.location.longitude || 0) };
  } catch (_) {}
  return { kind: "unknown" };
}
...
function addAdminInboxMessage(chatId, msg, fromSide, extra = {}) {
  ...
  const payload = extractSupportPayload(msg);
  const item = {
...
async function sendStoredInboxMedia(chatId, item, fallbackSourceChatId) {
  const p = item?.payload || {};
  const kind = String(p.kind || "");
  try {
    if (kind === "text" && p.text) return await bot.sendMessage(chatId, String(p.text));
    if (kind === "photo" && p.fileId) return await bot.sendPhoto(chatId, p.fileId, p.caption ? { caption: String(p.caption) } : {});
    if (kind === "video" && p.fileId) return await bot.sendVideo(chatId, p.fileId, p.caption ? { caption: String(p.caption) } : {});
    if (kind === "audio" && p.fileId) return await bot.sendAudio(chatId, p.fileId, p.caption ? { caption: String(p.caption) } : {});
    if (kind === "voice" && p.fileId) return await bot.sendVoice(chatId, p.fileId, p.caption ? { caption: String(p.caption) } : {});
    if (kind === "document" && p.fileId) return await bot.sendDocument(chatId, p.fileId, p.caption ? { caption: String(p.caption) } : {});
    if (kind === "sticker" && p.fileId) return await bot.sendSticker(chatId, p.fileId);
    if (kind === "animation" && p.fileId) return await bot.sendAnimation(chatId, p.fileId, p.caption ? { caption: String(p.caption) } : {});
    if (kind === "video_note" && p.fileId) return await bot.sendVideoNote(chatId, p.fileId);
    if (kind === "contact" && p.contact?.phone_number) return await bot.sendContact(chatId, String(p.contact.phone_number), String(p.contact.first_name || "Contact"), { last_name: String(p.contact.last_name || "") });
    if (kind === "location" && Number.isFinite(Number(p.latitude)) && Number.isFinite(Number(p.longitude))) return await bot.sendLocation(chatId, Number(p.latitude), Number(p.longitude));
  } catch (_) {}
  try {
    return await bot.copyMessage(chatId, Number(item?.sourceChatId || fallbackSourceChatId || 0), Number(item?.sourceMessageId || 0));
  } catch (_) {}
  return null;
}
...
function adminInboxListText() {
  ensureSupportStore();
  const rows = Object.values(db.adminInbox.threads || {})
    .sort((a, b) => Number(b.lastMessageAt || 0) - Number(a.lastMessageAt || 0));
  if (!rows.length) return "📥 Admin Inbox\n\nNo messages yet.";
  return "📥 Admin Inbox\n\n" + rows.slice(0, 20).map((t, i) => adminInboxSummaryLine(t, i + 1)).join("\n");
}
...
async function openAdminInboxThread(adminChatId, targetChatId) {
  const thread = ensureAdminInboxThread(targetChatId);
  thread.unreadCount = 0;
  saveDB();
  await upsertAdminInboxNotification(targetChatId);
  await bot.sendMessage(adminChatId, `📂 Inbox of ${thread.name}
Total ${Number(thread.totalMessages || 0)} messages.`, {
    reply_markup: { inline_keyboard: [[{ text: `💬 Reply to ${thread.name}`.slice(0,64), callback_data: `ainbox:reply:${thread.chatId}` }]] }
  }).catch(() => {});
  const rows = (thread.messages || []).filter((m) => !m.deleted);
  if (!rows.length) {
    await bot.sendMessage(adminChatId, "No messages in this inbox.").catch(() => {});
    return;
  }
  for (const m of rows.slice(-50)) {
    const copied = await sendStoredInboxMedia(adminChatId, m, targetChatId).catch(() => null);
    const copiedId = copied?.message_id || 0;
    await bot.sendMessage(adminChatId, `From ${thread.name} • ${kenyaDateTimeFromTs(Number(m.ts || Date.now()))}
${m.preview}`, {
      reply_markup: { inline_keyboard: [[
        { text: "🗑 Delete", callback_data: `ainbox:msgdel:${thread.chatId}:${m.id}:${copiedId}`.slice(0,64) },
        { text: "💬 Reply", callback_data: `ainbox:reply:${thread.chatId}` },
      ]] }
    }).catch(() => {});
  }
}
...
if (data.startsWith("ainbox:")) {
  ...
  if (action === "open") {
    ...
    await openAdminInboxThread(chatId, targetChatId);
    return;
  }
  if (action === "reply") {
...
Send /cancel to stop.`).catch(() => {});
  ...
  if (action === "msgdel") {
    ...
    const copiedId = Number(parts[4] || 0);
    const t = ensureAdminInboxThread(targetChatId);
    const row = (t.messages || []).find((x) => String(x.id) === msgId);
    if (row) row.deleted = true;
    saveDB();
    if (copiedId) { try { await bot.deleteMessage(chatId, copiedId).catch(() => {}); } catch (_) {} }
    try { await bot.editMessageText("🗑 Deleted from inbox.", { chat_id: chatId, message_id: q.message.message_id }); } catch (_) {}
...
if (data === "help:support_chat") {
  supportState.set(String(chatId), { step: "support", startedAt: Date.now() });
  await bot.answerCallbackQuery(q.id, { text: "Talk to admin started" }).catch(() => {});
  const text = "💬 You are now chatting with admin.\n\nSend any text, photo, video, voice, document, sticker, or album.\nTap ❌ End Chat when you are done.";
  const opts = {
    reply_markup: {
      inline_keyboard: [[{ text: "❌ End Chat", callback_data: "help:end_support_chat" }]],
    },
  };
  const edited = await safeEditMessageText(chatId, _helpMsgId, text, opts);
  if (edited) return;
  await sendTracked(chatId, text, opts);
  return;
}

if (data === "help:end_support_chat") {
  supportState.delete(String(chatId));
  await bot.answerCallbackQuery(q.id, { text: "Chat ended" }).catch(() => {});
  const text = "✅ Chat ended.";
  const edited = await safeEditMessageText(chatId, _helpMsgId, text, { ...mainMenuKeyboard(chatId) });
  if (edited) return;
  await sendTracked(chatId, text, { ...mainMenuKeyboard(chatId) });
  return;
}
...
function helpSupportKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "💬 Talk to Admin", callback_data: "help:support_chat" }],
        [
          { text: "⬅️ Back to Help", callback_data: "help:menu" },
          { text: "🏠 Main Menu", callback_data: "help:home" },
        ],
...
async function forwardToAdminWithUserTag(userMsg) {
  ...
  try {
    await bot.copyMessage(ADMIN_ID, fromChatId, userMsg.message_id).catch(() => null);
  } catch (e) {
    console.log("support copy to admin failed:", e?.message || e);
  }

  try {
    addAdminInboxMessage(fromChatId, userMsg, "user", {});
    await upsertAdminInboxNotification(fromChatId);
  } catch (e) {
    console.log("support inbox update failed:", e?.message || e);
  }
}
...
if (/^\/broadcast$/i.test(text)) {
  ...
  if (!isAdmin(chatId) && sup?.step === "support") {
    ...
    return sendTracked(chatId, "✅ Sent to admin. Keep chatting here or tap End Chat in Help when done.", {
      ...mainMenuKeyboard(chatId),
    });
  }
...
bot.on("message", async (msg) => {
  ...
  if (text === "📥 Inbox" && isAdmin(chatId)) {
    return sendTracked(chatId, adminInboxListText(), adminInboxListKeyboard());
  }
