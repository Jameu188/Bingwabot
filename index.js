...
async function confirmGmailPhoneAndPush(chatId, qty, phoneInput, total) {
  ...
  const externalRef = makeExternalRef(chatId, "Gmail_Accounts", total);
  ...
  try {
    let lastErr = null;
    const channelsToTry = [
      PAYHERO_CHANNEL_ID_DATA,
      PAYHERO_CHANNEL_ID_SMS,
      PAYHERO_CHANNEL_ID_BUSINESS
    ].filter((v, i, a) => Number(v) > 0 && a.indexOf(v) === i);

    let stkOk = false;
    for (const cid of channelsToTry) {
      try {
        await payheroStkPush({
          amount: total,
          phone: phone254,
    ...
    if (!stkOk) {
      throw lastErr || new Error("No valid PayHero channel accepted the STK request");
    }
    ...
    for (const [ref, rec] of Object.entries(store.pendingOrders || {})) {
      try {
        ...
        const msg =
          ...
          `4. Add recovery details if needed.\n\n` +
          `Your payment has been received and your request is being processed.`;
...
bot.on("message", async (msg) => {
  try {
    const chatId = msg.chat.id;
    const text = String(msg.text || "").trim();
    if (text === "/buy" || text === "🛒 Buy Offers" || text === "🛒 Nunua Ofa") {
