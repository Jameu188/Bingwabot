...
async function sendHelpAnswer(chatId, key, ctx = {}) {
  if (key === "support") {
    try {
      if (ctx.messageId) await bot.deleteMessage(chatId, ctx.messageId).catch(() => {});
    } catch (_) {}
...
bot.on("message", async (msg) => {
  ...
  if (text === "⏳ Temporary Ban") {
    adminState.set(String(chatId), { step: "await_temp_ban", createdAt: Date.now() });
    await sendTracked(chatId, `⏳ Temporary Ban
...
Use /cancel to stop.`, adminPanelSimpleKeyboard("ap:users"));
    return;
  }

  if (text === "🛠 Maintenance Mode") {
    const enabled = setMaintenanceMode(!isMaintenanceMode(), chatId);
    addAdminLog(chatId, enabled ? "Maintenance enabled" : "Maintenance disabled");
    await sendTracked(chatId, `🛠 Maintenance mode is now ${enabled ? "ON" : "OFF"}.`, adminPanelUsersKeyboard());
    return;
  }
  ...
  if (text === "📨 Support Requests") {
    const rows = adminOpenSupportTickets(10);
    if (!rows.length) {
      await sendTracked(chatId, "✅ No open support requests right now.", adminPanelSimpleKeyboard("ap:users"));
      return;
    }
    await sendTracked(chatId, `🆘 Open Support Requests: ${rows.length}`, adminPanelSimpleKeyboard("ap:users"));
    for (const t of rows) {
      await bot.sendMessage(chatId, supportTicketText(t), supportTicketKeyboard(t.ticketId)).catch(() => {});
    }
    return;
  ...
  if (text === "📢 Broadcast Message") {
    adminState.set(String(chatId), { step: "await_broadcast", createdAt: Date.now() });
    await sendTracked(chatId, `📣 Broadcast mode ON.
...
Send /cancel to stop.`, adminPanelSimpleKeyboard("ap:users"));
    return;
  }
...
• Admins: ${getAdminIdList().length}`;
    ...
    await sendTracked(chatId, txt, adminPanelSimpleKeyboard("ap:users"));
    return;
  }
...
Use /cancel to stop.`, adminPanelSimpleKeyboard("ap:users"), q);
  ...
  if (data === "ap:maintenance") {
    const enabled = setMaintenanceMode(!isMaintenanceMode(), chatId);
    addAdminLog(chatId, enabled ? "Maintenance enabled" : "Maintenance disabled");
    await adminPanelSendOrEdit(chatId, `🛠 Maintenance mode is now ${enabled ? "ON" : "OFF"}.`, adminPanelUsersKeyboard(), q);
    return;
  }
  if (data === "ap:support") {
    ...
    const rows = adminOpenSupportTickets(10);
    if (!rows.length) {
      await adminPanelSendOrEdit(chatId, "✅ No open support requests right now.", adminPanelSimpleKeyboard("ap:users"), q);
      return;
    }
    await adminPanelSendOrEdit(chatId, `🆘 Open Support Requests: ${rows.length}`, adminPanelSimpleKeyboard("ap:users"), q);
    for (const t of rows) {
      await bot.sendMessage(chatId, supportTicketText(t), supportTicketKeyboard(t.ticketId)).catch(() => {});
    }
    return;
