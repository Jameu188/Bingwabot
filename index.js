...
// ===============================================================
// 🚀 ULTRA FAST RENDER STARTUP SERVER
// ===============================================================
const express = require("express");
const app = express();
const PORT = process.env.PORT || 10000;

app.get("/", (req,res)=>res.send("🚀 Bingwa Bot Running"));
...
function adminNotify(text) {
  try {
    ...
    bot && bot.sendMessage(String(ADMIN_ID), String(text)).catch(() => {});
  } catch (_) {}
}
    ...
    try {
      if (img) {
        await bot.sendPhoto(targetId, img, { caption: msgText, ...WD_BROADCAST_JOIN_KB }).catch(async () => {
          await bot && bot.sendMessage(targetId, msgText, WD_BROADCAST_JOIN_KB).catch(() => {});
        });
      } else {
        await bot && bot.sendMessage(targetId, msgText, WD_BROADCAST_JOIN_KB).catch(() => {});
      }
...
async function openWeeklyDrawUserMenu(chatId) {
  const c = ensureWeeklyDrawObject();
  if (!c.enabled) {
    return bot && bot.sendMessage(chatId, "❌ WEEKLY DRAW is not active right now.", mainMenuKeyboard(chatId)).catch(() => {});
  }
  ...
  try {
    if (img) {
      ...
      const sent = await bot && bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId));
      // Live update (text)
    ...
    // fallback to message
    const sent = await bot && bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId)).catch(() => null);
    if (sent) {
      try { startWeeklyDrawLiveUpdate(chatId, sent.message_id, false); } catch (_) {}
      return sent;
    }
  ...
  async function editAdminStateMessage(chatId, state, text, keyboard) {
    ...
    if (mid) {
      await bot.editMessageText(text, { chat_id: chatId, message_id: mid, ...keyboard }).catch(async () => {
        await bot && bot.sendMessage(chatId, text, keyboard).catch(() => {});
      });
      return;
    }
    await bot && bot.sendMessage(chatId, text, keyboard).catch(() => {});
...
async function broadcastToAllUsers(messageText) {
  ...
  for (const cid of ids) {
    ...
    if (!chatId) continue;
    try { if (isWeeklyDrawOptedIn(chatId) && !isStopCampaignNotifs(chatId)) await bot && bot.sendMessage(chatId, messageText, mainMenuKeyboard(chatId)); } catch (_) {}
  }
}
...
async function weeklyDrawFreezeAndAnnounce(reasonText) {
  ...
  for (const w of winners) {
    ...
    try { await bot && bot.sendMessage(w.chatId, winMsg, mainMenuKeyboard(w.chatId)); } catch (_) {}
  }
}
...
async function maybeSendNearQualifyReminder(chatId) {
  ...
  try { await bot && bot.sendMessage(chatId, msg, mainMenuKeyboard(chatId)); } catch (_) {}
}
...
async function weeklyDrawRankChangeNotify() {
  ...
  for (const [cid, newRank] of Object.entries(newRanks)) {
    ...
    if (newRank < oldRank) {
      ...
      try { if (isWeeklyDrawOptedIn(Number(cid))) await bot && bot.sendMessage(Number(cid), msg, mainMenuKeyboard(Number(cid))); } catch (_) {}
    } else {
      // dropped (overtaken)
      const msg =
...
async function weeklyDrawDailyReminderTick() {
  try {
    ...
    for (const [cidStr, u] of Object.entries(db.users || {})) {
      ...
      await bot && bot.sendMessage(cid, msg, weeklyDrawUserMenuKeyboard(cid)).catch(() => {});
...
try {
  ...
  try {
    if (!isStopCashbackNotifs(chatId)) bot && bot.sendMessage(
      chatId,
      `🎁 Cashback Earned!
...
bot = (function(){ const __TOKEN=TELEGRAM_TOKEN; let __bot=null; setTimeout(()=>{ __bot=new TelegramBot(__TOKEN,{polling:true}); global.bot=__bot; console.log("🤖 Bot polling started (delayed for fast startup)"); },1200); return global.bot; })();
...
bot && bot && bot.onText(/\/settoken\s+(.+)/i, async (msg, match) => {
  const chatId = msg?.chat?.id;
  if (!chatId) return;

  if (Number(chatId) !== Number(ADMIN_ID)) {
    return bot && bot.sendMessage(chatId, "❌ Only admin can set the bot token.").catch(() => {});
  }

  const token = String(match?.[1] || "").trim();
  if (token.length < 20 || !token.includes(":")) {
    return bot && bot.sendMessage(chatId, "❌ Invalid token format. Example: 123456:ABC...").catch(() => {});
  }
  ...
  try {
    ...
    await bot && bot.sendMessage(chatId, "✅ Token saved to db.json. Restarting now…").catch(() => {});
  } catch (e) {
    console.log("/settoken error:", e?.message || e);
    return bot && bot.sendMessage(chatId, "❌ Failed to save token. Check logs.").catch(() => {});
  }
...
async function sendJoinGate(chatId) {
  ...
  const sent = await bot && bot.sendMessage(chatId,
...
function isMessageFlood(chatId) {
  ...
  if (arr.length >= 10) {
    ...
    if (now - lastWarn > 15000) {
      ...
      bot && bot.sendMessage(chatId, "⏳ Slow down. Please wait a moment before sending more messages.").catch(() => {});
    }
    return true;
  }
  return false;
...
async function __withdrawApprove(targetId, adminChatId, adminMsgId) {
  try {
    ...
    if (!uid) {
      if (adminChatId) await bot && bot.sendMessage(adminChatId, "❌ Invalid user id.").catch(() => {});
      return false;
    }
    ...
    if (!req || req.status !== "pending" || req.type !== "withdraw") {
      if (adminChatId) await bot && bot.sendMessage(adminChatId, "❌ No pending withdraw for that user.").catch(() => {});
...
if (adminChatId && adminMsgId) {
  ...
  await bot.deleteMessage(adminChatId, adminMsgId).catch(() => {});
    ...
    if (adminChatId) await bot && bot.sendMessage(adminChatId, "❌ Error approving withdrawal. Check Render logs.").catch(() => {});
...
You can try again later.`
  ...
  if (adminChatId) await bot && bot.sendMessage(adminChatId, `❌ Withdraw declined for ${targetId}`).catch(() => {});
  return true;
}
// ===================== AUTO DELETE SYSTEM =====================
  ...
  const scheduleOne = (attemptNo) => {
    const tid = setTimeout(async () => {
      try {
        ...
        await bot && bot.sendMessage(chatId, `🔁 Auto Retry STK sent (attempt ${attemptNo}/${AUTO_RETRY_STK_MAX}).
...
Check your phone and complete payment.`).catch(() => {});
  ...
  try {
    bot && bot.sendMessage(chatId, `⏳ Auto Retry STK is ON. I will retry in ~1.1 minutes (up to ${AUTO_RETRY_STK_MAX} times).`).catch(() => {});
  } catch (_) {}
}
...
bot && bot && bot.on("inline_query", async (q) => {
  try {
...
bot && bot && bot.on("callback_query", async (q) => {
  try {
...
if (data === "biz:backup_once") {
  ...
  try {
    ...
    await bot.sendDocument(
      ...
      { caption: "✅ Your backup is ready.\n\n⚠️ Your account will now be cleared (blank).\nTo restore (ONE-TIME per file): send /restorebackup then upload this backup file." },
    ...
    await bot && bot.sendMessage(chatId, "✅ Backup saved.", mainMenuKeyboard(chatId)).catch(() => {});
  } catch (e) {
    ...
    await bot && bot.sendMessage(chatId, "❌ Failed to create backup: " + (e?.message || e), mainMenuKeyboard(chatId)).catch(() => {});
  }
  return;
}
...
if (data === "ar:toggle") {
 ...
 if (data === "bh:toggle") {
   ...
   try {
     await bot && bot.sendMessage(chatId, u.autoHideButtonsEnabled ? "🔒 Buttons are now hidden (only Help is shown)." : "✅ Buttons are now visible.", { ...mainMenuKeyboard(chatId) }).catch(() => {});
   } catch (_) {}
   return;
...
try {
  ...
  saveDB();
      ...
      await bot && bot.sendMessage(actorId, "✅ Access granted.", mainMenuKeyboard(actorId)).catch(() => {});
  return;
}
...
if (action === "set_name") {
  ...
  await bot && bot.sendMessage(chatId, "✏️ Send new campaign *name* now:", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
  return;
}
...
if (action === "add_media") {
  ...
  await bot && bot.sendMessage(chatId, "🖼 Send *photo/video* to add (you can send many). Send `DONE` when finished.", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
  return;
}
...
if (action === "reset_factory" || action === "reset_referrals" || action === "reset_balances") {
  ...
  await bot && bot.sendMessage(chatId, warn, { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
  return;
}
...
if (action === "set_winnerscount") {
  ...
  await bot && bot.sendMessage(chatId, "👥 Send the *number of winners* to award (e.g. `5`).", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
  return;
}
...
if (action === "set_heading") {
  ...
  if (action === "restart") {
    ...
    const statusText = formatWeeklyDrawAdminStatus();
    await bot && bot.sendMessage(chatId, `🔄 WEEKLY DRAW restarted.\n\n${statusText}`, weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }
...
bot && bot && bot.on("callback_query", async (q) => {
  try {
    ...
    // Admin only
    if (!isAdmin(chatId)) {
      await bot && bot.sendMessage(chatId, "❌ Not allowed.").catch(() => {});
      await bot.answerCallbackQuery(q.id).catch(() => {});
      return;
    }
    ...
    if (data.startsWith("rot:stop:")) {
      ...
      if (ok) await bot && bot.sendMessage(chatId, `✅ Rotation stopped: ${rid}`).catch(() => {});
      return;
    }
    ...
    if (data.startsWith("bcast:")) {
      ...
      if (choice === "cancel") {
        ...
        await bot.answerCallbackQuery(q.id).catch(() => {});
        await bot && bot.sendMessage(chatId, "✅ Cancelled.").catch(() => {});
        return;
      }
      ...
      try {
        if (choice === "once") {
          ...
          saveDB();
          await bot && bot.sendMessage(chatId, `✅ Rotation started (NOW)\nID: ${rot.id}\nDays: ${days}\nUse /rotlist to stop.`).catch(() => {});
        } else if (choice === "rot_tom") {
          const rot = addRotation({ fromChatId, messageId, days });
          rot.nextRunAt = Date.now() + 24 * 60 * 60 * 1000; // start tomorrow
          saveDB();
          await bot && bot.sendMessage(chatId, `✅ Rotation scheduled (TOMORROW)\nID: ${rot.id}\nDays: ${days}\nUse /rotlist to stop.`).catch(() => {});
        }
      } catch (e) {
        await bot && bot.sendMessage(chatId, `⚠️ Broadcast error: ${e?.message || e}`).catch(() => {});
      } finally {
        adminState.delete(String(chatId));
        await bot.answerCallbackQuery(q.id).catch(() => {});
      }
...
// Withdraw quick actions via inline buttons (same behavior as /ac /dc)
    if (data.startsWith("ac:") || data.startsWith("dc:")) {
      ...
      if (!targetId) {
        await bot && bot.sendMessage(chatId, "❌ Invalid request.").catch(() => {});
        await bot.answerCallbackQuery(q.id).catch(() => {});
        return;
      }
      if (cmd === "ac") {
    ...
    if (!found) {
      await bot && bot.sendMessage(chatId, "ℹ️ Request already handled.").catch(() => {});
      await bot.answerCallbackQuery(q.id).catch(() => {});
    ...
    if (action === "redeem_accept") {
      ...
      await bot && bot.sendMessage(chatId, "✅ Redeem approved.").catch(() => {});
      await bot.answerCallbackQuery(q.id).catch(() => {});
  ...
  const sent = await bot && bot.sendMessage(
        targetChatId,
    ...
    if (action === "redeem_decline") {
      ...
      await bot && bot.sendMessage(chatId, "❌ Redeem declined / cancelled.").catch(() => {});
      await bot.answerCallbackQuery(q.id).catch(() => {});
  ...
  const sent = await bot && bot.sendMessage(
    ...
    try {
      await bot && bot.sendMessage(q.message?.chat?.id, "❌ Error processing request. Please try again.").catch(() => {});
...
bot && bot && bot.on("callback_query", async (q) => {
  try {
    ...
    if (action === "withdraw_accept") {
      ...
      await bot && bot.sendMessage(chatId, "✅ Withdrawal approved.").catch(() => {});
      await bot.answerCallbackQuery(q.id).catch(() => {});
    ...
    if (action === "withdraw_decline") {
      ...
      await bot && bot.sendMessage(chatId, "❌ Withdrawal declined.").catch(() => {});
      await bot.answerCallbackQuery(q.id).catch(() => {});
...
function translateReplyMarkup(chatId, replyMarkup) {
  ...
  try {
    ...
    if (Array.isArray(rm.inline_keyboard)) {
      rm.inline_keyboard = rm.inline_keyboard.map(row => Array.isArray(row) ? row.map(btn => {
        if (btn && typeof btn.text === "string") btn.text = translateOutgoing(chatId, btn.text);
        return btn;
      }) : row);
    }
    // Reply keyboard
    if (Array.isArray(rm.keyboard)) {
      rm.keyboard = rm.keyboard.map(row => Array.isArray(row) ? row.map(btn => {
        if (btn && typeof btn.text === "string") btn.text = translateOutgoing(chatId, btn.text);
        return btn;
      }) : row);
    }
    // Remove keyboard / force reply etc don't need changes
    return rm;
  } catch (_) {
    return replyMarkup;
  }
  ...
  for (const [ref, p] of Object.entries(db.pendingPayments || {})) {
    if (!p || isPendingExpired(p, ttl)) {
      ...
      try {
        ...
        if (cid) {
          bot && bot.sendMessage(cid, "⌛ STK request expired. Please try again if you still want to pay.").catch(() => {});
        }
      } catch (_) {}
...
async function sendTracked(chatId, text, options = {}) {
  ...
  const sent = await bot && bot.sendMessage(chatId, translateOutgoing(chatId, text), options);
...
async function notifyAdmin(text) {
  ...
  const sent = await bot && bot.sendMessage(ADMIN_ID, text, { parse_mode: "Markdown" });
  } catch (e) {
    console.log("notifyAdmin failed:", e?.message || e);
    try {
      

  const sent = await bot && bot.sendMessage(ADMIN_ID, String(text));
    } catch (_) {}
  }
}
...
` +
    ...
    if (isWeeklyDrawOptedIn(chatId) && !isStopCampaignNotifs(chatId)) {
      await bot && bot.sendMessage(chatId, msg, mainMenuKeyboard(chatId)).catch(() => {});
    }
...
//(PORT, () => console.log("🌐 Webhook server listening on", PORT));

// ===================== MAIN FLOW (SINGLE HANDLER) =====================
bot && bot && bot.on("message", async (msg) => {
...
if (text === "/mybackup" || text === "/backup") {
  try {
    ...
    await bot && bot.sendMessage(chatId, "❌ Failed to create backup: " + (e?.message || e)).catch(()=>{});
  }
  return;
}
...
if (text === "/mybackupreset" || text === "/backupreset") {
  try {
    ...
    await bot.sendDocument(
      chatId,
      Buffer.from(pretty, "utf8"),
      { caption: "✅ Backup created.\n\n⚠️ Now resetting your account data... (you can restore anytime with /restorebackup)" },
      { filename: fname, contentType: "application/json" }
    );
    ...
    await bot && bot.sendMessage(
      chatId,
      ok
        ? "🧹 Reset complete.\n\nYour data is deleted from the bot.\nTo restore: send /restorebackup then upload your backup file."
        : "⚠️ Backup sent, but reset failed. Contact admin."
    ).catch(()=>{});

  } catch (e) {
    await bot && bot.sendMessage(chatId, "❌ Failed to backup+reset: " + (e?.message || e)).catch(()=>{});
  }
  return;
}
...
if (text === "/restorebackup" || text === "/restore") {
  const u = getUser(chatId);
  u.pendingAction = { type: "await_restore_backup", at: Date.now() };
  saveDB();
  await bot && bot.sendMessage(chatId, "📤 Send the backup JSON file now (the one you received from /mybackup).", mainMenuKeyboard(chatId)).catch(()=>{});
  return;
}
...
try {
  ...
  if (u?.pendingAction?.type === "await_restore_backup" && msg.document) {
    ...
    if (name && !name.toLowerCase().endsWith(".json")) {
      await bot && bot.sendMessage(chatId, "❌ Please upload the .json backup file.").catch(()=>{});
      return;
    }

    await bot && bot.sendMessage(chatId, "⏳ Restoring your backup...", mainMenuKeyboard(chatId)).catch(()=>{});
    ...
    try {
      ...
      await bot && bot.sendMessage(chatId, "❌ Backup file is not valid JSON.").catch(()=>{});
      return;
    }
    ...
    if (backupId && isBackupAlreadyUsed(backupId)) {
      ...
      u.pendingAction = null;
      saveDB();
      await bot && bot.sendMessage(chatId, "❌ This backup file was already used for restoration. You cannot restore the same file twice.").catch(()=>{});
      return;
    }
    ...
    await bot && bot.sendMessage(chatId, "✅ Backup restored successfully.", mainMenuKeyboard(chatId)).catch(()=>{});
    return;
  }
} catch (e) {
  await bot && bot.sendMessage(chatId, "❌ Restore failed: " + (e?.message || e)).catch(()=>{});
  try {
    const u = getUser(chatId);
    u.pendingAction = null;
    saveDB();
...
if (text.startsWith("/deactivatebiz")) {
  if (!isAdmin(chatId)) {
    await bot && bot.sendMessage(chatId, "❌ Admin only command.").catch(() => {});
    return;
  }

  const parts = String(text || "").split(" ").filter(Boolean);
  if (parts.length < 2) {
    await bot && bot.sendMessage(chatId, "Usage:\n/deactivatebiz <userId>").catch(() => {});
    return;
  }

  const targetId = Number(parts[1]);
  if (!Number.isFinite(targetId)) {
    await bot && bot.sendMessage(chatId, "❌ Invalid user ID.").catch(() => {});
    return;
  }
...
try {
  if (!isAdmin(chatId)) {
    const u = getUser(chatId);
    if (u && u.banned) {
      await bot && bot.sendMessage(chatId, "⛔ You are blocked from using this bot. Contact support/admin.").catch(() => {});
      return;
    }
  }
} catch (_) {}
...
// ===================== ADMIN: FREE BUSINESS 10 DAYS =====================
  if (text.startsWith("/freebiz")) {
    ...
    if (ok) {
      await bot && bot.sendMessage(chatId, `✅ 10 FREE Business days granted to ${targetId}`).catch(() => {});
    } else {
      await bot && bot.sendMessage(chatId, "❌ Failed to grant Business. Check logs.").catch(() => {});
    }
    return;
  }
...
if (isStartCmd) {
  ...
  if (/^\/admin(?:@\w+)?$/i.test(text)) {
    if (!isAdmin(chatId)) {
      await bot && bot.sendMessage(chatId, "⛔ Not authorized.").catch(() => {});
      return;
    }
...
Tip: Use the bot's admin panel buttons for Weekly Draw settings, broadcast, paying winners, and resets.`;
    await bot && bot.sendMessage(chatId, msg, {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
  ...
  if (/^\/stats(?:@\w+)?$/i.test(text)) {
    ...
    try {
      ...
      await bot && bot.sendMessage(chatId, txt, { parse_mode: "Markdown" }).catch(() => {});
    } catch (_) {
      await bot && bot.sendMessage(chatId, "⚠️ Could not compute stats.").catch(() => {});
    }
    return;
  }
  ...
  if (/^\/backup(?:@\w+)?$/i.test(text)) {
    ...
    try {
      ...
      await bot && bot.sendMessage(chatId, "⚠️ Backup failed.").catch(() => {});
    }
    return;
  }
  ...
  if (/^\/(ban|unban)(?:@\w+)?\b/i.test(text)) {
    ...
    await bot && bot.sendMessage(chatId, u.banned ? `✅ Banned ${target}` : `✅ Unbanned ${target}`).catch(() => {});
    // notify user (optional)
    try {
      await bot && bot.sendMessage(target, u.banned ? "⛔ You have been blocked by admin." : "✅ You have been unblocked. You can use the bot again.").catch(() => {});
    } catch (_) {}
    return;
  }
...
if (/^\/autodelete(?:@\w+)?\b/i.test(text)) {
  ...
  if (!(typeof isAdmin === "function" && isAdmin(chatId)) && !(typeof isBusinessActive === "function" && isBusinessActive(chatId))) {
    await bot && bot.sendMessage(chatId, "⛔ /autodelete is available for *Business* accounts only.\n\nTo upgrade, type /sell", { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }).catch(() => {});
    return;
  }
  ...
  if (arg === "off" || arg === "0" || arg === "disable") {
    ...
    try { saveDB(); } catch (_) {}
    await bot && bot.sendMessage(chatId, "✅ Auto-delete set to *24hr* (maximum).", { parse_mode: "Markdown" }).catch(() => {});
    return;
  }

  const ms = parseDurationToMs(arg);
  if (!ms) {
    await bot && bot.sendMessage(chatId, "❌ Invalid duration. Examples: 1s, 10min, 1h, 24hr (max 24hr).").catch(() => {});
    return;
  }
  ...
  await bot && bot.sendMessage(chatId, `✅ Auto-delete set to *${formatMsShort(ms)}* for bot messages.`, { parse_mode: "Markdown" }).catch(() => {});
  return;
}
// ===================== END /autodelete =====================
...
// ✅ /start cooldown (5 seconds) to reduce spam
  if (isStartCmd && !isAdmin(chatId)) {
    ...
    if (last && now - last < START_COOLDOWN_MS) {
      ...
      await bot && bot.sendMessage(chatId, `⏳ Please wait ${wait}s and try /start again.`).catch(() => {});
      return;
    }
...
if (/^\/credit\s+\d+\s+-?\d+\s+-?\d+/i.test(text)) {
  ...
  await bot && bot.sendMessage(targetId, `✅ Admin credited your account.\nTickets: ${tickets}\nPoints: ${pts}\nReason: ${reason}`).catch(() => {});
  return;
}
...
Waiting for admin approval…`,
  ...
  if (text.startsWith("/")) {
    ...
    if (startPayload !== null) {
      ...
      try {
        await bot && bot.sendMessage(chatId, "🔎 Search offers inline:", {
          reply_markup: {
            inline_keyboard: [
...
async function __withdrawApprove(targetId, adminChatId, adminMsgId) {
  ...
  const req = db.pendingRedeems?.[String(targetId)];
  if (!req || req.status !== "pending" || req.type !== "withdraw") {
    await bot && bot.sendMessage(adminChatId, "❌ No pending withdraw for that user.").catch(() => {});
    return false;
  }
...
Thank you for using Bingwa Mtaani 💙`,
  ...
  await bot && bot.sendMessage(adminChatId, `✅ Withdraw approved for ${targetId}`).catch(() => {});
  return true;
}
...
You can try again later.`,
  ...
  await bot && bot.sendMessage(adminChatId, `❌ Withdraw declined for ${targetId}`).catch(() => {});
  return true;
}
...
if (/^\/pending$/i.test(text)) {
  ...
  const sent = await bot && bot.sendMessage(chatId, adminText, {
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
...
if (bcastMatch) {
  ...
  return bot && bot.sendMessage(chatId, `Choose broadcast mode (rotation = ${days} day(s)) :`, {
    reply_markup: {
      inline_keyboard: [
...
if (_wdState.mode === "confirm_reset") {
  ...
  if (!/^YES$/i.test(t)) {
    await bot && bot.sendMessage(chatId, "❌ Cancelled. Nothing was changed.", mainMenuKeyboard(chatId)).catch(() => {});
    setWeeklyDrawAdminState(chatId, null);
    return;
  }
  ...
  await bot && bot.sendMessage(chatId, "✅ Done.", mainMenuKeyboard(chatId)).catch(() => {});
  return;
}
  ...
  try {
    ...
    if (s.step === "fast_phone_pick") {
      ...
      if (s.fpPhoneMap && s.fpPhoneMap[text]) {
        ...
        sessions.delete(chatId);
        try { await bot && bot.sendMessage(chatId, `📲 Sending STK push… (${maskPhone(phone254)})`); } catch (_) {}
...
setInterval(async () => {
  try {
    ...
    if (isMonday && db.stats.lastLeaderboardWeek !== weekKey) {
      ...
      for (const uid of userIds) {
        // eslint-disable-next-line no-await-in-loop
        await bot && bot.sendMessage(uid, text, { parse_mode: "Markdown", ...mainMenuKeyboard(uid) }).catch(() => {});
        // eslint-disable-next-line no-await-in-loop
        await sleep(35);
      }
...
bot && bot && bot.on("callback_query", async (q) => {
  ...
  if (action === "phone_approve") {
    ...
    await bot && bot.sendMessage(q.message.chat.id, "✅ Phone override approved.").catch(() => {});
  }
  ...
  if (action === "phone_decline") {
    ...
    await bot && bot.sendMessage(q.message.chat.id, "❌ Phone override declined.").catch(() => {});
  }
...
bot && bot && bot.on("message", async (msg) => {
  ...
  if (blockMatch) {
    ...
    await bot && bot.sendMessage(msg.chat.id, `⛔ User ${targetId} blocked for 72 hours.`);
    return;
  }
  ...
  if (unblockMatch) {
    ...
    await bot && bot.sendMessage(msg.chat.id, `✅ User ${targetId} unblocked.`);
    return;
  }
});
...
maybeRewardInviterOnSuccessPurchase = function(buyerChatId, amountKsh, when) {
  ...
  if (!isStopReferralNotifs(inviterId)) bot && bot.sendMessage(
    inviterId,
  ...
  try {
    ...
    bot && bot && bot.on("message", async (msg) => {
      ...
      if (text === "🎛 WEEKLY DRAW ADMIN" || /^\/weeklydraw_admin$/i.test(text)) {
        if (!(typeof isAdmin === "function" && isAdmin(chatId))) {
          return bot && bot.sendMessage(chatId, "⛔ Admin only.", mainMenuKeyboard(chatId)).catch(() => {});
        }
        const statusText = formatWeeklyDrawAdminStatus();
        return bot && bot.sendMessage(chatId, statusText, weeklyDrawAdminMenuKeyboard()).catch(() => {});
      }
...
if (/\bWEEKLY\s*DRAW\b/i.test(__wdText)) {
  const c = ensureWeeklyDrawObject();
  if (!c.enabled) {
    return bot && bot.sendMessage(chatId, "⛔ WEEKLY DRAW is currently not active.", mainMenuKeyboard(chatId)).catch(() => {});
  }
  ...
  if (img) {
    return bot.sendPhoto(chatId, img, { caption, ...weeklyDrawUserMenuKeyboard(chatId) })
      .catch(() => bot && bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId)).catch(() => {}));
  }
  return bot && bot.sendMessage(chatId, caption, weeklyDrawUserMenuKeyboard(chatId)).catch(() => {});
}
...
bot && bot && bot.on("callback_query", async (q) => {
  try {
    ...
    if (q.data === "copy_ref_link") {
      ...
      await bot && bot.sendMessage(chatId, `📋 Copy this link:\n\n${referralLink}`);
...
if (typeof bot !== "undefined") {
  bot && bot && bot.onText(/\/showtokenstatus/, async (msg) => {
    const chatId = msg.chat.id;

    if (Number(chatId) !== Number(ADMIN_ID)) {
      return bot && bot.sendMessage(chatId, "❌ Only admin can view token status.");
    }
    ...
    try {
      ...
      if (hasToken) {
        await bot && bot.sendMessage(chatId, "✅ Bot token is SET in database.");
      } else {
        await bot && bot.sendMessage(chatId, "⚠️ Bot token is NOT set.");
      }
    } catch (err) {
      await bot && bot.sendMessage(chatId, "❌ Error checking token status.");
    }
  });
}
...
// Privacy
bot && bot && bot.onText(/📄?\s*Terms of Service/i, async (msg)=>{
  const chatId=msg.chat.id;
...
// Privacy
bot && bot && bot.onText(/🔒?\s*Privacy/i, async (msg)=>{
  const chatId=msg.chat.id;
  bot && bot.sendMessage(chatId,
`🔒 Privacy Policy
...
bot && bot && bot.onText(/🏆?\s*Leaderboard/i, async (msg)=>{
  const chatId=msg.chat.id;
  ...
  try{
    ...
    bot && bot.sendMessage(chatId,txt);
  }catch(e){
    bot && bot.sendMessage(chatId,"Leaderboard unavailable.");
  }
});

// History (user transactions)
bot && bot && bot.onText(/📜?\s*History/i, async (msg)=>{
  const chatId=msg.chat.id;
  ...
  try{
    ...
    if(!log.length){
      return bot && bot.sendMessage(chatId,"📜 No activity yet.");
    }
    ...
    bot && bot.sendMessage(chatId,"History unavailable.");
  }
});
...
// Toggle command
bot && bot && bot.onText(/🔁\s*STK Retry/i, (msg)=>{
  const chatId=msg.chat.id;
  try{
    const u=getUser(chatId);
    ...
    bot && bot.sendMessage(chatId,"Retry toggle error.");
  }
});
...
// ===============================================================
// 🌍 GLOBAL MULTILINGUAL SYSTEM (EN / SW)
// ===============================================================
...
choose:"Choose:",
withdraw_locked:"🚫 Withdrawal Locked",
withdraw_rule:"You must spend at least Ksh 300 before making a withdrawal request.",
keep_buying:"Keep buying to unlock withdrawals.",
miss_you_title:"👋 We miss you!",
miss_you_msg:"Come back today and grab your best offer.",
miss_you_tip:"🎁 Tip: Share your referral link to earn points on every successful purchase!"
},
...
back:"⬅️ Rudi",
selected:"✅ Umechagua:",
saved_number:"📱 Namba iliyohifadhiwa",
choose:"Chagua:",
withdraw_locked:"🚫 Kutoa Kumezuiwa",
withdraw_rule:"Lazima utumie angalau Ksh 300 kabla ya kuomba kutoa.",
keep_buying:"Endelea kununua ili kufungua utoaji.",
miss_you_title:"👋 Tumekumiss!",
miss_you_msg:"Rudi leo uchukue ofa zako bora.",
miss_you_tip:"🎁 Dokezo: Shiriki link yako ya rufaa upate pointi kwa kila ununuzi."
}
...
console.log("🌍 Multilingual system loaded (EN/SW)");



// ===============================================================
// 🚀 RENDER KEEP-ALIVE (prevents cold sleep)
// ===============================================================
setInterval(()=>{
try{
...
console.log("🚀 Fast-start optimization enabled");



// ===============================================================
// ⚡ LAZY DB ACCESS (faster startup)
// ===============================================================
function getDBSafe(){
  try{
    if(!global.__DB_CACHE){
      ...
      global.__DB_CACHE = JSON.parse(fs.readFileSync(DB_FILE));
    }
    return global.__DB_CACHE;
  }catch(e){
    return {users:{},stats:{}};
  }
}
...
// ===============================================================
// 🌐 RENDER KEEP ALIVE (prevents cold start)
// ===============================================================
setInterval(()=>{
  try{
    const url = process.env.RENDER_EXTERNAL_URL;
...
console.log("⚡ Ultra startup optimization active (~1s boot)");