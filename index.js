...
if (data.startsWith("wdadm:")) {
  ...
  if (action === "end") {
    // Freeze top 3 winners
    c.status = "ENDED";
    c.enabled = true;
    const entries = c.entries && typeof c.entries === "object" ? c.entries : {};
    const arr = Object.keys(entries).map((k) => {
      const e = entries[k] || {};
      return { chatId: String(k), name: e.name || k, tickets: Number(e.tickets || 0) };
    }).sort((a,b) => b.tickets - a.tickets);
    const p = c.prizes || { first: 250, second: 150, third: 100 };
    const top3 = arr.slice(0,3);
    const prizes = [Number(p.first||0), Number(p.second||0), Number(p.third||0)];
    c.winners = top3.map((w, i) => ({ ...w, prize: prizes[i] || 0, paid: false }));
    try { saveDB(); } catch (_) {}
    await bot.answerCallbackQuery(q.id, { text: "Ended" }).catch(() => {});
    const winnersTxt = c.winners.length
      ? c.winners.map((w, i) => `${["🥇","🥈","🥉"][i]||""} ${w.name} — ${w.prize} KSH (${w.paid ? "Paid ✅" : "Pending"})`).join("\n")
      : "No winners (no entries).";
    await bot.sendMessage(chatId, `🏁 WEEKLY DRAW ENDED. Winners frozen:\n\n${winnersTxt}`, weeklyDrawAdminMenuKeyboard()).catch(() => {});
    return;
  }
