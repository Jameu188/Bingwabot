...
// ===== GLOBAL CANCEL (MERGE INTO EXISTING CALLBACK) =====
// Add this inside your main bot.on("callback_query") at the TOP
...
// ===================== DASHBOARD + UNDO (MERGED SAFE) =====================

// View all payment IDs
bot.onText(/\/viewids/, async (msg) => {
  const chatId = msg.chat.id;
  if (Number(chatId) !== Number(ADMIN_ID)) return;

  db = repairDB(db);
  const ids = db.paymentIds || {};

  const text = `📊 Current Payment IDs

📶 Bingwa: ${ids.BINGWA || "Not set"}
💬 SMS: ${ids.SMS || "Not set"}
📞 Minutes: ${ids.MINUTES || "Not set"}
♾ Unlimited: ${ids.UNLIMITED || "Not set"}
🏢 Business: ${ids.BUSINESS || "Not set"}
📧 Gmail: ${ids.GMAIL || "Not set"}`;

  return bot.sendMessage(chatId, text, {
    reply_markup: {
      inline_keyboard: [
        [{ text: "↩️ Undo Last Change", callback_data: "undo:last" }],
        [{ text: "🏠 Main Menu", callback_data: "set:home" }]
      ]
    }
  });
...
// ===== IMPORTANT: integrate into your EXISTING callback_query =====
// Add this inside your main callback_query handler
...
if (data === "undo:last") {
  ...
  if (!db.lastChange) {
    return bot.answerCallbackQuery(q.id, { text: "Nothing to undo", show_alert: true });
  }

  const { category, oldValue } = db.lastChange;

  db.paymentIds = db.paymentIds || {};
  db.paymentIds[category] = oldValue;
...
// ===== IMPORTANT: integrate into confirm save =====
// Before updating payment ID, add:

/*
saveLastChange(category, db.paymentIds?.[category] || null);
*/
...
/*
REQUIRED:
Replace any hardcoded IDs like "5577" with:
getPaymentId("SMS")
*/
