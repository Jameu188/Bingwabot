...
function phoneReuseIsAllowedInternal(chatId, phone254) {
...
// Alias wrappers to avoid ReferenceError from inconsistent naming
function phoneReuseIsAllowed(chatId, phone254) {
  return phoneReuseIsAllowedInternal(chatId, phone254);
}
function phoneReuseIsAllowed(chatId, phone254) {
  return phoneReuseIsAllowedInternal(chatId, phone254);
}
...
⏳ Waiting for admin approval…`,
  ...
  if (text === "🔁 Retry STK") {
    ...
    const pr = phoneReuseIsAllowedInternal(chatId, phone254);
  ...
  try {
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        const pr = phoneReuseIsAllowedInternal(chatId, savedPhone);
        if (!pr.ok) {
          const day = todayKey();
          const u = getUser(chatId);
      ...
      if (phoneMaybe) {
        // 📱 Phone reuse / anti-farming checks
        const pr = phoneReuseIsAllowedInternal(chatId, phoneMaybe);
        if (!pr.ok) {
          const day = todayKey();
          const u = getUser(chatId);
    ...
    if (s.step === "phone") {
      ...
      // 📱 Phone reuse / anti-farming checks
      const pr = phoneReuseIsAllowedInternal(chatId, phone);
      if (!pr.ok) {
        const day = todayKey();
        const u = getUser(chatId);
