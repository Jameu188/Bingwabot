...
if (s.step === "cart_edit_action") {
  ...
  if (text === "📞 Edit Payment Number") {
    s.cartEditField = "payment";
    s.step = "phone";
    markCartActivity(s, true);
    sessions.set(chatId, s);
    return sendTracked(chatId, `📱 Send new PAYMENT number for item ${idx + 1}:
Current: ${maskPhone(item.phone254 || "") || "Not set"}
• 07XXXXXXXX
• 01XXXXXXXX
• 2547XXXXXXXX
• 2541XXXXXXXX`, confirmKeyboard(false, !!item.receivePhone254, chatId));
  }
...
// Multi-Purchase Mode and STK Confirmation Messages (Detailed vs. Simple)
function isMultiPurchaseModeOn(chatId) {
    const user = getUser(chatId);
    return user.multiPurchaseEnabled;
}

function isStkModeOn(chatId) {
    const user = getUser(chatId);
    return user.stkModeEnabled;
}

// Payment confirmation
function sendPaymentConfirmation(chatId, paymentDetails) {
    const multiPurchaseModeOn = isMultiPurchaseModeOn(chatId);
    const confirmationMessage = multiPurchaseModeOn
        ? `
✅ Payment Confirmed

🆔 Reference: ${paymentDetails.reference}
📦 Package: ${paymentDetails.packageLabel}
📱 Payment Number: ${paymentDetails.paymentNumber}
📥 Receive Numbers:
${paymentDetails.receiveNumbers.map((num, index) => `${index + 1}. ${num}`).join('
')}
🕒 Transaction Date & Time: ${paymentDetails.transactionTime}
💵 Total Cashback Earned So Far: Ksh ${paymentDetails.cashbackEarned} (${(paymentDetails.cashbackEarned / 0.7).toFixed(2)} pts)
💰 Amounts: ${paymentDetails.amount}
🛒 Total Purchases: ${paymentDetails.totalPurchases}
...
// STK Payment confirmation
function sendStkPaymentConfirmation(chatId, stkDetails) {
    const stkModeOn = isStkModeOn(chatId);
    const stkConfirmationMessage = stkModeOn
        ? `
✅ STK sent!

Offer: ${stkDetails.offer}
Pay: ${stkDetails.paymentAmount}
Payment Number: ${stkDetails.paymentNumber}
Receive Numbers:
${stkDetails.receiveNumbers.map((num, index) => `${index + 1}. ${num}`).join('
')}
Ref: ${stkDetails.reference}
Time: ${stkDetails.transactionTime}
...
Offer: ${stkDetails.offer}
Pay: ${stkDetails.paymentAmount}
Your STK request has been sent successfully. Please wait for payment confirmation.`;

    bot.sendMessage(chatId, stkConfirmationMessage);
}

// Sample payment details object
const paymentDetails = {
    reference: "BINGWA-1774214384250-Unlimited_Deals-23-7859465542",
    packageLabel: "Ksh 23 • 1GB 1HR",
    amount: "Ksh 23",
    paymentNumber: "0759xxx842",
    receiveNumbers: [
        "0759xxx842 - 250mbs 24hr",
        "0707xxx678 - 20sms 24hrs"
    ],
    transactionTime: "23/03/2026, 00:19:56 am",
    cashbackEarned: 0.66,
    totalPurchases: 1
};

// Send confirmation when payment is confirmed
sendPaymentConfirmation(chatId, paymentDetails);

// Sample STK payment details object
const stkDetails = {
    offer: "Ksh 5 • 20 SMS 24HRS",
    paymentAmount: "Ksh 5",
    paymentNumber: "0759xxx842",
    receiveNumbers: [
        "0707xxx631",
    ...
    reference: "BINGWA-1774348160632-SMS_Offers-5-991624101",
    transactionTime: "2026-03-24T10:29:25.239Z"
};

// Send STK confirmation when STK payment is initiated
sendStkPaymentConfirmation(chatId, stkDetails);
