...
function weeklyDrawLastWinnersText(chatId) {
  ...
  function buildWeeklyDrawMainCaption(chatId) {
    ...
    let ranked = [];
    try {
      ranked = (getWeeklyDrawRankedParticipants(10) || []).map((x) => ({ ...x, cid: x.cid }));
    } catch (_) {
      ranked = [];
    }

    const listLines = ranked.length
      ? ranked.map((r, i) => `${i + 1}️⃣ ${String(r?.name || r?.cid || "User")} — ${Number(r?.tickets || 0)} ${tr(chatId,"tickets","tiketi")}`).join("\n")
...
async function openWeeklyDrawUserMenu(chatId) {
  ...
  let caption = "";
  try {
    caption = buildWeeklyDrawMainCaption(chatId) + (weeklyDrawLastWinnersText(chatId) || "");
  } catch (_) {
    const heading = String(c.heading || "WEEKLY DRAW").toUpperCase();
    caption = `🎉 ${heading}
${tr(chatId,"Status","Hali")}: ${String(c.status || "OFF").toUpperCase()}
⏳ ${tr(chatId,"Ends in","Inaisha baada ya")}: ${getCampaignCountdown()}

${tr(chatId,'✅ Weekly Draw is active. Tap the buttons below.','✅ Weekly Draw ipo hewani. Bonyeza vitufe hapa chini.')}`;
  }
  const img = String(c.image || "").trim();
...
function getWeeklyDrawRankedParticipants(limit = 0) {
  ...
  let items = [];
  for (const rawCid of ids) {
    try {
      const cid = String(rawCid || "").trim();
      if (!/^\d+$/.test(cid)) continue;
      const tickets = Number(entries[cid] || 0);
      const forcedCurrent = !!admin.frozenCurrent[cid];
      const forcedParticipant = !!admin.forcedParticipants[cid];
      ...
      items.push({
        ...
        name: String(safeDisplayName(u, cid) || cid),
        qualified: !!q.qualified,
        forcedPos: Number(admin.forcedPosition[cid] || 0),
      });
    } catch (_) {}
  }

  items.sort((a, b) => (Number(b.tickets || 0) - Number(a.tickets || 0)) || String(a.name || a.cid).localeCompare(String(b.name || b.cid)));
  const pinned = items.filter(x => x && x.forcedPos > 0).sort((a, b) => a.forcedPos - b.forcedPos || Number(b.tickets || 0) - Number(a.tickets || 0));
  const normal = items.filter(x => x && !(x.forcedPos > 0));
  const used = new Set();
  const byPos = {};
  ...
  const result = [];
  let idx = 0;
  const pinnedPositions = Object.keys(byPos).map(x => Number(x || 0)).filter(x => Number.isFinite(x) && x > 0);
  const maxPos = Math.max(0, items.length, ...(pinnedPositions.length ? pinnedPositions : [0]));
  for (let pos = 1; pos <= maxPos; pos += 1) {
    const forcedHere = byPos[pos] || [];
    if (forcedHere.length) {
      for (const item of forcedHere) {
        if (item && !used.has(item.cid)) {
          result.push(item);
          used.add(item.cid);
        }
      }
  ...
  while (idx < normal.length) {
    if (normal[idx] && !used.has(normal[idx].cid)) result.push(normal[idx]);
    idx += 1;
  }
