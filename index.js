...
if (/^\/addplan(?:@\w+)?\b/i.test(text)) {
  if (!isAdmin(chatId)) {
    await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
    return;
  }

  const raw = text.replace(/^\/addplan(?:@\w+)?/i, "").trim();
  const parts = raw.split("|").map((x) => String(x || "").trim()).filter(Boolean);

  if (parts.length < 3) {
    const cats = Object.keys(PACKAGES || {}).join(", ");
    await sendTracked(
      chatId,
      "⚙️ *Add Plan Command*\n\n" +
        "Use:\n`/addplan Category | Package Label | Price`\n\n" +
        "Example:\n`/addplan SMS Offers | Ksh 5 • 20 SMS 24HRS | 5`\n\n" +
  ...
  if (!label || label.length < 3) {
    await sendTracked(chatId, "❌ Invalid label.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  if (!Number.isFinite(price) || price <= 0) {
    await sendTracked(chatId, "❌ Invalid price.", { ...mainMenuKeyboard(chatId) });
    return;
  }

  // Prevent duplicates
  const exists = (PACKAGES[catKey] || []).some((x) => x && x.label === label);
  if (exists) {
    await sendTracked(chatId, "⚠️ That package label already exists in this category.", { ...mainMenuKeyboard(chatId) });
    return;
  }
  ...
  await sendTracked(
    chatId,
    `✅ Added plan to *${mdEscape(catKey)}*\n\n• *${mdEscape(label)}*\n• Price: *Ksh ${mdEscape(String(price))}*`,
    { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
  );
  return;
}
