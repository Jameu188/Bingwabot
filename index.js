  ...
  return;
    ...
    if (pending) {
      ...
      recordSuccessfulTransaction(pending, externalRef, mpesaOut, when);
            ...
            // ✅ Permanent phone lock applies ONLY after SUCCESS purchase (skip admin/business)
      if (pending && pending.phone254) {
        try {
          if (!(isAdmin(chatId) || isBusinessActive(chatId))) {
            markPhoneUsage(chatId, pending.phone254);
          }
        } catch (_) {
          // If helper functions are not available for some reason, still lock on success
          markPhoneUsage(chatId, pending.phone254);
        }
      }
...
if (text === "🔗 My Referral") {
  ...
  // Personal deep-link referral URL (Telegram start parameter)
  const referralLink = `https://t.me/${BOT_USERNAME}?start=${chatId}`;

  const successful = Number(u.referralSuccessCount || 0);
  const pts = (typeof getPoints === "function") ? getPoints(chatId) : Number(u.points || 0);

  const msg =
    `🔗 *Your Referral Link*\n\n` +
    `${mdEscape(referralLink)}\n\n` +
    `👥 Successful Referrals: *${mdEscape(String(successful))}*\n` +
    `💰 Points: *${mdEscape(String(Number(pts || 0).toFixed(2)))} pts*\n\n` +
    `Share this link with friends.\n` +
    `You earn referral commission when they make successful purchases.`;
...
Approve override?`,
  ...
  // ✅ Phone lock is applied ONLY after SUCCESS callback (not on STK send)
...
// ===================== CLEAN INVITE MESSAGE OVERRIDE =====================
function inviteMessage(chatId) {
  const BOT_USERNAME = process.env.BOT_USERNAME || "";
  const link = `https://t.me/${BOT_USERNAME}?start=${chatId}`;

  return (
    `🚀 *Earn FREE Points Every Week!*\n\n` +
    `Buy cheap offers, earn tickets, win cash prizes 🏆\n\n` +
    `🎁 Join now using my link and start earning instantly:\n\n` +
    `${link}\n\n` +
    `🔥 Limited weekly prizes available — don’t miss out!`
  );
}


// ===================== WITHDRAW PAGE BALANCE DISPLAY =====================
bot.on("message", async (msg) => {
  const chatId = msg.chat.id;
  const text = (msg.text || "").trim();

  if (text === "💸 Withdraw Points") {
    const user = (typeof getUser === "function") ? getUser(chatId) : {};
    const points = Number(user.points || 0).toFixed(2);
