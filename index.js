...
function formatAdminConfirmedReceipt(chatId, pending, txTs, txCostKsh = 0) {
  const parts = kenyaReceiptDateTimeParts(txTs || Date.now());
  const amount = Number(pending?.price || 0).toFixed(2);
  const receive = String(normalizePhone(pending?.receivePhone254 || pending?.phone254 || "") || (pending?.receivePhone254 || pending?.phone254 || "")).replace(/^\+/, "");

  // Prefer the user's Telegram account name saved on profile, then fall back safely.
  const u = getUser(chatId) || {};
  const tgFullName = [
    u?.profile?.firstName,
    u?.profile?.lastName,
    ...
    .replace(/\s+/g, " ")
    .trim();
  const name = tgFullName || safeDisplayName(u, chatId);

  // Remaining balance shown in Ksh value derived from current points balance.
  const balanceKes = Number(pointsToKes(getPoints(chatId) || 0)).toFixed(2);
  const cost = Number(txCostKsh || 0).toFixed(2);

  return `${randomReceiptCode(10)} Confirmed.on ${parts.date} at ${parts.time} Ksh${amount} received from ${receive} ${name}. New Account balance is Ksh${balanceKes}. Transaction cost, Ksh${cost}.`;
}
...
// Inline action buttons (Upgrade + Buy Again + Buy Offers)
    ...
    try {
      await notifyAdmin(formatAdminConfirmedReceipt(chatId, pending, txTs, 0));
    } catch (_) {}
