...
function updatePlanAtIndex(cat, idx, fields = {}) {
  ...
  const i = Number(idx);
  if (!Number.isInteger(i) || i < 0 || i >= list.length) return { ok: false, err: "Plan not found." };
  if (fields.label) list[i].label = String(fields.label).trim();
...
bot.on("message", async (msg) => {
  try {
    ...
    if (st.mode === "await_plan_price") {
      ...
      const label = `Ksh ${price} • ${String(st.amountText || "").trim()} ${String(st.periodText || "").trim()}`.replace(/\s+/g, " ").trim();
...
Choose behaviour.`, { chat_id: chatId, message_id: st.messageId, reply_markup: { inline_keyboard: [
        [{ text: "📦 Normal Flow", callback_data: `planadm:settpl:${base64UrlEncode(name)}:normal` }],
        [{ text: "📧 Gmail Style", callback_data: `planadm:settpl:${base64UrlEncode(name)}:gmail` }],
      ] } }).catch(() => {});
      return;
    }
    if (st.mode === "await_plan_amount") {
...
Ksh ${res.plan.price}` : `❌ ${res.err}`, { chat_id: chatId, message_id: st.messageId, ...plansAdminKeyboard() }).catch(() => {});
    ...
    if (st.mode === "await_load_stock") {
      const tpl = categoryTemplate(st.category);
      if (tpl !== "gmail") return;
      const res = addGmailStockFromLines(text);
      setPlanAdminState(chatId, { mode: "menu", messageId: st.messageId });
      await bot.editMessageText(`✅ Accounts loaded for ${st.category}
