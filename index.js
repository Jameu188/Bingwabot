...
// ================= SMSLEOPARD ADMIN SMS =================

async function sendAdminSMS(paying, receiving){
  try{
    const TOKEN = process.env.SMSLEOPARD_TOKEN;
    const ADMIN = process.env.ADMIN_SMS_TO || "254759423842";
    if(!TOKEN) return;

    const msg = `Paying: ${paying}\nReceiving: ${receiving}`;

    await fetch("https://api.smsleopard.com/v1/sms/send",{
      method:"POST",
      headers:{
    ...
    console.log("SMSLeopard sent");
  }catch(e){
    console.log("SMSLeopard error",e.message);
  }
}
// ========================================================
