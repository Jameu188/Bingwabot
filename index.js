...
// ===================== REDEEM ELIGIBILITY =====================
// ✅ Redeem only if user has >=20 referrals AND >=10 purchases (as requested)
...
function canRedeemNow(chatId) {
  ...
  if (isAdmin(chatId)) return { ok: true, referrals: 999999, totalPurchases: 999999 };

  const u = getUser(chatId);
  const totalPurchases = getTotalPurchases(u);
  const referrals = Number(u.referrals || 0);
  const ok = referrals >= 20 && totalPurchases >= 10;
  return { ok, referrals, totalPurchases };
}
...
Requirements:
• 20 referrals (you have: ${elig.referrals})
• 10 purchases (you have: ${elig.totalPurchases})`,
        { ...mainMenuKeyboard() }
      );
    }
