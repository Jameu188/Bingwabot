...
function cloneCartStateFromSession(s) {
  if (!s) return {};
  return {
    cart: Array.isArray(s.cart) ? s.cart.map((item) => ({ ...item })) : [],
    cartEditIndex: s.cartEditIndex ?? null,
    cartEditField: s.cartEditField ?? null,
    cartReminderMsgId: s.cartReminderMsgId ?? null,
    cartLastActionAt: s.cartLastActionAt || 0,
    cartLastReminderAt: s.cartLastReminderAt || 0,
  };
}

function applyCartStateToSession(target, cartState) {
  if (!target) return target;
  const src = cartState || {};
  target.cart = Array.isArray(src.cart) ? src.cart.map((item) => ({ ...item })) : [];
  target.cartEditIndex = src.cartEditIndex ?? null;
  target.cartEditField = src.cartEditField ?? null;
  target.cartReminderMsgId = src.cartReminderMsgId ?? null;
  target.cartLastActionAt = src.cartLastActionAt || 0;
  target.cartLastReminderAt = src.cartLastReminderAt || 0;
  return target;
}
...
function addItemToSessionCart(s, item) {
  ...
  if (cart.length >= 10) return false;
  cart.push({
...
if (pending && pending.category === "Business Account") {
  ...
  if (plan) {
    ...
    try {
      const ss = sessions.get(chatId);
      if (ss && Array.isArray(ss.cart) && ss.cart.length) {
        ss.cartEditIndex = null;
        ss.cartEditField = null;
        markCartActivity(ss, true);
...
if (text === "🏠 Main Menu") {
  ...
  try {
    ...
    const nextSession = applyCartStateToSession({ step: null }, cloneCartStateFromSession(existing));
...
if (isBtn(text,"🛒 Buy Offers","🛒 Nunua Ofa")) {
    const currentSession = sessions.get(chatId) || { step: null };
    pushPrev(chatId, currentSession);
    const nextSession = applyCartStateToSession({ step: "category", category: null, pkgKey: null, createdAt: Date.now() }, cloneCartStateFromSession(currentSession));
    sessions.set(chatId, nextSession);
    await sendBannerIfAvailable(chatId);
...
• 2541XXXXXXXX`, confirmKeyboard(true, false, chatId));
        ...
        const addedToCart = addItemToSessionCart(s, { category: s.category, pkgLabel: pkg.label, price: pkg.price, phone254: paymentPhone, receivePhone254: receivePhoneNow });
        if (!addedToCart) {
          markCartActivity(s, true);
          sessions.set(chatId, s);
          return sendTracked(chatId, "⚠️ Cart limit reached. You can only keep up to 10 orders in cart at once.", { ...cartKeyboard() });
        }
        await deleteCartReminderMessage(chatId, s);
        s.step = "cart_review";
        s.category = null;
...
✏️ Choose what to edit for item ${Number(s.cartEditIndex) + 1}:
...
• 📥 Change Receive Number`,
  ...
  if (text === "🏠 Main Menu") {
    const nextSession = applyCartStateToSession({ step: null }, cloneCartStateFromSession(s));
    sessions.set(chatId, nextSession);
    return sendTracked(chatId, "", { ...mainMenuKeyboard(chatId) });
  }
  if (isBtn(text, "🛒 Buy Offers", "🛒 Nunua Ofa")) {
    const nextSession = applyCartStateToSession({ step: "category", category: null, pkgKey: null, prev: [] }, cloneCartStateFromSession(s));
    sessions.set(chatId, nextSession);
    return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard(chatId));
  }
