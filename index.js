...
function generateBackupKey() {
  return `BINGWABACKUP${randomUpperToken(11)}${kenyaBackupStamp()}${randomUpperToken(4)}`;
}

function backupKeyInfo(key) {
  db = repairDB(db);
  return db.backupVault?.[String(key || "").trim()] || null;
}
...
function markBackupUsed(backupKey, restoredByChatId) {
  try {
    ...
    if (!key || !db.backupVault?.[key]) return;
    db.backupVault[key].usedAt = Date.now();
    db.backupVault[key].restoredByChatId = String(restoredByChatId || "");
    db.usedBackupIds[key] = Date.now();
    saveDB();
  } catch (_) {}
}
...
function createServerBackupKey(chatId) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (!db.backupVault[key]) break;
  }
  if (!key) throw new Error("Failed to generate backup key.");
...
if (data === "biz:backup_once") {
  ...
  await bot.answerCallbackQuery(q.id, { text: "⏳ Creating your backup key..." }).catch(() => {});
  ...
  try {
    ...
    await bot.sendMessage(chatId, "✅ Backup key saved. Use /restorebackup YOUR_KEY whenever you want to restore.", mainMenuKeyboard(chatId)).catch(() => {});
  } catch (e) {
...
// ===================== USER RESTORE: /restorebackup =====================
{
  ...
  if (restoreMatch) {
    ...
    if (!suppliedKey) {
      ...
      await bot.sendMessage(
        chatId,
        "🔐 Send your backup restore key now.\n\nExample:\nBINGWABACKUPR67TG8JSRON080220260745AMXXXX",
        mainMenuKeyboard(chatId)
      ).catch(()=>{});
      return;
    }
    ...
    try {
      ...
      if (!rec || !rec.payload) throw new Error("Invalid backup key.");
      if (isBackupAlreadyUsed(suppliedKey)) throw new Error("This backup key has already been used.");
      ...
      await bot.sendMessage(chatId, "❌ Restore failed: " + (e?.message || e), mainMenuKeyboard(chatId)).catch(()=>{});
      try {
        const u = getUser(chatId);
        u.pendingAction = null;
        saveDB();
