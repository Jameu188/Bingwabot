...
// ======================================================================
  ...
  // ===================== SAVED NUMBERS PICKER (TAP BUTTONS) =====================
  if (text === "📋 Saved Numbers") {
    ...
    // Build inline tap buttons (2 per row)
    const shown = list.slice(0, 10);
    const inline_keyboard = [];
    for (let i = 0; i < shown.length; i += 2) {
      const row = [];
      const a = shown[i];
      row.push({ text: `${i + 1}. ${maskPhone(a)}`, callback_data: `ui:pick_phone:${base64UrlEncode(a)}` });
      const b = shown[i + 1];
      if (b) row.push({ text: `${i + 2}. ${maskPhone(b)}`, callback_data: `ui:pick_phone:${base64UrlEncode(b)}` });
      inline_keyboard.push(row);
    }
    inline_keyboard.push([{ text: "⬅️ Back", callback_data: "ui:pick_phone_back" }]);
    inline_keyboard.push([{ text: "❌ Cancel", callback_data: "ui:pick_phone_cancel" }]);

    return sendTracked(
      chatId,
      "📋 *Saved Numbers*\\n\\nTap a number to use it now:",
