...
function isOnButtonCooldown(chatId) {
  ...
  if (now - last < Number(ANTI_SPAM.BUTTON_COOLDOWN_MS || 3000)) return true;
...
Rule: You earn when your referred user completes a successful purchase.`,
  ...
  if (data.startsWith("ui:upgrade:")) {
    ...
    if (action.startsWith("stk_cancel:")) {
      ...
      try {
        const keep = sessions.get(chatId);
        if (hasActiveCart(keep)) sessions.set(chatId, preserveCartSessionForMenu(keep, "menu"));
        else sessions.set(chatId, { step: "menu", createdAt: Date.now() });
      } catch (_) {}

      await bot.answerCallbackQuery(q.id, { text: "Cancelled" }).catch(() => {});
      return sendTracked(
        chatId,
        "❌ Pending STK request cancelled. Returning to main menu.",
        { ...mainMenuKeyboard(chatId) }
      );
    }
...
function cloneSession(s) {
  ...
  try {
    ...
    return {
      ...
      cartPaymentPhone254: s.cartPaymentPhone254 || "",
      cartPaymentEditUsed: !!s.cartPaymentEditUsed,
...
await sendTracked(chatId, "🔔 Sending STK push… Check your phone.");
  ...
  try {
    ...
    if (hasActiveCart(s)) sessions.set(chatId, preserveCartSessionForMenu(s, "menu"));
    else sessions.delete(chatId);

    return sendTracked(
      chatId,
      `✅ STK sent!\n\nOffer: ${pkg.label}\nPay: Ksh ${pkg.price}\nPayment Number: ${maskPhone(phone)}\nReceive Number: ${maskPhone(receivePhone)}\nChannel: ${channelId}\nRef: ${ref}\nTime: ${nowISO()}\n\nWhen payment is successful you will receive a confirmation message here.\n\nIf delay: ${HELP_PHONE}`,
      { ...mainMenuKeyboard(chatId) }
    );
  } catch (err) {
...
function mainMenuKeyboard(chatId = null) {
  ...
  try {
    if (chatId && isMultiPurchaseEnabled(chatId)) {
      keyboard.push(["🛒 View Orders in Cart"]);
    }
  } catch (_) {}
  try {
...
function clearSessionCart(s) {
  ...
  s.cartPaymentPhone254 = "";
  s.cartPaymentEditUsed = false;
}

function getCartRoundPaymentPhone(s) {
  if (!s) return "";
  const direct = String(s.cartPaymentPhone254 || "").trim();
  if (direct) return direct;
  const first = getSessionCartItems(s)[0];
  return String(first?.phone254 || "").trim();
}

function hasActiveCart(s) {
  return !!(s && Array.isArray(s.cart) && s.cart.length);
}

function applyCartPaymentNumberToAllItems(s, phone) {
  if (!s) return "";
  const normalized = String(phone || "").trim();
  if (!normalized) return "";
  const cart = getSessionCartItems(s);
  s.cartPaymentPhone254 = normalized;
  for (const item of cart) {
    item.phone254 = normalized;
    if (!item.receivePhone254) item.receivePhone254 = normalized;
  }
  markCartActivity(s, true);
  return normalized;
}

function normalizeCartPaymentNumbers(s) {
  if (!s) return "";
  const roundPhone = getCartRoundPaymentPhone(s);
  if (!roundPhone) return "";
  return applyCartPaymentNumberToAllItems(s, roundPhone);
}

function preserveCartSessionForMenu(s, step = "menu") {
  const base = s && typeof s === "object" ? cloneSession(s) || {} : {};
  const keep = {
    ...base,
    step,
    createdAt: Date.now(),
  };
  if (!Array.isArray(keep.cart)) keep.cart = [];
  if (keep.cart.length) normalizeCartPaymentNumbers(keep);
  return keep;
}
...
function addItemToSessionCart(s, item) {
  if (!s) return;
  const cart = getSessionCartItems(s);
  const sharedPayment = getCartRoundPaymentPhone(s) || String(item?.phone254 || "");
  if (sharedPayment) s.cartPaymentPhone254 = sharedPayment;
  cart.push({
    category: String(item?.category || ""),
    pkgLabel: String(item?.pkgLabel || ""),
    price: Number(item?.price || 0),
    phone254: String(sharedPayment || item?.phone254 || ""),
    receivePhone254: String(item?.receivePhone254 || item?.phone254 || sharedPayment || ""),
  });
  if (sharedPayment) applyCartPaymentNumberToAllItems(s, sharedPayment);
  markCartActivity(s, true);
}
...
Tap 🛒 View Orders in Cart to continue, edit numbers, remove items, or complete all cart.`,
...
Tap ➕ Add to Cart from any offer to start building your cart.`, { ...mainMenuKeyboard(chatId) });
  ...
  normalizeCartPaymentNumbers(session);
...
function confirmKeyboard(hasSavedPhone, hasReceivePhone = true, chatId = null) {
  ...
  if (cartHasItems) normalizeCartPaymentNumbers(liveSession);
  const canChangeCartPayment = !(multiEnabled && cartHasItems && liveSession?.cartPaymentEditUsed);
  ...
  if (canChangeCartPayment) rows.push(["📞 Change Payment Number", "📥 Change Receive Number"]);
  else rows.push(["📥 Change Receive Number"]);
  rows.push([PREV_BTN, "🏠 Main Menu"]);
  return { reply_markup: { keyboard: rows, resize_keyboard: true, one_time_keyboard: false } };
}
...
function confirmKeyboardV2(chatId, s, hasSavedPhone) {
  ...
  if (!(multiEnabled && cartHasItems && s?.cartPaymentEditUsed)) rows.push(["📞 Change Payment Number", "📥 Change Receive Number"]);
  else rows.push(["📥 Change Receive Number"]);
  rows.push([PREV_BTN, "🏠 Main Menu"]);
...
async function proceedPurchase(chatId, s) {
  const cart = getSessionCartItems(s);
  if (cart.length) {
    normalizeCartPaymentNumbers(s);
    let workingCart = getSessionCartItems(s).slice();
...
` +
    ...
    const receiveOrders = isCartPayment
      ? pending.cartOrders.map((order) => ({
          receivePhone254: order.receivePhone254 || order.phone254 || pending?.receivePhone254 || pending?.phone254 || "",
          pkgLabel: order.pkgLabel || pending?.pkgLabel || "",
          price: Number(order.price || 0),
        }))
      : [{
          receivePhone254: pending?.receivePhone254 || pending?.phone254 || "",
          pkgLabel: pending?.pkgLabel || "",
          price: Number(pending?.price || 0),
        }];
    const receiveLines = receiveOrders.map((order, idx) => `${idx + 1}. ${maskPhone(order.receivePhone254 || "")} - ${order.pkgLabel || "Offer"}`).join("\n");
    const amountLine = receiveOrders.map((order) => `Ksh ${Number(order.price || 0)}`).join(", ");
...
` +
      `💰 Amount${receiveOrders.length > 1 ? "s" : ""}: ${amountLine}
` +
      `🛒 Total Purchases: *${totalPurchases}*
...
// ===================== PURCHASE FLOW =====================
  ...
  try {
    ...
    if (text === "⬅ Back" || text === "⬅️ Back" || text === "⬅ Rudi" || text === "⬅️ Rudi") {
      if (hasActiveCart(s)) {
        sessions.set(chatId, preserveCartSessionForMenu(s, "menu"));
        return sendTracked(chatId, "🛒 Pending orders kept in cart. Use 🛒 View Orders in Cart when ready.", { ...mainMenuKeyboard(chatId) });
      }
  ...
  return sendTracked(
    ...
    confirmKeyboardV2(chatId, s, false)
      ...
      if (text === "➕ Add to Cart") {
        ...
        const currentCartPayment = getCartRoundPaymentPhone(s);
        const paymentPhone = currentCartPayment || getUserPhone(chatId);
...
${buildCartSummaryText(chatId, s)}`,
      ...
      if (phoneMaybe) {
        ...
        if (hasActiveCart(s)) {
          const previousCartPhone = getCartRoundPaymentPhone(s);
          if (previousCartPhone && previousCartPhone !== phoneMaybe) {
            if (s.cartPaymentEditUsed) {
              return sendTracked(chatId, "🚫 Cart payment number can only be changed once per cart round. All orders in this cart use the same payment number.", { ...cartKeyboard() });
            }
            s.cartPaymentEditUsed = true;
          }
          applyCartPaymentNumberToAllItems(s, phoneMaybe);
        } else if (s.cartEditIndex !== null && s.cartEditIndex !== undefined && Array.isArray(s.cart) && s.cart[s.cartEditIndex]) {
...
if (s.cartEditIndex !== null && s.cartEditIndex !== undefined && Array.isArray(s.cart) && s.cart[s.cartEditIndex]) {
  if (s.cartEditField === "payment") {
    ...
    markCartActivity(s, true);
    sessions.set(chatId, s);
    return sendTracked(chatId, `✅ Cart payment number updated for all orders in this cart round.
...
if (s.step === "cart_review") {
  ...
  if (text === "✏️ Edit Numbers") {
    ...
    markCartActivity(s, true);
    sessions.set(chatId, s);
    return sendTracked(chatId, "✏️ Choose a cart item to edit receive number:", cartEditKeyboard(s));
  }
...
📱 Payment: *${mdEscape(maskPhone(getCartRoundPaymentPhone(s) || cart[idx].phone254 || "") || "Not set")}*
📥 Receive: *${mdEscape(maskPhone(cart[idx].receivePhone254 || cart[idx].phone254 || "") || "Not set")}*

🛒 All cart orders use one shared payment number.`, { parse_mode: "Markdown", ...cartEditActionKeyboard(s) });
}
...
if (s.step === "cart_edit_action") {
  ...
  if (text === "📞 Edit Payment Number") {
    return sendTracked(chatId, "🚫 All orders in the same cart use one shared payment number. Use 📞 Change Payment Number before checkout if you need the one allowed cart-round change.", { ...cartKeyboard() });
  }
...
• 2541XXXXXXXX`, confirmKeyboard(!!item.phone254, false, chatId));
  ...
  if (text === "✏️ Edit Both Numbers") {
    return sendTracked(chatId, "🚫 All orders in the same cart use one shared payment number. Only receive numbers can be edited per item.", { ...cartEditActionKeyboard(s) });
  }
...
📱 Payment: *${mdEscape(maskPhone(getCartRoundPaymentPhone(s) || s.cart[s.cartEditIndex].phone254 || "") || "Not set")}*
📥 Receive: *${mdEscape(maskPhone(s.cart[s.cartEditIndex].receivePhone254 || s.cart[s.cartEditIndex].phone254 || "") || "Not set")}*
...
📱 Payment number: *${maskPhone(getCartRoundPaymentPhone(s) || getUserPhone(chatId) || "")}*
📥 Receive number: *${maskPhone(s.receivePhone254 || getUserReceivePhone(chatId) || "")}*

Choose:
• ✅ Proceed
• ➕ Add to Cart${(isMultiPurchaseEnabled(chatId) && hasActiveCart(s) && s.cartPaymentEditUsed) ? `` : `
• 📞 Change Payment Number`}
• 📥 Change Receive Number`,
  ...
  if (text === "🏠 Main Menu") {
    sessions.set(chatId, hasActiveCart(s) ? preserveCartSessionForMenu(s, "menu") : preserveCartSessionForMenu({}, "menu"));
    return sendTracked(chatId, "", { ...mainMenuKeyboard(chatId) });
  }
  if (isBtn(text, "🛒 Buy Offers", "🛒 Nunua Ofa")) {
    sessions.set(chatId, { step: "category", category: null, pkgKey: null, prev: [] });
    return sendTracked(chatId, section("CHOOSE CATEGORY") + "✅ Choose a category:", categoriesKeyboard(chatId));
  }
  if (isBtn(text, "📞 Change Payment Number", "📞 Badilisha Namba ya Malipo")) {
    if (isMultiPurchaseEnabled(chatId) && hasActiveCart(s) && s.cartPaymentEditUsed) {
      return sendTracked(chatId, "🚫 Cart payment number can only be changed once per cart round.", { ...cartKeyboard() });
    }
    s.step = "phone";
    sessions.set(chatId, s);
    return sendTracked(
      chatId,
      `📱 Send your PAYMENT number:
Current: ${maskPhone(getCartRoundPaymentPhone(s) || getUserPhone(chatId) || "") || "Not set"}
• 07XXXXXXXX
• 01XXXXXXXX
• 2547XXXXXXXX
• 2541XXXXXXXX`,
...
async function confirmGmailPhoneAndPush(chatId, qty, phoneInput, total) {
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `✅ STK sent for Gmail Accounts.\n\n📦 Package: ${pkgLabel}\n💰 Amount: Ksh ${total}\n📱 Number: ${formatTo07(phone254)}\n\nCheck your phone and complete payment.`
    ).catch(() => {});
  } catch (e) {
    deletePendingPayment(externalRef);
