...
function ensureSupportStore() {
  ...
  if (!db.adminInbox || typeof db.adminInbox !== "object") db.adminInbox = { threads: {}, order: [] };
  if (!db.adminInbox.threads || typeof db.adminInbox.threads !== "object") db.adminInbox.threads = {};
  if (!Array.isArray(db.adminInbox.order)) db.adminInbox.order = [];
  return db.supportTickets;
}

function supportThreadKey(chatId) {
  return String(chatId || "");
}

function ensureAdminInboxThread(chatId) {
  ensureSupportStore();
  const key = supportThreadKey(chatId);
  const u = getUser(chatId);
  const box = db.adminInbox;
  if (!box.threads[key] || typeof box.threads[key] !== "object") {
    box.threads[key] = {
      chatId: key,
      name: safeDisplayName(u, chatId),
      username: u?.username || "",
      totalMessages: 0,
      unreadCount: 0,
      notificationMsgId: 0,
  ...
  const t = box.threads[key];
  t.name = safeDisplayName(u, chatId);
  if (u?.username) t.username = String(u.username);
  if (!Array.isArray(t.messages)) t.messages = [];
  if (typeof t.panelMsgId !== "number") t.panelMsgId = 0;
  if (!box.order.includes(key)) box.order.unshift(key);
  box.order = [key, ...box.order.filter((x) => x !== key)].slice(0, 500);
  return t;
}

function describeSupportMessage(msg) {
  const caption = String(msg?.caption || "").trim();
  const text = String(msg?.text || caption || "").trim();
  if (text) return text.slice(0, 120);
  if (msg?.photo) return "[Photo]" + (caption ? ` ${caption.slice(0, 100)}` : "");
  if (msg?.video) return "[Video]" + (caption ? ` ${caption.slice(0, 100)}` : "");
  if (msg?.audio) return "[Audio]" + (caption ? ` ${caption.slice(0, 100)}` : "");
...
function addAdminInboxMessage(chatId, msg, fromSide, extra = {}) {
  const t = ensureAdminInboxThread(chatId);
  const item = {
    id: `${Date.now()}_${Math.random().toString(36).slice(2,8)}`,
    from: String(fromSide || "user"),
    ts: Date.now(),
    sourceChatId: String(msg?.chat?.id || chatId || ""),
    sourceMessageId: Number(msg?.message_id || 0),
    mediaGroupId: msg?.media_group_id ? String(msg.media_group_id) : "",
    preview: describeSupportMessage(msg),
    issueKey: String(extra.issueKey || ""),
    ticketId: String(extra.ticketId || ""),
    deleted: false,
  };
  t.messages.push(item);
  t.totalMessages = Number(t.totalMessages || 0) + 1;
  if (item.from === "user") t.unreadCount = Number(t.unreadCount || 0) + 1;
  t.lastMessageAt = item.ts;
  saveDB();
  return { thread: t, item };
}

function adminInboxSummaryLine(t, idx) {
  return `${idx}. ${t.name} [${Number(t.totalMessages || 0)} messages, ${Number(t.unreadCount || 0)} unread]`;
}

function adminInboxListRows(limit = 20) {
  ensureSupportStore();
  return Object.values(db.adminInbox.threads || {})
    .sort((a, b) => Number(b.lastMessageAt || 0) - Number(a.lastMessageAt || 0))
    .slice(0, limit);
}

function adminInboxListText() {
  const rows = adminInboxListRows(20);
  if (!rows.length) return "📥 Admin Inbox\n\nNo messages yet.";
  return "📥 Admin Inbox\n\n" + rows.map((t, i) => adminInboxSummaryLine(t, i + 1)).join("\n");
}

function adminInboxListKeyboard() {
  const rows = adminInboxListRows(20)
    .map((t) => [{ text: `${t.name} (${Number(t.totalMessages || 0)})`, callback_data: `ainbox:open:${t.chatId}` }]);
  if (!rows.length) rows.push([{ text: "No inbox messages", callback_data: "ainbox:none" }]);
  rows.push([{ text: "🔄 Refresh", callback_data: "ainbox:list" }]);
  return { reply_markup: { inline_keyboard: rows } };
}

function adminInboxNotificationText(thread) {
  return `📩 New message from ${thread.name}
Total ${Number(thread.unreadCount || 0)} unread messages.
Total ${Number(thread.totalMessages || 0)} messages.`;
}

function adminInboxNotificationKeyboard(thread) {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: `📂 Open inbox of ${thread.name}`.slice(0, 64), callback_data: `ainbox:open:${thread.chatId}` }],
        [{ text: `💬 Reply to ${thread.name}`.slice(0, 64), callback_data: `ainbox:reply:${thread.chatId}` }],
        [{ text: "🗑 Delete this notification", callback_data: `ainbox:delnotif:${thread.chatId}` }],
      ],
    },
  };
}

function adminInboxThreadText(thread, limit = 20) {
  const rows = (thread.messages || []).filter((m) => !m.deleted);
  const latest = rows.slice(-limit).reverse();
  const lines = [
    `📂 Inbox of ${thread.name}`,
    `Total ${Number(thread.totalMessages || 0)} messages.`,
    `Unread ${Number(thread.unreadCount || 0)} messages.`,
    "",
  ];

  if (!latest.length) {
    lines.push("No messages in this inbox.");
    return lines.join("\n");
  }

  latest.forEach((m, idx) => {
    lines.push(`${idx + 1}. ${kenyaDateTimeFromTs(Number(m.ts || Date.now()))}`);
    lines.push(`${m.from === "admin" ? "Admin" : thread.name}: ${String(m.preview || "[Message]")}`);
    lines.push("");
  });

  if (rows.length > latest.length) {
    lines.push(`Showing latest ${latest.length} of ${rows.length} messages.`);
  }

  return lines.join("\n").trim();
}

function adminInboxThreadKeyboard(thread) {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: `💬 Reply to ${thread.name}`.slice(0, 64), callback_data: `ainbox:reply:${thread.chatId}` }],
        [
          { text: "🔄 Refresh", callback_data: `ainbox:open:${thread.chatId}` },
          { text: "⬅️ Back to Inbox", callback_data: "ainbox:list" },
        ],
      ],
    },
  };
...
async function showAdminInboxList(adminChatId, sourceMessageId = 0) {
  const text = adminInboxListText();
  const opts = adminInboxListKeyboard();
  try {
    if (sourceMessageId) {
      await bot.editMessageText(text, { chat_id: adminChatId, message_id: sourceMessageId, ...opts });
      return sourceMessageId;
    }
  } catch (_) {}
  const sent = await bot.sendMessage(adminChatId, text, opts).catch(() => null);
  return sent?.message_id || 0;
}

async function upsertAdminInboxNotification(chatId) {
  const thread = ensureAdminInboxThread(chatId);
  if (!ADMIN_ID) return;
  const text = adminInboxNotificationText(thread);
  const opts = adminInboxNotificationKeyboard(thread);
  try {
    if (thread.notificationMsgId) {
      await bot.editMessageText(text, { chat_id: ADMIN_ID, message_id: thread.notificationMsgId, ...opts }).catch(() => {});
      await bot.editMessageReplyMarkup(opts.reply_markup, { chat_id: ADMIN_ID, message_id: thread.notificationMsgId }).catch(() => {});
    } else {
      const sent = await bot.sendMessage(ADMIN_ID, text, opts).catch(() => null);
      if (sent?.message_id) {
        thread.notificationMsgId = sent.message_id;
        saveDB();
      }
    }
  } catch (e) {
    console.log("admin inbox notification failed:", e?.message || e);
  }
}

async function upsertAdminInboxThreadPanel(adminChatId, targetChatId, sourceMessageId = 0) {
  const thread = ensureAdminInboxThread(targetChatId);
  const text = adminInboxThreadText(thread, 20);
  const opts = adminInboxThreadKeyboard(thread);
  const editId = Number(sourceMessageId || thread.panelMsgId || 0);
  try {
    if (editId) {
      await bot.editMessageText(text, { chat_id: adminChatId, message_id: editId, ...opts });
      thread.panelMsgId = editId;
      saveDB();
      return editId;
    }
  } catch (_) {}
  const sent = await bot.sendMessage(adminChatId, text, opts).catch(() => null);
  if (sent?.message_id) {
    thread.panelMsgId = sent.message_id;
    saveDB();
...
async function openAdminInboxThread(adminChatId, targetChatId, sourceMessageId = 0) {
  const thread = ensureAdminInboxThread(targetChatId);
  thread.unreadCount = 0;
  saveDB();
  await upsertAdminInboxNotification(targetChatId);
  return await upsertAdminInboxThreadPanel(adminChatId, targetChatId, sourceMessageId);
}
...
if (data === "ainbox:none") {
  await bot.answerCallbackQuery(q.id, { text: "No inbox messages" }).catch(() => {});
  return;
}
...
if (data === "ainbox:list") {
  if (!isAdmin(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "Not allowed" }).catch(() => {});
    return;
  }
  await bot.answerCallbackQuery(q.id, { text: "Refreshing inbox" }).catch(() => {});
  await showAdminInboxList(chatId, q.message?.message_id || 0);
  return;
}

if (data.startsWith("ainbox:")) {
  const parts = data.split(":");
  const action = parts[1] || "";
  const targetChatId = String(parts[2] || "");
  if (!isAdmin(chatId)) {
    await bot.answerCallbackQuery(q.id, { text: "Not allowed" }).catch(() => {});
    return;
  }
  if (action === "open") {
    await bot.answerCallbackQuery(q.id, { text: "Opening inbox" }).catch(() => {});
    await openAdminInboxThread(chatId, targetChatId, q.message?.message_id || 0);
    return;
  }
  if (action === "reply") {
    adminReplyState.set(String(chatId), { targetChatId: Number(targetChatId), until: Date.now() + 30 * 60 * 1000 });
    await bot.answerCallbackQuery(q.id, { text: "Reply mode enabled" }).catch(() => {});
    await bot.sendMessage(chatId, `💬 Reply mode ON for ${targetChatId}.
Send any text, media, or album now.
Send /cancel to stop.`).catch(() => {});
    return;
  }
  if (action === "delnotif") {
    const t = ensureAdminInboxThread(targetChatId);
    if (t.notificationMsgId) {
      try { await bot.deleteMessage(chatId, t.notificationMsgId).catch(() => {}); } catch (_) {}
      t.notificationMsgId = 0;
      saveDB();
    }
    await bot.answerCallbackQuery(q.id, { text: "Notification deleted" }).catch(() => {});
    return;
  }
  if (action === "msgdel") {
    const msgId = String(parts[3] || "");
    const t = ensureAdminInboxThread(targetChatId);
    const row = (t.messages || []).find((x) => String(x.id) === msgId);
    if (row) row.deleted = true;
    saveDB();
    await upsertAdminInboxThreadPanel(chatId, targetChatId, q.message?.message_id || 0);
...
if (data === "support:talkadmin") {
  await bot.answerCallbackQuery(q.id).catch(() => {});
  try { setPendingAction(chatId, { type: "support_issue", issueKey: "admin" }); } catch (_) {}
  const prompt = `${supportAutoReply(chatId, "admin")}

${supportPromptFor("admin")}

Send /cancel to stop.`;
  await sendTracked(chatId, prompt, supportStatusKeyboard()).catch(() => {});
  return;
}
...
function helpSupportKeyboard() {
  const nav = helpAnswerKeyboard()?.reply_markup?.inline_keyboard || [];
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "💬 Talk to Admin", callback_data: "support:talkadmin" }],
        ...nav,
      ],
    },
  };
  ...
  const opts =
    key === "cmds"
      ...
      : key === "settings"
        ...
        : key === "support"
          ? { parse_mode: "Markdown", ...helpSupportKeyboard() }
...
async function sendToUserFromAdmin(targetChatId, adminMsg) {
  const sent = await bot.copyMessage(targetChatId, adminMsg.chat.id, adminMsg.message_id);
  try {
    addAdminInboxMessage(targetChatId, adminMsg, "admin", {});
  } catch (_) {}
  return sent;
}
...
app.get("/promo/click", (req, res) => {
  try {
    const ownerId = String(req.query && req.query.o || "").trim();
    const sourceId = String(req.query && req.query.s || "").trim();
    const buttonKey = String(req.query && req.query.b || "0").trim();
    const encodedDest = String(req.query && req.query.d || "").trim();
    const targetUrl = __promoB64Decode(encodedDest) || "/";

    if (ownerId) {
      try { recordChannelPromoClick(ownerId, sourceId, buttonKey, targetUrl); } catch (_) {}
    }
...

{
  ...
  if (act && act.type === "support_issue") {
    ...
    const details = String(text || msg.caption || describeSupportMessage(msg) || "").trim();
...
function ensureChannelPromoStore() {
  ...
  if (!db.channelPromo.analytics || typeof db.channelPromo.analytics !== "object") {
    db.channelPromo.analytics = { totalClicks: 0, byButton: {}, recent: [] };
  }
...
function normalizeChannelPromoButtons(input) {
  ...
  for (const row of rowsIn) {
    ...
    for (const b of cols) {
      ...
      const url = normalizePromoUrl((b && b.url) || "");
      if (text && url) rowBtns.push({ text, url });
    }
    if (rowBtns.length) rowsOut.push(rowBtns);
  }

  return rowsOut;
}

function getChannelPromoButtons() {
  const cfg = channelPromoCfg();
  const rows = normalizeChannelPromoButtons(cfg.buttons || []);
  if (rows.length) return rows;
  if (cfg.buttonText && cfg.buttonUrl) {
    return [[{ text: String(cfg.buttonText || "Open").trim().slice(0, 64), url: normalizePromoUrl(cfg.buttonUrl) }]];
  }
  return [];
}

function __promoB64Encode(v) {
  try { return Buffer.from(String(v || ""), "utf8").toString("base64url"); } catch (_) { return ""; }
}

function __promoB64Decode(v) {
  try { return Buffer.from(String(v || ""), "base64url").toString("utf8"); } catch (_) { return ""; }
}

function buildTrackedPromoUrl(rawUrl, ownerId = "", sourceId = "", buttonKey = "0") {
  const dest = wrapPromoUrlForOwner(rawUrl, ownerId, sourceId);
  if (!BASE_URL || !/^https?:\/\//i.test(String(BASE_URL || ""))) return dest;
  const base = String(BASE_URL).replace(/\/+$/, "");
  const qp = new URLSearchParams({
    o: String(ownerId || ""),
    s: String(sourceId || ""),
    b: String(buttonKey || "0"),
    d: __promoB64Encode(dest)
  });
  return `${base}/promo/click?${qp.toString()}`;
}

function promoButtonKeyboard(ownerId = "", sourceId = "") {
  const rows = getChannelPromoButtons();
  if (!rows.length) return null;

  const inline_keyboard = rows.map((row, rIdx) =>
    row.map((btn, cIdx) => ({
      text: String(btn.text || "Open").slice(0, 64),
      url: buildTrackedPromoUrl(btn.url, ownerId, sourceId, `${rIdx}_${cIdx}`)
    }))
  );
...
function setChannelPromoButton(buttonInput, buttonUrl) {
  const cfg = channelPromoCfg();

  if (Array.isArray(buttonInput)) {
    const rows = normalizeChannelPromoButtons(buttonInput);
    cfg.buttons = rows;
    if (rows[0] && rows[0][0]) {
      cfg.buttonText = rows[0][0].text;
      cfg.buttonUrl = rows[0][0].url;
    } else {
      cfg.buttonText = "";
      cfg.buttonUrl = "";
    }
  ...
  const text = String(buttonInput || "").trim();
  const url = normalizePromoUrl(buttonUrl);
  cfg.buttonText = text || "BUY NOW";
  cfg.buttonUrl = url;
  cfg.buttons = text && url ? [[{ text: cfg.buttonText, url: cfg.buttonUrl }]] : [];
  saveDB();
}

function recordChannelPromoClick(ownerId, sourceId, buttonKey, targetUrl) {
  try {
    db = repairDB(db);
    const store = ensureChannelPromoStore();
    store.analytics = store.analytics && typeof store.analytics === "object"
      ? store.analytics
      : { totalClicks: 0, byButton: {}, recent: [] };

    const cfg = channelPromoCfg();
    const rows = getChannelPromoButtons();
    let label = "";
    try {
      const parts = String(buttonKey || "0_0").split("_");
      const r = Number(parts[0] || 0);
      const c = Number(parts[1] || 0);
      label = String(rows?.[r]?.[c]?.text || "");
    } catch (_) {}

    const key = `${String(sourceId || "")}:${String(buttonKey || "0")}`;
    const rec = store.analytics.byButton[key] || {
      ownerId: String(ownerId || ""),
      sourceId: String(sourceId || ""),
      buttonKey: String(buttonKey || "0"),
      label: label || "(button)",
    ...
    rec.ownerId = String(ownerId || rec.ownerId || "");
    rec.sourceId = String(sourceId || rec.sourceId || "");
    rec.buttonKey = String(buttonKey || rec.buttonKey || "0");
    rec.label = label || rec.label || "(button)";
    rec.url = String(targetUrl || rec.url || "");
    rec.clicks = Number(rec.clicks || 0) + 1;
    rec.updatedAt = Date.now();

    store.analytics.totalClicks = Number(store.analytics.totalClicks || 0) + 1;
    store.analytics.byButton[key] = rec;
    store.analytics.recent = Array.isArray(store.analytics.recent) ? store.analytics.recent : [];
    store.analytics.recent.unshift({
      ownerId: rec.ownerId,
      sourceId: rec.sourceId,
      buttonKey: rec.buttonKey,
      label: rec.label,
      url: rec.url,
      ts: rec.updatedAt
    });
    if (store.analytics.recent.length > 50) store.analytics.recent = store.analytics.recent.slice(0, 50);
    saveDB();
  } catch (_) {}
}

function channelPromoAnalyticsText() {
  const store = ensureChannelPromoStore();
  const an = store.analytics || { totalClicks: 0, byButton: {}, recent: [] };
  const top = Object.values(an.byButton || {})
    .sort((a, b) => Number(b.clicks || 0) - Number(a.clicks || 0))
    .slice(0, 10);

  let text = "📊 BUTTON ANALYTICS\n\n";
  text += `Total clicks: ${Number(an.totalClicks || 0)}\n\n`;

  if (!top.length) {
    text += "No button clicks recorded yet.";
    return text;
  }

  text += "Top buttons:\n";
  for (const item of top) {
    text += `• ${String(item.label || "(button)")} — ${Number(item.clicks || 0)} click(s)\n`;
  }

  const recent = Array.isArray(an.recent) ? an.recent.slice(0, 8) : [];
  if (recent.length) {
    text += "\nRecent clicks:\n";
    for (const item of recent) {
      text += `• ${String(item.label || "(button)")} • ${kenyaDateTimeFromTs(Number(item.ts || 0))}\n`;
    }
  }
...
async function sendChannelPromoPreview(chatId) {
  ...
  const btnRows = getChannelPromoButtons();
  const btnCount = btnRows.reduce((n, row) => n + row.length, 0);
  const summary =
...
` +
    `Mode: ${cfg.mode}
` +
    `Buttons: ${btnCount ? `${btnCount} in ${btnRows.length} row(s)` : "(none)"}
` +
    `First link: ${btnCount && btnRows[0] && btnRows[0][0] ? btnRows[0][0].url : "(none)"}
` +
  ...
  return {
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: "👀 Preview", callback_data: "cpadm:preview" }, { text: "📊 Button Stats", callback_data: "cpadm:stats" }],
        [{ text: "🧹 Clear Promo", callback_data: "cpadm:clear" }, { text: "♻️ Reset Chat Flag", callback_data: "cpadm:reset_chat" }],
...
function channelPromoAdminText() {
  ...
  const btnRows = getChannelPromoButtons();
  const btnCount = btnRows.reduce((n, row) => n + row.length, 0);
  const btnText = btnCount ? `${btnCount} button(s) in ${btnRows.length} row(s)` : "(none)";
  const btnUrl = btnCount && btnRows[0] && btnRows[0][0] ? String(btnRows[0][0].url || "").trim() || "(none)" : "(none)";
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (data === "cpadm:stats") {
      await answerPromoCb(q, "Analytics opened");
      await bot.sendMessage(chatId, channelPromoAnalyticsText()).catch(() => {});
      return;
    }
    ...
    if (data === "cpadm:set_button") {
      ...
      await bot.sendMessage(chatId, "🔘 Send buttons in this format:\nBUY NOW | https://t.me/yourbot\n\nMultiple rows:\nBUY | https://a.com\nJOIN | https://b.com\n\nSame row:\nBUY | https://a.com || JOIN | https://b.com").catch(() => {});
      return;
    }
      ...
      if (text && !text.startsWith("/")) {
        ...
        for (const row of rows) {
          ...
          for (const col of cols) {
            ...
            const btnText = String(parts[0] || "").trim();
            const btnUrl = String(parts.slice(1).join("|") || "").trim();
            if (btnText && btnUrl) rowBtns.push({ text: btnText, url: btnUrl });
          }
        ...
        setChannelPromoButton(buttons);
        clearChannelPromoEditState(chatId);
        await bot.sendMessage(chatId, "✅ Promo buttons updated.").catch(() => {});
        await openChannelPromoAdminMenu(chatId);
      }
      return;
    }
...
bot.on("message", async (msg) => {
  ...
  if (text === "📥 Inbox" && isAdmin(chatId)) {
    return showAdminInboxList(chatId);
  }
