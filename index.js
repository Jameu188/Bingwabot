...
async function handlePayheroCallback(req, res) {
  try {
    ...
    const amountPaid = pending ? pending.price : "N/A";
    const statusVal = String(statusOutRaw ?? "").trim();
    const statusValWithDot = statusVal ? (statusVal.endsWith(".") ? statusVal : statusVal + ".") : "";
...
M-Pesa Ref: ${mpesaOut}
Time: ${when}
ResultCode/Status: ${statusValWithDot}`;

    await sendTracked(chatId, paymentMsg, { ...mainMenuKeyboard() });

    // ✅ Notify admin
    await notifyAdmin(paymentMsg);
  } catch (e) {
