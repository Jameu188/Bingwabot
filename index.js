...
function signValue(value, scope = "generic") {
  return crypto.createHmac("sha256", CART_SIGNING_SECRET).update(`${scope}:${String(value || "")}`).digest("hex").slice(0, 24);
}
...
function parseSignedCallbackData(action, data) {
  ...
  const expected = signValue(`${action}:${raw}`, "cb");
  if (!safeEq(sig, expected)) return { ok: false, reason: "sig" };
...
function getFrozenPendingCheckout(ref) {
  ensurePendingCheckoutStore();
  return db.pendingCheckoutSnapshots[String(ref)] || null;
}
...
function verifyPayheroCallbackRequest(req, rawBody, parsedPayload) {
  ...
  if (!PAYHERO_CALLBACK_SECRET) return { ok: true, mode: "unsigned" };
  const basis = typeof rawBody === "string" && rawBody ? rawBody : JSON.stringify(parsedPayload || req?.body || {});
  const expected = crypto.createHmac("sha256", PAYHERO_CALLBACK_SECRET).update(String(basis || "")).digest("hex");
  if (!safeEq(String(sig || ""), expected)) return { ok: false, reason: "bad_signature" };
...
function addPendingPayment(externalRef, payload) {
  ...
  freezePendingCheckout(externalRef, cleanPayload?.chatId || cleanPayload?.ownerChatId || "", {
    ...
    paymentPhone: String(cleanPayload?.phone254 || ""),
    receivePhone: String(cleanPayload?.receivePhone254 || cleanPayload?.phone254 || ""),
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    if (!callbackCheck.ok) {
      alertSuspiciousActivity("unknown", "callback_signature_invalid", { headers: Object.keys(req.headers || {}) });
      return;
    }
...
try {
  ...
  return;
    ...
    if (pending) {
      ...
      if (!amountOk || !phoneOk) {
        alertSuspiciousActivity(chatId, "callback_amount_or_phone_mismatch", { externalRef, cbAmount, cbPhone });
