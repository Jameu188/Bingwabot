...
function mainMenuKeyboard(chatId = null) {
  ...
  try {
    if (chatId && isMultiPurchaseEnabled(chatId)) {
      const s = sessions.get(chatId);
      if (s && Array.isArray(s.cart) && s.cart.length) keyboard.push(["🛒 View Cart"]);
    }
  } catch (_) {}
  if (chatId && isMultiPurchaseEnabled(chatId)) {
  ...
  return sendTracked(
    ...
    confirmKeyboardV2(chatId, s, false)
      ...
      if (text === "🛒 View Orders in Cart" || text === "🛒 View Cart") {
        if (!getSessionCartItems(s).length) {
          return sendTracked(chatId, "🛒 Your cart is empty.", { ...mainMenuKeyboard(chatId) });
        }
        s.step = "cart_review";
        markCartActivity(s, true);
        sessions.set(chatId, s);
