...
function darajaDebugSnapshot() {
  return {
    env: DARAJA_ENV,
    shortcode: DARAJA_SHORTCODE,
    callback: DARAJA_CALLBACK_URL,
    oauthUrl: DARAJA.oauthUrl,
    stkUrl: DARAJA.stkUrl,
    hasConsumerKey: !!DARAJA_CONSUMER_KEY,
    consumerKeyLength: DARAJA_CONSUMER_KEY.length,
    hasConsumerSecret: !!DARAJA_CONSUMER_SECRET,
    consumerSecretLength: DARAJA_CONSUMER_SECRET.length,
    hasPasskey: !!DARAJA_PASSKEY,
    passkeyLength: DARAJA_PASSKEY.length,
  };
}

function darajaFriendlyErrorMessage(error) {
  const raw = String(error?.message || error || "");
  if (raw.includes("500.001.1001") || /Wrong credentials/i.test(raw)) {
    return [
      "❌ Daraja credentials failed.",
      "Check Render env vars and redeploy:",
      `• DARAJA_ENV=${DARAJA_ENV}`,
      `• DARAJA_SHORTCODE=${DARAJA_SHORTCODE}`,
      `• has passkey: ${DARAJA_PASSKEY ? "yes" : "no"} (${DARAJA_PASSKEY.length} chars)`,
      `• full passkey: ${DARAJA_PASSKEY || "not set"}`,
      `• has consumer key: ${DARAJA_CONSUMER_KEY ? "yes" : "no"} (${DARAJA_CONSUMER_KEY.length} chars)`,
      `• has consumer secret: ${DARAJA_CONSUMER_SECRET ? "yes" : "no"} (${DARAJA_CONSUMER_SECRET.length} chars)`,
      "",
      "After fixing env vars, Save Changes and redeploy on Render.",
      "",
      `Raw error: ${raw}`,
    ].join("\n");
  }
...
async function getDarajaAccessToken() {
  ...
  console.log("Daraja token request config:", {
    ...darajaDebugSnapshot(),
...
async function darajaStkPush({ amount, phone, accountReference, transactionDesc }) {
  ...
  console.log("Daraja STK request config:", {
    ...darajaDebugSnapshot(),
    timestamp,
  ...
  const data = await res.json();
  console.log("Daraja STK raw response:", { status: res.status, ok: res.ok, data });
  if (!res.ok) {
...
bot.on("callback_query", async (query) => {
  ...
  try {
    ...
    await bot.sendMessage(chatId, darajaFriendlyErrorMessage(error), mainMenuKeyboard()).catch(() => {});
  }
});
...
app.listen(PORT, () => {
  ...
  console.log("Daraja startup config:", darajaDebugSnapshot());
});
