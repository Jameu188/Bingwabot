...
// ===================== PAYMENT IDS (PER CATEGORY) =====================
function ensurePaymentIds() {
  db.paymentIds = db.paymentIds && typeof db.paymentIds === "object"
    ? db.paymentIds
    : { data: null, sms: null, minutes: null, business: null };
}
ensurePaymentIds();

function getPaymentChannelId(category) {
  ensurePaymentIds();
  if (category === "Bingwa Deals" || category === "Unlimited Deals") return db.paymentIds.data;
  if (category === "SMS Offers") return db.paymentIds.sms;
  if (category === "Minutes") return db.paymentIds.minutes;
  if (category === "Business") return db.paymentIds.business;
  return null;
}
...
function setMenuKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "📶 Bingwa Deals", callback_data: "setcat:BINGWA" }],
        [{ text: "💬 SMS Offers", callback_data: "setcat:SMS" }],
        [{ text: "📞 Minutes", callback_data: "setcat:MINUTES" }],
        [{ text: "♾ Unlimited Deals", callback_data: "setcat:UNLIMITED" }],
        [{ text: "🏢 Business Plans", callback_data: "setcat:BUSINESS" }],
        [{ text: "📧 Gmail Payments", callback_data: "setcat:GMAIL" }],
        [{ text: "🏠 Main Menu", callback_data: "set:home" }]
      ]
    }
  };
}
...
function backButtons() {
  return {
    reply_markup: {
      inline_keyboard: [
        [
          { text: "⬅️ Back", callback_data: "set:back" },
          { text: "🏠 Main Menu", callback_data: "set:home" }
        ]
      ]
    }
...
bot.onText(/\/set/, async (msg) => {
  ...
  return bot.sendMessage(chatId, "⚙️ Select category to update payment ID:", setMenuKeyboard());
});
...
bot.on("callback_query", async (q) => {
  ...
  if (data === "set:back") {
    return bot.editMessageText("⚙️ Select category to update payment ID:", {
      chat_id: chatId,
      message_id: msgId,
      ...setMenuKeyboard()
    });
  ...
  if (data.startsWith("setcat:")) {
    ...
    const current = getPaymentId(category) || "Not set";

    const u = getUser(chatId);
    u.pendingAction = { type: "SET_PAYMENT_ID_INPUT", category };
    saveDB();

    return bot.editMessageText(
      `📌 Current ID: ${current}

✏️ Send new payment ID for ${category}:`,
      {
        chat_id: chatId,
        message_id: msgId,
        ...backButtons()
  ...
  if (data.startsWith("confirmset:")) {
    const parts = data.split(":");
    const category = parts[1];
    const value = parts[2];

    db = repairDB(db);
    db.paymentIds = db.paymentIds || {};
    db.paymentIds[category] = value;
...
bot.on("message", async (msg) => {
  ...
  if (Number(chatId) === Number(ADMIN_ID) && u.pendingAction?.type === "SET_PAYMENT_ID_INPUT") {
    const category = u.pendingAction.category;
...
New ID: ${text}`,
      {
        reply_markup: {
          inline_keyboard: [
            [
              { text: "✅ Confirm", callback_data: `confirmset:${category}:${text}` },
...
// GET ID
function getPaymentId(category) {
  db = repairDB(db);
  return db.paymentIds?.[category] || null;
}



function saveLastChange(category, oldValue) {
  db.lastChange = { category, oldValue };
}


// IMPORTANT: All payment usage now dynamic
// Example usage everywhere:
// const paymentId = getPaymentId(category);


// ===================== ADMIN PAYMENT SET =====================
function validateId(v) { return /^\d+$/.test(v); }

function setPaymentId(type, value) {
  ensurePaymentIds();
  const oldVal = db.paymentIds[type] || "Not set";
  db.paymentIds[type] = Number(value);
  saveDB();
  return `✅ ${type.toUpperCase()} Payment ID updated\nOld: ${oldVal}\nNew: ${value}`;
}

bot.onText(/\/setdata (.+)/, (msg, match) => {
  if (msg.chat.id != ADMIN_ID) return;
  if (!validateId(match[1])) return bot.sendMessage(msg.chat.id, "Invalid ID");
  bot.sendMessage(msg.chat.id, setPaymentId("data", match[1]));
});

bot.onText(/\/setsms (.+)/, (msg, match) => {
  if (msg.chat.id != ADMIN_ID) return;
  if (!validateId(match[1])) return bot.sendMessage(msg.chat.id, "Invalid ID");
  bot.sendMessage(msg.chat.id, setPaymentId("sms", match[1]));
});

bot.onText(/\/setminutes (.+)/, (msg, match) => {
  if (msg.chat.id != ADMIN_ID) return;
  if (!validateId(match[1])) return bot.sendMessage(msg.chat.id, "Invalid ID");
  bot.sendMessage(msg.chat.id, setPaymentId("minutes", match[1]));
});

bot.onText(/\/setbiz (.+)/, (msg, match) => {
  if (msg.chat.id != ADMIN_ID) return;
  if (!validateId(match[1])) return bot.sendMessage(msg.chat.id, "Invalid ID");
  bot.sendMessage(msg.chat.id, setPaymentId("business", match[1]));
});
