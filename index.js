...
bot.onText(/\/showtokenstatus/, async (msg) => {
  ...
  if (Number(chatId) !== Number(ADMIN_ID)) {
    return bot.sendMessage(chatId, "❌ Only admin can check token status.");
  }

  if (TELEGRAM_TOKEN && TELEGRAM_TOKEN.length > 10) {
    return bot.sendMessage(chatId, "✅ Bot token is currently SET in database.");
  } else {
    return bot.sendMessage(chatId, "⚠️ Bot token is NOT set.");
  }
});
}
...
bot.onText(/\/cleartoken/, async (msg) => {
  ...
  if (Number(chatId) !== Number(ADMIN_ID)) {
    return bot.sendMessage(chatId, "❌ Only admin can clear the bot token.");
  }
  ...
  try {
    ...
    return bot.sendMessage(chatId, "🗑 Bot token cleared. Bot is now stopped until a new token is set with /settoken.");
  } catch (err) {
    console.log("Clear token error:", err);
    return bot.sendMessage(chatId, "❌ Failed to clear token. Check logs.");
  }
});
}
