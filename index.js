...
// ✅ Referral bonus ONLY for purchases >= 20ksh and <= 1000ksh
// ✅ Referral reward format (admin):
//    🎯 Referral Reward
//    Buyer: <name>
//    Purchase: Ksh X
//    Phone number: 0723xxx435
//    Bonus: 0.10 pts
//    Time: <Kenya time>
...
// ✅ Max reward per day per account = 150 points (caps ALL point additions: referral + purchase points)
// ✅ Transaction history (account-only): last 15 transactions (success/failed) with date/time, package, price, status
//
// AUTH / ACCOUNT (as requested):
// ✅ First time user clicks /start => show ONLY welcome message (NO packages list) + ask Login/Register or Skip
// ✅ If user skips OR later not logged in => bot works for purchasing, BUT:
//    - No points shown
//    - Redeem disabled
//    - History disabled
//    - Referral disabled
//    - No "Referral Reward" messages to user
// ✅ Login session expires after 2 hours (auto logout)
// ✅ Account allows viewing points/history/redeem from ANY Telegram (login with username/password)
// ✅ Password: 4-8 chars
// ✅ Username: starts with Capital letter + must contain letters AND numbers (A... + includes at least one digit)
//
// NOTE:
...
// Rewards caps
const REWARDS = {
  MAX_PER_DAY: 150, // ✅ max points per day per account
};

// Auth session
const AUTH = {
  SESSION_MS: 2 * 60 * 60 * 1000, // 2 hours
};
...
//         pendingAction: null,
//         onboarded: false,
//         auth: { username: "", until: 0 },
//         dailyRewards: { "YYYY-MM-DD": number }
//      }
//   },
//   "accounts": {
//      "<Username>": {
...
//        points: 0,
//        dailyRewards: { "YYYY-MM-DD": number },
//        transactions: [ { at, when, externalRef, pkgLabel, price, status, rawStatus } ],
//        inviterId: ""   // inviter chatId stored (from /start ref_)
//      }
//   },
//   "bingwaByPhone": { "YYYY-MM-DD": { "2547xxxxxxx": 1 } },
//   "pendingPayments": { "<externalRef>": { chatId, category, pkgLabel, phone254, price, accountUsername?, createdAt } },
//   "processedPayments": { "<externalRef>": <timestamp> },
...
function repairDB(obj) {
  ...
  if (!db0.accounts || typeof db0.accounts !== "object") db0.accounts = {};
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.onboarded !== "boolean") x.onboarded = false;

    if (!x.auth || typeof x.auth !== "object") x.auth = { username: "", until: 0 };
    if (typeof x.auth.username !== "string") x.auth.username = "";
    if (typeof x.auth.until !== "number") x.auth.until = 0;

    if (!x.dailyRewards || typeof x.dailyRewards !== "object") x.dailyRewards = {};
    const drKeys = Object.keys(x.dailyRewards || {}).sort();
    while (drKeys.length > 14) {
      const oldest = drKeys.shift();
      delete x.dailyRewards[oldest];
    }
  }

  for (const [uname, a] of Object.entries(db0.accounts)) {
    if (!a || typeof a !== "object") db0.accounts[uname] = {};
    const x = db0.accounts[uname];
    if (typeof x.username !== "string") x.username = String(uname);
    if (typeof x.passHash !== "string") x.passHash = "";
    if (typeof x.passSalt !== "string") x.passSalt = "";
    ...
    const dayKeys = Object.keys(x.dailyRewards || {}).sort();
    while (dayKeys.length > 14) {
      const oldest = dayKeys.shift();
      delete x.dailyRewards[oldest];
    }

    if (x.transactions.length > 50) x.transactions = x.transactions.slice(0, 50);
  }
...
function isValidUsername(username) {
  ...
  // starts with capital letter, contains letters+digits, allowed only A-Za-z0-9
  // must include at least one digit and at least one letter
  if (!/^[A-Z][A-Za-z0-9]{3,19}$/.test(u)) return false; // total 4..20
...
function createAccount(username, password, inviterId = "") {
  db = repairDB(db);
  const u = String(username).trim();
  if (db.accounts[u]) return { ok: false, reason: "Username already exists." };
...
function verifyAccount(username, password) {
  ...
  const computed = hashPassword(password, a.passSalt);
  if (computed !== a.passHash) return { ok: false, reason: "Wrong password." };
  return { ok: true };
}

function loginChatToAccount(chatId, username) {
  const u = getUser(chatId);
  u.auth.username = String(username || "").trim();
  u.auth.until = Date.now() + AUTH.SESSION_MS;
  saveDB();
}

function logoutChat(chatId) {
  const u = getUser(chatId);
  u.auth.username = "";
  u.auth.until = 0;
  saveDB();
...
function getLoggedInUsername(chatId) {
  const u = getUser(chatId);
  const until = Number(u.auth?.until || 0);
  const uname = String(u.auth?.username || "").trim();
  if (!uname) return "";
  if (Date.now() > until) {
    // expired
    logoutChat(chatId);
...
function getAccount(username) {
  db = repairDB(db);
  return db.accounts[String(username || "").trim()] || null;
}

function addAccountPointsCapped(username, pts, whenDayKey = todayKey()) {
  db = repairDB(db);
  const a = getAccount(username);
  if (!a) return { added: 0, capped: false };

  const add = Number(pts || 0);
  if (!Number.isFinite(add) || add <= 0) return { added: 0, capped: false };

  const day = String(whenDayKey || todayKey());
  a.dailyRewards = a.dailyRewards || {};
  const earnedToday = Number(a.dailyRewards[day] || 0);
  const remaining = REWARDS.MAX_PER_DAY - earnedToday;
  ...
  const toAdd = Math.min(add, remaining);
  a.points = Number(a.points || 0) + toAdd;
  a.dailyRewards[day] = earnedToday + toAdd;

  saveDB();
  return { added: toAdd, capped: toAdd < add };
}

function getAccountPoints(username) {
  const a = getAccount(username);
  return Number(a?.points || 0);
}

function pushAccountTransaction(username, tx) {
  db = repairDB(db);
  const a = getAccount(username);
  if (!a) return;
  if (!Array.isArray(a.transactions)) a.transactions = [];
  a.transactions.unshift({ at: Date.now(), ...tx });
  if (a.transactions.length > 50) a.transactions = a.transactions.slice(0, 50);
  saveDB();
}

function getLastAccountTransactions(username, limit = 15) {
  const a = getAccount(username);
  const arr = Array.isArray(a?.transactions) ? a.transactions : [];
...
// ✅ Earn points only on SUCCESS purchase (account-only)
function awardSuccessPurchasePointsToAccount(username, pending, whenDayKey) {
  if (!username) return;
  if (!pending || !pending.pkgLabel) return;
  const label = String(pending.pkgLabel).toLowerCase();

  if (label.includes("20 sms")) addAccountPointsCapped(username, 5, whenDayKey);
  if (label.includes("250mb")) addAccountPointsCapped(username, 20, whenDayKey);
  if (label.includes("20 min")) addAccountPointsCapped(username, 20, whenDayKey);
}
...
// discover bot username for clickable referral link
let BOT_USERNAME = "";
bot
  .getMe()
  .then((me) => {
    BOT_USERNAME = String(me?.username || "").trim();
    console.log("🤖 Bot username:", BOT_USERNAME ? `@${BOT_USERNAME}` : "(unknown)");
  })
  .catch(() => {});
...
const broadcastAlbums = new Map();
const supportState = new Map();
const adminReplyState = new Map();

// auth flow state
const authState = new Map(); // chatId -> { step: "username"|"password", mode:"login"|"register", usernameTemp:"" }
...
function mainMenuKeyboard(chatId) {
  const uname = getLoggedInUsername(chatId);
  const loggedIn = !!uname;

  if (!loggedIn) {
    return {
      reply_markup: {
        keyboard: [["🛒 Buy Offers"], ["🔐 Login/Register"], ["ℹ️ Help", "❌ Cancel"]],
...
function authKeyboard() {
  return {
    reply_markup: {
      keyboard: [["🔐 Login/Register"], ["⏭ Skip"], ["❌ Cancel"]],
      resize_keyboard: true,
      one_time_keyboard: false,
    },
  };
...
function redeemKeyboard() {
  return {
    reply_markup: {
      keyboard: [["1️⃣ 20 free SMS (5 pts)"], ["2️⃣ 250MB free (20 pts)"], ["⬅ Back", "❌ Cancel"]],
      resize_keyboard: true,
      one_time_keyboard: false,
    },
  };
...
// ===================== TEXTS =====================
function welcomeTextAgent(name) {
  return (
    `👋 Hello 𝐀𝐠𝐞𝐧𝐭, welcome to Bingwa Mtaani Data Services.\n\n` +
...
// ✅ Referral bonus credit ONLY on SUCCESS callback AND only if buyer was logged-in (account-based)
function maybeRewardInviterOnSuccessPurchaseAccount(buyerChatId, buyerName, buyerPhone07, amountKsh, when, buyerAccountUsername) {
  if (!buyerAccountUsername) return; // buyer not logged in => no referral system at all

  const buyerUser = getUser(buyerChatId);
  const inviterChatId = String(buyerUser.inviterId || "");
  if (!inviterChatId) return;

  // inviter must have an account too to receive points
  const inviterUser = getUser(inviterChatId);
  const inviterAccount = String(inviterUser.auth?.username || "").trim();
  if (!inviterAccount || !getAccount(inviterAccount)) return;

  const amt = Number(amountKsh || 0);
  if (!Number.isFinite(amt) || amt < REFERRAL.MIN_KSH || amt > REFERRAL.MAX_KSH) return;

  const bonus = amt * REFERRAL.BONUS_PERCENT;
  if (!Number.isFinite(bonus) || bonus <= 0) return;

  const dayKey = todayKey();
  const r = addAccountPointsCapped(inviterAccount, bonus, dayKey);

  awaitNotifyAdminReferral({
    inviterChatId,
    inviterAccount,
    buyerName,
    amt,
...
async function awaitNotifyAdminReferral({ inviterChatId, inviterAccount, buyerName, amt, buyerPhone07, bonusAdded, when, capped }) {
  const cappedNote = capped ? " (CAPPED)" : "";
  await notifyAdmin(
    `🎯 *Referral Reward*\n` +
      `Buyer: *${mdEscape(String(buyerName || "Unknown"))}*\n` +
      `Purchase: *Ksh ${mdEscape(amt)}*\n` +
      `Phone number: *${mdEscape(String(buyerPhone07 || ""))}*\n` +
      `Bonus: *${mdEscape(Number(bonusAdded || 0).toFixed(2))} pts*${mdEscape(cappedNote)}\n` +
      `Time: \`${mdEscape(String(when || kenyaDateTime()))}\`\n` +
      `Inviter ChatID: \`${mdEscape(inviterChatId)}\`\n` +
      `Inviter Account: \`${mdEscape(inviterAccount)}\``
  );
}
...
app.post("/payhero/callback", async (req, res) => {
  try {
    ...
    // ✅ SUCCESS RULE (true = success, missing/blank = fail):
    let isSuccess = false;
    ...
    if (resultCodeNum !== null && Number.isFinite(resultCodeNum)) {
      isSuccess = resultCodeNum === 0;
    } else if (isTrueStatus) {
      isSuccess = true;
      ...
      isSuccess = false; // ✅ empty/missing behaves like NOT TRUE
    }
    ...
    const statusDisplay = String(resultCodeRaw ?? statusRaw ?? woostatus ?? "").trim(); // may be blank

    const pending = getPendingPayment(externalRef);
    const buyerName = pending?.buyerName || "";
    const buyerPhone07 = pending?.phone254 ? formatTo07(pending.phone254) : "";
    ...
    if (!isSuccess) {
      ...
      // store transaction only if account existed
      if (pending?.accountUsername) {
        pushAccountTransaction(pending.accountUsername, {
          when,
          externalRef,
    ...
    if (pending) {
      ...
      // ✅ account-only points + transactions
      if (pending.accountUsername) {
        awardSuccessPurchasePointsToAccount(pending.accountUsername, pending, dayKey);
        ...
        pushAccountTransaction(pending.accountUsername, {
          ...
          price: pending.price ?? null,
          status: "SUCCESS",
          rawStatus: statusDisplay || "true/0/receipt",
        });

        // referral (account-only + min/max amount)
        await maybeRewardInviterOnSuccessPurchaseAccount(
          chatId,
          pending.buyerName || "",
          formatTo07(pending.phone254 || ""),
          pending.price,
          when,
          pending.accountUsername
        );
      }
    ...
    await notifyAdmin(
      `✅ *Payment Confirmed*\n` +
        ...
        `Time: \`${mdEscape(when)}\`\n` +
        `ResultCode/Status: \`${mdEscape(String(resultCodeRaw ?? statusRaw ?? woostatus ?? ""))}\``
    );
  } catch (e) {
...
bot.on("message", async (msg) => {
  ...
  // ===================== AUTH FLOW HANDLER =====================
  const as = authState.get(String(chatId));
  if (as && !isAdmin(chatId)) {
    if (text === "❌ Cancel" || text === "/cancel") {
      authState.delete(String(chatId));
      return sendTracked(chatId, "❌ Cancelled.", { ...mainMenuKeyboard(chatId) });
    }

    if (as.step === "username") {
      const uname = String(text || "").trim();
      ...
      const exists = !!getAccount(uname);
      as.usernameTemp = uname;
      as.mode = exists ? "login" : "register";
      as.step = "password";
      authState.set(String(chatId), as);

      return sendTracked(
        chatId,
        exists
          ? "🔐 Enter your password (4-8 characters) to login:"
          : "🆕 Create a password (4-8 characters) to register:",
        { ...mainMenuKeyboard(chatId) }
      );
    }

    if (as.step === "password") {
      const pw = String(text || "");

      if (!isValidPassword(pw)) {
        return sendTracked(chatId, "❌ Password must be 4 - 8 characters. Try again:", { ...mainMenuKeyboard(chatId) });
      }

      const uname = as.usernameTemp;

      if (as.mode === "login") {
        const v = verifyAccount(uname, pw);
        if (!v.ok) {
          authState.delete(String(chatId));
          return sendTracked(chatId, `❌ Login failed: ${v.reason}`, { ...mainMenuKeyboard(chatId) });
        }
        loginChatToAccount(chatId, uname);
        authState.delete(String(chatId));
        return sendTracked(
          chatId,
          `✅ Logged in as *${mdEscape(uname)}* (auto logout in 2 hours).`,
          { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
        );
      }

      // register
      const inviterId = String(getUser(chatId).inviterId || "");
      const c = createAccount(uname, pw, inviterId);
      if (!c.ok) {
        authState.delete(String(chatId));
        return sendTracked(chatId, `❌ Registration failed: ${c.reason}`, { ...mainMenuKeyboard(chatId) });
      }
      loginChatToAccount(chatId, uname);
      authState.delete(String(chatId));
      ...
      return sendTracked(
        chatId,
        `✅ Account created & logged in as *${mdEscape(uname)}* (auto logout in 2 hours).`,
        { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
      );
    }
  }
  ...
  if (text.startsWith("/")) {
    ...
    if (startPayload !== null) {
      ...
      const u = getUser(chatId);
      const loggedInUser = getLoggedInUsername(chatId);

      // FIRST TIME: welcome only + ask login/register or skip
      if (!u.onboarded) {
        u.onboarded = true;
        saveDB();

        await sendTracked(chatId, welcomeTextAgent(msg.from?.first_name), { ...authKeyboard() });
        return sendTracked(
          chatId,
          "To access points, redeem, history from any phone:\nTap 🔐 Login/Register.\n\nOr tap ⏭ Skip to continue as guest.",
          { ...authKeyboard() }
        );
      }

      // not first time:
      // if logged in => show short status + menu (NO forced login)
      if (loggedInUser) {
        const pts = getAccountPoints(loggedInUser);
        return sendTracked(
          chatId,
          `👋 Welcome back, *${mdEscape(loggedInUser)}*.\n⭐ Points: *${pts.toFixed(2)}*\n\nUse the menu below.`,
          { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
        );
      }

      // guest mode (show packages list + guest menu)
      await sendTracked(chatId, packagesOverviewText(), { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) });
      return;
    }

    if (/^\/help$/i.test(text)) return sendTracked(chatId, helpText(), { ...mainMenuKeyboard(chatId) });
    ...
    if (isAdmin(chatId)) {
      ...
      if (/^\/stats$/i.test(text)) {
        ...
        const users = userEntries.map(([, u0]) => u0);
        ...
        for (const u0 of users) {
          if (isInactiveUser(u0)) inactiveUsers++;
          if (isActiveUser(u0)) activeUsers++;
        }
        ...
        const weeklyTotals = {};
        for (const u0 of users) {
          const wkMap = (u0.weeklyPurchases && u0.weeklyPurchases[wk]) || {};
  ...
  if (text === "🔐 Login/Register") {
    authState.set(String(chatId), { step: "username", mode: "login", usernameTemp: "" });
    return sendTracked(chatId, "Enter your username (Example: Agent7).", { ...mainMenuKeyboard(chatId) });
  }
  ...
  if (text === "👤 My Account") {
    if (!loggedIn) return sendTracked(chatId, "🔐 Please login to access your account.", { ...mainMenuKeyboard(chatId) });
    const pts = getAccountPoints(uname);
    return sendTracked(
      chatId,
      `👤 Account: *${mdEscape(uname)}*\n⭐ Points: *${pts.toFixed(2)}*\n⏳ Auto logout: 2 hours inactivity.`,
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  }

  if (text === "📜 History") {
    if (!loggedIn) return sendTracked(chatId, "🔐 Please login to view history.", { ...mainMenuKeyboard(chatId) });

    const list = getLastAccountTransactions(uname, 15);
    if (!list.length) return sendTracked(chatId, "📜 No transactions yet.", { ...mainMenuKeyboard(chatId) });
    ...
    for (const t of list) {
      const when = t.when || new Date(t.at || Date.now()).toLocaleString();
      const status = t.status || "FAILED";
      const pkg = t.pkgLabel || "(unknown)";
      const price = t.price != null ? `Ksh ${t.price}` : "";
      lines.push(`• *${status}* — ${when}`);
      lines.push(`  ${pkg}${price ? ` — ${price}` : ""}`);
      if (t.externalRef) lines.push(`  Ref: \`${mdEscape(t.externalRef)}\``);
      lines.push("");
    }
  ...
  if (text === "🔗 My Referral") {
    if (!loggedIn) return sendTracked(chatId, "🔐 Please login to use referrals.", { ...mainMenuKeyboard(chatId) });

    const link = BOT_USERNAME
      ? `https://t.me/${BOT_USERNAME}?start=ref_${chatId}`
      : `Use: /start ref_${chatId} (share this code)`;

    const pts = getAccountPoints(uname);

    return sendTracked(
      chatId,
      `🔗 Your referral link:\n${link}\n\n⭐ Points: *${pts.toFixed(2)}*\n\nRule:\n• Referral rewards only for purchases between Ksh ${REFERRAL.MIN_KSH} and Ksh ${REFERRAL.MAX_KSH}\n• Daily reward cap: ${REWARDS.MAX_PER_DAY} pts`,
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  }

  if (text === "🎁 Redeem Points") {
    if (!loggedIn) return sendTracked(chatId, "🔐 Please login to redeem points.", { ...mainMenuKeyboard(chatId) });
  ...
  if (text === "1️⃣ 20 free SMS (5 pts)" || text === "2️⃣ 250MB free (20 pts)") {
    ...
    const bal = getAccountPoints(uname);
    if (bal < item.cost) {
    ...
    setPendingAction(chatId, { kind: "redeem", step: "confirm", data: { item, account: uname }, createdAt: Date.now() });
  ...
  if (act && !isAdmin(chatId)) {
    ...
    if (act.kind === "redeem") {
      ...
      const accountU = String(act.data.account || "");
      ...
      if (act.step === "number") {
        ...
        if (!phone) return sendTracked(chatId, "❌ Invalid phone. Use 07/01/2547/2541 format.", { ...mainMenuKeyboard(chatId) });

        const cost = item.cost;
        const bal = getAccountPoints(accountU);
        if (bal < cost) {
          clearPendingAction(chatId);
          return sendTracked(chatId, `❌ Not enough points.\nYou have: ${bal.toFixed(2)} pts\nNeed: ${cost} pts`, {
            ...mainMenuKeyboard(chatId),
        ...
        // deduct from account
        const a = getAccount(accountU);
        a.points = Number(a.points || 0) - Number(cost || 0);
  ...
  try {
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        // If logged in => attach account for points/history/referral; otherwise guest purchase (no points)
        const accountUsername = getLoggedInUsername(chatId);
        ...
        addPendingPayment(ref, {
          ...
          buyerName: `${msg.from?.first_name || ""} ${msg.from?.last_name || ""}`.trim(),
        });
