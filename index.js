...
const ACCOUNT_AGE_WITHDRAW_LOCK_MS = 2 * 24 * 60 * 60 * 1000; // <2 days => no withdraw (day 3+)
...
if (!isAdmin(chatId) && !uWR.privilegedBypass && (isFrozen(uWR) || isHighRisk(uWR))) {
...
if (text === "💸 Withdraw Points") {
  ...
  const uLimit = getUser(chatId);
  const availableLimit = Math.max(0, uLimit.withdrawCapacityPts - (uLimit.totalWithdrawnPts || 0));
const uWR = getUser(chatId);
if (!isAdmin(chatId) && !uWR.privilegedBypass && (isFrozen(uWR) || isHighRisk(uWR))) {
  autoFreezeIfHighRisk(chatId, "User attempted withdraw/redeem while high risk");
  const d = freezeRemainingDays(uWR);
...
if (ageMs > 0 && ageMs < ACCOUNT_AGE_WITHDRAW_LOCK_MS) {
      fraudAlert("Withdraw blocked: account age < 2 days", chatId, "Auto-declined until day 3");
return sendTracked(chatId, "🚫 Withdrawals are available after day 3 of account activity.", { ...mainMenuKeyboard() });
}
// Minimum spend before withdraw
if (Number(u.totalSpentKsh || 0) < WITHDRAW_MIN_SPEND_KES) {
