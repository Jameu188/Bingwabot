  ...
  try {
    ...
    try {
      if (isAutoHideButtonsEnabled(chatId) && !data.startsWith("help:") && data !== "bh:toggle") {
        await bot.answerCallbackQuery(q.id, { text: "🔒 Buttons are hidden. Open Help → Commands & Settings to turn off." }).catch(() => {});
        return;
      }
    } catch (_) {}
...
try {
  ...
  const isHelpEntry = (text === "ℹ️ Help" || /^\/help\b/i.test(text) || /^help$/i.test(text));
  try {
    if (!isAdmin(chatId) && isAutoHideButtonsEnabled(chatId) && !isHelpEntry) {
      await sendTracked(
        chatId,
        "🔒 Buttons are hidden.\n\nTap ℹ️ *Help* → *Commands & Settings* → *Auto hide buttons (except Help)* to turn it OFF.\n\n(Shortcut: /buttons off)",
        { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
      );
      return;
    }
