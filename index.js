  ...
  return {
    reply_markup: {
      keyboard: [
        ...
        ["2b️⃣ Paid, received but no internet"],
...
function supportIssueKeyFromText(text) {
  ...
  if (t === "2b️⃣ Paid, received but no internet") return "internet";
...
function supportPromptFor(issueKey) {
  ...
  if (issueKey === "paid") {
    return "📝 Payment Support\n\nSend details in one message:\n• Offer/package\n• Amount paid\n• Phone number used\n• Approx time\n• M-Pesa message / transaction code (if available)";
  }
  if (issueKey === "internet") {
    return "📝 No Internet After Purchase\n\nSend details in one message:\n• Offer/package bought\n• Phone number used\n• Approx payment time\n• What you received\n• Error or problem shown after purchase";
  }
  if (issueKey === "wrong") {
...
function supportAutoReply(chatId, issueKey) {
  try {
    ...
    if (issueKey === "internet") {
      ...
      if (pending) {
        return (
          `ℹ️ I found a recent purchase request.\n\n` +
          `Offer: ${pending.pkgLabel || pending.category}\n` +
          `Phone: ${formatTo07(pending.phone254 || "") || pending.phone254 || "-"}\n` +
          `Amount: Ksh ${Number(pending.price || 0)}\n\n` +
          `If the offer was delivered but data is not working, send the exact problem so admin can check it faster.`
        );
      }
      return "ℹ️ If the offer arrived but data is not working, send the exact package, number used, and what problem you are seeing. Your ticket will be sent to admin.";
    }
    if (issueKey === "withdraw") {
...
// ===================== PROFILE SUPPORT CALLBACKS =====================
if (data.startsWith("profile_support:")) {
  const action = String(data.split(":")[1] || "menu");
  await bot.answerCallbackQuery(q.id).catch(() => {});

  if (action === "menu") {
    const txt =
      "🆘 *Support Center*\n\n" +
      "Choose the issue you want help with.\n\n" +
      "You can report things like:\n" +
      "• bought but not received\n" +
      "• bought, received but no internet\n" +
      "• STK not received\n" +
      "• wrong number used\n" +
      "• withdrawal / referral / Gmail issues\n\n" +
      "After choosing one option, send the details in one message and the ticket will go to admin.";
    await safeEditMessageText(chatId, q.message?.message_id, txt, { parse_mode: "Markdown", ...profileSupportIssueInlineKeyboard() });
    return;
  }

  if (action === "issue") {
    const issueKey = String(data.split(":")[2] || "admin");
    setPendingAction(chatId, { type: "support_issue", issueKey, source: "profile_inline" });
    const auto = supportAutoReply(chatId, issueKey);
    const txt = `${auto}\n\n${supportPromptFor(issueKey)}\n\nSend /cancel to stop.`;
    await safeEditMessageText(chatId, q.message?.message_id, txt, { ...profileSupportIssueInlineKeyboard() });
    return;
  }

  if (action === "tickets") {
    await safeEditMessageText(chatId, q.message?.message_id, mySupportTicketsText(chatId), profileSupportIssueInlineKeyboard());
    return;
  }
  ...
  if (action === "status") {
    ...
    const txt = !t
      ...
      : `📌 *Ticket Status*\n\nTicket ID: ${mdEscape(t.ticketId)}\nType: ${mdEscape(t.issueLabel)}\nStatus: *${mdEscape(t.status)}*\nUpdated: ${mdEscape(kenyaDateTimeFromTs(Number(t.updatedAt || t.createdAt || 0)))}`;
    await safeEditMessageText(chatId, q.message?.message_id, txt, { parse_mode: "Markdown", ...profileSupportIssueInlineKeyboard() });
    return;
  }
...
`;
  ...
  return sendTracked(chatId, msg, { parse_mode: "Markdown", ...profileSupportInlineKeyboard() });
}
