...
if (!db0.supportTickets || typeof db0.supportTickets !== "object") db0.supportTickets = {};
if (typeof db0.supportTicketCounter !== "number") db0.supportTicketCounter = 1000;
...
const SUPPORT_TYPES = {
  ...
  internet: "Paid, received but no internet",
...
function ensureSupportStore() {
  ...
  for (const [ticketId, raw] of Object.entries(db.supportTickets || {})) {
    const t = raw && typeof raw === "object" ? raw : (db.supportTickets[ticketId] = {});
    if (typeof t.ticketId !== "string") t.ticketId = String(ticketId || "");
    if (!Array.isArray(t.thread)) t.thread = [];
    if (typeof t.chatOpen !== "boolean") t.chatOpen = !["Resolved", "Deleted", "Closed", "Closed by user"].includes(String(t.status || ""));
    if (typeof t.deleted !== "boolean") t.deleted = false;
    if (typeof t.userEnded !== "boolean") t.userEnded = false;
    if (typeof t.adminReply !== "string") t.adminReply = "";
  }
...
function isSupportResolvedStatus(status) {
  const s = String(status || "").toLowerCase();
  return s === "resolved" || s === "deleted" || s === "closed" || s === "closed by user";
}

function isSupportActiveForAdmin(t) {
  if (!t || t.deleted) return false;
  if (isSupportResolvedStatus(t.status)) return false;
  return true;
}

function supportShortEntryLabel(entry) {
  const sender = String(entry?.sender || "user");
  if (sender === "admin") return "Admin";
  if (sender === "system") return "System";
  return "User";
}

function supportExtractMessageSummary(msg) {
  if (!msg) return "";
...
function supportAppendThread(ticketId, sender, msg, extra = {}) {
  ensureSupportStore();
  const t = db.supportTickets[String(ticketId)];
  if (!t) return null;
  if (!Array.isArray(t.thread)) t.thread = [];
  const entry = {
    sender: String(sender || "user"),
    text: String(extra.text || supportExtractMessageSummary(msg) || "").trim(),
...
function supportThreadText(t, limit = 12) {
  const rows = Array.isArray(t?.thread) ? t.thread.slice(-Number(limit || 12)) : [];
  if (!rows.length) return "No messages yet.";
  return rows.map((x) => {
    const when = kenyaDateTimeFromTs(Number(x.at || 0));
    const who = supportShortEntryLabel(x);
    const body = String(x.text || "-").slice(0, 500);
    return `[${when}] ${who}: ${body}`;
  }).join("\n\n");
}

function supportAdminListKeyboard(page = 0, totalPages = 1) {
  const p = Math.max(0, Number(page || 0));
  const last = Math.max(1, Number(totalPages || 1)) - 1;
  const row = [];
  if (p > 0) row.push({ text: "⬅️ Prev", callback_data: `spt:list:${p - 1}` });
  if (p < last) row.push({ text: "Next ➡️", callback_data: `spt:list:${p + 1}` });
  return {
    reply_markup: {
      inline_keyboard: [
        row.length ? row : [{ text: `Page ${p + 1}/${last + 1}`, callback_data: "noop" }],
...
function supportTicketKeyboard(ticketId, page = 0, forUser = false) {
  if (forUser) {
    const t = (db.supportTickets || {})[String(ticketId)] || null;
    const rows = [];
    if (t && t.chatOpen && !isSupportResolvedStatus(t.status)) rows.push([{ text: "💬 Reply to Support", callback_data: `sptu:reply:${ticketId}` }]);
    if (t && t.chatOpen && !isSupportResolvedStatus(t.status)) rows.push([{ text: "🛑 End Chat", callback_data: `sptu:end:${ticketId}` }]);
    if (t && !t.chatOpen && !isSupportResolvedStatus(t.status)) rows.push([{ text: "🔁 Raise Again", callback_data: `sptu:raise:${ticketId}` }]);
    rows.push([{ text: "📂 My Tickets", callback_data: "profile_support:tickets" }, { text: "🏠 Main Menu", callback_data: "profile_support:home" }]);
    return { reply_markup: { inline_keyboard: rows } };
  }
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "✅ Resolve", callback_data: `spt:resolve:${ticketId}:${page}` }, { text: "🗑 Delete", callback_data: `spt:delete:${ticketId}:${page}` }],
        [{ text: "⏳ Waiting", callback_data: `spt:waiting:${ticketId}:${page}` }, { text: "💬 Reply", callback_data: `spt:reply:${ticketId}:${page}` }],
        [{ text: "⬅️ Back to Requests", callback_data: `spt:list:${page}` }, { text: "🏠 Main Menu", callback_data: "ap:home" }],
      ]
    }
  };
}
...
function profileSupportInlineKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🆘 Support / Report Issue", callback_data: "profile_support:menu" }],
        [{ text: "📂 My Tickets", callback_data: "profile_support:tickets" }, { text: "📌 Ticket Status", callback_data: "profile_support:status" }],
      ]
    }
  };
}

function profileSupportIssueInlineKeyboard() {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "💸 Bought but not received", callback_data: "profile_support:issue:paid" }],
        [{ text: "📶 Bought, received but no internet", callback_data: "profile_support:issue:internet" }],
        [{ text: "📲 STK not received", callback_data: "profile_support:issue:stk" }],
        [{ text: "📱 Wrong number used", callback_data: "profile_support:issue:wrong" }],
        [{ text: "💸 Withdrawal issue", callback_data: "profile_support:issue:withdraw" }],
        [{ text: "⭐ Referral / Points", callback_data: "profile_support:issue:referral" }, { text: "📧 Gmail issue", callback_data: "profile_support:issue:gmail" }],
        [{ text: "💬 Talk to admin", callback_data: "profile_support:issue:admin" }],
        [{ text: "📂 My Tickets", callback_data: "profile_support:tickets" }, { text: "📌 Ticket Status", callback_data: "profile_support:status" }],
        [{ text: "⬅️ Back to Profile", callback_data: "profile_support:profile" }, { text: "🏠 Main Menu", callback_data: "profile_support:home" }],
      ]
    }
  };
}
...
function supportPromptFor(issueKey) {
  if (issueKey === "stk") return "📝 STK Support\n\nSend details in one message:\n• Offer/package\n• Phone number used\n• Approx time\n\nExample:\nKsh 20 • 250MB\n0707xxxxxx\n10:25 PM";
  if (issueKey === "paid") return "📝 Payment Support\n\nSend details in one message:\n• Offer/package\n• Amount paid\n• Phone number used\n• Approx time";
  if (issueKey === "internet") return "📝 Internet Issue Support\n\nSend details in one message:\n• Package received\n• Phone number used\n• When you bought\n• What internet problem you are seeing";
  if (issueKey === "wrong") return "📝 Wrong Number Support\n\nSend details in one message:\n• Wrong number used\n• Correct number\n• Offer/package";
  if (issueKey === "withdraw") return "📝 Withdrawal Support\n\nSend details in one message:\n• Issue\n• Amount\n• Phone number";
  if (issueKey === "referral") return "📝 Referral / Points Support\n\nSend details in one message:\n• Referral issue\n• Expected points\n• Any related user/number";
  if (issueKey === "gmail") return "📝 Gmail Support\n\nSend details in one message:\n• Quantity bought\n• Payment time\n• Delivery/login issue";
  return "📝 Talk to Admin\n\nSend your message here. You can continue chatting until you tap End Chat or admin resolves the ticket.";
}
...
function supportAutoReply(chatId, issueKey) {
  try {
    if (issueKey === "stk") {
      ...
      if (pending) {
        return (`ℹ️ I found a recent pending payment request.
...
` + `STK may delay for 1–2 minutes. Please avoid sending multiple requests.`);
    ...
    if (issueKey === "paid") return "ℹ️ If money was deducted, your request is prioritized automatically. Your details will be sent to admin for review.";
    if (issueKey === "internet") return "ℹ️ If the offer arrived but internet is not working, send the exact package and the problem you see. Your ticket will be sent to admin.";
    if (issueKey === "withdraw") {
      const pending = getPendingRedeem(chatId);
      if (pending && pending.type === "withdraw") return "ℹ️ Your withdrawal request is currently pending admin review.";
...
Your issue will be sent to admin.`;
    ...
    if (issueKey === "gmail") return "ℹ️ Please confirm whether the Gmail account was not delivered or was delivered but failed login. Your ticket will be sent to admin.";
    return "ℹ️ Your message will open a live support chat with admin.";
  } catch (_) {
...
function createSupportTicket(chatId, issueKey, details) {
  ...
  db.supportTickets[ticketId] = {
    ...
    status: issueKey === "admin" ? "Open Chat" : "Pending",
    createdAt: Date.now(),
    updatedAt: Date.now(),
  ...
  supportAppendThread(ticketId, "user", null, { text: String(details || "").trim(), kind: "text" });
  saveDB();
...
` + `Status: ${t.status}
` + `Chat Open: ${t.chatOpen ? "Yes" : "No"}

` + `Details:
${t.details || "-"}

` + `Conversation:
${supportThreadText(t, 10)}`);
}

function mySupportTicketsText(chatId) {
  ensureSupportStore();
  const rows = Object.values(db.supportTickets || {}).filter((x) => String(x.chatId || "") === String(chatId) && !x.deleted).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0)).slice(0, 10);
  if (!rows.length) return "📂 You have no support tickets yet.";
  return "📂 My Tickets\n\n" + rows.map((t) => `• ${t.ticketId} — ${t.issueLabel}\n  Status: ${t.status}\n  Chat: ${t.chatOpen ? 'Open' : 'Closed'}\n  Time: ${kenyaDateTimeFromTs(Number(t.createdAt || 0))}`).join("\n\n");
}

function latestUserSupportTicket(chatId) {
  ensureSupportStore();
  const rows = Object.values(db.supportTickets || {}).filter((x) => String(x.chatId || "") === String(chatId) && !x.deleted).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
  return rows[0] || null;
}

function userLatestOpenAdminTicket(chatId) {
  ensureSupportStore();
  const rows = Object.values(db.supportTickets || {}).filter((x) => String(x.chatId || "") === String(chatId) && String(x.issueKey || "") === "admin" && x.chatOpen && !isSupportResolvedStatus(x.status) && !x.deleted).sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
  return rows[0] || null;
}

function adminOpenSupportTickets(pageSize = 10, page = 0) {
  ensureSupportStore();
  const all = Object.values(db.supportTickets || {}).filter((x) => isSupportActiveForAdmin(x)).sort((a, b) => Number(b.updatedAt || b.createdAt || 0) - Number(a.updatedAt || a.createdAt || 0));
  const size = Math.max(1, Number(pageSize || 10));
  const p = Math.max(0, Number(page || 0));
  return all.slice(p * size, p * size + size);
}

function adminSupportTicketCount() {
  ensureSupportStore();
  return Object.values(db.supportTickets || {}).filter((x) => isSupportActiveForAdmin(x)).length;
}

async function forwardSupportMessageToAdmin(ticket, msg) {
  try { await bot.forwardMessage(ADMIN_ID, msg.chat.id, msg.message_id).catch(() => {}); } catch (_) {}
  await bot.sendMessage(ADMIN_ID, `🆘 ${ticket.ticketId} • ${ticket.userName}
Status: ${ticket.status}
Type: ${ticket.issueLabel}

Last message added to this chat.`, supportTicketKeyboard(ticket.ticketId, 0, false)).catch(() => {});
}

async function forwardSupportMessageToUser(ticket, msg) {
  try { await bot.forwardMessage(Number(ticket.chatId), msg.chat.id, msg.message_id).catch(() => {}); } catch (_) {}
  await bot.sendMessage(Number(ticket.chatId), `💬 Support reply for ${ticket.ticketId}

You can keep replying until you tap End Chat or admin resolves the ticket.`, supportTicketKeyboard(ticket.ticketId, 0, true)).catch(() => {});
}
...
if (data.startsWith("spt:")) {
  ...
  if (action === "resolve") {
    ...
    t.chatOpen = false;
    t.updatedAt = Date.now();
    supportAppendThread(ticketId, "system", null, { text: "Admin resolved this ticket.", kind: "system" });
    saveDB();
    if (String(getPendingAction(Number(t.chatId))?.ticketId || "") === ticketId) clearPendingAction(Number(t.chatId));
    await bot.answerCallbackQuery(q.id, { text: "Resolved" }).catch(() => {});
    await bot.sendMessage(Number(t.chatId), `✅ Your support ticket ${ticketId} has been resolved.`, supportTicketKeyboard(ticketId, 0, true)).catch(() => {});
    await safeEditMessageText(chatId, q.message?.message_id, `✅ ${ticketId} resolved and removed from active requests.`, { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back to Requests", callback_data: `spt:list:${page}` }, { text: "🏠 Main Menu", callback_data: "ap:home" }]] } });
    return;
  }
  ...
  if (action === "delete") {
    ...
    t.deleted = true;
    t.updatedAt = Date.now();
    supportAppendThread(ticketId, "system", null, { text: "Admin deleted this ticket.", kind: "system" });
    saveDB();
    if (String(getPendingAction(Number(t.chatId))?.ticketId || "") === ticketId) clearPendingAction(Number(t.chatId));
    await bot.answerCallbackQuery(q.id, { text: "Deleted" }).catch(() => {});
    await bot.sendMessage(Number(t.chatId), `❌ Your support ticket ${ticketId} was closed by support.`, supportTicketKeyboard(ticketId, 0, true)).catch(() => {});
    await safeEditMessageText(chatId, q.message?.message_id, `🗑 ${ticketId} deleted and removed from active requests.`, { reply_markup: { inline_keyboard: [[{ text: "⬅️ Back to Requests", callback_data: `spt:list:${page}` }, { text: "🏠 Main Menu", callback_data: "ap:home" }]] } });
    return;
  }
  ...
  if (action === "waiting") {
    ...
    t.chatOpen = true;
    t.updatedAt = Date.now();
    supportAppendThread(ticketId, "system", null, { text: "Admin marked the ticket as waiting.", kind: "system" });
    saveDB();
    await bot.answerCallbackQuery(q.id, { text: "Marked waiting" }).catch(() => {});
    await bot.sendMessage(Number(t.chatId), `⏳ Your support ticket ${ticketId} is waiting for follow-up.`, supportTicketKeyboard(ticketId, 0, true)).catch(() => {});
    await safeEditMessageText(chatId, q.message?.message_id, supportTicketText(t), supportTicketKeyboard(ticketId, page, false));
    return;
  }

  if (action === "reply") {
    t.chatOpen = true;
    t.status = isSupportResolvedStatus(t.status) ? "Open Chat" : (t.status || "Open Chat");
    t.updatedAt = Date.now();
    saveDB();
    setPendingAction(chatId, { type: "support_admin_chat", ticketId, page });
    await bot.answerCallbackQuery(q.id, { text: "Reply mode enabled" }).catch(() => {});
    await safeEditMessageText(chatId, q.message?.message_id, supportTicketText(t), supportTicketKeyboard(ticketId, page, false));
    await bot.sendMessage(chatId, `💬 Reply mode ON for ${ticketId}.

Send text, photo, video, voice, document, sticker or GIF.
Use /cancel to stop reply mode.`, { reply_markup: { keyboard: [["❌ Cancel"]], resize_keyboard: true, one_time_keyboard: false } }).catch(() => {});
    return;
  }
}
...
// ===================== ACTIONS FLOW (REDEEM ONLY) =====================
  ...
  if (act && act.type === "support_admin_chat") {
    if (!isAdmin(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Not authorized.", { ...mainMenuKeyboard(chatId) });
      return;
    ...
    if (/^\/cancel$/i.test(text) || text === "❌ Cancel") {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Reply mode cancelled.", { ...mainMenuKeyboard(chatId) });
      return;
    }
    ...
    if (!t.chatOpen || isSupportResolvedStatus(t.status)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, `❌ ${ticketId} is already closed.`, { ...mainMenuKeyboard(chatId) });
      return;
    }

    t.status = "Open Chat";
    t.updatedAt = Date.now();
    supportAppendThread(ticketId, "admin", msg);
    saveDB();

    await forwardSupportMessageToUser(t, msg);
    await sendTracked(chatId, `✅ Sent reply to ${ticketId}. Reply mode remains ON.`, { reply_markup: { keyboard: [["❌ Cancel"]], resize_keyboard: true, one_time_keyboard: false } });
    return;
  }

  if (act && act.type === "support_user_chat") {
    if (/^\/cancel$/i.test(text) || text === "❌ Cancel") {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Support reply cancelled.", supportTicketKeyboard(String(act.ticketId || ""), 0, true));
      return;
    }
    ...
    const ticketId = String(act.ticketId || "");
    const t = db.supportTickets[ticketId];
    if (!t || String(t.chatId || "") !== String(chatId)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, "❌ Ticket not found.", { ...mainMenuKeyboard(chatId) });
      return;
    }
    if (!t.chatOpen || isSupportResolvedStatus(t.status)) {
      clearPendingAction(chatId);
      await sendTracked(chatId, `❌ ${ticketId} is closed.`, supportTicketKeyboard(ticketId, 0, true));
      return;
    }

    t.status = "Customer Replied";
    t.updatedAt = Date.now();
    supportAppendThread(ticketId, "user", msg);
    saveDB();

    await forwardSupportMessageToAdmin(t, msg);
    await sendTracked(chatId, `✅ Message sent for ${ticketId}. Chat is still open.`, supportTicketKeyboard(ticketId, 0, true));
    return;
  }
...

{
  ...
  {
    const act2 = getPendingAction(chatId);
    if ((!act2 || !String(act2.type || "").startsWith("support_")) && !isAdmin(chatId) && msg && !String(text || "").startsWith("/")) {
      const live = userLatestOpenAdminTicket(chatId);
      if (live) {
        setPendingAction(chatId, { type: "support_user_chat", ticketId: live.ticketId });
        live.status = "Customer Replied";
        live.updatedAt = Date.now();
        supportAppendThread(live.ticketId, "user", msg);
        saveDB();
        await forwardSupportMessageToAdmin(live, msg);
        await sendTracked(chatId, `✅ Message sent for ${live.ticketId}. Chat is still open.`, supportTicketKeyboard(live.ticketId, 0, true));
        return;
      }
    }
  }
