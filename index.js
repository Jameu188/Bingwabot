...
function mainMenuKeyboard(chatId = null) {
  ...
  try { if (chatId && Number(chatId)===Number(ADMIN_ID)) keyboard.push(["📣 Channel Promo"]); keyboard.push(["📧 Gmail Admin"]); } catch(_){}
...
const GMAIL_DELIVERY_AUTO_DELETE_MS = 5 * 60 * 1000;

async function sendProtectedGmailDelivery(chatId, text) {
  const sent = await bot.sendMessage(chatId, text, {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🛒 Buy Offers", callback_data: "buy_offers" }],
  ...
  try {
    if (sent && sent.message_id) scheduleAutoDelete(sent, GMAIL_DELIVERY_AUTO_DELETE_MS);
  } catch (_) {}
  ...
  try {
    ...
    for (const [ref, rec] of Object.entries(store.pendingOrders || {})) {
      try {
        ...
        const msg =
          `━━━━━━━━━━━━━━━\n` +
          `✅ Gmail Purchase Successful\n` +
          `━━━━━━━━━━━━━━━\n\n` +
          `Your Gmail account(s) have been delivered below.\n\n` +
          `${details}` +
          `🔐 Security Reminder:\n` +
          `• Login immediately\n` +
          `• Change the password\n` +
          `• Update recovery details\n\n` +
          `🛡 Auto-Delete Protection: This delivery message will be removed automatically in 5 minutes for your privacy.\n\n` +
          `Thank you for choosing us! 🚀\n` +
          `━━━━━━━━━━━━━━━`;

        await sendProtectedGmailDelivery(chatId, msg).catch(() => {});
        rec.delivered = true;
