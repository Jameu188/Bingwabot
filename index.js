...
function repairDB(obj) {
  ...
  if (!db0.savedPhoneOwners || typeof db0.savedPhoneOwners !== "object") db0.savedPhoneOwners = {};
...
// 🔁 Build/repair savedPhoneOwners mapping from existing users (best-effort)
(function rebuildSavedPhoneOwners() {
  db = repairDB(db);
  if (!db.savedPhoneOwners || typeof db.savedPhoneOwners !== "object") db.savedPhoneOwners = {};

  const current = Object.keys(db.savedPhoneOwners).length;
  if (current > 0) return; // keep existing mapping if already present

  for (const [cid, u] of Object.entries(db.users || {})) {
    const p = String(u?.phone || "").trim();
    if (!p) continue;
    // First-seen wins; later duplicates will be blocked on next save attempt
    if (!db.savedPhoneOwners[p]) db.savedPhoneOwners[p] = String(cid);
  }
  saveDB();
})();
...
function setUserPhone(chatId, phone254) {
  // 🔒 Enforce: a phone number can be SAVED on only one Telegram account at a time.
  // STRICT-LOCK MODE: if a phone number is saved by another account, it cannot be used to purchase on this account.
  db = repairDB(db);
  const cid = String(chatId);
  const u = getUser(chatId);

  const next = String(phone254 || "").trim();

  // If clearing phone
  if (!next) {
    const old = String(u.phone || "");
    if (old && db.savedPhoneOwners?.[old] === cid) {
      delete db.savedPhoneOwners[old];
    }
    u.phone = "";
    saveDB();
    return { ok: true };
  ...
  const owner = String(db.savedPhoneOwners?.[next] || "");
  if (owner && owner !== cid) {
    return {
      ok: false,
      reason:
        "This phone number is already saved on another account.

" +
        "You cannot SAVE or PURCHASE using a number that is saved on a different account.
" +
        "Please use a different number, or ask the owner to change their saved number first.",
    };
  }

  // If user is changing from old phone, release it (only if they owned it)
  const old = String(u.phone || "");
  if (old && old !== next && db.savedPhoneOwners?.[old] === cid) {
    delete db.savedPhoneOwners[old];
  }

  // Assign ownership + save
  db.savedPhoneOwners[next] = cid;
  u.phone = next;
  saveDB();
  return { ok: true };
}
...
function isPhoneLockedToAnotherAccount(phone254, chatId) {
  db = repairDB(db);
  const owner = String(db.savedPhoneOwners?.[String(phone254)] || "");
  ...
  return sendTracked(chatId, msgText, { 
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        // 🔒 STRICT LOCK: block if this number is saved on another account
        if (isPhoneLockedToAnotherAccount(savedPhone, chatId)) {
          sessions.delete(chatId);
          return sendTracked(chatId, "This phone number is saved on another account and cannot be used.", { ...mainMenuKeyboard(chatId) });
        }
      ...
      if (phoneMaybe) {
        // 🔒 STRICT LOCK: block if this number is saved on another account
        if (isPhoneLockedToAnotherAccount(phoneMaybe, chatId)) {
          return sendTracked(chatId, "This phone number is saved on another account and cannot be used.", { ...mainMenuKeyboard(chatId) });
        }

        const saveRes = setUserPhone(chatId, phoneMaybe);
        if (!saveRes.ok) {
          return sendTracked(chatId, saveRes.reason, { ...mainMenuKeyboard(chatId) });
        }
...
if (s.step === "phone") {
  ...
  // 🔒 STRICT LOCK: block if this number is saved on another account
  if (isPhoneLockedToAnotherAccount(phone, chatId)) {
    sessions.delete(chatId);
    return sendTracked(chatId, "This phone number is saved on another account and cannot be used.", { ...mainMenuKeyboard(chatId) });
  }

  const saveRes = setUserPhone(chatId, phone);
  if (!saveRes.ok) {
    sessions.delete(chatId);
    return sendTracked(chatId, saveRes.reason, { ...mainMenuKeyboard(chatId) });
  }
