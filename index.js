...
` +
    `Buy cheap offers fast, earn points on purchases, and stand a chance to win weekly prizes 🏆
...
` +
    `🔗 *Your invite link:* [Tap here to open](${referralLink})
...
` +
    `Tap a button below to get your ready-to-share invite text instantly 👇`;
  ...
  await sendTracked(chatId, msg, {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: "📤 Get Invite Text", callback_data: "ref:invite_text" }],
...
${referralLink}`;
      ...
      await bot.sendMessage(chatId, `📢 Share this invite text:

${inviteText}`, { disable_web_page_preview: true }).catch(() => {});
      await bot.answerCallbackQuery(q.id, { text: "Invite text sent ✅", show_alert: false }).catch(() => {});
      return;
    }
...
function gmailPercentButtons(stockCount) {
  const qtyOptions = [1, 3, 5, 10, 15, 21, 27, stockCount];
  const rows = [];
  const seen = new Set();
  const price = gmailPrice();

  for (const rawQty of qtyOptions) {
    const qty = Math.max(1, Math.min(stockCount, Number(rawQty || 0)));
    if (qty < 1 || qty > stockCount || seen.has(qty)) continue;
    seen.add(qty);
    const total = qty * price;
    const label = `${qty} Email${qty > 1 ? "s" : ""}`;
    rows.push([{ text: `📧 Buy ${label} - Ksh ${total}`, callback_data: `gmail_qty:${qty}` }]);
  }

  if (!rows.length && stockCount > 0) {
    rows.push([{ text: `📧 Buy ${stockCount} Emails - Ksh ${stockCount * price}`, callback_data: `gmail_qty:${stockCount}` }]);
  }
