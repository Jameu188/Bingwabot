...
if (data === "qa:toggle") {
  const u = getUser(chatId);
  u.hideQuickActionsEnabled = !u.hideQuickActionsEnabled;

  // Auto turn OFF if there are no quick buttons available (expired/inactive)
  try {
    const quick = getQuickCategories(chatId);
    if (!quick || quick.length === 0) u.hideQuickActionsEnabled = false;
  } catch (_) {}
...
// ===== Quick Actions: Hide button on main menu =====
bot.onText(/^🙈 Hide Quick Actions$/i, async (msg) => {
  const chatId = msg.chat.id;
  try {
    const u = getUser(chatId);
    const quick = getQuickCategories(chatId);
    if (!quick || quick.length === 0) {
      u.hideQuickActionsEnabled = false; // auto-off when none
    } else {
      u.hideQuickActionsEnabled = true;
    }
    saveDB();
  } catch (_) {}
  return bot.sendMessage(chatId, "🙈 Quick Actions hidden.", { ...mainMenuKeyboard(chatId) }).catch(() => {});
});
