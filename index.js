...
// ===================== PROFIT CONFIG =====================
// Default cost percent if no overrides (e.g. 0.85 => 15% profit margin)
const DEFAULT_COST_PERCENT = 0.85;

// Optional: override cost percent per category
const CATEGORY_COST_PERCENT = {
  "Bingwa Deals": 0.88,
  "Unlimited Deals": 0.87,
  "SMS Offers": 0.80,
  Minutes: 0.82,
  "Bonga Points": 0.90,
  "Flex Deals": 0.86,
...
function getCostPercent(category) {
  return CATEGORY_COST_PERCENT[String(category || "")] ?? DEFAULT_COST_PERCENT;
}

function calculateProfit(amountKsh, category) {
  const amt = Number(amountKsh || 0);
  const cost = amt * getCostPercent(category);
  return Math.round((amt - cost) * 100) / 100;
}
...
function bumpBucket(map, key, amountKsh) {
  map[key] = map[key] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
  map[key].revenueKsh = round2(Number(map[key].revenueKsh || 0) + Number(amountKsh || 0));
  map[key].orders = Number(map[key].orders || 0) + 1;
}

function bumpBucketProfit(map, key, profitKsh) {
  map[key] = map[key] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
  map[key].profitKsh = round2(Number(map[key].profitKsh || 0) + Number(profitKsh || 0));
}

function bumpNested(map, k1, k2, amountKsh, profitKsh) {
  map[k1] = map[k1] || {};
  map[k1][k2] = map[k1][k2] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
  map[k1][k2].revenueKsh = round2(Number(map[k1][k2].revenueKsh || 0) + Number(amountKsh || 0));
  map[k1][k2].profitKsh = round2(Number(map[k1][k2].profitKsh || 0) + Number(profitKsh || 0));
  map[k1][k2].orders = Number(map[k1][k2].orders || 0) + 1;
}
...
function recordSuccessfulRevenue(pending) {
  ...
  const profit = calculateProfit(amount, pending.category);

  const day = todayKey();
  const wk = isoWeekKey(new Date());

  bumpBucket(db.analytics.revenueByDay, day, amount);
  bumpBucketProfit(db.analytics.revenueByDay, day, profit);

  bumpBucket(db.analytics.revenueByWeek, wk, amount);
  bumpBucketProfit(db.analytics.revenueByWeek, wk, profit);

  bumpNested(db.analytics.revenueByCategoryDay, day, String(pending.category || "Unknown"), amount, profit);
  bumpNested(db.analytics.revenueByPackageDay, day, String(pending.pkgLabel || "Unknown"), amount, profit);

  saveDB();
}

function bumpCatCount(map, day, category) {
  map[day] = map[day] || { count: 0, byCategory: {} };
  map[day].count = Number(map[day].count || 0) + 1;
  map[day].byCategory[category] = Number(map[day].byCategory[category] || 0) + 1;
}

function recordStkAttempt(pending) {
  if (!pending) return;
  db = repairDB(db);
  const day = todayKey();
  const cat = String(pending.category || "Unknown");
  bumpCatCount(db.analytics.attemptsByDay, day, cat);
  saveDB();
}

function recordPaymentSuccess(pending) {
  if (!pending) return;
  db = repairDB(db);
  const day = todayKey();
  const cat = String(pending.category || "Unknown");
  bumpCatCount(db.analytics.successByDay, day, cat);
  saveDB();
}

function recordPaymentFail(pending) {
  if (!pending) return;
  db = repairDB(db);
  const day = todayKey();
  const cat = String(pending.category || "Unknown");
  bumpCatCount(db.analytics.failByDay, day, cat);
  saveDB();
}

function conversionSummary(days = 0) {
  db = repairDB(db);
  const out = { attempts: 0, success: 0, failed: 0 };
  for (let i = 0; i <= days; i++) {
    const day = kenyaDayKeyOffset(i);
    out.attempts += Number(db.analytics.attemptsByDay?.[day]?.count || 0);
    out.success += Number(db.analytics.successByDay?.[day]?.count || 0);
    out.failed += Number(db.analytics.failByDay?.[day]?.count || 0);
  }
  out.rate = out.attempts ? round2((out.success / out.attempts) * 100) : 0;
  return out;
}

function sumDaysRevenue(daysBack) {
  db = repairDB(db);
  const out = { revenueKsh: 0, orders: 0, profitKsh: 0, byCategory: {}, byPackage: {} };
  for (let i = 0; i <= daysBack; i++) {
    ...
    if (dTot) {
      ...
      out.profitKsh += Number(dTot.profitKsh || 0);
    }
    const cat = db.analytics.revenueByCategoryDay?.[day] || {};
    for (const [k, v] of Object.entries(cat)) {
      out.byCategory[k] = out.byCategory[k] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
      out.byCategory[k].revenueKsh += Number(v.revenueKsh || 0);
      out.byCategory[k].orders += Number(v.orders || 0);
      out.byCategory[k].profitKsh += Number(v.profitKsh || 0);
    }
    const pkg = db.analytics.revenueByPackageDay?.[day] || {};
    for (const [k, v] of Object.entries(pkg)) {
      out.byPackage[k] = out.byPackage[k] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
      out.byPackage[k].revenueKsh += Number(v.revenueKsh || 0);
      out.byPackage[k].orders += Number(v.orders || 0);
      out.byPackage[k].profitKsh += Number(v.profitKsh || 0);
    }
  }
  out.revenueKsh = round2(out.revenueKsh);
  out.profitKsh = round2(out.profitKsh);
  return out;
}

function formatBuckets(title, obj, limit = 8) {
  const rows = Object.entries(obj || {})
    .map(([k, v]) => ({ k, revenueKsh: round2(v.revenueKsh), profitKsh: round2(v.profitKsh || 0), orders: Number(v.orders || 0) }))
  ...
  return (
    `${title}\n` +
    rows.map(r => `• ${r.k} — Ksh ${r.revenueKsh} (profit ${r.profitKsh}) • ${r.orders} orders`).join("\n")
  );
}

// ===== Phone ownership risk: one account per phone per Kenya day =====
function checkPhoneOwnershipSameDay(chatId, phone254) {
  if (!phone254) return { ok: true };
  if (isAdmin(chatId)) return { ok: true };
  db = repairDB(db);
  const day = todayKey();
  db.phoneOwnersByDay[day] = db.phoneOwnersByDay[day] || {};
  db.flaggedPhonesByDay[day] = db.flaggedPhonesByDay[day] || {};

  const owner = db.phoneOwnersByDay[day][String(phone254)];
  if (!owner) {
    db.phoneOwnersByDay[day][String(phone254)] = String(chatId);
    saveDB();
    return { ok: true };
  }
  if (String(owner) === String(chatId)) return { ok: true };

  const list = db.flaggedPhonesByDay[day][String(phone254)] || [];
  const uniq = Array.from(new Set([...list, String(owner), String(chatId)]));
  db.flaggedPhonesByDay[day][String(phone254)] = uniq;
  saveDB();

  notifyAdmin(
    `🚨 *Phone Reuse Flag*\\n` +
    `Day: \`${mdEscape(day)}\`\\n` +
    `Phone: *${mdEscape(formatTo07(phone254))}*\\n` +
    `First ChatID: \`${mdEscape(owner)}\`\\n` +
    `Second ChatID: \`${mdEscape(chatId)}\`\\n` +
    `Action: *Verify ownership*`
  );

  return { ok: false, reason: "🚫 This phone is already in use today. Admin verification required. Please contact support." };
}

function freezeAccount(chatId, reason) {
  db = repairDB(db);
  const u = getUser(chatId);
  if (!isAdmin(chatId) && u.isFrozen) {
    return bot.sendMessage(chatId, "🧊 Your account is frozen. Contact support.", mainMenuKeyboard());
  }
  const ageMs = Date.now() - Number(u.createdAt || 0);
  if (!isAdmin(chatId) && amount >= HIGH_WITHDRAW_PTS && (!u.createdAt || ageMs < NEW_ACCOUNT_MIN_AGE_MS)) {
    freezeAccount(chatId, `High-value withdraw (${amount} pts) too soon after signup`);
    return bot.sendMessage(chatId, "🚫 Withdrawal blocked. Your account has been frozen for verification. Contact support.", mainMenuKeyboard());
  }
  u.isFrozen = true;
  u.points = 0;
  saveDB();
...
function repairDB(obj) {
  ...
  for (const [cid, u] of Object.entries(db0.users)) {
    ...
    if (typeof x.createdAt !== "number") x.createdAt = 0;
    if (typeof x.isFrozen !== "boolean") x.isFrozen = false;
    if (typeof x.lifetimeProfitKsh !== "number") x.lifetimeProfitKsh = 0;
...
if (!isSuccess) {
  ...
  return;
    ...
    const pending = getPendingPayment(externalRef);
    if (pending) recordPaymentFail(pending);
...
// ===================== ANALYTICS COMMANDS =====================
const cmdRevenue = text.match(/^\/revenue(?:\s+(\d+))?$/i);
if (cmdRevenue) {
  if (!isAdmin(chatId)) {
    const u = getUser(chatId);
    const totalSpent = round2(Number(u.totalSpentKsh || 0));
    const totalOrders = Object.values(u.purchasesByDay || {}).reduce((a, b) => a + Number(b || 0), 0);
  ...
  const days = Math.max(0, Math.min(90, Number(cmdRevenue[1] || 0)));
  ...
  return bot.sendMessage(chatId, header + byCat + "\n\n" + byPkg, { parse_mode: "Markdown" });
}

if (/^\/revenueweek$/i.test(text)) {
  if (!isAdmin(chatId)) {
    const u = getUser(chatId);
    return bot.sendMessage(chatId, `👤 Weekly view is admin only.\nYour total spent: Ksh ${round2(Number(u.totalSpentKsh || 0))}`, mainMenuKeyboard());
  }
  db = repairDB(db);
  const wk = isoWeekKey(new Date());
  const x = db.analytics?.revenueByWeek?.[wk] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
  const aov = x.orders ? round2(Number(x.revenueKsh || 0) / Number(x.orders || 0)) : 0;

  return bot.sendMessage(
    chatId,
    `🗓️ *Bot Revenue This Week* (${wk})\n\nRevenue: *Ksh ${round2(x.revenueKsh)}*\nProfit: *Ksh ${round2(Number(x.profitKsh || 0))}*\nOrders: *${Number(x.orders || 0)}*\nAOV: *Ksh ${aov}*`,
    { parse_mode: "Markdown" }
  );
}

const cmdTopOffers = text.match(/^\/topoffers(?:\s+(\d+))?$/i);
if (cmdTopOffers) {
  if (!isAdmin(chatId)) return bot.sendMessage(chatId, "👤 Top offers analytics is admin only.", mainMenuKeyboard());
  const days = Math.max(0, Math.min(90, Number(cmdTopOffers[1] || 7)));
  const data = sumDaysRevenue(days);
  ...
  if (!rows.length) return bot.sendMessage(chatId, "No revenue data yet.");

  const lines = rows.map((r, i) => `${i + 1}. ${r.k} — *Ksh ${r.revenueKsh}* (${r.orders})`).join("\n");
  return bot.sendMessage(chatId, `🏆 *Top Offers (last ${days} day(s))*\n\n${lines}`, { parse_mode: "Markdown" });
}
...
function buildRevenueExportRows(days) {
  db = repairDB(db);
  const rows = [];
  rows.push(["date", "type", "key", "orders", "revenue_ksh", "profit_ksh"]);

  for (let i = days - 1; i >= 0; i--) {
    const day = kenyaDayKeyOffset(i);

    const tot = db.analytics?.revenueByDay?.[day] || { orders: 0, revenueKsh: 0, profitKsh: 0 };
    rows.push([day, "TOTAL", "ALL", Number(tot.orders || 0), Number(tot.revenueKsh || 0), Number(tot.profitKsh || 0)]);

    const byCat = db.analytics?.revenueByCategoryDay?.[day] || {};
    for (const [cat, v] of Object.entries(byCat)) {
      rows.push([day, "CATEGORY", cat, Number(v.orders || 0), Number(v.revenueKsh || 0), Number(v.profitKsh || 0)]);
    }

    const byPkg = db.analytics?.revenueByPackageDay?.[day] || {};
    for (const [pkg, v] of Object.entries(byPkg)) {
      rows.push([day, "PACKAGE", pkg, Number(v.orders || 0), Number(v.revenueKsh || 0), Number(v.profitKsh || 0)]);
    }
  }
  return rows;
}
...
const cmdExport = text.match(/^\/exportrevenue(?:\s+(\d+))?$/i);
if (cmdExport) {
  if (!isAdmin(chatId)) return bot.sendMessage(chatId, "👤 CSV export is admin only.", mainMenuKeyboard());
  const days = Number(cmdExport[1] || 30);
  try {
    const { filepath, filename, days: usedDays, rowCount } = writeRevenueCsvFile(days);
    await bot.sendDocument(chatId, filepath, { caption: `📎 Revenue CSV export\nWindow: last ${usedDays} day(s)\nRows: ${rowCount}` });
    try { fs.unlinkSync(filepath); } catch (_) {}
    return bot.sendMessage(chatId, `✅ Export sent: ${filename}`);
  } catch (e) {
    return bot.sendMessage(chatId, `⚠️ Export failed: ${e?.message || e}`);
  }
}

const cmdConv = text.match(/^\/conversion(?:\s+(\d+))?$/i);
if (cmdConv) {
  if (!isAdmin(chatId)) return bot.sendMessage(chatId, "👤 Conversion analytics is admin only.", mainMenuKeyboard());
  const days = Math.max(0, Math.min(90, Number(cmdConv[1] || 0)));
  const c = conversionSummary(days);
  return bot.sendMessage(
    chatId,
    `📈 *Conversion*\nWindow: *${days === 0 ? "Today" : `Last ${days} day(s)`}*\n\nAttempts: *${c.attempts}*\nSuccess: *${c.success}*\nFailed: *${c.failed}*\nRate: *${c.rate}%*`,
    { parse_mode: "Markdown" }
...
if (/^\/dashboard$/i.test(text)) {
  if (!isAdmin(chatId)) {
    const u = getUser(chatId);
    return bot.sendMessage(chatId, `👤 Dashboard is admin only.\nYour spent: Ksh ${round2(Number(u.totalSpentKsh || 0))}`, mainMenuKeyboard());
  }
  db = repairDB(db);
  const day = todayKey();
  const r = db.analytics?.revenueByDay?.[day] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
  const c = conversionSummary(0);
  const usersTotal = Object.keys(db.users || {}).filter(x => String(x) !== String(ADMIN_ID)).length;
  const pendingCount = Object.entries(db.pendingRedeems || {}).filter(([, rr]) => rr && rr.status === "pending").length;
  const phoneFlags = Object.keys(db.flaggedPhonesByDay?.[day] || {}).length;

  return bot.sendMessage(
    chatId,
    `📊 *Admin Dashboard* (${day})\n\nRevenue: *Ksh ${round2(r.revenueKsh)}*\nProfit: *Ksh ${round2(r.profitKsh || 0)}*\nOrders: *${Number(r.orders || 0)}*\nConversion: *${c.rate}%* (${c.success}/${c.attempts})\nUsers: *${usersTotal}*\nPending: *${pendingCount}*\nPhone flags today: *${phoneFlags}*`,
    { parse_mode: "Markdown" }
  );
}

const cmdTopProfit = text.match(/^\/topprofit(?:\s+(\d+))?$/i);
if (cmdTopProfit) {
  if (!isAdmin(chatId)) {
    const u = getUser(chatId);
    return bot.sendMessage(chatId, `👤 Your lifetime profit contribution: Ksh ${round2(Number(u.lifetimeProfitKsh || 0))}`, mainMenuKeyboard());
  }
  const limit = Math.max(3, Math.min(50, Number(cmdTopProfit[1] || 10)));
  db = repairDB(db);
  const entries = Object.entries(db.users || {})
    .filter(([cid]) => String(cid) !== String(ADMIN_ID))
    .map(([cid, u]) => ({ cid, profit: Number(u.lifetimeProfitKsh || 0), spent: Number(u.totalSpentKsh || 0) }))
    .filter(x => x.profit > 0)
    .sort((a, b) => b.profit - a.profit)
    .slice(0, limit);

  if (!entries.length) return bot.sendMessage(chatId, "No profit data yet.");

  const lines = entries.map((x, i) => `${i + 1}. \`${x.cid}\` — *Ksh ${round2(x.profit)}* (spent Ksh ${round2(x.spent)})`);
  return bot.sendMessage(chatId, `🏅 *Top Profit Users* (lifetime)\n\n${lines.join("\n")}`, { parse_mode: "Markdown" });
}

if (/^\/myprofit$/i.test(text)) {
  const u = getUser(chatId);
  return bot.sendMessage(chatId, `👤 *Your Profit Summary*\n\nTotal Spent: *Ksh ${round2(Number(u.totalSpentKsh || 0))}*\nLifetime Profit: *Ksh ${round2(Number(u.lifetimeProfitKsh || 0))}*\nPoints: *${Number(u.points || 0).toFixed(2)}*`, { parse_mode: "Markdown", ...mainMenuKeyboard() });
}

if (/^\/dbupgrade$/i.test(text)) {
  if (!isAdmin(chatId)) return bot.sendMessage(chatId, "Not allowed.", mainMenuKeyboard());
  return bot.sendMessage(
    chatId,
    "🗄️ DB Upgrade (Recommended)\n\nFor 10k+ users, move from db.json to PostgreSQL/SQLite.\n\nSuggested approach:\n1) Create tables: users, payments, analytics_daily, pending_requests.\n2) Write a one-time migration script to import db.json.\n3) Replace saveDB/loadDB with DB queries.\n\nTell me if you want PostgreSQL or SQLite and I’ll generate the full migration + refactor.",
    mainMenuKeyboard()
  );
}
  ...
  // NEW: block most actions if account is frozen
  const u0 = getUser(chatId);
  if (!isAdmin(chatId) && u0.isFrozen) {
    if (!/^\/start\b/i.test(text) && text !== "ℹ️ Help") {
      return bot.sendMessage(chatId, "🧊 Your account is frozen for verification. Contact support.", mainMenuKeyboard());
    }
  }
...
// NEW: one-tap personalized offer (250MB @ Ksh 20)
if (text === "⚡ Buy 250MB (Ksh 20)") {
  sessions.set(chatId, { step: "confirm", category: "Bingwa Deals", pkgKey: "Ksh 20 • 250MB 24HRS", createdAt: Date.now() });
  const savedPhone = getUserPhone(chatId);
  ...
  return bot.sendMessage(chatId, msgText, { parse_mode: "Markdown", ...confirmKeyboard(hasSaved) });
}
  ...
  return sendTracked(chatId, msgText, { 
    ...
    if (s.step === "confirm") {
      ...
      if (text === "✅ Proceed") {
        ...
        // NEW: phone ownership risk (one account per phone per day)
        const own = checkPhoneOwnershipSameDay(chatId, savedPhone);
        if (!own.ok) { sessions.delete(chatId); return bot.sendMessage(chatId, own.reason, mainMenuKeyboard()); }
        ...
        recordStkAttempt({ chatId, category: s.category, pkgLabel: pkg.label, phone254: savedPhone, price: pkg.price });
await notifyAdmin(
          `📌 *STK Attempt*\n` +
      ...
      if (phoneMaybe) {
        ...
        const own2 = checkPhoneOwnershipSameDay(chatId, phoneMaybe);
        if (!own2.ok) { sessions.delete(chatId); return bot.sendMessage(chatId, own2.reason, mainMenuKeyboard()); }
...
setInterval(async () => {
  try {
    // NEW: Daily financial report to admin (once per Kenya day shortly after midnight)
    db = repairDB(db);
    const kenyaNow = new Date();
    const kenyaHour = Number(new Intl.DateTimeFormat("en-US", { timeZone: "Africa/Nairobi", hour: "2-digit", hour12: false }).format(kenyaNow));
    const kenyaMinute = Number(new Intl.DateTimeFormat("en-US", { timeZone: "Africa/Nairobi", minute: "2-digit" }).format(kenyaNow));
    const dayKeyKenya = todayKey();

    if (kenyaHour === 0 && kenyaMinute < 8 && String(db.stats.lastDailyReportDay || "") !== String(dayKeyKenya)) {
      const dayData = db.analytics?.revenueByDay?.[dayKeyKenya] || { revenueKsh: 0, orders: 0, profitKsh: 0 };
      const conv = conversionSummary(0);

      const pkgMap = db.analytics?.revenueByPackageDay?.[dayKeyKenya] || {};
      const top = Object.entries(pkgMap)
        .map(([k, v]) => ({ k, rev: Number(v.revenueKsh || 0), orders: Number(v.orders || 0) }))
        .sort((a, b) => b.rev - a.rev)[0];

      const pendingCount = Object.entries(db.pendingRedeems || {}).filter(([, r]) => r && r.status === "pending").length;

      await bot.sendMessage(
        ADMIN_ID,
        `📊 *Daily Report* (${mdEscape(dayKeyKenya)})\n\n` +
          `Revenue: *Ksh ${mdEscape(round2(dayData.revenueKsh))}*\n` +
          `Profit: *Ksh ${mdEscape(round2(dayData.profitKsh || 0))}*\n` +
          `Orders: *${mdEscape(dayData.orders)}*\n` +
          `Conversion: *${mdEscape(conv.rate)}%* (${mdEscape(conv.success)}/${mdEscape(conv.attempts)})\n` +
          `Pending requests: *${mdEscape(pendingCount)}*\n` +
          (top ? `Top offer: *${mdEscape(top.k)}* (Ksh ${mdEscape(round2(top.rev))})\n` : `Top offer: *(none)*\n`) +
          `Time: \`${mdEscape(kenyaDateTime())}\``,
        { parse_mode: "Markdown" }
      ).catch(() => {});

      db.stats.lastDailyReportDay = dayKeyKenya;
      saveDB();
    }
