...
function setUserPhone(chatId, phone254) {
  ...
  // Keep a list of saved phones (unique, most-recent first)
  if (!Array.isArray(u.phones)) u.phones = [];
  u.phones = u.phones.filter((p) => p !== phone254);
  u.phones.unshift(phone254);

  // Optional: cap list size to avoid huge DB growth
  if (u.phones.length > 20) u.phones = u.phones.slice(0, 20);
...
function getUserPhones(chatId) {
  const u = getUser(chatId);
  const list = Array.isArray(u.phones) ? u.phones.slice() : [];
  if (u.phone && !list.includes(u.phone)) list.unshift(u.phone);
  return list;
}


function splitPhonesFromText(text) {
  const raw = String(text || "").trim();
  if (!raw) return [];
  // Accept: newlines, commas, slashes, spaces
  const parts = raw.split(/[\n,\/\s]+/g).map(s => s.trim()).filter(Boolean);
  return parts;
}

function addUserPhonesBulk(chatId, phones254) {
  const u = getUser(chatId);
  if (!Array.isArray(u.phones)) u.phones = [];

  // keep unique, most-recent first
  for (const p of phones254) {
    u.phones = u.phones.filter(x => x !== p);
    u.phones.unshift(p);
  }
  ...
  // set default phone to first if not set
  if (!u.phone && u.phones.length) u.phone = u.phones[0];
...
• 📞 Change Number`,
      { parse_mode: "Markdown", ...confirmKeyboard(hasSaved, getUserPhones(chatId).length > 0) }
    );
  }
...
async function renderSessionSnapshot(chatId, snap) {
  ...
  if (s.step === "confirm") {
    ...
    return sendTracked(chatId, msgText, { parse_mode: "Markdown", ...confirmKeyboard(hasSaved, getUserPhones(chatId).length > 0) });
  }
  ...
  if (s.step === "phone") {
    return sendTracked(
      ...
      confirmKeyboard(false, getUserPhones(chatId).length > 0)
    );
  }
...
// ======================================================================
  ...
  // ===================== SAVED NUMBERS PICKER =====================
  if (text === "📋 Saved Numbers") {
    const list = getUserPhones(chatId);
    if (!list.length) {
      return sendTracked(chatId, "📱 No saved phone yet. Tap 📞 Change Number to save one.", { ...mainMenuKeyboard(chatId) });
    }

    const s2 = sessions.get(chatId) || {};
    s2.step = "pick_saved_number";
    sessions.set(chatId, s2);

    const lines = ["📋 *Saved Numbers*\nReply with a number to use it now:"];
    list.slice(0, 10).forEach((p, idx) => lines.push(`${idx + 1}. ${mdEscape(maskPhone(p))}`));
    lines.push("\n❌ Send *cancel* to exit.");
  ...
  if ((sessions.get(chatId) || {}).step === "pick_saved_number") {
    if (text === "⬅️ Back") {
      const s2 = sessions.get(chatId) || {};
      s2.step = "confirm";
      sessions.set(chatId, s2);
      ...
      const saved = getUserPhone(chatId);
      const savedLine = saved ? maskPhone(saved) : "No saved phone yet";

      return sendTracked(
        chatId,
        `✅ Selected:\n${pkg ? pkg.label : ""}\n\n📱 ${savedLine}.\n\nChoose:\n• ✅ Proceed (use saved number)\n• 📞 Change Number`,
        { ...confirmKeyboard(!!saved, getUserPhones(chatId).length > 0) }
      );
    }
    if (/^(cancel|back|exit)$/i.test(text)) {
      const s2 = sessions.get(chatId) || {};
      s2.step = "confirm";
      sessions.set(chatId, s2);
      return sendTracked(chatId, "✅ Cancelled. Tap ✅ Proceed or 📞 Change Number.", { ...mainMenuKeyboard(chatId) });
    }

    const n = Number(String(text).trim());
    const list = getUserPhones(chatId);
    if (!Number.isFinite(n) || n < 1 || n > list.length) {
      return sendTracked(chatId, "⚠️ Invalid choice. Reply with 1, 2, 3…", { ...mainMenuKeyboard(chatId) });
    }

    const phone = list[n - 1];
    setUserPhone(chatId, phone);

    // Immediately start STK with the current selected offer in session
    const s2 = sessions.get(chatId) || {};
    if (s2.category && s2.pkgKey) {
      s2.step = "confirm";
      sessions.set(chatId, s2);
      // simulate proceed flow by calling a helper through an internal label
      return startStkFromSession(chatId, msg, s2, phone);
    }

    sessions.delete(chatId);
    return sendTracked(chatId, "✅ Number selected. Tap 🛒 Buy Offers to continue.", { ...mainMenuKeyboard(chatId) });
  }
  // ================================================================
...
function confirmKeyboard(hasSavedPhone, hasAnySavedList = false) {
  const rows = [];
  if (hasSavedPhone) rows.push(["✅ Proceed", "📞 Change Number"]);
  else rows.push(["📞 Change Number"]);
  if (hasAnySavedList) rows.push(["📋 Saved Numbers"]);
  rows.push([PREV_BTN]);
  return { reply_markup: { keyboard: rows, resize_keyboard: true, one_time_keyboard: false } };
}

function recoveryKeyboard(ref) {
  return {
    reply_markup: {
      inline_keyboard: [
        [{ text: "🔁 Retry STK", callback_data: `ui:stk_retry:${ref}` }],
        [{ text: "📞 Change Number", callback_data: `ui:stk_change_phone:${ref}` }],
...
if (!isSuccess) {
  ...
  if (pending) {
    ...
    pending.retryUntil = Date.now() + 2 * 60 * 1000;
    saveDB();
  }
  ...
  await sendTracked(chatId, out, { parse_mode: "Markdown", ...recoveryKeyboard(externalRef) });
  return;
}
...
` +
    ...
    // ✅ Save last successful purchase for 🔁 Buy Last Offer (ALL categories)
    try {
      if (pending) {
        const uu = getUser(chatId);
        uu.lastPurchase = {
...
async function startStkFromSession(chatId, msg, s, phone254) {
  const pkg = findPackageByLabel(s.category, s.pkgKey);
  if (!pkg) {
    sessions.delete(chatId);
    return sendTracked(chatId, "❌ Offer not found. Tap 🛒 Buy Offers.", { ...mainMenuKeyboard(chatId) });
  }

  // Bingwa once per day per phone (hard block ONLY here)
  if (!isAdmin(chatId) && s.category === "Bingwa Deals" && bingwaAlreadyPurchasedToday(phone254)) {
    sessions.delete(chatId);
    return sendTracked(
      chatId,
  ...
  const sig = makeSig({ category: s.category, pkgLabel: pkg.label, phone254 });
  ...
  if (recent) {
    // ensure retry window
    const p = getPendingPayment(recent.ref);
    if (p) {
      p.retryUntil = Date.now() + 2 * 60 * 1000;
      saveDB();
    }
    return sendTracked(
      chatId,
      `⚠️ *STK already sent recently*\n\nIf you receive the M-PESA prompt, complete it.\n\nIf no prompt appears within 1 minute:`,
      { parse_mode: "Markdown", ...recoveryKeyboard(recent.ref) }
    );
  }

  const spam = checkAndMarkSpam(chatId, sig, phone254);
  ...
  const blockedUntil = isStkBlocked(chatId, phone254);
  if (blockedUntil) {
  ...
  addPendingPayment(ref, {
    ...
    phone254,
    price: pkg.price,
    retryUntil: Date.now() + 2 * 60 * 1000, // allow retry for 2 mins
    delayNotified: false,
  });
  ...
  try {
    await payheroStkPush({ amount: pkg.price, phone: phone254, externalRef: ref, channelId });

    // 30s delayed message if no callback yet
    try {
      setTimeout(async () => {
        try {
          ...
          if (p.delayNotified) return;
          p.delayNotified = true;
          p.retryUntil = Date.now() + 2 * 60 * 1000;
          saveDB();
          await sendTracked(
            chatId,
            `⚠️ *STK Request Delayed*\n\nIf you receive the M-PESA prompt, complete it.\n\nIf no prompt appears, choose an option below:`,
            { parse_mode: "Markdown", ...recoveryKeyboard(ref) }
          );
        } catch (_) {}
      }, 30 * 1000);
    } catch (_) {}
    ...
    if (isStkDelayedError(err)) {
      ...
      // keep pending; set retry window
      const p = getPendingPayment(ref);
      if (p) { p.retryUntil = Date.now() + 2 * 60 * 1000; saveDB(); }
      return sendTracked(
        chatId,
        `⚠️ *STK Request Delayed*\n\nIf you receive the M-PESA prompt, complete it.\n\nIf no prompt appears, choose an option below:`,
        { parse_mode: "Markdown", ...recoveryKeyboard(ref) }
    ...
    sessions.delete(chatId);
    // keep pending only briefly? here it's hard failure from gateway; delete
    deletePendingPayment(ref);
...
// ===================== /save (bulk save numbers) =====================
  if (text === "/save") {
    const sSave = sessions.get(chatId) || {};
    pushPrev(chatId, sSave);
    sSave.step = "save_numbers";
    sessions.set(chatId, sSave);

    return sendTracked(
      chatId,
      "📌 *Save Numbers*\n\nSend phone numbers in any of these formats:\n\n• 07XXXXXXXX\n• 01XXXXXXXX\n• 2547XXXXXXXX\n• 2541XXXXXXXX\n\nYou can send multiple numbers:\n- Each on new line\n- Or separated by commas: 07.., 07..\n- Or slashes: 07../07..\n\nWhen finished, reply with: *Done* (or Dn / D).",
      { parse_mode: "Markdown", ...mainMenuKeyboard(chatId) }
    );
  }

  // Collect numbers until Done
  if ((sessions.get(chatId) || {}).step === "save_numbers") {
    if (/^(done|dn|d)$/i.test(text)) {
      const sSave = sessions.get(chatId) || {};
      const added = Array.isArray(sSave._savedBulk) ? sSave._savedBulk.length : 0;
      delete sSave._savedBulk;
      sSave.step = "confirm";
      sessions.set(chatId, sSave);

      return sendTracked(
        chatId,
        `✅ Saved. Total added this time: ${added}\n\n📋 Tap 📋 Saved Numbers to choose anytime.`,
        { ...mainMenuKeyboard(chatId) }
      );
    }

    const parts = splitPhonesFromText(text);
    if (!parts.length) {
      return sendTracked(chatId, "⚠️ No numbers detected. Send numbers or reply Done.", { ...mainMenuKeyboard(chatId) });
    }
    ...
    if (!normalized.length) {
      return sendTracked(chatId, "⚠️ No valid numbers found. Use 07XXXXXXXX / 01XXXXXXXX / 2547XXXXXXXX / 2541XXXXXXXX.", { ...mainMenuKeyboard(chatId) });
    }

    // Add to DB (cap 20)
    addUserPhonesBulk(chatId, normalized);

    // Track how many were added in this /save session (for Done message)
    const sSave = sessions.get(chatId) || {};
    if (!Array.isArray(sSave._savedBulk)) sSave._savedBulk = [];
    for (const p of normalized) sSave._savedBulk.push(p);
    // cap counter
    if (sSave._savedBulk.length > 100) sSave._savedBulk = sSave._savedBulk.slice(-100);
    sessions.set(chatId, sSave);

    const list = getUserPhones(chatId).slice(0, 20);
    const lines = ["✅ Saved numbers so far (max 20):"];
    list.forEach((ph, idx) => lines.push(`${idx + 1}. ${maskPhone(ph)}`));
    lines.push("\nReply with more numbers, or *Done* to finish.");
...
if (s.step === "package") {
  ...
  return sendTracked(chatId, msgText, { 
    parse_mode: "Markdown", 
    ...confirmKeyboard(hasSaved, getUserPhones(chatId).length > 0) 
  });
}  // ✅ THIS WAS MISSING
    // STEP 3: CONFIRM
    if (s.step === "confirm") {
        ...
        if (!savedPhone) {
          ...
          return sendTracked(
            ...
            "✅ Selected:\n" +
              `${pkg.label}\n\n` +
              `📱 No saved phone yet\n\nChoose:\n• ✅ Proceed (use saved number)\n• 📞 Change Number`,
            { ...confirmKeyboard(false, false) }
          );
        }

        // Continue with saved phone
        return startStkFromSession(chatId, msg, s, savedPhone);
      }

      // accept phone typed here
      
      // If changing phone for a failed/delayed pending ref
      if ((sessions.get(chatId) || {}).step === "phone_for_pending") {
        const phoneNew = normalizePhone(text);
        if (!phoneNew) {
          return sendTracked(chatId, "⚠️ Invalid phone. Send 07XXXXXXXX / 01XXXXXXXX / 2547XXXXXXXX / 2541XXXXXXXX", { ...mainMenuKeyboard(chatId) });
        }
        setUserPhone(chatId, phoneNew);

        const s2 = sessions.get(chatId) || {};
        const ref = s2.pendingRef;
        const pnd = ref ? getPendingPayment(ref) : null;
        if (pnd) {
          // free old pending and create new one through normal flow
          deletePendingPayment(ref);
          const s3 = { category: pnd.category, pkgKey: pnd.pkgLabel };
          sessions.set(chatId, s3);
          return startStkFromSession(chatId, msg, s3, phoneNew);
        }

        sessions.delete(chatId);
        return sendTracked(chatId, "✅ Number updated. Tap 🛒 Buy Offers to continue.", { ...mainMenuKeyboard(chatId) });
      }
