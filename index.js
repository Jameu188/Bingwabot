...
function dayKeyAt(ts) {
  // Kenya day key for an arbitrary timestamp (Africa/Nairobi)
  try {
...
function businessReferralPercent(successRefCount) {
  const n = Number(successRefCount || 0);
  // Business commission tiers (counted during active subscription period only)
  // 0–29 => 2%
  // 30+  => 2.5%
  // 100+ => 3%
  // 140+ => 3.5%
  // 200+ => 4%
  // 250+ => 4.5%
  if (n >= 250) return 0.045;
  if (n >= 200) return 0.04;
  if (n >= 140) return 0.035;
  if (n >= 100) return 0.03;
  if (n >= 30) return 0.025;
  return 0.02;
}


function isBusinessEligibleForPerks(chatId) {
  // Business perks unlock after 3 successful purchases AND active subscription
  if (!isBusinessActive(chatId)) return false;
  const u = getUser(chatId);
  return Number(u.successPurchases || 0) >= 3;
}
...
function isBusinessActive(chatId) {
  ...
  if (Date.now() > u.subscriptionExpiry) {
    ...
    u.businessReferralSuccessCount = 0;
    u.businessReferralDayCounts = {};
    saveDB();
    return false;
  }
  return true;
...
function hasPendingRedeem(chatId) {
  ...
  if (isLimitBypass(chatId)) return false;
  db = repairDB(db);
...
function setPendingRedeem(chatId, obj) {
  ...
  db.pendingRedeems[String(chatId)] = obj;
    // Track successful purchases per user (used for Business perk unlock)
  const u = getUser(chatId);
  u.successPurchases = Number(u.successPurchases || 0) + 1;
...
function redeemCooldownRemainingMs(chatId) {
  // ✅ Admin bypasses ALL limitations
  if (isLimitBypass(chatId)) return 0;
  db = repairDB(db);
...
function formatTodayLeaderboardPlain(limit = 10) {
  const { day, sorted } = getTodayInviterLeaderboard(limit);
  if (!sorted.length) {
    return `🏆 Top Inviters Today (${day})\n\nNo successful referrals yet today.`;
  }
  const lines = sorted.map((x, i) => {
    const name = getDisplayName(x.chatId);
...
// ✅ Privilege to bypass user limitations (Business behaves like Admin for limits ONLY)
function isLimitBypass(chatId) {
  return isAdmin(chatId) || isBusinessActive(chatId);
}
...
// Mask 2547xxxxxxxx -> 254759xxx842 (shows first 6 + last 3)
function mask254(phone254) {
  const p = String(phone254 || "").trim();
  if (!/^254\d{9}$/.test(p)) return p || "N/A";
  return p.slice(0, 6) + "xxx" + p.slice(-3);
}
...
function linkAccountToPhone(chatId, phone254) {
  // ✅ Admin bypass
  if (isLimitBypass(chatId)) return { ok: true, linked: true, total: 999 };
...
function mdEscape(s) {
  // Telegram parse_mode: "Markdown" (legacy)
  // Escape only the characters that Markdown interprets (avoid escaping dots/hyphens so URLs look normal)
...
// ✅ Referral bonus credit ONLY on SUCCESS callback
// Rules:
// - NORMAL accounts earn 1.5% on every successful purchase by their referral (subject to normal referral expiry).
// - BUSINESS accounts earn tiered % (2%..4.5%) ONLY when:
//    • subscription is active AND perks unlocked (>=3 successful purchases), AND
//    • purchase amount >= 20 KSH.
//   For purchases < 20 KSH, business accounts still earn 1.5% (but that small purchase does NOT count toward business referral tier counts).
function maybeRewardInviterOnSuccessPurchase(buyerChatId, amountKsh, when) {
  ...
  const dayKey = dayKeyAt(ts);

  const isBizEligible = isBusinessEligibleForPerks(inviterId) && isBusinessActive(inviterId);
  const qualifiesForBizCount = isBizEligible && amt >= 20;

  // ✅ If this is the buyer's FIRST successful purchase that QUALIFIES FOR COUNTING, enforce referral caps & increment counts.
  // Normal: count on first successful purchase (any amount > 0).
  // Business: count only on first successful purchase with amount >= 20 KSH.
  const shouldCountNow = !buyer.referralCounted && (qualifiesForBizCount || !isBizEligible);

  if (shouldCountNow) {
    // Normal users: max 15 successful referrals per day (business is unlimited)
    if (!isBizEligible) {
      inv.dailyReferralCounts = inv.dailyReferralCounts || {};
      inv.dailyReferralCounts[dayKey] = Number(inv.dailyReferralCounts[dayKey] || 0);

      if (inv.dailyReferralCounts[dayKey] >= 15) {
        // Do not count/award the "counted" flag now; allow a future successful purchase on another day to count.
        saveDB();
        return;
      }

      inv.dailyReferralCounts[dayKey] += 1;
    } else {
      // Business: track UNIQUE referral activations within active subscription period
      // (tier purchase-counting is handled separately per qualifying purchase)
      inv.businessReferralDayCounts = inv.businessReferralDayCounts || {};
      inv.businessReferralDayCounts[dayKey] = Number(inv.businessReferralDayCounts[dayKey] || 0) + 1;
      inv.businessUniqueReferralCount = Number(inv.businessUniqueReferralCount || 0) + 1;
    }

    // Global unique referral stats (lifetime)
    inv.referralSuccessCount = Number(inv.referralSuccessCount || 0) + 1;

    buyer.referralCounted = true;
    buyer.referralCountedAt = ts;
    saveDB();


  // ✅ Business tier counting: EVERY successful referral purchase counts during active subscription,
  // but ONLY if the purchase amount is >= 20 KSH (small amounts still earn normal rate, but don't count for tiers).
  if (qualifiesForBizCount) {
    inv.businessReferralSuccessCount = Number(inv.businessReferralSuccessCount || 0) + 1;
    saveDB();
  }
  }

  // Referral expiry applies to NORMAL users only (business earns only while subscription active anyway)
  if (!isBizEligible) {
    const setAt = Number(buyer.inviterSetAt || buyer.referralSetAt || 0);
    if (setAt) {
      const maxAgeMs = Number(REFERRAL.EXPIRY_DAYS || 0) * 24 * 60 * 60 * 1000;
      if (maxAgeMs > 0 && Date.now() - setAt > maxAgeMs) return;
    }
  } else {
    // Business perks require active subscription; if it expired, no business earnings
    if (!isBusinessActive(inviterId)) return;
  }

  // ✅ Commission percent:
// - Normal: 1.5% for 1–50 KSH, 2% for 51–1000 KSH (and above).
// - Business: tiered percent ONLY if amount >= 20 KSH AND subscription+perks are active.
//   If amount < 20 KSH, business still earns the NORMAL small-amount rate (1.5%).
let percent = 0.015;

// Normal tiered percent (applies to normal users, and as fallback for business/non-eligible cases)
const normalPercent = (amt <= 50 ? 0.015 : 0.02);

if (qualifiesForBizCount) {
  // Business qualifying purchase: tiered commission based on referral purchases counted within active subscription
  percent = businessReferralPercent(inv.businessReferralSuccessCount || 0);
} else {
  // Normal users (and business purchases that don't qualify for tier counting) use normal tier
  percent = normalPercent;
}

  const bonusKes = amt * percent;
  if (!Number.isFinite(bonusKes) || bonusKes <= 0) return;

  // Convert KES bonus to points so that pointsToKes(points) matches real money value
  const bonusPts = bonusKes / 0.7;
  if (!Number.isFinite(bonusPts) || bonusPts <= 0) return;

  // ✅ Add points for EVERY successful purchase by the referred user
  addPoints(inviterId, bonusPts);

  const cleanBonusPts = Number(bonusPts.toFixed(2));
  const cleanBonusKes = Number(bonusKes.toFixed(2));

  // Notify inviter every time the referred user purchases successfully
  try {
    const buyerName = buyer.firstName || buyer.username || "Your referral";
    const msg =
      `🎉 Referral purchase!
...
` +
      `✅ Purchase: ${amt} KSH
` +
      `💰 You earned: ${cleanBonusKes} KSH (${cleanBonusPts} pts)
` +
      (qualifiesForBizCount
        ? `🏷 Your business rate: ${(percent * 100).toFixed(1)}%
`
        : `🏷 Rate: ${(percent * 100).toFixed(1)}%
`) +
      (isBizEligible && amt < 20 ? `ℹ️ Note: Purchases below 20 KSH earn 1.5% and don’t count toward business tiers.
` : "");
    sendTracked(inviterId, msg);
  } catch (_) {}
}
...
// ===================== ADMIN: LIST ALL COMMANDS =====================
  ...
  if (isStartCmd && !isLimitBypass(chatId)) {
...
const pts = getPoints(chatId);
      return sendTracked(chatId, `⭐ Your points: ${pts.toFixed(2)} pts
...
// ===================== GLOBAL BUTTONS =====================
  ...
  const referralPts = totalEarned / 0.7;
  ...
  // Use PLAIN text leaderboard here to avoid Markdown escaping issues (backslashes, parse errors)
  const leaderboardText = formatTodayLeaderboardPlain(10);

  const msg =
    `🔗 <b>Your Referral:</b>\n\n` +
    `Use: <code>${escapeHtml(referralCommand)}</code> (share this code)\n\n` +
    `Or\n${escapeHtml(referralLink)}\n\n` +
    `⭐ Your points: <b>${escapeHtml(referralPts.toFixed(2))} pts</b>
💸 Use "💸 Withdraw Points" to withdraw.\n` +
    `👤 Total referrals: <b>${escapeHtml(String(totalReferrals))}</b>\n` +
    `🤑 Total earned: <b>${escapeHtml(totalEarned.toFixed(0))} KSH</b>\n\n` +
    `📌 Rule: You earn when your referred user completes a successful purchase.\n\n` +
    `${escapeHtml(leaderboardText)}`;

  return sendTracked(chatId, msg, {
    parse_mode: "HTML",
    disable_web_page_preview: true,
    reply_markup: {
      inline_keyboard: [[{ text: "📤 Share Referral", url: shareUrl }]],
    },
    // Keep the normal reply-keyboard menu
    ...mainMenuKeyboard(chatId),
  });
}
  ...
  if (text === "🔄 Resend STK") {
    ...
    if (!isLimitBypass(chatId) && lastAt && now - lastAt < 60 * 1000) {
...
if (text === "💸 Withdraw Points") {
  ...
  // ✅ Admin bypasses ALL limitations
  if (!isLimitBypass(chatId) && Number(u.totalSpentKsh || 0) < MIN_WITHDRAW_SPEND) {
...
function canUsePhone(chatId, phone254) {
  // ✅ Business/Admin bypass phone-account lock
...
payheroStkPush = async function (payload) {
  ...
  if (!check.ok && !isLimitBypass(chatId)) {
...
Approve override?`,
    ...
    throw new Error("This phone number is currently linked to another account. An approval request has been sent to admin.");
  }
...
bot.on("message", async (msg) => {
  ...
  if (totalSpent < MIN_WITHDRAW_SPEND_KSH && !isLimitBypass(chatId)) {
    await bot.sendMessage(
      chatId,
      `🚫 Withdrawal Locked
...
console.log("💳 Minimum 300 KSH withdrawal requirement enforced.")  // 💼 Business cashback boost (+0.3%) while subscription active (unlocks after 3 purchases)
  if (isBusinessEligibleForPerks(chatId)) {
    percent += 0.003;
  }
